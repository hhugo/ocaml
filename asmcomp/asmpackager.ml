...
let read_member_info pack_path file =
  ...
  let kind =
    ...
    then PM_intf
      ...
      if info.ui_symbol
         <> (Compilenv.current_unit_infos ()).ui_symbol
            ^ "__"
            ^ info.ui_name
      ...
      List.map
        (fun info ->
          { info with
            ui_export_info =
              Flambda
                (Export_info_for_pack.import_for_pack
                   ~pack_units
...
let package_object_files ~ppf_dump files targetcmx targetobj targetname
    coercion ~backend =
  let pack_path =
