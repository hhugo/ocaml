...
let read_member_info pack_path file =
  ...
  let kind =
    if Filename.check_suffix file ".cmi" then
      ...
      ((if info.ui_name <> name then
          raise (Error (Illegal_renaming (name, file, info.ui_name))));
       (if
          info.ui_symbol <>
  ...
  let rec check forbidden =
    ...
    | mb :: tl ->
      ...
      | PM_impl infos ->
        List.iter
          (fun (unit, _) ->
             if List.mem unit forbidden then
               raise (Error (Forward_reference (mb.pm_file, unit))))
...
=
  Profile.record_call (Printf.sprintf "pack(%s)" targetname)
    (fun () ->
       let objtemp =
         if !Clflags.keep_asm_file then
           Filename.remove_extension targetobj ^ ".pack" ^ Config.ext_obj
         else
         (* Put the full name of the module in the temporary file name
            to avoid collisions with MSVC's link /lib in case of successive
            packs *)
           Filename.temp_file (Compilenv.make_symbol (Some "")) Config.ext_obj
       in
       let components =
         List.map
           (fun m ->
              ...
              | PM_intf -> None
              | PM_impl _ -> Some (Ident.create_persistent m.pm_name)) members
       in
...
let build_package_cmx members cmxfile =
  ...
  let filter lst =
    List.filter (fun (name, _crc) -> not (List.mem name unit_names)) lst
  in
  let union lst =
    List.fold_left
      (List.fold_left
         (fun accu n -> if List.mem n accu then accu else n :: accu)) [] lst
  in
  let units =
    List.fold_right
      (fun m accu ->
         match m.pm_kind with
         | PM_intf -> accu
         | PM_impl info -> info :: accu) members []
  in
  let pack_units =
    List.fold_left
      (fun set info ->
         let unit_id = Compilenv.unit_id_from_name info.ui_name in
         Compilation_unit.Set.add (Compilenv.unit_for_global unit_id) set)
             ...
             ui_export_info =
               Flambda
                 (Export_info_for_pack.import_for_pack ~pack_units
                    ~pack:(Compilenv.current_unit ()) (get_export_info info))
           }) units
    else
      units
  in
  ...
  let ui_export_info =
    if Config.flambda then
      let ui_export_info =
        List.fold_left
          (fun acc info -> Export_info.merge acc (get_export_info info))
          (Export_info_for_pack.import_for_pack ~pack_units
             ~pack:(Compilenv.current_unit ()) (get_export_info ui)) units
      in
...
let package_object_files
    ~ppf_dump files targetcmx targetobj targetname coercion ~backend
=
  let pack_path =
...
let report_error ppf =
  ...
  | Assembler_error file -> fprintf ppf "Error while assembling %s" file
