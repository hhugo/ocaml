...
let add_closure_offsets {function_offsets; free_variable_offsets}
  ...
  let assign_function_offset id function_decl (map, env_pos) =
    ...
    let env_pos =
      ...
      env_pos
      + 1
      (* GC header; either [Closure_tag] or [Infix_tag] *)
      + 1
      (* full application code pointer *)
      + 1 (* arity *)
      + if arity > 1 then 1 else 0
      (* partial application code pointer *)
    in
    let closure_id = Closure_id.wrap id in
    if Closure_id.Map.mem closure_id map
    then
      Misc.fatal_errorf
  ...
  let assign_free_variable_offset var _ (map, pos) =
    ...
    if Var_within_closure.Map.mem var_within_closure map
    then
      Misc.fatal_errorf
