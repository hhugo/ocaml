...
let equal_descr (d1 : descr) (d2 : descr) : bool =
  ...
  | Value_constptr i1, Value_constptr i2 -> i1 = i2
  ...
  | ( ( Value_block (_, _)
      ...
      | Value_int _ | Value_char _ | Value_constptr _ | Value_float _
      | Value_float_array _ | Value_boxed_int _ | Value_string _
      | Value_closure _ | Value_set_of_closures _ | Value_unknown_descr )
    , ( Value_block (_, _)
      | Value_mutable_block (_, _)
      | Value_int _ | Value_char _ | Value_constptr _ | Value_float _
      | Value_float_array _ | Value_boxed_int _ | Value_string _
...
type t =
  ...
  ; invariant_params :
      Variable.Set.t Variable.Map.t Set_of_closures_id.Map.t
  ...
  let offset_fun =
    let fold_map set =
      Closure_id.Map.fold (fun key value unchanged ->
          if Closure_id.Set.mem key set
          then Closure_id.Map.add key value unchanged
          else unchanged )
    in
  ...
  let offset_fv =
    let fold_map set =
      Var_within_closure.Map.fold (fun key value unchanged ->
          if Var_within_closure.Set.mem key set
          then Var_within_closure.Map.add key value unchanged
          else unchanged )
    in
    Var_within_closure.Map.empty
    |> fold_map transient.relevant_local_vars_within_closure local_offset_fv
    |> fold_map
         transient.relevant_imported_vars_within_closure
         imported_offset_fv
  in
...
let print_value_set_of_closures ppf (t : value_set_of_closures) =
  ...
  let print_results ppf results =
    Format.fprintf
      ppf
      "(%a)"
      (Closure_id.Map.print print_raw_approx)
      results
  in
  ...
  Format.fprintf
    ...
    print_results
    t.results
    print_aliased_symbol
    t.aliased_symbol

let print_value_closure ppf (t : value_closure) =
  Format.fprintf
    ppf
    "((closure_id %a) (set_of_closures %a))"
    Closure_id.print
    t.closure_id
    print_value_set_of_closures
    t.set_of_closures

let print_value_float_array_contents ppf
    (value : value_float_array_contents) =
  match value with
...
let print_value_float_array ppf (value : value_float_array) =
  Format.fprintf
    ...
    "((size %d) (contents %a))"
    value.size
    print_value_float_array_contents
    value.contents
...
let print_approx_components ppf ~symbol_id ~values
  ...
  let rec print_approx ppf (approx : approx) =
    ...
    | Value_symbol sym ->
        if not (Symbol.Set.mem sym !recorded_symbol)
        then (
  ...
  let rec print_recorded_symbols () =
    if not (Queue.is_empty symbols_to_print)
    then (
