...
module Make (T : Branch_relaxation_intf.S) = struct
  ...
  let fixup_branches ~code_size ~max_out_of_line_code_offset map f =
    let expand_optbranch lbl n arg next =
      ...
      | None -> next
      | Some l ->
        instr_cons (Lcondbranch (Iinttest_imm (Isigned Cmm.Ceq, n), l)) arg
          [| |] next
    in
  ...
  let rec relax f ~max_out_of_line_code_offset =
      ...
      List.fold_left
        (fun min_of_max_branch_offsets branch ->
           Int.min min_of_max_branch_offsets
             (T.Cond_branch.max_displacement branch)) max_int T.Cond_branch.all
    in
    ...
    then
      relax f ~max_out_of_line_code_offset
    else
      ()
end
