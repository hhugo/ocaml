...
let rec combine i allocstate =
  ...
  | Iop (Ialloc { bytes = sz; dbginfo; _ }) ->
    ...
    | Pending_alloc { reg; dbginfos; totalsz }
      ...
      in
      instr_cons_debug (Iop (Iintop_imm (Iadd, ~-sz))) [| reg |] i.res i.dbg
        next,
      state
    | No_alloc | Pending_alloc _ ->
      let (next, state) =
        combine i.next
          (Pending_alloc { reg = i.res.(0); dbginfos = dbginfo; totalsz = sz })
      in
      let (totalsz, dbginfo) =
        match state with
      ...
      let next =
        ...
        if offset = 0 then
          ...
          instr_cons_debug (Iop (Iintop_imm (Iadd, offset))) i.res i.res i.dbg
            next
      in
      instr_cons_debug (Iop (Ialloc { bytes = totalsz; dbginfo })) i.arg i.res
        i.dbg next,
      allocstate
    end
  | Iop
  ...
  | Icatch (rec_flag, handlers, body) ->
    ...
    in
    let newnext = combine_restart i.next in
    instr_cons (Icatch (rec_flag, newhandlers, newbody)) i.arg i.res newnext, s'
