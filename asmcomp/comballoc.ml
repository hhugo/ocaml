...
type pending_alloc = {
  reg : Reg.t;
  (* register holding the result of the last allocation *)
  dbginfos : Debuginfo.alloc_dbginfo;
  (* debug info for each pending alloc *)
  totalsz : int;
}
(* amount to be allocated in this block *)

type allocation_state = No_alloc | Pending_alloc of pending_alloc

let rec combine i allocstate =
  match i.desc with
  | Iend | Ireturn | Iexit _ | Iraise _ -> (i, allocstate)
  | Iop (Ialloc { bytes = sz; dbginfo; _ }) -> (
      ...
      | No_alloc | Pending_alloc _ ->
          let next, state =
            combine i.next
              (Pending_alloc
                 { reg = i.res.(0); dbginfos = dbginfo; totalsz = sz })
          in
          let totalsz, dbginfo =
            match state with
            | No_alloc -> assert false
            | Pending_alloc { totalsz; dbginfos; _ } -> (totalsz, dbginfos)
          in
          let next =
            ...
            if offset = 0 then next
              ...
              instr_cons_debug
                (Iop (Iintop_imm (Iadd, offset)))
                i.res i.res i.dbg next
          in
          ( instr_cons_debug
              (Iop (Ialloc { bytes = totalsz; dbginfo }))
              i.arg i.res i.dbg next,
            allocstate ))
  | Iop
      ( Icall_ind | Icall_imm _ | Iextcall _ | Itailcall_ind | Itailcall_imm _
      | Ipoll _ ) ->
  ...
  | Iswitch (table, cases) ->
      ...
      let newnext = combine_restart i.next in
      (instr_cons (Iswitch (table, newcases)) i.arg i.res newnext, allocstate)
  | Icatch (rec_flag, handlers, body) ->
      let newbody, s' = combine body allocstate in
      let newhandlers =
        List.map (fun (io, handler) -> (io, combine_restart handler)) handlers
      in
      let newnext = combine_restart i.next in
      ( instr_cons (Icatch (rec_flag, newhandlers, newbody)) i.arg i.res newnext,
        s' )
...
and combine_restart i =
  let newi, _ = combine i No_alloc in
  newi
