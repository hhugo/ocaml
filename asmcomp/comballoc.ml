...
let rec combine i allocstate =
  match i.desc with
  | Iend | Ireturn | Iexit _ | Iraise _ -> i, allocated_size allocstate
  | Iop (Ialloc {bytes = sz; _}) -> (
    ...
    | Pending_alloc {reg; totalsz}
      when totalsz + sz < Config.max_young_wosize * Arch.size_addr ->
        let next, totalsz =
          combine
            i.next
            (Pending_alloc {reg = i.res.(0); totalsz = totalsz + sz})
        in
        ( instr_cons_debug
    ...
    | No_alloc | Pending_alloc _ ->
        let next, totalsz =
          combine i.next (Pending_alloc {reg = i.res.(0); totalsz = sz})
        in
        let next =
  ...
  | Iop
      ( Icall_ind _ | Icall_imm _ | Iextcall _ | Itailcall_ind _
      | Itailcall_imm _ ) ->
  ...
  | Iifthenelse (test, ifso, ifnot) ->
      ...
      let newnext = combine_restart i.next in
      ( instr_cons
          (Iifthenelse (test, newifso, newifnot))
          i.arg
          i.res
          newnext
      , allocated_size allocstate )
  ...
  | Icatch (rec_flag, handlers, body) ->
      ...
      let newnext = combine_restart i.next in
      ( instr_cons
          (Icatch (rec_flag, newhandlers, newbody))
          i.arg
          i.res
          newnext
      , sz )
