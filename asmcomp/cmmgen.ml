...
let rec transl env e =
  ...
  | Uclosure (fundecls, []) ->
      ...
      List.iter (fun f -> Cmmgen_state.add_function f) fundecls;
      let dbg =
        match fundecls with [] -> Debuginfo.none | fundecl :: _ -> fundecl.dbg
      in
      Cconst_symbol (sym, dbg)
  | Uclosure (fundecls, clos_vars) ->
        ...
        | f :: rem ->
            ...
            let without_header =
              if f.arity = 1 || f.arity = 0 then
                ...
                :: alloc_closure_info ~arity:f.arity ~startenv:(startenv - pos)
                     dbg
  ...
  | Uprim (prim, args, dbg) -> (
      ...
      | ( ( Pfield_computed | Psequand | Psequor | Pnot | Pnegint | Paddint
          ...
          | Pcompare_ints | Pcompare_floats | Pcompare_bints _ | Poffsetref _
          | Pfloatcomp _ | Parraylength _ | Parrayrefu _ | Parraysetu _
          | Parrayrefs _ | Parraysets _ | Pbintofint _ | Pintofbint _
          | Pcvtbint (_, _)
      ...
      return_unit dbg
        (ccatch
           ( raise_num,
             [],
             create_loop
               (transl_if env Unknown dbg cond dbg
...
and transl_prim_1 env p arg dbg =
  ...
  (* Boolean operations *)
  | Pnot ->
      transl_if env Then_false_else_true dbg arg dbg
        (Cconst_int (1, dbg))
        dbg
        (Cconst_int (3, dbg))
  (* Test integer/block *)
  ...
  | Pbswap16 ->
      tag_int
        (bswap16 (ignore_high_bit_int (untag_int (transl env arg) dbg)) dbg)
        dbg
  | Pfield_computed | Psequand | Psequor | Paddint | Psubint | Pmulint | Pandint
  | Porint | Pxorint | Plslint | Plsrint | Pasrint | Paddfloat | Psubfloat
  | Pmulfloat | Pdivfloat | Pstringrefu | Pstringrefs | Pbytesrefu | Pbytessetu
  ...
  | Pbigstring_load _ | Pbigstring_set _ ->
      fatal_errorf "Cmmgen.transl_prim_1: %a" Printclambda_primitives.primitive
        p
...
and transl_prim_2 env p arg1 arg2 dbg =
  ...
  | Pstringlength | Pbyteslength | Pbytessetu | Pbytessets | Pisint | Pbswap16
  | Pint_as_pointer | Popaque | Pread_symbol _
  | Pmakeblock (_, _, _)
  | Pfield _
  | Psetfield_computed (_, _)
  | Pfloatfield _
  ...
  | Pbigarraydim _ | Pbytes_set _ | Pbigstring_set _ | Pbbswap _ ->
      fatal_errorf "Cmmgen.transl_prim_2: %a" Printclambda_primitives.primitive
        p
...
and transl_prim_3 env p arg1 arg2 arg3 dbg =
  ...
  | Pbbswap _ ->
      fatal_errorf "Cmmgen.transl_prim_3: %a" Printclambda_primitives.primitive
        p
...
and transl_letrec env bindings cont =
  ...
  let op_alloc prim args =
    Cop (Cextcall (prim, typ_val, [], true), args, dbg)
  in
...
let transl_all_functions cont =
  let rec aux already_translated cont translated_functions =
    if Cmmgen_state.no_more_functions () then (cont, translated_functions)
    else
...
let compunit (ulam, preallocated_blocks, constants) =
  ...
  let init_code =
    if !Clflags.afl_instrument then
      Afl_instrument.instrument_initialiser (transl empty_env ulam) (fun () ->
          dbg)
      ...
      Cfunction
        {
          ...
          (* This function is often large and run only once.
             ...
             See MPR#7630 *)
          fun_codegen_options =
            (if Config.flambda then [ Reduce_code_size; No_CSE ]
