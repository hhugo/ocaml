...
let add_notify_catch n f env =
  { env with  notify_catch = IntMap.add n f env.notify_catch }
...
let rec expr_size env =
  function
  | Uvar id ->
    (try V.find_same id env
     with
     | Not_found -> RHS_nonrec)
  ...
  | Uoffset (exp, offset) ->
    begin match expr_size env exp with
...
let transl_constant dbg =
  function
  | Uconst_int n -> int_const dbg n
  | Uconst_ref (label, def_opt) ->
    Option.iter (fun def -> Cmmgen_state.add_structured_constant label def)
      def_opt;
    Cconst_symbol (label, dbg)

let emit_constant cst cont =
  match cst with
  | Uconst_int n -> cint_const n :: cont
...
let join_unboxed_number_kind ~strict k1 k2 =
  ...
  | Boxed (b1, c1), Boxed (b2, c2) when equal_boxed_number b1 b2 ->
    Boxed (b1, c1 && c2)
  | No_result, k | k, No_result -> k
  (* if a branch never returns, it is safe to unbox it *)
  ...
  let rec aux =
    ...
    | Cop (Calloc, [ Cconst_natint (hdr, _); Cconst_symbol (ops, _); _ ], dbg)
      ...
      then
        ...
        Nativeint.equal hdr boxedint32_header && String.equal ops caml_int32_ops
      then
        notify (Boxed (Boxed_integer (Pint32, dbg), false))
      else if
        Nativeint.equal hdr boxedint64_header && String.equal ops caml_int64_ops
      then
    ...
    | l -> if not (Cmm.iter_shallow_tail aux l) then notify No_unboxing
  in
  aux cmm;
  !r
...
let rec transl env e =
        ...
        let without_header =
          if f.arity = 1 || f.arity = 0 then
            Cconst_symbol (f.label, dbg) ::
              alloc_closure_info ~arity:f.arity ~startenv:(startenv - pos) dbg ::
                transl_fundecls (pos + 3) rem
          else
            Cconst_symbol (curry_function_sym f.arity, dbg) ::
              alloc_closure_info ~arity:f.arity ~startenv:(startenv - pos) dbg ::
                Cconst_symbol (f.label, dbg) :: transl_fundecls (pos + 4) rem
        in
        if pos = 0 then
          without_header
        else
          alloc_infix_header pos f.dbg :: without_header
    in
    let dbg =
      match fundecls with
      | [] -> Debuginfo.none
  ...
  | Uprim (prim, args, dbg) ->
    ...
    | Pbigarrayref (unsafe, _num_dims, elt_kind, layout), arg1 :: argl ->
      let elt =
        bigarray_get unsafe elt_kind layout (transl env arg1)
          (List.map (transl env) argl) dbg
      in
    ...
    | Pbigarrayset (unsafe, _num_dims, elt_kind, layout), arg1 :: argl ->
      ...
      return_unit dbg
        (bigarray_set unsafe elt_kind layout (transl env arg1)
  ...
  | Uswitch (arg, s, dbg) ->
    ...
    if Array.length s.us_index_blocks = 0 then
      make_switch (untag_int (transl env arg) dbg) s.us_index_consts
        (Array.map (fun expr -> transl env expr, dbg) s.us_actions_consts) dbg
    else if Array.length s.us_index_consts = 0 then
      bind "switch" (transl env arg)
        (fun arg ->
           transl_switch dbg env (get_tag arg dbg) s.us_index_blocks
             s.us_actions_blocks)
    else
      bind "switch" (transl env arg)
        (fun arg ->
           Cifthenelse
             (Cop (Cand, [ arg; Cconst_int (1, dbg) ], dbg),
              dbg,
              transl_switch dbg env (untag_int arg dbg) s.us_index_consts
                s.us_actions_consts,
              dbg,
              transl_switch dbg env (get_tag arg dbg) s.us_index_blocks
                s.us_actions_blocks,
  ...
  | Uifthenelse (cond, ifso, ifnot) ->
    ...
    let dbg = Debuginfo.none in
    transl_if env Unknown dbg cond ifso_dbg (transl env ifso) ifnot_dbg
      (transl env ifnot)
  | Usequence (exp1, exp2) ->
    ...
    return_unit dbg
      (ccatch
         (raise_num,
          [],
          create_loop
            (transl_if env Unknown dbg cond dbg (remove_unit (transl env body))
    ...
    return_unit dbg
      (Clet_mut
         (id,
          typ_int,
          transl env low,
          bind_nonvar "bound" (transl env high)
            (fun high ->
               ccatch
                 (raise_num,
                  [],
  ...
  let report args =
    List.iter2
      (fun (_id, kind, u) c ->
         ...
         in
         u :=
           join_unboxed_number_kind ~strict !u (is_unboxed_number_cmm ~strict c))
      ids args
  in
    ...
    List.fold_right
      (fun (id, _kind, u) (env, rewrite, ids) ->
         ...
         | No_unboxing | Boxed (_, true) | No_result ->
           env, (fun x -> x) :: rewrite, (id, Cmm.typ_val) :: ids
         | Boxed (bn, false) ->
           ...
           (VP.create unboxed_id, typ_of_boxed_number bn) :: ids) ids
      (env, [], [])
  in
  if env == new_env then
    (* No unboxing *)
    ccatch (nfail, ids, body, transl env handler, dbg)
  else (* allocate new "nfail" to catch errors more easily *)
...
and transl_make_array dbg env kind args =
  ...
  | Pgenarray ->
    Cop
      (Cextcall ("caml_make_array", typ_val, [], true),
...
and transl_ccall env prim args dbg =
  let transl_arg native_repr arg =
    ...
    | Same_as_ocaml_repr -> XInt, transl env arg
    | Unboxed_float -> XFloat, transl_unbox_float dbg env arg
    | Unboxed_integer bi ->
      let xty =
        match bi with
        | Pnativeint -> XInt
    ...
    | Untagged_int -> XInt, untag_int (transl env arg) dbg
  in
  ...
  wrap_result
    (Cop
       (Cextcall
          (Primitive.native_name prim, typ_res, typ_args, prim.prim_alloc),
        args,
        dbg))
...
and transl_prim_1 env p arg dbg =
  match p with
  (* Generic operations *)
  | Popaque -> opaque (transl env arg) dbg
  (* Heap operations *)
  ...
  (* Boolean operations *)
  | Pnot ->
    transl_if env Then_false_else_true dbg arg dbg (Cconst_int (1, dbg)) dbg
      (Cconst_int (3, dbg))
  (* Test integer/block *)
  | Pisint ->
  ...
  | Pnegbint bi ->
    box_int dbg bi
      (Cop
         (Csubi, [ Cconst_int (0, dbg); transl_unbox_int dbg env bi arg ], dbg))
  | Pbbswap bi ->
    box_int dbg bi (bbswap bi (transl_unbox_int dbg env bi arg) dbg)
  | Pbswap16 ->
    tag_int (bswap16 (ignore_high_bit_int (untag_int (transl env arg) dbg)) dbg)
      dbg
  | Pfield_computed
  | Psequand
  | Psequor
  ...
  | Pbigstring_set _
    ->
    fatal_errorf "Cmmgen.transl_prim_1: %a" Printclambda_primitives.primitive p
...
and transl_prim_2 env p arg1 arg2 dbg =
  ...
  | Psequand ->
    let dbg' = Debuginfo.none in
    transl_sequand env Then_true_else_false dbg arg1 dbg' arg2 dbg
      (Cconst_int (3, dbg)) dbg' (Cconst_int (1, dbg))
  ...
  | Paddbint bi ->
    box_int dbg bi
      (add_int (transl_unbox_int_low dbg env bi arg1)
         (transl_unbox_int_low dbg env bi arg2) dbg)
  | Psubbint bi ->
    box_int dbg bi
      (sub_int (transl_unbox_int_low dbg env bi arg1)
         (transl_unbox_int_low dbg env bi arg2) dbg)
  | Pmulbint bi ->
    box_int dbg bi
      (mul_int (transl_unbox_int_low dbg env bi arg1)
         (transl_unbox_int_low dbg env bi arg2) dbg)
  | Pdivbint { size = bi; is_safe } ->
    box_int dbg bi
      (safe_div_bi is_safe (transl_unbox_int dbg env bi arg1)
         (transl_unbox_int dbg env bi arg2) bi dbg)
  | Pmodbint { size = bi; is_safe } ->
    box_int dbg bi
      (safe_mod_bi is_safe (transl_unbox_int dbg env bi arg1)
         (transl_unbox_int dbg env bi arg2) bi dbg)
  | Pandbint bi ->
    box_int dbg bi
      (Cop
         (Cand,
          [ transl_unbox_int_low dbg env bi arg1;
            transl_unbox_int_low dbg env bi arg2 ],
          dbg))
  | Porbint bi ->
    box_int dbg bi
      (Cop
  ...
  | Plslbint bi ->
    box_int dbg bi
      (lsl_int (transl_unbox_int_low dbg env bi arg1)
         (untag_int (transl env arg2) dbg) dbg)
  | Plsrbint bi ->
    box_int dbg bi
      (lsr_int (make_unsigned_int bi (transl_unbox_int dbg env bi arg1) dbg)
         (untag_int (transl env arg2) dbg) dbg)
  | Pasrbint bi ->
    box_int dbg bi
      (asr_int (transl_unbox_int dbg env bi arg1)
         (untag_int (transl env arg2) dbg) dbg)
  | Pbintcomp (bi, cmp) ->
    tag_int
      (Cop
  ...
  | Pbbswap _
    ->
    fatal_errorf "Cmmgen.transl_prim_2: %a" Printclambda_primitives.primitive p

and transl_prim_3 env p arg1 arg2 arg3 dbg =
  match p with
  (* Heap operations *)
  | Psetfield_computed (ptr, init) ->
    setfield_computed ptr init (transl env arg1) (transl env arg2)
      (transl env arg3) dbg
  (* String operations *)
  | Pbytessetu ->
    bytesset_unsafe (transl env arg1) (transl env arg2) (transl env arg3) dbg
  | Pbytessets ->
    bytesset_safe (transl env arg1) (transl env arg2) (transl env arg3) dbg
  (* Array operations *)
  | Parraysetu kind ->
    let newval =
      match kind with
  ...
  | Pbbswap _
    ->
    fatal_errorf "Cmmgen.transl_prim_3: %a" Printclambda_primitives.primitive p
...
and transl_let env str kind id exp transl_body =
  ...
  | Boxed (boxed_number, false) ->
    ...
    let body =
      transl_body (add_unboxed_id (VP.var id) unboxed_id boxed_number env)
    in
...
and transl_if
    env (approx : then_else) (dbg : Debuginfo.t) cond (then_dbg : Debuginfo.t)
    then_ (else_dbg : Debuginfo.t) else_
=
  match cond with
  ...
  | Uifthenelse (arg1, arg2, Uconst (Uconst_int 0)) ->
    ...
    let ifso_dbg = Debuginfo.none in
    transl_sequand env approx inner_dbg arg1 ifso_dbg arg2 then_dbg then_
      else_dbg else_
  | Ulet (str, kind, id, exp, cond) ->
    transl_let env str kind id exp
      (fun env -> transl_if env approx dbg cond then_dbg then_ else_dbg else_)
  | Uprim (Psequand, [ arg1; arg2 ], inner_dbg) ->
    transl_sequand env approx inner_dbg arg1 inner_dbg arg2 then_dbg then_
      else_dbg else_
  | Uifthenelse (arg1, Uconst (Uconst_int 1), arg2) ->
    let inner_dbg = Debuginfo.none in
    let ifnot_dbg = Debuginfo.none in
    transl_sequor env approx inner_dbg arg1 ifnot_dbg arg2 then_dbg then_
      else_dbg else_
  | Uprim (Psequor, [ arg1; arg2 ], inner_dbg) ->
    transl_sequor env approx inner_dbg arg1 inner_dbg arg2 then_dbg then_
      else_dbg else_
  | Uprim (Pnot, [ arg ], _dbg) ->
    transl_if env (invert_then_else approx) dbg arg else_dbg else_ then_dbg
      then_
    ...
    make_shareable_cont then_dbg
      (fun shareable_then ->
         make_shareable_cont else_dbg
           (fun shareable_else ->
              mk_if_then_else inner_dbg (test_bool inner_dbg (transl env cond))
                ifso_dbg
                (transl_if env approx ifso_dbg ifso then_dbg shareable_then
                   else_dbg shareable_else) ifnot_dbg
                (transl_if env approx ifnot_dbg ifnot then_dbg shareable_then
                   else_dbg shareable_else)) else_) then_
  | _ ->
    begin match approx with
    | Then_true_else_false -> transl env cond
    | Then_false_else_true -> mk_not dbg (transl env cond)
    | Unknown ->
      mk_if_then_else dbg (test_bool dbg (transl env cond)) then_dbg then_
        else_dbg else_
    end

and transl_sequand
    env (approx : then_else) (arg1_dbg : Debuginfo.t) arg1
    (arg2_dbg : Debuginfo.t) arg2 (then_dbg : Debuginfo.t) then_
    (else_dbg : Debuginfo.t) else_
=
  make_shareable_cont else_dbg
    (fun shareable_else ->
       transl_if env Unknown arg1_dbg arg1 arg2_dbg
         (transl_if env approx arg2_dbg arg2 then_dbg then_ else_dbg
            shareable_else) else_dbg shareable_else) else_
...
=
  make_shareable_cont then_dbg
    (fun shareable_then ->
       transl_if env Unknown arg1_dbg arg1 then_dbg shareable_then arg2_dbg
         (transl_if env approx arg2_dbg arg2 then_dbg shareable_then else_dbg
            else_)) then_
...
and transl_letrec env bindings cont =
  let dbg = Debuginfo.none in
  let bsz = List.map (fun (id, exp) -> id, exp, expr_size V.empty exp) bindings
  in
  let op_alloc prim args = Cop (Cextcall (prim, typ_val, [], true), args, dbg)
  in
  let rec init_blocks =
    function
    | [] -> fill_nonrec bsz
    | (id, _exp, RHS_block sz) :: rem ->
      Clet
        (id, op_alloc "caml_alloc_dummy" [ int_const dbg sz ], init_blocks rem)
  ...
  and fill_blocks =
    ...
    | (id, exp, (RHS_block _ | RHS_infix _ | RHS_floatblock _)) :: rem ->
      let op =
        Cop
          (Cextcall ("caml_update_dummy", typ_void, [], false),
...
let rec transl_all_functions already_translated cont =
  ...
  | Some f ->
    ...
    if String.Set.mem sym already_translated then
      ...
      transl_all_functions (String.Set.add sym already_translated)
...
=
  ...
  List.iter
    (fun { symbol; exported; definition = cst; provenance = _ } ->
       let global : Cmmgen_state.is_global = if exported then Global else Local
       in
       emit_clambda_constant symbol global cst) constants;
  !c
...
let transl_all_functions cont =
  let rec aux already_translated cont translated_functions =
    if Cmmgen_state.no_more_functions () then
      cont, translated_functions
    else
  ...
  let translated_functions =
    ...
    List.map snd
      (List.sort (fun (dbg1, _) (dbg2, _) -> Debuginfo.compare dbg1 dbg2)
