...
let empty_env = {unboxed_ids = V.empty; environment_param = None}

let create_env ~environment_param =
  {unboxed_ids = V.empty; environment_param}
...
let alloc_floatarray_header len dbg =
  Cblockheader (floatarray_header len, dbg)
...
let rec add_const c n dbg =
  ...
  then c
    ...
    | Cop (Caddi, [Cconst_int (x, _); c], _) when no_overflow_add n x ->
        add_no_overflow n x c dbg
    | Cop (Caddi, [c; Cconst_int (x, _)], _) when no_overflow_add n x ->
        add_no_overflow n x c dbg
...
let force_tag_int i dbg =
  ...
  | c ->
      Cop
        ( Cor
        , [lsl_int c (Cconst_int (1, dbg)) dbg; Cconst_int (1, dbg)]
        , dbg )
...
let untag_int i dbg =
  ...
  | Cop
      (Caddi, [Cop (Clsl, [c; Cconst_int (1, _)], _); Cconst_int (1, _)], _)
    ->
      c
...
let divimm_parameters d =
  Nativeint.(
    ...
    let rec loop p (q1, r1) (q2, r2) =
      ...
      let q2, r2 =
        if ucompare r2 d >= 0 then succ q2, sub r2 d else q2, r2
      in
...
let raise_regular dbg exc =
  Csequence
    ( Cop
        ...
        , [Cconst_symbol ("caml_backtrace_pos", dbg); Cconst_int (0, dbg)]
        , dbg )
    , Cop (Craise Raise_withtrace, [exc], dbg) )

let raise_symbol dbg symb = raise_regular dbg (Cconst_symbol (symb, dbg))
...
let map_ccatch f rec_flag handlers body =
  let handlers =
    List.map (fun (n, ids, handler, dbg) -> n, ids, f handler, dbg) handlers
  in
  Ccatch (rec_flag, handlers, f body)

let rec unbox_float dbg cmm =
  match cmm with
  | Cop (Calloc, [Cblockheader (header, _); c], _)
    when header = float_header ->
      c
  ...
  | Cswitch (e, tbl, el, dbg') ->
      Cswitch
        ( e
        , tbl
        , Array.map (fun (expr, dbg) -> unbox_float dbg expr, dbg) el
        , dbg' )
...
let rec remove_unit = function
  ...
  | Cifthenelse (cond, ifso_dbg, ifso, ifnot_dbg, ifnot, dbg) ->
      Cifthenelse
        (cond, ifso_dbg, remove_unit ifso, ifnot_dbg, remove_unit ifnot, dbg)
...
let get_header_without_profinfo ptr dbg =
  ...
  then
    Cop
      (Cand, [get_header ptr dbg; Cconst_int (non_profinfo_mask, dbg)], dbg)
...
let get_size ptr dbg =
  Cop
    (Clsr, [get_header_without_profinfo ptr dbg; Cconst_int (10, dbg)], dbg)
...
let lookup_tag obj tag dbg =
  bind "tag" tag (fun tag ->
      Cop
        ( Cextcall ("caml_get_public_method", typ_val, false, None)
        , [obj; tag]
        , dbg ) )
...
let make_checkbound dbg = function
  | [Cop (Clsr, [a1; Cconst_int (n, _)], _); Cconst_int (m, _)]
    when m lsl n > n ->
      Cop
        (Ccheckbound, [a1; Cconst_int ((m lsl n) + (1 lsl n) - 1, dbg)], dbg)
...
let rec emit_structured_constant (sym, is_global) cst cont =
  ...
  | Uconst_closure (fundecls, lbl, fv) ->
      Cmmgen_state.add_constant
        lbl
        (Const_closure (is_global, fundecls, fv));
...
let split_int64_for_32bit_target arg dbg =
  bind "split_int64" arg (fun arg ->
      ...
      let second =
        Cop (Cadda, [Cconst_int (2 * size_int, dbg); arg], dbg)
      in
      Ctuple
...
let rec unbox_int bi arg dbg =
  ...
  | Cop
      ( Calloc
      , [hdr; ops; Cop (Clsl, [contents; Cconst_int (32, _)], dbg')]
      , _dbg )
    when bi = Pint32
         && size_int = 8
         && big_endian
      ...
      Cop
        ( Casr
        , [ Cop (Clsl, [contents; Cconst_int (32, dbg)], dbg')
          ; Cconst_int (32, dbg) ]
        , dbg )
  ...
  | Cifthenelse (cond, ifso_dbg, e1, ifnot_dbg, e2, dbg) ->
      Cifthenelse
        ( cond
        , ifso_dbg
        , unbox_int bi e1 ifso_dbg
        , ifnot_dbg
        , unbox_int bi e2 ifnot_dbg
        , dbg )
  ...
  | Ctrywith (e1, id, e2, handler_dbg) ->
      Ctrywith
        (unbox_int bi e1 dbg, id, unbox_int bi e2 handler_dbg, handler_dbg)
  | _ ->
      if size_int = 4 && bi = Pint64
      then split_int64_for_32bit_target arg dbg
      else
        Cop
          ( Cload
              ((if bi = Pint32 then Thirtytwo_signed else Word_int), Mutable)
...
let unaligned_load_16 ptr idx dbg =
  ...
  then Cop (Cload (Sixteen_unsigned, Mutable), [add_int ptr idx dbg], dbg)
    ...
    let v1 =
      Cop (Cload (Byte_unsigned, Mutable), [add_int ptr idx dbg], dbg)
    in
    let v2 =
      Cop
        ( Cload (Byte_unsigned, Mutable)
...
let unaligned_set_64 ptr idx newval dbg =
  ...
  if Arch.allow_unaligned_access
  then
    Cop (Cstore (Word_int, Assignment), [add_int ptr idx dbg; newval], dbg)
  else
    let cconst_int i = Cconst_int (i, dbg) in
    let v1 =
      Cop
...
let max_or_zero a dbg =
  bind "size" a (fun a ->
      ...
      let sign =
        Cop (Casr, [a; Cconst_int ((size_int * 8) - 1, dbg)], dbg)
      in
...
let simplif_primitive_32bits = function
  ...
  | Pbintcomp (Pint64, Lambda.Cgt) ->
      Pccall (default_prim "caml_greaterthan")
  | Pbintcomp (Pint64, Lambda.Cle) -> Pccall (default_prim "caml_lessequal")
  | Pbintcomp (Pint64, Lambda.Cge) ->
      Pccall (default_prim "caml_greaterequal")
  ...
  | Pstring_load (Sixty_four, _) ->
      Pccall (default_prim "caml_string_get64")
...
let make_switch arg cases actions dbg =
  let is_const = function
    ...
    | Cconst_int (n, _) | Cconst_pointer (n, _) -> n land 1 = 1
    | Cconst_natint (n, _) | Cconst_natpointer (n, _) ->
        Nativeint.(to_int (logand n one) = 1)
    | Cconst_symbol _ -> true
    | _ -> false
  in
  if Array.for_all (fun (expr, _dbg) -> is_const expr) actions
  then (
    let to_data_item (expr, _dbg) =
      match expr with
      | Cconst_int (n, _) | Cconst_pointer (n, _) ->
          Cint (Nativeint.of_int n)
      | Cconst_natint (n, _) | Cconst_natpointer (n, _) -> Cint n
  ...
  else Cswitch (arg, cases, actions, dbg)
...
module StoreExpForSwitch = Switch.CtxStore (struct
  ...
  let make_key index expr =
    let continuation =
      match expr with Cexit (i, []) -> Some i | _ -> None
    in
...
let transl_int_switch loc arg low high cases default =
  match cases with
  | [] -> assert false
  | _ :: _ ->
      ...
      bind "switcher" arg (fun a ->
          SwitcherBlocks.zyva loc (low, high) a (Array.of_list inters) store
      )
 ...
 *)
  ...
  | Uprim (p, _, dbg) -> (
    match simplif_primitive p with
    | Pccall p -> unboxed_number_kind_of_unbox dbg p.prim_native_repr_res
    | Pfloatfield _ | Pfloatofint | Pnegfloat | Pabsfloat | Paddfloat
     |Psubfloat | Pmulfloat | Pdivfloat
    ...
    | Pstring_load (Thirty_two, _) | Pbytes_load (Thirty_two, _) ->
        Boxed (Boxed_integer (Pint32, dbg), false)
    | Pstring_load (Sixty_four, _) | Pbytes_load (Sixty_four, _) ->
        Boxed (Boxed_integer (Pint64, dbg), false)
  ...
  | Ustringswitch (_, actions, default_opt) -> (
      ...
      match default_opt with None -> k | Some default -> join k default )
...
let rec transl env e =
  ...
  | Ugeneric_apply (clos, args, dbg) ->
      let arity = List.length args in
      let cargs =
        Cconst_symbol (apply_function arity, dbg)
        :: List.map (transl env) (args @ [clos])
      in
      Cop (Capply typ_val, cargs, dbg)
  | Usend (kind, met, obj, args, dbg) ->
      let call_met obj args clos =
        if args = []
        then Cop (Capply typ_val, [get_field env clos 0 dbg; obj; clos], dbg)
        else
      ...
      bind "obj" (transl env obj) (fun obj ->
          ...
          | Self, _ ->
              bind
                "met"
                (lookup_label obj (transl env met) dbg)
                (call_met obj args)
          ...
          | _ ->
              bind
                "met"
                (lookup_tag obj (transl env met) dbg)
                (call_met obj args) )
  ...
  | Uprim (prim, args, dbg) -> (
    ...
    | ( ( Pfield_computed | Psequand | Psequor | Pnot | Pnegint | Paddint
        | Psubint | Pmulint | Pandint | Porint | Pxorint | Plslint | Plsrint
        | Pasrint | Pintoffloat | Pfloatofint | Pnegfloat | Pabsfloat
        | Paddfloat | Psubfloat | Pmulfloat | Pdivfloat | Pstringlength
        | Pstringrefu | Pstringrefs | Pbyteslength | Pbytesrefu | Pbytessetu
        | Pbytesrefs | Pbytessets | Pisint | Pisout | Pbswap16
        ...
        | Poffsetref _ | Pfloatcomp _ | Parraylength _ | Parrayrefu _
        | Parraysetu _ | Parrayrefs _ | Parraysets _ | Pbintofint _
        | Pintofbint _
        | Pcvtbint (_, _)
        | Pnegbint _ | Paddbint _ | Psubbint _ | Pmulbint _ | Pdivbint _
        | Pmodbint _ | Pandbint _ | Porbint _ | Pxorbint _ | Plslbint _
        | Plsrbint _ | Pasrbint _
        | Pbintcomp (_, _)
  ...
  | Ustringswitch (arg, sw, d) ->
      ...
      bind "switch" (transl env arg) (fun arg ->
          strmatch_compile
            dbg
            arg
            (Misc.may_map (transl env) d)
            (List.map (fun (s, act) -> s, transl env act) sw) )
  | Ustaticfail (nfail, args) -> Cexit (nfail, List.map (transl env) args)
  ...
  | Ucatch (nfail, ids, body, handler) ->
      ...
      let ids_with_types = List.map (fun (i, _) -> i, Cmm.typ_val) ids in
      ccatch
        (nfail, ids_with_types, transl env body, transl env handler, dbg)
  ...
  | Uassign (id, exp) -> (
      let dbg = Debuginfo.none in
      match is_unboxed_id id env with
      | None -> return_unit dbg (Cassign (id, transl env exp))
      | Some (unboxed_id, bn) ->
          return_unit
            dbg
            (Cassign (unboxed_id, transl_unbox_number dbg env bn exp)) )
  | Uunreachable ->
      let dbg = Debuginfo.none in
...
and transl_ccall env prim args dbg =
  ...
  wrap_result
    (Cop
       ( Cextcall
           (Primitive.native_name prim, typ_res, prim.prim_alloc, None)
       , args
       , dbg ))
...
and transl_prim_1 env p arg dbg =
  ...
  | Praise Lambda.Raise_regular -> raise_regular dbg (transl env arg)
  (* Integer operations *)
  | Pnegint -> Cop (Csubi, [Cconst_int (2, dbg); transl env arg], dbg)
  | Poffsetint n ->
      if no_overflow_lsl n 1
      then add_const (transl env arg) (n lsl 1) dbg
      else transl_prim_2 env Paddint arg (Uconst (Uconst_int n)) dbg
  | Poffsetref n ->
      return_unit
        dbg
            ...
            then Cop (Clsr, [hdr; Cconst_int (wordsize_shift, dbg)], dbg)
              ...
              bind "header" hdr (fun hdr ->
                  Cifthenelse
                    ...
                    , dbg
                    , Cop
                        (Clsr, [hdr; Cconst_int (wordsize_shift, dbg)], dbg)
                    , dbg
                    , Cop
                        (Clsr, [hdr; Cconst_int (numfloat_shift, dbg)], dbg)
                    , dbg ) )
          in
  ...
  | Pintofbint bi -> force_tag_int (transl_unbox_int dbg env bi arg) dbg
  | Pcvtbint (bi1, bi2) ->
  ...
  | Pnegbint bi ->
      box_int
        ...
        (Cop
           ( Csubi
           , [Cconst_int (0, dbg); transl_unbox_int dbg env bi arg]
           , dbg ))
  | Pbbswap bi ->
      let prim =
        match bi with
      ...
      box_int
        ...
        (Cop
           ( Cextcall
               ( Printf.sprintf "caml_%s_direct_bswap" prim
               , typ_int
               , false
               , None )
  ...
  | Pbswap16 ->
      tag_int
        (Cop
           ( Cextcall ("caml_bswap16_direct", typ_int, false, None)
           , [untag_int (transl env arg) dbg]
           , dbg ))
        dbg
  | Pfield_computed | Psequand | Psequor | Paddint | Psubint | Pmulint
   |Pandint | Porint | Pxorint | Plslint | Plsrint | Pasrint | Paddfloat
   |Psubfloat | Pmulfloat | Pdivfloat | Pstringrefu | Pstringrefs
   |Pbytesrefu | Pbytessetu | Pbytesrefs | Pbytessets | Pisout
   |Pread_symbol _
   |Pmakeblock (_, _, _)
   |Psetfield (_, _, _)
   |Psetfield_computed (_, _)
   ...
   |Pmakearray (_, _)
   |Pduparray (_, _)
   |Parrayrefu _ | Parraysetu _ | Parrayrefs _ | Parraysets _ | Paddbint _
   ...
   |Pbigstring_load _ | Pbigstring_set _ ->
      fatal_errorf
        "Cmmgen.transl_prim_1: %a"
        Printclambda_primitives.primitive
        p
...
and transl_prim_2 env p arg1 arg2 dbg =
  ...
  | Psetfield (n, ptr, init) -> (
    ...
    | Caml_modify ->
        return_unit
          ...
          (Cop
             ( Cextcall ("caml_modify", typ_void, false, None)
             , [field_address (transl env arg1) n dbg; transl env arg2]
             , dbg ))
    | Caml_initialize ->
        return_unit
          dbg
  ...
  | Pintcomp cmp ->
      tag_int
        (Cop
           ( Ccmpi (transl_int_comparison cmp)
  ...
  | Pfloatcomp cmp ->
      tag_int
        (Cop
           ( Ccmpf (transl_float_comparison cmp)
           , [ transl_unbox_float dbg env arg1
             ; transl_unbox_float dbg env arg2 ]
           , dbg ))
        dbg
        ...
        bind "arr" (transl env arg1) (fun arr ->
            bind "index" (transl env arg2) (fun idx ->
                Cifthenelse
                  ...
                  , dbg ) ) )
    | Paddrarray -> addr_array_ref (transl env arg1) (transl env arg2) dbg
    | Pintarray ->
        (* CR mshinwell: for int/addr_array_ref move "dbg" to first arg *)
        int_array_ref (transl env arg1) (transl env arg2) dbg
    | Pfloatarray -> float_array_ref dbg (transl env arg1) (transl env arg2)
    )
  | Parrayrefs kind -> (
    match kind with
    | Pgenarray ->
                    ...
                    then
                      ...
                      Cifthenelse
                        ...
                        , Csequence
                            ( make_checkbound
                                dbg
                                [float_array_length hdr dbg; idx]
                            , float_array_ref dbg arr idx )
                        , dbg ) ) ) )
    | Paddrarray ->
        bind "index" (transl env arg2) (fun idx ->
  ...
  | Pbintcomp (bi, cmp) ->
      tag_int
        (Cop
           ...
           , dbg ))
        dbg
  | Pnot | Pnegint | Pintoffloat | Pfloatofint | Pnegfloat | Pabsfloat
   |Pstringlength | Pbyteslength | Pbytessetu | Pbytessets | Pisint
   ...
   |Pmakearray (_, _)
   |Pduparray (_, _)
   |Parraylength _ | Parraysetu _ | Parraysets _ | Pbintofint _
   |Pintofbint _
   |Pcvtbint (_, _)
   |Pnegbint _
   |Pbigarrayref (_, _, _, _)
...
and transl_prim_3 env p arg1 arg2 arg3 dbg =
  ...
  | Pfield_computed | Psequand | Psequor | Pnot | Pnegint | Paddint
   ...
   |Pasrint | Pintoffloat | Pfloatofint | Pnegfloat | Pabsfloat
   |Paddfloat | Psubfloat | Pmulfloat | Pdivfloat | Pstringlength
   |Pstringrefu | Pstringrefs | Pbyteslength | Pbytesrefu | Pbytesrefs
   |Pisint | Pisout | Pbswap16 | Pint_as_pointer | Popaque | Pread_symbol _
   |Pmakeblock (_, _, _)
   |Pfield _
   |Psetfield (_, _, _)
   |Pfloatfield _
   |Psetfloatfield (_, _)
   |Pduprecord (_, _)
   |Pccall _ | Praise _ | Pdivint _ | Pmodint _ | Pintcomp _
   |Poffsetint _ | Poffsetref _ | Pfloatcomp _
   |Pmakearray (_, _)
   |Pduparray (_, _)
   |Parraylength _ | Parrayrefu _ | Parrayrefs _ | Pbintofint _
   |Pintofbint _
   |Pcvtbint (_, _)
   |Pnegbint _ | Paddbint _ | Psubbint _ | Pmulbint _ | Pdivbint _
   |Pmodbint _ | Pandbint _ | Porbint _ | Pxorbint _ | Plslbint _
   ...
   |Pbigarrayref (_, _, _, _)
   |Pbigarrayset (_, _, _, _)
   |Pbigarraydim _ | Pstring_load _ | Pbytes_load _ | Pbigstring_load _
   |Pbbswap _ ->
      fatal_errorf
        "Cmmgen.transl_prim_3: %a"
        Printclambda_primitives.primitive
        p
...
and is_shareable_cont exp =
  match exp with Cexit (_, []) -> true | _ -> false
...
and transl_if env (approx : then_else) (dbg : Debuginfo.t) cond
    (then_dbg : Debuginfo.t) then_ (else_dbg : Debuginfo.t) else_ =
  match cond with
...
and transl_sequand env (approx : then_else) (arg1_dbg : Debuginfo.t) arg1
    (arg2_dbg : Debuginfo.t) arg2 (then_dbg : Debuginfo.t) then_
...
and transl_sequor env (approx : then_else) (arg1_dbg : Debuginfo.t) arg1
    (arg2_dbg : Debuginfo.t) arg2 (then_dbg : Debuginfo.t) then_
    (else_dbg : Debuginfo.t) else_ =
  make_shareable_cont
    then_dbg
...
and transl_switch loc env arg index cases =
  ...
  | _ -> (
      ...
      let index =
        Array.map (fun j -> store.Switch.act_store j cases.(j)) index
      in
      ...
      | inters ->
          bind "switcher" arg (fun a ->
              SwitcherBlocks.zyva
                ...
                (0, n_index - 1)
                a
                (Array.of_list inters)
                store ) )

and transl_letrec env bindings cont =
  let dbg = Debuginfo.none in
  let bsz =
    List.map (fun (id, exp) -> id, exp, expr_size V.empty exp) bindings
  in
  let op_alloc prim sz =
    Cop (Cextcall (prim, typ_val, true, None), [int_const dbg sz], dbg)
  in
...
let transl_function ~ppf_dump f =
  let body =
    if Config.flambda
    then Un_anf.apply ~ppf_dump f.body ~what:f.label
    else f.body
  in
  let cmm_body =
    let env = create_env ~environment_param:f.env in
    if !Clflags.afl_instrument
    then Afl_instrument.instrument_function (transl env body) f.dbg
    else transl env body
  in
...
let rec transl_all_functions ~ppf_dump already_translated cont =
  ...
  | Some f ->
      ...
      then transl_all_functions ~ppf_dump already_translated cont
      else
...
let emit_constant_closure ((_, global_symb) as symb) fundecls clos_vars cont
  ...
  | [] ->
      ...
      assert (clos_vars = []);
      cdefine_symbol symb @ List.fold_right emit_constant clos_vars cont
  | f1 :: remainder ->
...
let transl_clambda_constants
    (constants : Clambda.preallocated_constant list) cont =
  let c = ref cont in
...
let emit_cmm_data_items_for_constants cont =
  let c = ref cont in
  String.Map.iter
    (fun symbol (cst : Cmmgen_state.constant) ->
      match cst with
  ...
  let space =
    ...
    List.map
      (fun field ->
        ...
        | Some (Uconst_field_int n) -> cint_const n
        | Some (Uconst_field_ref label) -> Csymbol_address label )
      fields
  in
  let data =
    ...
    ::
    ( if exported
    then Cglobal_symbol symbol :: Cdefine_symbol symbol :: space
    else Cdefine_symbol symbol :: space )
  in
  Cdata data :: cont
                       ...
                       , Csequence
                           ( Cifthenelse
                               ...
                               , Cassign
                                   ( hi
                                   , Cop
                                       (Csubi, [Cvar mi; cconst_int 2], dbg)
                                   )
                               , dbg
                               , Cassign (li, Cvar mi)
                               , dbg )
...
let tuplify_function arity =
  ...
  let rec access_components i =
    if i >= arity
    then []
    else get_field env (Cvar arg) i (dbg ()) :: access_components (i + 1)
  in
  ...
  Cfunction
    ...
    ; fun_body =
        Cop
          ( Capply typ_val
          , (get_field env (Cvar clos) 2 (dbg ()) :: access_components 0)
            @ [Cvar clos]
          , dbg () )
    ; fun_codegen_options = []
    ; fun_dbg }
...
let final_curry_function arity =
  ...
  let rec curry_fun args clos n =
    ...
    then
      Cop
        ( Capply typ_val
        , (get_field env (Cvar clos) 2 (dbg ()) :: args)
    ...
    then
      ...
      Clet
        ...
        , curry_fun
            (get_field env (Cvar clos) 3 (dbg ()) :: args)
            newclos
            (n - 1) )
  in
  let fun_name =
          ...
          then
            Cop
              ...
              , [ alloc_closure_header 5 Debuginfo.none
                ; Cconst_symbol
                    (name1 ^ "_" ^ Int.to_string (num + 1), dbg ())
...
let generic_functions shared units =
  ...
  let accu =
    Int.Set.fold (fun n accu -> apply_function n :: accu) apply []
  in
  let accu =
    Int.Set.fold (fun n accu -> send_function n :: accu) send accu
  in
...
let predef_exception i name =
  ...
  let data_items =
    emit_block
      name_sym
      Local
      (string_header (String.length name))
