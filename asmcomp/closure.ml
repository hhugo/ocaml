...
let occurs_var var u =
  let rec occurs = function
    ...
    | Ufor (_id, lo, hi, _dir, body) ->
        occurs lo || occurs hi || occurs body
...
let lambda_smaller lam threshold =
  ...
  let rec lambda_size lam =
    ...
    | Uoffset (lam, _ofs) -> incr size; lambda_size lam
    ...
    | Uifthenelse (cond, ifso, ifnot) ->
        ...
        lambda_size ifso;
        lambda_size ifnot
    | Usequence (lam1, lam2) -> lambda_size lam1; lambda_size lam2
    | Uwhile (cond, body) ->
        size := !size + 2;
        lambda_size cond;
        lambda_size body
    | Ufor (_id, low, high, _dir, body) ->
        ...
        lambda_size high;
        lambda_size body
    | Uassign (_id, lam) -> incr size; lambda_size lam
    | Usend (_, met, obj, args, _) ->
        size := !size + 8;
        lambda_size met;
        lambda_size obj;
  ...
  try lambda_size lam; !size <= threshold with Exit -> false
...
let simplif_arith_prim_pure fpc p (args, approxs) dbg =
  ...
  | [Value_const (Uconst_ref (_, Some (Uconst_nativeint n)))] -> (
    ...
    | Pcvtbint (Pnativeint, Pint32) ->
        make_const_int32 (Nativeint.to_int32 n)
    | Pcvtbint (Pnativeint, Pint64) ->
        make_const_int64 (Int64.of_nativeint n)
...
let simplif_prim_pure fpc p (args, approxs) dbg =
  ...
  (* Catch-all *)
  | _ -> simplif_arith_prim_pure fpc p (args, approxs) dbg
...
let find_action idxs acts tag =
  if 0 <= tag && tag < Array.length idxs
  then (
    let idx = idxs.(tag) in
...
let rec substitute loc fpc sb rn ulam =
  ...
  | Ulet (str, kind, id, u1, u2) ->
      ...
      Ulet
        ...
        , substitute
            loc
            fpc
            (V.Map.add (VP.var id) (Uvar (VP.var id')) sb)
            rn
            u2 )
  | Uphantom_let _ -> no_phantom_lets ()
  ...
  | Uprim (p, args, dbg) ->
      ...
      let dbg = subst_debuginfo loc dbg in
      let res, _ =
        simplif_prim fpc p (sargs, List.map approx_ulam sargs) dbg
      in
      res
  | Uswitch (arg, sw, dbg) -> (
  ...
  | Uifthenelse (u1, u2, u3) -> (
    match substitute loc fpc sb rn u1 with
    | Uconst (Uconst_ptr n) ->
        if n <> 0
        then substitute loc fpc sb rn u2
        else substitute loc fpc sb rn u3
    | Uprim (P.Pmakeblock _, _, _) -> substitute loc fpc sb rn u2
    | su1 ->
        Uifthenelse
          (su1, substitute loc fpc sb rn u2, substitute loc fpc sb rn u3) )
  | Usequence (u1, u2) ->
  ...
  | Usend (k, u1, u2, ul, dbg) ->
      ...
      Usend
        ...
        , substitute loc fpc sb rn u1
        , substitute loc fpc sb rn u2
        , List.map (substitute loc fpc sb rn) ul
        , dbg )
  | Uunreachable -> Uunreachable

(* Perform an inline expansion *)

let is_simple_argument = function Uvar _ | Uconst _ -> true | _ -> false

let no_effects = function Uclosure _ -> true | u -> is_pure u
...
let rec bind_params_rec loc fpc subst params args body =
  ...
  | p1 :: pl, a1 :: al ->
      ...
      then
        bind_params_rec loc fpc (V.Map.add (VP.var p1) a1 subst) pl al body
      else
        ...
        let body' =
          bind_params_rec
            loc
            fpc
            (V.Map.add (VP.var p1) u2 subst)
            pl
            al
            body
        in
        if occurs_var (VP.var p1) body
        then Ulet (Immutable, Pgenval, p1', u1, body')
        else if no_effects a1
        then body'
...
let warning_if_forced_inline ~loc ~attribute warning =
  if attribute = Always_inline
  then Location.prerr_warning loc (Warnings.Inlining_impossible warning)
...
let close_var fenv cenv id =
  let ulam, _app = close_approx_var fenv cenv id in
  ulam

let rec close fenv cenv = function
  | Lvar id -> close_approx_var fenv cenv id
  | Lconst cst ->
      ...
      let rec transl = function
        ...
        | Const_base (Const_float x) ->
            str (Uconst_float (float_of_string x))
  ...
  | Lapply
    ...
    -> (
      ...
      match close fenv cenv funct, close_list fenv cenv args with
      | ( (ufunct, Value_closure (fundesc, approx_res))
        , [Uprim (P.Pmakeblock _, uargs, _)] )
        when List.length uargs = -fundesc.fun_arity ->
          let app = direct_apply ~loc ~attribute fundesc ufunct uargs in
        ...
        when nargs < fundesc.fun_arity ->
          ...
          let new_fun =
            iter
              first_args
              (Ulet
                 (Immutable, Pgenval, VP.create funct_var, ufunct, new_fun))
          in
      ...
      | (ufunct, Value_closure (fundesc, _approx_res)), uargs
        when fundesc.fun_arity > 0 && nargs > fundesc.fun_arity ->
          ...
          let body =
            Ugeneric_apply
              ( direct_apply ~loc ~attribute fundesc ufunct first_args
              , rem_args
              , dbg )
          in
          let result =
  ...
  | Llet (str, kind, id, lam, body) -> (
      ...
      | _, Value_const _ when str = Alias || is_pure ulam ->
  ...
  | Lprim (Pctconst c, [arg], _loc) ->
      let cst, approx =
        ...
        | Max_wosize ->
            make_const_int ((1 lsl ((8 * Arch.size_int) - 10)) - 1)
        | Ostype_unix -> make_const_bool (Sys.os_type = "Unix")
        | Ostype_win32 -> make_const_bool (Sys.os_type = "Win32")
        | Ostype_cygwin -> make_const_bool (Sys.os_type = "Cygwin")
        | Backend_type -> make_const_ptr 0
  ...
  | Lswitch (arg, sw, dbg) -> (
      let fn fail =
        ...
        and block_index, block_actions, fblock =
          close_switch fenv cenv sw.sw_blocks sw.sw_numblocks fail
        in
        let ulam =
          Uswitch
            ( uarg
  ...
  | Lifthenelse (arg, ifso, ifnot) -> (
    match close fenv cenv arg with
  ...
  | Levent (lam, _) -> close fenv cenv lam
...
and close_named fenv cenv id = function
  | Lfunction _ as funct -> close_one_function fenv cenv id funct
  | lam -> close fenv cenv lam
...
and close_functions fenv cenv fun_defs =
  ...
  let fv =
    V.Set.elements (free_variables (Lletrec (fun_defs, lambda_unit)))
  in
  ...
  let clos_fundef (id, params, return, body, fundesc, dbg) env_pos =
    ...
    let f =
      ...
      ; params =
          List.map (fun (var, kind) -> VP.create var, kind) fun_params
      ; return
      ; body = ubody
      ; dbg
      ; env = Some env_param }
...
let collect_exported_structured_constants a =
  ...
  and structured_constant = function
    ...
    | Uconst_float _ | Uconst_int32 _ | Uconst_int64 _
     |Uconst_nativeint _ | Uconst_float_array _ | Uconst_string _ ->
        ()
