...
module Env : sig
  ...
  let record_descr t id (descr : Export_info.descr) =
    if Export_id.Map.mem id !(t.ex_table)
    then
      Misc.fatal_errorf
  ...
  let new_unit_descr t = new_descr t (Value_constptr 0)

  let add_approx t var approx =
    if Variable.Map.mem var t.var
    then
      Misc.fatal_errorf
...
let descr_of_allocated_constant (c : Allocated_const.t) : Export_info.descr
  ...
  | Immutable_float_array fs ->
      Value_float_array
        { contents =
            Contents (Array.map (fun x -> Some x) (Array.of_list fs))
        ; size = List.length fs }
  | Float_array fs ->
      Value_float_array
        {contents = Unknown_or_mutable; size = List.length fs}
...
and descr_of_named (env : Env.t) (named : Flambda.named) :
  ...
  | Project_closure {set_of_closures; closure_id} -> (
    ...
    | Some (Value_set_of_closures set_of_closures) ->
        if not (Closure_id.Map.mem closure_id set_of_closures.results)
        then
          Misc.fatal_errorf
  ...
  | Project_var {closure; closure_id = closure_id'; var} -> (
    ...
    | Some (Value_closure {set_of_closures = {bound_vars; _}; closure_id})
      ->
        assert (Closure_id.equal closure_id closure_id');
        if not (Var_within_closure.Map.mem var bound_vars)
        then
          Misc.fatal_errorf
            "Project_var from %a (closure ID %a) of variable %a that is \
             not bound by the closure.  Variables bound by the closure \
             are: %a"
            Variable.print
            closure
            Closure_id.print
  ...
  let rec loop env (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (defs, program) ->
        ...
        let project_closures, other_constants =
          List.partition
            (function
              | _, _, Flambda.Project_closure _ -> true | _ -> false)
            defs
        in
        List.iter
          (fun (id, symbol, def) ->
...
let build_transient ~(backend : (module Backend_intf.S))
  ...
  then
      ...
      Export_id.Map.fold
        (fun _eid (descr : Export_info.descr) invariant_params ->
          ...
          | Export_info.Value_boxed_int (_, _)
           |Value_block _ | Value_mutable_block _ | Value_int _
           |Value_char _ | Value_constptr _ | Value_float _
           |Value_float_array _ | Value_string _ | Value_unknown_descr ->
              invariant_params )
        unnested_values
        invariant_params
    in
    ...
    let { Traverse_for_exported_symbols.set_of_closure_ids =
      ...
      Traverse_for_exported_symbols.traverse
        ...
        ~function_declarations_map
        ~values:
          (Compilation_unit.Map.find (Compilenv.current_unit ()) values)
        ~symbol_id
    ...
    let sets_of_closures =
      Set_of_closures_id.Map.filter_map
        function_declarations_map
        ~f:(fun key
           (fun_decls : Simple_value_approx.function_declarations)
           ->
          if Set_of_closures_id.Set.mem key relevant_set_of_closures
          then Some fun_decls
          else if Set_of_closures_id.Set.mem
                    key
                    relevant_set_of_closures_declaration_only
          then
            if fun_decls.is_classic_mode
            then Some (Simple_value_approx.clear_function_bodies fun_decls)
            else Some fun_decls
          else None )
    in
    let values =
