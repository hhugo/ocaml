...
let rec release_expired_active ci pos = function
  | i :: il when i.iend >= pos ->
      Interval.remove_expired_ranges i pos;
      if Interval.is_live i pos
      then i :: release_expired_active ci pos il
      else (
...
let rec release_expired_inactive ci pos = function
  | i :: il when i.iend >= pos ->
      Interval.remove_expired_ranges i pos;
      if not (Interval.is_live i pos)
      then i :: release_expired_inactive ci pos il
      else (
  ...
  | Unknown, _ -> (
      ...
      | rn ->
          ...
          let remove_bound_overlapping = function
            | {reg = {loc = Reg r}} as j ->
                if r - r0 < rn && regmask.(r - r0) && Interval.overlap j i
                then regmask.(r - r0) <- false
            | _ -> ()
          in
