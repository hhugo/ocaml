...
let check_invariants (instr : M.instruction) ~(avail_before : RAS.t) =
  ...
  | Ok avail_before ->
      ...
      if not
           (R.Set.subset instr.live (RD.Set.forget_debug_info avail_before))
      then
        Misc.fatal_errorf
...
*)
let rec available_regs (instr : M.instruction) ~(avail_before : RAS.t) :
    RAS.t =
  ...
  let avail_across, avail_after =
    ...
    | Ok avail_before -> (
      ...
      | Iop (Imove | Ireload | Ispill) ->
          ...
          let results =
            Array.map2
              (fun arg_reg result_reg ->
                ...
                | exception Not_found ->
                    assert false
                    (* see second invariant in [check_invariants] *)
                | arg_reg ->
                    RD.create_copying_debug_info
                      ~reg:result_reg
          ...
          let avail_after =
            RD.Set.union avail_across (RD.Set.of_array results)
          in
      ...
      | Icatch (recursive, handlers, body) ->
            ...
            let avail_at_top_of_handlers' =
              List.map
                (fun (nfail, _handler) ->
                  ...
                  | avail_at_top_of_handler ->
                      nfail, avail_at_top_of_handler )
                handlers
            in
          ...
          in
          let avail_after_handlers =
            fixpoint init_avail_at_top_of_handlers
          in
          List.iter
            (fun (nfail, _handler) -> Hashtbl.remove avail_at_exit nfail)
            handlers;
          let avail_after =
            List.fold_left
      ...
      | Iexit nfail ->
          ...
          let avail_at_top_of_handler =
            match Hashtbl.find avail_at_exit nfail with
            | exception Not_found ->
                (* also see top of [Icatch] clause above *)
