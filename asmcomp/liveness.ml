  ...
  | Iop op ->
    ...
    then
      ...
      let across =
        ...
        if operation_can_raise op then Reg.Set.union across1 exn else across1
      in
      (i.live <- across;
       Reg.add_set_array across i.arg)
  | Iifthenelse _ | Iswitch _ ->
    i.live <- next;
...
let fundecl f =
  let (initially_live, _) = Analyzer.analyze ~exnhandler ~transfer f.fun_body in
  (* Sanity check: only function parameters can be live at entrypoint *)
  let wrong_live = Reg.Set.diff initially_live (Reg.set_of_array f.fun_args) in
  if not (Reg.Set.is_empty wrong_live) then
    Misc.fatal_errorf "@[Liveness.fundecl:@\n%a@]" Printmach.regset wrong_live
