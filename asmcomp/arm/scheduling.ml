...
class scheduler =
  object
    ...
    method oper_latency =
      ...
      | Iconst_symbol _ | Iconst_float _
      | Iload (_, _, _)
      | Ireload | Ifloatofint (* mcr/mrc count as memory access *)
      | Iintoffloat ->
          2
      (* Multiplys have a latency of two cycles *)
      | Iintop (Imul | Imulh) | Ispecific (Imuladd | Imulsub | Imulhadd) -> 2
      (* VFP instructions *)
      | Iaddf | Isubf | Idivf | Imulf
      | Ispecific Inegmulf
    ...
    method! is_checkbound =
      function
      | Ispecific (Ishiftcheckbound _) -> true | op -> super#is_checkbound op
    ...
    method oper_issue_cycles =
      ...
      | Iintop (Icomp _) | Iintop_imm (Icomp _, _) -> 3
      | Iintop Icheckbound | Iintop_imm (Icheckbound, _) -> 2
      | Ispecific (Ishiftcheckbound _) -> 3
      | Iintop (Imul | Imulh) | Ispecific (Imuladd | Imulsub | Imulhadd) -> 2
      (* VFP instructions *)
      | Iaddf | Isubf -> 7
      | Imulf | Ispecific Inegmulf -> 9
      | Ispecific (Imuladdf | Inegmuladdf | Imulsubf | Inegmulsubf) -> 17
      | Idivf | Ispecific Isqrtf -> 27
