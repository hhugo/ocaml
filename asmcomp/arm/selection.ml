...
let is_offset chunk n =
  ...
  | (Single | Double | Double_u) when !fpu >= VFPv2 ->
  ...
  | _ when !arch > ARMv6 && !thumb -> n >= -255 && n <= 4095
...
let pseudoregs_for_operation op arg res =
  ...
  | Iintop Imulh when !arch < ARMv6 -> [|r7; arg.(1)|], [|r6|]
  (* Soft-float Iabsf and Inegf: arg.(0) and res.(0) must be the same *)
  | (Iabsf | Inegf) when !fpu = Soft -> [|res.(0); arg.(1)|], res
  ...
  | Iextcall {func = "__aeabi_idivmod"; alloc = false} -> arg, [|r1|]
...
class selector =
  object (self)
      ...
      Reg.createv
        ...
        then
          ...
          let rec expand = function
            ...
            | ty :: tyl -> ty :: expand tyl
          in
          Array.of_list (expand (Array.to_list tyv))
        else tyv )

    method is_immediate n = is_immediate (Int32.of_int n)
    ...
    method select_shift_arith op dbg arithop arithrevop args =
      match args with
      | [ arg1
        ; Cop (((Clsl | Clsr | Casr) as op), [arg2; Cconst_int (n, _)], _)
        ]
        when n > 0 && n < 32 ->
          ( Ispecific (Ishiftarith (arithop, select_shiftop op, n))
          , [arg1; arg2] )
      | [ Cop (((Clsl | Clsr | Casr) as op), [arg1; Cconst_int (n, _)], _)
        ; arg2 ]
        when n > 0 && n < 32 ->
          ( Ispecific (Ishiftarith (arithrevop, select_shiftop op, n))
          , [arg2; arg1] )
      | args -> (
        match super#select_operation op args dbg with
        (* Recognize multiply high and add *)
        | ( Iintop Iadd, [Cop (Cmulhi, args, _); arg3]
          | Iintop Iadd, [arg3; Cop (Cmulhi, args, _)] ) as op_args
          when !arch >= ARMv6 -> (
          match self#select_operation Cmulhi args dbg with
        ...
        | ( Iintop Iadd, [Cop (Cmuli, args, _); arg3]
          ...
          match self#select_operation Cmuli args dbg with
    ...
    method! select_operation op args dbg =
      ...
      | Csubi, [arg; Cconst_int (n, _)] when n < 0 && self#is_immediate (-n)
        ->
          Iintop_imm (Iadd, -n), [arg]
      | Csubi, [Cconst_int (n, _); arg] when self#is_immediate n ->
      ...
      | Cdivi, args -> self#iextcall ("__aeabi_idiv", false), args
      | Cmodi, args ->
      ...
      | op, args when !fpu = Soft ->
          self#select_operation_softfp op args dbg
    ...
    method private select_operation_softfp op args dbg =
      ...
      | Cstore (Single, init), [arg1; arg2] ->
          let arg2' =
            Cop (Cextcall ("__aeabi_d2f", typ_int, false, None), [arg2], dbg)
          in
    ...
    method private select_operation_vfpv3 op args dbg =
      ...
      | Cnegf, [Cop (Cmulf, args, _)] -> Ispecific Inegmulf, args
      ...
      | Cextcall ("sqrt", _, false, _), args -> Ispecific Isqrtf, args
      (* Other operations are regular *)
      | op, args -> super#select_operation op args dbg
