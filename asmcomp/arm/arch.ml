...
let print_specific_operation printreg op ppf arg =
  ...
  | Ishiftarith (op, shiftop, amount) ->
    let (op1_name, op2_name) =
      ...
      | Ishiftxor -> "", "^"
    in
    fprintf ppf "%s%a %s (%a %s %i)" op1_name printreg arg.(0) op2_name printreg
      arg.(1) (shiftop_name shiftop) amount
  | Ishiftcheckbound (shiftop, amount) ->
    fprintf ppf "check (%a %s %i) > %a" printreg arg.(0) (shiftop_name shiftop)
      amount printreg arg.(1)
