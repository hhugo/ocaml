...
and arith_operation = Ishiftadd | Ishiftsub
...
let print_addressing printreg addr ppf arg =
  ...
  | Ibased (s, 0) -> fprintf ppf "\"%s\"" s
  | Ibased (s, n) -> fprintf ppf "\"%s\" + %i" s n

let print_specific_operation printreg op ppf arg =
  match op with
  | Ifar_poll _ -> fprintf ppf "(far) poll"
  | Ifar_alloc { bytes } -> fprintf ppf "(far) alloc %i" bytes
  | Ifar_intop_checkbound ->
  ...
  | Ishiftarith (op, shift) ->
    ...
    in
    let shift_mark =
      if shift >= 0 then sprintf "<< %i" shift else sprintf ">> %i" ~-shift
    in
    fprintf ppf "%a %s %a %s" printreg arg.(0) (op_name op) printreg arg.(1)
      shift_mark
  | Ishiftcheckbound { shift } ->
    fprintf ppf "check %a >> %i > %a" printreg arg.(0) shift printreg arg.(1)
  | Ifar_shiftcheckbound { shift } ->
    fprintf ppf "(far) check %a >> %i > %a" printreg arg.(0) shift printreg
      arg.(1)
  | Imuladd ->
  ...
  | Imove32 -> fprintf ppf "move32 %a" printreg arg.(0)
  | Isignext n -> fprintf ppf "signext%d %a" n printreg arg.(0)
...
let auto_table =
  ...
  (* state 0 *)
  [| false, 1, 4; (* state 1 *) false, 1, 2; (* state 2 *) true, 3, 2;
  (* state 3 *) true, 3, 7; (* state 4 *) false, 5, 4; (* state 5 *) true, 5, 6;
  (* state 6 *) true, 7, 6; (* state 7 *) false, 7, 7 |]
...
let logical_imm_length x =
  ...
  if not (test 32) then
    64
  else if not (test 16) then
    32
  else if not (test 8) then
    16
  else if not (test 4) then
    8
  else if not (test 2) then
    4
  else
    2
