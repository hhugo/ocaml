...
let is_logical_immediate n = Arch.is_logical_immediate (Nativeint.of_int n)

(* Signed immediates are simpler *)
let is_immediate n =
  let mn = ~-n in
  n land 0xFFF = n ||
    n land 0xFFF_000 = n || mn land 0xFFF = mn || mn land 0xFFF_000 = mn
  ...
  object (self)
    ...
    method! select_operation op args dbg =
      ...
      | Caddi | Caddv | Cadda ->
        ...
        | [ arg1; Cop (Casr, [ arg2; Cconst_int (n, _) ], _) ]
          when n > 0 && n < 64 ->
          Ispecific (Ishiftarith (Ishiftadd, ~-n)), [ arg1; arg2 ]
        | [ Cop (Clsl, [ arg1; Cconst_int (n, _) ], _); arg2 ]
          when n > 0 && n < 64 ->
          Ispecific (Ishiftarith (Ishiftadd, n)), [ arg2; arg1 ]
        | [ Cop (Casr, [ arg1; Cconst_int (n, _) ], _); arg2 ]
          when n > 0 && n < 64 ->
          Ispecific (Ishiftarith (Ishiftadd, ~-n)), [ arg2; arg1 ]
        (* Multiply-add *)
        | [ arg1; Cop (Cmuli, args2, dbg) ]
        | [ Cop (Cmuli, args2, dbg); arg1 ]
          ->
      ...
      | Ccheckbound ->
        begin match args with
        | [ Cop (Clsr, [ arg1; Cconst_int (n, _) ], _); arg2 ]
          when n > 0 && n < 64 ->
          Ispecific (Ishiftcheckbound { shift = n }), [ arg1; arg2 ]
        | _ -> super#select_operation op args dbg
        end
      ...
      | Csubf ->
        begin match args with
        | [ arg; Cop (Cmulf, args, _) ] -> Ispecific Imulsubf, arg :: args
        | [ Cop (Cmulf, args, _); arg ] -> Ispecific Inegmulsubf, arg :: args
        | _ -> super#select_operation op args dbg
        end
      (* Recognize floating-point square root *)
      | Cextcall ("sqrt", _, _, _) -> Ispecific Isqrtf, args
      (* Recognize bswap instructions *)
      | Cextcall ("caml_bswap16_direct", _, _, _) -> Ispecific (Ibswap 16), args
      | Cextcall ("caml_int32_direct_bswap", _, _, _) ->
        Ispecific (Ibswap 32), args
      | Cextcall
          (("caml_int64_direct_bswap" | "caml_nativeint_direct_bswap"), _, _, _)
        ->
    ...
    method! insert_move_extcall_arg env ty_arg src dst =
      if macosx && ty_arg = XInt32 && is_stack_slot dst then
