...
let is_logical_immediate n = n <> 0 && n <> -1 && run_automata 64 0 n
...
let use_direct_addressing _symb = not !Clflags.dlcode
    ...
    method! select_operation op args dbg =
      ...
      | Caddi | Caddv | Cadda -> (
        ...
        | [arg; Cconst_int (n, _)] when self#is_immediate n ->
            ( ( if n >= 0
              then Iintop_imm (Iadd, n)
              else Iintop_imm (Isub, -n) )
            , [arg] )
      ...
      | Csubi -> (
        ...
        | [arg; Cconst_int (n, _)] when self#is_immediate n ->
            ( ( if n >= 0
              then Iintop_imm (Isub, n)
              else Iintop_imm (Iadd, -n) )
            , [arg] )
        (* Shift-sub *)
        | [arg1; Cop (Clsl, [arg2; Cconst_int (n, _)], _)]
          when n > 0 && n < 64 ->
      ...
      | Ccheckbound -> (
        ...
        | [Cop (Clsr, [arg1; Cconst_int (n, _)], _); arg2]
          when n > 0 && n < 64 ->
            ( Ispecific
                (Ishiftcheckbound {shift = n; label_after_error = None})
            , [arg1; arg2] )
        | _ -> super#select_operation op args dbg )
      (* Integer multiplication *)
      (* ARM does not support immediate operands for multiplication *)
      | Cmuli -> Iintop Imul, args
