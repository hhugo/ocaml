...
(* Debug *)
  let dbg = false
  let mask = let open Nativeint in sub (shift_left one 8) one
    ...
    List.iter
      (fun (ps, _) ->
         Printf.fprintf chan "  [%s]\n"
           (String.concat "; " (List.map pat_as_string ps))) cases
  ...
  let pat_of_string str =
    ...
    let get_byte i =
      if i < len then
        int_of_char str.[i]
      else if i < n * Arch.size_addr - 1 then
        0
      else
        n * Arch.size_addr - 1 - len
    in
    let mk_word ind =
      let w = ref 0n in
      let imin = ind * Arch.size_addr
      and imax = (ind + 1) * Arch.size_addr - 1
      in
      (if Arch.big_endian then
         for i = imin to imax do
           w :=
             Nativeint.logor (Nativeint.shift_left !w 8)
    ...
    let rec mk_words ind =
      if ind >= n then [] else mk_word ind :: mk_words (ind + 1)
    in
    mk_words 0
  (*****************************)
  ...
  let count_arities cases =
    ...
    | (ps, _) :: _ ->
      let sets =
        List.fold_left (fun sets (ps, _) -> add_one sets ps)
          (List.map (fun _ -> NativeSet.empty) ps) cases
      in
    ...
    let set =
      List.fold_left
        (fun set case ->
           ...
           | _ -> assert false) NativeSet.empty cases
    in
    NativeSet.cardinal set
  
  let count_arities_length cases =
    let set =
      List.fold_left (fun set (ps, _) -> IntSet.add (List.length ps) set)
        IntSet.empty cases
    in
    IntSet.cardinal set
  
  let best_col =
    let rec do_rec kbest best k =
      function
      | [] -> kbest
      | x :: xs ->
  ...
  let swap k idxs cases =
    if k = 0 then
      ...
      and cases = List.map (fun (ps, act) -> swap_list k ps, act) cases
      in
        ...
        List.fold_left
          (fun env (p, psact) ->
             let old =
               ...
               | Not_found -> []
             in
             OMap.add p (psact :: old) env) OMap.empty cases
      in
  ...
  let match_oncell compile_rec str default idx env =
    ...
    let rec comp_rec env =
      ...
      if len <= 3 then
        List.fold_right
          (fun (key, cases) ifnot ->
             mk_eq id key (compile_rec str default cases) ifnot) env default
      else
 ...
 *)
  let rec do_compile_pats idxs str default cases =
    ...
    | _ :: _ ->
      ...
      | [] -> assert false
      | idx :: idxs ->
        match_oncell (do_compile_pats idxs) str default idx (by_cell cases)
      end
  ...
  let by_size cases =
    DivideInt.divide
      (List.map (fun (ps, _ as case) -> List.length ps, case) cases)
    ...
    let size_cases =
      List.map
        (fun (len, cases) ->
           ...
           let act = do_compile_pats (interval from_ind len) str default cases
           in
           len, act) (by_size cases)
    in
 ...
 *)
  let top_compile debuginfo str default cases =
    ...
    and a_fst = count_arities_first cases
    in
    if a_len <= a_fst then
