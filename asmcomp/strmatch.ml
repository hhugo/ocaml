...
module Make (I : I) = struct
  ...
  let best_col =
    let rec do_rec kbest best k = function
      ...
      | x :: xs ->
          if x < best
          then do_rec k x (k + 1) xs
          else do_rec kbest best (k + 1) xs
    in
  ...
  let match_oncell compile_rec str default idx env =
    ...
    let rec comp_rec env =
      ...
      then
        List.fold_right
          (fun (key, cases) ifnot ->
            mk_eq id key (compile_rec str default cases) ifnot )
          env
          default
      else
 ...
 *)
  ...
  let compile_by_size dbg from_ind str default cases =
    let size_cases =
      List.map
        (fun (len, cases) ->
          ...
          let act =
            do_compile_pats (interval from_ind len) str default cases
          in
          len, act )
        (by_size cases)
    in
    ...
    let switch =
      I.transl_switch loc (Cvar id) 1 max_int size_cases default
    in
