...
let for_primitive (prim : Clambda_primitives.primitive) =
  match prim with
  | Pmakeblock _ | Pmakearray (_, Mutable) ->
  ...
  | Pnot | Pnegint | Paddint | Psubint | Pmulint | Pandint | Porint
   |Pxorint | Plslint | Plsrint | Pasrint | Pintcomp _ ->
      No_effects, No_coeffects
  ...
  | Pintoffloat | Pfloatofint | Pnegfloat | Pabsfloat | Paddfloat
   |Psubfloat | Pmulfloat | Pdivfloat | Pfloatcomp _ ->
      No_effects, No_coeffects
  | Pstringlength | Pbyteslength | Parraylength _ ->
      No_effects, Has_coeffects (* That old chestnut: [Obj.truncate]. *)
  | Pisint | Pisout | Pbintofint _ | Pintofbint _ | Pcvtbint _
   |Pnegbint _ | Paddbint _ | Psubbint _ | Pmulbint _ | Pandbint _
   |Porbint _ | Pxorbint _ | Plslbint _ | Plsrbint _ | Pasrbint _
   |Pbintcomp _ ->
      No_effects, No_coeffects
  | Pbigarraydim _ ->
      No_effects, Has_coeffects (* Some people resize bigarrays in place. *)
  | Pread_symbol _ | Pfield _ | Pfield_computed | Pfloatfield _
   |Parrayrefu _ | Pstringrefu | Pbytesrefu
  ...
  | Psetfield _ | Psetfield_computed _ | Psetfloatfield _ | Parraysetu _
   |Parraysets _ | Pbytessetu | Pbytessets | Pbytes_set _ | Pbigarrayset _
   |Pbigstring_set _ ->
      (* Whether or not some of these are "unsafe" is irrelevant; they always
         have an effect. *)
      Arbitrary_effects, No_coeffects
  | Pbswap16 | Pbbswap _ -> No_effects, No_coeffects
...
let return_type_of_primitive (prim : Clambda_primitives.primitive) =
  ...
  | Pfloatofint | Pnegfloat | Pabsfloat | Paddfloat | Psubfloat
   |Pmulfloat | Pdivfloat | Pfloatfield _
