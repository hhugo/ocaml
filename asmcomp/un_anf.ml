...
let closure_environment_var (ufunction : Clambda.ufunction) =
  ...
  if List.length ufunction.params = ufunction.arity + 1
  then (
...
let make_var_info (clam : Clambda.ulambda) : var_info =
  ...
  let rec loop : Clambda.ulambda -> unit = function
    ...
    | Ugeneric_apply (func, args, dbg) ->
        loop func; List.iter loop args; ignore_debuginfo dbg
    | Uclosure (functions, captured_variables) ->
        List.iter loop captured_variables;
        List.iter
          (fun ( {Clambda.label; arity; params; return; body; dbg; env} as
               clos ) ->
          ...
          functions
    | Uoffset (expr, offset) -> loop expr; ignore_int offset
    | Ulet (_let_kind, _value_kind, _var, def, body) -> loop def; loop body
    ...
    | Uletrec (defs, body) ->
        ...
        loop body
    | Uprim (prim, args, dbg) ->
        ignore_primitive prim; List.iter loop args; ignore_debuginfo dbg
    | Uswitch
        ( cond
        , { us_index_consts
          ; us_actions_consts
    ...
    | Ustringswitch (cond, branches, default) ->
        loop cond;
        List.iter
          (fun (str, branch) -> ignore_string str; loop branch)
          branches;
        Misc.may loop default
    | Ustaticfail (static_exn, args) ->
        ignore_int static_exn; List.iter loop args
    ...
    | Utrywith (body, var, handler) ->
        ...
        ignore_var_with_provenance var;
        loop handler
    | Uifthenelse (cond, ifso, ifnot) -> loop cond; loop ifso; loop ifnot
    | Usequence (e1, e2) -> loop e1; loop e2
    | Uwhile (cond, body) -> loop cond; loop body
  ...
  let used =
    ...
    V.Tbl.fold (fun var _n acc -> V.Set.add var acc) t assigned
  in
  { used
  ; linear
  ; assigned
...
let let_bound_vars_that_can_be_moved var_info (clam : Clambda.ulambda) =
  ...
  let rec loop : Clambda.ulambda -> unit = function
    ...
    | Ulet (_let_kind, _value_kind, var, def, body) -> (
        ...
        | _ ->
            loop def;
            if V.Set.mem var var_info.linear
            then let_stack := var :: !let_stack
            else
...
let rec substitute_let_moveable is_let_moveable env (clam : Clambda.ulambda)
  ...
  match clam with
  | Uvar var -> (
      if not (V.Set.mem var is_let_moveable)
      then clam
      else
        match V.Map.find var env with
        | clam -> clam
        | exception Not_found ->
            Misc.fatal_errorf
              "substitute_let_moveable: Unbound variable %a"
              V.print
              var )
  | Uconst _ -> clam
  | Udirect_apply (label, args, dbg) ->
      ...
      let functions =
        List.map
          (fun (ufunction : Clambda.ufunction) ->
            { ufunction with
              body =
                substitute_let_moveable is_let_moveable env ufunction.body
            } )
          functions
      in
      let variables_bound_by_the_closure =
        substitute_let_moveable_list
          is_let_moveable
          env
  ...
  | Ulet (let_kind, value_kind, var, def, body) ->
      ...
      if V.Set.mem (VP.var var) is_let_moveable
      then
...
let primitive_moveable (prim : Clambda_primitives.primitive)
    (args : Clambda.ulambda list) (var_info : var_info) =
...
let rec un_anf_and_moveable var_info env (clam : Clambda.ulambda) :
  ...
  | Ulet (let_kind, value_kind, var, def, body) -> (
      ...
      let maybe_for_debugger (body, moveable) : Clambda.ulambda * moveable =
        ...
        then body, moveable
          ...
          | Uvar alias_of ->
              ( Uphantom_let
                  (var, Some (Clambda.Uphantom_var alias_of), body)
              , moveable )
          | _ -> Uphantom_let (var, None, body), moveable
      in
      match def_moveable, is_linear, is_used, is_assigned with
  ...
  | Uprim (prim, args, dbg) ->
      let args, args_moveable =
        un_anf_list_and_moveable var_info env args
      in
      let moveable =
