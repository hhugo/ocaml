...
let node_num_header_words = 2

(* [Node_num_header_words] in the runtime. *)
...
let next_index_within_node ~part_of_shape ~label =
  ...
  | Mach.Direct_call_point _ ->
      ...
      if Config.enable_call_counts then incr index_within_node
  | Mach.Indirect_call_point -> incr index_within_node
  | Mach.Allocation_point ->
      incr index_within_node; incr index_within_node; incr index_within_node
  );
        ...
        (fun init_code index ->
          ...
          Csequence
            ( Cop
                ...
                , [ Cop
                      ( Caddi
                      , [Cvar new_node; cconst_int offset_in_bytes]
                      , dbg )
  ...
  Clet
    ...
    , Clet
        ...
        , Cifthenelse
            ...
            , dbg
            , Clet
                ( VP.create is_new_node
                , Clet
                    ( VP.create pc
                    , cconst_symbol function_name
                    , Cop
                        ( Cextcall
                            ( "caml_spacetime_allocate_node"
                            , [|Int|]
                            , false
                            , None )
...
let code_for_blockheader ~value's_header ~node ~dbg =
  ...
  let generate_new_profinfo =
    ...
    *)
    Cop
      ( Cextcall
          ("caml_spacetime_generate_profinfo", [|Int|], false, Some label)
      , [Cvar address_of_profinfo; cconst_int (index_within_node + 1)]
      , dbg )
  in
  (* Check if we have already allocated a profinfo value for this allocation
     ...
     allocate a new one. *)
  Clet
    ( VP.create address_of_profinfo
    , Cop (Caddi, [Cvar node; cconst_int offset_into_node], dbg)
    , Clet
        ( VP.create existing_profinfo
        , Cop
            ( Cload (Word_int, Asttypes.Mutable)
        ...
        , Clet
            ...
            , Cifthenelse
                ( Cop
                    ( Ccmpi Cne
                    , [Cvar existing_profinfo; cconst_int 1 (* () *)]
                    , dbg )
                , dbg
                ...
                , Csequence
                    ( Cop
                     ...
                     The "1 +" is to count the value's header. *)
                                  cconst_int
                                    (2 * (1 + Nativeint.to_int num_words))
                                ]
                              , dbg ) ]
                        , dbg )
               ...
               header, we can use an XOR trick, to keep code size down. *)
                      let value's_header =
                        Nativeint.logxor
                          ...
                          (Nativeint.logor
                             (Nativeint.logor
                                (Nativeint.of_int Obj.infix_tag)
                                (Nativeint.shift_left
                                   3n
                                   (* <- Caml_black *) 8))
                             (Nativeint.shift_left
                                (* The following is the [Infix_offset_val], in words. *)
                                (Nativeint.of_int (index_within_node + 1))
                                10))
                      in
                      Cop
                        ( Cxor
                        , [Cvar profinfo; cconst_natint value's_header]
                        , dbg ) ) ) ) ) )
...
let code_for_call ~node ~callee ~is_tail ~label dbg =
  ...
  let is_self_recursive_call =
    match callee with
    | Direct callee -> (
      match !current_function_label with
  ...
  let index_within_node =
    ...
    | Indirect _ ->
        next_index_within_node
          ~part_of_shape:Mach.Indirect_call_point
          ~label
  in
  ( match callee with
  ...
  let open Cmm in
  Clet
    ( VP.create place_within_node
    , Cop
        (Caddi, [node; cconst_int (index_within_node * Arch.size_addr)], dbg)
    , (* The following code returns the address that is to be moved into the
      ...
      match callee with
      | Direct _callee ->
          if Config.enable_call_counts
          then
            let count_addr = V.create_local "call_count_addr" in
            let count = V.create_local "call_count" in
            Clet
              ( VP.create count_addr
              , Cop
                  ( Caddi
                  , [Cvar place_within_node; cconst_int Arch.size_addr]
                  , dbg )
              , Clet
                  ( VP.create count
                  , Cop
                  ...
                  , Csequence
                   ...
                   as an OCaml integer. *)
                            [ Cvar count_addr
                            ; Cop (Caddi, [Cvar count; cconst_int 2], dbg)
                            ]
                          , dbg )
                      , Cvar place_within_node ) ) )
          else Cvar place_within_node
      | Indirect callee ->
          let caller_node =
            if is_tail then node else cconst_int 1
            (* [Val_unit] *)
          in
          Cop
            ( Cextcall
...
class virtual instruction_selection =
  object (self)
    ...
    method private emit_prologue f ~node_hole ~env =
      ...
      if something_was_instrumented ()
      then (
        let prologue_cmm =
          code_for_function_prologue
            ~function_name:f.Cmm.fun_name
            ~node_hole
        ...
        let node_temp_reg =
          ...
          | None ->
              Misc.fatal_error
                "Spacetime prologue instruction selection did not yield a \
    ...
    method! emit_blockheader env n dbg =
      ...
      then (
        disable_instrumentation <- true;
        let result =
          self#instrument_blockheader ~env ~value's_header:n ~dbg
        in
        disable_instrumentation <- false;
        result )
      else super#emit_blockheader env n dbg
    ...
    method! select_allocation bytes =
      ...
      then
        ...
        let index =
          next_index_within_node
            ~part_of_shape:
              (Mach.Direct_call_point {callee = "caml_call_gc"})
            ~label
        in
        Mach.Ialloc
          {bytes; label_after_call_gc = Some label; spacetime_index = index}
      else super#select_allocation bytes

    method! select_allocation_args env =
      if self#can_instrument ()
      then
        let regs =
          Selectgen.env_find (Lazy.force !spacetime_node_ident) env
        in
        match regs with
        | [|reg|] -> [|reg|]
        | _ ->
            failwith "Expected one register only for spacetime_node_ident"
      else super#select_allocation_args env
    ...
    method! select_checkbound () =
      ...
      then
        ...
        in
        Mach.Icheckbound
          {label_after_error = Some label; spacetime_index = index}
      else super#select_checkbound ()
    ...
    method! select_checkbound_extra_args () =
      ...
      else super#select_checkbound_extra_args ()
    ...
    method! initial_env () =
      ...
      then
        Selectgen.env_add
          (VP.create (Lazy.force !spacetime_node_ident))
          (self#regs_for Cmm.typ_int)
          env
      else env
