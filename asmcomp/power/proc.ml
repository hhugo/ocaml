  ...
  for i = 0 to Array.length arg - 1 do
    ...
    | [|arg1; arg2|] -> (
        ...
        | _, _ ->
            ...
            fatal_error
              (Printf.sprintf
                 "Proc.calling_conventions: bad register type(s) for \
                  multi-register argument: %s, %s"
                 (f arg1.typ)
                 (f arg2.typ)) )
    | _ ->
        fatal_error
          "Proc.calling_conventions: bad number of registers for \
...
let ensure_single_regs res =
  Array.map
    (function [|res|] -> res | _ -> failwith "Proc.ensure_single_regs")
    res
...
let loc_external_arguments =
  match abi with
  | ELF32 -> calling_conventions 0 7 100 107 outgoing 8 false
  | ELF64v1 ->
      fun args ->
        let loc, ofs =
          calling_conventions 0 7 100 112 outgoing 0 true args
        in
        loc, max ofs 64
  | ELF64v2 ->
      fun args ->
...
let safe_register_pressure = function Iextcall _ -> 15 | _ -> 23
