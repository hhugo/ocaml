...
let rec select_addr =
  function
  | Cconst_symbol (s, _) -> Asymbol s, 0, Debuginfo.none
  | Cop ((Caddi | Caddv | Cadda), [ arg; Cconst_int (m, _) ], dbg) ->
    let (a, n, _) = select_addr arg in
    a, n + m, dbg
  ...
  | Cop ((Caddi | Caddv | Cadda), [ arg1; arg2 ], dbg) ->
    ...
    | (Alinear e1, n1, _), (Alinear e2, n2, _) -> Aadd (e1, e2), n1 + n2, dbg
    | _ -> Aadd (arg1, arg2), 0, dbg
    end
  | exp -> Alinear exp, 0, Debuginfo.none
...
class selector =
  object (self)
    ...
    method select_addressing _chunk exp =
      ...
      | Asymbol s, d, _dbg -> Ibased (s, d), Ctuple []
      | Alinear e, d, _dbg -> Iindexed d, e
      | Aadd (e1, e2), d, dbg ->
        if d = 0 then
          Iindexed2, Ctuple [ e1; e2 ]
        else
          Iindexed d, Cop (Cadda, [ e1; e2 ], dbg)
