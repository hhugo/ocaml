...
let get_fun_offset t closure_id =
  ...
  with Not_found ->
    Misc.fatal_errorf
      "Flambda_to_clambda: missing offset for closure %a"
      Closure_id.print
      closure_id

let get_fv_offset t var_within_closure =
  let fv_offset_table =
    if Var_within_closure.in_compilation_unit
         var_within_closure
  ...
  with Not_found ->
    Misc.fatal_errorf
      "Flambda_to_clambda: missing offset for variable %a"
      Var_within_closure.print
      var_within_closure

let is_function_constant t closure_id =
  if Closure_id.Set.mem closure_id t.current_unit.closures
  then Closure_id.Set.mem closure_id t.current_unit.constant_closures
  else if Closure_id.Set.mem closure_id t.imported_units.closures
  then Closure_id.Set.mem closure_id t.imported_units.constant_closures
  else
    Misc.fatal_errorf
      "Flambda_to_clambda: missing closure %a"
      Closure_id.print
      closure_id
...
let check_closure ulam named : Clambda.ulambda =
  if not !Clflags.clambda_checks
  then ulam
  else
    let desc =
      Primitive.simple
        ~name:"caml_check_value_is_closure"
        ~arity:2
        ~alloc:false
    in
...
let subst_var env var : Clambda.ulambda =
  ...
  with Not_found -> (
    ...
    with Not_found ->
      Misc.fatal_errorf
        "Flambda_to_clambda: unbound variable %a@."
        Variable.print
        var )
...
let to_uconst_symbol env symbol : Clambda.ustructured_constant option =
  ...
  | None (* CR-soon mshinwell: Try to make this an error. *) | Some _ ->
      None
...
let to_clambda_const env
    (const : Flambda.constant_defining_value_block_field) :
    Clambda.uconstant =
  match const with
...
let rec to_clambda t env (flam : Flambda.t) : Clambda.ulambda =
  ...
  | Apply {func; args; kind = Indirect; dbg} ->
      ...
      Ugeneric_apply
        ( check_closure callee (Flambda.Expr (Var func))
        , subst_vars env args
        , dbg )
  | Switch (arg, sw) -> (
  ...
  | Assign {being_assigned; new_value} ->
      let id =
        ...
        with Not_found ->
          Misc.fatal_errorf
            "Unbound mutable variable %a in [Assign]: %a"
...
and to_clambda_named t env var (named : Flambda.named) : Clambda.ulambda =
  ...
  | Read_mutable mut_var -> (
    ...
    with Not_found ->
      Misc.fatal_errorf
        "Unbound mutable variable %a in [Read_mutable]: %a"
        Mutable_variable.print
        mut_var
        Flambda.print_named
        named )
  ...
  | Project_var {closure; var; closure_id} ->
      ...
      Uprim
        ( Pfield pos
        , [ check_field
              (check_closure ulam (Expr (Var closure)))
              pos
              (Some named) ]
        , Debuginfo.none )
  | Prim (Pfield index, [block], dbg) ->
      Uprim
        (Pfield index, [check_field (subst_var env block) index None], dbg)
...
and to_clambda_switch t env cases num_keys default =
  let num_keys =
    if Numbers.Int.Set.cardinal num_keys = 0
    then 0
    ...
    let env =
      ...
      let add_env_free_variable id _ env =
        let var_offset =
          ...
          with Not_found ->
            Misc.fatal_errorf
              "Clambda.to_clambda_set_of_closures: offset for free \
               variable %a is unknown.  Set of closures: %a"
              Variable.print
              id
...
let accumulate_structured_constants t env symbol
  ...
  | Block (tag, fields) ->
      ...
      Symbol.Map.add
        symbol
        (Clambda.Uconst_block (Tag.to_int tag, fields))
        acc
  | Set_of_closures set_of_closures ->
  ...
  let rec loop env constants (program : Flambda.program_body) :
    ...
    | Initialize_symbol (symbol, tag, fields, program) ->
        ...
        let preallocated_block : Clambda.preallocated_block =
          ...
          ; provenance = None }
        in
        let e2, constants, preallocated_blocks =
          loop env constants program
        in
        ( Usequence (e1, e2)
        , constants
    ...
    | End _ -> Uconst (Uconst_ptr 0), constants, []
  in
