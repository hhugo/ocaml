...
let rec repres_reg r =
  try repres_reg (Reg.Map.find r !equiv_classes)
  with
  | Not_found -> r
...
let identify r1 r2 =
  ...
  if repres1.stamp = repres2.stamp then
    ()
  else
...
let rec rename i sub =
  ...
  | Ireturn | Iop Itailcall_ind | Iop (Itailcall_imm _) ->
    instr_cons_debug i.desc (subst_regs i.arg sub) [| |] i.dbg i.next, None
  | Iop Ireload when i.res.(0).loc = Unknown ->
    ...
    | Some s ->
      ...
      let (new_next, sub_next) = rename i.next (Some (Reg.Map.add oldr newr s))
      in
      instr_cons i.desc i.arg [| newr |] new_next, sub_next
    end
  | Iop _ ->
    let (new_next, sub_next) = rename i.next sub in
    instr_cons_debug i.desc (subst_regs i.arg sub) (subst_regs i.res sub) i.dbg
      new_next,
    sub_next
  | Iifthenelse (tst, ifso, ifnot) ->
    ...
    in
    instr_cons (Iifthenelse (tst, new_ifso, new_ifnot)) (subst_regs i.arg sub)
      [| |] new_next,
    sub_next
  | Iswitch (index, cases) ->
    let new_sub_cases = Array.map (fun c -> rename c sub) cases in
    let sub_merge =
      merge_subst_array (Array.map (fun (_n, s) -> s) new_sub_cases) i.next
    in
  ...
  | Icatch (rec_flag, handlers, body) ->
    let new_subst = List.map (fun (nfail, _) -> nfail, ref None) handlers in
    ...
    let merged_subst =
      List.fold_left
        (fun acc (_, sub_handler) -> merge_substs acc sub_handler i.next)
        sub_body res
    in
    let (new_next, sub_next) = rename i.next merged_subst in
    let new_handlers =
      List.map2 (fun (nfail, _) (handler, _) -> nfail, handler) handlers res
    in
    instr_cons (Icatch (rec_flag, new_handlers, new_body)) [| |] [| |] new_next,
    sub_next
  | Iexit nfail ->
  ...
  | Itrywith (body, handler) ->
    ...
    in
    instr_cons (Itrywith (new_body, new_handler)) [| |] [| |] new_next, sub_next
  | Iraise k ->
    instr_cons_debug (Iraise k) (subst_regs i.arg sub) [| |] i.dbg i.next, None
