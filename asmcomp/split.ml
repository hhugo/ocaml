...
let subst_reg r (sub : subst) = try Reg.Map.find r sub with Not_found -> r
...
let rec repres_reg r =
  try repres_reg (Reg.Map.find r !equiv_classes) with Not_found -> r

let repres_regs rv =
  let n = Array.length rv in
  for i = 0 to n - 1 do
    rv.(i) <- repres_reg rv.(i)
  done
...
let identify r1 r2 =
  ...
  else equiv_classes := Reg.Map.add repres1 repres2 !equiv_classes
...
let identify_sub sub1 sub2 reg =
  try
    ...
    try
      ...
      identify r1 r2
    with Not_found -> identify r1 reg
  with Not_found -> (
    try
      let r2 = Reg.Map.find reg sub2 in
      identify r2 reg
...
let merge_subst_array subv instr =
  let rec find_one_subst i =
    if i >= Array.length subv then None
    else
      match subv.(i) with
...
let find_exit_subst k =
  ...
  with Not_found -> Misc.fatal_error "Split.find_exit_subst"
...
let rec rename i sub =
  ...
  | Iifthenelse (tst, ifso, ifnot) ->
      ...
      in
      ( instr_cons
          (Iifthenelse (tst, new_ifso, new_ifnot))
  ...
  | Icatch (rec_flag, handlers, body) ->
      ...
      let res =
        List.map2
          (fun (_, handler) (_, new_subst) -> rename handler !new_subst)
          handlers new_subst
      in
