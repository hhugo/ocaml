...
module Backward (D : DOMAIN) = struct
  let analyze ?(exnhandler=(fun x -> x)) ?(exnescape=D.bot) ~transfer instr =
    let lbls = (Hashtbl.create 20 : (int, D.t) Hashtbl.t) in
    let get_lbl n =
      match Hashtbl.find_opt lbls n with
      | None -> D.bot
      | Some b -> b
    and set_lbl n x = Hashtbl.replace lbls n x
    in
    let rec before end_ exn i =
      match i.desc with
      | Iend -> transfer i ~next:end_ ~exn
      ...
      | Iifthenelse (_, ifso, ifnot) ->
        ...
        and b0 = before bx exn ifnot
        in
        transfer i ~next:(D.join b1 b0) ~exn
      | Iswitch (_, cases) ->
        let bx = before end_ exn i.next in
        let b1 =
          Array.fold_left (fun accu case -> D.join accu (before bx exn case))
            D.bot cases
        in
      ...
      | Icatch (rc, handlers, body) ->
        ...
        begin match rc with
        | Cmm.Nonrecursive ->
          List.iter (fun (n, h) -> set_lbl n (before bx exn h)) handlers
        | Cmm.Recursive ->
          let update changed (n, h) =
            let b0 = get_lbl n in
            let b1 = before bx exn h in
            if D.lessequal b1 b0 then
              changed
            else
              (set_lbl n b1;
               true)
