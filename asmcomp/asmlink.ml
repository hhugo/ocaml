...
let object_file_name_of_file = function
  ...
  | Library (fname, infos) ->
      ...
      if infos.lib_units = [] && not (Sys.file_exists obj_file) then None
      else Some obj_file
...
let make_startup_file ~ppf_dump units_list ~crc_interfaces =
  ...
  else compile_phrase (Cmm_helpers.code_segment_table ("_startup" :: name_list));
  ...
  | Missing_cmx (filename, name) ->
      fprintf ppf
        "@[<hov>File %a@ was compiled without access@ to the .cmx file@ for \
         ...
         found.@]"
        Location.print_filename filename name Location.print_filename filename
        name
