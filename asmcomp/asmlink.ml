  ...
  (try
     List.iter
       (fun (name, crco) ->
          ...
          | None -> ()
          | Some crc ->
            if name = unit.ui_name then
              Cmi_consistbl.set crc_interfaces name crc file_name
            else
...
let runtime_lib () =
  ...
  with
  | Not_found -> raise (Error (File_not_found libname))
...
let object_file_name_of_file =
  ...
  | Library (fname, infos) ->
    ...
    if infos.lib_units = [] && not (Sys.file_exists obj_file) then
      None
    else
      Some obj_file
...
let read_file obj_name =
  ...
  if Filename.check_suffix file_name ".cmx" then
    ...
    let infos =
      ...
      with
      | Compilenv.Error (Not_a_unit_info _) ->
  ...
  | Library (file_name, infos) ->
    ...
    List.fold_right
      (fun (info, crc) reqd ->
         if
           info.ui_force_link ||
             !Clflags.link_everything || is_required info.ui_name
         then
           (remove_required info.ui_name;
            List.iter
              (add_required (Printf.sprintf "%s(%s)" file_name info.ui_name))
              info.ui_imports_cmx;
            (info, file_name, crc) :: reqd)
         else
           reqd) infos.lib_units tolink
  ...
  let defined =
    List.map
      (fun (unit, _, impl_crc) ->
         ...
         unit.ui_name, intf_crc, Some impl_crc, unit.ui_defines) units_list
  in
  String.Tbl.fold (fun name intf acc -> (name, intf, None, []) :: acc)
...
let make_startup_file ~ppf_dump units_list ~crc_interfaces =
  ...
  Emit.begin_assembly ();
  let name_list =
    List.flatten (List.map (fun (info, _, _) -> info.ui_defines) units_list)
  in
...
let make_shared_startup_file ~ppf_dump units =
  ...
  compile_phrase
    (Cmm_helpers.global_table (List.map (fun (ui, _) -> ui.ui_symbol) units));
  (if !Clflags.output_complete_object then force_linking_of_startup ~ppf_dump);
...
let link_shared ~ppf_dump objfiles output_name =
  Profile.record_call output_name
    (fun () ->
       ...
       in
       let startup =
         if !Clflags.keep_startup_file || !Emitaux.binary_backend_available then
           output_name ^ ".startup" ^ ext_asm
         else
           Filename.temp_file "camlstartup" ext_asm
       in
...
let call_linker file_list startup_file output_name =
  ...
  let (files, c_lib) =
    if not !Clflags.output_c_object || main_dll || main_obj_runtime then
      ...
      (if !Clflags.nopervasives || main_obj_runtime && not main_dll then
         ""
       else
...
let report_error ppf =
  ...
  | Missing_implementations l ->
    ...
    let print_modules ppf =
      List.iter
        (fun (md, rq) ->
           fprintf ppf "@ @[<hov 2>%s referenced from %a@]" md print_references
             rq)
    in
    fprintf ppf
  ...
  | Inconsistent_implementation (intf, file1, file2) ->
    fprintf ppf
      "@[<hov>Files %a@ and %a@ make inconsistent assumptions over \
       implementation %s@]" Location.print_filename file1
      Location.print_filename file2 intf
  ...
  | Multiple_definition (modname, file1, file2) ->
    fprintf ppf "@[<hov>Files %a@ and %a@ both define a module named %s@]"
      Location.print_filename file1 Location.print_filename file2 modname
  | Missing_cmx (filename, name) ->
    fprintf ppf
      "@[<hov>File %a@ was compiled without access@ to the .cmx file@ for \
       module %s,@ which was produced by `ocamlopt -for-pack'.@ Please \
       recompile %a@ with the correct `-I' option@ so that %s.cmx@ is \
       found.@]" Location.print_filename filename name Location.print_filename
      filename name
