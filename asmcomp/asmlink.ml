...
let check_consistency file_name unit crc =
  ...
  if unit.ui_symbol <> unit.ui_name
  then cmx_required := unit.ui_name :: !cmx_required
...
let runtime_lib () =
  ...
  try if !Clflags.nopervasives then [] else [Load_path.find libname]
...
let object_file_name name =
  let file_name =
    ...
    with Not_found ->
      fatal_errorf "Asmlink.object_file_name: %s not found" name
  in
  if Filename.check_suffix file_name ".cmx"
  then Filename.chop_suffix file_name ".cmx" ^ ext_obj
  else if Filename.check_suffix file_name ".cmxa"
  then Filename.chop_suffix file_name ".cmxa" ^ ext_lib
  else fatal_error "Asmlink.object_file_name: bad ext"

(* First pass: determine which units are needed *)

let missing_globals : (string, string list ref) Hashtbl.t =
  Hashtbl.create 17
...
let make_startup_file ~ppf_dump units_list ~crc_interfaces =
  ...
  if Config.spacetime
  then compile_phrase (Cmmgen.spacetime_shapes all_names);
...
let make_shared_startup_file ~ppf_dump units =
  ...
  compile_phrase
    (Cmmgen.global_table (List.map (fun (ui, _) -> ui.ui_symbol) units));
  if !Clflags.output_complete_object then force_linking_of_startup ~ppf_dump;
...
let call_linker file_list startup_file output_name =
  ...
  let libunwind =
    ...
    then []
    else if not Config.libunwind_available
    then []
...
let report_error ppf = function
  ...
  | Linking_error -> fprintf ppf "Error during linking"
  ...
  | Missing_cmx (filename, name) ->
      fprintf
        ppf
        "@[<hov>File %a@ was compiled without access@ to the .cmx file@ \
         for module %s,@ which was produced by `ocamlopt -for-pack'.@ \
         Please recompile %a@ with the correct `-I' option@ so that \
         %s.cmx@ is found.@]"
        Location.print_filename
        filename
        name
