...
let check_label n =
  match n.desc with Lbranch lbl -> lbl | Llabel lbl -> lbl | _ -> -1
...
let find_exit_label_try_depth k =
  try List.assoc k !exit_label
  with Not_found -> Misc.fatal_error "Linearize.find_exit_label"
...
let linear i n contains_calls =
  let rec linear i n =
    ...
    | Iop (Ipoll { return_label = None; _ } as op) ->
        ...
        let op, n =
          match n.desc with
          | Lbranch lbl -> (Mach.Ipoll { return_label = Some lbl }, n.next)
          | _ -> (op, n)
        in
        copy_instr (Lop op) i n
    | Iop op -> copy_instr (Lop op) i (linear i.Mach.next n)
    | Ireturn ->
        let n1 = copy_instr Lreturn i (discard_dead_code n) in
        if contains_calls then cons_instr Lreloadretaddr n1 else n1
    | Iifthenelse (test, ifso, ifnot) -> (
        let n1 = linear i.Mach.next n in
        match (ifso.Mach.desc, ifnot.Mach.desc, n1.desc) with
        | Iend, _, Lbranch lbl ->
        ...
        | Iexit nfail1, Iexit nfail2, _
          when is_next_catch nfail1 && local_exit nfail2 ->
            ...
            copy_instr (Lcondbranch (invert_test test, lbl2)) i (linear ifso n1)
        | Iexit nfail, _, _ when local_exit nfail ->
            let n2 = linear ifnot n1 and lbl = find_exit_label nfail in
        ...
        | _, Iend, _ ->
            let lbl_end, n2 = get_label n1 in
            copy_instr
              (Lcondbranch (invert_test test, lbl_end))
              i (linear ifso n2)
        | _, _, _ ->
            ...
            copy_instr
              (Lcondbranch (invert_test test, lbl_else))
              i
    ...
    | Iswitch (index, cases) ->
        ...
        if Array.length index = 3 then
          let fallthrough_lbl = check_label !n2 in
          let find_label n =
            let lbl = lbl_cases.(index.(n)) in
            if lbl = fallthrough_lbl then None else Some lbl
          in
          copy_instr
            (Lcondbranch3 (find_label 0, find_label 1, find_label 2))
            i !n2
        else
    ...
    | Icatch (_rec_flag, handlers, body) ->
        ...
        let labels_at_entry_to_handlers =
          List.map
            (fun (_nfail, handler) ->
        ...
        let n2 =
          List.fold_left2
            (fun n (_nfail, handler) lbl_handler ->
              match handler.Mach.desc with
              | Iend -> n
              | _ ->
    ...
    | Iexit nfail ->
        ...
        let rec loop i tt =
          if t = tt then i else loop (cons_instr Lpoptrap i) (tt - 1)
        in
    ...
    | Itrywith (body, handler) ->
        ...
        decr try_depth;
        n3
    | Iraise k -> copy_instr (Lraise k) i (discard_dead_code n)
  in
  linear i n
...
let add_prologue first_insn prologue_required =
  ...
  else (tailrec_entry_point_label, tailrec_entry_point)
...
let fundecl f =
  ...
  let fun_tailrec_entry_point_label, fun_body =
    add_prologue
      (linear f.Mach.fun_body end_instr contains_calls)
