...
let rec adjust_trap_depth delta_traps next =
  ...
  | _ ->
    if delta_traps = 0 then
      next
    else
      cons_instr (Ladjust_trap_depth { delta_traps }) next
...
let linear i n contains_calls =
  let rec linear i n =
    ...
    | Iifthenelse (test, ifso, ifnot) ->
      ...
      | _, Iend, _ ->
        let (lbl_end, n2) = get_label n1 in
        copy_instr (Lcondbranch (invert_test test, lbl_end)) i (linear ifso n2)
      | _, _, _ ->
    ...
    | Iswitch (index, cases) ->
      ...
      if Array.length index = 3 then
        ...
        in
        copy_instr (Lcondbranch3 (find_label 0, find_label 1, find_label 2)) i
          !n2
      else
      ...
      let labels_at_entry_to_handlers =
        List.map
          (fun (_nfail, handler) ->
             ...
             | Iend -> lbl_end
             | _ -> Cmm.new_label ()) handlers
      in
      let exit_label_add =
        List.map2 (fun (nfail, _) lbl -> nfail, (lbl, !try_depth)) handlers
          ...
          (fun n (_nfail, handler) lbl_handler ->
             ...
             | _ ->
               cons_instr (Llabel lbl_handler)
                 (linear handler (add_branch lbl_end n))) n1 handlers
          labels_at_entry_to_handlers
      in
