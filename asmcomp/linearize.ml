...
let rec linear i n =
  ...
  | Iop ((Itailcall_ind _ | Itailcall_imm _) as op) ->
      ...
      then copy_instr (Lop op) i (discard_dead_code n)
      else copy_instr (Lop op) i (linear i.Mach.next n)
  ...
  | Ireturn ->
      ...
      if !Proc.contains_calls then cons_instr Lreloadretaddr n1 else n1
  ...
  | Iswitch (index, cases) ->
      ...
      then
        ...
        copy_instr
          (Lswitch (Array.map (fun n -> lbl_cases.(n)) index))
          i
          !n2
...
let fundecl f =
  ...
  ; fun_body
  ; fun_fast =
      not (List.mem Cmm.Reduce_code_size f.Mach.fun_codegen_options)
