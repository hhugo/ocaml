            ...
            , Csequence
                ( op
                    ...
                    ; op
                        Cadda
                        [ op
                            (Cload (Byte_unsigned, Asttypes.Mutable))
                            [op Cadda [Cvar afl_area; Cvar cur_pos]]
                        ; Cconst_int (1, dbg) ] ]
                , op
...
and instrument = function
  ...
  | Ccatch (isrec, cases, body) ->
      let cases =
        List.map
          (fun (nfail, ids, e, dbg) -> nfail, ids, instrument e, dbg)
          cases
      in
  ...
  | ( Cconst_int _ | Cconst_natint _ | Cconst_float _ | Cconst_symbol _
    | Cconst_pointer _ | Cconst_natpointer _ | Cblockheader _ | Cvar _ ) as
    c ->
      c
...
let instrument_initialiser c dbg =
  ...
  with_afl_logging
    (Csequence
       ( Cop
           ( Cextcall ("caml_setup_afl", typ_int, false, None)
           , [Cconst_int (0, dbg ())]
           , dbg () )
       , c ))
    (dbg ())
