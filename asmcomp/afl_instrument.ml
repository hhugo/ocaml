...
let rec with_afl_logging b dbg =
  if
    !Clflags.afl_inst_ratio < 100 && Random.int 100 >= !Clflags.afl_inst_ratio
  then
    instrument b
  else
    let instrumentation =
      ...
      Clet
        (VP.create afl_area,
         ...
         Clet
           (VP.create cur_pos,
            op Cxor
              [ op (Cload (Word_int, Asttypes.Mutable)) [ afl_prev_loc dbg ];
            ...
            Csequence
              (op (Cstore (Byte_unsigned, Assignment))
                 [ op Cadda [ Cvar afl_area; Cvar cur_pos ];
                   op Cadda
                     [
                       op (Cload (Byte_unsigned, Asttypes.Mutable))
                         [ op Cadda [ Cvar afl_area; Cvar cur_pos ] ];
                       Cconst_int (1, dbg) ] ],
...
and instrument =
  ...
  | Clet_mut (v, k, e, body) -> Clet_mut (v, k, instrument e, instrument body)
  ...
  | Ccatch (isrec, cases, body) ->
    let cases =
      List.map (fun (nfail, ids, e, dbg) -> nfail, ids, instrument e, dbg) cases
    in
...
let instrument_initialiser c dbg =
  ...
  with_afl_logging
    (Csequence
       (Cop
          (Cextcall ("caml_setup_afl", typ_int, [], false),
           [ Cconst_int (0, dbg ()) ],
           dbg ()),
        c)) (dbg ())
