...
let machtype ppf mty =
  ...
  | n ->
    ...
    for i = 1 to n - 1 do fprintf ppf "*%a" machtype_component mty.(i) done
...
let rec expr ppf =
  ...
  | Clet_mut (id, kind, def, body) ->
    fprintf ppf "@[<2>(let_mut@ @[<2>%a: %a@ %a@]@ %a)@]" VP.print id machtype
      kind expr def sequence body
  ...
  | Ctrywith (e1, id, e2, _dbg) ->
    fprintf ppf "@[<2>(try@ %a@;<1 -2>with@ %a@ %a)@]" sequence e1 VP.print id
      sequence e2
...
let fundecl ppf f =
  let print_cases ppf cases =
    ...
    List.iter
      (fun (id, ty) ->
         (if !first then first := false else fprintf ppf "@ ");
         fprintf ppf "%a: %a" VP.print id machtype ty) cases
  in
  fprintf ppf "@[<1>(function%s %s@;<1 4>@[<1>(%a)@]@ @[%a@])@]@."
    (location f.fun_dbg) f.fun_name print_cases f.fun_args sequence f.fun_body
