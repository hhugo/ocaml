...
let closure_info ~arity ~startenv =
  ...
  Nativeint.(
    add (shift_left (of_int arity) pos_arity_in_closinfo)
...
let int_const dbg n =
  if n <= max_repr_int && n >= min_repr_int then
    Cconst_int (n lsl 1 + 1, dbg)
  else
    Cconst_natint
      (Nativeint.add (Nativeint.shift_left (Nativeint.of_int n) 1) 1n, dbg)

let natint_const_untagged dbg n =
  if n > Nativeint.of_int max_int || n < Nativeint.of_int min_int then
    Cconst_natint (n, dbg)
  else
...
let ignore_high_bit_int =
  function
  | Cop
      (Casr, [ Cop (Clsl, [ c; Cconst_int (1, _) ], _); Cconst_int (1, _) ], _)
    ->
    c
  | c -> c
...
let mk_not dbg cmm =
  ...
  | Cop
    ...
    | Cop (Ccmpi cmp, [ c1; c2 ], dbg'') ->
      tag_int (Cop (Ccmpi (negate_integer_comparison cmp), [ c1; c2 ], dbg''))
        dbg'
    | Cop (Ccmpa cmp, [ c1; c2 ], dbg'') ->
      tag_int (Cop (Ccmpa (negate_integer_comparison cmp), [ c1; c2 ], dbg''))
        dbg'
    | Cop (Ccmpf cmp, [ c1; c2 ], dbg'') ->
      tag_int (Cop (Ccmpf (negate_float_comparison cmp), [ c1; c2 ], dbg''))
        dbg'
    | _ ->
      (* 0 -> 3, 1 -> 1 *)
      Cop
    ...
    (fun a1 ->
       bind "float_cmp" a2
         (fun a2 ->
            ...
            tag_int (add_int (sub_int op1 op2 dbg) (sub_int op3 op4 dbg) dbg)
              dbg))
...
let divimm_parameters d =
  Nativeint.(
    ...
    let rec loop p (q1, r1) (q2, r2) =
      ...
      and r1 = shift_left r1 1
      in
      let (q1, r1) = if ucompare r1 nc >= 0 then succ q1, sub r1 nc else q1, r1
      in
      let q2 = shift_left q2 1
      and r2 = shift_left r2 1
      in
      let (q2, r2) = if ucompare r2 d >= 0 then succ q2, sub r2 d else q2, r2 in
      let delta = sub d r2 in
      if ucompare q1 delta < 0 || q1 = delta && r1 = 0n then
        loop p (q1, r1) (q2, r2)
      else
        succ q2, p - size
    in
         ...
         [
           bind "dividend" c1
             (fun c1 ->
                ...
                add_int c1 t dbg); Cconst_int (l, dbg) ],
         dbg)
    else if n < 0 then
      sub_int (Cconst_int (0, dbg))
        (div_int c1 (Cconst_int (~-n, dbg)) is_safe dbg) dbg
    else
...
let mod_int c1 c2 is_safe dbg =
  ...
  | c1, (Cconst_int (n, _) as c2) when n <> min_int ->
    ...
    if n = 1 lsl l then
      ...
      bind "dividend" c1
        (fun c1 -> sub_int c1 (mul_int (div_int c1 c2 is_safe dbg) c2 dbg) dbg)
  | c1, c2 when !Clflags.unsafe || is_safe = Lambda.Unsafe ->
...
let field_address ptr n dbg =
  if n = 0 then
    ...
    Cop (Cadda, [ ptr; Cconst_int (n * size_addr, dbg) ], dbg)
...
let get_tag ptr dbg =
  if Proc.word_addressed then
    (* If byte loads are slow *)
    Cop (Cand, [ get_header ptr dbg; Cconst_int (255, dbg) ], dbg)
  else
  (* If byte loads are efficient *)(* Same comment as [get_header] above *)
...
let array_indexing ?typ log2size ptr ofs dbg =
  ...
  | Cconst_int (n, _) ->
    ...
    if i = 0 then
      ...
      Cop (add, [ ptr; Cconst_int (i lsl log2size, dbg) ], dbg)
  | Cop
      (Caddi,
       [ Cop (Clsl, [ c; Cconst_int (1, _) ], _); Cconst_int (1, _) ],
       dbg')
...
let addr_array_set arr ofs newval dbg =
  Cop
    (Cextcall ("caml_modify", typ_void, [], false),
     [ array_indexing log2_size_addr arr ofs dbg; newval ],
     dbg)

let addr_array_initialize arr ofs newval dbg =
  Cop
    (Cextcall ("caml_initialize", typ_void, [], false),
     [ array_indexing log2_size_addr arr ofs dbg; newval ],
     dbg)
...
let lookup_tag obj tag dbg =
  bind "tag" tag
    (fun tag ->
       Cop
         (Cextcall ("caml_get_public_method", typ_val, [], false),
          [ obj; tag ],
          dbg))
...
let make_alloc_generic set_fn dbg tag wordsize args =
  if wordsize <= Config.max_young_wosize then
    ...
    Clet
      (VP.create id,
       Cop
         (Cextcall ("caml_alloc", typ_val, [], true),
...
(* Big arrays *)
let bigarray_elt_size : Lambda.bigarray_kind -> int =
  function
...
let bigarray_indexing unsafe elt_kind layout b args dbg =
  ...
  let rec ba_indexing dim_ofs delta_ofs =
    function
    | [] -> assert false
    | [ arg ] ->
      if unsafe then
    ...
    | arg1 :: argl ->
      ...
      in
      if unsafe then
        add_int (mul_int (decr_int rem dbg) bound dbg) arg1 dbg
      else
        bind "idx" arg1
          (fun idx ->
             bind "bound" bound
  ...
  and elt_size = bigarray_elt_size elt_kind
  in
  (* [array_indexing] can simplify the given expressions *)
  array_indexing ~typ:Addr (Misc.log2 elt_size)
    (Cop (Cload (Word_int, Mutable), [ field_address b 1 dbg ], dbg)) offset dbg

let bigarray_word_kind : Lambda.bigarray_kind -> memory_chunk =
  function
  | Pbigarray_unknown -> assert false
    ...
    (fun b ->
       ...
       | Pbigarray_complex32 | Pbigarray_complex64 ->
         ...
         bind "addr" (bigarray_indexing unsafe elt_kind layout b args dbg)
           (fun addr ->
              bind "reval" (Cop (Cload (kind, Mutable), [ addr ], dbg))
                (fun reval ->
                   bind "imval"
                     (Cop
                        (Cload (kind, Mutable),
                         [ Cop (Cadda, [ addr; Cconst_int (sz, dbg) ], dbg) ],
                         dbg)) (fun imval -> box_complex dbg reval imval)))
       | _ ->
         Cop
...
let rec low_32 dbg =
  ...
  | Cop
      (Casr, [ Cop (Clsl, [ x; Cconst_int (32, _) ], _); Cconst_int (32, _) ], _)
...
let box_int_gen dbg (bi : Primitive.boxed_integer) arg =
  let arg' =
    if bi = Primitive.Pint32 && size_int = 8 then
      (if big_endian then
         Cop (Clsl, [ arg; Cconst_int (32, dbg) ], dbg)
       else
         sign_extend_32 dbg arg)
    else
      arg
  in
  Cop
    (Calloc,
     [ alloc_header_boxed_int bi dbg;
       Cconst_symbol (operations_boxed_int bi, dbg); arg' ],
     dbg)

let split_int64_for_32bit_target arg dbg =
  bind "split_int64" arg
    (fun arg ->
  ...
  map_tail
    (function
     | Cop
       ...
       when
         bi = Primitive.Pint32 &&
           size_int = 8 && big_endian && alloc_matches_boxed_int bi ~hdr ~ops
       ->
     ...
     | Cop (Calloc, [ hdr; ops; contents ], _dbg)
       when
         bi = Primitive.Pint32 &&
           size_int = 8 &&
             not big_endian && alloc_matches_boxed_int bi ~hdr ~ops
       ->
       (* Force sign-extension of low 32 bits *)
       sign_extend_32 dbg contents
     | Cop (Calloc, [ hdr; ops; contents ], _dbg)
...
let unaligned_load_16 ptr idx dbg =
  if Arch.allow_unaligned_access then
    Cop (Cload (Sixteen_unsigned, Mutable), [ add_int ptr idx dbg ], dbg)
  else
    let cconst_int i = Cconst_int (i, dbg) in
    let v1 = Cop (Cload (Byte_unsigned, Mutable), [ add_int ptr idx dbg ], dbg)
    in
    let v2 =
      Cop
        (Cload (Byte_unsigned, Mutable),
...
let unaligned_load_32 ptr idx dbg =
  if Arch.allow_unaligned_access then
    Cop (Cload (Thirtytwo_unsigned, Mutable), [ add_int ptr idx dbg ], dbg)
  else
    let cconst_int i = Cconst_int (i, dbg) in
    let v1 = Cop (Cload (Byte_unsigned, Mutable), [ add_int ptr idx dbg ], dbg)
    in
       ...
       [
         Cop
           (Cor,
            ...
            dbg); Cop (Cor, [ lsl_int b3 (cconst_int 8) dbg; b4 ], dbg) ],
       dbg)
...
let unaligned_load_64 ptr idx dbg =
  assert (size_int = 8);
  if Arch.allow_unaligned_access then
    Cop (Cload (Word_int, Mutable), [ add_int ptr idx dbg ], dbg)
  else
    let cconst_int i = Cconst_int (i, dbg) in
    let v1 = Cop (Cload (Byte_unsigned, Mutable), [ add_int ptr idx dbg ], dbg)
    in
            ...
            [
              Cop
                (Cor,
                 ...
                 dbg); Cop (Cor, [ lsl_int b7 (cconst_int 8) dbg; b8 ], dbg) ],
            dbg) ],
       dbg)
...
let int64_native_prim name arity ~alloc =
  ...
  let rec make_args =
    ...
    | n -> u64 :: make_args (n - 1)
  in
  Primitive.make ~name ~native_name:(name ^ "_native") ~alloc
    ~native_repr_args:(make_args arity) ~native_repr_res:u64
...
let make_switch arg cases actions dbg =
  ...
  let extract_affine ~cases ~const_actions =
    ...
    if length >= 2 then
      ...
      | Cint v0, Cint v1 ->
        ...
        in
        if
          Misc.Stdlib.Array.for_alli (fun i idx -> check i const_actions.(idx))
            cases
        then
          Some (v0, slope)
        else
  ...
  let make_affine_computation ~offset ~slope arg dbg =
    ...
    add_int (mul_int arg (natint_const_untagged dbg slope) dbg)
      (natint_const_untagged dbg offset) dbg
  in
  match Misc.Stdlib.Array.all_somes (Array.map extract_uconstant actions) with
  | None -> Cswitch (arg, cases, actions, dbg)
  | Some const_actions ->
    begin match extract_affine ~cases ~const_actions with
...
let transl_int_switch dbg arg low high cases default =
  ...
  | _ :: _ ->
    ...
    in
    let rec inters plow phigh pact =
      function
      | [] ->
        if phigh = high then
          [ plow, phigh, pact ]
        else
          [ plow, phigh, pact; phigh + 1, high, 0 ]
      | (i, act) :: rem ->
        if i = phigh + 1 then
          (if pact = act then
        ...
        else if(* insert default *)  pact = 0 then
          (if act = 0 then
             inters plow i 0 rem
           else
             (plow, i - 1, pact) :: inters i i act rem)
        else
        (* pact <> 0 *)
          (plow, phigh, pact) ::
            if act = 0 then
              ...
              (phigh + 1, i - 1, 0) :: inters i i act rem
    in
    let inters =
      match cases with
      | [] -> assert false
...
let ptr_offset ptr offset dbg =
  if offset = 0 then
    ...
    Cop (Caddv, [ ptr; Cconst_int (offset * size_addr, dbg) ], dbg)
...
let generic_apply mut clos args dbg =
  ...
  | _ ->
    let arity = List.length args in
    let cargs = Cconst_symbol (apply_function_sym arity, dbg) :: args @ [ clos ]
    in
...
let cache_public_method meths tag cache dbg =
  ...
  and tagged = V.create_local "*tagged*"
  in
  Clet_mut
    (VP.create li,
     typ_int,
          ...
          (ccatch
                    ...
                    Csequence
                      (Cifthenelse
                         (Cop
                          ...
                          dbg,
                          Cassign
                            (hi, Cop (Csubi, [ Cvar mi; cconst_int 2 ], dbg)),
                          dbg,
                          Cassign (li, Cvar mi),
                          dbg),
                       Cifthenelse
           ...
           Clet
             (VP.create tagged,
              ...
              Csequence
                (Cop
                   (Cstore (Word_int, Assignment), [ cache; Cvar tagged ], dbg),
                 Cvar tagged)))))
...
let apply_function_body arity =
  ...
  let rec app_fun clos n =
    if n = arity - 1 then
      Cop
        (Capply typ_val,
         [ get_field_gen Asttypes.Mutable (Cvar clos) 0 (dbg ()); Cvar arg.(n);
           Cvar clos ],
         dbg ())
    else
...
let send_function arity =
  ...
  let clos =
    ...
    and cached = V.create_local "cached"
    in
    ...
    if n = 0 then
      Cop
        (Capply typ_val,
         ...
         dbg ())
    else if n = arity - 1 || arity > max_arity_optimized then
      let newclos = V.create_local "clos" in
      Clet
        (VP.create newclos,
             ...
             Cop
               (Calloc,
                [ alloc_closure_header 5 (dbg ());
                  ...
                  alloc_closure_info ~arity:(arity - num - 1) ~startenv:3
                    (dbg ());
                  Cconst_symbol
             ...
             Cop
               (Calloc,
                [ alloc_closure_header 4 (dbg ());
                  Cconst_symbol (name1 ^ "_" ^ Int.to_string (num + 1), dbg ());
                  alloc_closure_info ~arity:1 ~startenv:2 (dbg ()); Cvar arg;
                  Cvar clos ],
                dbg ()));
        fun_codegen_options = [];
        fun_dbg
          ...
          Cfunction
            {
              ...
              fun_args;
              fun_body =
                iter (num + 1) (List.map (fun (arg, _) -> Cvar arg) direct_args)
                  clos;
              fun_codegen_options = [];
              fun_dbg
            }
...
let generic_functions shared units =
  let (apply, send, curry) =
    List.fold_left
      ...
      (Int.Set.empty, Int.Set.empty, Int.Set.empty) units
  in
...
let bbswap bi arg dbg =
  ...
  Cop
    (Cextcall
       (Printf.sprintf "caml_%s_direct_bswap" prim, typ_int, [ tyarg ], false),
     [ arg ],
     dbg)

let bswap16 arg dbg =
  Cop (Cextcall ("caml_bswap16_direct", typ_int, [], false), [ arg ], dbg)
       ...
       (Cstore (Double, init),
        [
          if n = 0 then
            ...
            Cop (Cadda, [ arg1; Cconst_int (n * size_float, dbg) ], dbg); arg2 ],
        dbg))
...
let arrayset_unsafe kind arg1 arg2 arg3 dbg =
  return_unit dbg
    begin match (kind : Lambda.array_kind) with
    | Pgenarray ->
      bind "newval" arg3
        (fun newval ->
           bind "index" arg2
      ...
      (fun name next ->
         ...
         Csequence
           (Cop (Capply typ_void, [ cconst_symbol entry_sym ], dbg ()),
            Csequence (incr_global_inited (), next))) namelist (cconst_int 1)
  in
...
let global_table namelist =
  ...
  Cdata
    (Cglobal_symbol "caml_globals" ::
       Cdefine_symbol "caml_globals" :: List.map mksym namelist @
       [ cint_zero ])
...
let frame_table namelist =
  ...
  Cdata
    (Cglobal_symbol "caml_frametable" ::
       Cdefine_symbol "caml_frametable" :: List.map mksym namelist @
       [ cint_zero ])
...
let segment_table namelist symbol begname endname =
  ...
  Cdata
    (Cglobal_symbol symbol ::
       Cdefine_symbol symbol :: List.fold_right addsyms namelist [ cint_zero ])
...
let predef_exception i name =
  ...
  let data_items = emit_block (exn_sym, Global) (block_header tag size) fields
  in
  Cdata data_items
...
let fundecls_size fundecls =
  ...
  List.iter
    (fun (f : Clambda.ufunction) ->
       ...
       in
       sz := !sz + 1 + 2 + indirect_call_code_pointer_size) fundecls;
  !sz
        ...
        if f2.arity = 1 || f2.arity = 0 then
          ...
          Cint (infix_header pos) :: closure_symbol f2 @
            Csymbol_address (curry_function_sym f2.arity) ::
              Cint (closure_info ~arity:f2.arity ~startenv:(startenv - pos)) ::
                Csymbol_address f2.label :: emit_others (pos + 4) rem
    in
