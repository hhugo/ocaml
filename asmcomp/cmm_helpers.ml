...
let int_const dbg n =
  if n <= max_repr_int && n >= min_repr_int then Cconst_int ((n lsl 1) + 1, dbg)
  else
    Cconst_natint
...
let udivmod n d =
  Nativeint.(
    if d < 0n then if ucompare n d < 0 then (0n, n) else (1n, sub n d)
    else
...
let curry_function_sym n =
  ...
  if n >= 0 then "caml_curry" ^ Int.to_string n
...
let zero_extend_32 dbg e =
  if size_int = 4 then e
  else Cop (Cand, [ low_32 dbg e; natint_const_untagged dbg 0xFFFFFFFFn ], dbg)
...
let box_int_gen dbg (bi : Primitive.boxed_integer) arg =
  let arg' =
    if bi = Primitive.Pint32 && size_int = 8 then
      if big_endian then Cop (Clsl, [ arg; Cconst_int (32, dbg) ], dbg)
...
let unaligned_load_64 ptr idx dbg =
  ...
  if Arch.allow_unaligned_access then
    ...
    let b1, b2, b3, b4, b5, b6, b7, b8 =
      if Arch.big_endian then (v1, v2, v3, v4, v5, v6, v7, v8)
...
let transl_int_switch dbg arg low high cases default =
  ...
  | _ :: _ ->
      ...
      let rec inters plow phigh pact = function
        ...
        | (i, act) :: rem ->
            if i = phigh + 1 then
              if pact = act then inters plow i pact rem
              else (plow, phigh, pact) :: inters i i act rem
            else if (* insert default *)
                    pact = 0 then
              if act = 0 then inters plow i 0 rem
...
let transl_switch_clambda loc arg index cases =
  ...
  for i = n_index - 2 downto 0 do
    let act = index.(i) in
    if act = !this_act then decr this_low
    else (
...
let apply_function_body arity =
  ...
  ( args,
    clos,
    if arity = 1 then app_fun clos 0
    else
      Cifthenelse
        ( Cop
            ( Ccmpi Ceq,
  ...
  Cfunction
    {
      fun_name;
      fun_args =
        [ (VP.create last_arg, typ_val); (VP.create last_clos, typ_val) ];
...
let rec intermediate_curry_functions arity num =
  let dbg = placeholder_dbg in
  if num = arity - 1 then [ final_curry_function arity ]
  else
...
let curry_function arity =
  ...
  if arity > 0 then intermediate_curry_functions arity 0
...
let raise_prim raise_kind arg dbg =
  if !Clflags.debug then Cop (Craise raise_kind, [ arg ], dbg)
...
let offsetint n arg dbg =
  if Misc.no_overflow_lsl n 1 then add_const arg (n lsl 1) dbg
         ...
         bind "index" (untag_int arg2 dbg) (fun idx ->
             Csequence
               ( make_checkbound dbg [ string_length str dbg; idx ],
                 Cop
                   (Cload (Byte_unsigned, Mutable), [ add_int str idx dbg ], dbg)
               ))))
    dbg
...
let emit_boxed_int64_constant_fields n cont =
  ...
  if size_int = 8 then Csymbol_address caml_int64_ops :: Cint lo :: cont
  else
...
let emit_constant_closure ((_, global_symb) as symb) fundecls clos_vars cont =
  let closure_symbol (f : Clambda.ufunction) =
    if Config.flambda then cdefine_symbol (f.label ^ "_closure", global_symb)
    else []
  in
  ...
  | f1 :: remainder ->
        ...
        | (f2 : Clambda.ufunction) :: rem ->
            if f2.arity = 1 || f2.arity = 0 then
              ...
              @ Csymbol_address f2.label
                :: Cint
                     (closure_info ~arity:f2.arity ~startenv:(startenv - pos))
      ...
      in
      Cint
        (black_closure_header (fundecls_size fundecls + List.length clos_vars))
      :: cdefine_symbol symb
      @ closure_symbol f1
      @
