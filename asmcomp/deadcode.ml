...
type d = {
  ...
  regs : Reg.Set.t;
  (* a set of registers live "before" instruction [i] *)
  exits : Int.Set.t; (* indexes of Iexit instructions "live before" [i] *)
}

let append a b =
  let rec append a b =
    match a.desc with Iend -> b | _ -> { a with next = append a.next b }
  in
  match b.desc with Iend -> a | _ -> append a b
...
let rec deadcode i =
  ...
  | Iop op ->
      ...
      if
        operation_is_pure op (* no side effects *)
  ...
  | Iifthenelse (test, ifso, ifnot) ->
      ...
      let s = deadcode i.next in
      {
        ...
        regs = Reg.add_set_array i.live i.arg;
        exits = Int.Set.union s.exits (Int.Set.union ifso'.exits ifnot'.exits);
      }
  | Iswitch (index, cases) ->
      ...
      let s = deadcode i.next in
      {
        ...
        regs = Reg.add_set_array i.live i.arg;
        exits =
          Array.fold_left (fun acc c -> Int.Set.union acc c.exits) s.exits dc;
      }
  | Icatch (rec_flag, handlers, body) -> (
      ...
      let rec add_live nfail (live_exits, used_handlers) =
        if Int.Set.mem nfail live_exits then (live_exits, used_handlers)
        else
      ...
      let live_exits =
        ...
        | Cmm.Nonrecursive ->
            List.fold_left
              (fun exits (_, h) -> Int.Set.union h.exits exits)
      ...
      | [] ->
          ...
          { i = append body'.i s.i; regs = body'.regs; exits }
      | _ ->
  ...
  | Iexit nfail -> { i; regs = i.live; exits = Int.Set.singleton nfail }
  | Itrywith (body, handler) ->
      ...
      let s = deadcode i.next in
      {
        ...
        regs = i.live;
        exits = Int.Set.union s.exits (Int.Set.union body'.exits handler'.exits);
      }
