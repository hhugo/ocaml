...
let pass_dump_linear_if ppf flag message phrase =
  if !flag
  then fprintf ppf "*** %s@.%a@." message Printlinear.fundecl phrase;
  phrase
    ...
    Symbol.Map.iter
      (fun sym cst ->
        Format.fprintf
          ...
          Symbol.print
          sym
          Printclambda.structured_constant
          cst )
      structured_constants );
...
let raw_clambda_dump_if ppf
  ...
  if !dump_rawclambda || !dump_clambda
  then (
    Format.fprintf ppf "@.clambda:@.";
    Printclambda.clambda ppf ulambda;
    List.iter
      (fun {Clambda.symbol; definition} ->
        Format.fprintf
          ppf
          "%s:@ %a@."
          symbol
...
let compile_unit _output_prefix asm_filename keep_asm obj_filename gen =
...
let flambda_gen_implementation ?toplevel ~backend ~ppf_dump
  ...
  let clambda, preallocated, constants =
    Profile.record_call "backend" (fun () ->
        ...
        ++ (fun { Flambda_to_clambda.expr
                ; preallocated_blocks
                ; structured_constants
                ; exported } ->
             (* "init_code" following the name used in
                [Cmmgen.compunit_and_constants]. *)
             ( Un_anf.apply ~ppf_dump expr ~what:"init_code"
             , preallocated_blocks
             , structured_constants
             , exported ) )
        ++ set_export_info )
...
let lambda_gen_implementation ?toplevel ~ppf_dump (lambda : Lambda.program)
    =
...
let compile_implementation_gen ?toplevel prefixname ~required_globals
    ~ppf_dump gen_implementation program =
  let asmfile =
...
let compile_implementation_clambda ?toplevel prefixname ~ppf_dump
    (program : Lambda.program) =
  compile_implementation_gen
    ?toplevel
    prefixname
    ~required_globals:program.Lambda.required_globals
    ~ppf_dump
    lambda_gen_implementation
    program

let compile_implementation_flambda ?toplevel prefixname ~required_globals
    ~backend ~ppf_dump (program : Flambda.program) =
  compile_implementation_gen
    ?toplevel
    prefixname
    ~required_globals
    ~ppf_dump
    (flambda_gen_implementation ~backend)
    program
