...
let cmm_invariants ppf fd_cmm =
  let print_fundecl =
    if !Clflags.dump_cmm then
      Printcmm.fundecl
    else
      (fun ppf fdecl -> Format.fprintf ppf "%s" fdecl.fun_name)
  in
...
let linear_unit_info =
  { Linear_format.unit_name = ""; items = []; for_pack = None }
...
let emit_fundecl fd =
  if should_emit () then
    try Profile.record ~accumulate:true "emit" Emit.fundecl fd
    with
    | Emitaux.Error e -> raise (Error (Asm_generation (fd.Linear.fun_name, e)))
...
let compile_unit ~output_prefix ~asm_filename ~keep_asm ~obj_filename gen =
  ...
  Misc.try_finally ~exceptionally:(fun () -> remove_file obj_filename)
    (fun () ->
       ...
       (if should_emit () then
          let assemble_result =
            Profile.record "assemble" (Proc.assemble_file asm_filename)
              obj_filename
          in
          (if assemble_result <> 0 then
             raise (Error (Assembler_error asm_filename))));
       if create_asm && not keep_asm then remove_file asm_filename)

let end_gen_implementation
    ?toplevel ~ppf_dump (clambda : Clambda.with_constants)
=
  emit_begin_assembly ();
  clambda ++ Profile.record "cmm" Cmmgen.compunit ++
    Profile.record "compile_phrases" (compile_phrases ~ppf_dump) ++
    (fun () -> ());
  begin match toplevel with
  | None -> ()
  | Some f -> compile_genfuns ~ppf_dump f
  end;
  ...
  compile_phrase ~ppf_dump
    (Cmm_helpers.reference_symbols
       (List.filter_map
          (fun prim ->
             if not (Primitive.native_name_is_external prim) then
               None
             else
               Some (Primitive.native_name prim))
...
let asm_filename output_prefix =
  if !keep_asm_file || !Emitaux.binary_backend_available then
    output_prefix ^ ext_asm
  else
    Filename.temp_file "camlasm" ext_asm

let compile_implementation
    ?toplevel ~backend ~prefixname ~middle_end ~ppf_dump
    (program : Lambda.program)
=
  compile_unit ~output_prefix:prefixname ~asm_filename:(asm_filename prefixname)
    ~keep_asm:!keep_asm_file ~obj_filename:(prefixname ^ ext_obj)
    (fun () ->
...
let linear_gen_implementation filename =
  ...
  begin match !Clflags.for_package, linear_unit_info.for_pack with
  | None, None -> ()
...
let compile_implementation_linear output_prefix ~progname =
  compile_unit ~output_prefix ~asm_filename:(asm_filename output_prefix)
    ~keep_asm:!keep_asm_file ~obj_filename:(output_prefix ^ ext_obj)
    (fun () -> linear_gen_implementation progname)
...
let report_error ppf =
  ...
  | Asm_generation (fn, err) ->
    fprintf ppf "Error producing assembly code for function %s: %a" fn
