  ...
  (if node.length < 0 then
     ...
     | sons ->
       node.length <-
         List.fold_left
           (fun len (son, delay) ->
              Int.max len (longest_path critical_outputs son + delay)) 0 sons);
  node.length

(* Remove an instruction from the ready queue *)
let rec remove_instr node =
  function
  | [] -> []
  | instr :: rem ->
...
class virtual scheduler_generic =
  object (self)
      ...
      (if self#instr_is_load instr then
       ...
       (* If this is a store, add edges from the most recent store,
         ...
         (List.iter (add_edge_after node) !code_stores;
          ...
          code_loads := [];
          code_checkbounds := [])
       else if self#instr_is_checkbound instr then
    ...
    method private ready_instruction date queue =
      let rec extract best =
        function
        | [] -> if best == dummy_node then None else Some best
        | instr :: rem ->
          let new_best =
            if instr.date <= date && instr.length > best.length then
              instr
            else
              best
          in
          ...
          List.iter
            (fun (son, delay) ->
               ...
               if son.emitted_ancestors = son.ancestors then
                 new_queue := son :: !new_queue) node.sons;
          { node.instr with
      ...
      and schedule_block ready_queue i try_nesting =
        if self#instr_in_basic_block i try_nesting then
          ...
          let critical_outputs =
            ...
            | _ -> i.arg
          in
          (List.iter (fun x -> ignore (longest_path critical_outputs x))
             ready_queue;
