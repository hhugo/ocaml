...
let rec longest_path critical_outputs node =
  (if node.length < 0 then
   ...
   | [] ->
       node.length <-
         (if
          is_critical critical_outputs node.instr.res
...
class virtual scheduler_generic =
  object (self)
    ...
    method is_load = function Iload (_, _, _) -> true | _ -> false
    ...
    method private add_instruction ready_queue instr =
      ...
      if self#instr_is_load instr then (
      ...
      else if self#instr_is_store instr then (
        List.iter (add_edge_after node) !code_stores;
        ...
        code_loads := [];
        code_checkbounds := [])
      else if self#instr_is_checkbound instr then code_checkbounds := [ node ];
    ...
    method private ready_instruction date queue =
      let rec extract best = function
        ...
        | instr :: rem ->
            let new_best =
              if instr.date <= date && instr.length > best.length then instr
              else best
            in
    ...
    method schedule_fundecl f =
      ...
      if f.fun_fast && !Clflags.insn_sched then (
