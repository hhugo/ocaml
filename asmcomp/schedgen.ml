  ...
  then
    ...
    | [] ->
        ...
        <- ( if is_critical critical_outputs node.instr.res
                || node.instr.desc = Lreloadretaddr
                (* always critical *)
           then node.delay
           else 0 )
    | sons ->
...
class virtual scheduler_generic =
  object (self)
   ...
   load or store instructions (e.g. on the I386). *)
    method is_store = function Istore (_, _, _) -> true | _ -> false

    method is_load = function Iload (_, _) -> true | _ -> false

    method is_checkbound =
      function
    ...
    method private instr_is_store instr =
      match instr.desc with Lop op -> self#is_store op | _ -> false

    method private instr_is_load instr =
      match instr.desc with Lop op -> self#is_load op | _ -> false

    method private instr_is_checkbound instr =
      match instr.desc with Lop op -> self#is_checkbound op | _ -> false
    ...
    method private add_instruction ready_queue instr =
     ...
     discarding the previous stores, loads and checkbounds. *) )
      else if self#instr_is_store instr
      then (
        List.iter (add_edge_after node) !code_stores;
        ...
        code_loads := [];
        code_checkbounds := [] )
      else if self#instr_is_checkbound instr
      then code_checkbounds := [node];
   ...
   instruction sequence *)
    method private reschedule ready_queue date cont =
      ...
      then cont
        ...
        | None -> self#reschedule ready_queue (date + 1) cont
        | Some node ->
           ...
           in the queue. *)
            ...
            List.iter
              (fun (son, delay) ->
                ...
                if son.date < completion_date
                then son.date <- completion_date;
                son.emitted_ancestors <- son.emitted_ancestors + 1;
                if son.emitted_ancestors = son.ancestors
                then new_queue := son :: !new_queue )
              node.sons;
            { node.instr with
              next = self#reschedule !new_queue (date + issue_cycles) cont
            }
    ...
    method schedule_fundecl f =
      let rec schedule i try_nesting =
        ...
        | _ ->
            if self#instr_in_basic_block i try_nesting
            then (
              clear_code_dag ();
              schedule_block [] i try_nesting )
            else {i with next = schedule i.next try_nesting}
      and schedule_block ready_queue i try_nesting =
        ...
        then
          ...
          in
          List.iter
            (fun x -> ignore (longest_path critical_outputs x))
            ready_queue;
          self#reschedule ready_queue 0 (schedule i try_nesting)
      in
      if f.fun_fast && !Clflags.insn_sched
      then (
