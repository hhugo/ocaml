...
let overlap i0 i1 =
  let rec overlap_ranges rl0 rl1 =
    match rl0, rl1 with
    | r0 :: rl0', r1 :: rl1' ->
        if r0.rend >= r1.rbegin && r1.rend >= r0.rbegin
        then true
        else if r0.rend < r1.rend
        then overlap_ranges rl0' rl1
        else if r0.rend > r1.rend
        then overlap_ranges rl0 rl1'
...
let remove_expired_ranges i pos =
  let rec filter = function
    | [] -> []
    | r :: rl' as rl -> if pos < r.rend then rl else filter rl'
  in
...
let update_interval_position intervals pos kind reg =
  ...
  then (
    ...
    if pos - ridx <= 1
    then r.rend <- rend
    else i.ranges <- {rbegin; rend} :: i.ranges );
  i.iend <- rend
...
let insert_destroyed_at_oper intervals instr pos =
  ...
  then update_interval_position_by_array intervals destroyed pos Result
...
let build_intervals fd =
  let intervals =
    Array.init (Reg.num_registers ()) (fun _ ->
        {reg = Reg.dummy; ibegin = 0; iend = 0; ranges = []} )
  in
  let pos = ref 0 in
  let rec walk_instruction i =
    ...
    | Itrywith (body, handler) ->
        ...
        walk_instruction i.next
    | Iraise _ -> walk_instruction i.next
  in
  ...
  Array.iter
    (fun i ->
      ...
      then (
        ...
        | Reg _ -> fixed_interval_list := i :: !fixed_interval_list
        | _ -> interval_list := i :: !interval_list ) )
    intervals;
  (* Sort the intervals according to their start position *)
  interval_list :=
    List.sort (fun i0 i1 -> i0.ibegin - i1.ibegin) !interval_list
