...
let overlap i0 i1 =
  let rec overlap_ranges rl0 rl1 =
    match rl0, rl1 with
    | r0 :: rl0', r1 :: rl1' ->
      if r0.rend >= r1.rbegin && r1.rend >= r0.rbegin then
        true
      else if r0.rend < r1.rend then
        overlap_ranges rl0' rl1
      else if r0.rend > r1.rend then
        overlap_ranges rl0 rl1'
      else
        overlap_ranges rl0' rl1'
...
let is_live i pos =
  let rec is_live_in_ranges =
    function
    | [] -> false
    | r :: rl ->
      if pos < r.rbegin then
        false
      else if pos <= r.rend then
        true
      else
        is_live_in_ranges rl
  in
...
let build_intervals fd =
  let intervals =
    Array.init (Reg.num_registers ())
      (fun _ -> { reg = Reg.dummy; ibegin = 0; iend = 0; ranges = [] })
  in
  let pos = ref 0 in
    ...
    (fun i ->
       if i.iend != 0 then
         (i.ranges <- List.rev i.ranges;
          ...
          | _ -> interval_list := i :: !interval_list)) intervals;
