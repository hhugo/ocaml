...
let arg_mem b {arch; typ = _; idx; scale; base; sym; displ} =
  ...
  | None ->
      if displ <> 0 || scale = 0
      then Buffer.add_string b (Int.to_string displ)
  | Some s -> Buffer.add_string b s; opt_displ b displ );
  if scale <> 0
  then (
    Buffer.add_char b '(';
    ( match base with
...
let i2_ss b s x y =
  bprintf b "\t%s%s%s\t%a, %a" s (suf x) (suf y) arg x arg y
...
let print_line b = function
  ...
  | Bytes s ->
      if system = S_solaris
      then buf_bytes_directive b ".byte" s
  ...
  | Space n ->
      if system = S_solaris
      then bprintf b "\t.zero\t%d" n
      else bprintf b "\t.space\t%d" n
  | Word n ->
      if system = S_solaris
      then bprintf b "\t.value\t%a" cst n
