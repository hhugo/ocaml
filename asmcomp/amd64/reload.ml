...
let stackp r = match r.loc with Stack _ -> true | _ -> false
...
class reload =
  object (self)
    ...
    method! reload_operation op arg res =
      ...
      | Iintop (Imulh | Idiv | Imod | Ilsl | Ilsr | Iasr) | Iintop_imm (_, _) ->
      ...
      | Iconst_int n ->
          if n <= 0x7FFFFFFFn && n >= -0x80000000n then (arg, res)
    ...
    method! reload_test tst arg =
      ...
      | Ifloattest (CFlt | CFnlt | CFle | CFnle) ->
          ...
          if stackp arg.(1) then [| arg.(0); self#makereg arg.(1) |] else arg
      | Ifloattest (CFeq | CFneq | CFgt | CFngt | CFge | CFnge) ->
          (* Second argument can be on stack, first must be in register *)
          if stackp arg.(0) then [| self#makereg arg.(0); arg.(1) |] else arg
      | _ ->
          (* The argument(s) can be either in register or on stack *)
          arg
  end

let fundecl f num_stack_slots = (new reload)#fundecl f num_stack_slots
