...
let int_reg_name =
  ...
  | "msvc" ->
    [| "rax"; "rbx"; "rdi"; "rsi"; "rdx"; "rcx"; "r8"; "r9"; "r12"; "r13"; "r10";
      "r11"; "rbp" |]
  | _ ->
    [| "%rax"; "%rbx"; "%rdi"; "%rsi"; "%rdx"; "%rcx"; "%r8"; "%r9"; "%r12";
      "%r13"; "%r10"; "%r11"; "%rbp" |]
...
let loc_results res =
  let (loc, _ofs) = calling_conventions 0 0 100 100 not_supported 0 res in
  loc
...
let win64_int_external_arguments = [| 5 (*rcx*); 4 (*rdx*); 6 (*r8*); 7 |]

(*r9*)
let win64_float_external_arguments =
  [| 100 (*xmm0*); 101 (*xmm1*); 102 (*xmm2*); 103 |]
...
let loc_external_arguments ty_args =
  ...
  let (loc, stack_ofs) =
    if win64 then
      win64_loc_external_arguments arg
    else
      unix_loc_external_arguments arg
  in
...
let destroyed_at_c_call =
  if win64 then
    ...
    Array.of_list
      (List.map phys_reg [ 0; 4; 5; 6; 7; 10; 11; 100; 101; 102; 103; 104; 105 ])
  else (* Unix: rbp, rbx, r12-r15 preserved *)
    Array.of_list
      (List.map phys_reg
         [ 0; 2; 3; 4; 5; 6; 7; 10; 11; 100; 101; 102; 103; 104; 105; 106; 107;
...
let destroyed_at_oper =
  function
  | Iop (Icall_ind | Icall_imm _ | Iextcall { alloc = true }) -> all_phys_regs
  | Iop (Iextcall { alloc = false }) -> destroyed_at_c_call
  | Iop (Iintop (Idiv | Imod)) | Iop (Iintop_imm ((Idiv | Imod), _)) ->
    [| rax; rdx |]
  | Iop (Istore (Single, _, _)) -> [| rxmm15 |]
  | Iop (Ialloc _ | Ipoll _) -> destroyed_at_alloc_or_poll
  | Iop (Iintop (Imulh | Icomp _) | Iintop_imm (Icomp _, _)) -> [| rax |]
  | Iswitch (_, _) -> [| rax; rdx |]
  | Itrywith _ -> [| r11 |]
  | _ ->
    if fp then (* prevent any use of the frame pointer ! *) [| rbp |] else [| |]
...
let max_register_pressure =
  ...
  | Ialloc _ | Ipoll _ ->
    if fp then
      [| 11 - num_destroyed_by_plt_stub; 16 |]
    else
      [| 12 - num_destroyed_by_plt_stub; 16 |]
  | Iintop (Icomp _) | Iintop_imm (Icomp _, _) ->
    if fp then [| 11; 16 |] else [| 12; 16 |]
  | Istore (Single, _, _) -> if fp then [| 12; 15 |] else [| 13; 15 |]
