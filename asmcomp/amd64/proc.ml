...
let max_int_args_in_regs () = if Config.spacetime then 9 else 10
...
let win64_loc_external_arguments arg =
  let loc = Array.make (Array.length arg) Reg.dummy in
  let reg = ref 0 and ofs = ref 32 in
  for i = 0 to Array.length arg - 1 do
    match arg.(i).typ with
    | (Val | Int | Addr) as ty ->
        if !reg < 4
...
let destroyed_at_c_call =
  ...
  then
    ...
    Array.of_list
      (List.map
         phys_reg
         [0; 4; 5; 6; 7; 10; 11; 100; 101; 102; 103; 104; 105])
  else
...
let destroyed_at_alloc =
  let regs =
    if Config.spacetime then [|rax; loc_spacetime_node_hole|] else [|rax|]
  in
...
let destroyed_at_oper = function
  ...
  | _ ->
      if fp
      then (* prevent any use of the frame pointer ! *)
        [|rbp|]
      else [||]
...
let max_register_pressure = function
  | Iextcall _ ->
      if win64
      then if fp then [|7; 10|] else [|8; 10|]
      else if fp
      then [|3; 0|]
      else [|4; 0|]
  ...
  | Ialloc _ ->
      if fp
      then [|11 - num_destroyed_by_plt_stub; 16|]
...
let init () =
  if fp
  then num_available_registers.(0) <- 12
  else num_available_registers.(0) <- 13
