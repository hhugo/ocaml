...
let reg ppf r =
  if not (Reg.anonymous r)
  then fprintf ppf "%s" (Reg.name r)
  else
    fprintf
      ppf
      "%s"
  ...
  match r.loc with
  | Unknown -> ()
  | Reg r -> fprintf ppf "[%s]" (Proc.register_name r)
  | Stack (Local s) -> fprintf ppf "[s%i]" s
  | Stack (Incoming s) -> fprintf ppf "[si%i]" s
  | Stack (Outgoing s) -> fprintf ppf "[so%i]" s
...
let test tst ppf arg =
  ...
  | Iinttest_imm (cmp, n) ->
      fprintf ppf "%a%s%i" reg arg.(0) (intcomp cmp) n
  | Ifloattest cmp ->
...
let operation op arg ppf res =
  ...
  | Ialloc {bytes = n; _} ->
      ...
      if Config.spacetime
      then fprintf ppf "(spacetime node = %a)" reg arg.(0)
...
let rec instr ppf i =
  ...
  then (
    ...
    fprintf ppf "}@]@,";
    if !Clflags.dump_avail
    then (
      let module RAS = Reg_availability_set in
      fprintf
        ppf
        "@[<1>AB={%a}"
        (RAS.print ~print_reg:reg)
        i.available_before;
  ...
  | Icatch (flag, handlers, body) ->
      fprintf
        ...
        body;
      let h (nfail, handler) =
        fprintf ppf "(%d)@,%a@;" nfail instr handler
      in
      let rec aux = function
        | [] -> ()
        | [v] -> h v
        | v :: t -> h v; fprintf ppf "@ and"; aux t
      in
      aux handlers
  ...
  | Itrywith (body, handler) ->
      fprintf
        ...
        instr
        handler
  | Iraise k -> fprintf ppf "%a %a" Printcmm.raise_kind k reg i.arg.(0) );
  if not (Debuginfo.is_none i.dbg)
  then fprintf ppf "%s" (Debuginfo.to_string i.dbg);
  match i.next.desc with Iend -> () | _ -> fprintf ppf "@,%a" instr i.next

let fundecl ppf f =
  let dbg =
    if Debuginfo.is_none f.fun_dbg
    then ""
...
let phase msg ppf f = fprintf ppf "*** %s@.%a@." msg fundecl f

let interference ppf r =
  let interf ppf = List.iter (fun r -> fprintf ppf "@ %a" reg r) r.interf in
...
let interval ppf i =
  let interv ppf =
    List.iter (fun r -> fprintf ppf "@ [%d;%d]" r.rbegin r.rend) i.ranges
  in
...
let preference ppf r =
  let prefs ppf =
    List.iter (fun (r, w) -> fprintf ppf "@ %a weight %i" reg r w) r.prefer
  in
