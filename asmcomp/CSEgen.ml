...
module Equations = struct
  ...
  let empty =
    {load_equations = Rhs_map.empty; other_equations = Rhs_map.empty}

  let add op_class op v m =
    match op_class with
    | Op_load -> {m with load_equations = Rhs_map.add op v m.load_equations}
    | _ -> {m with other_equations = Rhs_map.add op v m.other_equations}

  let find op_class op m =
    match op_class with
    | Op_load -> Rhs_map.find op m.load_equations
    | _ -> Rhs_map.find op m.other_equations

  let remove_loads m =
    {load_equations = Rhs_map.empty; other_equations = m.other_equations}
end

type numbering =
  { num_next : int
  ; (* next fresh value number *)
    num_eqs : valnum array Equations.t
  ; (* mapping rhs -> valnums *)
...
let find_reg_containing n v =
  Reg.Map.fold
    (fun r v' res -> if v' = v then Some r else res)
    n.num_reg
    None
...
let kill_addr_regs n =
  { n with
    num_reg = Reg.Map.filter (fun r _n -> r.Reg.typ <> Cmm.Addr) n.num_reg
  }
...
let insert_move srcs dsts i =
  ...
  | _ ->
      (* Parallel move: first copy srcs into tmps one by one,
  ...
  object (self)
   ...
   processor-specific files to classify specific operations better. *)
    method class_of_operation op =
      ...
      | Inegf | Iabsf | Iaddf | Isubf | Imulf | Idivf | Ifloatofint
       |Iintoffloat ->
          Op_pure
    ...
    method is_cheap_operation op =
      match op with Iconst_int _ -> true | _ -> false

    (* Forget all equations involving memory loads.  Performed after a
   non-initializing store *)
    method private kill_loads n = remove_load_numbering n
    ...
    method private cse n i =
      ...
      | Iop op -> (
        ...
        | (Op_pure | Op_checkbound | Op_load) as op_class -> (
            ...
            | Some vres -> (
              ...
              | Some res
                when (not (self#is_cheap_operation op))
     ...
     join point, but propagate the current numbering across fork points. *)
      | Iifthenelse (test, ifso, ifnot) ->
          let n1 = set_unknown_regs n (Proc.destroyed_at_oper i.desc) in
          { i with
            desc = Iifthenelse (test, self#cse n1 ifso, self#cse n1 ifnot)
          ; next = self#cse empty_numbering i.next }
      | Iswitch (index, cases) ->
          let n1 = set_unknown_regs n (Proc.destroyed_at_oper i.desc) in
          { i with
            desc = Iswitch (index, Array.map (self#cse n1) cases)
          ; next = self#cse empty_numbering i.next }
      | Icatch (rec_flag, handlers, body) ->
          ...
          in
          { i with
            desc = Icatch (rec_flag, List.map aux handlers, self#cse n body)
          ; next = self#cse empty_numbering i.next }
      | Itrywith (body, handler) ->
          { i with
            desc =
              Itrywith (self#cse n body, self#cse empty_numbering handler)
          ; next = self#cse empty_numbering i.next }

    method fundecl f =
      (* CSE can trigger bad register allocation behaviors, see MPR#7630 *)
      if List.mem Cmm.No_CSE f.fun_codegen_options
      then f
      else {f with fun_body = self#cse empty_numbering f.fun_body}
  end
