...
let valnum_reg n r =
  ...
  with
  | Not_found -> fresh_valnum_reg n r
...
let find_equation op_class n rhs =
  try Some (Equations.find op_class rhs n.num_eqs)
  with
  | Not_found -> None
...
let find_regs_containing n vs =
  ...
  | 0 -> Some [| |]
  | 1 ->
    begin match find_reg_containing n vs.(0) with
    | None -> None
    | Some r -> Some [| r |]
    end
  | l ->
  ...
  (* Default classification of operations.  Can be overridden in
    ...
    method private cse n i =
      ...
      | Iop op ->
        ...
        | (Op_pure | Op_checkbound | Op_load _) as op_class ->
          ...
          | Some vres ->
            ...
            | Some res
              when
                not (self#is_cheap_operation op) &&
      ...
      | Icatch (rec_flag, handlers, body) ->
        let aux (nfail, handler) = nfail, self#cse empty_numbering handler in
        { i with
