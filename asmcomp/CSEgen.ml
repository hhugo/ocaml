...
module Equations = struct
  ...
  let empty =
    { mutable_load_equations = Rhs_map.empty; other_equations = Rhs_map.empty }
...
class cse_generic =
  object (self)
    ...
    method private kill_loads n = remove_mutable_load_numbering n
    ...
    method private cse n i =
      ...
      | Itrywith (body, handler) ->
          {
            i with
            desc = Itrywith (self#cse n body, self#cse empty_numbering handler);
            next = self#cse empty_numbering i.next;
          }

    method fundecl f =
      (* CSE can trigger bad register allocation behaviors, see MPR#7630 *)
      if List.mem Cmm.No_CSE f.fun_codegen_options then f
