...
let insert_move src dst next =
  if src.loc = dst.loc then
    ...
    instr_cons (Iop Imove) [| src |] [| dst |] next
...
class reload_generic =
  object (self)
    ...
    method reload_test _tst args = self#makeregs args
    ...
    method private reload i =
      ...
      | Iop Itailcall_ind ->
        ...
        insert_moves i.arg newarg { i with  arg = newarg }
      | Iop (Icall_imm _ | Iextcall _) -> { i with  next = self#reload i.next }
      | Iop Icall_ind ->
        let newarg = self#makereg1 i.arg in
      ...
      | Iop op ->
        ...
        insert_moves i.arg newarg
          { i with
            ...
            arg = newarg;
            res = newres;
            next = insert_moves newres i.res (self#reload i.next)
          }
      | Iifthenelse (tst, ifso, ifnot) ->
        let newarg = self#reload_test tst i.arg in
        insert_moves i.arg newarg
          (instr_cons (Iifthenelse (tst, self#reload ifso, self#reload ifnot))
             newarg [| |] (self#reload i.next))
      | Iswitch (index, cases) ->
      ...
      | Icatch (rec_flag, handlers, body) ->
        let new_handlers =
          List.map (fun (nfail, handler) -> nfail, self#reload handler) handlers
        in
        instr_cons (Icatch (rec_flag, new_handlers, self#reload body)) [| |]
          [| |] (self#reload i.next)
      | Iexit i -> instr_cons (Iexit i) [| |] [| |] dummy_instr
