...
let record_use regv =
  for i = 0 to Array.length regv - 1 do
    ...
    then use_date := Reg.Map.add r !current_date !use_date
  done
...
let add_superpressure_regs op live_regs res_regs spilled =
  ...
  let rec check_pressure cl spilled =
    ...
    then spilled
    else if pressure.(cl) <= max_pressure.(cl)
    then check_pressure (cl + 1) spilled
    else
...
let find_reload_at_exit k =
  try List.assoc k !reload_at_exit
  with Not_found -> Misc.fatal_error "Spill.find_reload_at_exit"
...
let rec reload i before =
  ...
  | Icatch (rec_flag, handlers, body) ->
      let new_sets =
        List.map (fun (nfail, _) -> nfail, ref Reg.Set.empty) handlers
      in
      ...
      let rec fixpoint () =
        let at_exits =
          List.map (fun (nfail, set) -> nfail, !set) new_sets
        in
        let res =
          List.map2
        ...
        | Cmm.Recursive ->
            let equal =
              List.for_all2
                (fun (nfail', at_exit) (nfail, new_set) ->
                  assert (nfail = nfail');
...
let find_spill_at_exit k =
  try
    ...
    used := true;
    set
  with Not_found -> Misc.fatal_error "Spill.find_spill_at_exit"
...
let rec spill i finally =
  ...
  | Iop _ ->
      ...
      let before =
        ...
        | Iop (Icall_ind _)
         ...
         |Iop (Iextcall _)
         |Iop (Ialloc _)
         |Iop (Iintop (Icheckbound _))
         |Iop (Iintop_imm (Icheckbound _, _)) ->
  ...
  | Iifthenelse (test, ifso, ifnot) ->
      ...
      if !inside_loop || !inside_arm || !inside_catch
      then
        ( instr_cons
            (Iifthenelse (test, new_ifso, new_ifnot))
            i.arg
  ...
  | Icatch (rec_flag, handlers, body) ->
      ...
      let spill_at_exit_add at_exits =
        List.map2
        ...
        | Cmm.Recursive ->
            let equal =
              List.for_all2
                ...
                spill_at_exit_add
            in
            if equal then res else fixpoint (List.map snd res)
      in
      ...
      ( instr_cons
          ...
          new_next
      , before )
  | Iexit nfail -> i, find_spill_at_exit nfail
  | Itrywith (body, handler) ->
      let new_next, at_join = spill i.next finally in
