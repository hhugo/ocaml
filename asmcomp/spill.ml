...
let add_superpressure_regs op live_regs res_regs spilled =
  ...
  let rec check_pressure cl spilled =
    ...
    (* Find the least recently used, unspilled, unallocated, live register
       in the class *)
      let lru_date = ref 1000000
      and lru_reg = ref Reg.dummy
      in
      (Reg.Set.iter
         (fun r ->
            if
              Proc.register_class r = cl &&
                not (Reg.Set.mem r spilled) && r.loc = Unknown
            then
              try
                ...
                if d < !lru_date then
                  (lru_date := d;
                   lru_reg := r)
              with
              | Not_found -> (* Should not happen *) ()) live_regs;
       if !lru_reg != Reg.dummy then
         (pressure.(cl) <- pressure.(cl) - 1;
          check_pressure cl (Reg.Set.add !lru_reg spilled))
       else (* Couldn't find any spillable register, give up for this class *)
         check_pressure (cl + 1) spilled)
  in
...
let add_reloads regset i =
  Reg.Set.fold
    (fun r i -> instr_cons (Iop Ireload) [| (spill_reg r) |] [| r |] i) regset i
...
let set_reload_at_exit k s = Hashtbl.replace reload_at_exit k s
...
let rec reload i before =
  ...
  | Iop op ->
    let new_before =
      ...
      then
        ...
        add_superpressure_regs op i.live i.res before
    in
    let after = Reg.diff_set_array (Reg.diff_set_array new_before i.arg) i.res
    in
  ...
  | Iifthenelse (test, ifso, ifnot) ->
    ...
    in
    let new_i =
      instr_cons (Iifthenelse (test, new_ifso, new_ifnot)) i.arg i.res new_next
    in
    destroyed_at_fork := (new_i, at_fork) :: !destroyed_at_fork;
    add_reloads (Reg.inter_set_array before i.arg) new_i, finally
  | Iswitch (index, cases) ->
    let at_fork = Reg.diff_set_array before i.arg in
    let date_fork = !current_date in
  ...
  | Icatch (rec_flag, handlers, body) ->
    ...
    let rec fixpoint () =
      let at_exits =
        List.map (fun (nfail, _) -> nfail, get_reload_at_exit nfail) handlers
      in
      let res =
        List.map2
          (fun (nfail', handler) (nfail, at_exit) ->
             assert (nfail = nfail');
             reload handler at_exit) handlers at_exits
      in
      match rec_flag with
      | Cmm.Nonrecursive -> res
      | Cmm.Recursive ->
        let equal =
          List.for_all2
            (fun (nfail', _) (nfail, at_exit) ->
               assert (nfail = nfail');
               Reg.Set.equal at_exit (get_reload_at_exit nfail)) handlers
            at_exits
        in
        if equal then res else fixpoint ()
    in
    let res = fixpoint () in
    let union =
      List.fold_left
    ...
    let new_handlers =
      List.map2 (fun (nfail, _) (new_handler, _) -> nfail, new_handler) handlers
        res
    in
    instr_cons (Icatch (rec_flag, new_handlers, new_body)) i.arg i.res new_next,
    finally
  | Iexit nfail ->
    set_reload_at_exit nfail (Reg.Set.union (get_reload_at_exit nfail) before);
    i, Reg.Set.empty
  | Itrywith (body, handler) ->
    ...
    in
    instr_cons (Itrywith (new_body, new_handler)) i.arg i.res new_next, finally
  | Iraise _ -> add_reloads (Reg.inter_set_array before i.arg) i, Reg.Set.empty
...
let set_spill_at_exit k s = Hashtbl.replace spill_at_exit k s
...
let add_spills regset i =
  Reg.Set.fold
    (fun r i -> instr_cons (Iop Ispill) [| r |] [| (spill_reg r) |] i) regset i

let rec spill i finally =
  match i.desc with
  | Iend -> i, finally
  | Ireturn | Iop Itailcall_ind | Iop (Itailcall_imm _) -> i, Reg.Set.empty
  | Iop Ireload ->
    let (new_next, after) = spill i.next finally in
    let before1 = Reg.diff_set_array after i.res in
    instr_cons i.desc i.arg i.res new_next, Reg.add_set_array before1 i.res
  | Iop op ->
    let (new_next, after) = spill i.next finally in
    let before1 = Reg.diff_set_array after i.res in
    let before =
      if operation_can_raise op then
  ...
  | Iifthenelse (test, ifso, ifnot) ->
    ...
    if !inside_loop || !inside_arm || !inside_catch then
      instr_cons (Iifthenelse (test, new_ifso, new_ifnot)) i.arg i.res new_next,
      ...
      and spill_ifnot_branch =
        Reg.Set.diff (Reg.Set.diff before_ifnot before_ifso) destroyed
      in
      instr_cons
        (Iifthenelse
           (test,
            add_spills spill_ifso_branch new_ifso,
            add_spills spill_ifnot_branch new_ifnot)) i.arg i.res new_next,
      Reg.Set.diff
        (Reg.Set.diff (Reg.Set.union before_ifso before_ifnot) spill_ifso_branch)
        spill_ifnot_branch
  | Iswitch (index, cases) ->
    let (new_next, at_join) = spill i.next finally in
    ...
    instr_cons (Iswitch (index, new_cases)) i.arg i.res new_next, !before
  | Icatch (rec_flag, handlers, body) ->
    let (new_next, at_join) = spill i.next finally in
    ...
    let rec fixpoint () =
      ...
      let update changed (k, _handler) (_new_handler, before_handler) =
        if Reg.Set.equal before_handler (get_spill_at_exit k) then
          changed
        else
          (set_spill_at_exit k before_handler;
           true)
      in
      let changed = List.fold_left2 update false handlers res in
      if rec_flag = Cmm.Recursive && changed then fixpoint () else res
    in
    let res = fixpoint () in
    ...
    instr_cons (Icatch (rec_flag, new_handlers, new_body)) i.arg i.res new_next,
    before
  | Iexit nfail -> i, get_spill_at_exit nfail
  | Itrywith (body, handler) ->
    let (new_next, at_join) = spill i.next finally in
...
let fundecl f =
  ...
  let new_body =
    add_spills (Reg.inter_set_array tospill_at_entry f.fun_args) body2
  in
  reset ();
  {
    fun_name = f.fun_name;
