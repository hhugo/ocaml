...
let add_superpressure_regs op live_regs res_regs spilled =
  ...
  let rec check_pressure cl spilled =
    if cl >= Proc.num_register_classes then spilled
...
let rec reload i before =
  ...
  | Iop op ->
      let new_before =
        ...
        then before
        else add_superpressure_regs op i.live i.res before
      in
      let after =
        Reg.diff_set_array (Reg.diff_set_array new_before i.arg) i.res
      in
  ...
  | Iifthenelse (test, ifso, ifnot) ->
      ...
      let new_i =
        instr_cons
          (Iifthenelse (test, new_ifso, new_ifnot))
  ...
  | Icatch (rec_flag, handlers, body) ->
      ...
      let rec fixpoint () =
        let at_exits =
          List.map
            (fun (nfail, _) -> (nfail, get_reload_at_exit nfail))
            handlers
        in
        let res =
          List.map2
      ...
      in
      ( instr_cons
          (Icatch (rec_flag, new_handlers, new_body))
          i.arg i.res new_next,
        finally )
  | Iexit nfail ->
...
let rec spill i finally =
  ...
  | Iop op ->
      ...
      let before1 = Reg.diff_set_array after i.res in
      let before =
        if operation_can_raise op then Reg.Set.union before1 !spill_at_raise
        else before1
      in
  ...
  | Icatch (rec_flag, handlers, body) ->
      ...
      let rec fixpoint () =
        ...
        let update changed (k, _handler) (_new_handler, before_handler) =
          if Reg.Set.equal before_handler (get_spill_at_exit k) then changed
          else (
