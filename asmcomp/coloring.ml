...
module OrderedRegSet =
  ...
  (struct
    ...
    let compare r1 r2 =
      let open Reg in
      let c1 = r1.spill_cost
      and d1 = r1.degree
      in
      let c2 = r2.spill_cost
      and d2 = r2.degree
      in
...
let allocate_registers () =
  ...
  let remove_reg reg =
    ...
    else if reg.degree < Proc.num_available_registers.(cl) then
  ...
  let assign_location reg =
       ...
       (iter_preferred
          (fun r w ->
             ...
             | Reg n ->
               ...
               if n < num_regs then score.(n) <- score.(n) + w
             | Unknown ->
               List.iter
                 (fun neighbour ->
        ...
        List.iter
          (fun neighbour ->
             ...
             | Reg n ->
               let n = n - first_reg in
               if n < num_regs then score.(n) <- (-1000000)
             | _ -> ()
             end;
    ...
    (if !best_reg >= 0 then
     ...
     else (* Sorry, we must put the pseudoreg in a stack location *)
       let nslots = num_stack_slots.(cl) in
       ...
       (List.iter
        ...
        else (* Allocate a new stack slot *)
