...
module OrderedRegSet = Set.Make (struct
  type t = Reg.t
  ...
  let remove_reg reg =
    ...
    if reg.spill then (
      ...
      List.iter
        ...
        reg.interf;
      let slot = ref 0 in
      while !slot < nslots && conflict.(!slot) do
        incr slot
      done;
    ...
    else constrained := OrderedRegSet.add reg !constrained
  in
  ...
  let assign_location reg =
    ...
    (if !best_reg >= 0 then (
     ...
     if Proc.rotate_registers then
      ...
      (* Found one? *)
      if !best_slot >= 0 then reg.loc <- Stack (Local !best_slot)
      else (
