...
let traverse
  ...
  let conditionally_add_symbol symbol =
    if not (Symbol.Set.mem symbol !relevant_symbols)
    then (
  ...
  let conditionally_add_export_id export_id =
    if not (Export_id.Set.mem export_id !relevant_export_ids)
    then (
      relevant_export_ids :=
  ...
  let rec loop () =
    ...
    then ()
      ...
      | Q_symbol symbol -> (
          ...
          if Compilation_unit.is_current compilation_unit
          then
            match Symbol.Map.find symbol symbol_id with
            | exception Not_found ->
                Misc.fatal_errorf
                  "cannot find symbol's export id %a\n"
                  Symbol.print
                  symbol
