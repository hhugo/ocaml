...
let env_empty = {vars = V.Map.empty; static_exceptions = Int.Map.empty}
...
let oper_result_type = function
  ...
  | Caddi | Csubi | Cmuli | Cmulhi | Cdivi | Cmodi | Cand | Cor | Cxor
   |Clsl | Clsr | Casr | Ccmpi _ | Ccmpa _ | Ccmpf _ ->
      typ_int
  | Caddv -> typ_val
  | Cadda -> typ_addr
...
let join_array env rs =
  ...
  | Some (template, types) ->
      ...
      for i = 0 to Array.length rs - 1 do
        let r, s = rs.(i) in
        match r with None -> () | Some r -> s#insert_moves env r res
      done;
      Some res
...
*)
    method is_simple_expr =
      ...
      | Cphantom_let (_var, _defining_expr, body) ->
          self#is_simple_expr body
      | Csequence (e1, e2) ->
          self#is_simple_expr e1 && self#is_simple_expr e2
      | Cop (op, args, _) -> (
        match op with
        (* The following may have side effects *)
        | Capply _ | Cextcall _ | Calloc | Cstore _ | Craise _ -> false
        (* The remaining operations are simple if their args are *)
        | Cload _ | Caddi | Csubi | Cmuli | Cmulhi | Cdivi | Cmodi | Cand
         |Cor | Cxor | Clsl | Clsr | Casr | Ccmpi _ | Caddv | Cadda
         |Ccmpa _ | Cnegf | Cabsf | Caddf | Csubf | Cmulf | Cdivf
         |Cfloatofint | Cintoffloat | Ccmpf _ | Ccheckbound ->
   ...
   combined with the temporaries. *)
      ...
      | Cop (op, args, _) ->
          let from_op =
            ...
            | Caddi | Csubi | Cmuli | Cmulhi | Cdivi | Cmodi | Cand | Cor
             |Cxor | Clsl | Clsr | Casr | Ccmpi _ | Caddv | Cadda
             |Ccmpa _ | Cnegf | Cabsf | Caddf | Csubf | Cmulf | Cdivf
    ...
    method mark_call = Proc.contains_calls := true
    ...
    method mark_instr =
      function
      | Iop (Icall_ind _ | Icall_imm _ | Iextcall _) -> self#mark_call
    ...
    method select_operation op args _dbg =
      ...
      | Cstore (chunk, init), [arg1; arg2] ->
          ...
          then
            ...
            , [arg2; eloc] (* Inversion addr/datum in Istore *) )
    ...
    method private select_shift op =
      function
    ...
    method select_condition =
      ...
      | Cop (Ccmpi cmp, [arg1; Cconst_pointer (n, _)], _)
        when self#is_immediate n ->
          Iinttest_imm (Isigned cmp, n), arg1
      | Cop (Ccmpi cmp, [Cconst_pointer (n, _); arg2], _)
        when self#is_immediate n ->
          Iinttest_imm (Isigned (swap_integer_comparison cmp), n), arg2
      | Cop (Ccmpi cmp, args, _) -> Iinttest (Isigned cmp), Ctuple args
      | Cop (Ccmpa cmp, [arg1; Cconst_pointer (n, _)], _)
        when self#is_immediate n ->
          Iinttest_imm (Iunsigned cmp, n), arg1
      | Cop (Ccmpa cmp, [arg1; Cconst_int (n, _)], _)
        when self#is_immediate n ->
          Iinttest_imm (Iunsigned cmp, n), arg1
      | Cop (Ccmpa cmp, [Cconst_pointer (n, _); arg2], _)
        when self#is_immediate n ->
      ...
      | Cop (Ccmpf cmp, args, _) -> Ifloattest cmp, Ctuple args
   ...
   in pairs of integer registers. *)
    ...
    method extract = self#extract_core ~end_instr:(end_instr ())
   ...
   at the end of the self sequence *)
    method emit_expr (env : environment) exp =
      ...
      | Cconst_float (n, _dbg) ->
          ...
          Some
            (self#insert_op
               env
               (Iconst_float (Int64.bits_of_float n))
               [||]
               r)
      ...
      | Cconst_pointer (n, _dbg) ->
          let r = self#regs_for typ_val in
          (* integer as Caml value *)
          Some (self#insert_op env (Iconst_int (Nativeint.of_int n)) [||] r)
      | Cconst_natpointer (n, _dbg) ->
          let r = self#regs_for typ_val in
          (* integer as Caml value *)
          Some (self#insert_op env (Iconst_int n) [||] r)
      | Cblockheader (n, dbg) -> self#emit_blockheader env n dbg
      | Cvar v -> (
        try Some (env_find v env)
        with Not_found ->
          Misc.fatal_error
            ("Selection.emit_expr: unbound var " ^ V.unique_name v) )
      | Clet (v, e1, e2) -> (
        match self#emit_expr env e1 with
        | None -> None
        | Some r1 -> self#emit_expr (self#bind_let env v r1) e2 )
      | Cphantom_let (_var, _defining_expr, body) -> self#emit_expr env body
      | Cassign (v, e1) -> (
          let rv =
            try env_find v env
            with Not_found ->
              Misc.fatal_error
                ("Selection.emit_expr: unbound var " ^ V.name v)
          in
          match self#emit_expr env e1 with
          | None -> None
          | Some r1 ->
              self#adjust_types r1 rv;
      ...
      | Cop (Ccmpf _, _, dbg) ->
          self#emit_expr
            env
            (Cifthenelse
               (exp, dbg, Cconst_int (1, dbg), dbg, Cconst_int (0, dbg), dbg))
      | Cop (op, args, dbg) -> (
        ...
        | Some (simple_args, env) -> (
            ...
            let new_op, new_args =
              self#select_operation op simple_args dbg
            in
            match new_op with
            | Icall_ind _ ->
            ...
            | Iextcall _ ->
                ...
                let loc_arg, stack_ofs =
                  self#emit_extcall_args env new_args
                in
                self#maybe_emit_spacetime_move env ~spacetime_reg;
                let rd = self#regs_for ty in
                let loc_res =
                  self#insert_op_debug
                    env
                    new_op
                    dbg
                    loc_arg
                    (Proc.loc_external_results rd)
                in
                self#insert_move_results env loc_res rd stack_ofs;
                Some rd
      ...
      | Cswitch (esel, index, ecases, _dbg) -> (
        match self#emit_expr env esel with
        | None -> None
        | Some rsel ->
            let rscases =
              Array.map
                (fun (case, _dbg) -> self#emit_sequence env case)
                ecases
            in
            ...
            self#insert
              ...
              [||];
            r )
      | Ccatch (_, [], e1) -> self#emit_expr env e1
      | Ccatch (rec_flag, handlers, body) ->
          let handlers =
            List.map
              (fun (nfail, ids, e2, dbg) ->
                let rs =
                  List.map
                    (fun (id, typ) ->
          ...
          let env =
           ...
           the body. *)
            List.fold_left
              (fun env (nfail, _ids, rs, _e2, _dbg) ->
          ...
          self#insert
            env
            (Icatch (rec_flag, List.map aux l, s_body#extract))
            [||]
            [||];
          r
      | Cexit (nfail, args) -> (
    ...
    method private bind_let (env : environment) v r1 =
      ...
      then ( name_regs v r1; env_add v r1 env )
      else
   ...
   (and to be consistent with the bytecode compiler). *)
    ...
    method emit_tail (env : environment) exp =
      ...
      | Cop ((Capply ty as op), args, dbg) -> (
        ...
        | Some (simple_args, env) -> (
            ...
            | Icall_imm {func; label_after} ->
                ...
                else if func = !current_function_name
                then (
                  let call = Iop (Itailcall_imm {func; label_after}) in
      ...
      | Ccatch (_, [], e1) -> self#emit_tail env e1
      | Ccatch (rec_flag, handlers, e1) ->
          ...
          self#insert
            env
            (Icatch (rec_flag, List.map aux handlers, s_body))
            [||]
            [||]
      | Ctrywith (e1, v, e2, _dbg) -> (
          ...
          | Some r1 ->
              ...
              self#insert env Ireturn loc [||] )
      | _ -> self#emit_return env exp

    method private emit_tail_sequence env exp =
      let s = {<instr_seq = dummy_instr>} in
      s#emit_tail env exp; s#extract
...
let _ = Simplif.is_tail_native_heuristic := is_tail_call

let reset () = current_function_name := ""
