...
let env_find_mut id env =
  ...
  | Asttypes.Mutable -> ()
  | Asttypes.Immutable -> Misc.fatal_error "Selectgen.env_find_mut: not mutable"
  end;
  regs
...
let join env opt_r1 seq1 opt_r2 seq2 =
  ...
  | Some r1, Some r2 ->
    ...
    for i = 0 to l1 - 1 do
      if Reg.anonymous r1.(i) && Cmm.ge_component r1.(i).typ r2.(i).typ then
        (r.(i) <- r1.(i);
         seq2#insert_move env r2.(i) r1.(i))
      else if
        Reg.anonymous r2.(i) && Cmm.ge_component r2.(i).typ r1.(i).typ
      then
        (r.(i) <- r2.(i);
...
let join_array env rs =
  ...
  | Some (template, types) ->
    ...
    for i = 0 to size_res - 1 do res.(i) <- Reg.create types.(i) done;
    for i = 0 to Array.length rs - 1 do
      let (r, s) = rs.(i) in
      match r with
      | None -> ()
...
module Coeffect = struct
  type t = None | Read_mutable | Arbitrary
  ...
  (* A syntactic criterion used in addition to judgements about (co)effects as
    ...
    *)
    method is_simple_expr =
      ...
      | Cop (op, args, _) ->
        ...
        | Capply _ | Cextcall _ | Calloc | Cstore _ | Craise _ | Copaque ->
          false (* The remaining operations are simple if their args are *)
        | Cload _
        | Caddi
        | Csubi
        | Cmuli
    ...
    method effects_of exp =
      let module EC = Effect_and_coeffect 
      in
      match exp with
      | Cconst_int _
      | Cconst_natint _
    ...
    method mark_instr =
      function
      | Iop (Icall_ind | Icall_imm _ | Iextcall _) -> self#mark_call
      | Iop (Itailcall_ind | Itailcall_imm _) -> self#mark_tailcall
      | Iop (Ialloc _) | Iop (Ipoll _) -> self#mark_call
    ...
    method select_operation op args _dbg =
      ...
      | Cstore (chunk, init), [ arg1; arg2 ] ->
        ...
        in
        if chunk = Word_int || chunk = Word_val then
    ...
    method emit_expr (env : environment) exp =
      ...
      | Cifthenelse (econd, _ifso_dbg, eif, _ifnot_dbg, eelse, _dbg) ->
        ...
        | Some rarg ->
          ...
          self#insert env (Iifthenelse (cond, sif#extract, selse#extract)) rarg
            [| |];
          r
        end
      | Cswitch (esel, index, ecases, _dbg) ->
        ...
        | Some rsel ->
          ...
          self#insert env
            (Iswitch (index, Array.map (fun (_, s) -> s#extract) rscases)) rsel
            [| |];
          r
        end
        ...
        let env =
          ...
          List.fold_left
            (fun env (nfail, _ids, rs, _e2, _dbg) ->
               env_add_static_exception nfail rs env) env handlers
        in
        let (r_body, s_body) = self#emit_sequence env body in
        let translate_one_handler (nfail, ids, rs, e2, _dbg) =
          assert (List.length ids = List.length rs);
          let new_env =
            List.fold_left (fun env ((id, _typ), r) -> env_add id r env) env
              (List.combine ids rs)
          in
      ...
      | Ctrywith (e1, v, e2, _dbg) ->
        ...
        self#insert env
          (Itrywith
             (s1#extract,
              instr_cons (Iop Imove) [| Proc.loc_exn_bucket |] rv s2#extract))
          [| |] [| |];
        r
    ...
    method private emit_parts (env : environment) ~effects_after exp =
      ...
      let may_defer_evaluation =
        ...
        | Effect.None ->
          begin match EC.coeffect ec with
          | Coeffect.None ->
          ...
          | Coeffect.Read_mutable ->
            (* Read-mutable expressions may only be deferred if evaluation of
               ...
               an exception. *)
            begin match EC.effect effects_after with
      ...
      if may_defer_evaluation && self#is_simple_expr exp then
        ...
        | Some r ->
          if Array.length r = 0 then
            ...
            else (* Introduce a fresh temp to hold the result *)
    ...
    method private emit_parts_list (env : environment) exp_list =
      ...
      List.fold_left
        (fun results_and_env (exp, effects_after) ->
           ...
           | None -> None
           | Some (result, env) ->
             begin match self#emit_parts env exp ~effects_after with
             | None -> None
             | Some (exp_result, env) -> Some (exp_result :: result, env)
             end) (Some ([], env)) exp_list_right_to_left
    ...
    method private emit_tuple_not_flattened env exp_list =
      let rec emit_list =
        ...
        | exp :: rem ->
          ...
          begin match self#emit_expr env exp with
    ...
    method emit_tail (env : environment) exp =
      ...
      | Cop ((Capply ty as op), args, dbg) ->
        ...
        | Some (simple_args, env) ->
          ...
          | Icall_imm { func } ->
            ...
            else if func = !current_function_name then
              let call = Iop (Itailcall_imm { func }) in
          ...
          self#insert env
            (Iifthenelse
               (cond,
                ...
                self#emit_tail_sequence env eelse)) rarg [| |]
        end
      | Cswitch (esel, index, ecases, _dbg) ->
        begin match self#emit_expr env esel with
        | None -> ()
      ...
      | Ctrywith (e1, v, e2, _dbg) ->
        ...
        self#insert env
          (Itrywith
             (s1#extract, instr_cons (Iop Imove) [| Proc.loc_exn_bucket |] rv s2))
          [| |] [| |];
    ...
    method emit_fundecl ~future_funcnames f =
      ...
      let env =
        List.fold_right2 (fun (id, _ty) r env -> env_add id r env)
      ...
      instr_iter (fun instr -> self#mark_instr instr.Mach.desc)
        body_with_prologue;
      {
