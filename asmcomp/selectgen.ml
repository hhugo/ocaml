...
let oper_result_type = function
  ...
  | Caddi | Csubi | Cmuli | Cmulhi | Cdivi | Cmodi | Cand | Cor | Cxor | Clsl
  | Clsr | Casr | Ccmpi _ | Ccmpa _ | Ccmpf _ ->
      typ_int
  | Caddv -> typ_val
  | Cadda -> typ_addr
    ...
    *)
    ...
    method effects_of exp =
      ...
      | Cop (op, args, _) ->
          let from_op =
            ...
            | Clsl | Clsr | Casr | Ccmpi _ | Caddv | Cadda | Ccmpa _ | Cnegf
    ...
    method emit_expr (env : environment) exp =
      ...
      | Cconst_float (n, _dbg) ->
          ...
          Some
            (self#insert_op env (Iconst_float (Int64.bits_of_float n)) [||] r)
      ...
      | Cop (op, args, dbg) -> (
          ...
          | Some (simple_args, env) -> (
              ...
              | Ialloc { bytes = _ } ->
                  ...
                  let op =
                    Ialloc
                      { bytes; dbginfo = [ { alloc_words; alloc_dbg = dbg } ] }
                  in
    ...
    method private emit_parts (env : environment) ~effects_after exp =
      ...
      if may_defer_evaluation && self#is_simple_expr exp then Some (exp, env)
      else
        match self#emit_expr env exp with
        | None -> None
        | Some r ->
            if Array.length r = 0 then Some (Ctuple [], env)
            else
              (* The normal case *)
    ...
    method emit_tail (env : environment) exp =
      ...
      | Ctrywith (e1, v, e2, _dbg) -> (
          ...
          | Some r1 ->
              ...
              self#insert env Ireturn loc [||])
      | Cop _ | Cconst_int _ | Cconst_natint _ | Cconst_float _
      | Cconst_symbol _ | Cvar _ | Cassign _ | Ctuple _ | Cexit _ ->
    ...
    method emit_fundecl ~future_funcnames f =
      ...
      let polled_body =
        ...
        then instr_cons (Iop (Ipoll { return_label = None })) [||] [||] body
        else body
      in
