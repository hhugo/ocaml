...
let import_descr_for_pack units pack (descr : Export_info.descr) :
    Export_info.descr =
  match descr with
  | ( Value_int _ | Value_char _ | Value_constptr _ | Value_string _
    | Value_float _ | Value_float_array _ | Export_info.Value_boxed_int _
  ...
  | Value_closure {closure_id; set_of_closures} ->
      Value_closure
        { closure_id
        ; set_of_closures =
            import_set_of_closures units pack set_of_closures }
