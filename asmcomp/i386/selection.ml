...
let rec select_addr exp =
  ...
  | Cop ((Caddi | Cadda | Caddv), [arg1; arg2], _) -> (
    ...
    | (Alinear e1, n1), (Alinear e2, n2) -> Aadd (e1, e2), n1 + n2
...
let rec float_needs = function
  | Cop ((Cnegf | Cabsf), [arg], _) -> float_needs arg
...
let pseudoregs_for_operation op arg res =
  ...
  | Iintop Idiv -> [|eax; ecx|], [|eax|], true
  | Iintop Imod -> [|eax; ecx|], [|edx|], true
  ...
  | Iconst_float _ | Inegf | Iabsf | Iaddf | Isubf | Imulf | Idivf
   |Ifloatofint
   |Iload ((Single | Double | Double_u), _)
...
class selector =
  object (self)
    ...
    method select_addressing _chunk exp =
      ...
      | Asymbol s, d -> Ibased (s, d), Ctuple []
      | Alinear e, d -> Iindexed d, e
      | Aadd (e1, e2), d -> Iindexed2 d, Ctuple [e1; e2]
      | Ascale (e, scale), d -> Iscaled (scale, d), e
    ...
    method! select_store is_assign addr exp =
      match exp with
      | Cconst_int (n, _) ->
          ( Ispecific (Istore_int (Nativeint.of_int n, addr, is_assign))
          , Ctuple [] )
      ...
      | _ -> super#select_store is_assign addr exp
    ...
    method! select_operation op args dbg =
     ...
     In passing, apply Ershov's algorithm to reduce stack usage *)
      | Caddf -> self#select_floatarith Iaddf Iaddf Ifloatadd Ifloatadd args
      | Csubf ->
          self#select_floatarith
            Isubf
            (Ispecific Isubfrev)
            Ifloatsub
            Ifloatsubrev
            args
      | Cmulf -> self#select_floatarith Imulf Imulf Ifloatmul Ifloatmul args
      | Cdivf ->
          self#select_floatarith
            Idivf
            (Ispecific Idivfrev)
      ...
      | Cstore (((Word_int | Word_val) as chunk), _) -> (
        match args with
        | [ loc
          ; Cop (Caddi, [Cop (Cload _, [loc'], _); Cconst_int (n, _)], _) ]
          when loc = loc' ->
    ...
    method select_floatarith regular_op reversed_op mem_op mem_rev_op args =
      ...
      | _ -> fatal_error "Proc_i386: select_floatarith"
    ...
    method! mark_c_tailcall = Proc.contains_calls := true

    method! emit_extcall_args env args =
      let rec size_pushes = function
        | [] -> 0
        | e :: el -> Selectgen.size_expr env e + size_pushes el
      in
      ...
      let rec emit_pushes = function
        | [] ->
            if sz2 > sz1
            then self#insert env (Iop (Istackoffset (sz2 - sz1))) [||] [||]
        | e :: el -> (
            emit_pushes el;
            let op, arg = self#select_push e in
            match self#emit_expr env arg with
            | None -> ()
            | Some r -> self#insert env (Iop op) r [||] )
      in
      emit_pushes args;
      [||], sz2
  end
