...
let masm = match Config.ccomp_type with "msvc" -> true | _ -> false
...
let int_reg_name =
  if masm then [| "eax"; "ebx"; "ecx"; "edx"; "esi"; "edi"; "ebp" |]
  else [| "%eax"; "%ebx"; "%ecx"; "%edx"; "%esi"; "%edi"; "%ebp" |]

let float_reg_name = if masm then [| "tos" |] else [| "%tos" |]
...
let calling_conventions first_int last_int first_float last_float make_stack arg
    =
...
let loc_arguments arg = calling_conventions 0 5 100 99 outgoing arg

let loc_parameters arg =
  let loc, _ofs = calling_conventions 0 5 100 99 incoming arg in
  loc

let loc_results res =
  let loc, _ofs = calling_conventions 0 5 100 100 not_supported res in
  loc

let max_arguments_for_tailcalls = 6 (* in registers *) + 64
(* in domain state *)

let loc_external_arguments _arg = fatal_error "Proc.loc_external_arguments"

let loc_external_results res =
  match res with
  | [| Int; Int |] -> [| eax; edx |]
  | _ ->
      let loc, _ofs = calling_conventions 0 0 100 100 not_supported res in
      loc
...
let max_register_pressure = function
  ...
  | Ialloc _ | Iintop (Icomp _) | Iintop_imm (Icomp _, _) | Iintoffloat ->
      [| 6; max_int |]
  | _ -> [| 7; max_int |]
...
let frame_required fd =
  let frame_size_at_top_of_function =
    ...
    Misc.align
      ((4 * fd.fun_num_stack_slots.(0)) + (8 * fd.fun_num_stack_slots.(1)) + 4)
      stack_alignment
  in
