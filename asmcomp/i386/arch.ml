...
type specific_operation =
  ...
  | Isubfrev
  | Idivfrev (* Reversed float sub and div *)
...
let print_specific_operation printreg op ppf arg =
  ...
  | Istore_int (n, addr, is_assign) ->
    fprintf ppf "[%a] := %nd %s" (print_addressing printreg addr) arg n
      (if is_assign then "(assign)" else "(init)")
  | Istore_symbol (lbl, addr, is_assign) ->
    fprintf ppf "[%a] := \"%s\" %s" (print_addressing printreg addr) arg lbl
      (if is_assign then "(assign)" else "(init)")
  | Ioffset_loc (n, addr) ->
  ...
  | Ipush_int n -> fprintf ppf "push %s" (Nativeint.to_string n)
  | Ipush_symbol s -> fprintf ppf "push \"%s\"" s
  | Ipush_load addr ->
  ...
  | Isubfrev -> fprintf ppf "%a -f(rev) %a" printreg arg.(0) printreg arg.(1)
  | Idivfrev -> fprintf ppf "%a /f(rev) %a" printreg arg.(0) printreg arg.(1)
