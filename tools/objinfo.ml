...
let p_list title print = function
  | [] -> ()
  | l -> p_title title; List.iter print l
...
let dump_obj filename =
  ...
  if magic_number = cmo_magic_number
  then (
    let cu_pos = input_binary_int ic in
    seek_in ic cu_pos;
    let cu : compilation_unit = input_value ic in
    close_in ic; print_cmo_infos cu )
  else if magic_number = cma_magic_number
  then (
    let toc_pos = input_binary_int ic in
    seek_in ic toc_pos;
    let toc : library = input_value ic in
    close_in ic; print_cma_infos toc )
  else if magic_number = cmi_magic_number || magic_number = cmt_magic_number
  then (
    close_in ic;
  ...
  else if magic_number = cmx_magic_number
  then (
    let ui : unit_infos = input_value ic in
    let crc = Digest.input ic in
    close_in ic;
    print_cmx_infos (ui, crc) )
  else if magic_number = cmxa_magic_number
  then (
    let li : library_infos = input_value ic in
    close_in ic; print_cmxa_infos li )
  else
    let pos_trailer = in_channel_length ic - len_magic_number in
    let _ = seek_in ic pos_trailer in
    let magic_number = really_input_string ic len_magic_number in
    if magic_number = Config.exec_magic_number
    then ( dump_byte ic; close_in ic )
    else if Filename.check_suffix filename ".cmxs"
    then (
      flush stdout;
      ...
      | None ->
          ...
          exit 2
      | Some header ->
          if header.dynu_magic = Config.cmxs_magic_number
          then print_cmxs_infos header
          else (
