...
let skip_section name = name = "DBUG"
...
let cmpbyt file1 file2 =
  ...
  let rec cmpsections t1 t2 =
    ...
    | (name1, len1) :: t1, (name2, len2) :: t2 -> (
        ...
        then (
          eprintf
            "Section mismatch: %s (in %s) / %s (in %s)\n"
            name1
            file1
            name2
            file2;
        ...
        then (
          eprintf
            "Length of section %s differ: %d (in %s) / %d (in %s)\n"
            name1
            len1
            file1
            len2
          ...
          | Differ ofs ->
              eprintf
                "Files %s and %s differ: section %s, offset %d\n"
                file1
                file2
                name1
                ofs;
              false
          | Same -> cmpsections t1 t2 )
  in
  let res = cmpsections toc1 toc2 in
  close_in ic1; close_in ic2; res
