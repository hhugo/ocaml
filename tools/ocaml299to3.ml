    ...
    while true do
      ...
      | LABEL l0 -> (
          ...
          | PREFIXOP "?(" ->
              ...
              input_pos := stop
          | QUESTION | LPAREN | LBRACE | SEMI | MINUSGREATER | EQUAL
           |COLON | COLONGREATER | VAL | MUTABLE | EXTERNAL | METHOD | OF ->
              if l0 = "fun"
              then (
                modified := true;
                copy_input start;
      ...
      | _ -> () );
      if last_token = QUESTION && token = LPAREN
      then last := PREFIXOP "?(", last_start, stop
      else last := token, start, stop
    done
