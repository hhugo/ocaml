        ...
        (fun inherits -> function
          | {psig_desc = Psig_attribute a; _}
            when Doc.get_doc ["ocaml.doc"; "ocaml.text"] [a] <> None ->
              f inherits Location.none [a] | _ -> inherits )
        inherits
        items
    in
...
module Diff = struct
    ...
    let info_seen ppf = function
      ...
      | Some a ->
          ...
          | Some v ->
              Format.fprintf
                ppf
                "%s was not seen in version %a"
                k
                Version.pp
                v
          | None ->
              Format.fprintf ppf "%s was seen in all analyzed versions" k );
          Format.fprintf
            ppf
            "@,%s was seen in version %a"
            k
            Version.pp
            a.first_seen;
          if a.deprecated
  ...
  let file path tags =
    ...
    in
    let current =
      Ast.parse_file ~orig:path ~f:Doc.get ~init:Doc.empty path
    in
