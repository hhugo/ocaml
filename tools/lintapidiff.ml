...
let ignore_changes_for =
  ...
  (* discarded by stop comments: *) "type Unix.map_file_impl";
...
module Version : sig
  ...
  let of_string_exn str =
    ...
    with
    | _ -> Scanf.sscanf str "%u.%u" (fun a b -> a, b, 0)
    ...
    Misc.try_finally
      (fun () ->
         ...
         | r ->
           ...
           Error `Exit) (fun () -> Misc.remove_file tmp)
end
...
module Diff = struct
  ...
  let parse_file_at_rev ~path (prev, accum) rev =
    ...
    let f =
      Ast.parse_file ~orig:path ~init:empty
        ~f:(fun _ _ attrs ->
          {
            last_not_seen = None;
            first_seen;
  ...
  let check_changes ~first ~last default k seen latest =
    ...
    in
    if List.mem k ignore_changes_for then
      None (* ignored *)
    else
      let open! Doc in
  ...
  let file path tags =
    ...
    and last = List.hd (List.rev tags) |> Version.of_string_exn
    in
  ...
  let count =
    List.fold_left
      (fun count path ->
         ...
         count + IdMap.cardinal problems) 0 paths
  in
