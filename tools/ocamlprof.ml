...
let insert_profile rw_exp ex =
  ...
  if gh || st = en
  then rw_exp true ex
  else ( insert_action st en; rw_exp false ex )
...
and rw_exp iflag sexp =
  ...
  | Pexp_function caselist ->
      if !instr_fun
      then rewrite_function iflag caselist
  ...
  | Pexp_match (sarg, caselist) ->
      ...
      if !instr_match && not sexp.pexp_loc.loc_ghost
      then rewrite_funmatching caselist
      else rewrite_cases iflag caselist
  | Pexp_try (sbody, caselist) ->
      rewrite_exp iflag sbody;
      if !instr_try && not sexp.pexp_loc.loc_ghost
      then rewrite_trymatching caselist
      else rewrite_cases iflag caselist
  ...
  | Pexp_setfield (srecord, _, snewval) ->
      rewrite_exp iflag srecord; rewrite_exp iflag snewval
  ...
  | Pexp_sequence (sexp1, sexp2) ->
      rewrite_exp iflag sexp1; rewrite_exp iflag sexp2
...
and rewrite_ifbody iflag ghost sifbody =
  if !instr_if && not ghost
  then insert_profile rw_exp sifbody
...
and rewrite_class_field iflag cf =
  ...
  | Pcf_method
      ...
      , _
      , Cfk_concrete
          (_, ({pexp_desc = Pexp_function _ | Pexp_fun _} as sexp)) ) ->
      rewrite_exp iflag sexp
  | Pcf_method (_, _, Cfk_concrete (_, sexp)) ->
      let loc = cf.pcf_loc in
      if !instr_fun && not loc.loc_ghost
      then insert_profile rw_exp sexp
...
and rewrite_mod iflag smod =
  ...
  | Pmod_apply (smod1, smod2) ->
      rewrite_mod iflag smod1; rewrite_mod iflag smod2
...
and rewrite_str_item iflag item =
  ...
  | Pstr_module x ->
      rewrite_mod iflag x.pmb_expr (* todo: Pstr_recmodule?? *)
  | Pstr_class classes ->
      List.iter (rewrite_class_declaration iflag) classes
  | _ -> ()
...
let process_impl_file filename =
  ...
  then (
    ...
    let modes, cv =
      ...
      with Not_found ->
        raise
          (Profiler ("Module " ^ modname ^ " not used in this profile."))
    in
    counters := cv;
    set_flags modes;
    init_rewrite modes modname;
...
let process_anon_file filename =
  ...
  then process_impl_file filename
