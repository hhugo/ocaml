...
let add_incr_counter modul (kind, pos) =
  ...
  match kind with
  | Open ->
    fprintf !outchan "(%sProfiling.incr %s%s_cnt %d; " modprefix idprefix modul
      !prof_counter;
    incr prof_counter
    ...
    (fprintf !outchan "module %sProfiling = Profiling;; " modprefix;
     ...
     fprintf !outchan
       " 0;; Profiling.counters := (\"%s\", (\"%s\", %s%s_cnt)) :: \
        !Profiling.counters;; " mod_name modes idprefix mod_name)
  ...
  List.iter
    (function
     ...
     | { pc_rhs = { pexp_desc = Pexp_constraint (sbody, _) } }
       (* let f x : t = e *) ->
       insert_profile rw_exp sbody
     | { pc_rhs = sexp } -> insert_profile rw_exp sexp) l

and rewrite_function iflag =
  function
  | [
...
and rewrite_class_field iflag cf =
  ...
  | Pcf_method (_, _, Cfk_concrete (_, sexp)) ->
    ...
    if !instr_fun && not loc.loc_ghost then
