...
let convert_impl buffer =
  ...
  let next_token () =
    ...
    match token with
    | RPAREN | RBRACKET | BARRBRACKET | GREATERRBRACKET | END | RBRACE
     |GREATERRBRACE ->
        raise (Closing token)
  ...
  and dropext (token, s, e) =
    ...
    | DOT -> (
        ...
        | LPAREN | LBRACKET | LBRACE ->
            ...
            dropext (next_token ())
        | UIDENT _ | LIDENT _ -> dropext (next_token ())
        | _ ->
  ...
  and may_discard (token, s, e) =
    ...
    | _ when !alllabels -> may_discard (next_token ())
    | LPAREN | LBRACKET | LBRACKETBAR | LBRACKETLESS | BEGIN | LBRACE
     |LBRACELESS | STRUCT | SIG | OBJECT ->
        process_paren (token, s, e);
        may_discard (next_token ())
    | PREFIXOP _ -> may_discard (next_token ())
    | LIDENT _ | UIDENT _ -> may_discard (dropext (next_token ()))
    | BACKQUOTE ->
        ignore (next_token ());
        may_discard (next_token ())
  ...
  and search_start (token, s, e) =
    ...
    | LPAREN | LBRACKET | LBRACKETBAR | LBRACKETLESS | BEGIN | LBRACE
     |LBRACELESS | STRUCT | SIG | OBJECT ->
        process_paren (token, s, e);
        search_start (next_token ())
    | EQUAL | SEMI | SEMISEMI | MINUSGREATER | LESSMINUS | COMMA | IF
     |THEN | ELSE | WHILE | TO | DOWNTO | DO | IN | MATCH | TRY
     |INFIXOP0 _ | INFIXOP1 _ | INFIXOP2 _ | INFIXOP3 _ | INFIXOP4 _
     |PLUS | MINUS | MINUSDOT | STAR | LESS | GREATER | OR | BARBAR
     |AMPERSAND | AMPERAMPER | COLONEQUAL ->
  ...
  try if !alllabels then may_discard first else may_start first with
  | End_of_file ->
      copy_input (Buffer.length input_buffer);
      if (not !alllabels)
         && List.exists
              (fun r -> !r = Some true)
              [openstd; openunix; openmore]
      then (
        modified := true;
...
let convert_intf buffer =
  ...
  try
    while true do
      ...
      ( match token with
      | EXCEPTION | CONSTRAINT -> state := In
      | VAL | EXTERNAL | CLASS | METHOD | TYPE | AND -> state := Enter
      | EQUAL when !state = Enter -> state := In
      | COLON -> (
        ...
        | Escape, _ -> state := In
        | _ -> state := Out )
      | (LBRACE | SEMI | QUESTION) when !state = In -> state := Escape
  ...
  with End_of_file -> copy_input (Buffer.length input_buffer)
...
let convert_file ~intf name =
  ...
  (let convert = if intf then convert_intf else convert_impl in
   try
     convert (Lexing.from_function (input_function ic));
     close_in ic
...
let _ =
  let files = ref []
  and intf = ref false
  and keepstd = ref false
  and keepmore = ref false in
  Arg.parse
    [ ( "-intf"
      , Arg.Set intf
    ...
    ; ( "-m"
      , Arg.String (fun s -> modules := s :: !modules)
  ...
  List.iter (List.rev !files) ~f:(fun name ->
