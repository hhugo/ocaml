...
let parse_arguments argv =
  ...
  while not (Stack.is_empty args) do
    ...
    if s = "-args"
    then push_args ~first:0 (Arg.read_arg (next_arg s))
    else if s = "-args0"
    then push_args ~first:0 (Arg.read_arg0 (next_arg s))
    else if ends_with s ".cmo" || ends_with s ".cma"
    then bytecode_objs := s :: !bytecode_objs
    else if ends_with s ".cmx" || ends_with s ".cmxa"
    then native_objs := s :: !native_objs
    else if ends_with s ".ml" || ends_with s ".mli"
    then (
    ...
    then caml_libs := next_arg s :: "-cclib" :: !caml_libs
    else if s = "-ccopt"
    then caml_opts := next_arg s :: "-ccopt" :: !caml_opts
    else if s = "-custom"
    then dynlink := false
    else if s = "-I"
    then caml_opts := next_arg s :: "-I" :: !caml_opts
    else if s = "-failsafe"
    then failsafe := true
    else if s = "-g"
    then debug := true
    else if s = "-h" || s = "-help" || s = "--help"
    then raise (Bad_argument "")
    else if s = "-ldopt"
    then ld_opts := next_arg s :: !ld_opts
    else if s = "-linkall"
    then caml_opts := s :: !caml_opts
    else if starts_with s "-l"
    then
      let s =
        if Config.ccomp_type = "msvc"
        then String.sub s 2 (String.length s - 2) ^ ".lib"
        else s
      in
      c_libs := s :: !c_libs
    else if starts_with s "-L"
    ...
    else if s = "-ocamlcflags"
    then ocamlc_opts := next_arg s :: !ocamlc_opts
    else if s = "-ocamlc"
    then ocamlc := next_arg s
    else if s = "-ocamlopt"
    then ocamlopt := next_arg s
    else if s = "-ocamloptflags"
    then ocamlopt_opts := next_arg s :: !ocamlopt_opts
    else if s = "-o"
    then output := next_arg s
    else if s = "-oc"
    then output_c := next_arg s
    else if s = "-dllpath" || s = "-R" || s = "-rpath"
    then rpath := next_arg s :: !rpath
    else if starts_with s "-R"
    then rpath := chop_prefix s "-R" :: !rpath
    else if s = "-Wl,-rpath"
    then
      let a = next_arg s in
      if starts_with a "-Wl,"
    ...
    else if starts_with s "-Wl,-rpath,"
    then rpath := chop_prefix s "-Wl,-rpath," :: !rpath
    else if starts_with s "-Wl,-R"
    then rpath := chop_prefix s "-Wl,-R" :: !rpath
    else if s = "-v" || s = "-verbose"
    then verbose := true
    else if s = "-version"
    then print_version ()
    else if s = "-vnum"
    then print_version_num ()
    else if starts_with s "-F"
    then c_opts := s :: !c_opts
    else if s = "-framework"
    then
      let a = next_arg s in
      c_opts := a :: s :: !c_opts
    else if starts_with s "-"
    then prerr_endline ("Unknown option " ^ s)
...
let usage =
  "Usage: ocamlmklib [options] \
   <.cmo|.cma|.cmx|.cmxa|.ml|.mli|.o|.a|.obj|.lib|.dll|.dylib files>\n\
   Options are:\n\
  \  -args <file>   Read additional newline-terminated command line \
   arguments\n\
  \                 from <file>\n\
  ...
  \  -o <name>      Generated OCaml library is named <name>.cma or \
   <name>.cmxa\n\
...
let transl_path s =
  ...
  | "Win32" ->
      ...
      let rec aux i =
        if i = Bytes.length s || Bytes.get s i = ' '
        then s
        else (
...
let _ =
  try parse_arguments Sys.argv; build_libs () with
  | Bad_argument "" -> prerr_string usage; exit 0
  | Bad_argument s -> prerr_endline s; prerr_string usage; exit 4
  | Sys_error s ->
