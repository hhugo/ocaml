...
let code_env ?(newline = true) env out s =
  ...
  Format.fprintf
    out
    "%a%s%s%a"
    (fun ppf env -> start false ppf env [])
    env
    s
    sep
    (stop newline)
...
module Toplevel = struct
  ...
  let fatal ic oc fmt =
    Format.kfprintf
      (fun ppf ->
        Format.fprintf ppf "@]@."; close_in ic; close_out oc; exit 1 )
      self_error_fmt
      ("@[<hov 2>  Error " ^^ fmt)
  ...
  let parse fname mode s =
    ...
    try
      ...
      | Signature ->
          ...
          let str =
            Ast_helper.
              [Str.modtype @@ Mtd.mk ~typ:(Mty.signature sign) name]
          in
          Ptop_def str
  ...
  let read_output () =
    ...
    let values =
      replace_first ~!{|^#\( *\*\)* *|} "" @@ flush_fmt out_fmt
    in
    ...
    then
      ...
      Toplevel.eprintf
        "Error when evaluating a guarded caml_example environment in %a\n\
         ...
         If %a status was in fact expected, change the status annotation \
         to [@@expect %a].\n"
        print_source
        source
        pp_status
    ...
    | Option ->
        Toplevel.eprintf
          "Unknown caml_example option: [%s].\n\
           Supported options are \"ok\",\"error\", or \"warning=n\" (with \
           n a warning number).\n"
          s
    | Annotation ->
        Toplevel.eprintf
...
module Text_transform = struct
  ...
  let rec apply_transform input (pos, underline_stop, out) t =
    ...
    then pos, underline_stop, out
      ...
      | Some stop when stop <= t.start ->
          let f =
            escape_specials (String.sub input ~pos ~len:(stop - pos))
          in
          let out = {|\>|} :: f :: out in
          apply_transform input (stop, None, out) t
      | _ -> (
          let out =
            escape_specials (String.sub input ~pos ~len:(t.start - pos))
            :: out
          in
          match t.kind with
          | Ellipsis -> t.stop, underline_stop, {|\ldots|} :: out
          | Underline -> t.start, Some t.stop, {|\<|} :: out )
  ...
  let apply ts file line s =
    ...
    let last, ls =
      ...
      | Some stop ->
          let f =
            escape_specials (String.sub s ~pos:last ~len:(stop - last))
          in
          stop, {|\>|} :: f :: ls
    in
    let ls =
...
let process_file file =
      ...
      then (
        ...
        try
          while true do
            ...
            let error_msgs =
              String.concat "" (out.warnings @ [out.error])
            in
            let output =
            ...
            let final_output =
              if omit_answer && String.length error_msgs > 0
              then global_replace ~!{|^\(.\)|} camlout error_msgs
              else if omit_answer
              then ""
              else output
            in
  ...
  | Missing_double_semicolon (file, line_number) ->
      fatal
        "when evaluating a caml_example environment in %s:@;missing \";;\" \
         at line %d"
        file
        (line_number - 2)
  | Missing_mode (file, line_number) ->
      fatal
        "when parsing a caml_example environment in %s:@;missing mode \
         argument at line %d,@ available modes {toplevel,verbatim}"
        file
        (line_number - 2)
  | Incompatible_options (Signature_with_visible_answer (file, line_number))
    ->
      fatal
        "when parsing a caml_example environment in@ %s, line %d:@,the \
         signature mode is only compatible with \"caml_example*\"@ Hint: \
  ...
  | Text_transform.Intersection {line; file; left; right} ->
      fatal
        "when evaluating a caml_example environment in %s, line %d:@ \
         ...
         spanned the interval %d-%d,@ intersecting with another \"%a\" \
         transform @ on the %d-%d interval.@ Hind: did you try to elide a \
         code fragment which raised a warning?"
        file
        (line - 2)
        Text_transform.pp
        left.kind
  ...
  | Ellipsis.Unmatched_ellipsis {kind; start; stop} ->
      fatal
        "when evaluating a caml_example environment,@ the %s mark at \
         position %d-%d was unmatched"
        kind
        start
        stop
  | Ellipsis.Nested_ellipses {first; second} ->
      fatal
        "when evaluating a caml_example environment,@ there were two \
         nested ellipsis attribute.@ The first one started at position \
         %d,@ the second one at %d"
        first
        second
