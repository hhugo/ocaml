...
let code_env env out s =
  ...
  Format.fprintf out "%a%s%s%a" (fun ppf env -> start ppf env []) env s sep stop
    env
  ...
  let fatal ic oc fmt =
    Format.kfprintf
      (fun ppf ->
         ...
         close_in ic;
         close_out oc;
         exit 1) self_error_fmt ("@[<hov 2>  Error " ^^ fmt)
  ...
  let parse fname mode s =
    ...
    try
      ...
      | Signature ->
        ...
        Ptop_def str
    with
    | Lexer.Error _ | Syntaxerr.Error _ -> raise (Phrase_parsing s)
...
module Output = struct
  ...
  exception Parsing_error of kind * string (** {1 Exceptions } *)
    ...
    (if expected = Ok then
       ...
       Toplevel.eprintf
         "Error when evaluating a guarded caml_example environment in %a\n\
          ...
          [@@expect %a].\n" print_source source pp_status got pp_a_status
         expected pp_a_status got pp_status got);
    flush stderr
  ...
  let check_partition line file l =
    ...
    and underline u n =
      ...
      | ({ kind = Ellipsis; _ } as t) :: q ->
        if t.stop < u.stop then
          underline u (t :: n) q
        else
          end_underline u n (t :: q)
    and end_underline u n l = U (u, List.rev n) :: partition l
    in
  ...
  let apply ts file line s =
    ...
    let ls =
      ...
      if last = n then
        ...
        escape_specials (String.sub s last (n - last)) :: ls
    in
...
module Ellipsis = struct
  ...
  exception
    Unmatched_ellipsis of { kind : string; start : int; stop : int }
    (** raised when an [[@@@ellipsis.start]] or [[@@@ellipsis.stop]] is
        not paired with another ellipsis attribute *)
  
  exception
    Nested_ellipses of { first : int; second : int }
  ...
  let extract f x =
    ...
    let attribute _this attr =
      let module L = Location 
      in
      let module P = Parsetree 
      in
...
let process_file file =
  let ic =
    try open_in file
    with
    | _ -> failwith "Cannot read input file"
  in
  let phrase_start = ref 1
  and phrase_stop = ref 1
  in
  ...
  let oc =
    try
      if !outfile = "-" then
        ...
        open_out_gen [ Open_wronly; Open_creat; Open_append; Open_text ] 0x666
          !outfile
    with
    | _ -> failwith "Cannot open output file"
  in
           ...
           let rec read () =
             ...
             if not stop then
               ...
               (decr phrase_stop;
                ...
                let expected =
                  if
                    string_match ~!{|\(.*\)\[@@expect \(.*\)\]|} last_input 0
                  then
  ...
  | Incompatible_options (Signature_with_visible_answer (file, line_number)) ->
    fatal
      "when parsing a caml_example environment in@ %s, line %d:@,\
       ...
       did you forget to add \"*\"?" file (line_number - 2)
  | Text_transform.Intersection { line; file; left; right } ->
    fatal
      "when evaluating a caml_example environment in %s, line %d:@ Textual \
       ...
       raised a warning?" file (line - 2) Text_transform.pp left.kind left.start
      left.stop Text_transform.pp right.kind right.start right.stop
  | Ellipsis.Unmatched_ellipsis { kind; start; stop } ->
    fatal
      "when evaluating a caml_example environment,@ the %s mark at position \
       %d-%d was unmatched" kind start stop
