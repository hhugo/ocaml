...
let rec labels_of_cty cty =
  ...
  | Pcty_signature {pcsig_fields = fields} ->
      ( []
      , List.fold_left fields ~init:[] ~f:(fun meths -> function
          | {pctf_desc = Pctf_meth (s, _, sty)} ->
              (s, labels_of_sty sty) :: meths
          | _ -> meths ) )
  | _ -> [], []
...
let rec pattern_vars pat =
  ...
  | Ppat_var s -> [s.txt]
  | Ppat_alias (pat, s) -> s.txt :: pattern_vars pat
  | Ppat_tuple l | Ppat_array l -> List.concat (List.map pattern_vars l)
  ...
  | Ppat_or (pat1, pat2) -> pattern_vars pat1 @ pattern_vars pat2
...
let rec insertion_point pos ~text =
  ...
  while is_space text.[!pos'] do
    decr pos'
  done;
  if text.[!pos'] = '('
  then insertion_point !pos' ~text
  else if !pos' >= 5
          && String.sub text ~pos:(!pos' - 4) ~len:5 = "begin"
          && not (is_alphanum text.[!pos' - 5])
  then insertion_point (!pos' - 4) ~text
  else pos
...
let rec insertion_point2 pos ~text =
  ...
  if text.[!pos'] = '('
  then insertion_point2 !pos' ~text
  else if !pos' >= 5
          && String.sub text ~pos:(!pos' - 4) ~len:5 = "begin"
          && not (is_alphanum text.[!pos' - 5])
  then insertion_point2 (!pos' - 4) ~text
  else if text.[!pos'] = '='
  then Some !pos'
  else if !pos' >= 1 && text.[!pos' - 1] = '-' && text.[!pos'] = '>'
  then Some (!pos' - 1)
  else None
...
let rec insert_labels ~labels ~text expr =
  ...
  | l :: labels, Pexp_function (l', _, lst) ->
      ...
      if l <> ""
         && l.[0] <> '?'
         && l' = ""
         && String.sub text ~pos ~len:8 = "function"
      then (
        String.blit ~src:"match th" ~src_pos:0 ~dst:text ~dst_pos:pos ~len:8;
        add_insertion (pos + 6) (l ^ " wi");
        match insertion_point2 pos ~text with
        | Some pos' -> add_insertion pos' ("~" ^ l ^ " ")
        | None -> add_insertion pos ("fun ~" ^ l ^ " -> ") );
...
let insert_labels_app ~labels ~text args =
  let labels, opt_labels =
    List.partition labels ~f:(fun l -> l = "" || l.[0] <> '?')
  in
  let nopt_labels =
    List.map opt_labels ~f:(fun l ->
        String.sub l ~pos:1 ~len:(String.length l - 1) )
  in
  (* avoid ambiguous labels *)
  if List.exists labels ~f:(List.mem ~set:nopt_labels)
  then ()
  else
...
let rec add_labels_expr ~text ~values ~classes expr =
  ...
  | Pexp_apply
      ( { pexp_desc =
            Pexp_send
              ({pexp_desc = Pexp_ident {txt = Longident.Lident s}}, meth) }
      , args ) -> (
    try
      if SMap.find s values = ["<object>"]
      then
  ...
  | Pexp_let (recp, lst, expr) ->
      let vars =
        List.concat (List.map lst ~f:(fun (p, _) -> pattern_vars p))
      in
      let vals = SMap.removes vars values in
      List.iter lst ~f:(fun (_, e) ->
          add_labels_rec
            e
            ~values:(if recp = Recursive then vals else values) );
      add_labels_rec expr ~values:vals
  | Pexp_function (_, None, lst) ->
      List.iter lst ~f:(fun (p, e) ->
          add_labels_rec e ~values:(SMap.removes (pattern_vars p) values) )
  | Pexp_function (_, Some e, lst) | Pexp_match (e, lst) | Pexp_try (e, lst)
    ->
      add_labels_rec e;
      List.iter lst ~f:(fun (p, e) ->
          add_labels_rec e ~values:(SMap.removes (pattern_vars p) values) )
  | Pexp_apply (e, args) -> List.iter add_labels_rec (e :: List.map snd args)
  | Pexp_tuple l | Pexp_array l -> List.iter add_labels_rec l
  ...
  | Pexp_record (lst, opt) -> (
      ...
      match opt with Some e -> add_labels_rec e | None -> () )
  ...
  | Pexp_override lst -> List.iter lst ~f:(fun (_, e) -> add_labels_rec e)
...
let rec add_labels_class ~text ~classes ~values ~methods cl =
  ...
  | Pcl_structure {pcstr_self = p; pcstr_fields = l} ->
      ...
      in
      ignore
        (List.fold_left l ~init:values ~f:(fun values -> function
           | e -> (
             match e.pcf_desc with
             | Pcf_val (s, _, _, e) ->
             ...
             | Pcf_init e ->
                 add_labels_expr ~text ~classes ~values e;
                 values
             | Pcf_inher _ | Pcf_valvirt _ | Pcf_virt _ | Pcf_constr _ ->
                 values ) ))
...
let add_labels ~intf ~impl ~file =
  ...
  let values, classes =
    List.fold_left
      ...
      ~f:(fun ((values, classes) as acc) item ->
        ...
        | Psig_class l ->
            ...
            , List.fold_left
                l
                ~init:classes
                ~f:(fun classes {pci_name = name; pci_expr = cty} ->
  ...
  ignore
    (List.fold_right
       impl
       ~init:(values, classes)
       ~f:(fun item ((values, classes) as acc) ->
         match item.pstr_desc with
         | Pstr_value (recp, l) ->
             let names =
               List.concat (List.map l ~f:(fun (p, _) -> pattern_vars p))
             in
             List.iter l ~f:(fun (pat, expr) ->
                 match pattern_name pat with
                 | Some s -> (
                   try
         ...
         | Pstr_class l ->
             ...
             List.iter l ~f:(fun {pci_name = name; pci_expr = expr} ->
                 try
  ...
  then (
    ...
    let last_pos =
      List.fold_left (sort_insertions ()) ~init:0 ~f:(fun pos (pos', s) ->
          output oc text pos (pos' - pos);
          output_string oc s;
          pos' )
    in
    if last_pos < String.length text
    then output oc text last_pos (String.length text - last_pos);
    close_out oc )
...
let main () =
  let files = ref [] in
  Arg.parse
    ["-norec", Arg.Set norec, "do not labelize recursive calls"]
    (fun f -> files := f :: !files)
