...
let bind_variables scope =
  ...
  let pat sub p =
    ...
    | Tpat_var (id, _) | Tpat_alias (_, id, _) ->
        Stypes.record
          (Stypes.An_ident (p.pat_loc, Ident.name id, Annot.Idef scope))
    | _ -> () );
    super.pat sub p
  in
  {super with pat}
...
let record_module_binding scope mb =
  Stypes.record
    (Stypes.An_ident (mb.mb_name.loc, mb.mb_name.txt, Annot.Idef scope))
...
let rec iterator ~scope rebuild_env =
  ...
  and expr sub exp =
    ...
    | Texp_ident (path, _, _) ->
        ...
        let env =
          if rebuild_env
          then (
            try Env.env_of_only_summary Envaux.env_from_summary exp.exp_env
        ...
        let annot =
          ...
          with Not_found -> Annot.Iref_external
        in
        Stypes.record (Stypes.An_ident (exp.exp_loc, full_name, annot))
    ...
    | Texp_match (_, f1, _) -> bind_cases f1
    | Texp_function {cases = f} | Texp_try (_, f) -> bind_cases f
  ...
  let structure_item_rem sub str rem =
    ...
    | Tstr_value (rec_flag, bindings) -> (
        let doit loc_start =
          bind_bindings {scope with loc_start} bindings
        in
        match rec_flag, rem with
    ...
    | Tstr_module mb ->
        record_module_binding
          {scope with Location.loc_start = loc.loc_end}
          mb
    | Tstr_recmodule mbs ->
        List.iter
          (record_module_binding
             {scope with Location.loc_start = loc.loc_start})
          mbs
    | _ -> () );
...
let record_cmt_info cmt =
  ...
  let record_info name value =
    ...
    Stypes.record
      (Stypes.An_ident
         (location_none, ident, Annot.Idef (location_file value)))
  in
  let open Cmt_format in
  (* record in reverse order to get them in correct order... *)
  List.iter
    (fun dir -> record_info "include" dir)
    (List.rev cmt.cmt_loadpath);
