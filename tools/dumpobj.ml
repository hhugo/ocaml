...
let rec print_struct_const = function
  ...
  | Const_block (tag, args) -> (
      ...
      match args with
      | [] -> ()
      | [a1] -> printf "("; print_struct_const a1; printf ")"
      | a1 :: al ->
          printf "(";
...
let rec print_obj x =
  ...
  then
    ...
    then printf "%S" (Obj.magic x : string)
    else if tag = Obj.double_tag
    then printf "%.12g" (Obj.magic x : float)
    else if tag = Obj.double_array_tag
    then (
      let a : floatarray = Obj.magic x in
      printf "[|";
    ...
    then printf "%ldl" (Obj.magic x : int32)
    else if tag = Obj.custom_tag && same_custom x 0n
    then printf "%ndn" (Obj.magic x : nativeint)
    else if tag = Obj.custom_tag && same_custom x 0L
    then printf "%LdL" (Obj.magic x : int64)
...
let dump_exe ic =
  ...
  let sym_table : Symtable.global_map = input_value ic in
  Symtable.iter_global_map
    (fun id pos -> !globals.(pos) <- Global id)
    sym_table;
  ( try
