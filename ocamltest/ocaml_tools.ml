...
class tool ~(name : string) ~(family : string) ~(flags : string)
  ...
  object (self)
    ...
    method reference_filename_suffix env =
      ...
      in
      if tool_reference_suffix <> "" then tool_reference_suffix ^ ".reference"
      else ".reference"
...
let ocamldoc =
  object
    inherit
      tool
        ~name:Ocaml_files.ocamldoc ~family:"doc" ~flags:"" ~directory:"ocamldoc"
