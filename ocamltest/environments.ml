...
let expand_aux env value =
  ...
  let subst s =
    try List.assoc s simple_bindings
    with
    | Not_found -> ""
  in
  let b = Buffer.create 100 in
  try
    Buffer.add_substitute b subst value;
    Buffer.contents b
  with
  | _ -> value
    ...
    let update env binding =
      let (name, value) =
        ...
        | c ->
          ...
          let value = String.sub binding (c + 1) (String.length binding - c - 1)
          in
          name, Some value
...
let lookup variable env =
  try Some (expand env (VariableMap.find variable env))
  with
  | Not_found -> None
...
let register_modifiers name modifiers =
  if name = "" then
    raise Empty_modifiers_name
  else if Hashtbl.mem registered_modifiers name then
    raise (Modifiers_name_already_registered name)
  else
    Hashtbl.add registered_modifiers name modifiers

let find_modifiers name =
  try Hashtbl.find registered_modifiers name
  with
  | Not_found -> raise (Modifiers_name_not_found name)

let rec apply_modifier environment =
  function
