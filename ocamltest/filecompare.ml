...
let cmp_result_of_exitcode commandline =
  function
  | 0 -> Same
  | 1 -> Different
...
let make_comparison_tool
    ?(result_of_exitcode=cmp_result_of_exitcode) name flags
=
    ...
    fun ic2 ->
      ...
      Sys.with_input_file ~bin:true file1 @@
        fun ic1 ->
          ...
          else (* file2 has at least one non-ignored line *)
        ...
        fun ic2 ->
          ...
          let rec compare () =
            ...
            if block1 = block2 then
              (if Bytes.length block1 > 0 then compare () else Same)
            else
              Different
          in
          compare ()
...
let check_file ?(tool=default_comparison_tool) files =
  if Sys.file_exists files.reference_filename then
    compare_files ~tool files
  else if Sys.file_is_empty files.output_filename then
    Same
  else
    Unexpected_output
...
let diff files =
  ...
  let diff_commandline =
    Filename.quote_command "diff" ~stdout:temporary_file
      [ "--strip-trailing-cr"; "-u"; files.reference_filename;
        files.output_filename ]
  in
  let result =
