  ...
  let crlf_endings2, line2, reached_end_file2 =
    let rec loop crlf_endings2 k =
      ...
      | line ->
          ...
          if k = 0 then (crlf_endings2, line, false)
...
let compare_binary_files bytes_to_ignore file1 file2 =
  ...
  let rec compare () =
    ...
    if block1 = block2 then if Bytes.length block1 > 0 then compare () else Same
    else Different
  in
  compare ()
...
let check_file ?(tool = default_comparison_tool) files =
  if Sys.file_exists files.reference_filename then compare_files ~tool files
  else if Sys.file_is_empty files.output_filename then Same
