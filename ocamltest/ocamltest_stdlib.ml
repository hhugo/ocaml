...
module Filename = struct
  ...
  let maybe_quote f =
    if
      String.contains f ' ' ||
        String.contains f '"' || String.contains f '\t' || f = ""
    then
      Filename.quote f
    else
      f
...
module String = struct
  ...
  let words s =
    ...
    let rec f quote w ws i =
      if i >= l then
        (if w <> "" then List.rev (w :: ws) else List.rev ws)
      else
        let j = i + 1 in
        match s.[i] with
        | ('\'' | '"') as c ->
...
module Sys = struct
  ...
  let rm_rf path =
    ...
    else (* path could be a dangling symlink *)
      try Sys.remove path
      with
      | Sys_error _ -> ()
  ...
  let copy_file src dest =
    with_input_file ~bin:true src @@
      fun ic -> with_output_file ~bin:true dest @@ fun oc -> copy_chan ic oc
  ...
  let rec copy_directory src dst =
    ...
    let cp_dir name = copy_directory (full_src_path name) (full_dst_path name)
    in
