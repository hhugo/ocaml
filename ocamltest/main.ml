...
let announce_test_error test_filename error =
  Printf.printf " ... testing '%s' => unexpected error (%s)\n%!"
    (Filename.basename test_filename)
    error
  ...
  | Node (testenvspec, test, env_modifiers, subtrees) ->
      ...
      let msg, children_behavior, summary =
        ...
        | Run env ->
            ...
            let testenv =
              List.fold_left apply_modifiers testenv0 env_modifiers
            in
...
let test_file test_filename =
  ...
  let test_directory =
    ...
    else Filename.concat test_dirname test_prefix
  in
  ...
  let clean_test_build_directory () =
    try Sys.rm_rf test_build_directory_prefix with Sys_error _ -> ()
  in
...
let is_test s =
  match tsl_block_of_file s with _ -> true | exception _ -> false
