...
type behavior = Skip_all_tests | Run of Environments.t
(*
...
let tsl_block_of_file_safe test_filename =
  try tsl_block_of_file test_filename
  with
  | Sys_error message ->
    Printf.eprintf "%s\n%!" message;
...
let rec run_test log common_prefix path behavior =
  ...
  | Node (testenvspec, test, env_modifiers, subtrees) ->
    ...
    let (msg, children_behavior, summary) =
      match behavior with
...
let get_test_build_directory_prefix test_dirname =
  ...
  in
  if test_dirname = "." then root else Filename.concat root test_dirname
...
let test_file test_filename =
  ...
  let action_names =
    ...
    Actions.ActionSet.fold f used_actions String.Set.empty
  in
  ...
  let test_directory =
    if test_dirname = "." then
      test_prefix
    else
      Filename.concat test_dirname test_prefix
  in
  ...
  let test_build_directory_prefix =
    get_test_build_directory_prefix test_directory
  in
  let clean_test_build_directory () =
    try Sys.rm_rf test_build_directory_prefix
    with
    | Sys_error _ -> ()
  in
  ...
  let log_filename =
    Filename.concat test_build_directory_prefix (test_prefix ^ ".log")
  in
  let log = if Options.log_to_stderr then stderr else open_out log_filename in
  let summary =
    Sys.with_chdir test_build_directory_prefix
      (fun () ->
         ...
         let install_hook name =
           ...
           if Sys.file_exists hook_name then
         ...
         let reference_filename =
           Filename.concat test_source_directory (test_prefix ^ ".reference")
         in
         let make =
           try Sys.getenv "MAKE"
           with
         ...
         let initial_environment =
           Environments.from_bindings
             [ Builtin_variables.make, make;
               ...
               test_build_directory_prefix; Builtin_variables.promote, promote;
         ...
         in
         let rootenv =
           interpret_environment_statements rootenv rootenv_statements
         in
         let rootenv = Environments.initialize Environments.Post log rootenv in
         let common_prefix = " ... testing '" ^ test_basename ^ "' with" in
         let initial_status = if skip_test then Skip_all_tests else Run rootenv
         in
         let summary =
           run_test_trees log common_prefix "" initial_status test_trees
         in
  ...
  | No_failure ->
    if not Options.keep_test_dir_on_success then clean_test_build_directory ()
      ...
      (fun s ->
         if ignored s then
           ...
           if Sys.is_directory s then
             loop s
           else if not !contains_tests && is_test s then
             contains_tests := true) (Sys.readdir dir);
