...
let exit_status_of_variable env variable =
  try int_of_string (Environments.safe_lookup variable env)
  with
  | _ -> 0
...
let setup_subdirectories source_directory build_directory subdirs =
  ...
  let cp_dir name = Sys.copy_directory (full_src_path name) (full_dst_path name)
  in
...
let setup_simple_build_env add_testfile additional_files log env =
  let build_env =
    Environments.add Builtin_variables.test_build_directory
      (test_build_directory_prefix env) env
  in
  setup_build_env add_testfile additional_files log build_env

let run_cmd
    ?(environment=[| |]) ?(stdin_variable=Builtin_variables.stdin)
    ?(stdout_variable=Builtin_variables.stdout)
    ?(stderr_variable=Builtin_variables.stderr) ?(append=false) ?timeout log env
    original_cmd
=
  ...
  let quoted_lst = if Sys.win32 then List.map Filename.maybe_quote lst else lst
  in
...
let run
    (log_message : string) (redirect_output : bool) (can_skip : bool)
  ...
  let final_value =
    ...
    | _ ->
      ...
      if exit_status = 125 then
        Result.skip_with_reason reason, hookenv
      else
        Result.fail_with_reason reason, hookenv
  in
  Sys.force_remove response_file;
...
let check_output kind_of_output output_variable reference_variable log env =
  ...
  | Filecompare.Different ->
    ...
    in
    (if
       Environments.lookup_as_bool Builtin_variables.promote env = Some true
     then
  ...
  | Filecompare.Error (commandline, exitcode) ->
    let reason =
      Printf.sprintf "The command %s failed with status %d" commandline exitcode
    in
    Result.fail_with_reason reason, env
