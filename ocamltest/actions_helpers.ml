...
let pass_or_skip test pass_reason skip_reason _log env =
  let open Result in
  let result =
    if test then pass_with_reason pass_reason else skip_with_reason skip_reason
  in
  (result, env)

let mkreason what commandline exitcode =
  Printf.sprintf "%s: command\n%s\nfailed with exit code %d" what commandline
    exitcode
...
let exit_status_of_variable env variable =
  try int_of_string (Environments.safe_lookup variable env) with _ -> 0
...
let setup_build_env add_testfile additional_files (_log : out_channel) env =
  ...
  let files = if add_testfile then testfile env :: some_files else some_files in
...
let setup_simple_build_env add_testfile additional_files log env =
  let build_env =
    Environments.add Builtin_variables.test_build_directory
      (test_build_directory_prefix env)
      env
  in
  setup_build_env add_testfile additional_files log build_env

let run_cmd ?(environment = [||]) ?(stdin_variable = Builtin_variables.stdin)
    ?(stdout_variable = Builtin_variables.stdout)
    ?(stderr_variable = Builtin_variables.stderr) ?(append = false) ?timeout log
    env original_cmd =
  ...
  let cmd =
    if Environments.lookup_as_bool Strace.strace env = Some true then
      ...
      let strace_cmd = [ "strace"; "-f"; "-o"; strace_logfile; strace_flags ] in
      strace_cmd @ original_cmd
    else original_cmd
  in
  let lst = List.concat (List.map String.words cmd) in
  let quoted_lst =
    if Sys.win32 then List.map Filename.maybe_quote lst else lst
  in
  ...
  let systemenv = Environments.append_to_system_env environment env in
  let timeout =
    match timeout with
  ...
  in
  let dump_file s fn =
    if not (Sys.file_is_empty fn) then (
...
let run (log_message : string) (redirect_output : bool) (can_skip : bool)
    (prog_variable : Variables.t) (args_variable : Variables.t option)
    (log : out_channel) (env : Environments.t) =
  match Environments.lookup prog_variable env with
  | None ->
      let msg =
        Printf.sprintf "%s: variable %s is undefined" log_message
          (Variables.name_of_variable prog_variable)
      in
  ...
  | Some program ->
      let arguments =
        ...
        | None -> ""
        | Some variable -> Environments.safe_lookup variable env
      in
      let commandline = [ program; arguments ] in
      let what =
        log_message ^ " " ^ program ^ " "
        ^
        if arguments = "" then "without any argument"
      ...
      if exit_status = expected_exit_status then (Result.pass, env)
        ...
        let reason =
          mkreason what (String.concat " " commandline) exit_status
        in
        if exit_status = 125 && can_skip then
          (Result.skip_with_reason reason, env)
        else (Result.fail_with_reason reason, env)

let run_program =
  run "Running program" true false Builtin_variables.program
...
let run_script log env =
  ...
  let scriptenv =
    Environments.add Builtin_variables.ocamltest_response response_file env
  in
  let result, newenv =
    run "Running script" true true Builtin_variables.script None log scriptenv
  in
  let final_value =
    if Result.is_pass result then
      ...
      | modifiers ->
          ...
          (result, modified_env)
      | exception Failure reason -> (Result.fail_with_reason reason, newenv)
...
let run_hook hook_name log input_env =
  ...
  let hookenv =
    Environments.add Builtin_variables.ocamltest_response response_file
      input_env
  in
  let systemenv = Environments.to_system_env hookenv in
  let timeout =
    Option.value ~default:0
      (Environments.lookup_as_int Builtin_variables.timeout input_env)
  in
  ...
  in
  let exit_status = run settings in
  let final_value =
    ...
    | 0 -> (
        ...
        | exception Failure reason -> (Result.fail_with_reason reason, hookenv)
        | exception Variables.No_such_variable name ->
            let reason =
              Printf.sprintf "error in script response: unknown variable %s"
                name
            in
    ...
    | _ ->
        ...
        let reason = String.trim (Sys.string_of_file response_file) in
        if exit_status = 125 then (Result.skip_with_reason reason, hookenv)
...
let check_output kind_of_output output_variable reference_variable log env =
  ...
  let skip_lines =
    to_int (Environments.lookup Builtin_variables.skip_header_lines env)
  in
  let skip_bytes =
    to_int (Environments.lookup Builtin_variables.skip_header_bytes env)
  in
  ...
  let ignore_header_conf =
    { Filecompare.lines = skip_lines; Filecompare.bytes = skip_bytes }
  in
  let tool = Filecompare.make_cmp_tool ~ignore:ignore_header_conf in
  ...
  | Filecompare.Different ->
      ...
      let diffstr =
        ...
        | Error diff_file -> "See " ^ diff_file
      in
      let reason =
        Printf.sprintf "%s output %s differs from reference %s: \n%s\n"
          kind_of_output output_filename reference_filename diffstr
      in
  ...
  | Filecompare.Unexpected_output ->
      ...
      let unexpected_output_with_banners =
        Printf.sprintf "%s\n%s%s\n" banner unexpected_output banner
      in
      let reason =
        Printf.sprintf
          "The file %s was expected to be empty because there is no reference \
           ...
           %s\n"
          output_filename reference_filename unexpected_output_with_banners
      in
      (Result.fail_with_reason reason, env)
  | Filecompare.Error (commandline, exitcode) ->
      let reason =
        Printf.sprintf "The command %s failed with status %d" commandline
          exitcode
      in
      (Result.fail_with_reason reason, env)
