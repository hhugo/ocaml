...
let string_of_test test =
  if test.Tests.test_run_by_default then
    test.Tests.test_name ^ " (run by default)"
  else
    test.Tests.test_name

let string_of_variable v =
  Printf.sprintf "%s: %s" (Variables.name_of_variable v)
    ...
    Arg.Int
      (fun t ->
         if t >= 0 then
           ...
           raise (Arg.Bad "negative timeout")),
    "<seconds> Set maximal execution time for every command (in seconds)";
    "-find-test-dirs",
    Arg.String (add_to_list find_test_dirs),
    " Find directories that contain tests (recursive).";
    "-list-tests",
    Arg.String (add_to_list list_tests),
    " List tests in given directory.";
    "-keep-test-dir-on-success",
    Arg.Set keep_test_dir_on_success,
