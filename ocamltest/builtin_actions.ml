...
let copy_action log env =
  let do_copy src dst =
    let entry_type, f =
      if Sys.is_directory src then ("directory", Sys.copy_directory)
  ...
  | Some src, Some dst ->
      let f =
        if String.ends_with ~suffix:"/" dst then fun src ->
          do_copy src (dst ^ Filename.basename src)
