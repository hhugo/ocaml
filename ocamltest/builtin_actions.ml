...
let pass =
  make "pass"
    (fun _log env ->
       let reason = reason_with_fallback env "the pass action always succeeds"
       in
...
let libwin32unix =
  make "libwin32unix"
    (Actions_helpers.pass_or_skip (Ocamltest_config.libunix = Some false)
       "libwin32unix available" "libwin32unix not available")
...
let hasstr =
  make "hasstr"
    (Actions_helpers.pass_or_skip Ocamltest_config.str "str library available"
...
let windows =
  make "windows"
    (Actions_helpers.pass_or_skip (get_OS () = windows_OS) "running on Windows"
...
let arch32 =
  make "arch32"
    (Actions_helpers.pass_or_skip (Sys.word_size = 32) "32-bit architecture"
       "non-32-bit architecture")

let arch64 =
  make "arch64"
    (Actions_helpers.pass_or_skip (Sys.word_size = 64) "64-bit architecture"
...
let has_symlink =
  make "has_symlink"
    (Actions_helpers.pass_or_skip (Unix.has_symlink ()) "symlinks available"
       "symlinks not available")

let setup_build_env =
  make "setup-build-env" (Actions_helpers.setup_build_env true [])

let setup_simple_build_env =
  make "setup-simple-build-env" (Actions_helpers.setup_simple_build_env true [])

let run = make "run" Actions_helpers.run_program
let script = make "script" Actions_helpers.run_script

let check_program_output =
  make "check-program-output"
...
let copy_action log env =
  ...
  | Some src, Some dst ->
    let f =
      if String.ends_with ~suffix:"/" dst then
        (fun src -> do_copy src (dst ^ Filename.basename src))
      else
        (fun src -> do_copy src dst)
    in
...
let initialize_test_exit_status_variables _log env =
  Environments.add_bindings
    [ Builtin_variables.test_pass, "0"; Builtin_variables.test_fail, "1";
      Builtin_variables.test_skip, "125" ] env

let _ =
  Environments.register_initializer Environments.Post
    "test_exit_status_variables" initialize_test_exit_status_variables;
  List.iter register
    [ pass; skip; fail; cd; dumpenv; hasunix; hassysthreads; hasstr; libunix;
      libwin32unix; windows; not_windows; bsd; not_bsd; macos; arch32; arch64;
      has_symlink; setup_build_env; setup_simple_build_env; run; script;
      check_program_output; arch_arm; arch_arm64; arch_amd64; arch_i386;
      arch_power; function_sections; naked_pointers; file_exists; copy ]
