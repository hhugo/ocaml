...
type program_body =
  ...
  | Let_rec_symbol of
      (Symbol.t * constant_defining_value) list * program_body
...
let rec lam ppf (flam : t) =
  ...
  | Static_catch (i, vars, lbody, lhandler) ->
      fprintf
        ...
        (fun ppf vars ->
          ...
          | [] -> ()
          | _ ->
              List.iter (fun x -> fprintf ppf " %a" Variable.print x) vars
          )
        vars
        lam
        lhandler
  ...
  | If_then_else (lcond, lif, lelse) ->
      fprintf
        ppf
        "@[<2>(if@ %a@ then begin@ %a@ end else begin@ %a@ end)@]"
        Variable.print
        lcond
        lam
        lif
...
let rec print_program_body ppf (program : program_body) =
  ...
  | Initialize_symbol (symbol, tag, fields, program) ->
      fprintf
        ppf
        "@[<2>initialize_symbol@ (@[<2>%a@ %a@ %a@])@]@."
        Symbol.print
        symbol
        Tag.print
        tag
        (Format.pp_print_list lam)
        fields;
      print_program_body ppf program
  | Effect (expr, program) ->
  ...
  | _ ->
      ...
      let rec aux (flam : t) : unit =
        ...
        | Static_catch (_, vars, e1, e2) ->
            ...
            aux e1;
            aux e2
        | Try_with (e1, var, e2) -> aux e1; bound_variable var; aux e2
        | If_then_else (var, e1, e2) -> free_variable var; aux e1; aux e2
        | While (e1, e2) -> aux e1; aux e2
...
let map_defining_expr_of_let let_expr ~f =
  ...
  if defining_expr == let_expr.defining_expr
  then Let let_expr
  else
    ...
    | _ -> (
        ...
        | Try_with (f1, _, f2) | While (f1, f2) | Static_catch (_, _, f1, f2)
          ->
            aux f1; aux f2
        | For {body; _} -> aux body
        | If_then_else (_, f1, f2) -> aux f1; aux f2
        | Switch (_, sw) ->
...
let fold_lets_option t ~init
    ~(for_defining_expr :
       'a -> Variable.t -> named -> 'a * Variable.t * named) ~for_last_body
...
let free_symbols_program (program : program) =
  ...
  loop program.program_body; !symbols
...
let create_function_declaration ~params ~body ~stub ~dbg
    ...
    ~closure_origin : function_declaration =
...
let create_set_of_closures ~function_decls ~free_vars ~specialised_args
  ...
  then (
    ...
    *)
    ...
    if not (Variable.Set.subset expected_free_vars free_vars_domain)
    then
      Misc.fatal_errorf
        "create_set_of_closures: [free_vars] mapping of variables bound by \
         the closure(s) is wrong.  (Must map at least %a but only maps \
         %a.)@ \n\
         function_decls:@ %a"
        Variable.Set.print
        expected_free_vars
    ...
    if not (Variable.Set.subset spec_args_domain all_params)
    then
      Misc.fatal_errorf
        "create_set_of_closures: [specialised_args] maps variable(s) that \
         are not parameters of the given function declarations.  \
         specialised_args domain=%a all_params=%a \n\
         function_decls:@ %a"
        Variable.Set.print
        spec_args_domain
        Variable.Set.print
      ...
      | Block (tag1, fields1), Block (tag2, fields2) ->
          ...
          then c
            ...
            Misc.Stdlib.List.compare
              compare_constant_defining_value_block_field
              fields1
              fields2
      | Set_of_closures set1, Set_of_closures set2 ->
          Set_of_closures_id.compare
            set1.function_decls.set_of_closures_id
