  ...
  Projection.Set.filter
    (fun projection ->
      ...
      | Project_var project_var -> (
        ...
        | Ok
            (_value_closure, _approx_var, _approx_sym, value_set_of_closures)
          ->
            Var_within_closure.Map.mem
              project_var.var
      ...
      | Move_within_set_of_closures move -> (
        match A.check_approx_for_closure approx with
        | Ok
            (value_closure, _approx_var, _approx_sym, _value_set_of_closures)
          ->
...
let rec analyse_expr ~which_variables expr =
  ...
  let check_free_variable var =
    if Variable.Map.mem var which_variables
    then used_which_variables := Variable.Set.add var !used_which_variables
  in
  ...
  let for_named (named : Flambda.named) =
    ...
    | Set_of_closures set_of_closures ->
        ...
        in
        if not (Variable.Map.is_empty aliasing_vars)
        then
          Variable.Map.iter
