...
module Inconstants (P : Param) (Backend : Backend_intf.S) = struct
  ...
  *)
  ...
  and complete_marking () =
    while not (Queue.is_empty mark_queue) do
      let deps = try Queue.take mark_queue with Not_found -> [] in
      mark_deps deps
    done

  (* adds 'curr in NC' *)
  let mark_curr curr = mark_deps curr; complete_marking ()
  ...
  let register_implication ~in_nc:dep ~implies_in_nc:curr =
    ...
    | Var id -> (
      match Variable.Tbl.find variables id with
      | Not_constant -> mark_deps curr; complete_marking ()
      | Implication deps ->
    ...
    | Symbol_field ((symbol, _) as field) -> (
      ...
      | exception Not_found ->
          ...
          if Symbol.Set.mem symbol imported_symbols
          then (
            Symbol_field.Tbl.add symbol_fields field Not_constant;
            mark_deps curr;
            complete_marking () )
          else Symbol_field.Tbl.add symbol_fields field (Implication curr) )
  ...
  *)
  let rec mark_loop ~toplevel (curr : dep list) (flam : Flambda.t) =
    ...
    | Apply {func; args; _} ->
        mark_curr curr; mark_var func curr; mark_vars args curr
    | Switch (arg, sw) ->
        mark_curr curr;
        mark_var arg curr;
  ...
  and mark_named ~toplevel curr (named : Flambda.named) =
    ...
    | Read_symbol_field (symbol, index) ->
        register_implication
          ~in_nc:(Symbol_field (symbol, index))
          ~implies_in_nc:curr
...
*)
    ...
    | Project_closure {set_of_closures; closure_id} ->
        if Closure_id.in_compilation_unit closure_id compilation_unit
        then mark_var set_of_closures curr
        else mark_curr curr
    | Move_within_set_of_closures {closure; start_from; move_to} ->
        (* CR-someday mshinwell: We should be able to deem these projections
         ...
         closures.) *)
        if Closure_id.in_compilation_unit start_from compilation_unit
        then (
    ...
    | Project_var {closure; closure_id; var = _} ->
        if Closure_id.in_compilation_unit closure_id compilation_unit
        then mark_var closure curr
        else mark_curr curr
    | Prim (Pfield _, [f1], _) -> mark_curr curr; mark_var f1 curr
    | Prim (_, args, _) -> mark_curr curr; mark_vars args curr
  ...
  and mark_loop_set_of_closures ~toplevel:_ curr
    ...
    register_implication
      ~in_nc:(Closure function_decls.set_of_closures_id)
      ~implies_in_nc:curr;
      ...
      | Initialize_symbol (symbol, _tag, fields, program) ->
          List.iteri
            (fun i field ->
              mark_loop
                ~toplevel:true
                [Symbol symbol; Symbol_field (symbol, i)]
                field )
            fields;
          loop program
      | Effect (expr, program) ->
