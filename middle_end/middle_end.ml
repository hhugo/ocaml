...
let middle_end ~ppf_dump ~prefixname ~backend ~size ~filename ~module_ident
    ~module_initializer =
  Profile.record_call "flambda" (fun () ->
      ...
      let flambda_warning_reporter loc w =
        let elt = loc, w in
        if not (WarningSet.mem elt !warning_set)
        then (
        ...
        (fun () ->
          ...
          let check flam =
            ...
            then
              ...
              with exn ->
                Misc.fatal_errorf
                  "After Flambda pass %d, round %d:@.%s:@.%a"
                  !pass_number
                  !round_number
          ...
          let ( +-+ ) flam (name, pass) =
            ...
            if !Clflags.flambda_invariant_checks
            then Profile.record ~accumulate:true "check" check flam;
            flam
          in
          Profile.record_call ~accumulate:true "middle_end" (fun () ->
              ...
              let rec loop flam =
                ...
                incr round_number;
                if !round_number > Clflags.rounds ()
                then flam
                else
                  flam
                  ...
                  (inline_and_simplify.ml line 710). *)
                      Location.prerr_warning
                        ...
                        (Warnings.Inlining_impossible
                           "[@inlined] attribute was not used on this \
                            function application (the optimizer did not \
                            know what function was being applied)")
                  | Unroll _ ->
                      Location.prerr_warning
                        (Debuginfo.to_location apply.dbg)
                        (Warnings.Inlining_impossible
                           "[@unroll] attribute was not used on this \
                            function application (the optimizer did not \
                            know what function was being applied)") );
