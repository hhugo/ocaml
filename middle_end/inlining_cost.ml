...
let prim_size (prim : Clambda_primitives.primitive) args =
  ...
  | Pccall p ->
      (if p.Primitive.prim_alloc then 10 else 4) + List.length args
  | Praise _ -> 4
  | Pstringlength -> 5
  | Pbyteslength -> 5
  | Pstringrefs -> 6
  ...
  | Psequand | Psequor ->
      Misc.fatal_error
        "Psequand and Psequor are not allowed in Prim expressions; \
         translate out instead (cf. closure_conversion.ml)"
...
let lambda_smaller' lam ~than:threshold =
  ...
  and lambda_named_size (named : Flambda.named) =
    ...
    | Set_of_closures {function_decls = ffuns} ->
        Variable.Map.iter
          (fun _ (ffun : Flambda.function_declaration) ->
...
module Threshold = struct
  ...
  let sub t1 t2 =
    ...
    | Can_inline_if_no_larger_than i1, Can_inline_if_no_larger_than i2 ->
        if i1 > i2
        then Can_inline_if_no_larger_than (i1 - i2)
        else Never_inline
...
module Benefit = struct
  ...
  let remove_code_helper_named b (named : Flambda.named) =
    ...
    | Prim _ | Project_closure _ | Project_var _
     |Move_within_set_of_closures _ | Read_symbol_field _ ->
        b := remove_prim !b
  ...
  let remove_code_named lam b =
    let b = ref b in
    Flambda_iterators.iter_named_toplevel
      (remove_code_helper b)
      (remove_code_helper_named b)
      lam;
    !b
  ...
  let print ppf b =
    Format.fprintf
      ppf
      "@[remove_call: %i@ remove_alloc: %i@ remove_prim: %i@ \
       remove_branch: %i@ direct: %i@ requested: %i@]"
      b.remove_call
      b.remove_alloc
      b.remove_prim
      b.remove_branch
    ...
    then
      ...
      let branch_taken_estimated_probability =
        let inline_branch_factor =
          ...
          in
          if is_nan factor
          then Clflags.default_inline_branch_factor
          else if Float.compare factor 0. < 0
          then 0.
          else factor
        in
  ...
  let print_description ~subfunctions ppf t =
    ...
    let pr_lifting ppf =
      ...
      then (
        ...
        Format.pp_print_text
          ppf
          "Inlining the function would also lift some definitions to \
           toplevel.";
        Format.pp_close_box ppf ();
        Format.pp_print_cut ppf ();
        Format.pp_print_cut ppf () )
