...
let simplify_project_closure env r
    ~(project_closure : Flambda.project_closure) : Flambda.named * R.t =
  simplify_free_variable_named
    env
    ...
    ~f:(fun _env set_of_closures set_of_closures_approx ->
      ...
      | Wrong ->
          Misc.fatal_errorf
            "Wrong approximation when projecting closure: %a"
            Flambda.print_project_closure
            project_closure
      | Unresolved value ->
      ...
      | Ok (set_of_closures_var, value_set_of_closures) -> (
          let closure_id =
            A.freshen_and_check_closure_id
          ...
          | Some (var, projection) ->
              simplify_free_variable_named
                ...
                ~f:(fun _env var var_approx ->
                  let r =
                    R.map_benefit r (B.remove_projection projection)
                  in
          ...
          | None -> (
            ...
            | None ->
                let set_of_closures_var =
                  match set_of_closures_var with
                  | Some set_of_closures_var'
                    when E.mem env set_of_closures_var' ->
                      set_of_closures_var
                  | Some _ | None -> None
                in
                let approx =
...
let simplify_move_within_set_of_closures env r
  ...
  simplify_free_variable_named
    ...
    ~f:(fun _env closure closure_approx ->
      match
        A.check_approx_for_closure_allowing_unresolved closure_approx
      with
      | Wrong ->
          Misc.fatal_errorf
            "Wrong approximation when moving within set of closures.  \
             Approximation: %a  Term: %a"
            A.print
            closure_approx
      ...
      | Ok
          ...
          | None -> (
            ...
            | Some (flam, approx) -> flam, ret r approx
            | None -> (
                if Closure_id.equal start_from move_to
                then
                  ...
                  | Some _ | None -> (
                    ...
                    | Some set_of_closures_symbol ->
                        ...
                        let project_closure_var =
                          Variable.create
                            Internal_variable_names.project_closure
                        in
                        let let1 =
                          Flambda.create_let
                            project_closure_var
                   ...
                   have no other information by which to simplify the move. *)
                        ...
                        in
                        let approx =
                          A.value_closure value_set_of_closures move_to
                        in
  ...
  simplify_free_variable_named
    ...
    ~f:(fun _env closure approx ->
      ...
      | Ok
          ...
          let closure_id =
            F.apply_closure_id freshening project_var.closure_id
          in
          let closure_id_in_approx = value_closure.closure_id in
          if not (Closure_id.equal closure_id closure_id_in_approx)
          then
            Misc.fatal_errorf
              "When simplifying [Project_var], the closure ID %a in the \
               approximation of the set of closures did not match the \
               closure ID %a in the [Project_var] term.  Approximation: \
               %a@. Var-within-closure being projected: %a@."
              Closure_id.print
          ...
          | None ->
              let approx =
                A.approx_for_bound_var value_set_of_closures var
              in
              ...
              let expr =
                if E.mem env unwrapped
                then Flambda.Expr (Var unwrapped)
                else expr
              in
         ...
         we take account of all freshenings. *)
          Misc.fatal_errorf
            "[Project_var] from a value with wrong approximation: \
             %a@.closure=%a@.approx of closure=%a@."
            Flambda.print_project_var
            project_var
            Variable.print
            closure
            Simple_value_approx.print
...
and simplify_set_of_closures original_env r
  ...
  let ( free_vars
    ...
    Inline_and_simplify_aux.prepare_to_simplify_set_of_closures
      ...
      ~set_of_closures
      ~function_decls
      ~only_for_function_decl:None
      ~freshen:true
  in
  let simplify_function fun_var
      (function_decl : Flambda.function_declaration) (funs, used_params, r)
      : Flambda.function_declaration Variable.Map.t * Variable.Set.t * R.t =
    let closure_env =
      Inline_and_simplify_aux.prepare_to_simplify_closure
        ~function_decl
        ~free_vars
        ~specialised_args
        ~parameter_approximations
  ...
  let direct_call_surrogates =
    Closure_id.Map.fold
      (fun existing surrogate surrogates ->
        Variable.Map.add
          (Closure_id.unwrap existing)
          (Closure_id.unwrap surrogate)
          surrogates )
      internal_value_set_of_closures.direct_call_surrogates
      Variable.Map.empty
  in
...
and simplify_apply env r ~(apply : Flambda.apply) : Flambda.t * R.t =
  let { Flambda.func = lhs_of_application
      ; args
      ; kind = _
      ; dbg
          ...
          | Ok
              ...
              let ( lhs_of_application
               ...
               repoint the call to the surrogate. *)
                let surrogates =
                  value_set_of_closures.direct_call_surrogates
                in
                match
                  Closure_id.Map.find closure_id_being_applied surrogates
                with
                | exception Not_found ->
                    ( lhs_of_application
                    , closure_id_being_applied
                ...
                | surrogate ->
                    ...
                    let surrogate_var =
                      Variable.rename lhs_of_application
                    in
                    let move_to_surrogate :
                    ...
                    in
                    let env =
                      E.add env surrogate_var approx_for_surrogate
                    in
                    let wrap expr =
                      Flambda.create_let
                        surrogate_var
                ...
                with Not_found ->
                  Misc.fatal_errorf
                    "When handling application expression, approximation \
                     references non-existent closure %a@."
                    Closure_id.print
                    closure_id_being_applied
              in
              let r =
                ...
                | Indirect ->
                    R.map_benefit
                      r
                      Inlining_cost.Benefit.direct_call_of_indirect
                | Direct _ -> r
              in
...
and simplify_full_application env r ~function_decls ~lhs_of_application
  ...
  Inlining_decision.for_call_site
    ~env
    ~r
    ~function_decls
    ~lhs_of_application
...
and simplify_partial_application env r ~lhs_of_application
  ...
  | Always_specialise | Never_specialise ->
      Location.prerr_warning
        (Debuginfo.to_location dbg)
        (Warnings.Inlining_impossible
           "[@specialised] attributes may not be used on partial \
            applications")
  | Default_specialise -> () );
  ...
  let wrapper_accepting_remaining_args =
    ...
    in
    Flambda_utils.make_closure_declaration
      ~id:closure_variable
...
and simplify_named env r (tree : Flambda.named) : Flambda.named * R.t =
  ...
  | Read_symbol_field (symbol, field_index) -> (
      ...
      match A.get_field approx ~field_index with
  ...
  | Set_of_closures set_of_closures -> (
      ...
      let simplify env r expr ~pass_name : Flambda.named * R.t =
        ...
        let value_set_of_closures =
          ...
          | Wrong ->
              Misc.fatal_errorf
                "Unexpected approximation returned from simplification of \
                 [%s] result: %a"
                pass_name
                A.print
                approx
          | Ok (_var, value_set_of_closures) ->
              let freshening =
                Freshening.Project_var.compose
                  ~earlier:first_freshening
                  ~later:value_set_of_closures.freshening
              in
              A.update_freshening_of_value_set_of_closures
                value_set_of_closures
                ~freshening
        in
      ...
      let set_of_closures =
        ...
        match
          Remove_free_vars_equal_to_args.run ~ppf_dump set_of_closures
        with
      ...
      (* Do [Unbox_closures] next to try to decide which things are
       ...
       unboxing them. *)
      match
        Unbox_closures.rewrite_set_of_closures
          ~env
          ~duplicate_function
          ~set_of_closures
      with
      ...
      | None -> (
        ...
        | None -> (
          ...
          match
            Unbox_specialised_args.rewrite_set_of_closures
              ~env
              ~duplicate_function
              ~set_of_closures
          with
  ...
  | Move_within_set_of_closures move_within_set_of_closures ->
      simplify_move_within_set_of_closures
        env
        r
        ~move_within_set_of_closures
  | Prim (prim, args, dbg) ->
      let dbg = E.add_inlined_debuginfo env ~dbg in
      simplify_free_variables_named
        env
        args
        ~f:(fun env args args_approxs ->
              ...
              | Some var ->
                  simplify_free_variable_named
                    ...
                    ~f:(fun _env var var_approx ->
                      ...
                      Expr (Var var), ret r var_approx )
              | None -> (
                match A.get_field arg_approx ~field_index with
                | Unreachable -> Flambda.Expr Proved_unreachable, r
                | Ok approx ->
                    let tree, approx =
                      ...
                      | Some (symbol, None) ->
                          let approx =
                            A.augment_with_symbol_field
                              ...
                              field_index
                          in
                          ( Flambda.Read_symbol_field (symbol, field_index)
                          , approx )
                      | None | Some (_, Some _) ->
          ...
          | (Psetfield _ | Parraysetu _ | Parraysets _), _, _ ->
              Misc.fatal_error
                "Psetfield / Parraysetu / Parraysets arity error"
...
and simplify env r (tree : Flambda.t) : Flambda.t * R.t =
  ...
  | Let _ ->
      ...
      let filter_defining_expr r var defining_expr free_vars_of_body =
        if Variable.Set.mem var free_vars_of_body
        then r, var, Some defining_expr
        else if Effect_analysis.no_effects_named defining_expr
        then
          let r = R.map_benefit r (B.remove_code_named defining_expr) in
          r, var, None
        else r, var, Some defining_expr
      in
      Flambda.fold_lets_option
        tree
        ~init:(env, r)
        ~for_defining_expr
        ~for_last_body
  ...
  | Let_mutable {var = mut_var; initial_value = var; body; contents_kind} ->
      ...
      simplify_free_variable env var ~f:(fun env var _var_approx ->
          ...
          let body, r =
            simplify
              (E.add_mutable env mut_var (A.value_unknown Other))
              r
              body
          in
          ( Flambda.Let_mutable
  ...
  | Static_catch (i, vars, body, handler) -> (
    ...
    | _ -> (
        ...
        (* CR-soon mshinwell: for robustness, R.used_static_exceptions should
           maybe be removed. *)
        if not (Static_exception.Set.mem i (R.used_static_exceptions r))
        then
          ...
          | _ ->
              ...
              let env =
                List.fold_left
                  (fun env id -> E.add env id (A.value_unknown Other))
                  (E.set_freshening env sb)
                  vars
              in
  ...
  | Try_with (body, id, handler) ->
      ...
      let env =
        E.add (E.set_freshening env sb) id (A.value_unknown Other)
      in
      let env = E.inside_branch env in
      let handler, r = simplify env r handler in
  ...
  | Switch (arg, sw) ->
      ...
      simplify_free_variable env arg ~f:(fun env arg arg_approx ->
          ...
          let filtered_consts =
            filter_branches
              A.potentially_taken_const_switch_branch
              sw.consts
              []
          in
          let filtered_blocks =
            filter_branches
              A.potentially_taken_block_switch_branch
              sw.blocks
              []
          in
              ...
              let sw, r =
                List.fold_right
                  (fun (str, lam) (sw, r) ->
                    ...
                    (str, lam) :: sw, R.meet_approx r env approx )
                  sw
                  ([], r)
              in
              let def, r =
...
and duplicate_function ~env ~(set_of_closures : Flambda.set_of_closures)
  ...
  let function_decl =
    ...
    | exception Not_found ->
        Misc.fatal_errorf
          "duplicate_function: cannot find function %a"
          Variable.print
          fun_var
  ...
  let ( free_vars
    ...
    Inline_and_simplify_aux.prepare_to_simplify_set_of_closures
      ...
      ~freshen:false
      ~only_for_function_decl:(Some function_decl)
  in
  let function_decl =
    match Variable.Map.find fun_var function_decls.funs with
    | exception Not_found ->
        Misc.fatal_errorf
          "duplicate_function: cannot find function %a (2)"
          Variable.print
          fun_var
    | function_decl -> function_decl
  in
...
let constant_defining_value_approx env
  ...
  | Project_closure (set_of_closures_symbol, closure_id) -> (
    ...
    | Some set_of_closures_approx -> (
        ...
        | Wrong ->
            Misc.fatal_errorf
              "Wrong approximation for [Project_closure] when being used \
    ...
    List.fold_left
      (fun building_env (symbol, _) ->
        E.add_symbol
          building_env
          symbol
          (A.value_unresolved (Symbol symbol)) )
      orig_env
      defs
  in
...
let simplify_constant_defining_value env r symbol
    (constant_defining_value : Flambda.constant_defining_value) =
  let r, constant_defining_value, approx =
    ...
    | Set_of_closures set_of_closures ->
        if Variable.Map.cardinal set_of_closures.free_vars <> 0
        then
          Misc.fatal_errorf
...
let rec simplify_program_body env r (program : Flambda.program_body) :
  ...
  | Let_rec_symbol (defs, program) ->
      let set_of_closures_defs, other_defs =
        List.partition
          (function _, Flambda.Set_of_closures _ -> true | _ -> false)
          defs
      in
  ...
  | Let_symbol (symbol, constant_defining_value, program) ->
      let r, constant_defining_value, approx =
        simplify_constant_defining_value
          env
          r
          symbol
          constant_defining_value
...
let run ~never_inline ~backend ~prefixname ~round ~ppf_dump program =
  ...
  then
    Misc.fatal_error
      (Format.asprintf
         "Remaining static exceptions: %a@.%a@."
         Static_exception.Set.print
         (R.used_static_exceptions r)
         Flambda.print_program
         result);
  assert (Static_exception.Set.is_empty (R.used_static_exceptions r));
  ( if !Clflags.inlining_report
  then
