...
let inline env r ~lhs_of_application ~closure_id_being_applied
    ...
    ~(function_body : A.function_body) ~value_set_of_closures
    ~only_use_of_function ~original ~recursive ~(args : Variable.t list)
  ...
  let unrolling, always_inline, never_inline, env =
    ...
    | Some count ->
        ...
        then
          let env =
            E.continue_actively_unrolling env set_of_closures_origin
          in
          true, true, false, env
  ...
  let try_inlining =
    ...
    then Don't_try_it S.Not_inlined.Self_call
    else if not (E.inlining_allowed env function_decl.closure_origin)
    then Don't_try_it S.Not_inlined.Unrolling_depth_exceeded
    else if only_use_of_function || always_inline
    then Try_it
    else if never_inline
    then Don't_try_it S.Not_inlined.Annotation
    else if (not (E.unrolling_allowed env set_of_closures_origin))
            && Lazy.force recursive
    then Don't_try_it S.Not_inlined.Unrolling_depth_exceeded
    else if T.equal remaining_inlining_threshold T.Never_inline
    then
      let threshold =
      ...
      | Some body_size ->
          let wsb =
              ...
              Variable.Set.fold
                (fun v acc ->
                  try
                    ...
                    | Some v ->
                        if E.mem env v
                        then Inlining_cost.Benefit.remove_prim acc
                        else acc
                    | None -> acc
                  with Not_found -> acc )
            ...
            W.create_estimate
              ...
              ~benefit
          in
          if not (W.evaluate wsb)
          then Don't_try_it (S.Not_inlined.Without_subfunctions wsb)
          else Try_it
      | None ->
  ...
  | Try_it ->
      ...
      let body, r_inlined =
        (* First we construct the code that would result from copying the body of
         ...
         site. *)
        Inlining_transforms.inline_by_copying_function_body
          ~env
          ~r:(R.reset_benefit r)
          ~lhs_of_application
      ...
      in
      if always_inline
      then keep_inlined_version S.Inlined.Annotation
      else if only_use_of_function
      then keep_inlined_version S.Inlined.Decl_local_to_application
      else
        let wsb =
          W.create
            ~original
        ...
        in
        if W.evaluate wsb
        then keep_inlined_version (S.Inlined.Without_subfunctions wsb)
        else if num_direct_applications_seen < 1
        then
...
let specialise env r ~lhs_of_application
    ...
    ~(value_set_of_closures : A.value_set_of_closures) ~args ~args_approxs
    ~dbg ~simplify ~original ~recursive ~self_call ~inlining_threshold
    ~fun_cost ~inline_requested ~specialise_requested =
  ...
  let try_specialising =
    ...
    then Don't_try_it S.Not_specialised.Classic_mode
    else if self_call
    then Don't_try_it S.Not_specialised.Self_call
    else if always_specialise && not (Lazy.force has_no_useful_approxes)
    then Try_it
    else if never_specialise
    then Don't_try_it S.Not_specialised.Annotation
    else if T.equal remaining_inlining_threshold T.Never_inline
    then
      let threshold =
        match inlining_threshold with
    ...
    else if not (Variable.Map.is_empty free_vars)
    then Don't_try_it S.Not_specialised.Not_closed
    else if not (Lazy.force recursive)
    then Don't_try_it S.Not_specialised.Not_recursive
    else if Variable.Map.is_empty (Lazy.force invariant_params)
    then Don't_try_it S.Not_specialised.No_invariant_parameters
    else if Lazy.force has_no_useful_approxes
    then Don't_try_it S.Not_specialised.No_useful_approximations
    else Try_it
  in
      ...
      | Some (expr, r_inlined) ->
          ...
          then
            let r_inlined =
              ...
              then
                R.map_benefit
                  ...
                  (Inlining_cost.Benefit.max
                     ~round:(E.round env)
                     Inlining_cost.Benefit.(
                       requested_inline ~size_of:expr zero))
              else r_inlined
            in
            let r =
              R.map_benefit
            ...
            then
              ...
              in
              let application_env =
                E.set_never_inline_inside_closures env
              in
              let res = simplify application_env r expr in
              let decision =
...
let for_call_site ~env ~r ~(function_decls : A.function_declarations)
    ...
    ~(function_decl : A.function_declaration)
    ~(value_set_of_closures : A.value_set_of_closures) ~args ~args_approxs
    ~dbg ~simplify ~inline_requested ~specialise_requested =
  if List.length args <> List.length args_approxs
  then
    Misc.fatal_error
  ...
  | Some {stub; _} ->
      ...
      then
        ...
        original, original_r
      else if function_decls.is_classic_mode
      then (
        let env =
        ...
        let simpl =
          ...
          | Some function_body -> (
              ...
              | Try_it ->
                 ...
                 recursive *)
                    E.inside_unrolled_function
                      env
                      function_decls.set_of_closures_origin
                  in
                  let env =
                    E.inside_inlined_function
                      env
                      function_decl.closure_origin
                  in
        ...
        let simpl =
          ...
          then Original (D.Prevented Function_prevented_from_inlining)
          else if E.inlining_level env >= max_level
          then Original (D.Prevented Level_exceeded)
          else
            let self_call =
              E.inside_set_of_closures_declaration
                function_decls.set_of_closures_origin
