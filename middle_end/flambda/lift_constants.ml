...
let rec tail_variable : Flambda.t -> Variable.t option = function
  | Var v -> Some v
  | Let_rec (_, e) | Let_mutable { body = e } | Let { body = e; _ } ->
      tail_variable e
  | _ -> None
...
let assign_symbols_and_collect_constant_definitions
    ~(backend : (module Backend_intf.S)) ~(program : Flambda.program)
  ...
  let assign_symbol var (named : Flambda.named) =
    if not (Inconstant_idents.variable var inconstants) then
      ...
      | Set_of_closures
          ...
          assert (not (Inconstant_idents.closure set_of_closures_id inconstants));
          assign_symbol ();
          ...
          Variable.Map.iter
            (fun fun_var _ ->
              ...
              in
              Variable.Tbl.add var_to_definition_tbl fun_var project_closure)
            funs
      | Move_within_set_of_closures
          ({ closure = _; start_from = _; move_to } as move) ->
      ...
      | Prim (Pmakearray ((Pfloatarray as kind), mutability), args, _) ->
          assign_symbol ();
          record_definition
            (AA.Allocated_const (Array (kind, mutability, args)))
      | Prim (Pduparray (kind, mutability), [ arg ], _) ->
          assign_symbol ();
          record_definition
            (AA.Allocated_const (Duplicate_array (kind, mutability, arg)))
      | Prim _ ->
  ...
  let record_set_of_closure_equalities
    ...
    Variable.Map.iter
      (fun arg (var : Flambda.specialised_to) ->
    ...
    Variable.Map.iter
      (fun arg (spec_to : Flambda.specialised_to) ->
        if not (Inconstant_idents.variable arg inconstants) then
          Variable.Tbl.add var_to_definition_tbl arg (AA.Variable spec_to.var))
...
let variable_field_definition (var_to_symbol_tbl : Symbol.t Variable.Tbl.t)
    (var_to_definition_tbl :
      Alias_analysis.constant_defining_value Variable.Tbl.t) (var : Variable.t)
    : Flambda.constant_defining_value_block_field =
  try Symbol (Variable.Tbl.find var_to_symbol_tbl var)
  with Not_found -> (
    ...
    | const_defining_value ->
        Misc.fatal_errorf "Unexpected pattern for a constant: %a: %a"
          Variable.print var Alias_analysis.print_constant_defining_value
          const_defining_value
    | exception Not_found ->
        Misc.fatal_errorf "No associated symbol for the constant %a"
          Variable.print var)

let resolve_variable (aliases : Alias_analysis.allocation_point Variable.Map.t)
...
let translate_set_of_closures (inconstants : Inconstant_idents.result)
...
let translate_constant_set_of_closures (inconstants : Inconstant_idents.result)
  ...
  Symbol.Map.map
    (fun (const : Flambda.constant_defining_value) ->
      match const with
      | Flambda.Allocated_const _ | Flambda.Block _ | Flambda.Project_closure _
        ->
          const
...
let find_original_set_of_closure
    ...
    (var_to_symbol_tbl : Symbol.t Variable.Tbl.t)
    (var_to_definition_tbl :
      Alias_analysis.constant_defining_value Variable.Tbl.t) project_closure_map
    var =
  let rec loop var =
    match Variable.Map.find var aliases with
    | Variable var -> (
    ...
    | Symbol s -> (
        ...
        | exception Not_found ->
            Misc.fatal_errorf
              "find_original_set_of_closure: cannot find symbol %a in the \
               project-closure map"
              Symbol.print s
        | s -> s)
  in
...
let translate_definition_and_resolve_alias inconstants
  ...
  let resolve_float_array_involving_variables
    ...
    let find_float_symbol_definition sym =
      ...
      | const_defining_value ->
          Misc.fatal_errorf "Bad definition for float array member %a: %a"
            Symbol.print sym Flambda.print_constant_defining_value
            const_defining_value
    in
    let floats =
      List.map
  ...
  | Block (tag, fields) ->
      Some
        (Flambda.Block
           ( tag,
             List.map
               (resolve_variable aliases var_to_symbol_tbl var_to_definition_tbl)
               fields ))
  ...
  | Allocated_const (Duplicate_array (Pfloatarray, mutability, var)) -> (
      ...
      let (constant_defining_value : Alias_analysis.constant_defining_value) =
        match Variable.Map.find var aliases with
        | exception Not_found -> Variable.Tbl.find var_to_definition_tbl var
        | Variable var -> Variable.Tbl.find var_to_definition_tbl var
        | Symbol sym -> (
            match Symbol.Map.find sym symbol_definition_map with
              ...
              | Project_closure _ ) as wrong ->
                Misc.fatal_errorf
                  "Lift_constants.translate_definition_and_resolve_alias: \
                   Duplicate Pfloatarray %a with symbol %a mapping to wrong \
                   constant defining value %a"
                  Variable.print var
            ...
            | exception Not_found -> (
                ...
                | Value_float_array value_float_array -> (
                    ...
                    | None ->
                        Misc.fatal_errorf
                          "Lift_constants.translate_definition_and_resolve_alias: \
                           Duplicate Pfloatarray %a with not completely known \
                           float array from symbol: %a"
                          Variable.print var
                          Alias_analysis.print_constant_defining_value
                          definition
                    | Some l ->
                        Alias_analysis.Allocated_const
                          (Normal (Immutable_float_array l)))
                | wrong ->
      ...
      | const ->
          Misc.fatal_errorf
            "Lift_constants.translate_definition_and_resolve_alias: Duplicate \
             Pfloatarray %a with wrong argument: %a"
            Variable.print var Alias_analysis.print_constant_defining_value
            const)
  ...
  | Allocated_const (Array (_, _, _)) ->
      Misc.fatal_errorf
        "Lift_constants.translate_definition_and_resolve_alias: Array with \
         non-Pfloatarray kind: %a"
        Alias_analysis.print_constant_defining_value definition
  | Project_closure { set_of_closures; closure_id } -> (
      match Variable.Map.find set_of_closures aliases with
      | Symbol s -> Some (Flambda.Project_closure (s, closure_id))
      ...
      | Variable v -> (
          ...
          | s -> Some (Flambda.Project_closure (s, closure_id))
          | exception Not_found ->
              Format.eprintf "var: %a@." Variable.print v;
              assert false))
  | Move_within_set_of_closures { closure; move_to } ->
      let set_of_closure_symbol =
        find_original_set_of_closure aliases var_to_symbol_tbl
          var_to_definition_tbl project_closure_map closure
      in
      Some (Flambda.Project_closure (set_of_closure_symbol, move_to))
  | Set_of_closures set_of_closures ->
      let set_of_closures =
        translate_set_of_closures inconstants aliases var_to_symbol_tbl
          var_to_definition_tbl set_of_closures
      in
...
let translate_definitions_and_resolve_alias inconstants
    (aliases : Alias_analysis.allocation_point Variable.Map.t)
    (var_to_symbol_tbl : Symbol.t Variable.Tbl.t)
    (var_to_definition_tbl :
      Alias_analysis.constant_defining_value Variable.Tbl.t)
    symbol_definition_map project_closure_map ~backend =
  Variable.Tbl.fold
    (fun var def map ->
      match
...
let add_definition_of_symbol constant_definitions
  ...
  | Symbol_SCC.No_loop sym -> (
      ...
      | tag, fields, _previous -> Initialize_symbol (sym, tag, fields, program)
      | exception Not_found -> (
          match Symbol.Tbl.find effect_tbl sym with
          | expr, _previous -> Effect (expr, program)
          | exception Not_found ->
...
let introduce_free_variables_in_set_of_closures
  ...
  let add_definition_and_make_substitution var (expr, subst) =
    ...
    | def ->
        ...
        let named : Flambda.named =
          match def with Symbol sym -> Symbol sym | Const c -> Const c
        in
      ...
      ~funs:
        (Variable.Map.map
           (fun (func_decl : Flambda.function_declaration) ->
             ...
             in
             if Variable.Map.is_empty subst then func_decl
             else (
               done_something := true;
  ...
  let free_vars =
    ...
    Variable.Map.filter
      ...
      free_vars
  in
  let free_vars = Flambda_utils.clean_projections ~which_variables:free_vars in
  let specialised_args =
    (* Keep only those that are not rewritten to constants. *)
    Variable.Map.filter
      (fun _ (spec_to : Flambda.specialised_to) ->
  ...
  in
  if not !done_something then set_of_closures
  else
...
let introduce_free_variables_in_sets_of_closures
  ...
  Symbol.Map.map
    (fun (def : Flambda.constant_defining_value) ->
      match def with
      | Allocated_const _ | Block _ | Project_closure _ -> def
      | Set_of_closures set_of_closures ->
          Flambda.Set_of_closures
            (introduce_free_variables_in_set_of_closures var_to_block_field_tbl
               set_of_closures))
    ...
    | Set_of_closures { function_decls = { funs } } ->
        Variable.Map.iter
          (fun fun_var _ ->
            ...
            in
            Symbol.Tbl.add symbol_definition_tbl closure_symbol project_closure)
          funs
    | Project_closure _ | Allocated_const _ | Block _ -> ()
  in
...
let project_closure_map symbol_definition_map =
  Symbol.Map.fold
    (fun sym (const : Flambda.constant_defining_value) acc ->
      match const with
      | Project_closure (set_of_closures, _) ->
          Symbol.Map.add sym set_of_closures acc
      | Set_of_closures _ -> Symbol.Map.add sym sym acc
      | Allocated_const _ | Block _ -> acc)
    symbol_definition_map Symbol.Map.empty
...
let lift_constants (program : Flambda.program) ~backend =
  ...
  let program_body : Flambda.program_body =
    Let_symbol
      (the_dead_constant, Allocated_const (Nativeint 0n), program.program_body)
  in
  let program : Flambda.program = { program with program_body } in
  let inconstants =
  ...
  in
  let aliases =
    Alias_analysis.run var_to_definition_tbl initialize_symbol_to_definition_tbl
      let_symbol_to_definition_tbl ~the_dead_constant
  in
  ...
  let translated_definitions =
    translate_definitions_and_resolve_alias inconstants
      (aliases : Alias_analysis.allocation_point Variable.Map.t)
      (var_to_symbol_tbl : Symbol.t Variable.Tbl.t)
      (var_to_definition_tbl
        : Alias_analysis.constant_defining_value Variable.Tbl.t)
  ...
  let constant_definitions =
    ...
    Symbol.Map.union
      (fun _sym (c1 : Flambda.constant_defining_value)
           (c2 : Flambda.constant_defining_value) ->
        match (c1, c2) with
        | Project_closure (s1, closure_id1), Project_closure (s2, closure_id2)
          when Symbol.equal s1 s2 && Closure_id.equal closure_id1 closure_id2 ->
            Some c1
        | Project_closure (s1, closure_id1), Project_closure (s2, closure_id2)
          ->
            Format.eprintf "not equal project closure@. s %a %a@. cid %a %a@."
              Symbol.print s1 Symbol.print s2 Closure_id.print closure_id1
              Closure_id.print closure_id2;
            assert false
        | _ -> assert false)
    ...
    Flambda_iterators.map_named
      (function
        ...
        | ( Symbol _ | Const _ | Allocated_const _ | Project_closure _
          | Move_within_set_of_closures _ | Prim _ | Expr _ | Read_mutable _
          | Read_symbol_field _ ) as named ->
            named)
      expr
  in
  let constant_definitions =
  ...
  in
  let program_body =
    add_definitions_of_symbols constant_definitions initialize_symbol_tbl
      effect_tbl
