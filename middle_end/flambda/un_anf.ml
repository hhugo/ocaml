...
let closure_environment_var (ufunction : Clambda.ufunction) =
  ...
  if List.length ufunction.params = ufunction.arity + 1 then
    ...
    (assert (VP.name env_var = "env");
     Some env_var)
  else (* closed function, no environment *)
    None

type var_uses = Zero | One | More_than_one | Assigned
type var = { definition_depth : int; uses : var_uses }

let incr_uses { definition_depth; uses } depth =
  assert (definition_depth <= depth);
  let uses =
    match uses with
    | Zero -> if definition_depth < depth then More_than_one else One
    | One -> More_than_one
...
let make_var_info (clam : Clambda.ulambda) : var_info =
  ...
  let rec loop ~depth : Clambda.ulambda -> unit =
    function
    ...
    | Uclosure (functions, captured_variables) ->
      ...
      List.iter
        (fun ({ Clambda.label; arity; params; return; body; dbg; env } as clos)
         ->
           begin match closure_environment_var clos with
           | None -> ()
           | Some env_var ->
           ...
           ignore_debuginfo dbg;
           ignore_var_option env) functions
    | Uoffset (expr, offset) ->
      loop ~depth expr;
      ignore_int offset
    ...
    | Uswitch
        (cond,
         { us_index_consts; us_actions_consts; us_index_blocks;
           us_actions_blocks },
         dbg)
      ->
      loop ~depth cond;
  ...
  let (linear_let_bound_vars, used_let_bound_vars, assigned) =
    V.Tbl.fold
      (fun var desc (linear, used, assigned as acc) ->
         match desc.uses with
         | Zero -> acc
         | One -> V.Set.add var linear, V.Set.add var used, assigned
         | More_than_one -> linear, V.Set.add var used, assigned
         | Assigned -> linear, V.Set.add var used, V.Set.add var assigned) t
      (V.Set.empty, V.Set.empty, V.Set.empty)
  in
  {
    used_let_bound_vars;
    linear_let_bound_vars;
    assigned;
...
let let_bound_vars_that_can_be_moved var_info (clam : Clambda.ulambda) =
  ...
  let examine_argument_list args =
    let rec loop let_bound_vars (args : Clambda.ulambda list) =
      ...
      | let_bound_var :: let_bound_vars, Uvar arg :: args
        when V.same let_bound_var arg && not (V.Set.mem arg var_info.assigned)
        ->
  ...
  in
  let rec loop : Clambda.ulambda -> unit =
    function
    | Uvar var -> if V.Set.mem var var_info.assigned then let_stack := []
    | Uconst const -> ignore_uconstant const
    | Udirect_apply (label, args, dbg) ->
      ignore_function_label label;
    ...
    | Uclosure (functions, captured_variables) ->
      ...
      List.iter
        (fun { Clambda.label; arity; params; return; body; dbg; env } ->
           ignore_function_label label;
           ignore_int arity;
           ignore_params_with_value_kind params;
           ignore_value_kind return;
    ...
    | Ulet (_let_kind, _value_kind, var, def, body) ->
      ...
      | _ ->
        loop def;
        (if V.Set.mem var var_info.linear_let_bound_vars then
    ...
    | Uswitch
      ...
      Array.iter
        (fun action ->
           let_stack := [];
           loop action) us_actions_consts;
      ignore_int_array us_index_blocks;
      Array.iter
        (fun action ->
           let_stack := [];
           loop action) us_actions_blocks;
      ignore_debuginfo dbg;
      let_stack := []
    | Ustringswitch (cond, branches, default) ->
      examine_argument_list [ cond ];
...
=
  ...
  | Uvar var ->
    if not (V.Set.mem var is_let_moveable) then
      ...
      | clam -> clam
      | exception Not_found ->
        Misc.fatal_errorf "substitute_let_moveable: Unbound variable %a" V.print
          var
      end
  | Uconst _ -> clam
  | Udirect_apply (label, args, dbg) ->
  ...
  | Uclosure (functions, variables_bound_by_the_closure) ->
    let functions =
      List.map
        (fun (ufunction : Clambda.ufunction) ->
           { ufunction with
  ...
  | Uletrec (defs, body) ->
    let defs =
      List.map
        (fun (var, def) -> var, substitute_let_moveable is_let_moveable env def)
        defs
    in
    let body = substitute_let_moveable is_let_moveable env body in
    Uletrec (defs, body)
  ...
  | Uswitch (cond, sw, dbg) ->
    ...
    let sw =
      { sw with
        ...
        us_actions_consts =
          substitute_let_moveable_array is_let_moveable env sw.us_actions_consts;
        us_actions_blocks =
          substitute_let_moveable_array is_let_moveable env sw.us_actions_blocks
      }
    in
    Uswitch (cond, sw, dbg)
  | Ustringswitch (cond, branches, default) ->
    ...
    let branches =
      List.map
        (fun (s, branch) ->
           s, substitute_let_moveable is_let_moveable env branch) branches
    in
    let default =
...
let both_moveable a b =
  ...
  | Constant, Moveable | Moveable, Constant | Moveable, Moveable -> Moveable
  | Constant, Fixed
  | Moveable, Fixed
  | Fixed, Constant
  | Fixed, Moveable
...
let primitive_moveable
    (prim : Clambda_primitives.primitive) (args : Clambda.ulambda list)
    (var_info : var_info)
=
  ...
  | Pfield _, [ Uvar var ] when V.Set.mem var var_info.closure_environment ->
    ...
    Moveable
  | _ ->
    begin match Semantics_of_primitives.for_primitive prim with
...
=
  ...
  | Uvar var ->
    ...
    | exception Not_found ->
      let moveable : moveable =
        if V.Set.mem var var_info.assigned then Fixed else Moveable
      in
      clam, moveable
    end
  | Uconst _ ->
  ...
  | Uclosure (functions, variables_bound_by_the_closure) ->
    let functions =
      List.map
        (fun (ufunction : Clambda.ufunction) ->
           { ufunction with  body = un_anf var_info env ufunction.body })
        functions
    in
    let variables_bound_by_the_closure =
  ...
  | Ulet (let_kind, value_kind, var, def, body) ->
    ...
    let maybe_for_debugger (body, moveable) : Clambda.ulambda * moveable =
      if not !Clflags.debug_full then
        ...
        | Uvar alias_of ->
          Uphantom_let (var, Some (Clambda.Uphantom_var alias_of), body),
          moveable
        | _ -> Uphantom_let (var, None, body), moveable
    in
    ...
    | Moveable, false, _, _ (* Moveable but not used linearly. *)
    | Fixed, _, _, _
      ->
  ...
  | Uletrec (defs, body) ->
    let defs = List.map (fun (var, def) -> var, un_anf var_info env def) defs in
  ...
  | Ustringswitch (cond, branches, default) ->
    let cond = un_anf var_info env cond in
    let branches =
      List.map (fun (s, branch) -> s, un_anf var_info env branch) branches
    in
  ...
  | Uifthenelse (cond, ifso, ifnot) ->
    ...
    let moveable =
      both_moveable cond_moveable (both_moveable ifso_moveable ifnot_moveable)
    in
  ...
  | Uunreachable -> Uunreachable, Fixed
...
=
  List.fold_right
    (fun clam (l, acc_moveable) ->
       let (clam, moveable) = un_anf_and_moveable var_info env clam in
       clam :: l, both_moveable moveable acc_moveable) clams
    ([], (Moveable : moveable))
...
let apply ~what ~ppf_dump clam =
  ...
  in
  let clam =
    substitute_let_moveable let_bound_vars_that_can_be_moved V.Map.empty clam
  in
  let var_info = make_var_info clam in
  let clam = un_anf var_info V.Map.empty clam in
  (if !Clflags.dump_clambda then
     Format.fprintf ppf_dump "@.un-anf (%a):@ %a@." Symbol.print what
