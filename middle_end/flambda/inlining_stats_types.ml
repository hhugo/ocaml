...
let print_calculation ~depth ~title ~subfunctions ppf wsb =
  ...
  Format.fprintf ppf "@[<h>%a %s@]@;@;@[%a@]" print_stars (depth + 1) title
    (Wsb.print_description ~subfunctions)
    wsb;
...
module Inlined = struct
  ...
  let summary ppf = function
    | Classic_mode ->
        Format.pp_print_text ppf
          "This function was inlined because it was small enough to be inlined \
           in `-Oclassic'"
    | Annotation ->
        Format.pp_print_text ppf
    ...
    | Without_subfunctions _ ->
        Format.pp_print_text ppf
          "This function was inlined because the expected benefit outweighed \
           the change in code size."
    | With_subfunctions _ ->
        Format.pp_print_text ppf
          "This function was inlined because the expected benefit outweighed \
           the change in code size."
  ...
  let calculation ~depth ppf = function
    ...
    | Without_subfunctions wsb ->
        print_calculation ~depth ~title:"Inlining benefit calculation"
...
module Not_inlined = struct
  ...
  let summary ppf = function
    | Classic_mode ->
        Format.pp_print_text ppf
          "This function was not inlined because it was too large to be \
           inlined in `-Oclassic'."
    | Above_threshold size ->
        Format.pp_print_text ppf
          "This function was not inlined because it was larger than the \
    ...
    | No_useful_approximations ->
        Format.pp_print_text ppf
          "This function was not inlined because there was no useful \
           information about any of its parameters, and it was not \
           particularly small."
    | Unrolling_depth_exceeded ->
        Format.pp_print_text ppf
          "This function was not inlined because its unrolling depth was \
           exceeded."
    | Self_call ->
        Format.pp_print_text ppf
    ...
    | Without_subfunctions _ ->
        Format.pp_print_text ppf
          "This function was not inlined because the expected benefit did not \
  ...
  let calculation ~depth ppf = function
    | Classic_mode | Above_threshold _ | Annotation | No_useful_approximations
    | Unrolling_depth_exceeded | Self_call ->
        ()
    | Without_subfunctions wsb ->
        print_calculation ~depth ~title:"Inlining benefit calculation"
          ~subfunctions:false ppf wsb
...
module Specialised = struct
  ...
  let calculation ~depth ppf = function
    | Annotation -> ()
    | Without_subfunctions wsb ->
        print_calculation ~depth ~title:"Specialising benefit calculation"
          ~subfunctions:false ppf wsb
    | With_subfunctions (_, wsb) ->
        print_calculation ~depth ~title:"Specialising benefit calculation"
          ~subfunctions:true ppf wsb
...
module Not_specialised = struct
  ...
  let summary ppf = function
    | Classic_mode ->
        Format.pp_print_text ppf
          "This function was not specialised because it was compiled with \
           `-Oclassic'."
    | Above_threshold size ->
        Format.pp_print_text ppf
          "This function was not specialised because it was larger than the \
           current size threshold";
        Format.fprintf ppf "(%i)" size
    | Annotation ->
        Format.pp_print_text ppf
    ...
    | No_invariant_parameters ->
        Format.pp_print_text ppf
          "This function was not specialised because it has no invariant \
           parameters."
    | No_useful_approximations ->
        Format.pp_print_text ppf
          "This function was not specialised because there was no useful \
           information about any of its invariant parameters."
    | Self_call ->
        Format.pp_print_text ppf
          "This function was not specialised because it was a self call."
    | Not_beneficial _ ->
        Format.pp_print_text ppf
          "This function was not specialised because the expected benefit did \
           not outweigh the change in code size."

  let calculation ~depth ppf = function
    | Classic_mode | Above_threshold _ | Annotation | Not_recursive | Not_closed
...
module Prevented = struct
  ...
  let summary ppf = function
    ...
    | Level_exceeded ->
        Format.pp_print_text ppf
          "This function was prevented from inlining or specialising because \
...
module Decision = struct
  ...
  let summary ppf = function
    | Prevented p -> Prevented.summary ppf p
    | Specialised s -> Specialised.summary ppf s
    | Inlined (s, i) ->
        Format.fprintf ppf "@[<v>@[%a@]@;@;@[%a@]@]" Not_specialised.summary s
          Inlined.summary i
    | Unchanged (s, i) ->
        Format.fprintf ppf "@[<v>@[%a@]@;@;@[%a@]@]" Not_specialised.summary s
          Not_inlined.summary i

  let calculation ~depth ppf = function
    | Prevented _ -> ()
    | Specialised s -> Specialised.calculation ~depth ppf s
    | Inlined (s, i) ->
