...
let equal_descr (d1 : descr) (d2 : descr) : bool =
  ...
  | ( ( Value_block (_, _)
      ...
      | Value_int _ | Value_char _ | Value_float _ | Value_float_array _
      | Value_boxed_int _ | Value_string _ | Value_closure _
      | Value_set_of_closures _ | Value_unknown_descr ),
      ( Value_block (_, _)
      | Value_mutable_block (_, _)
      | Value_int _ | Value_char _ | Value_float _ | Value_float_array _
      | Value_boxed_int _ | Value_string _ | Value_closure _
...
let create ~sets_of_closures ~values ~symbol_id ~offset_fun ~offset_fv
...
let create_transient ~sets_of_closures ~values ~symbol_id ~invariant_params
    ~recursive ~relevant_local_closure_ids ~relevant_imported_closure_ids
...
let t_of_transient transient ~program:_ ~local_offset_fun ~local_offset_fv
...
let print_value_set_of_closures ppf (t : value_set_of_closures) =
  ...
  Format.fprintf ppf
    "((set_of_closures_id %a) (bound_vars %a) (free_vars %a) (results %a) \
     (aliased_symbol %a))"
    Set_of_closures_id.print t.set_of_closures_id print_bound_vars t.bound_vars
    print_free_vars t.free_vars print_results t.results print_aliased_symbol
    t.aliased_symbol
...
let print_approx_components ppf ~symbol_id ~values
  ...
  and print_set_of_closures ppf
    ...
    then fprintf ppf "%a" Set_of_closures_id.print set_of_closures_id
      ...
      in
      fprintf ppf "{%a: %a%a => %a}" Set_of_closures_id.print set_of_closures_id
        print_binding bound_vars print_alias aliased_symbol
        (Closure_id.Map.print print_approx)
        results)
  ...
  let rec print_recorded_symbols () =
    if not (Queue.is_empty symbols_to_print) then (
      ...
      (match Symbol.Map.find sym symbol_id with
      | exception Not_found -> ()
      | id ->
