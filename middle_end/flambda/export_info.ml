...
type value_float_array = { contents : value_float_array_contents; size : int }
...
and approx = Value_unknown | Value_id of Export_id.t | Value_symbol of Symbol.t

let equal_approx (a1 : approx) (a2 : approx) =
  match a1, a2 with
  | Value_unknown, Value_unknown -> true
  | Value_id id1, Value_id id2 -> Export_id.equal id1 id2
  | Value_symbol s1, Value_symbol s2 -> Symbol.equal s1 s2
  | (Value_unknown
     | Value_symbol _
     | Value_id _),
    (Value_unknown
...
let equal_set_of_closures
    (s1 : value_set_of_closures) (s2 : value_set_of_closures)
=
...
let equal_descr (d1 : descr) (d2 : descr) : bool =
  match d1, d2 with
  | Value_unknown_descr, Value_unknown_descr -> true
  ...
  | Value_mutable_block (t1, s1), Value_mutable_block (t2, s2) ->
    Tag.equal t1 t2 && s1 = s2
  | Value_int i1, Value_int i2 -> i1 = i2
  | Value_char c1, Value_char c2 -> c1 = c2
  | Value_float f1, Value_float f2 -> f1 = f2
  | Value_float_array s1, Value_float_array s2 -> s1 = s2
  | Value_boxed_int (t1, v1), Value_boxed_int (t2, v2) ->
    A.equal_boxed_int t1 v1 t2 v2
  | Value_string s1, Value_string s2 -> s1 = s2
...
let create
    ~sets_of_closures ~values ~symbol_id ~offset_fun ~offset_fv
    ~constant_closures ~invariant_params ~recursive
=
  {
    sets_of_closures;
    values;
...
let create_transient
    ...
    ~relevant_local_vars_within_closure ~relevant_imported_vars_within_closure
=
  {
    sets_of_closures;
    values;
...
let t_of_transient
    transient ~program:_ ~local_offset_fun ~local_offset_fv ~imported_offset_fun
    ~imported_offset_fv ~constant_closures
=
  let offset_fun =
    let fold_map set =
      Closure_id.Map.fold
        (fun key value unchanged ->
  ...
  let eidmap_disjoint_union ?eq map1 map2 =
    Compilation_unit.Map.merge
      (fun _id map1 map2 ->
         ...
         | None, None -> None
         | None, Some map | Some map, None -> Some map
         | Some map1, Some map2 ->
           Some (Export_id.Map.disjoint_union ?eq map1 map2)) map1 map2
  in
  let int_eq (i : int) j = i = j in
  {
    ...
    sets_of_closures =
      Set_of_closures_id.Map.disjoint_union t1.sets_of_closures
        t2.sets_of_closures;
    symbol_id =
      Symbol.Map.disjoint_union ~print:Export_id.print t1.symbol_id t2.symbol_id;
    offset_fun =
      Closure_id.Map.disjoint_union ~eq:int_eq t1.offset_fun t2.offset_fun;
    offset_fv =
      Var_within_closure.Map.disjoint_union ~eq:int_eq t1.offset_fv t2.offset_fv;
    constant_closures =
    ...
    invariant_params =
      Set_of_closures_id.Map.disjoint_union
        ...
        ~eq:(Variable.Map.equal Variable.Set.equal) t1.invariant_params
        t2.invariant_params;
    recursive =
      Set_of_closures_id.Map.disjoint_union ~print:Variable.Set.print
        ~eq:Variable.Set.equal t1.recursive t2.recursive
  }
...
let find_description (t : t) eid = find_value eid t.values
...
let print_value_set_of_closures ppf (t : value_set_of_closures) =
  let print_bound_vars ppf bound_vars =
    Format.fprintf ppf "(%a)" (Var_within_closure.Map.print print_raw_approx)
      bound_vars
  in
  let print_free_vars ppf free_vars =
    Format.fprintf ppf "(%a)" (Variable.Map.print Flambda.print_specialised_to)
      free_vars
  in
  ...
  in
  Format.fprintf ppf
    "((set_of_closures_id %a) (bound_vars %a) (free_vars %a) (results %a) \
     (aliased_symbol %a))" Set_of_closures_id.print t.set_of_closures_id
    print_bound_vars t.bound_vars print_free_vars t.free_vars print_results
    t.results print_aliased_symbol t.aliased_symbol

let print_value_closure ppf (t : value_closure) =
  Format.fprintf ppf "((closure_id %a) (set_of_closures %a))" Closure_id.print
    t.closure_id print_value_set_of_closures t.set_of_closures

let print_value_float_array_contents ppf (value : value_float_array_contents) =
  match value with
...
let print_value_float_array ppf (value : value_float_array) =
  Format.fprintf ppf "((size %d) (contents %a))" value.size
...
let print_raw_descr ppf descr =
  ...
  | Value_block (tag, approx_array) ->
    fprintf ppf "(Value_block (%a %a))" Tag.print tag print_approx_array
      approx_array
  ...
  | Value_float_array value_float_array ->
    fprintf ppf "(Value_float_array %a)" print_value_float_array
      value_float_array
  | Value_boxed_int _ -> fprintf ppf "(Value_Boxed_int)"
  | Value_string value_string ->
    fprintf ppf "(Value_string %a)" print_value_string value_string
  | Value_closure value_closure ->
    fprintf ppf "(Value_closure %a)" print_value_closure value_closure
  | Value_set_of_closures value_set_of_closures ->
    fprintf ppf "(Value_set_of_closures %a)" print_value_set_of_closures
      value_set_of_closures
  | Value_unknown_descr -> fprintf ppf "(Value_unknown_descr)"

let print_approx_components
    ppf ~symbol_id ~values (root_symbols : Symbol.t list)
=
  let fprintf = Format.fprintf in
    ...
    | Value_id id ->
      if Export_id.Set.mem id !printed then
        ...
        (try
           ...
           fprintf ppf "@[<hov 2>(%a:@ %a)@]" Export_id.print id print_descr
             descr
         with
         | Not_found -> fprintf ppf "(%a: Not available)" Export_id.print id)
    | Value_symbol sym ->
      (if not (Symbol.Set.mem sym !recorded_symbol) then
  ...
  and print_set_of_closures
      ppf { set_of_closures_id; bound_vars; aliased_symbol; results }
  =
    if
      Set_of_closures_id.Set.mem set_of_closures_id !printed_set_of_closures
    then
      fprintf ppf "%a" Set_of_closures_id.print set_of_closures_id
    else
      (printed_set_of_closures :=
       ...
       let print_alias ppf =
         function
         | None -> ()
         | Some symbol -> Format.fprintf ppf "@ (alias: %a)" Symbol.print symbol
       in
       fprintf ppf "{%a: %a%a => %a}" Set_of_closures_id.print
         set_of_closures_id print_binding bound_vars print_alias aliased_symbol
         (Closure_id.Map.print print_approx) results)
  and print_binding ppf bound_vars =
    Var_within_closure.Map.iter
      (fun clos_id approx ->
         fprintf ppf "%a -> %a,@ " Var_within_closure.print clos_id print_approx
           approx) bound_vars
  in
  let rec print_recorded_symbols () =
    if not (Queue.is_empty symbols_to_print) then
      let sym = Queue.pop symbols_to_print in
      (begin match Symbol.Map.find sym symbol_id with
       | exception Not_found -> ()
       | id ->
         fprintf ppf "@[<hov 2>%a:@ %a@];@ " Symbol.print sym print_approx
           (Value_id id)
       end;
...
let print_offsets ppf (t : t) =
  ...
  Closure_id.Map.iter
    (fun cid off -> Format.fprintf ppf "%a -> %i@ " Closure_id.print cid off)
    t.offset_fun;
  Format.fprintf ppf "@]@ @[<v 2>offset_fv:@ ";
  Var_within_closure.Map.iter
    (fun vid off ->
       Format.fprintf ppf "%a -> %i@ " Var_within_closure.print vid off)
    t.offset_fv;
  Format.fprintf ppf "@]@ "

let print_functions ppf (t : t) =
  Set_of_closures_id.Map.print A.print_function_declarations ppf
    t.sets_of_closures

let print_all ppf (t, root_symbols : t * Symbol.t list) =
  let fprintf = Format.fprintf in
  fprintf ppf "approxs@ %a@.@." print_approx (t, root_symbols);
  fprintf ppf "functions@ %a@.@." print_functions t
