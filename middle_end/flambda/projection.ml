...
let compare_project_var
    ({ closure = closure1; closure_id = closure_id1; var = var1 } : project_var)
    ({ closure = closure2; closure_id = closure_id2; var = var2 } : project_var)
=
...
let print_project_closure ppf (project_closure : project_closure) =
  Format.fprintf ppf "@[<2>(project_closure@ %a@ from@ %a)@]" Closure_id.print
    project_closure.closure_id Variable.print project_closure.set_of_closures

let print_move_within_set_of_closures
    ppf (move_within_set_of_closures : move_within_set_of_closures)
=
  Format.fprintf ppf
    "@[<2>(move_within_set_of_closures@ %a <-- %a@ (closure = %a))@]"
    Closure_id.print move_within_set_of_closures.move_to Closure_id.print
    move_within_set_of_closures.start_from Variable.print
...
let print_project_var ppf (project_var : project_var) =
  Format.fprintf ppf "@[<2>(project_var@ %a@ from %a=%a)@]"
    Var_within_closure.print project_var.var Closure_id.print
    project_var.closure_id Variable.print project_var.closure
...
(struct
  ...
  let compare t1 t2 =
    ...
    | Field (index1, var1), Field (index2, var2) ->
      ...
      if c <> 0 then c else Variable.compare var1 var2
    | Project_var _, _ -> (-1)
    | _, Project_var _ -> 1
...
let map_projecting_from t ~f : t =
  ...
  | Project_var project_var ->
    let project_var : project_var =
      { project_var with  closure = f project_var.closure }
    in
    Project_var project_var
  | Project_closure project_closure ->
  ...
  | Move_within_set_of_closures move ->
    let move : move_within_set_of_closures =
      { move with  closure = f move.closure }
    in
    Move_within_set_of_closures move
