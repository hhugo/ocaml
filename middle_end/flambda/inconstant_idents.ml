...
module Inconstants (P : Param) (Backend : Backend_intf.S) = struct
  ...
  *)
  ...
  let rec mark_dep =
    function
    | Var id ->
      begin match Variable.Tbl.find variables id with
      | Not_constant -> ()
      | Implication deps ->
        Variable.Tbl.replace variables id Not_constant;
        Queue.push deps mark_queue
      | exception Not_found -> Variable.Tbl.add variables id Not_constant
      end
    | Closure cl ->
      begin match Set_of_closures_id.Tbl.find closures cl with
      | Not_constant -> ()
      | Implication deps ->
    ...
    | Symbol s ->
      begin match Symbol.Tbl.find symbols s with
      | Not_constant -> ()
      | Implication deps ->
        Symbol.Tbl.replace symbols s Not_constant;
        Queue.push deps mark_queue
      | exception Not_found -> Symbol.Tbl.add symbols s Not_constant
      end
    | Symbol_field s ->
      ...
      | Implication deps ->
        ...
        Queue.push deps mark_queue
      | exception Not_found -> Symbol_field.Tbl.add symbol_fields s Not_constant
      end
  
  and mark_deps deps = List.iter mark_dep deps
  
  and complete_marking () =
    while not (Queue.is_empty mark_queue) do
      let deps =
        try Queue.take mark_queue
        with
        | Not_found -> []
      in
      mark_deps deps
  ...
  let register_implication ~in_nc:dep ~implies_in_nc:curr =
    ...
    | Var id ->
      ...
      | exception Not_found -> Variable.Tbl.add variables id (Implication curr)
      end
    | Closure cl ->
      begin match Set_of_closures_id.Tbl.find closures cl with
      | Not_constant ->
    ...
    | Symbol symbol ->
      begin match Symbol.Tbl.find symbols symbol with
      | Not_constant ->
        mark_deps curr;
        complete_marking ()
      | Implication deps ->
      ...
      | exception Not_found -> Symbol.Tbl.add symbols symbol (Implication curr)
      end
  ...
  *)
  let rec mark_loop ~toplevel (curr : dep list) (flam : Flambda.t) =
    ...
    | Proved_unreachable -> mark_curr curr
  ...
  and mark_named ~toplevel curr (named : Flambda.named) =
    ...
    | Symbol symbol ->
      ...
      if
        Compilation_unit.equal current_unit (Symbol.compilation_unit symbol)
      then
        ()
      else
        begin match (Backend.import_symbol symbol).descr with
        | Value_unresolved _ ->
    ...
    *)
    | Prim (Pmakeblock (_tag, Asttypes.Immutable, _value_kind), args, _dbg) ->
      mark_vars args curr
    ...
    *)
    | Prim (Pmakearray (Pfloatarray, Immutable), args, _) -> mark_vars args curr
    | Prim (Pmakearray (Pfloatarray, Mutable), args, _) ->
      (* CR-someday pchambart: Toplevel float arrays could always be
           ...
           initialisation
        *)
      if toplevel then mark_vars args curr else mark_curr curr
    | Prim (Pduparray (Pfloatarray, Immutable), [ arg ], _) -> mark_var arg curr
    | Prim (Pduparray (Pfloatarray, Mutable), [ arg ], _) ->
      if toplevel then mark_var arg curr else mark_curr curr
    ...
    | Move_within_set_of_closures { closure; start_from; move_to } ->
      (* CR-someday mshinwell: We should be able to deem these projections
         ...
         closures.) *)
      if Closure_id.in_compilation_unit start_from compilation_unit then
    ...
    | Prim (_, args, _) ->
      mark_curr curr;
      mark_vars args curr
    | Expr flam -> mark_loop ~toplevel curr flam
  ...
  and mark_loop_set_of_closures
      ~toplevel:_ curr { Flambda.function_decls; free_vars; specialised_args }
  =
    ...
    Variable.Map.iter
      (fun _ (spec_arg : Flambda.specialised_to) ->
         register_implication ~in_nc:(Var spec_arg.var)
    ...
    Variable.Map.iter
      (fun inner_id (var : Flambda.specialised_to) ->
         register_implication ~in_nc:(Var var.var)
           ~implies_in_nc:
           [ Var inner_id; Closure function_decls.set_of_closures_id ])
      free_vars;
    Variable.Map.iter
      (fun fun_id (ffunc : Flambda.function_declaration) ->
         ...
         mark_loop ~toplevel:false [] ffunc.body) function_decls.funs
        ...
        List.iteri
          (fun i field ->
             mark_loop ~toplevel:true
               [ Symbol symbol; Symbol_field (symbol, i) ] field) fields;
        loop program
      | Effect (expr, program) ->
  ...
  let res =
    ...
    { id = variables; closure = closures }
end
  

let inconstants_on_program
    ~compilation_unit ~backend (program : Flambda.program)
=
  let module P = struct
    let program = program
  ...
  in
  let module I = Inconstants(P)(Backend)  in I.res
