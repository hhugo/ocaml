...
let tupled_function_call_stub original_params unboxed_version ~closure_bound_var
  ...
  Flambda.create_function_declaration ~params:[ tuple_param ] ~body ~stub:true
    ~dbg:Debuginfo.none ~inline:Default_inline ~specialise:Default_specialise
    ~is_a_functor:false
...
let rec declare_const t (const : Lambda.structured_constant) :
  ...
  | Const_base (Const_string (s, _, _)) ->
      let const, name =
        if Config.safe_string then
          (Flambda.Allocated_const (Immutable_string s), Names.const_immstring)
        else (Flambda.Allocated_const (String s), Names.const_string)
      in
  ...
  | Const_base (Const_int32 c) ->
      register_const t (Allocated_const (Int32 c)) Names.const_int32
  | Const_base (Const_int64 c) ->
      register_const t (Allocated_const (Int64 c)) Names.const_int64
...
let close_const t (const : Lambda.structured_constant) :
  ...
  | Const c, name -> (Const c, name)
  | Symbol s, name -> (Symbol s, name)

let lambda_const_bool b : Lambda.structured_constant =
  if b then Lambda.const_int 1 else Lambda.const_int 0
...
let rec close t env (lam : Lambda.lambda) : Flambda.t =
  ...
  | Lmutvar id -> (
      ...
      | mut_var -> name_expr (Read_mutable mut_var) ~name:Names.read_mutable
      | exception Not_found ->
          Misc.fatal_errorf
  ...
  | Lapply
      { ap_func; ap_args; ap_loc; ap_tailcall = _; ap_inlined; ap_specialised }
    ->
      Lift_code.lifting_helper (close_list t env ap_args)
        ~evaluation_order:`Right_to_left ~name:Names.apply_arg
  ...
  | Lletrec (defs, body) -> (
      ...
      let function_declarations =
        ...
        List.map
          (function
            | let_rec_ident, Lambda.Lfunction { kind; params; body; attr; loc }
              ->
  ...
  | Lsend (kind, meth, obj, args, loc) ->
      ...
      Flambda.create_let meth_var
        (Expr (close t env meth))
        (Flambda.create_let obj_var
           (Expr (close t env obj))
           (Lift_code.lifting_helper (close_list t env args)
              ~evaluation_order:`Right_to_left ~name:Names.send_arg
              ~create_body:(fun args ->
  ...
  | Lprim
    ...
    when not !Clflags.unsafe ->
      ...
      let exn_symbol = t.symbol_for_global' Predef.ident_division_by_zero in
      let dbg = Debuginfo.from_location loc in
      let zero_const : Flambda.named =
        match prim with
        | Pdivint _ | Pmodint _ -> Const (Int 0)
              ...
              (Flambda.create_let numerator (Expr arg1)
                 (Flambda.create_let is_zero
                    ...
                    (If_then_else
                       ( is_zero,
                         name_expr
                           (Prim (Praise Raise_regular, [ exn ], dbg))
                           ~name:Names.dummy,
  ...
  | Lprim ((Pbytes_to_string | Pbytes_of_string), [ arg ], _) -> close t env arg
  | Lprim (Pignore, [ arg ], _) ->
      let var = Variable.create Names.ignore in
      let defining_expr = close_let_bound_expression t var env arg in
      Flambda.create_let var defining_expr
        (name_expr (Const (Int 0)) ~name:Names.unit)
  | Lprim (Praise kind, [ arg ], loc) ->
      let arg_var = Variable.create Names.raise_arg in
      let dbg = Debuginfo.from_location loc in
      Flambda.create_let arg_var
        (Expr (close t env arg))
        (name_expr (Prim (Praise kind, [ arg_var ], dbg)) ~name:Names.raise)
  | Lprim (Pctconst c, [ arg ], _loc) ->
      ...
      let const =
        ...
        | Backend_type -> Lambda.const_int 0
        (* tag 0 is the same as Native *)
      in
      close t env
        (Lambda.Llet
           (Strict, Pgenval, Ident.create_local "dummy", arg, Lconst const))
  | Lprim (Pfield _, [ Lprim (Pgetglobal id, [], _) ], _)
    when Ident.same id t.current_unit_id ->
      Misc.fatal_errorf
        "[Pfield (Pgetglobal ...)] for the current compilation unit is \
  ...
  | Lprim (lambda_p, args, loc) ->
      ...
      Lift_code.lifting_helper (close_list t env args)
        ~evaluation_order:`Right_to_left ~name:(Names.of_primitive_arg lambda_p)
        ~create_body:(fun args ->
          name_expr (Prim (p, args, dbg)) ~name:(Names.of_primitive lambda_p))
  | Lswitch (arg, sw, _loc) ->
  ...
  | Lstaticraise (i, args) ->
      Lift_code.lifting_helper (close_list t env args)
        ~evaluation_order:`Right_to_left ~name:Names.staticraise_arg
        ~create_body:(fun args ->
  ...
  | Lfor (id, lo, hi, direction, body) ->
      ...
      Flambda.create_let from_value
        (Expr (close t env lo))
        (Flambda.create_let to_value
           (Expr (close t env hi))
  ...
  | Lassign (id, new_value) ->
      let being_assigned =
        ...
        | exception Not_found ->
            Misc.fatal_errorf
              "Closure_conversion.close: unbound mutable variable %s in \
               assignment"
              (Ident.unique_name id)
      in
      let new_value_var = Variable.create Names.new_value in
      Flambda.create_let new_value_var
        (Expr (close t env new_value))
...
and close_functions t external_env function_declarations : Flambda.named =
  let closure_env_without_parameters =
    Function_decls.closure_env_without_parameters external_env
  ...
  let set_of_closures =
    let free_vars =
      Ident.Set.fold
        (fun var map ->
          let internal_var = Env.find_var closure_env_without_parameters var in
          let external_var : Flambda.specialised_to =
            { var = Env.find_var external_env var; projection = None }
          in
...
let lambda_to_flambda ~backend ~module_ident ~size lam : Flambda.program =
  ...
  let fields =
    Array.init size (fun pos ->
        ...
        Flambda.create_let sym_v (Symbol block_symbol)
  ...
  let program_body =
    List.fold_left
      ...
      module_initializer t.declared_symbols
  in
  { imported_symbols = t.imported_symbols; program_body }
