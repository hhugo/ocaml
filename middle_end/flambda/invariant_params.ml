...
let _print ppf =
  ...
  | Implication args ->
    Format.fprintf ppf "Implication: @[<hv>%a@]" Variable.Pair.Set.print args
...
let implies relation from to_ =
  ...
  | Top -> relation
  | Implication set ->
    Variable.Pair.Map.add to_ (Implication (Variable.Pair.Set.add from set))
      relation
  | exception Not_found ->
    Variable.Pair.Map.add to_ (Implication (Variable.Pair.Set.singleton from))
      relation
...
let transitive_closure state =
  ...
  let update arg state =
    let original_set =
      try Variable.Pair.Map.find arg state
      with
    ...
    | Implication arguments ->
      let set =
        Variable.Pair.Set.fold
          (fun orig acc ->
             let set =
               try Variable.Pair.Map.find orig state
               with
               | Not_found -> Implication Variable.Pair.Set.empty
             in
             union set acc) arguments original_set
      in
  ...
  let rec fp state =
    ...
    if Variable.Pair.Map.equal equal state state' then state else fp state'
  in
  fp state
    ...
    (fun _ (function_decl : Flambda.function_declaration) ->
       Flambda_iterators.iter_all_toplevel_immutable_let_and_let_rec_bindings
         ~f:(fun var named ->
           ...
           | _ -> ()
         ) function_decl.body) function_decls.funs;
  !fun_var_bindings

let analyse_functions
    ~backend ~param_to_param ~anything_to_param ~param_to_anywhere
...
=
  ...
  let param_indexes_by_fun_vars =
    Variable.Map.map
      (fun (decl : Flambda.function_declaration) ->
         Array.of_list (Parameter.List.vars decl.params)) decls.funs
  in
  let find_callee_arg ~callee ~callee_pos =
    ...
    | arr ->
      (* Ignore overapplied parameters: they are applied to a different
         function. *)
      if callee_pos < Array.length arr then Some arr.(callee_pos) else None
  in
  ...
  let escaping_function fun_var =
    let fun_var =
      ...
      | fun_var -> fun_var
    in
    if Variable.Map.mem fun_var decls.funs then
      Variable.Tbl.add escaping_functions fun_var ()
  in
  ...
  let check_argument ~caller ~callee ~callee_pos ~caller_arg =
    ...
    | Some callee_arg ->
      begin match Variable.Map.find caller decls.funs with
  ...
  let check_expr ~caller (expr : Flambda.t) =
    ...
    | Apply { func; args } ->
      ...
      List.iteri
        (fun callee_pos caller_arg ->
           check_argument ~caller ~callee ~callee_pos ~caller_arg) args
    | _ -> ()
  in
  Variable.Map.iter
    (fun caller (decl : Flambda.function_declaration) ->
       Flambda_iterators.iter (check_expr ~caller)
         (fun (_ : Flambda.named) -> ()) decl.body;
       Variable.Set.iter
         (fun var ->
         ...
         (Flambda.free_variables ~ignore_uses_as_callee:()
            ~ignore_uses_as_argument:() decl.body)) decls.funs;
  Variable.Map.iter
            ...
            if Variable.Tbl.mem escaping_functions func_var then
              relation :=
                anything_to_param ~callee:func_var
                  ~callee_arg:(Parameter.var param) !relation) params)
    decls.funs;
...
*)
let invariant_params_in_recursion
    (decls : Flambda.function_declarations) ~backend
=
  ...
  let relation =
    analyse_functions ~backend ~param_to_param ~anything_to_param
      ~param_to_anywhere decls
  in
  let not_unchanging =
    Variable.Pair.Map.fold
      (fun (func, var) set not_unchanging ->
         match set with
         | Top -> Variable.Set.add var not_unchanging
         | Implication set ->
           if
             Variable.Pair.Set.exists
               (fun (func', var') ->
                  Variable.equal func func' && not (Variable.equal var var'))
               set
           then
             Variable.Set.add var not_unchanging
           else
             not_unchanging) relation Variable.Set.empty
  in
  let params =
    Variable.Map.fold
      (fun _ ({ params } : Flambda.function_declaration) set ->
         Variable.Set.union (Parameter.Set.vars params) set) decls.funs
      Variable.Set.empty
  in
  let unchanging = Variable.Set.diff params not_unchanging in
  let aliased_to =
    Variable.Pair.Map.fold
      (fun (_, var) set aliases ->
         match set with
         | Implication set when Variable.Set.mem var unchanging ->
           Variable.Pair.Set.fold
             (fun (_, caller_args) aliases ->
                if Variable.Set.mem caller_args unchanging then
                  let alias_set =
                    ...
                    | alias_set -> Variable.Set.add var alias_set
                  in
                  Variable.Map.add caller_args alias_set aliases
                else
                  aliases) set aliases
         | Top | Implication _ -> aliases) relation Variable.Map.empty
  in
...
let invariant_param_sources decls ~backend =
  ...
  in
  Variable.Pair.Map.fold
    (fun (_, var) set relation ->
       match set with
       | Top -> relation
       | Implication set -> Variable.Map.add var set relation) relation
    Variable.Map.empty
      ...
      (fun fun_var decl acc ->
         List.fold_left
           (fun acc param ->
              ...
              | Top -> acc) acc (Parameter.List.vars decl.Flambda.params))
