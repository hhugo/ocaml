      ...
      (fun subst param ->
        ...
        | (spec_to : Flambda.specialised_to) -> (
            ...
            | set ->
                ...
                Variable.Set.fold
                  (fun free_var subst -> Variable.Map.add free_var param subst)
                  set subst))
      Variable.Map.empty
      (Parameter.List.vars function_decl.params)
  in
  if Variable.Map.is_empty params_for_equal_free_vars then function_decl
  else
    let body =
  ...
  let funs =
    Variable.Map.map
      (fun function_decl ->
        ...
        in
        if not (new_function_decl == function_decl) then done_something := true;
        new_function_decl)
...
let run ~ppf_dump set_of_closures =
  Pass_wrapper.with_dump ~ppf_dump ~pass_name ~input:set_of_closures
    ...
    ~print_output:Flambda.print_set_of_closures ~f:(fun () ->
      rewrite_one_set_of_closures set_of_closures)
