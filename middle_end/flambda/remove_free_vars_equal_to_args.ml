...
let rewrite_one_function_decl
    ~(function_decl:Flambda.function_declaration) ~back_free_vars
    ~specialised_args
=
      ...
      (fun subst param ->
         ...
         | (spec_to : Flambda.specialised_to) ->
           ...
           | set ->
             ...
             Variable.Set.fold
               (fun free_var subst -> Variable.Map.add free_var param subst) set
               subst
           end) Variable.Map.empty (Parameter.List.vars function_decl.params)
  in
  if Variable.Map.is_empty params_for_equal_free_vars then
    function_decl
  else
    let body =
      Flambda_utils.toplevel_substitution params_for_equal_free_vars
        function_decl.body
    in
...
let rewrite_one_set_of_closures (set_of_closures : Flambda.set_of_closures) =
  let back_free_vars =
    Variable.Map.fold
      (fun var (outside_var : Flambda.specialised_to) map ->
         let set =
         ...
         in
         Variable.Map.add outside_var.var set map) set_of_closures.free_vars
      Variable.Map.empty
  in
  ...
  let funs =
    Variable.Map.map
      (fun function_decl ->
         ...
         in
         (if not (new_function_decl == function_decl) then
            done_something := true);
         new_function_decl) set_of_closures.function_decls.funs
  in
