...
and lift_lets_named _var (named : Flambda.named) ~toplevel : Flambda.named =
  ...
  | Symbol _ | Const _ | Allocated_const _ | Read_mutable _
  | Read_symbol_field (_, _)
  | Project_closure _ | Move_within_set_of_closures _ | Project_var _ | Prim _
  | Set_of_closures _ ->
      named
...
let rebuild_let_rec (defs : (Variable.t * Flambda.named) list) body =
  ...
  Array.fold_left
    (fun body (component : Sort_lets.component) ->
      ...
      | Has_loop l ->
          Flambda.Let_rec
            (List.map (fun v -> (v, Variable.Map.find v map)) l, body))
    body components

let lift_let_rec program =
  Flambda_iterators.map_exprs_at_toplevel_of_program program
    ~f:
      (Flambda_iterators.map_expr (fun expr ->
           match expr with
...
let lifting_helper exprs ~evaluation_order ~create_body ~name =
  ...
  in
  List.fold_left
    (fun body (v, expr) -> Flambda.create_let v (Expr expr) body)
