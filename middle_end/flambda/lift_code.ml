...
let rebuild_let (defs : def list) (body : Flambda.t) =
  ...
  List.fold_left
    (fun body def ->
       match def with
       | Immutable (var, def) -> W.create_let_reusing_defining_expr var def body
       | Mutable (var, initial_value, contents_kind) ->
         Flambda.Let_mutable { var; initial_value; contents_kind; body }) body
    defs
...
=
  ...
  let acc =
    ...
    | { var = v1; defining_expr = Expr (Let_mutable let_mut); _ } ->
      ...
      Immutable (v1, W.expr body2) :: acc
    | { var = v; _ } -> Immutable (v, W.of_defining_expr_of_let let_expr) :: acc
  in
...
and extract acc (expr : Flambda.t Flambda.With_free_variables.t) =
  ...
  | Let let_expr -> extract_let_expr acc let_expr
  | Let_mutable let_mutable -> extract_let_mutable acc let_mutable
  | _ -> acc, expr
...
let rec lift_lets_expr (expr : Flambda.t) ~toplevel : Flambda.t =
  ...
  | Let_mutable let_mut ->
    ...
    rebuild_let (List.rev rev_defs) body
  | e ->
    Flambda_iterators.map_subexpressions (lift_lets_expr ~toplevel)
      (lift_lets_named ~toplevel) e
...
and lift_lets_named _var (named : Flambda.named) ~toplevel : Flambda.named =
  match named with
  | Expr e -> Expr (lift_lets_expr e ~toplevel)
  | Set_of_closures set when not toplevel ->
    Set_of_closures
    ...
    Variable.Map.map
      (fun named ->
         Variable.Set.filter (fun v -> Variable.Map.mem v map)
           (Flambda.free_variables_named named)) map
  in
  let components =
    Sort_lets.connected_components_sorted_from_roots_to_leaf graph
  in
  Array.fold_left
    (fun body (component : Sort_lets.component) ->
       ...
       | Has_loop l ->
         Flambda.Let_rec
           (List.map (fun v -> v, Variable.Map.find v map) l, body)) body
    components

let lift_let_rec program =
  Flambda_iterators.map_exprs_at_toplevel_of_program program
    ~f:
    (Flambda_iterators.map_expr
       (fun expr ->
          match expr with
          | Let_rec (defs, body) -> rebuild_let_rec defs body
          | expr -> expr))
...
let lifting_helper exprs ~evaluation_order ~create_body ~name =
  ...
  in
  List.fold_left (fun body (v, expr) -> Flambda.create_let v (Expr expr) body)
