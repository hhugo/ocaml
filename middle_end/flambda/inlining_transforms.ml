...
let fold_over_projections_of_vars_bound_by_closure
    ~closure_id_being_applied ~lhs_of_application ~bound_variables ~init ~f
=
  Variable.Set.fold
    (fun var acc ->
       ...
       in
       f ~acc ~var ~expr) bound_variables init
...
let copy_of_function's_body_with_freshened_params
    env ~(function_decl:A.function_declaration) ~(function_body:A.function_body)
=
  ...
  if E.does_not_bind env param_vars && E.does_not_freshen env param_vars then
...
let inline_by_copying_function_body
    ~env ~r ~lhs_of_application ~(inline_requested:Lambda.inline_attribute)
    ~(specialise_requested:Lambda.specialise_attribute)
    ~closure_id_being_applied ~(function_decl:A.function_declaration)
    ~(function_body:A.function_body) ~fun_vars ~args ~dbg ~simplify
=
  assert (E.mem env lhs_of_application);
  assert (List.for_all (E.mem env) args);
  let r = if function_body.stub then r else R.map_benefit r B.remove_call in
  ...
  let body =
    ...
    in
    if function_body.stub && (not default_inline || not default_specialise) then
  ...
  let bindings_for_vars_bound_by_closure_and_params_to_args =
    let bound_variables =
      ...
      Variable.Set.diff (Variable.Set.diff function_body.free_variables params)
        fun_vars
    in
  ...
  *)
  let expr =
    Variable.Set.fold
      (fun another_closure_in_the_same_set expr ->
         let used =
...
let bind_free_vars
    ~lhs_of_application ~closure_id_being_applied ~state ~free_vars
=
  Variable.Map.fold
    (fun free_var (spec : Flambda.specialised_to) state ->
       ...
       in
       { state with  let_bindings; old_outside_to_new_outside }) free_vars state
...
let register_arguments
    ~specialised_args ~invariant_params ~state ~params ~args ~args_approxs
=
  let rec loop ~state ~params ~args ~args_approxs =
    match params, args, args_approxs with
    | [], [], [] -> state
    | param :: params, arg :: args, arg_approx :: args_approxs ->
      ...
      let old_params_to_new_outside =
        if worth_specialising then
          ...
          | Some set ->
            Variable.Set.fold (fun elem acc -> Variable.Map.add elem arg acc)
              set old_params_to_new_outside
          | None -> old_params_to_new_outside
        else
...
let add_param ~specialised_args ~state ~param =
  ...
  let new_specialised_args_with_old_projections =
    ...
    | None ->
      begin match
        Variable.Map.find_opt param state.old_params_to_new_outside
      with
...
let add_fun_var ~lhs_of_application ~closure_id_being_applied ~state ~fun_var =
  if Variable.Map.mem fun_var state.old_inside_to_new_inside then
    state
  else
    ...
    let spec : Flambda.specialised_to = { var = outside_var; projection = None }
    in
...
let add_free_var ~free_vars ~state ~free_var =
  if Variable.Map.mem free_var state.old_inside_to_new_inside then
    state
  else
    ...
    in
    let new_spec : Flambda.specialised_to = { spec with  var = new_outside_var }
    in
...
let add_function ~specialised_args ~state ~fun_var ~function_decl =
  ...
  | None -> None
  | Some _ ->
    let rec loop worth_specialising =
      function
      | [] -> worth_specialising
      | param :: params ->
        let param = Parameter.var param in
        begin match Variable.Map.find_opt param specialised_args with
        | Some (spec : Flambda.specialised_to) ->
    ...
    if not worth_specialising then
      None
    else
...
let specialisable_call ~specialised_args ~state ~args ~params =
  List.for_all2
    (fun arg param ->
       let param = Parameter.var param in
       if Variable.Map.mem param specialised_args then
         true
       else
         ...
         | None -> true
         | Some outside_var ->
           begin match Variable.Map.find_opt arg old_params_to_new_outside with
           | Some outside_var' -> Variable.equal outside_var outside_var'
           | None -> false
           end) args params
...
let rec rewrite_direct_call
    ~specialised_args ~funs ~direct_call_surrogates ~state ~closure_id
    ~(apply:Flambda.apply)
=
  match Closure_id.Map.find_opt closure_id direct_call_surrogates with
  | Some closure_id ->
    rewrite_direct_call ~specialised_args ~funs ~direct_call_surrogates ~state
      ~closure_id ~apply
  | None ->
    let fun_var = Closure_id.unwrap closure_id in
    begin match Variable.Map.find_opt fun_var funs with
    | None -> None
    | Some function_decl ->
      begin match
...
let rewrite_function
    ~lhs_of_application ~closure_id_being_applied ~direct_call_surrogates
    ~specialised_args ~free_vars ~funs ~state fun_var
=
  let function_decl : A.function_declaration = Variable.Map.find fun_var funs in
  let function_body =
    match function_decl.function_body with
    | None -> assert false
    | Some function_body -> function_body
  in
  let new_fun_var = Variable.Map.find fun_var state.old_fun_var_to_new_fun_var
  in
  let (state, params) =
    List.fold_right
      (fun param (state, params) ->
         let (state, param) = add_param ~specialised_args ~state ~param in
         state, param :: params) function_decl.params (state, [])
  in
  let state =
    Variable.Set.fold
      (fun var state ->
         if Variable.Map.mem var funs then
           add_fun_var ~lhs_of_application ~closure_id_being_applied ~state
             ~fun_var:var
         else if Variable.Map.mem var free_vars then
           add_free_var ~free_vars ~state ~free_var:var
         else
           state) function_body.free_variables state
  in
  ...
  let body =
    Flambda_iterators.map_toplevel_expr
      (fun (expr : Flambda.t) ->
         match expr with
         | Apply ({ kind = Direct closure_id } as apply) ->
           begin match
         ...
         | _ -> expr) function_body.body
  in
  let body =
    Flambda_utils.toplevel_substitution state.old_inside_to_new_inside body
  in
  let new_function_decl =
    Flambda.create_function_declaration ~params ~body ~stub:function_body.stub
      ~dbg:function_body.dbg ~inline:function_body.inline
  ...
  in
  let new_funs = Variable.Map.add new_fun_var new_function_decl state.new_funs
  in
  ...
  Variable.Map.map
    (fun (spec_to : Flambda.specialised_to) ->
       let projection : Projection.t option =
         ...
         | None -> None
         | Some (Project_var proj) ->
           begin match Variable.Map.find_opt proj.closure old_to_new with
           | None -> None
           | Some closure ->
             let proj = { proj with  closure } in
             Some (Projection.Project_var proj)
           end
         | Some (Project_closure proj) ->
           begin match
             Variable.Map.find_opt proj.set_of_closures old_to_new
           with
           | None -> None
         ...
         | Some (Move_within_set_of_closures proj) ->
           begin match Variable.Map.find_opt proj.closure old_to_new with
           | None -> None
           | Some closure ->
             let proj = { proj with  closure } in
         ...
         | Some (Field (index, var)) ->
           begin match Variable.Map.find_opt var old_to_new with
           | None -> None
           | Some var -> Some (Projection.Field (index, var))
           end
       in
       { spec_to with  projection }) projections

let inline_by_copying_function_declaration
    ~(env:Inline_and_simplify_aux.Env.t) ~(r:Inline_and_simplify_aux.Result.t)
    ~(function_decls:A.function_declarations) ~(lhs_of_application:Variable.t)
    ~(inline_requested:Lambda.inline_attribute)
    ~(closure_id_being_applied:Closure_id.t)
    ~(function_decl:A.function_declaration) ~(args:Variable.t list)
    ~(args_approxs:A.t list)
    ~(invariant_params:Variable.Set.t Variable.Map.t lazy_t)
    ~(specialised_args:Flambda.specialised_to Variable.Map.t)
    ~(free_vars:Flambda.specialised_to Variable.Map.t)
    ~(direct_call_surrogates:Closure_id.t Closure_id.Map.t) ~(dbg:Debuginfo.t)
    ~(simplify:Inlining_decision_intf.simplify)
=
  let state = empty_state in
  let state =
    bind_free_vars ~lhs_of_application ~closure_id_being_applied ~state
      ~free_vars
  in
  let params = function_decl.params in
  let state =
    register_arguments ~specialised_args ~invariant_params ~state ~params ~args
      ~args_approxs
  in
  ...
  | Some (state, new_fun_var) ->
    ...
    let function_decls =
      Flambda.create_function_declarations_with_origin ~funs:state.new_funs
        ~set_of_closures_origin:function_decls.set_of_closures_origin
        ~is_classic_mode:function_decls.is_classic_mode
    in
    let free_vars =
      update_projections ~state state.new_free_vars_with_old_projections
    in
    let specialised_args =
      update_projections ~state state.new_specialised_args_with_old_projections
    in
    ...
    let body =
      Flambda.create_let set_of_closures_var (Set_of_closures set_of_closures)
        (Flambda.create_let closure_var (Project_closure project) (Apply apply))
    in
