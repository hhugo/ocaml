...
let set_inline_attribute_on_all_apply body inline specialise =
  Flambda_iterators.map_toplevel_expr
    (function
      | Apply apply -> Apply { apply with inline; specialise } | expr -> expr)
    body
...
let inline_by_copying_function_body ~env ~r ~lhs_of_application
  ...
  let body =
    ...
    in
    if function_body.stub && ((not default_inline) || not default_specialise)
    then
  ...
  *)
  let expr =
    Variable.Set.fold
      ...
      fun_vars bindings_for_vars_bound_by_closure_and_params_to_args
  in
...
let bind_free_vars ~lhs_of_application ~closure_id_being_applied ~state
    ~free_vars =
  Variable.Map.fold
...
(* For arguments of specialised parameters:
   ...
   - Add them and their invariant aliases to [old_params_to_new_outside] *)
let register_arguments ~specialised_args ~invariant_params ~state ~params ~args
    ~args_approxs =
  let rec loop ~state ~params ~args ~args_approxs =
    ...
    | param :: params, arg :: args, arg_approx :: args_approxs ->
        ...
        let old_params_to_new_outside =
          ...
          else state.old_params_to_new_outside
        in
        let state =
...
let rewrite_function ~lhs_of_application ~closure_id_being_applied
    ~direct_call_surrogates ~specialised_args ~free_vars ~funs ~state fun_var =
...
let inline_by_copying_function_declaration
    ...
    ~(dbg : Debuginfo.t) ~(simplify : Inlining_decision_intf.simplify) =
