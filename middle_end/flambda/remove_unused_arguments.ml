...
let remove_params unused (fun_decl : Flambda.function_declaration) ~new_fun_var
    =
  let unused_params, used_params =
    List.partition
      (fun v -> Variable.Set.mem (Parameter.var v) unused)
      fun_decl.params
  in
  let unused_params =
    List.filter
...
let separate_unused_arguments ~only_specialised ~backend
  ...
  let non_stub_arguments =
    Variable.Map.fold
      (fun _ (decl : Flambda.function_declaration) acc ->
        if decl.stub then acc
        else Variable.Set.union acc (Parameter.Set.vars decl.Flambda.params))
  ...
  in
  if Variable.Set.is_empty unused then None
  else
    let funs, additional_specialised_args =
      Variable.Map.fold
        (fun fun_id (fun_decl : Flambda.function_declaration)
...
let should_split_only_specialised_args
  ...
  if not !Clflags.remove_unused_arguments then true
  else
