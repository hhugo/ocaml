...
let remove_params
    unused (fun_decl : Flambda.function_declaration) ~new_fun_var
=
  ...
  let unused_params =
    List.filter
      (fun v -> Variable.Set.mem (Parameter.var v) fun_decl.free_variables)
      unused_params
  in
  let body =
    List.fold_left
...
let make_stub
    unused var (fun_decl : Flambda.function_declaration) ~specialised_args
...
=
  ...
  let used_args' =
    List.filter
      (fun (param, _) -> not (Variable.Set.mem (Parameter.var param) unused))
      args'
  in
  let args'_var =
  ...
  let additional_specialised_args =
    List.fold_left
      (fun additional_specialised_args (original_arg, arg) ->
         ...
         | (outer_var : Flambda.specialised_to) ->
           ...
           let outer_var : Flambda.specialised_to =
             ...
             | Some projection ->
               ...
               in
               { outer_var with  projection = Some projection }
           in
...
let separate_unused_arguments
    ~only_specialised ~backend ~(set_of_closures:Flambda.set_of_closures)
=
  ...
  let non_stub_arguments =
    Variable.Map.fold
      (fun _ (decl : Flambda.function_declaration) acc ->
         if decl.stub then
           acc
         else
  ...
  let unused =
    if only_specialised then
      Variable.Set.inter specialised_args unused
    else
      unused
  in
  if Variable.Set.is_empty unused then
    ...
    let (funs, additional_specialised_args) =
      Variable.Map.fold
        (fun
           fun_id (fun_decl : Flambda.function_declaration)
           (funs, additional_specialised_args)
         ->
           if
             List.exists (fun v -> Variable.Set.mem (Parameter.var v) unused)
               fun_decl.params
           then
...
let should_split_only_specialised_args
    (fun_decls : Flambda.function_declarations) ~backend
=
  if not !Clflags.remove_unused_arguments then
    true
  else
    ...
    let number_of_non_stub_functions =
      Variable.Map.cardinal
        (Variable.Map.filter (fun _ { Flambda.stub } -> not stub) fun_decls.funs)
    in
...
let separate_unused_arguments_in_set_of_closures set_of_closures ~backend =
  ...
  let only_specialised =
    should_split_only_specialised_args set_of_closures.Flambda.function_decls
      ~backend
  in
  match
    separate_unused_arguments ~only_specialised ~backend ~set_of_closures
  with
  | None ->
    (if dump then
  ...
  | Some result ->
    (if dump then
       Format.eprintf
         "Before Remove_unused_arguments:@ %a@.@.After \
          Remove_unused_arguments:@ %a@.@." Flambda.print_set_of_closures
         set_of_closures Flambda.print_set_of_closures result);
    Some result
...
let separate_unused_arguments_in_closures program ~backend =
  Flambda_iterators.map_exprs_at_toplevel_of_program program
    ~f:(fun expr -> separate_unused_arguments_in_closures_expr expr ~backend
    )
