...
let rec add_sb_mutable_var sb id id' =
  ...
  let sb =
    try
      ...
      List.fold_left
        (fun sb pre_id -> add_sb_mutable_var sb pre_id id')
        sb pre_vars
    with Not_found -> sb
  in
...
let apply_variable t var =
  match t with
  | Inactive -> var
  | Active t -> ( try Variable.Map.find var t.sb_var with Not_found -> var)
...
let apply_mutable_variable t mut_var =
  ...
  | Active t -> (
      ...
      with Not_found -> mut_var)

let rewrite_recursive_calls_with_symbols t
    (function_declarations : Flambda.function_declarations) ~make_closure_symbol
    =
  match t with
...
module Project_var = struct
  ...
  let new_subst_fv t id subst =
    ...
    | Active subst ->
        ...
        let off' = Var_within_closure.wrap id' in
        let off_sb =
          Var_within_closure.Map.add off off' t.vars_within_closure
        in
  ...
  let subst_free_vars fv subst ~only_freshen_parameters :
    ...
    Variable.Map.fold
      (fun id lam (fv, subst, t) ->
        let id, subst, t =
          if only_freshen_parameters then (id, subst, t)
          else new_subst_fv t id subst
        in
  ...
  let func_decls_subst t (subst : subst)
      (func_decls : Flambda.function_declarations) ~only_freshen_parameters =
    match subst with
            ...
            if T.Map.mem src_var later then
              Misc.fatal_errorf
                "Freshening.Project_var.compose: domains of substitutions must \
                 be disjoint.  earlier=%a later=%a"
                (T.Map.print T.print) earlier (T.Map.print T.print) later;
...
let apply_function_decls_and_free_vars t fv func_decls ~only_freshen_parameters
    =
