...
let print ppf =
  ...
  | Active tbl ->
    ...
    Variable.Map.iter
      (fun var1 var2 ->
         Format.fprintf ppf "%a -> %a@ " Variable.print var1 Variable.print var2)
      tbl.sb_var;
    Mutable_variable.Map.iter
      (fun mut_var1 mut_var2 ->
         Format.fprintf ppf "(mutable) %a -> %a@ " Mutable_variable.print
           mut_var1 Mutable_variable.print mut_var2) tbl.sb_mutable_var;
    Variable.Map.iter
      (fun var vars ->
         Format.fprintf ppf "%a -> %a@ " Variable.print var Variable.Set.print
           (Variable.Set.of_list vars)) tbl.back_var;
    Mutable_variable.Map.iter
      (fun mut_var mut_vars ->
         Format.fprintf ppf "(mutable) %a -> %a@ " Mutable_variable.print
           mut_var Mutable_variable.Set.print
           (Mutable_variable.Set.of_list mut_vars)) tbl.back_mutable_var
...
let empty_preserving_activation_state =
  function
  | Inactive -> Inactive
...
let rec add_sb_var sb id id' =
  ...
  let sb =
    try
      let pre_vars = Variable.Map.find id sb.back_var in
  ...
  let back_var =
    let l =
      try Variable.Map.find id' sb.back_var
      with
      | Not_found -> []
    in
    Variable.Map.add id' (id :: l) sb.back_var
  in
  { sb with  back_var }
...
let rec add_sb_mutable_var sb id id' =
  ...
  let sb =
    try
      ...
      List.fold_left (fun sb pre_id -> add_sb_mutable_var sb pre_id id') sb
        pre_vars
    with
    | Not_found -> sb
  in
...
let active_find_var_exn t id =
  try Variable.Map.find id t.sb_var
  with
  | Not_found ->
    Misc.fatal_error
      (Format.asprintf "find_var: can't find %a@." Variable.print id)

let apply_variable t var =
  match t with
  | Inactive -> var
  | Active t ->
    (try Variable.Map.find var t.sb_var
     with
     | Not_found -> var)
...
let apply_mutable_variable t mut_var =
  ...
  | Inactive -> mut_var
  | Active t ->
    (try Mutable_variable.Map.find mut_var t.sb_mutable_var
     with
     | Not_found -> mut_var)

let rewrite_recursive_calls_with_symbols
    ...
    let closure_symbols =
      Variable.Map.fold
        (fun var _ map ->
           ...
           if Symbol.Set.mem sym all_free_symbols then
             (closure_symbols_used := true;
              Symbol.Map.add sym var map)
           else
             map) function_declarations.funs Symbol.Map.empty
    in
    if not !closure_symbols_used then
...
module Project_var = struct
  ...
  let print ppf t =
    Format.fprintf ppf "{ vars_within_closure %a, closure_id %a }"
      ...
      t.vars_within_closure (Closure_id.Map.print Closure_id.print) t.closure_id
  ...
  =
    Variable.Map.fold
      (fun id lam (fv, subst, t) ->
         ...
         in
         Variable.Map.add id lam fv, subst, t) fv
   ...
   *)
  let func_decls_subst
      t (subst : subst) (func_decls : Flambda.function_declarations)
  ...
  =
    ...
    | Active subst ->
      let subst_func_decl
          _fun_id (func_decl : Flambda.function_declaration) subst
      =
        ...
        in
        let function_decl =
          Flambda.create_function_declaration ~params ~body ~stub:func_decl.stub
            ~dbg:func_decl.dbg ~inline:func_decl.inline
      ...
      let (subst, t) =
        if only_freshen_parameters then
          ...
          Variable.Map.fold
            (fun orig_id _func_decl (subst, t) ->
               let (_id, subst, t) = new_subst_fun t orig_id subst in
               subst, t) func_decls.funs (subst, t)
      in
      let (funs, subst) =
        Variable.Map.fold
          (fun orig_id func_decl (funs, subst) ->
             let (func_decl, subst) = subst_func_decl orig_id func_decl subst in
             let id =
               if only_freshen_parameters then
                 orig_id
               else
                 active_find_var_exn subst orig_id
             in
             let funs = Variable.Map.add id func_decl funs in
             funs, subst) func_decls.funs (Variable.Map.empty, subst)
      in
      let function_decls = Flambda.update_function_declarations func_decls ~funs
      in
  ...
  let apply_closure_id t closure_id =
    ...
    with
    | Not_found -> closure_id
  
  let apply_var_within_closure t var_in_closure =
    try Var_within_closure.Map.find var_in_closure t.vars_within_closure
    with
    | Not_found -> var_in_closure
  
  module Compose (T : Identifiable.S) = struct
    let compose ~earlier ~later =
      if (T.Map.equal T.equal) earlier later || T.Map.cardinal later = 0 then
        earlier
      else
        T.Map.mapi
          (fun src_var var ->
             (if T.Map.mem src_var later then
                Misc.fatal_errorf
                  "Freshening.Project_var.compose: domains of substitutions must \
                   be disjoint.  earlier=%a later=%a" (T.Map.print T.print)
                  earlier (T.Map.print T.print) later);
  ...
  let compose ~earlier ~later : t =
    {
      ...
      closure_id = C.compose ~earlier:earlier.closure_id ~later:later.closure_id
    }
end
  

let apply_function_decls_and_free_vars
    t fv func_decls ~only_freshen_parameters
=
  let module I = Project_var 
  in
...
let freshen_projection
    (projection : Projection.t) ~freshening ~closure_freshening
  :
  Projection.t
=
...
let freshen_projection_relation relation ~freshening ~closure_freshening =
  Variable.Map.map
    (fun (spec_to : Flambda.specialised_to) ->
       ...
       in
       { spec_to with  projection }) relation

let freshen_projection_relation' relation ~freshening ~closure_freshening =
  Variable.Map.map
    (fun ((spec_to : Flambda.specialised_to), data) ->
       let projection =
         match spec_to.projection with
         | None -> None
         | Some projection ->
           Some (freshen_projection projection ~freshening ~closure_freshening)
       in
       { spec_to with  projection }, data) relation
