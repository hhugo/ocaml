...
module Env = struct
  ...
  let create ~never_inline ~backend ~round ~ppf_dump =
    {
      ...
      closure_depth = 0;
      inlining_stats_closure_stack = Inlining_stats.Closure_stack.create ();
  ...
  let print ppf t =
    Format.fprintf ppf "Environment maps: %a@.Projections: %a@.Freshening: %a@."
      Variable.Set.print (Variable.Map.keys t.approx)
      (Projection.Map.print Variable.print) t.projections Freshening.print
      t.freshening
  ...
  let add_mutable t mut_var approx =
    { t with
      ...
      approx_mutable = Mutable_variable.Map.add mut_var approx t.approx_mutable
    }
  
  let really_import_approx t =
    let module Backend = (val (t.backend : (module Backend_intf.S))) 
    in
  ...
  let find_symbol_exn t symbol =
    really_import_approx t (Symbol.Map.find symbol t.approx_sym)
  
  let find_symbol_opt t symbol =
    try Some (really_import_approx t (Symbol.Map.find symbol t.approx_sym))
    with
    | Not_found -> None
  ...
  let find_symbol_fatal t symbol =
    ...
    | exception Not_found ->
      Misc.fatal_errorf
        "Symbol %a is unbound.  Maybe there is a missing [Let_symbol], \
         [Import_symbol] or similar?" Symbol.print symbol
    | approx -> approx
  ...
  let find_or_load_symbol t symbol =
    ...
    | exception Not_found ->
      (if
       ...
       then
         Misc.fatal_errorf
           "Symbol %a from the current compilation unit is unbound.  Maybe \
            there is a missing [Let_symbol] or similar?" Symbol.print symbol);
      let module Backend = (val (t.backend : (module Backend_intf.S))) 
      in
  ...
  let add_projection t ~projection ~bound_to =
    { t with
      ...
      projections = Projection.Map.add projection bound_to t.projections
    }
  ...
  let does_not_bind t vars = not (List.exists (mem t) vars)
  let does_not_freshen t vars = Freshening.does_not_freshen t.freshening vars
  
  let add_symbol t symbol approx =
    match find_symbol_exn t symbol with
    | exception Not_found ->
      { t with  approx_sym = Symbol.Map.add symbol approx t.approx_sym }
    | _ ->
      Misc.fatal_errorf
        "Attempt to redefine symbol %a (to %a) in environment for \
         [Inline_and_simplify]" Symbol.print symbol Simple_value_approx.print
        approx
  
  let redefine_symbol t symbol approx =
    match find_symbol_exn t symbol with
    | exception Not_found -> assert false
    | _ -> { t with  approx_sym = Symbol.Map.add symbol approx t.approx_sym }
  
  let find_with_scope_exn t id =
    try really_import_approx_with_scope t (Variable.Map.find id t.approx)
    with
    | Not_found ->
      Misc.fatal_errorf
  ...
  let find_exn t id = snd (find_with_scope_exn t id)
  ...
  let find_list_exn t vars = List.map (fun var -> find_exn t var) vars
  
  let find_opt t id =
    try Some (really_import_approx t (snd (Variable.Map.find id t.approx)))
    with
    | Not_found -> None
  ...
  let at_toplevel t = t.closure_depth = 0
  let is_inside_branch env = env.inside_branch > 0
  let branch_depth env = env.inside_branch
  let inside_branch t = { t with  inside_branch = t.inside_branch + 1 }
  let set_freshening t freshening = { t with  freshening }
  
  let increase_closure_depth t =
    let approx =
      Variable.Map.map (fun (_scope, approx) -> Outer, approx) t.approx
    in
    { t with  approx; closure_depth = t.closure_depth + 1 }
  
  let set_never_inline t =
    if t.never_inline then t else { t with  never_inline = true }
  
  let set_never_inline_inside_closures t =
    if t.never_inline_inside_closures then
      t
    else
      { t with  never_inline_inside_closures = true }
  ...
  let set_never_inline_outside_closures t =
    if t.never_inline_outside_closures then
      ...
      { t with  never_inline_outside_closures = true }
  ...
  let continue_actively_unrolling t origin =
    let unrolling =
      try Set_of_closures_origin.Map.find origin t.actively_unrolling
      with
      | Not_found -> Misc.fatal_error "Unexpected actively unrolled function"
    in
    let actively_unrolling =
  ...
  let unrolling_allowed t origin =
    let unroll_count =
      try Set_of_closures_origin.Map.find origin t.unroll_counts
      with
      | Not_found ->
  ...
  let inlining_allowed t id =
    let inlining_count =
      try Closure_origin.Map.find id t.inlining_counts
      with
      | Not_found ->
        Int.max 1
          (Clflags.Int_arg_helper.get ~key:t.round !Clflags.inline_max_unroll)
  ...
  let enter_closure t ~closure_id ~inline_inside ~dbg ~f =
    let t =
      if inline_inside && not t.never_inline_inside_closures then
        t
      else
        set_never_inline t
    in
  ...
  let set_inline_debuginfo t ~dbg = { t with  inlined_debuginfo = dbg }
...
let initial_inlining_toplevel_threshold ~round : Inlining_cost.Threshold.t =
  ...
  in
  let unscaled = int_of_float ordinary_threshold + toplevel_threshold in
  (* CR-soon pchambart: Add a warning if this is too big
     mshinwell: later *)
  Can_inline_if_no_larger_than
...
module Result = struct
  ...
  let meet_approx t env approx =
    ...
    let meet = Simple_value_approx.meet ~really_import_approx t.approx approx in
    set_approx t meet
  ...
  let map_benefit t f = { t with  benefit = f t.benefit }
  
  let add_benefit t b =
    { t with  benefit = Inlining_cost.Benefit.(+) t.benefit b }
  
  let benefit t = t.benefit
  let reset_benefit t = { t with  benefit = Inlining_cost.Benefit.zero }
  ...
  let num_direct_applications t = t.num_direct_applications
end
  ...
  if not is_classic_mode then
    ...
    (fun (var : Variable.t) (fun_decl : Flambda.function_declaration) ->
       if fun_decl.stub then
         true
       else if Variable.Set.mem var (Lazy.force recursive) then
         false
       else
...
let prepare_to_simplify_set_of_closures
    ~env ~(set_of_closures:Flambda.set_of_closures) ~function_decls ~freshen
...
=
      ...
      Variable.Map.filter_map
        (fun param (spec_to : Flambda.specialised_to) ->
           ...
           if not keep then
             ...
             let var = Freshening.apply_variable (E.freshening env) external_var
             in
             let var =
               match
                 A.simplify_var_to_var_using_env (E.find_exn env var)
  ...
  let free_vars =
    Freshening.freshen_projection_relation' free_vars
      ~freshening:(E.freshening env) ~closure_freshening:freshening
  in
  let specialised_args =
    let specialised_args =
  ...
  let parameter_approximations =
    ...
    Variable.Map.map_keys (Freshening.apply_variable (E.freshening env))
      (Variable.Map.mapi
         (fun _id' (spec_to : Flambda.specialised_to) ->
            E.find_exn environment_before_cleaning spec_to.var) specialised_args)
  in
  let direct_call_surrogates =
    Variable.Map.fold
      ...
      set_of_closures.direct_call_surrogates Closure_id.Map.empty
  in
  let env =
  ...
  let internal_value_set_of_closures =
    ...
    in
    A.create_value_set_of_closures ~function_decls ~bound_vars ~free_vars
  ...
  let set_of_closures_env =
    Variable.Map.fold
      (fun closure _ env ->
         ...
         in
         E.add env closure approx) function_decls.funs env
  in
  free_vars,
  specialised_args,
  function_decls,
...
let populate_closure_approximations
    ...
    ~(parameter_approximations:A.t Variable.Map.t) ~set_of_closures_env
=
  (* Add approximations of free variables *)
  let env =
    Variable.Map.fold (fun id (_, desc) env -> E.add_outer_scope env id desc)
    ...
    List.fold_left
      (fun env id ->
         let approx =
           ...
           with
           | Not_found -> A.value_unknown Other
         in
         E.add env id approx) env (Parameter.List.vars function_decl.params)
  in
  env

let prepare_to_simplify_closure
    ~(function_decl:Flambda.function_declaration) ~free_vars ~specialised_args
      ...
      (fun inner_var spec_arg env ->
         ...
         | Some projection ->
           ...
           if Variable.Set.mem from function_decl.free_variables then
             ...
             env) which_variables closure_env
  in
  let closure_env =
