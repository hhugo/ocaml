...
module Env = struct
  ...
  let print ppf t =
    Format.fprintf ppf "Environment maps: %a@.Projections: %a@.Freshening: %a@."
      Variable.Set.print
      (Variable.Map.keys t.approx)
      (Projection.Map.print Variable.print)
      t.projections Freshening.print t.freshening
        ...
        then
          Misc.fatal_errorf
            "Symbol %a from the current compilation unit is unbound.  Maybe \
             there is a missing [Let_symbol] or similar?"
            Symbol.print symbol;
        let module Backend = (val t.backend : Backend_intf.S) in
...
let keep_body_check ~is_classic_mode ~recursive =
  if not is_classic_mode then fun _ _ -> true
  else
    ...
    fun (var : Variable.t) (fun_decl : Flambda.function_declaration) ->
      if fun_decl.stub then true
      else if Variable.Set.mem var (Lazy.force recursive) then false
      else
...
let prepare_to_simplify_set_of_closures ~env
    ~(set_of_closures : Flambda.set_of_closures) ~function_decls ~freshen
  ...
  let specialised_args =
    ...
    |> Variable.Map.filter_map (fun param (spec_to : Flambda.specialised_to) ->
           let keep =
             match only_for_function_decl with
             | None -> true
             | Some function_decl ->
                 Variable.Set.mem param
...
let populate_closure_approximations
  ...
  let env =
    Variable.Map.fold
      (fun id (_, desc) env -> E.add_outer_scope env id desc)
...
let prepare_to_simplify_closure ~(function_decl : Flambda.function_declaration)
    ~free_vars ~specialised_args ~parameter_approximations ~set_of_closures_env
    =
  let closure_env =
