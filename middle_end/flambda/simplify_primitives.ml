...
let is_known_to_be_some_kind_of_int (arg : A.descr) =
  ...
  | Value_block (_, _)
  | Value_float _ | Value_set_of_closures _ | Value_closure _ | Value_string _
  | Value_float_array _ | A.Value_boxed_int _ | Value_unknown _ | Value_extern _
...
let is_known_to_be_some_kind_of_block (arg : A.descr) =
  ...
  | Value_closure _ | Value_string _ ->
      true
  | Value_set_of_closures _ | Value_int _ | Value_char _ | Value_unknown _
  | Value_extern _ | Value_symbol _ | Value_unresolved _ | Value_bottom ->
      false

let rec structurally_different (arg1 : A.t) (arg2 : A.t) =
  match (arg1.descr, arg2.descr) with
  | Value_int n1, Value_int n2 when n1 <> n2 -> true
...
let phys_different (approxs : A.t list) =
  ...
  | [] | [ _ ] | _ :: _ :: _ :: _ ->
      Misc.fatal_error "wrong number of arguments for equality"
  | [ a1; a2 ] -> structurally_different a1 a2

let is_empty = function [] -> true | _ :: _ -> false

let is_pisint = function Clambda_primitives.Pisint -> true | _ -> false
...
let primitive (p : Clambda_primitives.primitive) (args, approxs) expr dbg
    ~size_int : Flambda.named * A.t * Inlining_cost.Benefit.t =
  ...
  | Pmakeblock (tag_int, Asttypes.Immutable, shape) ->
      ...
      ( Prim (Pmakeblock (tag_int, Asttypes.Immutable, Some shape), args, dbg),
        ...
        C.Benefit.zero )
  | Praise _ -> (expr, A.value_bottom, C.Benefit.zero)
  | Pmakearray (_, _) when is_empty approxs ->
      ( Prim (Pmakeblock (0, Asttypes.Immutable, Some []), [], dbg),
        A.value_block (Tag.create_exn 0) [||],
        C.Benefit.zero )
  | Pmakearray (Pfloatarray, Mutable) ->
      let approx = A.value_mutable_float_array ~size:(List.length args) in
      (expr, approx, C.Benefit.zero)
  | Pmakearray (Pfloatarray, Immutable) ->
      let approx = A.value_immutable_float_array (Array.of_list approxs) in
      (expr, approx, C.Benefit.zero)
  | Pintcomp Ceq when phys_equal approxs -> S.const_bool_expr expr true
  | Pintcomp Cne when phys_equal approxs ->
      ...
      (* N.B. Having [not (phys_equal approxs)] would not on its own tell us
         ...
         invalid. *)
  | Pintcomp Ceq when phys_different approxs -> S.const_bool_expr expr false
  ...
  | _ -> (
      ...
      | [ Value_int x ] -> (
          ...
          | Pbintofint Pint32 ->
              S.const_boxed_int_expr expr Int32 (Int32.of_int x)
          | Pbintofint Pint64 ->
              S.const_boxed_int_expr expr Int64 (Int64.of_int x)
      ...
      | [
       A.Value_boxed_int (A.Nativeint, n1); A.Value_boxed_int (A.Nativeint, n2);
      ] ->
      ...
      | [ A.Value_boxed_int (A.Int32, n1); Value_int n2 ] ->
          I.Simplify_boxed_int32.simplify_binop_int p Int32 expr n1 n2 ~size_int
      | [ A.Value_boxed_int (A.Int64, n1); Value_int n2 ] ->
          I.Simplify_boxed_int64.simplify_binop_int p Int64 expr n1 n2 ~size_int
      | [ Value_block _ ] when is_pisint p -> S.const_bool_expr expr false
      | [ Value_string { size } ] when is_pstring_length p || is_pbytes_length p
        ->
          S.const_int_expr expr size
      | [ Value_string { size; contents = Some s }; Value_int x ]
        when x >= 0 && x < size -> (
          match p with
          | Pstringrefu | Pstringrefs | Pbytesrefu | Pbytesrefs ->
              S.const_char_expr (Prim (Pstringrefu, args, dbg)) s.[x]
          | _ -> (expr, A.value_unknown Other, C.Benefit.zero))
      | [ Value_string { size; contents = None }; Value_int x ]
      ...
      | [ Value_float_array { size; contents } ] -> (
          ...
          | Pfloatfield i -> (
              ...
              | A.Contents a when i >= 0 && i < size -> (
                  match A.check_approx_for_float a.(i) with
      ...
      | _ -> (
          ...
          | Float -> (expr, A.value_any_float, C.Benefit.zero)
          | Other -> (expr, A.value_unknown Other, C.Benefit.zero)))
