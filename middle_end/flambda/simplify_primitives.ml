...
let phys_equal (approxs : A.t list) =
  ...
  | [ a1; a2 ] ->
    (* N.B. The following would be incorrect if the variables are not
         ...
         | _ -> ...
      *)
    begin match a1.symbol, a2.symbol with
...
let is_pstring_length =
  function
  | Clambda_primitives.Pstringlength -> true
  | _ -> false

let is_pbytes_length =
  function
...
let primitive
    (p : Clambda_primitives.primitive) (args, approxs) expr dbg ~size_int
  :
...
=
  ...
  (* If two values are structurally different we are certain they can never
     be shared*)
  | _ ->
    begin match A.descrs approxs with
    | [ Value_int x ] ->
      begin match p with
    ...
    | _ ->
      begin match Semantics_of_primitives.return_type_of_primitive p with
