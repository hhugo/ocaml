...
and value_float_array = { contents : value_float_array_contents; size : int }
...
let print_value_set_of_closures ppf
  ...
  Format.fprintf ppf
    ...
    (fun ppf -> Variable.Map.iter (fun id _ -> Variable.print ppf id))
    funs
    (Variable.Map.print Variable.Set.print)
    (Lazy.force invariant_params)
...
let print_unresolved_value ppf = function
  ...
  | Symbol symbol -> Format.fprintf ppf "Symbol %a" Symbol.print symbol
...
let print_function_declaration ppf var (f : function_declaration) =
  ...
  | None ->
      Format.fprintf ppf "@[<2>(%a@ =@ fun@[<2>%a@])@]@ " Variable.print var
        params f.params
  | Some (b : function_body) ->
      ...
      let specialise =
        ...
        | Default_specialise -> ""
      in
      let print_body ppf _ = Format.fprintf ppf "<Function Body>" in
      Format.fprintf ppf
        "@[<2>(%a%s%s%s%s@ =@ fun@[<2>%a@] ->@ @[<2><%a>@])@]@ " Variable.print
        var stub is_a_functor inline specialise params f.params print_body b
...
let rec print_descr ppf = function
  ...
  | Value_block (tag, fields) ->
      let p ppf fields =
        Array.iter (fun v -> Format.fprintf ppf "%a@ " print v) fields
      in
      Format.fprintf ppf "[%i:@ @[<1>%a@]]" (Tag.to_int tag) p fields
  | Value_unknown reason -> (
      match reason with
      | Unresolved_value value ->
          Format.fprintf ppf "?(due to unresolved %a)" print_unresolved_value
            value
  ...
  | Value_string { contents; size } -> (
      ...
      | Some s ->
          let s = if size > 10 then String.sub s 0 8 ^ "..." else s in
  ...
  | Value_float_array float_array -> (
      ...
      | Contents _ -> Format.fprintf ppf "float_array_imm %i" float_array.size)
  | Value_boxed_int (t, i) -> (
      match t with
...
and print ppf { descr; var; symbol } =
  ...
  in
  Format.fprintf ppf "{ descr=%a var=%a symbol=%a }" print_descr descr
    Variable.print_opt var print symbol
...
let augment_with_kind t (kind : Lambda.value_kind) =
  ...
  | Pfloatval -> (
      ...
      | Value_block _ | Value_int _ | Value_char _ | Value_boxed_int _
      | Value_set_of_closures _ | Value_closure _ | Value_string _
      | Value_float_array _ | Value_bottom ->
          (* Unreachable *)
...
let value_closure ?closure_var ?set_of_closures_var ?set_of_closures_symbol
  ...
  in
  let value_closure =
    { set_of_closures = approx_set_of_closures; closure_id }
  in
  { descr = Value_closure value_closure; var = closure_var; symbol = None }

let create_value_set_of_closures ~(function_decls : function_declarations)
    ~bound_vars ~free_vars ~invariant_params ~recursive ~specialised_args
...
let update_freshening_of_value_set_of_closures value_set_of_closures ~freshening
    =
...
let value_immutable_float_array (contents : t array) =
  ...
  let contents = Array.map (fun t -> augment_with_kind t Pfloatval) contents in
...
let make_const_int_named n : Flambda.named * t = (Const (Int n), value_int n)
...
let make_const_char_named n : Flambda.named * t = (Const (Char n), value_char n)
...
let make_const_boxed_int t i =
  name_expr_fst
    (make_const_boxed_int_named t i)
...
let simplify_named t (named : Flambda.named) : simplification_result_named =
  if Effect_analysis.no_effects_named named then
    ...
    | Value_boxed_int (t, i) ->
        ...
        (const, Replaced_term, approx)
    | Value_symbol sym -> (Symbol sym, Replaced_term, t)
    | Value_string _ | Value_float_array _
    | Value_float None
    | Value_block _ | Value_set_of_closures _ | Value_closure _
    | Value_unknown _ | Value_bottom | Value_extern _ | Value_unresolved _ ->
        (named, Nothing_done, t)
  else (named, Nothing_done, t)
...
let simplify_var t : (Flambda.named * t) option =
  ...
  | Value_string _ | Value_float_array _
  | Value_float None
  | Value_block _ | Value_set_of_closures _ | Value_closure _ | Value_unknown _
  | Value_bottom | Value_extern _ | Value_unresolved _ -> (
      match t.symbol with
...
let join_summaries summary ~replaced_by_var_or_symbol =
  ...
  | true, Nothing_done | true, Replaced_term | false, Replaced_term ->
      Replaced_term
...
let simplify_var_to_var_using_env t ~is_present_in_env =
  match t.var with Some var when is_present_in_env var -> Some var | _ -> None

let known t =
  match t.descr with
  | Value_unresolved _ | Value_unknown _ -> false
  | Value_string _ | Value_float_array _ | Value_bottom | Value_block _
  | Value_int _ | Value_char _ | Value_set_of_closures _ | Value_closure _
  | Value_extern _ | Value_float _ | Value_boxed_int _ | Value_symbol _ ->
      true
...
let useful t =
  ...
  | Value_char _ | Value_set_of_closures _ | Value_float _ | Value_boxed_int _
  | Value_closure _ | Value_extern _ | Value_symbol _ ->
      true
...
let warn_on_mutation t =
  ...
  | Value_int _ | Value_char _ | Value_set_of_closures _ | Value_float _
  | Value_boxed_int _ | Value_closure _ ->
      true
  | Value_string { contents = None }
  | Value_float_array _ | Value_unresolved _ | Value_unknown _ | Value_bottom ->
      false
...
let get_field t ~field_index:i : get_field_result =
  ...
  | Value_block (_tag, fields) ->
      if i >= 0 && i < Array.length fields then Ok fields.(i)
      else
  ...
  | Value_bottom | Value_int _ | Value_char _ ->
  ...
  | Value_symbol _ | Value_extern _ ->
      ...
      Ok (value_unknown Other)
  | Value_unknown reason -> Ok (value_unknown reason)
  | Value_unresolved value ->
...
let check_approx_for_block t =
  match t.descr with
  | Value_block (tag, fields) -> Ok (tag, fields)
  | Value_bottom | Value_int _ | Value_char _ | Value_float_array _
  | Value_string _ | Value_float _ | Value_boxed_int _ | Value_set_of_closures _
  | Value_closure _ | Value_symbol _ | Value_extern _ | Value_unknown _
  | Value_unresolved _ ->
      Wrong

let descrs approxs = List.map (fun v -> v.descr) approxs

let equal_boxed_int (type t1 t2) (bi1 : t1 boxed_int) (i1 : t1)
...
*)
let rec meet_descr ~really_import_approx d1 d2 =
  match (d1, d2) with
  | Value_int i, Value_int j when i = j -> d1
  | Value_symbol s1, Value_symbol s2 when Symbol.equal s1 s2 -> d1
  | Value_extern e1, Value_extern e2 when Export_id.equal e1 e2 -> d1
  | Value_float i, Value_float j when equal_floats i j -> d1
  | Value_boxed_int (bi1, i1), Value_boxed_int (bi2, i2)
    when equal_boxed_int bi1 i1 bi2 i2 ->
      d1
...
and meet ~really_import_approx a1 a2 =
  match (a1, a2) with
  | { descr = Value_bottom }, a | a, { descr = Value_bottom } -> a
  | { descr = Value_symbol _ | Value_extern _ }, _
  | _, { descr = Value_symbol _ | Value_extern _ } ->
      meet ~really_import_approx (really_import_approx a1)
        (really_import_approx a2)
  | _ ->
      let var =
        match (a1.var, a2.var) with
        | None, _ | _, None -> None
        | Some v1, Some v2 -> if Variable.equal v1 v2 then Some v1 else None
      in
      let symbol =
...
let freshen_and_check_closure_id (value_set_of_closures : value_set_of_closures)
    closure_id =
  let closure_id =
    Freshening.Project_var.apply_closure_id value_set_of_closures.freshening
      closure_id
  in
  try
    ignore
  ...
  with Not_found ->
    Misc.fatal_error
      (Format.asprintf "Function %a not found in the set of closures@ %a@.%a@."
         Closure_id.print closure_id print_value_set_of_closures
         value_set_of_closures print_function_declarations
...
let check_approx_for_set_of_closures t : checked_approx_for_set_of_closures =
  ...
  | Value_closure _ | Value_block _ | Value_int _ | Value_char _ | Value_float _
  | Value_boxed_int _ | Value_unknown _ | Value_bottom | Value_extern _
...
let check_approx_for_closure_allowing_unresolved t :
  ...
  | Value_closure value_closure -> (
      match value_closure.set_of_closures.descr with
      | Value_set_of_closures value_set_of_closures ->
          let symbol =
            match value_closure.set_of_closures.symbol with
            | Some (symbol, None) -> Some symbol
            | None | Some (_, Some _) -> None
          in
          Ok
            ( value_closure,
              value_closure.set_of_closures.var,
              symbol,
              value_set_of_closures )
      | Value_unresolved _ | Value_closure _ | Value_block _ | Value_int _
      | Value_char _ | Value_float _ | Value_boxed_int _ | Value_unknown _
  ...
  | Value_float _ | Value_boxed_int _ | Value_bottom | Value_extern _
  | Value_string _ | Value_float_array _ | Value_symbol _ ->
      Wrong
...
let approx_for_bound_var value_set_of_closures var =
  ...
  with Not_found ->
    Misc.fatal_errorf
      "The set-of-closures approximation %a@ does not bind the variable \
       %a@.%s@."
      print_value_set_of_closures value_set_of_closures Var_within_closure.print
      var
...
let check_approx_for_float t : float option =
  match t.descr with
  | Value_float f -> f
  | Value_unresolved _ | Value_unknown _ | Value_string _ | Value_float_array _
  ...
  | Contents contents ->
      Array.fold_right
        (fun elt acc ->
          ...
          | Some acc, Value_float (Some f) -> Some (f :: acc)
          | None, _
          | ( Some _,
              ( Value_float None
              | Value_unresolved _ | Value_unknown _ | Value_string _
              | Value_float_array _ | Value_bottom | Value_block _ | Value_int _
              | Value_char _ | Value_set_of_closures _ | Value_closure _
...
let check_approx_for_string t : string option =
  ...
  | Value_float _ | Value_unresolved _ | Value_unknown _ | Value_float_array _
  | Value_bottom | Value_block _ | Value_int _ | Value_char _
  | Value_set_of_closures _ | Value_closure _ | Value_extern _
  | Value_boxed_int _ | Value_symbol _ ->
      None
...
let potentially_taken_const_switch_branch t branch =
  ...
  | Value_unresolved _ | Value_unknown _ | Value_extern _ | Value_symbol _ ->
      ...
      Can_be_taken
  | Value_int i when i = branch -> Must_be_taken
  | Value_char c when Char.code c = branch -> Must_be_taken
  | Value_int _ | Value_char _ -> Cannot_be_taken
  | Value_block _ | Value_float _ | Value_float_array _ | Value_string _
  | Value_closure _ | Value_set_of_closures _ | Value_boxed_int _ | Value_bottom
    ->
      Cannot_be_taken
...
let potentially_taken_block_switch_branch t tag =
  ...
  | Value_unresolved _ | Value_unknown _ | Value_extern _ | Value_symbol _ ->
      Can_be_taken
  | Value_int _ | Value_char _ -> Cannot_be_taken
  | Value_block (block_tag, _) when Tag.to_int block_tag = tag -> Must_be_taken
  | Value_float _ when tag = Obj.double_tag -> Must_be_taken
  | Value_float_array _ when tag = Obj.double_array_tag -> Must_be_taken
  | Value_string _ when tag = Obj.string_tag -> Must_be_taken
  | (Value_closure _ | Value_set_of_closures _)
    when tag = Obj.closure_tag || tag = Obj.infix_tag ->
      Can_be_taken
  | Value_boxed_int _ when tag = Obj.custom_tag -> Must_be_taken
  | Value_block _ | Value_float _ | Value_set_of_closures _ | Value_closure _
  | Value_string _ | Value_float_array _ | Value_boxed_int _ ->
      Cannot_be_taken
  | Value_bottom -> Cannot_be_taken
...
let clear_function_bodies (function_decls : function_declarations) =
  let funs =
    Variable.Map.map
      (fun (fun_decl : function_declaration) ->
        match fun_decl.function_body with
        | None | Some { stub = true; _ } -> fun_decl
        | Some _ -> { fun_decl with function_body = None })
      function_decls.funs
  in
  { function_decls with funs }

let update_function_declaration_body (function_decl : function_declaration)
