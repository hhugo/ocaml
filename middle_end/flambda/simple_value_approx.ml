...
let print_value_set_of_closures
    ppf { function_decls = { funs }; invariant_params; freshening; size; _ }
=
  Format.fprintf ppf
    ...
    (Variable.Map.print
       (fun ppf some_size ->
          ...
          | Some size -> Format.fprintf ppf "Some %d" size)) (Lazy.force size)

let print_unresolved_value ppf =
  function
...
let print_function_declaration ppf var (f : function_declaration) =
  ...
  | Some (b : function_body) ->
    ...
    Format.fprintf ppf "@[<2>(%a%s%s%s%s@ =@ fun@[<2>%a@] ->@ @[<2><%a>@])@]@ "
      Variable.print var stub is_a_functor inline specialise params f.params
      print_body b
...
let rec print_descr ppf =
  ...
  | Value_float_array float_array ->
    ...
    | Unknown_or_mutable -> Format.fprintf ppf "float_array %i" float_array.size
...
let value_closure
    ?closure_var ?set_of_closures_var ?set_of_closures_symbol
...
=
  ...
  in
  let value_closure = { set_of_closures = approx_set_of_closures; closure_id }
  in
       ...
       Variable.Map.fold
         (fun fun_var function_decl sizes ->
            ...
            | Some function_body ->
              ...
              in
              Variable.Map.add fun_var size sizes) function_decls.funs
         Variable.Map.empty)
  in
  {
    function_decls;
...
let update_freshening_of_value_set_of_closures
    value_set_of_closures ~freshening
=
...
let simplify_named_using_env t ~is_present_in_env named =
  let (replaced_by_var_or_symbol, named) =
    ...
    | _ ->
      ...
      | Some (sym, Some field) -> true, Flambda.Read_symbol_field (sym, field)
      | None -> false, named
      end
  in
...
let equal_boxed_int
    (type t1) (type t2) (bi1 : t1 boxed_int) (i1 : t1) (bi2 : t2 boxed_int)
    (i2 : t2)
=
  match bi1, bi2 with
...
let rec meet_descr ~really_import_approx d1 d2 =
  ...
  | Value_block (tag1, a1), Value_block (tag2, a2)
    ...
    let fields = Array.mapi (fun i v -> meet ~really_import_approx v a2.(i)) a1
    in
    ...
    let symbol =
      ...
      | Some (v1, field1), Some (v2, field2) ->
        if Symbol.equal v1 v2 then
          begin match field1, field2 with
          | None, None -> a1.symbol
          ...
          None
    in
    { descr = meet_descr ~really_import_approx a1.descr a2.descr; var; symbol }
  ...
  | Not_found ->
    Misc.fatal_errorf
      "The set-of-closures approximation %a@ does not bind the variable \
       %a@.%s@." print_value_set_of_closures value_set_of_closures
...
let function_declaration_approx
    ~keep_body fun_var (fun_decl : Flambda.function_declaration)
=
  let function_body =
    if not (keep_body fun_var fun_decl) then
      None
    else
      Some
        {
...
let function_declarations_approx
    ~keep_body (fun_decls : Flambda.function_declarations)
=
  let funs =
...
let update_function_declaration_body
    (function_decl : function_declaration) (f : Flambda.t -> Flambda.t)
=
  match function_decl.function_body with
  | None -> function_decl
  | Some function_body ->
