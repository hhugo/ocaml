...
module Transform = struct
  ...
  let what_to_specialise ~env ~(set_of_closures:Flambda.set_of_closures) =
    ...
    if not (precondition ~env ~set_of_closures) then
      ...
      let module B = Inlining_cost.Benefit 
      in
      ...
      Flambda_iterators.fold_function_decls_ignoring_stubs set_of_closures
        ...
        ->
          ...
          let small_enough_to_duplicate =
            let module W = Inlining_cost.Whether_sufficient_benefit 
            in
            let wsb =
              W.create_estimate ~original_size:0 ~toplevel:false ~branch_depth:0
                ~new_size:(body_size / !Clflags.unbox_closures_factor + 1)
                ~benefit:saved_by_not_building_closure ~lifting:false ~round
            in
            W.evaluate wsb
          in
          ...
          let bound_by_the_closure =
            Flambda_utils.variables_bound_by_the_closure
              (Closure_id.wrap fun_var) set_of_closures.function_decls
          in
          Variable.Set.fold
            (fun inner_free_var what_to_specialise ->
               W.new_specialised_arg what_to_specialise ~fun_var
                 ~group:inner_free_var
                 ~definition:(Existing_inner_free_var inner_free_var))
            bound_by_the_closure what_to_specialise
        )
end
