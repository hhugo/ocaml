...
=
  ...
  | Block (tag, fields) ->
    ...
    =
      match field with
      | Const _ -> field
      | Symbol sym -> Symbol (substitute_symbol sym)
    in
    let fields = List.map subst_field fields in
    Block (tag, fields)
  | Set_of_closures set_of_closures ->
    Set_of_closures
      (Flambda_iterators.map_symbols_on_set_of_closures ~f:substitute_symbol
         set_of_closures)
...
let cannot_share (const : Flambda.constant_defining_value) =
  ...
  | Allocated_const _ | Set_of_closures _ | Project_closure _ | Block _ -> false

let share_definition
    constant_to_symbol_tbl sharing_symbol_tbl symbol def end_symbol
=
  ...
  if cannot_share def || Symbol.equal symbol end_symbol then
    ...
    match Constant_defining_value.Tbl.find constant_to_symbol_tbl def with
    | exception Not_found ->
...
let share_constants (program : Flambda.program) =
  ...
  let rec loop (program : Flambda.program_body) : Flambda.program_body =
    ...
    | Let_symbol (symbol, def, program) ->
      ...
      with
      | None -> loop program
      | Some def' -> Let_symbol (symbol, def', loop program)
      end
        ...
        List.map
          (fun field ->
             Flambda_iterators.map_symbols
               ~f:(fun symbol ->
                 try Symbol.Tbl.find sharing_symbol_tbl symbol
                 with
                 | Not_found -> symbol
               ) field) fields
      in
