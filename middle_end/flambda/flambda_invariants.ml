...
exception
  Mutable_binding_occurrence_not_from_current_compilation_unit of
    Mutable_variable.t

exception Binding_occurrence_of_variable_already_bound of Variable.t

exception
  Binding_occurrence_of_mutable_variable_already_bound of Mutable_variable.t
...
exception
  Vars_in_function_body_not_bound_by_closure_or_params of
    Variable.Set.t
...
exception
  Free_variables_set_is_lying of
    Variable.t
    * Variable.Set.t
    * Variable.Set.t
...
exception
  Sequential_logical_operator_primitives_must_be_expanded of
...
exception
  Move_to_a_closure_not_in_the_free_variables of Variable.t * Variable.Set.t
...
let variable_and_symbol_invariants (program : Flambda.program) =
  ...
  in
  let declare_variables vars = Variable.Set.iter declare_variable vars in
  ...
  let check_mutable_variable_is_bound (_, mut_var_env, _) mut_var =
    if not (Mutable_variable.Set.mem mut_var mut_var_env) then
      ...
      let env =
        List.fold_left
          (fun env (var, def) ->
             ...
             add_binding_occurrence env var) env defs
      in
      List.iter
        (fun (var, def) ->
  ...
  and loop_named env (named : Flambda.named) =
    ...
    | Prim (prim, args, dbg) ->
      ...
      check_variables_are_bound env args;
      ignore_debuginfo dbg
    | Expr expr -> loop env expr
  and loop_set_of_closures
      env
  ...
  =
    ...
    let
      { Flambda.is_classic_mode; set_of_closures_id; set_of_closures_origin;
        funs } =
      function_decls
    in
    ...
    let variables_in_closure =
      Variable.Map.fold
        (fun var (var_in_closure : Flambda.specialised_to) variables_in_closure
         ->
           ...
           Variable.Set.add var variables_in_closure) free_vars
        Variable.Set.empty
    in
    let (all_params, all_free_vars) =
      Variable.Map.fold
        (fun fun_var function_decl acc ->
           ...
           (if not (Variable.Set.subset free_variables' free_variables) then
              raise
                (Free_variables_set_is_lying
                   (fun_var, free_variables, free_variables', function_decl)));
           ...
           in
           let bad = Variable.Set.diff free_variables acceptable_free_variables
           in
           (if not (Variable.Set.is_empty bad) then
              raise
           ...
           (if all_params_size <> old_all_params_size + params_size then
           ...
           in
           loop body_env body;
           all_params, Variable.Set.union free_variables all_free_vars) funs
...
*)
    ...
    Variable.Map.iter
      (fun _inner_var (specialised_to : Flambda.specialised_to) ->
         ...
         | Some projection ->
           ...
           if not (Variable.Map.mem projecting_from free_vars) then
             raise (Projection_must_be_a_free_var projection)) free_vars;
    Variable.Map.iter
      (fun being_specialised (specialised_to : Flambda.specialised_to) ->
         (if not (Variable.Set.mem being_specialised all_params) then
         ...
         | Some projection ->
           let projecting_from = Projection.projecting_from projection in
           if not (Variable.Map.mem projecting_from specialised_args) then
             raise (Projection_must_be_a_specialised_arg projection))
      specialised_args
  in
  let loop_constant_defining_value
      env (const : Flambda.constant_defining_value)
  =
    match const with
    | Flambda.Allocated_const c -> ignore_allocated_const c
    | Flambda.Block (tag, fields) ->
      ignore_tag tag;
      List.iter
        (fun (fields : Flambda.constant_defining_value_block_field) ->
           match fields with
           | Const c -> ignore_const c
           | Symbol s -> check_symbol_is_bound env s) fields
  ...
  let rec loop_program_body env (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (defs, program) ->
      let env =
        List.fold_left
          (fun env (symbol, _) -> add_binding_occurrence_of_symbol env symbol)
          env defs
      in
      List.iter (fun (_, def) -> loop_constant_defining_value env def) defs;
    ...
    | Effect (expr, program) ->
      loop env expr;
      loop_program_body env program
    | End root -> check_symbol_is_bound env root
  in
  let env =
    Symbol.Set.fold
      (fun symbol env -> add_binding_occurrence_of_symbol env symbol)
...
let declared_var_within_closure (flam : Flambda.program) =
  ...
  let add_and_check var =
    (if Var_within_closure.Set.mem var !bound then
...
let every_declared_closure_is_from_current_compilation_unit flam =
  ...
  Flambda_iterators.iter_on_sets_of_closures
    (fun { Flambda.function_decls; _ } ->
       ...
       in
       if
         not (Compilation_unit.equal compilation_unit current_compilation_unit)
       then
         raise (Declared_closure_from_another_unit compilation_unit)) flam

let declared_closure_ids program =
  let bound = ref Closure_id.Set.empty in
  let bound_multiple_times = ref None in
  let add_and_check var =
    (if Closure_id.Set.mem var !bound then bound_multiple_times := Some var);
...
let no_closure_id_is_bound_multiple_times program =
  ...
  | _, Some closure_id -> raise (Closure_id_is_bound_multiple_times closure_id)
  | _, None -> ()
...
let declared_set_of_closures_ids program =
  ...
  let bound_multiple_times = ref None in
  let add_and_check var =
    (if Set_of_closures_id.Set.mem var !bound then
       bound_multiple_times := Some var);
...
=
  ...
  let used_from_current_unit =
    Closure_id.Set.filter
      (fun cu -> Closure_id.in_compilation_unit cu current_compilation_unit)
      used
  in
  let counter_examples = Closure_id.Set.diff used_from_current_unit declared in
  if Closure_id.Set.is_empty counter_examples then
    ()
  else
    raise (Unbound_closure_ids counter_examples)
...
=
  ...
  let counter_examples =
    Var_within_closure.Set.diff used_from_current_unit declared
  in
  if Var_within_closure.Set.is_empty counter_examples then
    ()
  else
    raise (Unbound_vars_within_closures counter_examples)

let every_static_exception_is_caught flam =
  let check env (flam : Flambda.t) =
    match flam with
    | Static_raise (exn, _) ->
      if not (Static_exception.Set.mem exn env) then
        raise (Static_exception_not_caught exn)
    | _ -> ()
  in
  let rec loop env (flam : Flambda.t) =
    match flam with
...
=
  ...
  Flambda_iterators.iter_named_of_program program
    ...
    | Move_within_set_of_closures { start_from; move_to; _ } ->
      let moved_to =
        try Closure_id.Map.find start_from !moves
        with
        | Not_found -> Closure_id.Set.empty
      in
      moves :=
        Closure_id.Map.add start_from (Closure_id.Set.add move_to moved_to)
          !moves
    | _ -> ()
    );
  Flambda_iterators.iter_on_set_of_closures_of_program program
...
let check_exn ?(kind=Normal) (flam : Flambda.program) =
  ...
  | exn ->
    ...
    | Binding_occurrence_not_from_current_compilation_unit var ->
      Format.eprintf
        ">> Binding occurrence of variable marked as not being from the \
         current compilation unit: %a" Variable.print var
    | Mutable_binding_occurrence_not_from_current_compilation_unit mut_var ->
      Format.eprintf
        ">> Binding occurrence of mutable variable marked as not being from \
         the current compilation unit: %a" Mutable_variable.print mut_var
    | Binding_occurrence_of_variable_already_bound var ->
      Format.eprintf
        ">> Binding occurrence of variable that was already bound: %a"
        Variable.print var
    ...
    | Binding_occurrence_of_symbol_already_bound sym ->
      Format.eprintf
        ">> Binding occurrence of symbol that was already bound: %a"
        Symbol.print sym
    | Unbound_variable var ->
      Format.eprintf ">> Unbound variable: %a" Variable.print var
    | Unbound_mutable_variable mut_var ->
      Format.eprintf ">> Unbound mutable variable: %a" Mutable_variable.print
        mut_var
    | Unbound_symbol sym ->
      Format.eprintf ">> Unbound symbol: %a %s" Symbol.print sym
        (Printexc.raw_backtrace_to_string (Printexc.get_callstack 100))
    | Vars_in_function_body_not_bound_by_closure_or_params
        (vars, set_of_closures, fun_var)
      ->
      Format.eprintf
        ">> Variable(s) (%a) in the body of a function declaration (fun_var \
         = %a) that is not bound by either the closure or the function's \
         parameter list.  Set of closures: %a" Variable.Set.print vars
        Variable.print fun_var Flambda.print_set_of_closures set_of_closures
    ...
    | Specialised_arg_that_is_not_a_parameter var ->
      Format.eprintf
        ">> Variable in [specialised_args] that is not a parameter of any of \
         the function(s) in the corresponding declaration(s): %a" Variable.print
        var
    | Projection_must_be_a_free_var var ->
      Format.eprintf
        ">> Projection %a in [free_vars] from a variable that is not a \
         (inner) free variable of the set of closures" Projection.print var
    | Projection_must_be_a_specialised_arg var ->
      Format.eprintf
        ">> Projection %a in [specialised_args] from a variable that is not \
         a (inner) specialised argument variable of the set of closures"
        Projection.print var
    | Free_variables_set_is_lying (var, claimed, calculated, function_decl) ->
      Format.eprintf
        ">> Function declaration whose [free_variables] set (%a) is not a \
         superset of the result of [Flambda.free_variables] applied to the \
         body of the function (%a).  Declaration: %a" Variable.Set.print claimed
        Variable.Set.print calculated Flambda.print_function_declaration
        (var, function_decl)
    | Set_of_closures_free_vars_map_has_wrong_range vars ->
      Format.eprintf
        ">> [free_vars] map in set of closures has in its range variables \
         that are not free variables of the corresponding functions: %a"
        Variable.Set.print vars
    | Sequential_logical_operator_primitives_must_be_expanded prim ->
      Format.eprintf
        ">> Sequential logical operator primitives must be expanded (see \
         closure_conversion.ml): %a" Printclambda_primitives.primitive prim
    ...
    | Declared_closure_from_another_unit compilation_unit ->
      Format.eprintf
        ">> Closure declared as being from another compilation unit: %a"
        Compilation_unit.print compilation_unit
    | Unbound_closure_ids closure_ids ->
      Format.eprintf
        ">> Unbound closure ID(s) from the current compilation unit: %a"
        Closure_id.Set.print closure_ids
    | Unbound_vars_within_closures vars_within_closures ->
      Format.eprintf
        ">> Unbound variable(s) within closure(s) from the current \
         compilation_unit: %a" Var_within_closure.Set.print vars_within_closures
    | Static_exception_not_caught static_exn ->
      Format.eprintf ">> Uncaught static exception: %a" Static_exception.print
        static_exn
    ...
    | Move_to_a_closure_not_in_the_free_variables (start_from, move_to) ->
      Format.eprintf
        ">> A Move_within_set_of_closures from the closure %a to closures \
         that are not parts of its free variables: %a" Variable.print start_from
