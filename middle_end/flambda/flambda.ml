...
let rec lam ppf (flam : t) =
  ...
  | Assign { being_assigned; new_value } ->
      fprintf ppf "@[<2>(assign@ %a@ %a)@]" Mutable_variable.print
        being_assigned Variable.print new_value
...
and print_set_of_closures ppf (set_of_closures : set_of_closures) =
  ...
  | { function_decls; free_vars; specialised_args } ->
      ...
      fprintf ppf
        ...
        Set_of_closures_id.print function_decls.set_of_closures_id funs
        function_decls.funs vars free_vars spec specialised_args
...
and print_const ppf (c : const) =
  match c with Int n -> fprintf ppf "%i" n | Char c -> fprintf ppf "%C" c
...
let rec print_program_body ppf (program : program_body) =
  ...
  | Let_symbol (symbol, constant_defining_value, body) ->
      ...
      fprintf ppf "@[<2>let_symbol@ @[%a@]@]@."
        (Format.pp_print_list symbol_binding)
        defs;
  ...
  | _ ->
      ...
      let rec aux (flam : t) : unit =
        ...
        | Apply { func; args; kind = _; dbg = _ } -> (
            (match ignore_uses_as_callee with
...
and variables_usage_named ?ignore_uses_in_project_var ?ignore_uses_as_callee
    ?ignore_uses_as_argument ~all_used_variables named =
  let free = ref Variable.Set.empty in
  let free_variable fv = free := Variable.Set.add fv !free in
  (match named with
  ...
  | Project_var { closure; closure_id = _; var = _ } -> (
      match ignore_uses_in_project_var with
...
let create_let var defining_expr body : t =
  (match !Clflags.dump_flambda_let with
  | None -> ()
  | Some stamp ->
...
let map_defining_expr_of_let let_expr ~f =
  ...
  if defining_expr == let_expr.defining_expr then Let let_expr
  else
...
let map_lets t ~for_defining_expr ~for_last_body ~after_rebuild =
  let rec loop (t : t) ~rev_lets =
    ...
    | Let { var; defining_expr; body; _ } ->
        ...
        let original =
          if new_defining_expr == defining_expr then Some t else None
        in
  ...
  and aux_named (named : named) =
    ...
    | Set_of_closures
        ...
        if not toplevel then
          Variable.Map.iter
            (fun _ (decl : function_declaration) -> aux decl.body)
            funcs.funs
    | Expr flam -> aux flam
  in
...
module With_free_variables = struct
  ...
  let expr (t : expr t) =
    match t with Expr (expr, free_vars) -> Named (Expr expr, free_vars)

  let contents (type a) (t : a t) : a =
    match t with Expr (expr, _) -> expr | Named (named, _) -> named
...
let update_body_of_function_declaration (func_decl : function_declaration) ~body
...
let update_function_decl's_params_and_body (func_decl : function_declaration)
    ~params ~body =
  {
    closure_origin = func_decl.closure_origin;
    params;
...
let equal_specialised_to (spec_to1 : specialised_to) (spec_to2 : specialised_to)
    =
