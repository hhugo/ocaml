...
let print_specialised_to ppf (spec_to : specialised_to) =
  ...
  | Some projection ->
    fprintf ppf "%a(= %a)" Variable.print spec_to.var Projection.print
      projection
...
let rec lam ppf (flam : t) =
  ...
  | Apply { func; args; kind; inline; dbg } ->
    ...
    fprintf ppf "@[<2>(apply%a%a<%s>@ %a%a)@]" direct () inline ()
      (Debuginfo.to_string dbg) Variable.print func Variable.print_list args
  | Assign { being_assigned; new_value } ->
    fprintf ppf "@[<2>(assign@ %a@ %a)@]" Mutable_variable.print being_assigned
  ...
  | Send { kind; meth; obj; args; dbg = _ } ->
    ...
    let kind =
      ...
      | Cached -> "cached"
    in
    fprintf ppf "@[<2>(send%s@ %a@ %a%a)@]" kind Variable.print obj
      Variable.print meth print_args args
  ...
  | Let { var = id; defining_expr = arg; body; _ } ->
    let rec letbody (ul : t) =
      ...
      | _ -> ul
    in
    fprintf ppf "@[<2>(let@ @[<hv 1>(@[<2>%a@ %a@]" Variable.print id
      print_named arg;
  ...
  | Let_mutable { var = mut_var; initial_value = var; body; contents_kind } ->
    ...
    in
    fprintf ppf "@[<2>(let_mutable%a@ @[<2>%a@ %a@]@ %a)@]" print_kind
      contents_kind Mutable_variable.print mut_var Variable.print var lam body
  ...
  | Switch (larg, sw) ->
    let switch ppf (sw : switch) =
      ...
      List.iter
        (fun (n, l) ->
           (if !spc then fprintf ppf "@ " else spc := true);
           fprintf ppf "@[<hv 1>case int %i:@ %a@]" n lam l) sw.consts;
      List.iter
        (fun (n, l) ->
           (if !spc then fprintf ppf "@ " else spc := true);
           fprintf ppf "@[<hv 1>case tag %i:@ %a@]" n lam l) sw.blocks;
    ...
    in
    fprintf ppf "@[<1>(%s(%i,%i) %a@ @[<v 0>%a@])@]"
      begin match sw.failaction with
      | None -> "switch*"
      | _ -> "switch"
      end (Int.Set.cardinal sw.numconsts) (Int.Set.cardinal sw.numblocks)
  ...
  | String_switch (arg, cases, default) ->
    let switch ppf cases =
      ...
      | None -> ()
    in
    fprintf ppf "@[<1>(stringswitch %a@ @[<v 0>%a@])@]" Variable.print arg
      switch cases
  | Static_raise (i, ls) ->
    let lams ppf largs =
      List.iter (fun l -> fprintf ppf "@ %a" Variable.print l) largs
    in
    fprintf ppf "@[<2>(exit@ %a%a)@]" Static_exception.print i lams ls
  | Static_catch (i, vars, lbody, lhandler) ->
    fprintf ppf "@[<2>(catch@ %a@;<1 -1>with (%a%a)@ %a)@]" lam lbody
      Static_exception.print i
      (fun ppf vars ->
         match vars with
         | [] -> ()
         | _ -> List.iter (fun x -> fprintf ppf " %a" Variable.print x) vars)
      vars lam lhandler
  | Try_with (lbody, param, lhandler) ->
    fprintf ppf "@[<2>(try@ %a@;<1 -1>with %a@ %a)@]" lam lbody Variable.print
      param lam lhandler
  | If_then_else (lcond, lif, lelse) ->
    fprintf ppf "@[<2>(if@ %a@ then begin@ %a@ end else begin@ %a@ end)@]"
      Variable.print lcond lam lif lam lelse
  | While (lcond, lbody) ->
    fprintf ppf "@[<2>(while@ %a@ %a)@]" lam lcond lam lbody
  | For { bound_var; from_value; to_value; direction; body } ->
    fprintf ppf "@[<2>(for %a@ %a@ %s@ %a@ %a)@]" Variable.print bound_var
      Variable.print from_value
      begin match direction with
      | Asttypes.Upto -> "to"
      | Asttypes.Downto -> "downto"
      end Variable.print to_value lam body
...
and print_named ppf (named : named) =
  ...
  | Project_closure project_closure -> print_project_closure ppf project_closure
  ...
  | Set_of_closures set_of_closures -> print_set_of_closures ppf set_of_closures
  | Prim (prim, args, dbg) ->
    fprintf ppf "@[<2>(%a<%s>%a)@]" Printclambda_primitives.primitive prim
      (Debuginfo.to_string dbg) Variable.print_list args
  | Expr expr -> fprintf ppf "*%a" lam expr
(* lam ppf expr *)

and print_function_declaration ppf var (f : function_declaration) =
  let param ppf p = Variable.print ppf (Parameter.var p) in
  let params ppf = List.iter (fprintf ppf "@ %a" param) in
  ...
  fprintf ppf "@[<2>(%a%s%s%s%s@ =@ fun@[<2>%a@] ->@ @[<2>%a@])@]@ "
    Variable.print var stub is_a_functor inline specialise params f.params lam
    f.body
...
and print_set_of_closures ppf (set_of_closures : set_of_closures) =
  ...
  | { function_decls; free_vars; specialised_args } ->
    ...
    let vars ppf =
      Variable.Map.iter
        (fun id v ->
           fprintf ppf "@ %a -rename-> %a" Variable.print id
             print_specialised_to v)
    in
    let spec ppf spec_args =
      if not (Variable.Map.is_empty spec_args) then
        (fprintf ppf "@ ";
         Variable.Map.iter
           (fun id (spec_to : specialised_to) ->
              fprintf ppf "@ %a := %a" Variable.print id print_specialised_to
                spec_to) spec_args)
    in
    fprintf ppf
      "@[<2>(set_of_closures id=%a@ %a@ @[<2>free_vars={%a@ }@]@ \
       @[<2>specialised_args={%a})@]@ @[<2>direct_call_surrogates=%a@]@ \
       @[<2>set_of_closures_origin=%a@]@]]" Set_of_closures_id.print
      function_decls.set_of_closures_id funs function_decls.funs vars free_vars
      spec specialised_args (Variable.Map.print Variable.print)
      set_of_closures.direct_call_surrogates Set_of_closures_origin.print
...
let print_constant_defining_value ppf (const : constant_defining_value) =
  ...
  | Block (tag, fields) ->
    ...
    in
    let print_fields ppf = List.iter (fprintf ppf "@ %a" print_field) in
    fprintf ppf "(Block (tag %d, %a))" (Tag.to_int tag) print_fields fields
...
let rec print_program_body ppf (program : program_body) =
  ...
  | Initialize_symbol (symbol, tag, fields, program) ->
    fprintf ppf "@[<2>initialize_symbol@ (@[<2>%a@ %a@ %a@])@]@." Symbol.print
      symbol Tag.print tag (Format.pp_print_list lam) fields;
    print_program_body ppf program
  | Effect (expr, program) ->
    fprintf ppf "@[<2>effect@ %a@]@." lam expr;
    print_program_body ppf program
  | End root -> fprintf ppf "End %a" Symbol.print root

let print_program ppf program =
  Symbol.Set.iter
    (fun symbol -> fprintf ppf "@[import_symbol@ %a@]@." Symbol.print symbol)
    program.imported_symbols;
  print_program_body ppf program.program_body

let rec variables_usage
    ?ignore_uses_as_callee ?ignore_uses_as_argument ?ignore_uses_in_project_var
  ...
  | _ ->
    ...
    let rec aux (flam : t) : unit =
      ...
      | Let_rec (bindings, body) ->
        List.iter
          (fun (var, defining_expr) ->
             bound_variable var;
             free_variables
               (variables_usage_named ?ignore_uses_in_project_var
                  ~all_used_variables defining_expr)) bindings;
        aux body
      ...
      | Static_raise (_, es) -> List.iter free_variable es
      ...
      | For { bound_var; from_value; to_value; direction = _; body } ->
        ...
        aux body
      | Assign { being_assigned = _; new_value } -> free_variable new_value
    ...
    in
    aux tree;
    if all_used_variables then !free else Variable.Set.diff !free !bound

and variables_usage_named
    ?ignore_uses_in_project_var ?ignore_uses_as_callee ?ignore_uses_as_argument
...
=
  ...
  | Set_of_closures { free_vars; specialised_args; _ } ->
    ...
    Variable.Map.iter
      (fun _ (renamed_to : specialised_to) ->
...
let free_variables_named ?ignore_uses_in_project_var named =
  variables_usage_named ?ignore_uses_in_project_var ~all_used_variables:false
    named
...
let used_variables_named ?ignore_uses_in_project_var named =
  variables_usage_named ?ignore_uses_in_project_var ~all_used_variables:true
    named
...
let create_let var defining_expr body : t =
  ...
  | None -> ()
  | Some stamp ->
    Variable.debug_when_stamp_matches var ~stamp
      ~f:(fun () ->
  ...
  let (defining_expr, free_vars_of_defining_expr) =
    ...
    | Expr
        (Let
           { var = var1; defining_expr; body = Var var2;
             free_vars_of_defining_expr; _ })
      when Variable.equal var1 var2 ->
...
let map_lets t ~for_defining_expr ~for_last_body ~after_rebuild =
  let rec loop (t : t) ~rev_lets =
    ...
    | Let { var; defining_expr; body; _ } ->
      ...
      let original = if new_defining_expr == defining_expr then Some t else None
      in
    ...
    | t ->
      ...
      List.fold_left
        (fun t (var, defining_expr, original) ->
           let let_expr =
             match original with
           ...
           (if not (new_let == let_expr) then seen_change := true);
           new_let) last_body rev_lets
  in
  loop t ~rev_lets:[]
...
let iter_general ~toplevel f f_named maybe_named =
  let rec aux (t : t) =
    match t with
    | Let _ ->
      iter_lets t ~for_defining_expr:(fun _var named -> aux_named named)
    ...
    | _ ->
      ...
      | Let_rec (defs, body) ->
        ...
        aux body
      | Try_with (f1, _, f2) | While (f1, f2) | Static_catch (_, _, f1, f2) ->
        aux f1;
        aux f2
      | For { body; _ } -> aux body
  ...
  and aux_named (named : named) =
    ...
    | Prim _
      ...
      ()
    | Set_of_closures
        { function_decls = funcs; free_vars = _; specialised_args = _ }
      ->
      if not toplevel then
        Variable.Map.iter (fun _ (decl : function_declaration) -> aux decl.body)
          funcs.funs
    | Expr flam -> aux flam
  in
...
module With_free_variables = struct
  ...
  let of_body_of_let let_expr = Expr (let_expr.body, let_expr.free_vars_of_body)
  let of_expr expr = Expr (expr, free_variables expr)
  let of_named named = Named (named, free_variables_named named)
...
=
  let finish ~last_body ~acc ~rev_lets =
    let module W = With_free_variables 
    in
    let (acc, t) =
      List.fold_left
        (fun (acc, t) (var, defining_expr) ->
           ...
           | None -> acc, t
           | Some defining_expr ->
             let let_expr = W.create_let_reusing_body var defining_expr t in
             acc, W.of_expr let_expr) (acc, W.of_expr last_body) rev_lets
    in
    W.contents t, acc
  in
  let rec loop (t : t) ~acc ~rev_lets =
    match t with
    | Let { var; defining_expr; body; _ } ->
      let (acc, var, defining_expr) = for_defining_expr acc var defining_expr in
...
let free_symbols_helper symbols (named : named) =
  match named with
  | Symbol symbol | Read_symbol_field (symbol, _) ->
    symbols := Symbol.Set.add symbol !symbols
  | Set_of_closures set_of_closures ->
    Variable.Map.iter
      (fun _ (function_decl : function_declaration) ->
...
let free_symbols expr =
  ...
  iter_general ~toplevel:true (fun (_ : t) -> ())
    (fun (named : named) -> free_symbols_helper symbols named) (Is_expr expr);
  !symbols

let free_symbols_named named =
  let symbols = ref Symbol.Set.empty in
  iter_general ~toplevel:true (fun (_ : t) -> ())
    (fun (named : named) -> free_symbols_helper symbols named) (Is_named named);
  !symbols

let free_symbols_allocated_constant_helper
    symbols (const : constant_defining_value)
=
  match const with
  | Allocated_const _ -> ()
  | Block (_, fields) ->
    List.iter
      (function
       ...
       | (Const _ : constant_defining_value_block_field) -> ()) fields
  | Set_of_closures set_of_closures ->
    symbols :=
      Symbol.Set.union !symbols
        (free_symbols_named (Set_of_closures set_of_closures))
  | Project_closure (s, _) -> symbols := Symbol.Set.add s !symbols
...
let free_symbols_program (program : program) =
  ...
  let rec loop (program : program_body) =
    ...
    | Let_rec_symbol (defs, program) ->
      List.iter
        (fun (_, const) -> free_symbols_allocated_constant_helper symbols const)
        defs;
      loop program
    | Initialize_symbol (_, _, fields, program) ->
      List.iter
        (fun field -> symbols := Symbol.Set.union !symbols (free_symbols field))
        fields;
      loop program
    | Effect (expr, program) ->
...
let update_body_of_function_declaration
    (func_decl : function_declaration) ~body
  :
...
let create_function_declaration
    ~params ~body ~stub ~dbg ~(inline:Lambda.inline_attribute)
    ~(specialise:Lambda.specialise_attribute) ~is_a_functor ~closure_origin
  :
  function_declaration
=
  ...
  | true, (Always_inline | Hint_inline | Unroll _) ->
    Misc.fatal_errorf
      "Stubs may not be annotated as [Always_inline], [Hint_inline] or \
       [Unroll]: %a" print body
  end;
  ...
  | true, Always_specialise ->
    Misc.fatal_errorf "Stubs may not be annotated as [Always_specialise]: %a"
      print body
  end;
  {
    closure_origin;
...
let create_function_declarations ~is_classic_mode ~funs =
  ...
  let set_of_closures_origin = Set_of_closures_origin.create set_of_closures_id
  in
...
let import_function_declarations_for_pack
    function_decls import_set_of_closures_id import_set_of_closures_origin
=
  let is_classic_mode = function_decls.is_classic_mode in
...
=
  (if !Clflags.flambda_invariant_checks then
    ...
    *)
     ...
     ((if not (Variable.Set.subset expected_free_vars free_vars_domain) then
         Misc.fatal_errorf
           "create_set_of_closures: [free_vars] mapping of variables bound by the \
            closure(s) is wrong.  (Must map at least %a but only maps %a.)@ \n\
            function_decls:@ %a" Variable.Set.print expected_free_vars
           Variable.Set.print free_vars_domain print_function_declarations
           function_decls);
      let all_params =
        Variable.Map.fold
          (fun _fun_var function_decl all_params ->
             Variable.Set.union (Parameter.Set.vars function_decl.params)
               all_params) function_decls.funs Variable.Set.empty
      in
      let spec_args_domain = Variable.Map.keys specialised_args in
      if not (Variable.Set.subset spec_args_domain all_params) then
        Misc.fatal_errorf
          "create_set_of_closures: [specialised_args] maps variable(s) that are \
           not parameters of the given function declarations.  specialised_args \
           domain=%a all_params=%a \n\
           function_decls:@ %a" Variable.Set.print spec_args_domain
          Variable.Set.print all_params print_function_declarations
          function_decls));
  { function_decls; free_vars; specialised_args; direct_call_surrogates }
...
module Constant_defining_value = struct
  ...
  (struct
    ...
    let compare (t1 : t) (t2 : t) =
      match t1, t2 with
      | Allocated_const c1, Allocated_const c2 -> Allocated_const.compare c1 c2
      ...
      | Project_closure (set1, closure_id1), Project_closure (set2, closure_id2)
        ->
        let c = Symbol.compare set1 set2 in
        if c <> 0 then c else Closure_id.compare closure_id1 closure_id2
    ...
    let output o v = output_string o (Format.asprintf "%a" print v)
  end)
end
...
let equal_specialised_to
    (spec_to1 : specialised_to) (spec_to2 : specialised_to)
=
