...
let traverse
    ...
    ~(symbol_id:Export_id.t Symbol.Map.t) ~(root_symbol:Symbol.t)
=
  ...
  let relevant_imported_vars_within_closure = ref Var_within_closure.Set.empty
  in
  ...
  let conditionally_add_symbol symbol =
    if not (Symbol.Set.mem symbol !relevant_symbols) then
      (relevant_symbols := Symbol.Set.add symbol !relevant_symbols;
       Queue.add (Q_symbol symbol) queue)
  in
  let conditionally_add_set_of_closures_id set_of_closures_id =
    if
      not
        (Set_of_closures_id.Set.mem set_of_closures_id !relevant_set_of_closures)
    then
  ...
  let conditionally_add_export_id export_id =
    if not (Export_id.Set.mem export_id !relevant_export_ids) then
      (relevant_export_ids := Export_id.Set.add export_id !relevant_export_ids;
       Queue.add (Q_export_id export_id) queue)
  in
  let process_approx (approx : Export_info.approx) =
    match approx with
    | Value_id export_id -> conditionally_add_export_id export_id
    | Value_symbol symbol -> conditionally_add_symbol symbol
    | Value_unknown -> ()
  in
  let process_value_set_of_closures (soc : Export_info.value_set_of_closures) =
    ...
    Var_within_closure.Map.iter (fun _ value -> process_approx value)
      soc.bound_vars;
    Closure_id.Map.iter (fun _ value -> process_approx value) soc.results;
    match soc.aliased_symbol with
    | None -> ()
  ...
  let process_function_body (function_body : A.function_body) =
    Flambda_iterators.iter
         ...
         | Flambda.Apply { kind; _ } ->
           ...
           | Direct closure_id ->
             ...
             | set_of_closures_id ->
               ...
               conditionally_add_set_of_closures_id set_of_closures_id
             end
           end
         | _ -> ())
      (fun (named : Flambda.named) ->
         let process_closure_id closure_id =
           ...
           | set_of_closure_id ->
             ...
             relevant_set_of_closures_declaration_only :=
               Set_of_closures_id.Set.add set_of_closure_id
                 !relevant_set_of_closures_declaration_only
         in
         match named with
         ...
         | Project_closure { closure_id; _ } -> process_closure_id closure_id
         ...
         | Project_var { closure_id; var; _ } ->
           ...
           | set_of_closure_id ->
             ...
             relevant_local_vars_with_closure :=
               Var_within_closure.Set.add var !relevant_local_vars_with_closure;
             relevant_set_of_closures_declaration_only :=
               Set_of_closures_id.Set.add set_of_closure_id
                 !relevant_set_of_closures_declaration_only
           end
         | Prim _ | Expr _ | Const _ | Allocated_const _ | Read_mutable _ -> ())
      function_body.body
  in
  let rec loop () =
    if Queue.is_empty queue then
      ...
      (begin match Queue.pop queue with
       | Q_export_id export_id ->
         ...
         | exception Not_found -> ()
         | Value_block (_, approxes) -> Array.iter process_approx approxes
         | Value_closure value_closure ->
           process_value_set_of_closures value_closure.set_of_closures
         | Value_set_of_closures soc -> process_value_set_of_closures soc
         | _ -> ()
         end
       | Q_symbol symbol ->
  ...
  loop ();
  Closure_id.Map.iter
    (fun closure_id set_of_closure_id ->
       if
         Set_of_closures_id.Set.mem set_of_closure_id !relevant_set_of_closures
       then
         relevant_local_closure_ids :=
           Closure_id.Set.add closure_id !relevant_local_closure_ids)
    ...
    (fun set_of_closures_id ->
       ...
       | set_of_closures ->
         Variable.Map.iter
           (fun var _ ->
              relevant_local_vars_with_closure :=
                Var_within_closure.Set.add (Var_within_closure.wrap var)
                  !relevant_local_vars_with_closure) set_of_closures.free_vars)
    !relevant_set_of_closures;
  {
    ...
    relevant_imported_vars_within_closure =
      !relevant_imported_vars_within_closure;
    relevant_local_vars_within_closure = !relevant_local_vars_with_closure
  }
