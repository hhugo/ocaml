...
let traverse
  ...
  let process_value_set_of_closures (soc : Export_info.value_set_of_closures) =
    ...
    Var_within_closure.Map.iter
      (fun _ value -> process_approx value)
      soc.bound_vars;
  ...
  let rec loop () =
    if Queue.is_empty queue then ()
      ...
      | Q_export_id export_id -> (
          match Export_id.Map.find export_id values with
          | exception Not_found -> ()
  ...
  Closure_id.Map.iter
    (fun closure_id set_of_closure_id ->
      if Set_of_closures_id.Set.mem set_of_closure_id !relevant_set_of_closures
      then
        relevant_local_closure_ids :=
          Closure_id.Set.add closure_id !relevant_local_closure_ids)
    closure_id_to_set_of_closures_id;
