...
let import_set_of_closures_id_for_pack units pack
    (set_of_closures_id : Set_of_closures_id.t) : Set_of_closures_id.t =
    ...
    Export_info.value_set_of_closures =
  {
    ...
    bound_vars =
      Var_within_closure.Map.map
        (import_approx_for_pack units pack)
...
let import_descr_for_pack units pack (descr : Export_info.descr) :
    Export_info.descr =
  match descr with
  | ( Value_int _ | Value_char _ | Value_string _ | Value_float _
    | Value_float_array _ | Export_info.Value_boxed_int _
  ...
  let funs =
    Variable.Map.map
      (fun (function_decl : A.function_declaration) ->
        A.update_function_declaration_body function_decl (fun body ->
            import_code_for_pack units pack body))
      function_decls.funs
  in
