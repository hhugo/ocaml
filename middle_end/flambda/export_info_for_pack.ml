...
let import_eid_for_pack units pack id =
  ...
  | Not_found ->
    ...
    let id' =
      if Compilation_unit.Set.mem unit_id units then
...
let import_symbol_for_pack units pack symbol =
  ...
  if Compilation_unit.Set.mem compilation_unit units then
    Symbol.import_for_pack ~pack symbol
  else
    symbol
...
let import_set_of_closures_id_for_pack
    units pack (set_of_closures_id : Set_of_closures_id.t)
  :
...
=
  ...
  in
  if Compilation_unit.Set.mem compilation_unit units then
    Set_of_closures_id.Tbl.memoize rename_set_of_closures_id_state
      (fun _ ->
         Set_of_closures_id.create
           ?name:(Set_of_closures_id.name set_of_closures_id) pack)
      set_of_closures_id
  else
    set_of_closures_id

let import_set_of_closures_origin_for_pack
    units pack (set_of_closures_origin : Set_of_closures_origin.t)
  :
  Set_of_closures_origin.t
=
  Set_of_closures_origin.rename (import_set_of_closures_id_for_pack units pack)
    set_of_closures_origin

let import_set_of_closures
    units pack (set_of_closures : Export_info.value_set_of_closures)
  :
...
=
  {
    ...
    aliased_symbol =
      Option.map (import_symbol_for_pack units pack)
  ...
  Flambda_iterators.map_named
    (function
     ...
     | Set_of_closures set_of_closures ->
       let set_of_closures =
         Flambda.create_set_of_closures ~free_vars:set_of_closures.free_vars
...
and import_function_declarations_for_pack_aux
    units pack (function_decls : Flambda.function_declarations)
=
  Flambda.import_function_declarations_for_pack function_decls
    (import_set_of_closures_id_for_pack units pack)
    (import_set_of_closures_origin_for_pack units pack)

let import_function_declarations_for_pack_aux
    units pack (function_decls : A.function_declarations)
  :
...
let import_function_declarations_approx_for_pack
    units pack (function_decls : A.function_declarations)
=
  ...
  | Not_found ->
    ...
    in
    Set_of_closures_id.Tbl.add imported_function_declarations_table
      original_set_of_closures_id function_decls;
    function_decls
       ...
       (Compilation_unit.Map.map
          (fun map ->
             Export_id.Map.map_keys (import_eid_for_pack units pack)
               (Export_id.Map.map f map)) map) Export_id.Map.empty)
...
let import_for_pack ~pack_units ~pack (exp : Export_info.t) =
  ...
  let sets_of_closures =
    Set_of_closures_id.Map.map_keys import_set_of_closures_id
      (Set_of_closures_id.Map.map import_function_declarations
         exp.sets_of_closures)
  in
  Export_info.create ~sets_of_closures ~offset_fun:exp.offset_fun
    ~offset_fv:exp.offset_fv ~values:(import_eidmap import_descr exp.values)
    ~symbol_id:
    (Symbol.Map.map_keys import_sym (Symbol.Map.map import_eid exp.symbol_id))
    ~constant_closures:exp.constant_closures
    ~invariant_params:
    (Set_of_closures_id.Map.map_keys import_set_of_closures_id
       exp.invariant_params)
    ~recursive:
    (Set_of_closures_id.Map.map_keys import_set_of_closures_id exp.recursive)
