...
let import_set_of_closures =
  ...
  let aux set_of_closures_id =
    ...
    | Some ex_info -> (
        ...
        with Not_found -> Misc.fatal_error "Cannot find set of closures")
  in
  ...
  let import_value_set_of_closures ~set_of_closures_id ~bound_vars ~free_vars
    ...
    | Some function_decls ->
        ...
        let invariant_params =
          ...
          with
          | exception Not_found ->
              if is_classic_mode then Variable.Map.empty
              else
                Misc.fatal_errorf
                  "Set of closures ID %a not found in invariant_params (when \
                   importing [%a: %s])"
                  Set_of_closures_id.print set_of_closures_id Export_id.print ex
                  what
          | found -> found
        in
        let recursive =
          ...
          with
          | exception Not_found ->
              if is_classic_mode then Variable.Set.empty
              else
                Misc.fatal_errorf
      ...
      | Value_float_array float_array -> (
          ...
          | Contents contents ->
              A.value_immutable_float_array
                (Array.map
                   (function
                     | None -> A.value_any_float | Some f -> A.value_float f)
                   contents))
      ...
      | Value_closure
          ...
          let value_set_of_closures =
            import_value_set_of_closures ~set_of_closures_id ~bound_vars
              ~free_vars ~ex_info
              ~what:
                (Format.asprintf "Value_closure %a" Closure_id.print closure_id)
          in
...
let import_symbol sym =
  if Compilenv.is_predefined_exception sym then A.value_unknown Other
  else
