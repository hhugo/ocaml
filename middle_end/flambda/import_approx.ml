    ...
    let sym_to_fun_var_map (clos : A.function_declarations) =
      Variable.Map.fold
        (fun fun_var _ acc ->
           ...
           Symbol.Map.add sym fun_var acc) clos.funs Symbol.Map.empty
    in
    let sym_map = sym_to_fun_var_map clos in
    let f_named (named : Flambda.named) =
      match named with
      | Symbol sym ->
        (try Flambda.Expr (Var (Symbol.Map.find sym sym_map))
         with
         | Not_found -> named)
      | named -> named
    in
      ...
      Variable.Map.map
        (fun (function_decl : A.function_declaration) ->
           A.update_function_declaration_body function_decl
             (Flambda_iterators.map_toplevel_named f_named)) clos.funs
    in
  ...
  let aux set_of_closures_id =
    ...
    | Some ex_info ->
      (try
       ...
       with
       | Not_found -> Misc.fatal_error "Cannot find set of closures")
  in
...
let rec import_ex ex =
  let import_value_set_of_closures
      ~set_of_closures_id ~bound_vars ~free_vars ~(ex_info:Export_info.t) ~what
    :
    A.value_set_of_closures option
  =
    ...
    | Some function_decls ->
      ...
      let invariant_params =
        ...
        | exception Not_found ->
          if is_classic_mode then
            ...
            Misc.fatal_errorf
              "Set of closures ID %a not found in invariant_params (when \
               importing [%a: %s])" Set_of_closures_id.print set_of_closures_id
              Export_id.print ex what
        | found -> found
      in
      let recursive =
        match
        ...
        | exception Not_found ->
          if is_classic_mode then
            ...
            Misc.fatal_errorf
              "Set of closures ID %a not found in recursive (when \
               importing [%a: %s])" Set_of_closures_id.print set_of_closures_id
              Export_id.print ex what
        | found -> found
      in
      Some
        (A.create_value_set_of_closures ~function_decls ~bound_vars ~free_vars
           ~invariant_params:(lazy invariant_params) ~recursive:(lazy recursive)
  ...
  | Some ex_info ->
    begin match Export_info.find_description ex_info ex with
    | exception Not_found ->
    ...
    | Value_float_array float_array ->
      ...
      | Unknown_or_mutable -> A.value_mutable_float_array ~size:float_array.size
      | Contents contents ->
        A.value_immutable_float_array
          (Array.map
             (function None -> A.value_any_float | Some f -> A.value_float f)
             contents)
      end
    ...
    | Value_closure
      ...
      let value_set_of_closures =
        import_value_set_of_closures ~set_of_closures_id ~bound_vars ~free_vars
          ~ex_info
    ...
    | Value_set_of_closures
      ...
      let value_set_of_closures =
        import_value_set_of_closures ~set_of_closures_id ~bound_vars ~free_vars
          ~ex_info ~what:"Value_set_of_closures"
      in
      begin match value_set_of_closures with
      | None -> A.value_unresolved (Set_of_closures_id set_of_closures_id)
      | Some value_set_of_closures ->
        let approx = A.value_set_of_closures value_set_of_closures in
        begin match aliased_symbol with
        | None -> approx
...
let import_symbol sym =
  if Compilenv.is_predefined_exception sym then
    ...
    | Some export_info ->
      begin match Symbol.Map.find sym export_info.symbol_id with
      | approx -> A.augment_with_symbol (import_ex approx) sym
      | exception Not_found ->
        Misc.fatal_errorf "Compilation unit = %a Cannot find symbol %a"
          Compilation_unit.print compilation_unit Symbol.print sym
      end
...
and really_import_ex ex = really_import (import_ex ex).descr

and really_import_symbol sym = really_import (import_symbol sym).descr
