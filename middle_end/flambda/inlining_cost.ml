...
let prim_size (prim : Clambda_primitives.primitive) args =
  ...
  | Psequand | Psequor ->
    Misc.fatal_error
      "Psequand and Psequor are not allowed in Prim expressions; translate \
       out instead (cf. closure_conversion.ml)"
...
let lambda_smaller' lam ~than:threshold =
  ...
  and lambda_named_size (named : Flambda.named) =
    ...
    | Set_of_closures { function_decls = ffuns } ->
      Variable.Map.iter
        (fun _ (ffun : Flambda.function_declaration) -> lambda_size ffun.body)
        ffuns.funs
    | Project_closure _ | Project_var _ -> size := !size + project_size
    | Move_within_set_of_closures _ -> incr size
    | Prim (prim, args, _) -> size := !size + prim_size prim args
    | Expr expr -> lambda_size expr
  in
  try
    lambda_size lam;
    if !size <= threshold then Some !size else None
  with
  | Exit -> None
...
module Threshold = struct
  ...
  let sub t1 t2 =
    ...
    | t, Never_inline -> t
    | Can_inline_if_no_larger_than i1, Can_inline_if_no_larger_than i2 ->
      if i1 > i2 then Can_inline_if_no_larger_than (i1 - i2) else Never_inline
  ...
  let equal t1 t2 =
    ...
    | Can_inline_if_no_larger_than i1, Can_inline_if_no_larger_than i2 ->
      i1 = i2
    | (Never_inline | Can_inline_if_no_larger_than _), _ -> false
end
  

let can_try_inlining
    lam inlining_threshold ~number_of_arguments ~size_from_approximation
=
  ...
  | Threshold.Can_inline_if_no_larger_than inlining_threshold ->
    ...
    | Some size ->
      Threshold.Can_inline_if_no_larger_than (inlining_threshold - size + bonus)
    end
...
let can_inline lam inlining_threshold ~bonus =
  ...
  | Threshold.Can_inline_if_no_larger_than inlining_threshold ->
    lambda_smaller lam ~than:(inlining_threshold + bonus)
...
module Benefit = struct
  type t =
    {
      ...
      remove_alloc : int;
      remove_prim : int;
      remove_branch : int; (* CR-someday pchambart: branch_benefit : t list; *)
  ...
  let print ppf b =
    Format.fprintf ppf
      "@[remove_call: %i@ remove_alloc: %i@ remove_prim: %i@ remove_branch: \
       %i@ direct: %i@ requested: %i@]" b.remove_call b.remove_alloc
      b.remove_prim b.remove_branch b.direct_call_of_indirect b.requested_inline
  ...
  let add_code lam b = b - remove_code lam zero
  let add_code_named lam b = b - remove_code_named lam zero
  let add_projection proj b = b - remove_projection proj zero
  
  (* Print out a benefit as a table *)
  let benefit_table =
    [ "Calls", (fun b -> b.remove_call); "Allocs", (fun b -> b.remove_alloc);
      "Prims", (fun b -> b.remove_prim); "Branches", (fun b -> b.remove_branch);
  ...
  let table_line =
    lazy
      (let benefits_table = Lazy.force benefits_table in
       let dashes = List.map (fun (_, _, n) -> String.make n '-') benefits_table
       in
  ...
  let print_table ppf b =
    ...
    Format.fprintf ppf
      "@[<v>@[<h>%s@]@;@[<h>%s@]@;@[<h>%s@]@;@[<h>%a@]@;@[<h>%s@]@]" table_line
      table_headers table_line print_table_values b table_line
end
...
module Whether_sufficient_benefit = struct
  ...
  let create_estimate
      ~original_size ~toplevel ~branch_depth ~new_size ~benefit ~lifting ~round
  =
    let evaluated_benefit = Benefit.evaluate benefit ~round in
    {
      round;
  ...
  let correct_branch_factor f = not (is_nan f) && Float.compare f 0. >= 0
  
  let estimated_benefit t =
    if t.toplevel && t.lifting && t.branch_depth = 0 then
      let lifting_benefit =
  ...
  let evaluate t =
    Float.compare (float t.new_size -. estimated_benefit t)
      (float t.original_size) <=
      0
  ...
  let to_string t =
    ...
    Printf.sprintf
      "{benefit%s{call=%d,alloc=%d,prim=%i,branch=%i,indirect=%i,req=%i,lifting=%B}, \
       orig_size=%d,new_size=%d,eval_size=%d,eval_benefit%s%d,branch_depth=%d}=%s"
      estimate t.benefit.remove_call t.benefit.remove_alloc
      t.benefit.remove_prim t.benefit.remove_branch
      t.benefit.direct_call_of_indirect t.benefit.requested_inline lifting
      t.original_size t.new_size (t.original_size - t.new_size) estimate
      evaluated_benefit t.branch_depth (if evaluate t then "yes" else "no")
  
  let print_description ~subfunctions ppf t =
    let pr_intro ppf =
      let estimate = if t.estimate then " at most" else "" in
      Format.pp_print_text ppf "Specialisation of the function body";
    ...
    let pr_requested ppf =
      if requested > 0 then
        (Format.pp_open_box ppf 0;
         Format.pp_print_text ppf "and inlined user-annotated functions worth ";
    ...
    in
    Format.fprintf ppf "%t@,@[<v>@[<v 2>@;%a@]@;@;%t%t@]%t" pr_intro
      Benefit.print_table t.benefit pr_requested pr_lifting pr_conclusion
end
