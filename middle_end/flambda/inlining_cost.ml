...
let lambda_smaller lam ~than =
  match lambda_smaller' lam ~than with Some _ -> true | None -> false
...
module Benefit = struct
  ...
  let remove_code_helper b (flam : Flambda.t) =
    ...
    | Switch _ | String_switch _ | Static_raise _ | Try_with _ | If_then_else _
    | While _ | For _ ->
        b := remove_branch !b
  ...
  let remove_code_helper_named b (named : Flambda.named) =
    ...
    | Prim _ | Project_closure _ | Project_var _ | Move_within_set_of_closures _
    | Read_symbol_field _ ->
        b := remove_prim !b
  ...
  let print ppf b =
    Format.fprintf ppf
      "@[remove_call: %i@ remove_alloc: %i@ remove_prim: %i@ remove_branch: \
       %i@ direct: %i@ requested: %i@]"
      b.remove_call b.remove_alloc b.remove_prim b.remove_branch
      b.direct_call_of_indirect b.requested_inline
...
module Whether_sufficient_benefit = struct
  ...
  let create_estimate ~original_size ~toplevel ~branch_depth ~new_size ~benefit
      ~lifting ~round =
    let evaluated_benefit = Benefit.evaluate benefit ~round in
    {
      round;
    ...
    if t.toplevel && t.lifting && t.branch_depth = 0 then
      ...
      let branch_taken_estimated_probability =
        let inline_branch_factor =
          ...
          in
          if is_nan factor then Clflags.default_inline_branch_factor
          else if Float.compare factor 0. < 0 then 0.
          else factor
        in
  ...
  let to_string t =
    ...
    Printf.sprintf
      ...
      (t.original_size - t.new_size)
      estimate evaluated_benefit t.branch_depth
