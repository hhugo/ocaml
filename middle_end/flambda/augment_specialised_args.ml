  ...
  include Identifiable.Make (struct
    ...
    let print ppf t =
      ...
      | Existing_inner_free_var var ->
          Format.fprintf ppf "Existing_inner_free_var %a" Variable.print var
...
module What_to_specialise = struct
  ...
  let new_specialised_arg t ~fun_var ~group ~definition =
    ...
    in
    let definitions =
      Variable.Pair.Map.add (fun_var, group)
  ...
  let make_direct_call_surrogate_for t ~fun_var =
    ...
    | exception Not_found ->
        Misc.fatal_errorf
          "use_direct_call_surrogate_for: %a is not a fun_var from the given \
           set of closures"
...
module Processed_what_to_specialise = struct
  ...
  let lift_projection t ~(projection : Projection.t) =
    ...
    let find_outer_var inner_var =
      ...
      | exception Not_found ->
          Misc.fatal_errorf
            "find_outer_var: expected %a to be in [specialised_args], but it \
             is not.  The projection was: %a.  Set of closures: %a"
            Variable.print inner_var Projection.print projection
    ...
    let new_outer_var, t =
      ...
      | None -> (
          ...
          | Existing_inner_free_var existing_inner_var -> (
              match
                Variable.Map.find existing_inner_var t.set_of_closures.free_vars
              with
              | exception Not_found ->
                  Misc.fatal_errorf
                    "really_add_new_specialised_arg: Existing_inner_free_var \
                     %a is not an inner free variable of %a in %a"
          ...
          | Projection_from_existing_specialised_arg projection ->
              ...
              let new_outer_vars_indexed_by_new_lifted_defns =
                Projection.Map.add projection new_outer_var
                  t.new_outer_vars_indexed_by_new_lifted_defns
              in
              let new_lifted_defns_indexed_by_new_outer_vars =
                Variable.Map.add new_outer_var projection
    ...
    let for_one_function : for_one_function =
      {
        ...
        all_new_definitions =
          Definition.Set.add definition for_one_function.all_new_definitions;
        new_inner_to_new_outer_vars;
        total_number_of_args = for_one_function.total_number_of_args + 1;
      }
    in
    { t with functions = Variable.Map.add fun_var for_one_function t.functions }
  ...
  let new_specialised_arg t ~fun_var ~group ~definition =
    let for_one_function : for_one_function =
      ...
      | exception Not_found -> (
          match
            Variable.Map.find fun_var t.set_of_closures.function_decls.funs
          with
          ...
          | (function_decl : Flambda.function_declaration) ->
              ...
              let existing_specialised_args =
                Variable.Map.filter
                  (fun inner_var _spec_to -> Variable.Set.mem inner_var params)
  ...
  let create ~env ~(what_to_specialise : W.t) =
    let existing_definitions_via_spec_args_indexed_by_fun_var =
      Variable.Map.map
        (fun (function_decl : Flambda.function_declaration) ->
          if function_decl.stub then Definition.Set.empty
          else
            let params = Parameter.Set.vars function_decl.params in
            Variable.Map.fold
              (fun inner_var (spec_to : Flambda.specialised_to) definitions ->
                if not (Variable.Set.mem inner_var params) then definitions
                else
    ...
    let by_group =
      Variable.Pair.Map.fold
        (fun (fun_var, group) definitions by_group ->
          ...
          Variable.Map.add group
            ...
            by_group)
        what_to_specialise.definitions Variable.Map.empty
    in
    let module Backend = (val E.backend env : Backend_intf.S) in
    Variable.Map.fold
      (fun group fun_vars_and_definitions t ->
        let original_t = t in
        let t =
          (* Try adding all specialised args in the current group. *)
          List.fold_left
            (fun t (fun_var, definitions) ->
              List.fold_left
                (fun t definition ->
                  new_specialised_arg t ~fun_var ~group ~definition)
                t definitions)
            t fun_vars_and_definitions
        in
        let some_function_has_too_many_args =
          Variable.Map.exists
            (fun _ (for_one_function : for_one_function) ->
              for_one_function.total_number_of_args
              > Backend.max_sensible_number_of_arguments)
            t.functions
        in
        if some_function_has_too_many_args then original_t (* drop this group *)
        else t)
      by_group t
end
...
let check_invariants ~pass_name ~(set_of_closures : Flambda.set_of_closures)
  ...
  if !Clflags.flambda_invariant_checks then
    Variable.Map.iter
      (fun fun_var (function_decl : Flambda.function_declaration) ->
        let params = Parameter.Set.vars function_decl.params in
        Variable.Map.iter
          (fun inner_var (outer_var : Flambda.specialised_to) ->
            if Variable.Set.mem inner_var params then (
              assert (
                not
                  (Variable.Set.mem outer_var.var function_decl.free_variables));
              match outer_var.projection with
              | None -> ()
              | Some projection ->
                  let from = Projection.projecting_from projection in
                  if not (Variable.Set.mem from params) then
                    Misc.fatal_errorf
                      "Augment_specialised_args (%s): specialised argument (%a \
                       -> %a) references a projection variable that is not a \
                       specialised argument of the function %a. @ The set of \
                       closures before the transformation was:@  %a. @ The set \
                       of closures after the transformation was:@ %a."
                      pass_name Variable.print inner_var
                      Flambda.print_specialised_to outer_var Variable.print
                      fun_var Flambda.print_set_of_closures
                      original_set_of_closures Flambda.print_set_of_closures
                      set_of_closures))
...
module Make (T : S) = struct
  ...
  let create_wrapper ~(for_one_function : P.for_one_function) ~benefit =
    ...
    let find_wrapper_param param =
      ...
      | exception Not_found ->
          Misc.fatal_errorf
            "find_wrapper_param: expected %a to be in [params_renaming], but \
             it is not."
            Variable.print param
    in
    let new_inner_vars_to_spec_args_bound_in_the_wrapper_renaming =
      Variable.Map.mapi
        (fun new_inner_var _ -> Variable.rename new_inner_var)
        for_one_function.new_definitions_indexed_by_new_inner_vars
    in
    ...
    let wrapper_body, benefit =
      ...
      in
      Variable.Map.fold
        (fun new_inner_var definition (wrapper_body, benefit) ->
          let definition : Definition.t =
    ...
    let rewritten_existing_specialised_args =
      Variable.Map.fold
        (fun inner_var (spec_to : Flambda.specialised_to) result ->
          ...
          let spec_to : Flambda.specialised_to =
            { var = spec_to.var; projection }
          in
          Variable.Map.add inner_var spec_to result)
        for_one_function.existing_specialised_args Variable.Map.empty
    in
    let new_function_decl =
      Flambda.create_function_declaration ~params:wrapper_params
        ~body:wrapper_body ~stub:true ~dbg:Debuginfo.none ~inline:Default_inline
  ...
  let rewrite_function_decl (t : P.t) ~env ~duplicate_function
    ...
    in
    if
      function_decl.stub || num_definitions < 1
    ...
    then None
      ...
      let new_specialised_args =
        Variable.Map.mapi
          (fun new_inner_var (definition : Definition.t) :
            ...
            assert (
              not
                (Variable.Map.mem new_inner_var set_of_closures.specialised_args));
            match
              Variable.Map.find new_inner_var
                for_one_function.new_inner_to_new_outer_vars
            with
            ...
            | new_outer_var -> (
                ...
                | Existing_inner_free_var _ ->
                    { var = new_outer_var; projection = None }
                | Projection_from_existing_specialised_arg projection ->
                    let projecting_from =
                      Projection.projecting_from projection
                    in
                    assert (
                      Variable.Map.mem projecting_from
                        set_of_closures.specialised_args);
                    assert (
                      Variable.Set.mem projecting_from
                        (Parameter.Set.vars function_decl.params));
                    { var = new_outer_var; projection = Some projection }))
          for_one_function.new_definitions_indexed_by_new_inner_vars
      in
      ...
      let all_params =
        let new_params =
          Variable.Set.elements
            (Variable.Map.keys for_one_function.new_inner_to_new_outer_vars)
        in
        let new_params = List.map Parameter.wrap new_params in
      ...
      let rewritten_function_decl =
        Flambda.create_function_declaration ~params:all_params
          ~body:function_decl.body ~stub:function_decl.stub
          ~dbg:function_decl.dbg ~inline:function_decl.inline
          ~specialise:function_decl.specialise
          ~is_a_functor:function_decl.is_a_functor ~closure_origin
      in
  ...
  let add_lifted_projections_around_set_of_closures
      ...
      ~new_lifted_defns_indexed_by_new_outer_vars =
    let body =
      Flambda_utils.name_expr ~name:Internal_variable_names.set_of_closures
        (Set_of_closures set_of_closures)
    in
    Variable.Map.fold
      (fun new_outer_var (projection : Projection.t) (expr, benefit) ->
        ...
        (expr, benefit))
      new_lifted_defns_indexed_by_new_outer_vars (body, benefit)
    ...
    let ( funs,
      ...
      Variable.Map.fold
        ...
        ( Variable.Map.empty,
          ...
          benefit )
    in
    if not done_something then None
    else
      let function_decls =
      ...
      in
      assert (
        Variable.Map.cardinal specialised_args
  ...
  let rewrite_set_of_closures ~env ~duplicate_function ~set_of_closures =
    Pass_wrapper.with_dump
      ~ppf_dump:(Inline_and_simplify_aux.Env.ppf_dump env)
      ~pass_name:T.pass_name ~input:set_of_closures
      ~print_input:Flambda.print_set_of_closures
      ~print_output:(fun ppf (expr, _) -> Flambda.print ppf expr)
      ~f:(fun () ->
        rewrite_set_of_closures_core ~env ~duplicate_function ~benefit:B.zero
          ~set_of_closures)
end
