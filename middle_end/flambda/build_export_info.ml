...
module Env : sig
  ...
  module Global : sig (* "Global" as in "without local variable bindings". *)
    type t
  ...
  module Global = struct
    ...
    let add_symbol t sym export_id =
      (if Symbol.Map.mem sym t.sym then
         Misc.fatal_errorf
           "Build_export_info.Env.Global.add_symbol: cannot rebind symbol %a in \
            environment" Symbol.print sym);
  ...
  let get_id_descr t export_id =
    ...
    with
    | Not_found -> extern_id_descr export_id
  ...
  let record_descr t id (descr : Export_info.descr) =
    (if Export_id.Map.mem id !(t.ex_table) then
       Misc.fatal_errorf
         "Build_export_info.Env.record_descr: cannot rebind export ID %a in \
          environment" Export_id.print id);
  ...
  let new_unit_descr t = new_descr t (Value_int 0)
  
  let add_approx t var approx =
    (if Variable.Map.mem var t.var then
       Misc.fatal_errorf
         "Build_export_info.Env.add_approx: cannot rebind variable %a in \
          environment" Variable.print var);
  ...
  let find_approx t var : Export_info.approx =
    try Variable.Map.find var t.var
    with
...
let descr_of_allocated_constant (c : Allocated_const.t) : Export_info.descr =
  ...
  | Float_array fs ->
    Value_float_array { contents = Unknown_or_mutable; size = List.length fs }
...
let rec approx_of_expr (env : Env.t) (flam : Flambda.t) : Export_info.approx =
  ...
  | Let_mutable { body } -> approx_of_expr env body
  | Let_rec (defs, body) ->
    let env =
      List.fold_left
        (fun env (var, defining_expr) ->
           let approx = descr_of_named env defining_expr in
           Env.add_approx env var approx) env defs
    in
    approx_of_expr env body
  | Apply { func; kind; _ } ->
    ...
    | Direct closure_id' ->
      begin match Env.get_descr env (Env.find_approx env func) with
      | Some (Value_closure { closure_id; set_of_closures = { results; _ } }) ->
...
and descr_of_named (env : Env.t) (named : Flambda.named) : Export_info.approx =
  match named with
  ...
  | Const const -> Value_id (Env.new_descr env (descr_of_constant const))
  ...
  | Project_closure { set_of_closures; closure_id } ->
    ...
    | Some (Value_set_of_closures set_of_closures) ->
      (if not (Closure_id.Map.mem closure_id set_of_closures.results) then
         Misc.fatal_errorf
           "Could not build export description for [Project_closure]: \
            closure ID %a not in set of closures" Closure_id.print closure_id);
      Value_id (Env.new_value_closure_descr env ~closure_id ~set_of_closures)
    | _ ->
  ...
  | Project_var { closure; closure_id = closure_id'; var } ->
    begin match Env.get_descr env (Env.find_approx env closure) with
    | Some (Value_closure { set_of_closures = { bound_vars; _ }; closure_id })
      ->
      assert (Closure_id.equal closure_id closure_id');
      (if not (Var_within_closure.Map.mem var bound_vars) then
         Misc.fatal_errorf
           "Project_var from %a (closure ID %a) of variable %a that is not \
            bound by the closure.  Variables bound by the closure are: %a"
           Variable.print closure Closure_id.print closure_id
...
=
  let bound_vars_approx =
    Variable.Map.map
      (fun (external_var : Flambda.specialised_to) ->
         Env.find_approx env external_var.var) set.free_vars
  in
  let specialised_args_approx =
    Variable.Map.map
      (fun (spec_to : Flambda.specialised_to) -> Env.find_approx env spec_to.var)
...
let approx_of_constant_defining_value_block_field
    env (c : Flambda.constant_defining_value_block_field)
  :
  Export_info.approx
=
  match c with
  | Symbol s ->
    if Env.is_symbol_being_defined env s then Value_unknown else Value_symbol s
  | Const c -> Value_id (Env.new_descr env (descr_of_constant c))

let describe_constant_defining_value
    env export_id symbol ~symbols_being_defined
  ...
  | Project_closure (sym, closure_id) ->
    ...
    | Some (Value_set_of_closures set_of_closures) ->
      (if not (Closure_id.Map.mem closure_id set_of_closures.results) then
         Misc.fatal_errorf
           "Could not build export description for [Project_closure] \
            constant defining value: closure ID %a not in set of closures"
           Closure_id.print closure_id);
      let descr = Export_info.Value_closure { closure_id; set_of_closures } in
      Env.record_descr env export_id descr
    | None ->
      Misc.fatal_errorf
        "Cannot project symbol %a to closure_id %a.  No available export \
         description@." Symbol.print sym Closure_id.print closure_id
    | Some (Value_closure _) ->
      Misc.fatal_errorf
        "Cannot project symbol %a to closure_id %a.  The symbol is a \
         closure instead of a set of closures.@." Symbol.print sym
        Closure_id.print closure_id
    | Some _ ->
      Misc.fatal_errorf
        "Cannot project symbol %a to closure_id %a.  The symbol is not a \
         set of closures.@." Symbol.print sym Closure_id.print closure_id
    end
...
let describe_program (env : Env.Global.t) (program : Flambda.program) =
  let rec loop env (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (defs, program) ->
      let (env, defs) =
        List.fold_left
          (fun (env, defs) (symbol, def) ->
             let (id, env) = Env.Global.new_symbol env symbol in
             env, (id, symbol, def) :: defs) (env, []) defs
      in
      ...
      let (project_closures, other_constants) =
        List.partition
          (function _, _, Flambda.Project_closure _ -> true | _ -> false) defs
      in
      ...
      List.iter
        (fun (id, symbol, def) ->
           describe_constant_defining_value env id symbol ~symbols_being_defined
             def) other_constants;
      List.iter
        (fun (id, symbol, def) ->
           describe_constant_defining_value env id symbol ~symbols_being_defined
             def) project_closures;
      loop env program
...
=
  ...
  (* CR-soon pchambart: Should probably use that instead of the ident of
    ...
    in
    let sets_of_closures_map = Flambda_utils.all_sets_of_closures_map program in
    let function_declarations_map =
      let set_of_closures_approx { Flambda.function_decls; _ } =
        ...
        in
        Simple_value_approx.function_declarations_approx ~keep_body
          function_decls
      in
      Set_of_closures_id.Map.map set_of_closures_approx sets_of_closures_map
    in
    let unnested_values = Env.Global.export_id_to_descr_map env in
        ...
        Set_of_closures_id.Map.map
          (fun { Flambda.function_decls; _ } ->
             if function_decls.is_classic_mode then
               ...
               Invariant_params.invariant_params_in_recursion ~backend
                 function_decls)
      ...
      Export_id.Map.fold
        (fun _eid (descr : Export_info.descr) invariant_params ->
           ...
           | Value_set_of_closures set_of_closures
             ...
             with
             | exception Not_found -> invariant_params
             | (set : Variable.Set.t Variable.Map.t) ->
               Set_of_closures_id.Map.add set_of_closures_id set
                 invariant_params
             end
           ...
           | Value_unknown_descr
             ->
             invariant_params) unnested_values invariant_params
    in
    let recursive =
      let recursive =
        Set_of_closures_id.Map.map
      ...
      Export_id.Map.fold
        (fun _eid (descr : Export_info.descr) recursive ->
           ...
           | Value_set_of_closures set_of_closures
             ...
             let { Export_info.set_of_closures_id } = set_of_closures in
             begin match
               Set_of_closures_id.Map.find set_of_closures_id export.recursive
             with
             | exception Not_found -> recursive
             | (set : Variable.Set.t) ->
           ...
           | Value_unknown_descr
             ->
             recursive) unnested_values recursive
    in
    let values = Export_info.nest_eid_map unnested_values in
    let symbol_id = Env.Global.symbol_to_export_id_map env in
    let
      {
        Traverse_for_exported_symbols.set_of_closure_ids =
          relevant_set_of_closures; symbols = relevant_symbols;
        export_ids = relevant_export_ids;
        set_of_closure_ids_keep_declaration =
          relevant_set_of_closures_declaration_only; relevant_local_closure_ids;
        relevant_imported_closure_ids; relevant_local_vars_within_closure;
          ...
          (fun
           ...
           ->
             Variable.Map.fold
               ...
               function_declarations.funs acc) function_declarations_map
          Closure_id.Map.empty
      in
      Traverse_for_exported_symbols.traverse ~sets_of_closures_map
        ~closure_id_to_set_of_closures_id ~function_declarations_map
        ~values:(Compilation_unit.Map.find (Compilenv.current_unit ()) values)
        ~symbol_id ~root_symbol:(Compilenv.current_unit_symbol ())
    in
    ...
    let values =
      Compilation_unit.Map.map
        (fun map ->
           Export_id.Map.filter
             (fun key _ -> Export_id.Set.mem key relevant_export_ids) map)
        values
    in
    let symbol_id =
      Symbol.Map.filter (fun key _ -> Symbol.Set.mem key relevant_symbols)
        symbol_id
    in
    Export_info.create_transient ~values ~symbol_id ~sets_of_closures
      ~invariant_params ~recursive ~relevant_local_closure_ids
      ~relevant_imported_closure_ids ~relevant_local_vars_within_closure
      ~relevant_imported_vars_within_closure
