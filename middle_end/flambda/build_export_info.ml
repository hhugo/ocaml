...
module Env : sig
  ...
  let get_symbol_descr t sym =
    ...
    with Not_found -> extern_symbol_descr sym
  ...
  let add_approx_map t vars_to_approxs =
    Variable.Map.fold
      (fun var approx t -> add_approx t var approx)
      vars_to_approxs t
  ...
  let find_approx t var : Export_info.approx =
    try Variable.Map.find var t.var with Not_found -> Value_unknown
...
let descr_of_constant (c : Flambda.const) : Export_info.descr =
  match c with Int i -> Value_int i | Char c -> Value_char c
...
let rec approx_of_expr (env : Env.t) (flam : Flambda.t) : Export_info.approx =
  ...
  | Apply { func; kind; _ } -> (
      ...
      | Direct closure_id' -> (
          ...
          | Some
              (Value_closure { closure_id; set_of_closures = { results; _ } })
            ->
  ...
  | Static_raise _ | Static_catch _ | Try_with _ | If_then_else _ | Switch _
  | String_switch _ | Send _ | Proved_unreachable ->
      Value_unknown
...
and descr_of_named (env : Env.t) (named : Flambda.named) : Export_info.approx =
  ...
  | Read_symbol_field (sym, i) -> (
      match Env.get_symbol_descr env sym with
  ...
  | Prim (Pfield i, [ arg ], _) -> (
      match Env.get_descr env (Env.find_approx env arg) with
      | Some (Value_block (_, fields)) when Array.length fields > i ->
          fields.(i)
      | _ -> Value_unknown)
  ...
  | Project_closure { set_of_closures; closure_id } -> (
      match Env.get_descr env (Env.find_approx env set_of_closures) with
  ...
  | Move_within_set_of_closures { closure; start_from; move_to } -> (
      match Env.get_descr env (Env.find_approx env closure) with
      | Some (Value_closure { set_of_closures; closure_id }) ->
          assert (Closure_id.equal closure_id start_from);
          Value_id
            (Env.new_value_closure_descr env ~closure_id:move_to
               ~set_of_closures)
      | _ -> Value_unknown)
        ...
        ->
          ...
          if not (Var_within_closure.Map.mem var bound_vars) then
            Misc.fatal_errorf
              ...
              (Var_within_closure.Map.print (fun _ _ -> ()))
              bound_vars;
    ...
    let initial_value_set_of_closures =
      {
        ...
        results =
          Closure_id.wrap_map
            (Variable.Map.map
               (fun _ -> Export_info.Value_unknown)
...
let approx_of_constant_defining_value_block_field env
  ...
  match c with
  | Symbol s ->
      if Env.is_symbol_being_defined env s then Value_unknown
      else Value_symbol s
  | Const c -> Value_id (Env.new_descr env (descr_of_constant c))

let describe_constant_defining_value env export_id symbol ~symbols_being_defined
      ...
      | None ->
          Misc.fatal_errorf
            "Cannot project symbol %a to closure_id %a.  No available export \
             description@."
            Symbol.print sym Closure_id.print closure_id
      | Some (Value_closure _) ->
          Misc.fatal_errorf
            "Cannot project symbol %a to closure_id %a.  The symbol is a \
             closure instead of a set of closures.@."
            Symbol.print sym Closure_id.print closure_id
      | Some _ ->
          Misc.fatal_errorf
  ...
  let rec loop env (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (defs, program) ->
        ...
        let project_closures, other_constants =
          List.partition
            (function _, _, Flambda.Project_closure _ -> true | _ -> false)
            defs
        in
  ...
  if !Clflags.opaque then
    ...
    let invariant_params =
      let invariant_params =
        Set_of_closures_id.Map.map
          (fun { Flambda.function_decls; _ } ->
            if function_decls.is_classic_mode then Variable.Map.empty
            else
      ...
      Export_id.Map.fold
        (fun _eid (descr : Export_info.descr) invariant_params ->
          ...
          | Value_block _ | Value_mutable_block _ | Value_int _ | Value_char _
          | Value_float _ | Value_float_array _ | Value_string _
          | Value_unknown_descr ->
              invariant_params)
    ...
    let recursive =
      let recursive =
        Set_of_closures_id.Map.map
          (fun { Flambda.function_decls; _ } ->
            if function_decls.is_classic_mode then Variable.Set.empty
            else
        ...
        Set_of_closures_id.Map.fold
          (fun set_of_closure_id
               (function_declarations :
                 Simple_value_approx.function_declarations) acc ->
            Variable.Map.fold
              (fun fun_var _ acc ->
                let closure_id = Closure_id.wrap fun_var in
                Closure_id.Map.add closure_id set_of_closure_id acc)
              function_declarations.funs acc)
          function_declarations_map Closure_id.Map.empty
      in
      ...
      Compilation_unit.Map.map
        (fun map ->
          Export_id.Map.filter
            (fun key _ -> Export_id.Set.mem key relevant_export_ids)
            map)
        values
    in
    let symbol_id =
