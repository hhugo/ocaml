  ...
  Projection.Set.filter
    (fun projection ->
       ...
       | Project_var project_var ->
         ...
         | Ok (_value_closure, _approx_var, _approx_sym, value_set_of_closures)
           ->
       ...
       | Move_within_set_of_closures move ->
         begin match A.check_approx_for_closure approx with
         | Ok (value_closure, _approx_var, _approx_sym, _value_set_of_closures)
           ->
...
let rec analyse_expr ~which_variables expr =
  ...
  let check_free_variable var =
    if Variable.Map.mem var which_variables then
      used_which_variables := Variable.Set.add var !used_which_variables
  in
  let for_expr (expr : Flambda.expr) =
    match expr with
    | Var var | Let_mutable { initial_value = var } -> check_free_variable var
    ...
    | Assign { new_value; _ } -> check_free_variable new_value
    | If_then_else (var, _, _) | Switch (var, _) | String_switch (var, _, _) ->
      check_free_variable var
    | Static_raise (_, args) -> List.iter check_free_variable args
  ...
  let for_named (named : Flambda.named) =
    ...
    | Project_var project_var
      ...
      projections := Projection.Set.add (Project_var project_var) !projections
    | Project_closure project_closure
      when Variable.Map.mem project_closure.set_of_closures which_variables ->
      projections :=
    ...
    | Prim (Pfield field_index, [ var ], _dbg)
      ...
      projections := Projection.Set.add (Field (field_index, var)) !projections
    | Set_of_closures set_of_closures ->
      ...
      in
      let aliasing_vars =
        Variable.Map.disjoint_union aliasing_free_vars aliasing_specialised_args
      in
      if not (Variable.Map.is_empty aliasing_vars) then
        Variable.Map.iter
          (fun _ (fun_decl : Flambda.function_declaration) ->
             ...
             Variable.Set.iter
               (fun var ->
                  ...
                  | exception Not_found -> assert false
                  | spec_to -> check_free_variable spec_to.var) used)
          set_of_closures.function_decls.funs
    | Prim (_, vars, _) -> List.iter check_free_variable vars
    | Symbol _
    | Const _
    | Allocated_const _
    | Read_mutable _
...
let from_function_decl
    ~env ~which_variables ~(function_decl:Flambda.function_declaration)
=
  ...
  (* We must use approximation information to determine which projections
     ...
     expressions too far. *)
  let projections = known_valid_projections ~env ~projections ~which_variables
  in
  ...
  Projection.Set.filter
    (fun projection ->
       ...
       not (Variable.Set.mem projecting_from used_which_variables)) projections
