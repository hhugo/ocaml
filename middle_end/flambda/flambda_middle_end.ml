    ...
    ~f:(fun ~constant:_ (set_of_closures : Flambda.set_of_closures) ->
      Variable.Map.iter
        (fun fun_var (function_decl : Flambda.function_declaration) ->
           ...
           | None -> assert false) set_of_closures.function_decls.funs
    )

let lambda_to_flambda
    ~ppf_dump ~prefixname ~backend ~size ~module_ident ~module_initializer
=
  Profile.record_call "flambda"
    (fun () ->
       ...
       Misc.protect_refs
         ...
         (fun () ->
            ...
            let (+-+) flam (name, pass) =
              ...
              flam
            in
            Profile.record_call ~accumulate:true "middle_end"
              (fun () ->
                 let flam =
                 ...
                 let fast_mode flam =
                   pass_number := 0;
                   let round = 0 in
                   flam +-+ ("lift_lets 1", Lift_code.lift_lets) +-+
                 ...
                 Flambda_iterators.iter_apply_on_program flam
                   ~f:(fun apply ->
...
let flambda_raw_clambda_dump_if
    ppf
      ...
      Symbol.Map.iter
        (fun sym cst ->
           Format.fprintf ppf "%a:@ %a@." Symbol.print sym
             Printclambda.structured_constant cst) structured_constants));
  (if !Clflags.dump_cmm then Format.fprintf ppf "@.cmm:@.");
  input

let lambda_to_clambda
    ~backend ~prefixname ~ppf_dump (program : Lambda.program)
=
  let program =
  ...
  let (clambda, preallocated_blocks, constants) =
    Profile.record_call "backend"
      (fun () ->
         (program, export) |> Flambda_to_clambda.convert ~ppf_dump |>
           flambda_raw_clambda_dump_if ppf_dump |>
           fun
             { Flambda_to_clambda.expr; preallocated_blocks;
               structured_constants; exported }
           ->
             Compilenv.set_export_info exported;
             let clambda =
               Un_anf.apply ~what:(Compilenv.current_unit_symbol ()) ~ppf_dump
                 expr
             in
