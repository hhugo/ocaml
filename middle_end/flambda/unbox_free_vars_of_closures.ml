...
=
  ...
  Variable.Map.fold
    (fun new_inner_var (projection : Projection.t) (expr, benefit) ->
         ...
         | exception Not_found ->
           Misc.fatal_errorf
             "(UFV) find_outer_var: expected %a to be in \
              ...
              was: %a)" Variable.print inner_var Projection.print projection
       in
       ...
       in
       let expr = Flambda.create_let (find_outer_var new_inner_var) named expr
       in
       expr, benefit) definitions_indexed_by_new_inner_vars (body, benefit)
  ...
  if not !Clflags.unbox_free_vars_of_closures then
    ...
    let (definitions_indexed_by_new_inner_vars, _, free_vars, done_something) =
      let all_existing_definitions =
        Variable.Map.fold
          (fun
             _inner_var (outer_var : Flambda.specialised_to)
    ...
    (* CR-someday mshinwell: could consider doing the grouping thing
      ...
      let num_free_vars_before = Variable.Map.cardinal set_of_closures.free_vars
      in
