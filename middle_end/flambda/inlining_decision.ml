...
let inline
    env r ~lhs_of_application ~closure_id_being_applied
    ~(function_decl:A.function_declaration) ~(function_body:A.function_body)
    ...
    ~(specialise_requested:Lambda.specialise_attribute) ~fun_vars
...
=
  ...
  let (unrolling, always_inline, never_inline, env) =
    ...
    | None ->
      ...
      | Unroll count ->
        if count > 0 then
          let env =
            E.start_actively_unrolling env set_of_closures_origin (count - 1)
          in
          true, true, false, env
        else
          false, false, true, env
  ...
  let remaining_inlining_threshold : Inlining_cost.Threshold.t =
    if always_inline then inlining_threshold else Lazy.force fun_cost
  in
  let try_inlining =
    if unrolling then
      Try_it
      ...
      not (E.unrolling_allowed env set_of_closures_origin) &&
            ...
            Variable.Set.fold
              (fun v acc ->
                 try
                   ...
                   | None -> acc
                 with
                 | Not_found -> acc) function_body.free_variables benefit
          in
          W.create_estimate ~original_size:Inlining_cost.direct_call_size
            ~new_size:body_size ~toplevel:(E.at_toplevel env)
            ~branch_depth:(E.branch_depth env)
            ~lifting:function_body.A.is_a_functor ~round:(E.round env) ~benefit
        in
        if not (W.evaluate wsb) then
          Don't_try_it (S.Not_inlined.Without_subfunctions wsb)
        else
          Try_it
      | None ->
    ...
    else (* There are useful approximations, so we should simplify. *)
      Try_it
  in
  ...
  | Try_it ->
    ...
    let (body, r_inlined) =
      ...
      Inlining_transforms.inline_by_copying_function_body ~env
        ~r:(R.reset_benefit r) ~lhs_of_application ~closure_id_being_applied
        ~specialise_requested ~inline_requested ~function_decl ~function_body
    ...
    let keep_inlined_version decision =
      ...
      let r_inlined =
        ...
        if always_inline then
          ...
          r_inlined
      in
      let r = R.map_benefit r_inlined (Inlining_cost.Benefit.(+) (R.benefit r))
      in
    ...
    if always_inline then
      ...
      let wsb =
        W.create ~original body ~toplevel:(E.at_toplevel env)
          ~branch_depth:(E.branch_depth env) ~lifting:function_body.is_a_functor
          ~round:(E.round env) ~benefit:(R.benefit r_inlined)
      in
      if W.evaluate wsb then
        keep_inlined_version (S.Inlined.Without_subfunctions wsb)
      else if num_direct_applications_seen < 1 then
        ...
        let wsb_with_subfunctions =
          W.create ~original body ~toplevel:(E.at_toplevel env)
            ~branch_depth:(E.branch_depth env)
            ~lifting:function_body.is_a_functor ~round:(E.round env)
            ~benefit:(R.benefit r_inlined)
        in
        if W.evaluate wsb_with_subfunctions then
          let res =
            body,
...
let specialise
    env r ~lhs_of_application ~(function_decls:A.function_declarations)
    ~(function_decl:A.function_declaration) ~closure_id_being_applied
    ~(value_set_of_closures:A.value_set_of_closures) ~args ~args_approxs ~dbg
...
=
  ...
  let (always_specialise, never_specialise) =
    ...
    | Default_specialise ->
      begin match function_decl.function_body with
      | None -> false, true
      | Some { specialise } ->
        begin match (specialise : Lambda.specialise_attribute) with
        | Always_specialise -> true, false
        | Never_specialise -> false, true
  ...
  in
  let remaining_inlining_threshold : Inlining_cost.Threshold.t =
    if always_specialise then inlining_threshold else Lazy.force fun_cost
  in
  ...
  | Try_it ->
    ...
    let copied_function_declaration =
      Inlining_transforms.inline_by_copying_function_declaration ~env
        ~r:(R.reset_benefit r) ~lhs_of_application ~function_decls
    ...
    in
    begin match copied_function_declaration with
    | Some (expr, r_inlined) ->
      let wsb =
        W.create ~original expr ~toplevel:false
          ~branch_depth:(E.branch_depth env) ~lifting:false ~round:(E.round env)
          ~benefit:(R.benefit r_inlined)
      in
      let env =
      ...
      in
      if always_specialise || W.evaluate wsb then
        let r_inlined =
        ...
        let decision =
          if always_specialise then
            S.Specialised.Annotation
          else
            S.Specialised.Without_subfunctions wsb
        in
        Changed (res, decision)
      else
        let closure_env =
        ...
        let wsb_with_subfunctions =
          W.create ~original expr ~toplevel:false
            ~branch_depth:(E.branch_depth env) ~lifting:false
            ~round:(E.round env) ~benefit:(R.benefit r_inlined)
        in
        if W.evaluate wsb_with_subfunctions then
          let r =
...
let for_call_site
    ~env ~r ~(function_decls:A.function_declarations) ~lhs_of_application
    ~closure_id_being_applied ~(function_decl:A.function_declaration)
    ~(value_set_of_closures:A.value_set_of_closures) ~args ~args_approxs ~dbg
    ~simplify ~inline_requested ~specialise_requested
=
  (if List.length args <> List.length args_approxs then
     Misc.fatal_error
       "Inlining_decision.for_call_site: inconsistent lengths of [args] and \
        [args_approxs]");
  ...
  | Some { stub; _ } ->
    if stub then
      ...
      let (body, r) =
        Inlining_transforms.inline_by_copying_function_body ~env ~r ~fun_vars
          ~lhs_of_application ~closure_id_being_applied ~specialise_requested
          ~inline_requested ~function_decl ~function_body ~args ~dbg ~simplify
      in
      simplify env r body
      ...
      original, original_r
    else if function_decls.is_classic_mode then
      let env =
        E.note_entering_call env ~closure_id:closure_id_being_applied ~dbg
      in
      let simpl =
        match function_decl.function_body with
        ...
        | Some function_body ->
          ...
          let try_inlining =
            if self_call then
              ...
              not (E.inlining_allowed env function_decl.closure_origin)
            then
              Don't_try_it S.Not_inlined.Unrolling_depth_exceeded
            else
              Try_it
          ...
          | Try_it ->
            ...
            let (body, r) =
              Inlining_transforms.inline_by_copying_function_body ~env ~r
                ~function_body ~lhs_of_application ~closure_id_being_applied
                ~specialise_requested ~inline_requested ~function_decl ~fun_vars
                ~args ~dbg ~simplify
            in
            let env = E.note_entering_inlined env in
            let env =
            ...
            in
            let env = E.inside_inlined_function env function_decl.closure_origin
            in
      ...
      let (res, decision) =
        ...
        | Changed ((expr, r), decision) ->
          ...
          in
          let res =
            if E.inlining_level env = 0 then
              expr, R.set_inlining_threshold r raw_inlining_threshold
            else
              expr, R.add_inlining_threshold r inlining_threshold_diff
          in
      ...
      let simpl =
        if inlining_prevented then
          ...
          let specialise_result =
            specialise env r ~function_decls ~function_decl ~lhs_of_application
              ~recursive ~closure_id_being_applied ~value_set_of_closures ~args
              ~args_approxs ~dbg ~simplify ~original ~inline_requested
              ...
              | exception Not_found ->
                Misc.fatal_errorf
                  "Approximation does not give a size for the function \
                   having fun_var %a.  value_set_of_closures: %a" Variable.print
                  fun_var A.print_value_set_of_closures value_set_of_closures
            in
            let fun_vars = Variable.Map.keys function_decls.funs in
            let set_of_closures_origin = function_decls.set_of_closures_origin
            in
            let inline_result =
              inline env r ~lhs_of_application ~closure_id_being_applied
                ~function_decl ~value_set_of_closures ~only_use_of_function
                ~original ~recursive ~inline_requested ~specialise_requested
                ~fun_vars ~set_of_closures_origin ~args ~size_from_approximation
                ~dbg ~simplify ~fun_cost ~self_call ~inlining_threshold
                ~function_body
            in
            begin match inline_result with
