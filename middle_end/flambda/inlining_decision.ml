...
let inline env r ~lhs_of_application ~closure_id_being_applied
  ...
  in
  let try_inlining =
    if unrolling then Try_it
    else if self_call then Don't_try_it S.Not_inlined.Self_call
    else if not (E.inlining_allowed env function_decl.closure_origin) then
      Don't_try_it S.Not_inlined.Unrolling_depth_exceeded
    else if only_use_of_function || always_inline then Try_it
    else if never_inline then Don't_try_it S.Not_inlined.Annotation
    else if
      (not (E.unrolling_allowed env set_of_closures_origin))
      && Lazy.force recursive
    then Don't_try_it S.Not_inlined.Unrolling_depth_exceeded
      ...
      | Some body_size ->
          let wsb =
            ...
            W.create_estimate ~original_size:Inlining_cost.direct_call_size
              ...
              ~lifting:function_body.A.is_a_functor ~round:(E.round env)
              ~benefit
          in
  ...
  | Try_it ->
      ...
      in
      if always_inline then keep_inlined_version S.Inlined.Annotation
...
let specialise env r ~lhs_of_application
  ...
  let try_specialising =
    ...
    else if self_call then Don't_try_it S.Not_specialised.Self_call
    else if always_specialise && not (Lazy.force has_no_useful_approxes) then
      Try_it
    else if never_specialise then Don't_try_it S.Not_specialised.Annotation
    else if T.equal remaining_inlining_threshold T.Never_inline then
      let threshold =
        match inlining_threshold with
        | T.Never_inline -> assert false
      ...
      if stub then
        ...
        let simpl =
          ...
          | Some function_body -> (
              ...
              in
              let try_inlining =
                if self_call then Don't_try_it S.Not_inlined.Self_call
                else if
              ...
              | Try_it ->
                  ...
                  let body, r =
                    Inlining_transforms.inline_by_copying_function_body ~env ~r
                      ~function_body ~lhs_of_application
                      ~closure_id_being_applied ~specialise_requested
                      ~inline_requested ~function_decl ~fun_vars ~args ~dbg
                      ~simplify
                  in
                  let env = E.note_entering_inlined env in
                  let env =
        ...
        let simpl =
          if inlining_prevented then
            ...
            let fun_cost =
              lazy
                (Inlining_cost.can_try_inlining function_body.body
                   inlining_threshold
                   ~number_of_arguments:
                     (List.length function_decl.params)
            ...
            let specialise_result =
              specialise env r ~function_decls ~function_decl
                ...
                let size_from_approximation =
                  ...
                  | exception Not_found ->
                      Misc.fatal_errorf
                        ...
                        Variable.print fun_var A.print_value_set_of_closures
