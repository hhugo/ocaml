...
let rec accumulate
    ~substitution ~copied_lets ~extracted_lets (expr : Flambda.t)
=
  match expr with
  | Let { var; body = Var var'; _ }
  | Let_rec ([ var, _ ], Var var')
  ...
  | Let_rec
      (defs,
       Let
         { var; body = Var var'; defining_expr = Prim (Pmakeblock _, fields, _)
         })
    when
  ...
  | Let_rec ([ var, Expr (Var alias) ], body)
    ...
    in
    accumulate ~substitution:(Variable.Map.add var alias substitution)
      ~copied_lets ~extracted_lets body
  | Let { var; defining_expr = named; body; _ }
  | Let_rec ([ var, named ], body)
    when should_copy named ->
    accumulate body ~substitution ~copied_lets:((var, named) :: copied_lets)
      ~extracted_lets
  | Let { var; defining_expr = named; body; _ } ->
    let extracted =
      ...
      | named ->
        ...
        Expr (var, expr)
    in
    accumulate body ~substitution ~copied_lets
  ...
  | Let_rec (defs, body) ->
    let (renamed_defs, def_substitution) =
      List.fold_right
        (fun (var, def) (acc, substitution) ->
           let new_var = Variable.rename var in
           (new_var, def) :: acc, Variable.Map.add var new_var substitution)
        defs ([], substitution)
    in
    let extracted =
      let expr =
...
let rebuild_expr
    ...
    ~(copied_definitions:Flambda.named Variable.Map.t) ~(substitute:bool)
    (expr : Flambda.t)
=
  ...
  in
  let expr_with_read_symbols =
    Flambda_utils.toplevel_substitution substitution expr_with_read_symbols
  in
  Variable.Map.fold
    (fun var declaration body ->
       let definition = Variable.Map.find var copied_definitions in
       Flambda.create_let declaration definition body) substitution
...
let rebuild (used_variables : Variable.Set.t) (accumulated : accumulated) =
    ...
    List.map
      (fun decl ->
         ...
         | Exprs _ ->
           ...
           Symbol.of_variable var, decl) accumulated.extracted_lets
  in
  let extracted_definitions =
    ...
    List.fold_left
      (fun map (symbol, decl) ->
         match decl with
         | Block (var, _tag, _fields) -> Variable.Map.add var (symbol, []) map
         | Expr (var, _expr) -> Variable.Map.add var (symbol, [ 0 ]) map
         | Exprs (vars, _expr) ->
           let (map, _) =
             List.fold_left
               (fun (map, field) var ->
                  Variable.Map.add var (symbol, [ field; 0 ]) map, field + 1)
               (map, 0) vars
           in
           map) Variable.Map.empty accumulated_extracted_lets
  in
  let extracted =
    List.map
      (fun (symbol, decl) ->
         ...
         | Block (_var, tag, fields) ->
           let fields =
             List.map
               (fun var ->
                  rebuild_expr ~extracted_definitions ~copied_definitions
                    ~substitute:true (Var var)) fields
           in
           Initialisation (symbol, tag, fields)) accumulated_extracted_lets
  in
  let terminator =
    ...
    rebuild_expr ~extracted_definitions ~copied_definitions ~substitute:false
...
let introduce_symbols expr =
  let accumulated =
    accumulate expr ~substitution:Variable.Map.empty ~copied_lets:[]
      ~extracted_lets:[]
  in
...
let lift ~backend:_ (program : Flambda.program) =
  { program with  program_body = split_program program.program_body }
