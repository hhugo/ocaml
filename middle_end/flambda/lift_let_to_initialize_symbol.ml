  ...
  | Let_rec
    ...
    when Variable.equal var var'
         && List.for_all
              (fun field ->
                List.exists
                  (fun (def_var, _) -> Variable.equal def_var field)
                  defs)
              fields ->
      {
        copied_lets;
        ...
        | Prim (Pmakeblock (tag, Asttypes.Immutable, _value_kind), args, _dbg)
          ->
            ...
            let args =
              List.map
                (fun v ->
                  try Variable.Map.find v substitution with Not_found -> v)
                args
            in
            Block (var, tag, args)
        | named ->
...
let rebuild_expr ~(extracted_definitions : (Symbol.t * int list) Variable.Map.t)
  ...
  let substitution =
    ...
    else Variable.Map.of_set (fun x -> x) free_variables
  in
  let expr_with_read_symbols =
...
let add_extracted introduced program =
  List.fold_right
    (fun extracted program ->
      ...
      | Effect effect -> Flambda.Effect (effect, program))
    introduced program
