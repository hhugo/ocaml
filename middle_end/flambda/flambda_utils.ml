...
let equal_direction_flag (x : Asttypes.direction_flag)
...
let rec same (l1 : Flambda.t) (l2 : Flambda.t) =
  ...
  | ( Let { var = var1; defining_expr = defining_expr1; body = body1; _ },
      ...
      && same_named defining_expr1 defining_expr2
      && same body1 body2
  ...
  | ( Send { kind = kind1; meth = meth1; obj = obj1; args = args1; dbg = _ },
    ...
    ->
      ...
      && Variable.equal meth1 meth2 && Variable.equal obj1 obj2
...
let toplevel_substitution sb tree =
  ...
  let aux (flam : Flambda.t) : Flambda.t =
    ...
    | Static_raise (static_exn, args) ->
        let args = List.map sb args in
        Static_raise (static_exn, args)
    | Static_catch _ | Try_with _ | While _ | Let _ | Let_rec _
    | Proved_unreachable ->
        flam
  in
...
let make_closure_declaration ~is_classic_mode ~id ~body ~params ~stub :
  ...
  assert (
    Variable.Set.equal
      (Variable.Set.map subst free_variables)
      function_declaration.free_variables);
  let free_vars =
    Variable.Map.fold
      (fun id id' fv' ->
        let spec_to : Flambda.specialised_to =
          { var = id; projection = None }
        in
...
let bind ~bindings ~body =
  List.fold_left
    (fun expr (var, var_def) -> Flambda.create_let var var_def expr)
    body bindings
...
let all_lifted_constants (program : Flambda.program) =
  let rec loop (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (decls, program) ->
        List.fold_left
          (fun l (symbol, decl) -> (symbol, decl) :: l)
          (loop program) decls
...
let substitute_read_symbol_field_for_variables
  ...
  let f (expr : Flambda.t) : Flambda.t =
    ...
    | Let ({ var = v; defining_expr = named; _ } as let_expr) ->
        ...
        in
        if Variable.Set.is_empty to_substitute then expr
        else
          let bindings =
    ...
    | Let_rec (defs, body) ->
        ...
        if Variable.Set.is_empty to_substitute then expr
          ...
          let defs =
            List.map
              (fun (var, named) -> (var, substitute_named bindings named))
              defs
          in
    ...
    (fun (func_decl : Flambda.function_declaration) ->
      let from_symbols =
        Symbol.Set.fold
          ...
          func_decl.free_symbols Variable.Set.empty
      in
      let from_variables =
  ...
  Variable.Map.map
    (fun (spec_to : Flambda.specialised_to) ->
      ...
      | Some projection ->
          ...
          if Variable.Map.mem from which_variables then spec_to
          else ({ spec_to with projection = None } : Flambda.specialised_to))
    which_variables
...
let parameters_specialised_to_the_same_variable
  ...
  let specialised_arg_aliasing =
    ...
    Variable.Map.transpose_keys_and_data_set
      (Variable.Map.map
         (fun ({ var; _ } : Flambda.specialised_to) -> var)
         specialised_args)
  in
  Variable.Map.map
