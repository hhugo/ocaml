...
let name_expr ~name (named : Flambda.named) : Flambda.t =
  let var =
    Variable.create
      ~current_compilation_unit:(Compilation_unit.get_current_exn ()) name
  in
...
let name_expr_from_var ~var (named : Flambda.named) : Flambda.t =
  let var =
    Variable.rename
      ~current_compilation_unit:(Compilation_unit.get_current_exn ()) var
  in
  Flambda.create_let var named (Var var)
...
let find_declaration_variable cf ({ funs } : Flambda.function_declarations) =
  ...
  if not (Variable.Map.mem var funs) then raise Not_found else var
...
let variables_bound_by_the_closure cf (decls : Flambda.function_declarations) =
...
let equal_direction_flag
    (x : Asttypes.direction_flag) (y : Asttypes.direction_flag)
=
  match x, y with
  | Upto, Upto -> true
  | Downto, Downto -> true
...
let rec same (l1 : Flambda.t) (l2 : Flambda.t) =
  l1 == l2 ||
    (* it is ok for the string case: if they are physically the same,
    ...
    | Let { var = var1; defining_expr = defining_expr1; body = body1; _ },
      ...
      Variable.equal var1 var2 &&
        same_named defining_expr1 defining_expr2 && same body1 body2
    | Let _, _ | _, Let _ -> false
    | Let_mutable
        { var = mv1; initial_value = v1; contents_kind = ck1; body = b1 },
      Let_mutable
        { var = mv2; initial_value = v2; contents_kind = ck2; body = b2 }
      ->
      Mutable_variable.equal mv1 mv2 &&
        Variable.equal v1 v2 && Lambda.equal_value_kind ck1 ck2 && same b1 b2
    ...
    | Static_raise (e1, a1), Static_raise (e2, a2) ->
      Static_exception.equal e1 e2 &&
        Misc.Stdlib.List.equal Variable.equal a1 a2
    | Static_raise _, _ | _, Static_raise _ -> false
    | Static_catch (s1, v1, a1, b1), Static_catch (s2, v2, a2, b2) ->
      Static_exception.equal s1 s2 &&
        Misc.Stdlib.List.equal Variable.equal v1 v2 && same a1 a2 && same b1 b2
    ...
    | While (a1, b1), While (a2, b2) -> same a1 a2 && same b1 b2
    | While _, _ | _, While _ -> false
    | For
        { bound_var = bound_var1; from_value = from_value1; to_value = to_value1;
          direction = direction1; body = body1 },
      For
        { bound_var = bound_var2; from_value = from_value2; to_value = to_value2;
          direction = direction2; body = body2 }
      ->
      Variable.equal bound_var1 bound_var2 &&
        Variable.equal from_value1 from_value2 &&
          Variable.equal to_value1 to_value2 &&
            equal_direction_flag direction1 direction2 && same body1 body2
...
and same_named (named1 : Flambda.named) (named2 : Flambda.named) =
  ...
  | Allocated_const c1, Allocated_const c2 -> Allocated_const.compare c1 c2 = 0
  ...
  | Move_within_set_of_closures _, _ | _, Move_within_set_of_closures _ -> false
...
and sameclosure
    (c1 : Flambda.function_declaration) (c2 : Flambda.function_declaration)
=
  Misc.Stdlib.List.equal Parameter.equal c1.params c2.params &&
    same c1.body c2.body

and same_set_of_closures
    (c1 : Flambda.set_of_closures) (c2 : Flambda.set_of_closures)
=
  Variable.Map.equal sameclosure c1.function_decls.funs c2.function_decls.funs &&
    Variable.Map.equal Flambda.equal_specialised_to c1.free_vars c2.free_vars &&
      Variable.Map.equal Flambda.equal_specialised_to c1.specialised_args
        c2.specialised_args

and same_project_closure
    (s1 : Flambda.project_closure) (s2 : Flambda.project_closure)
=
  Variable.equal s1.set_of_closures s2.set_of_closures &&
    Closure_id.equal s1.closure_id s2.closure_id

and same_move_within_set_of_closures
    (m1 : Flambda.move_within_set_of_closures)
...
and samebinding (v1, n1) (v2, n2) = Variable.equal v1 v2 && same_named n1 n2
...
let toplevel_substitution sb tree =
  ...
  let aux_named (named : Flambda.named) : Flambda.named =
    ...
    | Project_var project_var ->
      Project_var { project_var with  closure = sb project_var.closure }
    | Prim (prim, args, dbg) -> Prim (prim, List.map sb args, dbg)
  in
  if Variable.Map.is_empty sb' then
    tree
  else
...
let make_closure_declaration ~is_classic_mode ~id ~body ~params ~stub :
  Flambda.t
=
  let free_variables = Flambda.free_variables body in
  let param_set = Parameter.Set.vars params in
  (if not (Variable.Set.subset param_set free_variables) then
     Misc.fatal_error "Flambda_utils.make_closure_declaration");
  let sb =
    Variable.Set.fold (fun id sb -> Variable.Map.add id (Variable.rename id) sb)
  ...
  in
  assert
    (Variable.Set.equal (Variable.Set.map subst free_variables)
       function_declaration.free_variables);
  let free_vars =
    Variable.Map.fold
      (fun id id' fv' ->
         let spec_to : Flambda.specialised_to = { var = id; projection = None }
         in
  ...
  let set_of_closures =
    let function_decls =
      Flambda.create_function_declarations ~is_classic_mode
  ...
  let project_closure : Flambda.named =
    Project_closure
      { set_of_closures = set_of_closures_var; closure_id = Closure_id.wrap id }
  in
...
let bind ~bindings ~body =
  List.fold_left
    (fun expr (var, var_def) -> Flambda.create_let var var_def expr) body
    bindings
  ...
  let rec loop (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (decls, program) ->
      List.fold_left (fun l (symbol, decl) -> (symbol, decl) :: l)
        (loop program) decls
    | Initialize_symbol (_, _, _, program) | Effect (_, program) -> loop program
    | End _ -> []
  in
...
let needed_import_symbols (program : Flambda.program) =
  ...
  let defined_symbol =
    Symbol.Set.union
      (Symbol.Set.of_list (List.map fst (all_lifted_constants program)))
      (Symbol.Set.of_list
...
let introduce_needed_import_symbols program : Flambda.program =
  { program with  imported_symbols = needed_import_symbols program }
...
let make_closure_map program =
  ...
  let add_set_of_closures ~constant:_ : Flambda.set_of_closures -> unit =
    fun { function_decls } ->
      Variable.Map.iter
        (fun var _ ->
...
let all_lifted_constant_closures program =
  List.fold_left
    (fun unchanged flambda ->
       match flambda with
       | _, Flambda.Set_of_closures { function_decls = { funs } } ->
         Variable.Map.fold
           (fun key (_ : Flambda.function_declaration) acc ->
              Closure_id.Set.add (Closure_id.wrap key) acc) funs unchanged
       | _ -> unchanged) Closure_id.Set.empty (all_lifted_constants program)
...
let all_lifted_constant_sets_of_closures program =
  ...
  List.iter
    (function
     ...
     | _ -> ()) (all_lifted_constants program);
  !set

let all_sets_of_closures program =
  let list = ref [] in
  Flambda_iterators.iter_on_set_of_closures_of_program program
    ~f:(fun ~constant:_ set_of_closures -> list := set_of_closures :: !list
    );
  !list
    ...
    ~f:(fun ~constant:_ set_of_closures ->
      r :=
        Set_of_closures_id.Map.add
          set_of_closures.function_decls.set_of_closures_id set_of_closures !r
    );
  !r

let substitute_read_symbol_field_for_variables
    (substitution : (Symbol.t * int list) Variable.Map.t) (expr : Flambda.t)
=
        ...
        Expr
          (Flambda.create_let block (make_named t)
             (Flambda.create_let field
                (Prim (Pfield h, [ block ], Debuginfo.none)) (Var field)))
    in
  ...
  let f (expr : Flambda.t) : Flambda.t =
    ...
    | Let ({ var = v; defining_expr = named; _ } as let_expr) ->
      let to_substitute =
        Variable.Set.filter (fun v -> Variable.Map.mem v substitution)
      ...
      if Variable.Set.is_empty to_substitute then
        ...
        in
        let named = substitute_named bindings named in
        let expr =
        ...
        in
        Variable.Map.fold
          (fun to_substitute fresh expr -> bind to_substitute fresh expr)
          bindings expr
    | Let_mutable let_mutable
      when Variable.Map.mem let_mutable.initial_value substitution ->
    ...
    | Let_rec (defs, body) ->
      ...
      if Variable.Set.is_empty to_substitute then
        ...
        in
        let defs =
          List.map (fun (var, named) -> var, substitute_named bindings named)
            defs
        in
        let expr = Flambda.Let_rec (defs, body) in
        Variable.Map.fold
          (fun to_substitute fresh expr -> bind to_substitute fresh expr)
          bindings expr
    ...
    | Static_raise (exn, args) ->
      let (args, bind_args) = List.split (List.map make_var_subst args) in
    ...
    | Proved_unreachable | While _ | Try_with _ | Static_catch _ ->
  ...
  (struct
    ...
    let compare_key e1 e2 =
      ...
      let compare_var env v1 v2 =
        ...
        | bound -> Variable.compare v1 bound
      in
      let rec compare_expr env (e1 : key) (e2 : key) : int =
        match e1, e2 with
        | Var v1, Var v2 -> compare_var env v1 v2
        ...
        | Static_raise (sexn1, args1), Static_raise (sexn2, args2) ->
          ...
          if comp_sexn <> 0 then
            ...
            Misc.Stdlib.List.compare (compare_var env) args1 args2
...
=
  ...
  let symbols_to_fun_vars =
    Variable.Set.fold
      (fun fun_var symbols_to_fun_vars ->
         let closure_id = Closure_id.wrap fun_var in
         let symbol = closure_symbol closure_id in
         Symbol.Map.add symbol fun_var symbols_to_fun_vars) fun_vars
      Symbol.Map.empty
  in
  Variable.Map.map
         ...
         Symbol.Set.fold
           (fun symbol fun_vars' ->
              ...
              | fun_var ->
                ...
                Variable.Set.add fun_var fun_vars') func_decl.free_symbols
           Variable.Set.empty
       in
       let from_variables = Variable.Set.inter func_decl.free_variables fun_vars
       in
       Variable.Set.union from_symbols from_variables) function_decls.funs

let closures_required_by_entry_point
    ~(entry_point:Closure_id.t) ~closure_symbol
    (function_decls : Flambda.function_declarations)
=
  let dependencies = fun_vars_referenced_in_decls function_decls ~closure_symbol
  in
  let set = ref Variable.Set.empty in
  let queue = Queue.create () in
  let add v =
    if not (Variable.Set.mem v !set) then
  ...
  while not (Queue.is_empty queue) do
    ...
    | fun_dependencies ->
      Variable.Set.iter
        (fun dep -> if Variable.Map.mem dep function_decls.funs then add dep)
        fun_dependencies
  done;
  !set
...
let all_functions_parameters (function_decls : Flambda.function_declarations) =
  Variable.Map.fold
    (fun _ ({ params } : Flambda.function_declaration) set ->
       Variable.Set.union set (Parameter.Set.vars params)) function_decls.funs
    Variable.Set.empty

let all_free_symbols (function_decls : Flambda.function_declarations) =
  Variable.Map.fold
    (fun _ (function_decl : Flambda.function_declaration) syms ->
       Symbol.Set.union syms function_decl.free_symbols) function_decls.funs
    Symbol.Set.empty

let contains_stub (fun_decls : Flambda.function_declarations) =
  let number_of_stub_functions =
    Variable.Map.cardinal
      (Variable.Map.filter (fun _ { Flambda.stub } -> stub) fun_decls.funs)
  in
...
=
  ...
  in
  Variable.Map.map
    (fun ({ params; _ } : Flambda.function_declaration) ->
       List.map
         (fun param ->
            ...
            | { var; _ } ->
              Specialised_and_aliased_to
                (Variable.Map.find var specialised_arg_aliasing)) params)
    function_decls.funs
