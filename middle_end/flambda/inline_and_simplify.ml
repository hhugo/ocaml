...
let simplify_const (const : Flambda.const) =
  match const with Int i -> A.value_int i | Char c -> A.value_char c
    ...
    ~f:(fun _env closure closure_approx ->
      ...
      | Wrong ->
          Misc.fatal_errorf
            ...
            A.print closure_approx Flambda.print_move_within_set_of_closures
    ...
    ~f:(fun _env closure approx ->
      ...
      | Ok
          ...
          if not (Closure_id.equal closure_id closure_id_in_approx) then
            Misc.fatal_errorf
              ...
              Closure_id.print closure_id_in_approx Closure_id.print closure_id
          ...
          | None ->
              ...
              let expr =
                if E.mem env unwrapped then Flambda.Expr (Var unwrapped)
                else expr
              in
      ...
      | Wrong ->
          ...
          Misc.fatal_errorf
            ...
            Flambda.print_project_var project_var Variable.print closure
  ...
  let simplify_function fun_var (function_decl : Flambda.function_declaration)
    ...
    let body, r =
      E.enter_closure closure_env ~closure_id:(Closure_id.wrap fun_var)
        ...
        ~dbg:function_decl.dbg ~f:(fun body_env ->
          assert (
...
and simplify_apply env r ~(apply : Flambda.apply) : Flambda.t * R.t =
  let {
    Flambda.func = lhs_of_application;
    args;
    kind = _;
    dbg;
  ...
  | Prim (prim, args, dbg) ->
      ...
      simplify_free_variables_named env args ~f:(fun env args args_approxs ->
              ...
              | None -> (
                  ...
                  | Ok approx ->
                      let tree, approx =
                        ...
                        | Some (symbol, None) ->
                            let approx =
                              A.augment_with_symbol_field approx symbol
                                field_index
                            in
          ...
          | p, args, args_approxs ->
              ...
              let approx =
                match p with Popaque -> A.value_unknown Other | _ -> approx
              in
              (expr, ret r approx))
  | Expr expr ->
...
and simplify env r (tree : Flambda.t) : Flambda.t * R.t =
  ...
  | Let_rec (defs, body) ->
      ...
      let def_env =
        List.fold_left
          (fun env_acc (id, _lam) -> E.add env_acc id (A.value_unknown Other))
          env defs
      in
      let defs, body_env, r =
        List.fold_right
...
and duplicate_function ~env ~(set_of_closures : Flambda.set_of_closures)
  ...
  let body, _r =
    E.enter_closure closure_env ~closure_id:(Closure_id.wrap fun_var)
      ~inline_inside:false ~dbg:function_decl.dbg ~f:(fun body_env ->
        assert (
          E.inside_set_of_closures_declaration
            function_decls.set_of_closures_origin body_env);
  ...
  let env, r =
    Symbol.Set.fold
      (fun symbol (env, r) ->
        ...
        in
        (env, ret r approx))
      program.imported_symbols (env, r)
  in
