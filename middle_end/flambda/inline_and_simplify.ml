...
let simplify_free_variable_internal env original_var =
  ...
  *)
  ...
  | Outer, approx ->
    begin match A.simplify_var approx with
    | None -> No_binding var, approx
    | Some (named, approx) ->
      let module W = Flambda.With_free_variables 
      in
...
let simplify_free_variables env vars ~f : Flambda.t * R.t =
  let rec collect_bindings vars env bound_vars approxs : Flambda.t * R.t =
    ...
    | var :: vars ->
      begin match simplify_free_variable_internal env var with
...
let simplify_free_variable_named env var ~f : Flambda.named * R.t =
  simplify_free_variables_named env [ var ]
    ~f:(fun env vars vars_approxs ->
...
let approx_for_allocated_const (const : Allocated_const.t) =
  ...
  | Immutable_float_array a ->
    A.value_immutable_float_array (Array.map A.value_float (Array.of_list a))
  ...
  simplify_free_variable_named env project_closure.set_of_closures
    ~f:(fun _env set_of_closures set_of_closures_approx ->
      ...
      | Unresolved value ->
        ...
        Project_closure
          { set_of_closures; closure_id = project_closure.closure_id },
      ...
      | Ok (set_of_closures_var, value_set_of_closures) ->
        ...
        in
        begin match projecting_from with
        | Some (var, projection) ->
          simplify_free_variable_named env var
            ~f:(fun _env var var_approx ->
              let r = R.map_benefit r (B.remove_projection projection) in
              Expr (Var var), ret r var_approx
            )
        | None ->
          begin match reference_recursive_function_directly env closure_id with
...
(* Simplify an expression that, given one closure within some set of
   ...
   same set. *)
let simplify_move_within_set_of_closures
    env r ~(move_within_set_of_closures:Flambda.move_within_set_of_closures)
  :
  Flambda.named * R.t
=
  simplify_free_variable_named env move_within_set_of_closures.closure
    ~f:(fun _env closure closure_approx ->
      ...
      | Wrong ->
        Misc.fatal_errorf
          "Wrong approximation when moving within set of closures.  \
           Approximation: %a  Term: %a" A.print closure_approx
      ...
      | Ok
        ...
        | None ->
          begin match reference_recursive_function_directly env move_to with
          | Some (flam, approx) -> flam, ret r approx
          | None ->
            if Closure_id.equal start_from move_to then
              ...
              begin match set_of_closures_var with
              | Some set_of_closures_var when E.mem env set_of_closures_var ->
              ...
              | Some _ | None ->
                begin match set_of_closures_symbol with
                | Some set_of_closures_symbol ->
                  ...
                  let expr =
                    Flambda.create_let set_of_closures_var
                      (Symbol set_of_closures_symbol) let1
                  in
                  let approx =
...
=
  simplify_free_variable_named env project_var.closure
    ~f:(fun _env closure approx ->
      ...
      | Ok
          (value_closure,
           _set_of_closures_var,
           _set_of_closures_symbol,
           value_set_of_closures)
        ->
        let module F = Freshening.Project_var 
        in
        ...
        (if not (Closure_id.equal closure_id closure_id_in_approx) then
           Misc.fatal_errorf
             "When simplifying [Project_var], the closure ID %a in the \
              approximation of the set of closures did not match the closure \
              ID %a in the [Project_var] term.  Approximation: %a@. \
              Var-within-closure being projected: %a@." Closure_id.print
             closure_id_in_approx Closure_id.print closure_id
             Simple_value_approx.print approx Var_within_closure.print var);
        let projection : Projection.t = Project_var { closure; closure_id; var }
        in
        begin match E.find_projection env ~projection with
        | Some var ->
          simplify_free_variable_named env var
        ...
        | None ->
          ...
          let expr =
            if E.mem env unwrapped then Flambda.Expr (Var unwrapped) else expr
          in
      ...
      | Unknown ->
        Project_var { project_var with  closure }, ret r (A.value_unknown Other)
      | Unknown_because_of_unresolved_value value ->
        Project_var { project_var with  closure },
        ret r (A.value_unknown (Unresolved_value value))
      | Wrong ->
        ...
        Misc.fatal_errorf
          "[Project_var] from a value with wrong approximation: \
           %a@.closure=%a@.approx of closure=%a@." Flambda.print_project_var
          project_var Variable.print closure Simple_value_approx.print approx
    )
...
(* Transforms closure definitions by applying [loop] on the code of every
   ...
   * An approximation for the closures in the set. It contains the code of
   ...
   variable.
*)
and simplify_set_of_closures
    original_env r (set_of_closures : Flambda.set_of_closures)
  :
...
=
  let function_decls =
    let module Backend =
      (val
      (E.backend original_env : (module Backend_intf.S)))
  ...
  =
    ...
    in
    let function_decl =
      Flambda.create_function_declaration ~params:function_decl.params ~body
  ...
  in
  let function_decls = Flambda.update_function_declarations function_decls ~funs
  in
  ...
  let value_set_of_closures =
    A.create_value_set_of_closures ~function_decls:function_decls_approx
      ~bound_vars:internal_value_set_of_closures.bound_vars ~invariant_params
      ~recursive
  ...
  let direct_call_surrogates =
    Closure_id.Map.fold
      (fun existing surrogate surrogates ->
         Variable.Map.add (Closure_id.unwrap existing)
           (Closure_id.unwrap surrogate) surrogates)
      internal_value_set_of_closures.direct_call_surrogates Variable.Map.empty
  in
  let set_of_closures =
    Flambda.create_set_of_closures ~function_decls
      ~free_vars:(Variable.Map.map fst free_vars) ~specialised_args
      ~direct_call_surrogates
  in
...
and simplify_apply env r ~(apply:Flambda.apply) : Flambda.t * R.t =
  ...
  simplify_free_variable env lhs_of_application
    ~f:(fun env lhs_of_application lhs_of_application_approx ->
      simplify_free_variables env args
        ~f:(fun env args args_approxs ->
          ...
          | Ok
            ...
            let function_decl =
              ...
              | Not_found ->
                Misc.fatal_errorf
                  "When handling application expression, approximation \
                   references non-existent closure %a@." Closure_id.print
                  closure_id_being_applied
            in
            let r =
            ...
            let (result, r) =
              if nargs = arity then
                ...
                Misc.fatal_errorf
                  "Function with arity %d when simplifying application \
                   expression: %a" arity Flambda.print (Flambda.Apply apply)
            in
            wrap result, r
          | Wrong ->
...
and simplify_full_application
    ...
    ~inline_requested ~specialise_requested
=
  Inlining_decision.for_call_site ~env ~r ~function_decls ~lhs_of_application
...
and simplify_partial_application
    env r ~lhs_of_application ~closure_id_being_applied ~function_decl ~args
...
=
  ...
  | Unroll _ ->
    Location.prerr_warning (Debuginfo.to_location dbg)
      (Warnings.Inlining_impossible
         "[@unrolled] attributes may not be used on partial applications")
  ...
  | Always_specialise | Never_specialise ->
    Location.prerr_warning (Debuginfo.to_location dbg)
      (Warnings.Inlining_impossible
         "[@specialised] attributes may not be used on partial applications")
  | Default_specialise -> ()
  end;
  ...
  let (applied_args, remaining_args) =
    Misc.Stdlib.List.map2_prefix (fun arg id' -> id', arg) args freshened_params
  in
  let wrapper_accepting_remaining_args =
    ...
    let closure_variable =
      Variable.rename (Closure_id.unwrap closure_id_being_applied)
    in
    Flambda_utils.make_closure_declaration ~id:closure_variable
      ~is_classic_mode:false ~body ~params:remaining_args ~stub:true
  in
  let with_known_args =
    Flambda_utils.bind
      ~bindings:
      (List.map
         (fun (param, arg) -> Parameter.var param, Flambda.Expr (Var arg))
         applied_args) ~body:wrapper_accepting_remaining_args
  in
  simplify env r with_known_args

and simplify_over_application
    env r ~args ~args_approxs ~function_decls ~lhs_of_application
...
=
  ...
  let (full_app_approxs, _) = Misc.Stdlib.List.split_at arity args_approxs in
  let (expr, r) =
    simplify_full_application env r ~function_decls ~lhs_of_application
      ~closure_id_being_applied ~function_decl ~value_set_of_closures
      ~args:full_app_args ~args_approxs:full_app_approxs ~dbg ~inline_requested
      ~specialise_requested
  in
...
and simplify_named env r (tree : Flambda.named) : Flambda.named * R.t =
  ...
  | Read_mutable mut_var ->
    ...
    let mut_var = Freshening.apply_mutable_variable (E.freshening env) mut_var
    in
  ...
  | Set_of_closures set_of_closures ->
    ...
    let simplify env r expr ~pass_name : Flambda.named * R.t =
      ...
      let value_set_of_closures =
        ...
        | Wrong ->
          Misc.fatal_errorf
            "Unexpected approximation returned from simplification of [%s] \
             result: %a" pass_name A.print approx
    ...
    (* Do [Unbox_closures] next to try to decide which things are
       ...
       unboxing them. *)
    begin match
      Unbox_closures.rewrite_set_of_closures ~env ~duplicate_function
        ~set_of_closures
    with
    | Some (expr, benefit) ->
  ...
  | Prim (prim, args, dbg) ->
    let dbg = E.add_inlined_debuginfo env ~dbg in
    simplify_free_variables_named env args
      ~f:(fun env args args_approxs ->
        let tree = Flambda.Prim (prim, args, dbg) in
        match prim, args, args_approxs with
        ...
        | (Parraysetu kind
          ...
          ->
          (if A.warn_on_mutation block_approx then
             Location.prerr_warning (Debuginfo.to_location dbg)
          ...
          let kind =
            let check () =
              ...
              | Paddrarray | Pintarray ->
                ...
                Misc.fatal_errorf
                  "Assignment of a float to a specialised non-float \
                   array: %a" Flambda.print_named tree
            in
        ...
        | (Psequand | Psequor), _, _ ->
          Misc.fatal_error
            "Psequand and Psequor must be expanded (see handling in \
             closure_conversion.ml)"
        | p, args, args_approxs ->
          let (expr, approx, benefit) =
            let module Backend =
              (val
              (E.backend env : (module Backend_intf.S)))
...
and simplify env r (tree : Flambda.t) : Flambda.t * R.t =
  ...
  | Let _ ->
    ...
    in
    let for_last_body (env, r) body = simplify env r body in
    let filter_defining_expr r var defining_expr free_vars_of_body =
      if Variable.Set.mem var free_vars_of_body then
        ...
        r, var, Some defining_expr
    in
    Flambda.fold_lets_option tree ~init:(env, r) ~for_defining_expr
      ~for_last_body ~filter_defining_expr
  | Let_mutable { var = mut_var; initial_value = var; body; contents_kind } ->
    (* CR-someday mshinwell: add the dead let elimination, as above. *)
    simplify_free_variable env var
      ~f:(fun env var _var_approx ->
        let (mut_var, sb) =
        ...
        in
        Flambda.Let_mutable
          { var = mut_var; initial_value = var; body; contents_kind },
        r
      )
  | Let_rec (defs, body) ->
    ...
    let def_env =
      List.fold_left
        (fun env_acc (id, _lam) -> E.add env_acc id (A.value_unknown Other)) env
        defs
    in
    let (defs, body_env, r) =
      List.fold_right
        (fun (id, lam) (defs, env_acc, r) ->
           let (lam, r) = simplify_named def_env r lam in
           let defs = (id, lam) :: defs in
           let env_acc = E.add env_acc id (R.approx r) in
           defs, env_acc, r) defs ([], env, r)
    in
  ...
  | Static_catch (i, vars, body, handler) ->
    ...
    | _ ->
      ...
      if not (Static_exception.Set.mem i (R.used_static_exceptions r)) then
        ...
        begin match (body : Flambda.t) with
        | Static_raise (j, args) ->
          assert (Static_exception.equal i j);
          let handler =
            List.fold_left2
              (fun body var arg -> Flambda.create_let var (Expr (Var arg)) body)
              handler vars args
          in
        ...
        | _ ->
          ...
          let approx = R.approx r in
          let env =
            List.fold_left (fun env id -> E.add env id (A.value_unknown Other))
              (E.set_freshening env sb) vars
          in
          let env = E.inside_branch env in
          let (handler, r) = simplify env r handler in
          let r = R.exit_scope_catch r i in
          Static_catch (i, vars, body, handler), R.meet_approx r env approx
        end
    end
  ...
  | If_then_else (arg, ifso, ifnot) ->
    ...
    simplify_free_variable env arg
      ~f:(fun env arg arg_approx ->
        match arg_approx.descr with
        | Value_int 0 ->
          (* Constant [false]: keep [ifnot] *)
          let (ifnot, r) = simplify env r ifnot in
          ifnot, R.map_benefit r B.remove_branch
        | Value_int _ | Value_block _ ->
        ...
        | _ ->
          ...
          let (ifnot, r) = simplify env r ifnot in
          If_then_else (arg, ifso, ifnot), R.meet_approx r env ifso_approx
      )
  | While (cond, body) ->
  ...
  | Send { kind; meth; obj; args; dbg } ->
    let dbg = E.add_inlined_debuginfo env ~dbg in
    simplify_free_variable env meth
      ~f:(fun env meth _meth_approx ->
        simplify_free_variable env obj
          ~f:(fun env obj _obj_approx ->
            simplify_free_variables env args
              ~f:(fun _env args _args_approx ->
                Send { kind; meth; obj; args; dbg },
                ret r (A.value_unknown Other)
              )
          )
  ...
  | For { bound_var; from_value; to_value; direction; body } ->
    simplify_free_variable env from_value
      ~f:(fun env from_value _approx ->
        simplify_free_variable env to_value
          ~f:(fun env to_value _approx ->
            let (bound_var, sb) =
              Freshening.add_variable (E.freshening env) bound_var
            in
            let env =
              E.add (E.set_freshening env sb) bound_var (A.value_unknown Other)
            in
            let (body, r) = simplify env r body in
            For { bound_var; from_value; to_value; direction; body },
            ret r (A.value_unknown Other)
  ...
  | Assign { being_assigned; new_value } ->
    ...
    in
    simplify_free_variable env new_value
      ~f:(fun _env new_value _approx ->
  ...
  | Switch (arg, sw) ->
    ...
    simplify_free_variable env arg
      ~f:(fun env arg arg_approx ->
        ...
        in
        match filtered_consts, filtered_blocks with
        | Must_be_taken _, Must_be_taken _ -> assert false
        | Must_be_taken branch, _ | _, Must_be_taken branch ->
          let (lam, r) = simplify env r branch in
          lam, R.map_benefit r B.remove_branch
        | Can_be_taken consts, Can_be_taken blocks ->
          begin match consts, blocks, sw.failaction with
          | [], [], None ->
          ...
          | _ ->
            ...
            let f (i, v) (acc, r) =
              ...
              (i, lam) :: acc, R.meet_approx r env approx
            in
            ...
            let (failaction, r) =
              ...
              | Some l ->
                ...
                Some l, R.meet_approx r env approx
            in
          ...
          let (sw, r) =
            List.fold_right
              (fun (str, lam) (sw, r) ->
                 ...
                 (str, lam) :: sw, R.meet_approx r env approx) sw ([], r)
          in
          let (def, r) =
            match def with
            | None -> def, r
            | Some def ->
              ...
              Some def, R.meet_approx r env approx
          in
          String_switch (arg, sw, def), ret r (A.value_unknown Other)
        | Some arg_string ->
          let branch =
            match
              List.find (fun (str, _) -> String.equal str arg_string) sw
            with
            | _, branch -> branch
            | exception Not_found ->
              begin match def with
              | None -> Flambda.Proved_unreachable
              | Some def -> def
              end
          in
...
and simplify_list env r l =
  ...
  | h :: t ->
    ...
    if t' == t && h' == h then l, approxs, r else h' :: t', approxs, r

and duplicate_function
    ~env ~(set_of_closures:Flambda.set_of_closures) ~fun_var ~new_fun_var
=
  let function_decl =
  ...
  let
    (free_vars,
     ...
     function_decls,
     parameter_approximations,
     _internal_value_set_of_closures,
     set_of_closures_env) =
    Inline_and_simplify_aux.prepare_to_simplify_set_of_closures ~env
  ...
  let (body, _r) =
    E.enter_closure closure_env ~closure_id:(Closure_id.wrap fun_var)
      ~inline_inside:false ~dbg:function_decl.dbg
      ~f:(fun body_env ->
        assert
          (E.inside_set_of_closures_declaration
             function_decls.set_of_closures_origin body_env);
...
let constant_defining_value_approx
    env (constant_defining_value : Flambda.constant_defining_value)
=
  match constant_defining_value with
  | Allocated_const const -> approx_for_allocated_const const
  | Block (tag, fields) ->
    let fields =
      List.map
        (function
         | Flambda.Symbol sym ->
           begin match E.find_symbol_opt env sym with
           | Some approx -> approx
           | None -> A.value_unresolved (Symbol sym)
           end
         | Flambda.Const cst -> simplify_const cst) fields
    in
  ...
  | Set_of_closures { function_decls; free_vars; specialised_args } ->
    ...
    let value_set_of_closures =
      ...
      A.create_value_set_of_closures ~function_decls
        ~bound_vars:Var_within_closure.Map.empty ~invariant_params ~recursive
        ~specialised_args:Variable.Map.empty ~free_vars:Variable.Map.empty
        ~freshening:Freshening.Project_var.empty
        ~direct_call_surrogates:Closure_id.Map.empty
    in
    A.value_set_of_closures value_set_of_closures
  | Project_closure (set_of_closures_symbol, closure_id) ->
    begin match E.find_symbol_opt env set_of_closures_symbol with
    | None -> A.value_unresolved (Symbol set_of_closures_symbol)
    | Some set_of_closures_approx ->
      let checked_approx =
        A.check_approx_for_set_of_closures set_of_closures_approx
      in
      begin match checked_approx with
      ...
      | Wrong ->
        Misc.fatal_errorf
          "Wrong approximation for [Project_closure] when being used as \
           a [constant_defining_value]: %a"
...
let simplify_constant_defining_value
    env r symbol (constant_defining_value : Flambda.constant_defining_value)
=
  let (r, constant_defining_value, approx) =
    ...
    | Set_of_closures set_of_closures ->
      (if Variable.Map.cardinal set_of_closures.free_vars <> 0 then
         Misc.fatal_errorf
           "Set of closures bound by [Let_symbol] is not closed: %a"
    ...
    | Project_closure (set_of_closures_symbol, closure_id) ->
      ...
      let set_of_closures_approx = E.find_symbol_exn env set_of_closures_symbol
      in
      let closure_approx =
        match A.check_approx_for_set_of_closures set_of_closures_approx with
        | Ok (_, value_set_of_closures) ->
    ...
    Symbol.Set.fold
      (fun symbol (env, r) ->
         let (env, approx) =
           ...
           | approx -> env, approx
         in
         env, ret r approx) program.imported_symbols (env, r)
  in
...
let add_predef_exns_to_environment ~env ~backend =
  let module Backend = (val (backend : (module Backend_intf.S))) 
  in
  List.fold_left
    (fun env predef_exn ->
       ...
       in
       E.add_symbol env symbol (A.augment_with_symbol approx symbol)) env
...
let run ~never_inline ~backend ~prefixname ~round ~ppf_dump program =
  ...
  let initial_env =
    add_predef_exns_to_environment
      ~env:(E.create ~never_inline ~backend ~round ~ppf_dump) ~backend
  in
  let (result, r) = simplify_program initial_env r program in
  let result = Flambda_utils.introduce_needed_import_symbols result in
  (if not (Static_exception.Set.is_empty (R.used_static_exceptions r)) then
     Misc.fatal_error
       (Format.asprintf "Remaining static exceptions: %a@.%a@."
