...
let print_constant_defining_value ppf = function
  ...
  | Allocated_const (Array (_, _, vars)) ->
      Format.fprintf ppf "[| %a |]" (Format.pp_print_list Variable.print) vars
  ...
  | Block (tag, vars) ->
      Format.fprintf ppf "[|%a: %a|]" Tag.print tag
        (Format.pp_print_list Variable.print)
        vars
...
let rec resolve_definition (definitions : definitions) (var : Variable.t)
    (def : constant_defining_value) ~the_dead_constant : allocation_point =
  match def with
  | Allocated_const _ | Block _ | Set_of_closures _ | Project_closure _
  | Const _ | Move_within_set_of_closures _ ->
      Variable var
  | Project_var { var } ->
      fetch_variable definitions
        (Var_within_closure.unwrap var)
        ~the_dead_constant
  | Variable v -> fetch_variable definitions v ~the_dead_constant
  | Symbol sym -> Symbol sym
  | Field (v, n) -> (
      match fetch_variable definitions v ~the_dead_constant with
      | Symbol s -> fetch_symbol_field definitions s n ~the_dead_constant
      | Variable v -> fetch_variable_field definitions v n ~the_dead_constant)
  | Symbol_field (symbol, field) ->
      fetch_symbol_field definitions symbol field ~the_dead_constant

and fetch_variable (definitions : definitions) (var : Variable.t)
...
and fetch_variable_field (definitions : definitions) (var : Variable.t)
  ...
  | exception Not_found ->
      Misc.fatal_errorf "No definition for field access to %a" Variable.print
        var
      ...
      | fields -> (
          ...
          | None ->
              Misc.fatal_errorf "Constant field access to an inconstant %a"
                Symbol.print sym
          | Some v -> fetch_variable definitions v ~the_dead_constant)
      | exception Not_found ->
          Misc.fatal_errorf "No definition for field access to %a" Symbol.print
            sym)
