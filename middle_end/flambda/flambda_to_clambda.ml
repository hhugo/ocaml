...
let rec to_clambda t env (flam : Flambda.t) : Clambda.ulambda =
  ...
  | Let_mutable { var = mut_var; initial_value = var; body; contents_kind } ->
      ...
      Ulet
        (Mutable, contents_kind, VP.create id, def, to_clambda t env_body body)
  | Let_rec (defs, body) ->
      let env, defs =
        List.fold_right
  ...
  | Switch (arg, sw) -> (
      ...
      *)
      ...
      | Some failaction ->
          ...
          let sw =
            { sw with failaction = Some (Flambda.Static_raise (exn, [])) }
          in
          let expr : Flambda.t =
  ...
  | Allocated_const _ ->
      Misc.fatal_errorf
        "[Allocated_const] should have been lifted to a [Let_symbol] \
         construction before [Flambda_to_clambda]: %a = %a"
        Variable.print var Flambda.print_named named
  ...
  | Project_var { closure; var; closure_id } ->
      ...
      Uprim
        ( Pfield pos,
          [
            check_field t
              (check_closure t ulam (Expr (Var closure)))
              pos (Some named);
          ],
          Debuginfo.none )
...
and to_clambda_set_of_closures t env
    (({ function_decls; free_vars } : Flambda.set_of_closures) as
    set_of_closures) : Clambda.ulambda =
          ...
          with Not_found ->
            Misc.fatal_errorf
              "Clambda.to_clambda_set_of_closures: offset for free variable %a \
               is unknown.  Set of closures: %a"
              Variable.print id Flambda.print_set_of_closures set_of_closures
        in
        let pos = var_offset - fun_offset in
  ...
  let rec loop env constants (program : Flambda.program_body) :
    ...
    | Initialize_symbol (symbol, tag, fields, program) ->
        ...
        let init_fields =
          List.filter_map
            (function
              | i, field, None -> Some (i, field) | _, _, Some _ -> None)
            fields
        in
        let constant_fields =
          List.map
            (fun (_, _, constant_field) ->
              ...
              | Some (Flambda.Const const) ->
                  let n =
                    match const with Int i -> i | Char c -> Char.code c
                  in
        ...
        ( Usequence (e1, e2),
          constants,
          preallocated_block :: preallocated_blocks )
    | Effect (expr, program) ->
      ...
      Set_of_closures_id.Map.fold
        (fun (_ : Set_of_closures_id.t) fun_decls acc ->
          Variable.Map.fold
            ...
            fun_decls.Simple_value_approx.funs acc)
        imported.sets_of_closures Closure_id.Set.empty
    in
    {
