...
let is_function_constant t closure_id =
  if Closure_id.Set.mem closure_id t.current_unit.closures then
    ...
    Misc.fatal_errorf "Flambda_to_clambda: missing closure %a" Closure_id.print
      closure_id

(* Instrumentation of closure and field accesses to try to catch compiler
   bugs. *)
let check_closure t ulam named : Clambda.ulambda =
  if not !Clflags.clambda_checks then
    ulam
  else
    let desc =
      Primitive.simple ~name:"caml_check_value_is_closure" ~arity:2 ~alloc:false
    in
...
let check_field t ulam pos named_opt : Clambda.ulambda =
  if not !Clflags.clambda_checks then
    ...
    let desc =
      Primitive.simple ~name:"caml_check_field_access" ~arity:3 ~alloc:false
    in
    let str =
    ...
    (t.constants_for_instrumentation <-
     ...
     Uprim
       (Pccall desc,
        [ ulam; Clambda.Uconst (Uconst_int pos);
...
module Env : sig
  ...
  let allocated_const_for_symbol t sym =
    try Some (Symbol.Map.find sym t.allocated_constant_for_symbol)
    with
    | Not_found -> None
...
let build_uoffset ulam offset : Clambda.ulambda =
  if offset = 0 then ulam else Uoffset (ulam, offset)
...
let to_uconst_symbol env symbol : Clambda.ustructured_constant option =
  ...
  | None (* CR-soon mshinwell: Try to make this an error. *) | Some _ -> None
...
let rec to_clambda t env (flam : Flambda.t) : Clambda.ulambda =
  ...
  | Let_rec (defs, body) ->
    let (env, defs) =
      List.fold_right
        (fun (var, def) (env, defs) ->
           let (id, env) = Env.add_fresh_ident env var in
           env, (id, var, def) :: defs) defs (env, [])
    in
    let defs =
      List.map
        (fun (id, var, def) -> VP.create id, to_clambda_named t env var def)
        defs
    in
  ...
  | Apply { func; args; kind = Indirect; dbg } ->
    ...
    Ugeneric_apply
      (check_closure t callee (Flambda.Expr (Var func)),
       subst_vars env args,
       dbg)
  | Switch (arg, sw) ->
    ...
    | Some failaction ->
      ...
      let sw = { sw with  failaction = Some (Flambda.Static_raise (exn, [])) }
      in
      let expr : Flambda.t =
  ...
  | Static_raise (static_exn, args) ->
    Ustaticfail
      (Static_exception.to_int static_exn, List.map (subst_var env) args)
  | Static_catch (static_exn, vars, body, handler) ->
    let (env_handler, ids) =
      List.fold_right
        (fun var (env, ids) ->
           let (id, env) = Env.add_fresh_ident env var in
           env, (VP.create id, Lambda.Pgenval) :: ids) vars (env, [])
    in
    Ucatch
      (Static_exception.to_int static_exn,
       ids,
  ...
  | Try_with (body, var, handler) ->
    ...
    Utrywith
      (to_clambda t env body, VP.create id, to_clambda t env_handler handler)
  | If_then_else (arg, ifso, ifnot) ->
    Uifthenelse
      (subst_var env arg, to_clambda t env ifso, to_clambda t env ifnot)
  | While (cond, body) -> Uwhile (to_clambda t env cond, to_clambda t env body)
  | For { bound_var; from_value; to_value; direction; body } ->
    let (id, env_body) = Env.add_fresh_ident env bound_var in
    Ufor
      (VP.create id,
       subst_var env from_value,
       subst_var env to_value,
       direction,
       to_clambda t env_body body)
    ...
    let id =
      ...
      | Not_found ->
        Misc.fatal_errorf "Unbound mutable variable %a in [Assign]: %a"
          Mutable_variable.print being_assigned Flambda.print flam
    in
    Uassign (id, subst_var env new_value)
  | Send { kind; meth; obj; args; dbg } ->
    Usend
      (kind, subst_var env meth, subst_var env obj, subst_vars env args, dbg)
...
and to_clambda_named t env var (named : Flambda.named) : Clambda.ulambda =
  ...
  | Allocated_const _ ->
    Misc.fatal_errorf
      "[Allocated_const] should have been lifted to a [Let_symbol] \
       construction before [Flambda_to_clambda]: %a = %a" Variable.print var
      Flambda.print_named named
  | Read_mutable mut_var ->
    (try Uvar (Env.ident_for_mutable_var_exn env mut_var)
     with
     | Not_found ->
       Misc.fatal_errorf "Unbound mutable variable %a in [Read_mutable]: %a"
         Mutable_variable.print mut_var Flambda.print_named named)
    ...
    check_closure t
      (build_uoffset
         (check_closure t (subst_var env set_of_closures)
            (Flambda.Expr (Var set_of_closures))) (get_fun_offset t closure_id))
      named
  | Move_within_set_of_closures { closure; start_from; move_to } ->
    check_closure t
      (build_uoffset
         (check_closure t (subst_var env closure) (Flambda.Expr (Var closure)))
         (get_fun_offset t move_to - get_fun_offset t start_from)) named
  | Project_var { closure; var; closure_id } ->
    ...
    Uprim
      (Pfield pos,
       [
         check_field t (check_closure t ulam (Expr (Var closure))) pos
           (Some named) ],
       Debuginfo.none)
  | Prim (Pfield index, [ block ], dbg) ->
    Uprim
      (Pfield index, [ check_field t (subst_var env block) index None ], dbg)
  | Prim (Psetfield (index, maybe_ptr, init), [ block; new_value ], dbg) ->
    Uprim
      (Psetfield (index, maybe_ptr, init),
       [ check_field t (subst_var env block) index None; subst_var env new_value
       ],
       dbg)
  | Prim (Popaque, args, dbg) -> Uprim (Popaque, subst_vars env args, dbg)
...
and to_clambda_switch t env cases num_keys default =
  let num_keys =
    if Numbers.Int.Set.cardinal num_keys = 0 then
      0
    else
      Numbers.Int.Set.max_elt num_keys + 1
  in
  let store = Flambda_utils.Switch_storer.mk_store () in
  let default_action =
    match default with
    | Some def when List.length cases < num_keys -> store.act_store () def
    | _ -> (-1)
  in
  ...
  List.iter
    (fun (key, lam) ->
       ...
       smallest_key := Int.min key !smallest_key) cases;
  (if !smallest_key < num_keys then
     let action = ref index.(!smallest_key) in
     Array.iteri
       (fun i act -> if act >= 0 then action := act else index.(i) <- !action)
       index);
...
*)
and to_clambda_set_of_closures
    t env
...
=
    ...
    let env =
      ...
      let add_env_free_variable id _ env =
        let var_offset =
          ...
          | Not_found ->
            Misc.fatal_errorf
              "Clambda.to_clambda_set_of_closures: offset for free variable %a \
               is unknown.  Set of closures: %a" Variable.print id
    ...
    let (env_body, params) =
      List.fold_right
        (fun var (env, params) ->
           ...
           env, id :: params) function_decl.params (env, [])
    in
    {
      label = Compilenv.function_label closure_id;
      arity = Flambda_utils.function_arity function_decl;
      params =
        List.map (fun var -> VP.create var, Lambda.Pgenval)
          (params @ [ env_var ]);
  ...
  let free_vars =
    Variable.Map.bindings
      (Variable.Map.map
         (fun (free_var : Flambda.specialised_to) -> subst_var env free_var.var)
         free_vars)
  in
  Uclosure (funs, List.map snd free_vars)

and to_clambda_closed_set_of_closures
    t env symbol ({ function_decls } : Flambda.set_of_closures)
  :
  Clambda.ustructured_constant
=
  ...
  =
    ...
    let env =
      List.fold_left
        ...
        (Env.keep_only_symbols env) functions
    in
    let (env_body, params) =
      List.fold_right
        (fun var (env, params) ->
...
let to_clambda_initialize_symbol t env symbol fields : Clambda.ulambda =
  ...
  | h :: t ->
    List.fold_left
      (fun acc (p, field) -> Clambda.Usequence (build_setfield (p, field), acc))
      (build_setfield h) t

let accumulate_structured_constants
    t env symbol (c : Flambda.constant_defining_value) acc
=
  match c with
  | Allocated_const c ->
...
let to_clambda_program t env constants (program : Flambda.program) =
  ...
  =
    ...
    | Initialize_symbol (symbol, tag, fields, program) ->
      ...
      let init_fields =
        List.filter_map
          (function i, field, None -> Some (i, field) | _, _, Some _ -> None)
          fields
      in
      let constant_fields =
        List.map
          (fun (_, _, constant_field) ->
             ...
             | Some (Flambda.Symbol sym) ->
               let lbl = Linkage_name.to_string (Symbol.label sym) in
               Some (Clambda.Uconst_field_ref lbl)) fields
      in
    ...
    | End _ -> Uconst (Uconst_int 0), constants, []
  in
...
let convert ~ppf_dump (program, exported_transient) : result =
  let current_unit =
    let closures = Closure_id.Map.keys (Flambda_utils.make_closure_map program)
    in
    let constant_closures = Flambda_utils.all_lifted_constant_closures program
    in
      ...
      Set_of_closures_id.Map.fold
        (fun (_ : Set_of_closures_id.t) fun_decls acc ->
           Variable.Map.fold
             ...
             fun_decls.Simple_value_approx.funs acc) imported.sets_of_closures
  ...
  in
  let exported =
    Export_info.t_of_transient exported_transient ~program
