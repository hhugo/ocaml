...
let apply_on_subexpressions f f_named (flam : Flambda.t) =
  match flam with
  | Var _ | Apply _ | Assign _ | Send _ | Proved_unreachable | Static_raise _ ->
      ()
...
let rec list_map_sharing f l =
  ...
  | h :: t ->
      ...
      let new_h = f h in
      if h == new_h && t == new_t then l else new_h :: new_t
...
let may_map_sharing f v =
  ...
  | Some s ->
      let new_s = f s in
      if s == new_s then v else Some new_s

let map_snd_sharing f ((a, b) as cpl) =
  let new_b = f a b in
  if b == new_b then cpl else (a, new_b)
...
let map_subexpressions f f_named (tree : Flambda.t) : Flambda.t =
  ...
  | Let { var; defining_expr; body; _ } ->
      ...
      if new_named == defining_expr && new_body == body then tree
      else Flambda.create_let var new_named new_body
  | Let_rec (defs, body) ->
      let new_defs = list_map_sharing (map_snd_sharing f_named) defs in
      let new_body = f body in
      if new_defs == defs && new_body == body then tree
      else Let_rec (new_defs, new_body)
  | Let_mutable mutable_let ->
      let new_body = f mutable_let.body in
      if new_body == mutable_let.body then tree
      else Let_mutable { mutable_let with body = new_body }
  | Switch (arg, sw) ->
      ...
      if
        sw.failaction == new_failaction
        && new_consts == sw.consts && new_blocks == sw.blocks
      then tree
      else
        let sw =
          {
  ...
  | String_switch (arg, sw, def) ->
      ...
      if sw == new_sw && def == new_def then tree
      else String_switch (arg, new_sw, new_def)
  | Static_catch (i, vars, body, handler) ->
      let new_body = f body in
      let new_handler = f handler in
      if new_body == body && new_handler == handler then tree
      else Static_catch (i, vars, new_body, new_handler)
  | Try_with (body, id, handler) ->
      let new_body = f body in
      let new_handler = f handler in
      if body == new_body && handler == new_handler then tree
      else Try_with (new_body, id, new_handler)
  | If_then_else (arg, ifso, ifnot) ->
      let new_ifso = f ifso in
      let new_ifnot = f ifnot in
      if new_ifso == ifso && new_ifnot == ifnot then tree
      else If_then_else (arg, new_ifso, new_ifnot)
  | While (cond, body) ->
      let new_cond = f cond in
      let new_body = f body in
      if new_cond == cond && new_body == body then tree
      else While (new_cond, new_body)
  | For { bound_var; from_value; to_value; direction; body } ->
      let new_body = f body in
      if new_body == body then tree
      else For { bound_var; from_value; to_value; direction; body = new_body }
  ...
  let rec loop (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (defs, program) ->
        List.iter
          ...
          defs;
        loop program
    | Let_symbol (_, _, program) -> loop program
        ...
        List.iter
          (function
            | _, Flambda.Set_of_closures set_of_closures ->
                ...
                Variable.Map.iter
                  (fun _ (function_decl : Flambda.function_declaration) ->
                    iter_on_sets_of_closures (f ~constant:false)
                      function_decl.body)
                  set_of_closures.function_decls.funs
            | _ -> ())
          defs;
...
let map_general ~toplevel f f_named tree =
    ...
    | _ ->
        let exp : Flambda.t =
          ...
          | Let_rec (defs, body) ->
              ...
              if not !done_something then tree else Let_rec (defs, body)
          | Switch (arg, sw) ->
              let done_something = ref false in
              let sw =
                {
                  ...
                  consts =
                    List.map
                      (fun (i, v) -> (i, aux_done_something v done_something))
                      sw.consts;
                  blocks =
                    List.map
                      (fun (i, v) -> (i, aux_done_something v done_something))
              ...
              in
              if not !done_something then tree else Switch (arg, sw)
          | String_switch (arg, sw, def) ->
              let done_something = ref false in
              let sw =
                List.map
              ...
              in
              if not !done_something then tree else String_switch (arg, sw, def)
          | Static_catch (i, vars, body, handler) ->
  ...
  and aux_done_something expr done_something =
    ...
    if not (new_expr == expr) then done_something := true;
    new_expr
  and aux_named (id : Variable.t) (named : Flambda.named) =
    let named : Flambda.named =
            ...
            let funs =
              Variable.Map.map
                (fun (func_decl : Flambda.function_declaration) ->
                  ...
                  if new_body == func_decl.body then func_decl
                  else (
                    done_something := true;
                ...
                function_decls.funs
            in
            if not !done_something then named
            else
              let function_decls =
      ...
      | Expr expr ->
          let new_expr = aux expr in
          if new_expr == expr then named else Expr new_expr
    in
    f_named id named
  and aux_named_done_something id named done_something =
    let new_named = aux_named id named in
    if not (new_named == named) then done_something := true;
    new_named
  in
  aux tree
...
let iter_apply_on_program program ~f =
  iter_exprs_at_toplevel_of_program program ~f:(fun expr ->
      iter (function Apply apply -> f apply | _ -> ()) (fun _ -> ()) expr)
  ...
  map_named
    (function
      | Symbol sym as named ->
          ...
          if new_sym == sym then named else Symbol new_sym
      | Read_symbol_field (sym, field) as named ->
          let new_sym = f sym in
          if new_sym == sym then named else Read_symbol_field (new_sym, field)
      | ( Const _ | Allocated_const _ | Set_of_closures _ | Read_mutable _
        | Project_closure _ | Move_within_set_of_closures _ | Project_var _
        | Prim _ | Expr _ ) as named ->
          named)
    tree
  ...
  let funs =
    Variable.Map.map
      (fun (func_decl : Flambda.function_declaration) ->
        ...
        if not (body == func_decl.body) then done_something := true;
        Flambda.update_function_declaration func_decl ~params:func_decl.params
          ~body)
      function_decls.funs
  in
  ...
  map_toplevel_named
    (function
      ...
      | ( Symbol _ | Const _ | Allocated_const _ | Read_mutable _
        ...
        | Move_within_set_of_closures _ | Project_var _ | Prim _ | Expr _ ) as
        named ->
          named)
    tree
  ...
  map
    (function
      | Apply apply as expr ->
          ...
          if new_apply == apply then expr else Apply new_apply
      | expr -> expr)
    (fun named -> named)
    tree
  ...
  map_named
    (function
      ...
      | ( Symbol _ | Const _ | Allocated_const _ | Project_closure _
        ...
        | Read_mutable _ | Read_symbol_field _ ) as named ->
          named)
    tree
  ...
  map_named
    (function
      | Project_var project_var as named -> (
          match f project_var with None -> named | Some expr -> Expr expr)
...
let map_project_var_to_named_opt tree ~f =
  map_named
    (function
      | Project_var project_var as named -> (
          match f project_var with None -> named | Some named -> named)
      | ( Symbol _ | Const _ | Allocated_const _ | Set_of_closures _
        | Project_closure _ | Move_within_set_of_closures _ | Prim _ | Expr _
        | Read_mutable _ | Read_symbol_field _ ) as named ->
          named)
  ...
  let funs =
    Variable.Map.map
      (fun (function_decl : Flambda.function_declaration) ->
        ...
        if new_body == function_decl.body then function_decl
        else (
          done_something := true;
          Flambda.update_function_declaration function_decl ~body:new_body
...
let map_sets_of_closures_of_program (program : Flambda.program)
  ...
  let rec loop (program : Flambda.program_body) : Flambda.program_body =
        ...
        let funs =
          Variable.Map.map
            (fun (function_decl : Flambda.function_declaration) ->
              ...
              if body == function_decl.body then function_decl
              else (
                done_something := true;
                Flambda.update_function_declaration function_decl ~body
                  ~params:function_decl.params))
            set_of_closures.function_decls.funs
        in
        if not !done_something then set_of_closures.function_decls
        else
          Flambda.update_function_declarations set_of_closures.function_decls
            ~funs
      in
      let new_set_of_closures = f set_of_closures in
      if new_set_of_closures == set_of_closures then set_of_closures
      else
        Flambda.create_set_of_closures ~function_decls
          ~free_vars:set_of_closures.free_vars
          ~specialised_args:set_of_closures.specialised_args
    ...
    | Let_symbol (symbol, Set_of_closures set_of_closures, program') ->
        ...
        if new_set_of_closures == set_of_closures && new_program' == program'
        then program
        else
          Let_symbol (symbol, Set_of_closures new_set_of_closures, new_program')
    | Let_symbol (symbol, const, program') ->
        let new_program' = loop program' in
        if new_program' == program' then program
        else Let_symbol (symbol, const, new_program')
    | Let_rec_symbol (defs, program') ->
        let done_something = ref false in
        let defs =
          List.map
            (function
              | var, Flambda.Set_of_closures set_of_closures ->
                  let new_set_of_closures =
                    map_constant_set_of_closures set_of_closures
                  in
                  if not (new_set_of_closures == set_of_closures) then
                    done_something := true;
        ...
        in
        let new_program' = loop program' in
        if new_program' == program' && not !done_something then program
        else Let_rec_symbol (defs, loop program')
        ...
        let fields =
          List.map
            (fun field ->
              ...
              if not (new_field == field) then done_something := true;
              new_field)
            fields
        in
        let new_program' = loop program' in
        if new_program' == program' && not !done_something then program
        else Initialize_symbol (symbol, tag, fields, new_program')
    | Effect (expr, program') ->
        let new_expr = map_sets_of_closures expr ~f in
        let new_program' = loop program' in
        if new_expr == expr && new_program' == program' then program
        else Effect (new_expr, new_program')
    | End _ -> program
  in
  { program with program_body = loop program.program_body }
...
let map_exprs_at_toplevel_of_program (program : Flambda.program)
  ...
  let rec loop (program : Flambda.program_body) : Flambda.program_body =
    ...
    | Let_rec_symbol (defs, program') ->
        ...
        let new_program' = loop program' in
        if new_program' == program' && not !done_something then program
        else Let_rec_symbol (defs, new_program')
    | Initialize_symbol (symbol, tag, fields, program') ->
        let done_something = ref false in
        let fields =
          List.map
...
let map_named_of_program (program : Flambda.program)
  ...
  map_exprs_at_toplevel_of_program program ~f:(fun expr ->
...
let fold_function_decls_ignoring_stubs
  ...
  Variable.Map.fold
    (fun fun_var function_decl acc -> f ~fun_var ~function_decl acc)
    set_of_closures.function_decls.funs init
