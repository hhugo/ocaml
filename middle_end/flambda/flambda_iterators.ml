    ...
    | Let_symbol (_, Set_of_closures set_of_closures, program) ->
      Variable.Map.iter
        (fun _ (function_decl : Flambda.function_declaration) ->
           f function_decl.body) set_of_closures.function_decls.funs;
      loop program
      ...
      let exp : Flambda.t =
        ...
        | Let_rec (defs, body) ->
          ...
          let defs =
            List.map
              (fun (id, lam) ->
                 id, aux_named_done_something id lam done_something) defs
          in
        ...
        | Switch (arg, sw) ->
          ...
          let sw =
            { sw with
              ...
              consts =
                List.map (fun (i, v) -> i, aux_done_something v done_something)
                  sw.consts;
              blocks =
                List.map (fun (i, v) -> i, aux_done_something v done_something)
                  sw.blocks
          ...
          let funs =
            Variable.Map.map
              (fun (func_decl : Flambda.function_declaration) ->
                 ...
                 if new_body == func_decl.body then
                   func_decl
                 else
                   (done_something := true;
...
let map_apply tree ~f =
  map
    (function
     ...
     | expr -> expr) (fun named -> named) tree
         ...
         if new_body == function_decl.body then
           ...
           (done_something := true;
            Flambda.update_function_declaration function_decl ~body:new_body
              ~params:function_decl.params)) set_of_closures.function_decls.funs
  in
  if not !done_something then
    set_of_closures
  else
...
let map_sets_of_closures_of_program
...
=
  let rec loop (program : Flambda.program_body) : Flambda.program_body =
    ...
    | Let_symbol (symbol, Set_of_closures set_of_closures, program') ->
      ...
      if new_set_of_closures == set_of_closures && new_program' == program' then
        program
      else
...
let map_exprs_at_toplevel_of_program
    (program : Flambda.program) ~(f:Flambda.t -> Flambda.t)
=
  let rec loop (program : Flambda.program_body) : Flambda.program_body =
    let map_constant_set_of_closures
        (set_of_closures : Flambda.set_of_closures)
...
let map_all_immutable_let_and_let_rec_bindings
    (expr : Flambda.t) ~(f:Variable.t -> Flambda.named -> Flambda.named)
  :
  Flambda.t
=
