...
let transitive_closure state =
  ...
  let update arg state =
    let original_set =
      ...
      with Not_found -> Implication Variable.Pair.Set.empty
    in
    ...
    (fun _ (function_decl : Flambda.function_declaration) ->
      Flambda_iterators.iter_all_toplevel_immutable_let_and_let_rec_bindings
        ~f:(fun var named ->
          ...
          match named with
          | Symbol sym -> (
            match Symbol.Map.find sym symbols_to_fun_vars with
            | exception Not_found -> ()
            | fun_var ->
                fun_var_bindings :=
...
let analyse_functions ~backend ~param_to_param ~anything_to_param
    ~param_to_anywhere (decls : Flambda.function_declarations) =
  ...
  let find_callee_arg ~callee ~callee_pos =
    ...
    | arr ->
        (* Ignore overapplied parameters: they are applied to a different
         function. *)
        if callee_pos < Array.length arr
        then Some arr.(callee_pos)
        else None
  in
    ...
    | Some callee_arg -> (
      ...
      | {params} ->
          let new_relation =
            ...
            if List.exists
                 (fun param ->
                   Variable.equal (Parameter.var param) caller_arg )
                 params
            then
              param_to_param
                ~caller
    ...
    (fun func_var ({params} : Flambda.function_declaration) ->
      List.iter
        (fun (param : Parameter.t) ->
          if Variable.Tbl.mem used_variables (Parameter.var param)
          then
            relation :=
              param_to_anywhere
                ~caller:func_var
                ~caller_arg:(Parameter.var param)
                !relation;
          if Variable.Tbl.mem escaping_functions func_var
          then
            relation :=
              anything_to_param
                ~callee:func_var
                ~callee_arg:(Parameter.var param)
                !relation )
        params )
    decls.funs;
  ...
  let not_unchanging =
    Variable.Pair.Map.fold
      (fun (func, var) set not_unchanging ->
        ...
        | Implication set ->
            if Variable.Pair.Set.exists
                 (fun (func', var') ->
                   Variable.equal func func'
                   && not (Variable.equal var var') )
                 set
        ...
        | Implication set when Variable.Set.mem var unchanging ->
            Variable.Pair.Set.fold
              (fun (_, caller_args) aliases ->
                if Variable.Set.mem caller_args unchanging
                then
                  let alias_set =
      ...
      (fun fun_var decl acc ->
        List.fold_left
          (fun acc param ->
            ...
            | Top -> acc )
          acc
          (Parameter.List.vars decl.Flambda.params) )
      decls.funs
      Variable.Set.empty
  in
  if dump
  then Format.printf "Unused arguments: %a@." Variable.Set.print arguments;
  arguments
