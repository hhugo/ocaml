...
let occurs_var var u =
  let rec occurs = function
    ...
    | Uswitch (arg, s, _dbg) ->
        occurs arg
        || occurs_array s.us_actions_consts
        || occurs_array s.us_actions_blocks
...
let prim_size prim args =
  ...
  | Psetfield (_f, isptr, init) -> (
      ...
      | Assignment | Heap_initialization -> (
          match isptr with Pointer -> 4 | Immediate -> 1))
  | Pfloatfield _ -> 1
  | Psetfloatfield _ -> 1
...
let simplif_arith_prim_pure ~backend fpc p (args, approxs) dbg =
  ...
  | [ Value_const (Uconst_ref (_, Some (Uconst_int32 n))) ] -> (
      ...
      | Pcvtbint (Pint32, Pnativeint) ->
          make_const_natint (Nativeint.of_int32 n)
  ...
  | [ Value_const (Uconst_ref (_, Some (Uconst_int64 n))) ] -> (
      ...
      | Pcvtbint (Pint64, Pnativeint) ->
          make_const_natint (Int64.to_nativeint n)
...
let simplif_prim_pure ~backend fpc p (args, approxs) dbg =
  ...
  | Pmakeblock (tag, Immutable, _kind), _, _ -> (
      let field = function Value_const c -> c | _ -> raise Exit in
      try
      ...
      with Exit -> (Uprim (p, args, dbg), Value_tuple (Array.of_list approxs)))
  (* Field access *)
...
let rec substitute loc ((backend, fpc) as st) sb rn ulam =
  match ulam with
  | Uvar v -> ( try V.Map.find v sb with Not_found -> ulam)
  | Uconst _ -> ulam
  ...
  | Uletrec (bindings, body) ->
      ...
      let sb' =
        List.fold_right
          (fun (id, id', _) s -> V.Map.add id (Uvar (VP.var id')) s)
          bindings1 sb
      in
      Uletrec
        ( List.map
  ...
  | Uifthenelse (u1, u2, u3) -> (
      match substitute loc st sb rn u1 with
      | Uconst (Uconst_int n) ->
          if n <> 0 then substitute loc st sb rn u2
          else substitute loc st sb rn u3
      | Uprim (P.Pmakeblock _, _, _) -> substitute loc st sb rn u2
      | su1 ->
          Uifthenelse
            (su1, substitute loc st sb rn u2, substitute loc st sb rn u3))
  | Usequence (u1, u2) ->
  ...
  | Uassign (id, u) ->
      let id' =
        try match V.Map.find id sb with Uvar i -> i | _ -> assert false
        with Not_found -> id
      in
  ...
  let rec aux subst pl al body =
    ...
    | [], [] ->
        substitute
          ...
          (backend, fpc) subst (Some Int.Map.empty) body
    | p1 :: pl, a1 :: al ->
...
let direct_apply env fundesc ufunct uargs ~loc ~attribute =
  ...
  if (not fundesc.fun_closed) || is_pure ufunct then app
...
let check_constant_result ulam approx =
  ...
  | Value_global_field (id, i) when is_pure ulam -> (
      match ulam with
      | Uprim (P.Pfield _, [ Uprim (P.Pread_symbol _, _, _) ], _) ->
          (ulam, approx)
      | _ ->
...
let rec close ({ backend; fenv; cenv; mutable_vars } as env) lam =
  ...
  | Lapply
    ...
    -> (
      ...
      match (close env funct, close_list env args) with
      ...
      | (ufunct, (Value_closure (fundesc, _) as fapprox)), uargs
        when nargs < fundesc.fun_arity ->
          let first_args =
            List.map (fun arg -> (V.create_local "arg", arg)) uargs
          in
          let final_args =
            Array.to_list
              (Array.init (fundesc.fun_arity - nargs) (fun _ ->
                   V.create_local "arg"))
          in
          ...
          let new_fun, approx =
            close
              { backend; fenv; cenv; mutable_vars }
              (Lfunction
                 {
                   kind = Curried;
                   return = Pgenval;
  ...
  | Lletrec (defs, body) ->
      if List.for_all (function _id, Lfunction _ -> true | _ -> false) defs
      then
        ...
        ( Ulet
            ( Immutable,
              ...
              clos,
              substitute Debuginfo.none
                (backend, !Clflags.float_const_prop)
                sb None ubody ),
          approx )
      else
...
and close_functions { backend; fenv; cenv; mutable_vars } fun_defs =
  ...
  ( Uclosure
      (clos, List.map (close_var { backend; fenv; cenv; mutable_vars }) fv),
    infos )
...
and close_switch env cases num_keys default =
  ...
  let index = Array.make num_keys 0 and store = Storer.mk_store () in
...
let collect_exported_structured_constants a =
  let rec approx = function
    | Value_closure (fd, a) -> (
        approx a;
        match fd.fun_inline with Some (_, u) -> ulam u | None -> ())
  ...
  and structured_constant = function
    ...
    | Uconst_float _ | Uconst_int32 _ | Uconst_int64 _ | Uconst_nativeint _
...
let intro ~backend ~size lam =
  ...
  in
  if opaque then Compilenv.set_global_approx Value_unknown
