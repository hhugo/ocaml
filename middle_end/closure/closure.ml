...
let rec split_list n l =
  if n <= 0 then
    ...
    | a :: l ->
      let (l1, l2) = split_list (n - 1) l in
      a :: l1, l2

let rec build_closure_env env_param pos =
  function
  | [] -> V.Map.empty
  | id :: rem ->
    V.Map.add id (Uprim (P.Pfield pos, [ Uvar env_param ], Debuginfo.none))
...
let occurs_var var u =
  ...
  and occurs_array a =
    try
      for i = 0 to Array.length a - 1 do if occurs a.(i) then raise Exit done;
      false
    with
    | Exit -> true
  in
...
let lambda_smaller lam threshold =
  ...
  let rec lambda_size lam =
    ...
    | Ulet (_str, _kind, _id, lam, body) ->
      ...
      lambda_size body
    | Uphantom_let _ -> no_phantom_lets ()
    | Uletrec _ -> raise Exit (* usually too large *)
    | Uprim (prim, args, _) ->
  ...
  and lambda_array_size a = Array.iter lambda_size a
  in
  try
    lambda_size lam;
    !size <= threshold
...
let rec is_pure =
  ...
  | Ulet (Immutable, _, _var, def, body) -> is_pure def && is_pure body
  | _ -> false
...
let make_const_ref c =
  make_const
    (Uconst_ref (Compilenv.new_structured_constant ~shared:true c, Some c))
...
let simplif_arith_prim_pure ~backend fpc p (args, approxs) dbg =
  ...
  | [ Value_const (Uconst_int n1) ] ->
    ...
    | Pbswap16 ->
      make_const_int ((n1 land 0xff) lsl 8 lor (n1 land 0xff00) lsr 8)
    | _ -> default
    end
  (* int (or enumerated type), int (or enumerated type) *)
  | [ Value_const (Uconst_int n1); Value_const (Uconst_int n2) ] ->
    begin match p with
    ...
    | Plslint when 0 <= n2 && n2 < 8 * B.size_int -> make_const_int (n1 lsl n2)
    | Plsrint when 0 <= n2 && n2 < 8 * B.size_int -> make_const_int (n1 lsr n2)
    | Pasrint when 0 <= n2 && n2 < 8 * B.size_int -> make_const_int (n1 asr n2)
  ...
  | [ Value_const (Uconst_ref (_, Some (Uconst_float n1)));
      Value_const (Uconst_ref (_, Some (Uconst_float n2))) ]
    when fpc ->
    begin match p with
...
let simplif_prim_pure ~backend fpc p (args, approxs) dbg =
  ...
  | Pmakeblock (tag, Immutable, _kind), _, _ ->
    ...
    (try
       ...
       let name = Compilenv.new_structured_constant cst ~shared:true in
       make_const (Uconst_ref (name, Some cst))
     with
     | Exit -> Uprim (p, args, dbg), Value_tuple (Array.of_list approxs))
  (* Field access *)
  ...
  (* Catch-all *)
  | _ -> simplif_arith_prim_pure ~backend fpc p (args, approxs) dbg

let simplif_prim ~backend fpc p (args, approxs as args_approxs) dbg =
  if List.for_all is_pure args then
    simplif_prim_pure ~backend fpc p args_approxs dbg
  else (* XXX : always return the same approxs as simplif_prim_pure? *)
    let approx =
      match p with
...
let find_action idxs acts tag =
  if 0 <= tag && tag < Array.length idxs then
    let idx = idxs.(tag) in
...
let subst_debuginfo loc dbg =
  if !Clflags.debug then Debuginfo.inline loc dbg else dbg
...
let rec substitute loc (backend, fpc as st) sb rn ulam =
  ...
  | Ugeneric_apply (fn, args, dbg) ->
    let dbg = subst_debuginfo loc dbg in
    Ugeneric_apply
      (substitute loc st sb rn fn, List.map (substitute loc st sb rn) args, dbg)
  | Uclosure (defs, env) ->
  ...
  | Ulet (str, kind, id, u1, u2) ->
    ...
    Ulet
      (str,
       kind,
       id',
       substitute loc st sb rn u1,
       substitute loc st (V.Map.add (VP.var id) (Uvar (VP.var id')) sb) rn u2)
  | Uphantom_let _ -> no_phantom_lets ()
  | Uletrec (bindings, body) ->
    let bindings1 =
      List.map (fun (id, rhs) -> VP.var id, VP.rename id, rhs) bindings
    in
    let sb' =
      List.fold_right (fun (id, id', _) s -> V.Map.add id (Uvar (VP.var id')) s)
        bindings1 sb
    in
    Uletrec
      (List.map (fun (_id, id', rhs) -> id', substitute loc st sb' rn rhs)
         bindings1,
  ...
  | Uprim (p, args, dbg) ->
    ...
    let dbg = subst_debuginfo loc dbg in
    let (res, _) =
      simplif_prim ~backend fpc p (sargs, List.map approx_ulam sargs) dbg
    in
    res
  | Uswitch (arg, sw, dbg) ->
  ...
  | Ucatch (nfail, ids, u1, u2) ->
    ...
    let sb' =
      List.fold_right2
        (fun (id, _) (id', _) s -> V.Map.add (VP.var id) (Uvar (VP.var id')) s)
        ids ids' sb
    in
    Ucatch
      (nfail, ids', substitute loc st sb rn u1, substitute loc st sb' rn u2)
  | Utrywith (u1, id, u2) ->
    let id' = VP.rename id in
    Utrywith
      (substitute loc st sb rn u1,
  ...
  | Uifthenelse (u1, u2, u3) ->
    ...
    | Uconst (Uconst_int n) ->
      if n <> 0 then substitute loc st sb rn u2 else substitute loc st sb rn u3
    | Uprim (P.Pmakeblock _, _, _) -> substitute loc st sb rn u2
    | su1 ->
      Uifthenelse (su1, substitute loc st sb rn u2, substitute loc st sb rn u3)
    end
  | Usequence (u1, u2) ->
  ...
  | Ufor (id, u1, u2, dir, u3) ->
    ...
    Ufor
      (id',
       ...
       dir,
       substitute loc st (V.Map.add (VP.var id) (Uvar (VP.var id')) sb) rn u3)
  | Uassign (id, u) ->
    let id' =
      try
        match V.Map.find id sb with
        | Uvar i -> i
        | _ -> assert false
      with
      | Not_found -> id
  ...
  | Usend (k, u1, u2, ul, dbg) ->
    ...
    Usend
      (k,
       substitute loc st sb rn u1,
       substitute loc st sb rn u2,
       List.map (substitute loc st sb rn) ul,
       dbg)
...
let is_substituable ~mutable_vars =
  function
...
let bind_params { backend; mutable_vars; _ } loc fpc params args body =
  let rec aux subst pl al body =
    match pl, al with
    | [], [] ->
      substitute (Debuginfo.from_location loc) (backend, fpc) subst
        (Some Int.Map.empty) body
    | p1 :: pl, a1 :: al ->
      if is_substituable ~mutable_vars a1 then
        ...
        let (u1, u2) =
          ...
          | "*opt*", Uprim (P.Pmakeblock (0, Immutable, kind), [ a ], dbg) ->
            a,
            Uprim
              (P.Pmakeblock (0, Immutable, kind), [ Uvar (VP.var p1') ], dbg)
          | _ -> a1, Uvar (VP.var p1')
        in
...
let direct_apply env fundesc ufunct uargs ~loc ~attribute =
  let app_args = if fundesc.fun_closed then uargs else uargs @ [ ufunct ] in
  let app =
    ...
    | Some (params, body), _ ->
      bind_params env loc fundesc.fun_float_const_prop params app_args body
  in
  ...
  if not fundesc.fun_closed || is_pure ufunct then
    app
  else
    Usequence (ufunct, app)
...
let strengthen_approx appl approx =
  ...
  | Value_const _ as intapprox -> intapprox
  | _ -> approx
...
let check_constant_result ulam approx =
  ...
  | Value_global_field (id, i) when is_pure ulam ->
    ...
    | _ ->
      let glb = Uprim (P.Pread_symbol id, [], Debuginfo.none) in
...
let close_var env id =
  let (ulam, _app) = close_approx_var env id in
  ulam
...
let rec close ({ backend; fenv; cenv; mutable_vars } as env) lam =
  ...
  in
  match lam with
  | Lvar id -> close_approx_var env id
  | Lmutvar id -> Uvar id, Value_unknown
  | Lconst cst ->
    let str ?(shared=true) cst =
      let name = Compilenv.new_structured_constant cst ~shared in
    ...
    let rec transl =
      ...
      | Const_immstring s -> str (Uconst_string s)
  ...
  | Lapply
      { ap_func = funct; ap_args = args; ap_loc = loc; ap_inlined = attribute }
    ->
    ...
    | (ufunct, Value_closure (fundesc, approx_res)),
      ...
      let app = direct_apply env ~loc ~attribute fundesc ufunct uargs in
    ...
    | (ufunct, (Value_closure (fundesc, _) as fapprox)), uargs
      when nargs < fundesc.fun_arity ->
      let first_args = List.map (fun arg -> V.create_local "arg", arg) uargs in
      let final_args =
        Array.to_list
          (Array.init (fundesc.fun_arity - nargs)
             (fun _ -> V.create_local "arg"))
      in
      let rec iter args body =
        match args with
        | [] -> body
        | (arg1, arg2) :: args ->
          iter args (Ulet (Immutable, Pgenval, VP.create arg1, arg2, body))
      in
      let internal_args =
      ...
      let (new_fun, approx) =
        close { backend; fenv; cenv; mutable_vars }
          (Lfunction
             {
               kind = Curried;
               return = Pgenval;
    ...
    | (ufunct, Value_closure (fundesc, _approx_res)), uargs
      ...
      let body =
        Ugeneric_apply
          (direct_apply env ~loc ~attribute fundesc ufunct first_args,
           rem_args,
           dbg)
      in
      let result =
        List.fold_left
          (fun body (id, defining_expr) ->
             Ulet (Immutable, Pgenval, VP.create id, defining_expr, body)) body
          args
      in
  ...
  | Lsend (kind, met, obj, args, loc) ->
    ...
    let dbg = Debuginfo.from_location loc in
    Usend (kind, umet, uobj, close_list env args, dbg), Value_unknown
  | Llet (str, kind, id, lam, body) ->
    let (ulam, alam) = close_named env id lam in
    begin match alam with
    | Value_const _ when str = Alias || is_pure ulam ->
      close { backend; fenv = V.Map.add id alam fenv; cenv; mutable_vars } body
    | _ ->
      let (ubody, abody) =
  ...
  | Lletrec (defs, body) ->
    if List.for_all (function _id, Lfunction _ -> true | _ -> false) defs then
      (* Simple case: only function definitions *)
      let (clos, infos) = close_functions env defs in
      let clos_ident = V.create_local "clos" in
      let fenv_body =
        List.fold_right
          (fun (id, _pos, approx) fenv -> V.Map.add id approx fenv) infos fenv
      in
      let (ubody, approx) =
        close { backend; fenv = fenv_body; cenv; mutable_vars } body
      in
      let sb =
        List.fold_right
          (fun (id, pos, _approx) sb ->
             V.Map.add id (Uoffset (Uvar clos_ident, pos)) sb) infos V.Map.empty
      in
      Ulet
        (Immutable,
         Pgenval,
         VP.create clos_ident,
         clos,
         substitute Debuginfo.none (backend, !Clflags.float_const_prop) sb None
           ubody),
      approx
    else (* General case: recursive definition of values *)
      let rec clos_defs =
        function
        | [] -> [], fenv
        | (id, lam) :: rem ->
          ...
          (VP.create id, ulam) :: udefs, V.Map.add id approx fenv_body
      in
  ...
  | Lprim (Pctconst c, [ arg ], _loc) ->
    let (cst, approx) =
      ...
      | Backend_type -> make_const_int 0
    (* tag 0 is the same as Native here *)
    in
  ...
  | Lprim (Pgetglobal id, [], loc) ->
    let dbg = Debuginfo.from_location loc in
    check_constant_result (getglobal dbg id) (Compilenv.global_approx id)
  ...
  | Lprim
      (Psetfield (n, is_ptr, init), [ Lprim (Pgetglobal id, [], _); lam ], loc)
    ->
    let (ulam, approx) = close env lam in
    (if approx <> Value_unknown then !global_approx.(n) <- approx);
    let dbg = Debuginfo.from_location loc in
  ...
  | Lprim (Praise k, [ arg ], loc) ->
    ...
    let dbg = Debuginfo.from_location loc in
    Uprim (P.Praise k, [ ulam ], dbg), Value_unknown
  | Lprim (Pmakearray _, [], _loc) -> make_const_ref (Uconst_block (0, []))
  | Lprim (p, args, loc) ->
    let p = Convert_primitives.convert p in
    let dbg = Debuginfo.from_location loc in
    simplif_prim ~backend !Clflags.float_const_prop p
      (close_list_approx env args) dbg
  | Lswitch (arg, sw, dbg) ->
    let fn fail =
      ...
      and (block_index, block_actions, fblock) =
        close_switch env sw.sw_blocks sw.sw_numblocks fail
      in
      let ulam =
        Uswitch
          (uarg,
      ...
      in
      fconst (fblock ulam), Value_unknown
    in
    ...
    | Some lamfail ->
      ...
      then
        ...
        and (uhandler, _) = close env lamfail
        in
  ...
  | Lifthenelse (arg, ifso, ifnot) ->
    ...
    | uarg, Value_const (Uconst_int n) ->
      sequence_constant_expr uarg (close env (if n = 0 then ifnot else ifso))
    | uarg, _ ->
...
and close_functions { backend; fenv; cenv; mutable_vars } fun_defs =
  ...
  let initially_closed =
    !function_nesting_depth < excessive_function_nesting_depth
  in
  (* Determine the free variables of the functions *)
  let fv = V.Set.elements (free_variables (Lletrec (fun_defs, lambda_unit))) in
  ...
  let uncurried_defs =
    List.map
      (function
       ...
       | _, _ -> fatal_error "Closure.close_functions") fun_defs
  in
  ...
  let clos_fundef (id, params, return, body, fundesc, dbg) env_pos =
    ...
    let cenv_fv = build_closure_env env_param (fv_pos - env_pos) fv in
    let cenv_body =
      List.fold_right2
    ...
    let fun_params =
      if !useless_env then params else params @ [ env_param, Pgenval ]
    in
    let f =
      {
    ...
    let n =
      List.fold_left
        (fun n (id, _) -> n + (if V.name id = "*opt*" then 8 else 1)) 0
        fun_params
    in
    let threshold =
    ...
    (if lambda_smaller ubody threshold then
       fundesc.fun_inline <- Some (fun_params, ubody));
    f, (id, env_pos, Value_closure (fundesc, approx))
  in
  (* Translate all function definitions. *)
  let clos_info_list =
    if initially_closed then
      ...
      | NotClosed ->
        ...
        List.iter
          (fun (_id, _params, _return, _body, fundesc, _dbg) ->
             ...
             fundesc.fun_inline <- None) uncurried_defs;
        useless_env := false;
        List.map2 clos_fundef uncurried_defs clos_offsets
    else (* Excessive closure nesting: assume environment parameter is used *)
      List.map2 clos_fundef uncurried_defs clos_offsets
  in
  ...
  Uclosure (clos, List.map (close_var { backend; fenv; cenv; mutable_vars }) fv),
  infos
...
and close_switch env cases num_keys default =
  ...
  and store = Storer.mk_store ()
  in
  (* First default case *)
  begin match default with
  | Some def when ncases < num_keys -> assert (store.act_store () def = 0)
  | _ -> ()
  end;
...
let collect_exported_structured_constants a =
  ...
  and ulam =
    ...
    | Uclosure (fl, ul) ->
      ...
      List.iter ulam ul
    | Uoffset (u, _) -> ulam u
    | Ulet (_str, _kind, _, u1, u2) ->
      ulam u1;
      ulam u2
    | Uphantom_let _ -> no_phantom_lets ()
    | Uletrec (l, u) ->
      List.iter (fun (_, u) -> ulam u) l;
      ulam u
    ...
    | Uwhile (u1, u2)
      ...
      ulam u1;
      ulam u2
    | Uifthenelse (u1, u2, u3) | Ufor (_, u1, u2, _, u3) ->
      ulam u1;
      ulam u2;
      ulam u3
    | Uassign (_, u) -> ulam u
    ...
    close
      {
        ...
        fenv = V.Map.empty;
        cenv = V.Map.empty;
        mutable_vars = V.Set.empty
      } lam
  in
  let opaque =
    !Clflags.opaque || Env.is_imported_opaque (Compilenv.current_unit_name ())
  in
  (if opaque then
