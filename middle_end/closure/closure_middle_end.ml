...
let raw_clambda_dump_if
    ppf (ulambda, _, structured_constants : Clambda.with_constants)
=
     ...
     (Format.fprintf ppf "@.clambda:@.";
      ...
      List.iter
        (fun { Clambda.symbol; definition; _ } ->
           Format.fprintf ppf "%s:@ %a@." symbol
             Printclambda.structured_constant definition) structured_constants));
  if !Clflags.dump_cmm then Format.fprintf ppf "@.cmm:@."

let lambda_to_clambda
    ~backend ~prefixname:_ ~ppf_dump (lambda : Lambda.program)
=
  let clambda =
  ...
  let clambda_and_constants = clambda, [ preallocated_block ], constants in
