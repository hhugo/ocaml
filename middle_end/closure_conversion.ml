...
let add_default_argument_wrappers lam =
  let defs_are_all_functions (defs : (_ * Lambda.lambda) list) =
    List.for_all
      (function _, Lambda.Lfunction _ -> true | _ -> false)
      defs
  in
  let f (lam : Lambda.lambda) : Lambda.lambda =
    ...
    | Lletrec (defs, body) as lam ->
        if defs_are_all_functions defs
        then
          let defs =
            List.flatten
              (List.map
...
let tupled_function_call_stub original_params unboxed_version
    ~closure_bound_var : Flambda.function_declaration =
...
let rec declare_const t (const : Lambda.structured_constant) :
  ...
  | Const_immstring c ->
      register_const
        t
        (Allocated_const (Immutable_string c))
        Names.const_immstring
  | Const_float_array c ->
      register_const
        t
...
let lambda_const_bool b : Lambda.structured_constant =
  if b then Const_pointer 1 else Const_pointer 0
...
let rec close t env (lam : Lambda.lambda) : Flambda.t =
  ...
  | Lvar id -> (
    ...
    | exception Not_found -> (
      ...
      | exception Not_found ->
          Misc.fatal_errorf
            "Closure_conversion.close: unbound identifier %a"
            Ident.print
            id ) )
  | Lconst cst ->
  ...
  | Lapply
      ...
      ; ap_args
      ; ap_loc
      ; ap_should_be_tailcall = _
      ; ap_inlined
      ; ap_specialised } ->
      Lift_code.lifting_helper
        (close_list t env ap_args)
            ...
            | let_rec_ident, Lambda.Lfunction {kind; params; body; attr; loc}
              ->
                ...
                let function_declaration =
                  Function_decl.create
                    ~let_rec_ident:(Some let_rec_ident)
                    ~closure_bound_var
                    ~kind
                    ~params:(List.map fst params)
                    ~body
  ...
  | Lprim (Pctconst c, [arg], _loc) ->
      ...
      let const =
        ...
        | Ostype_win32 ->
            lambda_const_bool (String.equal Sys.os_type "Win32")
        | Ostype_cygwin ->
            lambda_const_bool (String.equal Sys.os_type "Cygwin")
        | Backend_type -> Lambda.Const_pointer 0
  ...
  | Lprim (Psetfield (_, _, _), [Lprim (Pgetglobal _, [], _); _], _) ->
      Misc.fatal_errorf
        "[Psetfield (Pgetglobal ...)] is forbidden upon entry to the \
         middle end"
      ...
      Flambda.create_let
        ...
        (Switch
           ...
           , { numconsts = nums sw.sw_numconsts sw.sw_consts sw.sw_failaction
             ...
             ; numblocks =
                 nums sw.sw_numblocks sw.sw_blocks sw.sw_failaction
  ...
  | Ltrywith (body, id, handler) ->
      let var = Variable.create_with_same_name_as_ident id in
      Try_with
        (close t env body, var, close t (Env.add_var env id var) handler)
...
let lambda_to_flambda ~backend ~module_ident ~size ~filename lam :
  ...
  let program_body =
    List.fold_left
      (fun program_body (symbol, constant) ->
        (Let_symbol (symbol, constant, program_body) : Flambda.program_body)
        )
      module_initializer
      t.declared_symbols
  in
