...
let rec accumulate ~substitution ~copied_lets ~extracted_lets
  ...
  | Let {var; defining_expr = named; body; _} ->
      let extracted =
        ...
        match named with
        | Prim
            (Pmakeblock (tag, Asttypes.Immutable, _value_kind), args, _dbg)
          ->
...
let rebuild_expr
    ...
    ~(substitute : bool) (expr : Flambda.t) =
  ...
  let substitution =
    if substitute
    then Variable.Map.of_set (fun x -> Variable.rename x) free_variables
        ...
        | Expr (var, decl) ->
            let expr =
              rebuild_expr
                ...
                decl
            in
            if Variable.Set.mem var used_variables
            then Initialisation (symbol, Tag.create_exn 0, [expr])
            else Effect expr
        | Exprs (_vars, decl) ->
            let expr =
              rebuild_expr
...
let rec split_program (program : Flambda.program_body) :
