...
and same_set_of_closures (c1 : Flambda.set_of_closures)
  ...
  Variable.Map.equal
    sameclosure
    c1.function_decls.funs
    c2.function_decls.funs
  && Variable.Map.equal
       Flambda.equal_specialised_to
       c1.free_vars
       c2.free_vars
  && Variable.Map.equal
       Flambda.equal_specialised_to
...
let make_closure_declaration ~is_classic_mode ~id ~body ~params ~stub :
  ...
  if not (Variable.Set.subset param_set free_variables)
  then Misc.fatal_error "Flambda_utils.make_closure_declaration";
  let sb =
    Variable.Set.fold
  ...
  let set_of_closures_var =
    Variable.create
      Internal_variable_names.set_of_closures
  ...
  let rec loop (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (decls, program) ->
        List.fold_left
          ...
          (loop program)
          decls
    | Initialize_symbol (_, _, _, program) | Effect (_, program) ->
        loop program
    | End _ -> []
  in
...
let substitute_read_symbol_field_for_variables
  ...
  let f (expr : Flambda.t) : Flambda.t =
    ...
    | Let ({var = v; defining_expr = named; _} as let_expr) ->
        ...
        in
        if Variable.Set.is_empty to_substitute
        then expr
        else
          let bindings =
            Variable.Map.of_set
              (fun var -> Variable.rename var)
              to_substitute
          in
...
let closures_required_by_entry_point ~(entry_point : Closure_id.t)
  ...
  while not (Queue.is_empty queue) do
    ...
    | fun_dependencies ->
        Variable.Set.iter
          (fun dep ->
            if Variable.Map.mem dep function_decls.funs then add dep )
          fun_dependencies
  done;
  !set
  ...
  Variable.Map.map
    (fun (spec_to : Flambda.specialised_to) ->
      ...
      | Some projection ->
          ...
          if Variable.Map.mem from which_variables
          then spec_to
  ...
  Variable.Map.map
    (fun ({params; _} : Flambda.function_declaration) ->
      List.map
        (fun param ->
          match
            Variable.Map.find (Parameter.var param) specialised_args
          with
