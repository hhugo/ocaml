  ...
  let assign_symbol var (named : Flambda.named) =
    ...
    then
      ...
      | Set_of_closures
          ...
          assert (
            not (Inconstant_idents.closure set_of_closures_id inconstants)
          );
          assign_symbol ();
      ...
      | Prim (Pfield _, _, _) ->
          Misc.fatal_errorf
            "[Pfield] with the wrong number of arguments"
            Flambda.print_named
            named
      ...
      | Prim _ ->
          Misc.fatal_errorf
            "Primitive not expected to be constant: @.%a@."
            Flambda.print_named
            named
  ...
  let record_set_of_closure_equalities
    ...
    Variable.Map.iter
      (fun arg (var : Flambda.specialised_to) ->
        if not (Inconstant_idents.variable arg inconstants)
        then
...
let variable_field_definition (var_to_symbol_tbl : Symbol.t Variable.Tbl.t)
  ...
  with Not_found -> (
    ...
    | const_defining_value ->
        Misc.fatal_errorf
          "Unexpected pattern for a constant: %a: %a"
          Variable.print
          var
    ...
    | exception Not_found ->
        Misc.fatal_errorf
          "No associated symbol for the constant %a"
          Variable.print
          var )
...
let translate_constant_set_of_closures
  ...
  Symbol.Map.map
    (fun (const : Flambda.constant_defining_value) ->
      match const with
      | Flambda.Allocated_const _ | Flambda.Block _
...
let translate_definition_and_resolve_alias inconstants
  ...
  let resolve_float_array_involving_variables
    ...
    let find_float_var_definition var =
      ...
      | const_defining_value ->
          Misc.fatal_errorf
            "Bad definition for float array member %a: %a"
            Variable.print
            var
            Alias_analysis.print_constant_defining_value
            const_defining_value
          ...
          | exception Not_found -> (
              ...
              | Value_float_array value_float_array -> (
                  ...
                  | None ->
                      Misc.fatal_errorf
                        "Lift_constants.translate_definition_and_resolve_alias: \
                         Duplicate Pfloatarray %a with not completely \
                         known float array from symbol: %a"
                        Variable.print
                        var
                        Alias_analysis.print_constant_defining_value
                        definition
...
let program_graph ~backend imported_symbols symbol_to_constant
  ...
  let graph_with_only_constant_parts =
    Symbol.Map.map
      (fun const ->
        Symbol.Set.diff
          (constant_dependencies ~backend const)
          imported_symbols )
      symbol_to_constant
  in
...
let introduce_free_variables_in_set_of_closures
    ...
    ( { Flambda.function_decls
      ; free_vars
      ; specialised_args
      ; direct_call_surrogates } as set_of_closures ) =
      ...
      ~funs:
        (Variable.Map.map
           (fun (func_decl : Flambda.function_declaration) ->
             ...
             in
             if Variable.Map.is_empty subst
             then func_decl
             else (
               done_something := true;
...
let lift_constants (program : Flambda.program) ~backend =
  ...
  let inconstants =
    Inconstant_idents.inconstants_on_program
      program
      ~backend
    ...
    Symbol.Map.union
      (fun _sym
        ...
        | ( Project_closure (s1, closure_id1)
          , Project_closure (s2, closure_id2) ) ->
            Format.eprintf
              "not equal project closure@. s %a %a@. cid %a %a@."
              Symbol.print
              s1
              Symbol.print
              s2
