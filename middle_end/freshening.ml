...
let apply_variable t var =
  ...
  | Inactive -> var
  | Active t -> (
    try Variable.Map.find var t.sb_var with Not_found -> var )
      ...
      let closure_symbols =
        Variable.Map.fold
          (fun var _ map ->
            ...
            if Symbol.Set.mem sym all_free_symbols
            then (
...
module Project_var = struct
  ...
  let print ppf t =
    Format.fprintf
      ppf
      "{ vars_within_closure %a, closure_id %a }"
  ...
  let func_decls_subst t (subst : subst)
    ...
    | Active subst ->
        let subst_func_decl _fun_id
            (func_decl : Flambda.function_declaration) subst =
          let params, subst =
            active_add_parameters' subst func_decl.params
          in
        ...
        let funs, subst =
          Variable.Map.fold
            (fun orig_id func_decl (funs, subst) ->
              let func_decl, subst =
                subst_func_decl orig_id func_decl subst
              in
              let id =
                if only_freshen_parameters
                then orig_id
  ...
  module Compose (T : Identifiable.S) = struct
    let compose ~earlier ~later =
      if (T.Map.equal T.equal) earlier later || T.Map.cardinal later = 0
      then earlier
      else
        T.Map.mapi
          (fun src_var var ->
            if T.Map.mem src_var later
            then
              Misc.fatal_errorf
                "Freshening.Project_var.compose: domains of substitutions \
                 must be disjoint.  earlier=%a later=%a"
                (T.Map.print T.print)
                earlier
                (T.Map.print T.print)
                later;
  ...
  Variable.Map.map
    (fun (spec_to : Flambda.specialised_to) ->
      let projection =
        ...
        | Some projection ->
            Some
              (freshen_projection projection ~freshening ~closure_freshening)
      in
