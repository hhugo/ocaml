...
let apply_on_subexpressions f f_named (flam : Flambda.t) =
  ...
  | Var _ | Apply _ | Assign _ | Send _ | Proved_unreachable
   |Static_raise _ ->
      ()
  | Let {defining_expr; body; _} -> f_named defining_expr; f body
  ...
  | String_switch (_, sw, def) ->
      ...
      Misc.may f def
  | Static_catch (_, _, f1, f2) -> f f1; f f2
  | Try_with (f1, _, f2) -> f f1; f f2
  | If_then_else (_, f1, f2) -> f f1; f f2
  | While (f1, f2) -> f f1; f f2
  | For {body; _} -> f body
...
let iter_exprs_at_toplevel_of_program (program : Flambda.program) ~f =
  let rec loop (program : Flambda.program_body) =
    ...
    | Initialize_symbol (_, _, fields, program) ->
        List.iter f fields; loop program
    | Effect (expr, program) -> f expr; loop program
    | End _ -> ()
  in
...
let iter_constant_defining_values_on_program (program : Flambda.program) ~f
  ...
  let rec loop (program : Flambda.program_body) =
    match program with
    | Let_symbol (_, const, program) -> f const; loop program
    | Let_rec_symbol (defs, program) ->
            ...
            let funs =
              Variable.Map.map
                (fun (func_decl : Flambda.function_declaration) ->
                  ...
                  if new_body == func_decl.body
                  then func_decl
                  else (
                    done_something := true;
                    Flambda.update_function_declaration
                      func_decl
  ...
  map_named
    (function
      ...
      | Read_symbol_field (sym, field) as named ->
          ...
          if new_sym == sym
          then named
          else Read_symbol_field (new_sym, field)
...
let map_symbols_on_set_of_closures
    ( { Flambda.function_decls
      ; free_vars
      ; specialised_args
      ; direct_call_surrogates } as set_of_closures ) ~f =
  let done_something = ref false in
  let funs =
    Variable.Map.map
      (fun (func_decl : Flambda.function_declaration) ->
  ...
  map_named
    (function
      ...
      | ( Symbol _ | Const _ | Allocated_const _ | Set_of_closures _
        ...
        | Expr _ | Read_mutable _ | Read_symbol_field _ ) as named ->
          named)
    tree
  ...
  let funs =
    Variable.Map.map
      (fun (function_decl : Flambda.function_declaration) ->
        ...
        if new_body == function_decl.body
        then function_decl
        else (
          done_something := true;
          Flambda.update_function_declaration
  ...
  let rec loop (program : Flambda.program_body) : Flambda.program_body =
    let map_constant_set_of_closures
      ...
      let function_decls =
        ...
        in
        if not !done_something
        then set_of_closures.function_decls
        else
          Flambda.update_function_declarations
            set_of_closures.function_decls
            ~funs
    ...
    | Let_symbol (symbol, Set_of_closures set_of_closures, program') ->
        ...
        then program
          ...
          Let_symbol
            (symbol, Set_of_closures new_set_of_closures, new_program')
            ...
            (function
              | var, Flambda.Set_of_closures set_of_closures ->
                  let new_set_of_closures =
                    map_constant_set_of_closures set_of_closures
                  in
                  if not (new_set_of_closures == set_of_closures)
                  then done_something := true;
                  var, Flambda.Set_of_closures new_set_of_closures
              | def -> def)
            defs
        in
        let new_program' = loop program' in
        if new_program' == program' && not !done_something
        then program
    ...
    | Effect (expr, program') ->
        ...
        let new_program' = loop program' in
        if new_expr == expr && new_program' == program'
        then program
