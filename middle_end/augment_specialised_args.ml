...
module What_to_specialise = struct
  ...
  let make_direct_call_surrogate_for t ~fun_var =
    ...
    | exception Not_found ->
        Misc.fatal_errorf
          "use_direct_call_surrogate_for: %a is not a fun_var from the \
...
module Processed_what_to_specialise = struct
  ...
  let lift_projection t ~(projection : Projection.t) =
    ...
    let find_outer_var inner_var =
      match
        Variable.Map.find inner_var t.set_of_closures.specialised_args
      with
      | (outer_var : Flambda.specialised_to) -> outer_var.var
      | exception Not_found ->
          Misc.fatal_errorf
            "find_outer_var: expected %a to be in [specialised_args], but \
             it is not.  The projection was: %a.  Set of closures: %a"
            Variable.print
            inner_var
            Projection.print
            projection
  ...
  let new_specialised_arg t ~fun_var ~group ~definition =
    ...
    then t
      ...
      really_add_new_specialised_arg t ~group ~definition ~for_one_function
    ...
    let existing_definitions_via_spec_args_indexed_by_fun_var =
      Variable.Map.map
        (fun (function_decl : Flambda.function_declaration) ->
          ...
          then Definition.Set.empty
          else
            let params = Parameter.Set.vars function_decl.params in
            Variable.Map.fold
              (fun inner_var (spec_to : Flambda.specialised_to) definitions ->
                if not (Variable.Set.mem inner_var params)
                then definitions
                else
      ...
      (fun group fun_vars_and_definitions t ->
        ...
        let some_function_has_too_many_args =
          Variable.Map.exists
            ...
            t.functions
        in
        if some_function_has_too_many_args
        then original_t (* drop this group *)
        else t )
      by_group
      t
end
      ...
      (fun fun_var (function_decl : Flambda.function_declaration) ->
        ...
        Variable.Map.iter
          (fun inner_var (outer_var : Flambda.specialised_to) ->
            if Variable.Set.mem inner_var params
            then (
              assert (
                not
                  (Variable.Set.mem
              ...
              | Some projection ->
                  ...
                  if not (Variable.Set.mem from params)
                  then
                    Misc.fatal_errorf
                      "Augment_specialised_args (%s): specialised argument \
                       (%a -> %a) references a projection variable that is \
                       not a specialised argument of the function %a. @ \
                       The set of closures before the transformation \
                       was:@  %a. @ The set of closures after the \
                       transformation was:@ %a."
                      pass_name
                      Variable.print
                      inner_var
                      Flambda.print_specialised_to
                      outer_var
                      Variable.print
                      fun_var
                      Flambda.print_set_of_closures
...
module Make (T : S) = struct
  ...
  let create_wrapper ~(for_one_function : P.for_one_function) ~benefit =
    ...
    let find_wrapper_param param =
      ...
      | exception Not_found ->
          Misc.fatal_errorf
            "find_wrapper_param: expected %a to be in [params_renaming], \
             but it is not."
            Variable.print
            param
    in
          ...
          let definition : Definition.t =
            ...
            | Projection_from_existing_specialised_arg projection ->
                Projection_from_existing_specialised_arg
                  (Projection.map_projecting_from
                     projection
  ...
  let rewrite_function_decl (t : P.t) ~env ~duplicate_function
    ...
    then None
      ...
      Some
        (funs, free_vars, specialised_args, direct_call_surrogates, benefit)
