...
let rec structured_constant ppf =
  function
...
and one_fun ppf f =
  let idents ppf =
    List.iter
      (fun (x, k) -> fprintf ppf "@ %a%a" VP.print x Printlambda.value_kind k)
  in
  fprintf ppf "(fun@ %s%s@ %d@ @[<2>%a@]@ @[<2>%a@])" f.label
    (value_kind f.return) f.arity idents f.params lam f.body

and phantom_defining_expr ppf =
  function
  ...
  | Uphantom_block { tag; fields } ->
    ...
    List.iter (fun field -> Format.fprintf ppf "%a; " Backend_var.print field)
      fields;
    Format.fprintf ppf "]"

and phantom_defining_expr_opt ppf =
  function
...
and lam ppf =
    ...
    let bindings ppf id_arg_list =
      ...
      List.iter
        (fun (id, l) ->
           ...
           fprintf ppf "@[<2>%a@ %a@]" VP.print id lam l) id_arg_list
    in
  ...
  | Ustringswitch (larg, sw, d) ->
    let switch ppf sw =
      ...
      List.iter
        (fun (s, l) ->
           (if !spc then fprintf ppf "@ " else spc := true);
           fprintf ppf "@[<hv 1>case \"%s\":@ %a@]" (String.escaped s) lam l) sw;
      match d with
      | Some d ->
        (if !spc then fprintf ppf "@ " else spc := true);
  ...
  | Ucatch (i, vars, lbody, lhandler) ->
    fprintf ppf "@[<2>(catch@ %a@;<1 -1>with (%d%a)@ %a)@]" lam lbody i
      (fun ppf vars ->
         List.iter
           (fun (x, k) ->
              fprintf ppf " %a%a" VP.print x Printlambda.value_kind k) vars)
      vars lam lhandler
  ...
  | Usend (k, met, obj, largs, _) ->
    ...
    let kind =
      if k = Lambda.Self then
        "self"
      else if k = Lambda.Cached then
        "cache"
      else
        ""
    in
...
let rec approx ppf =
  ...
  | Value_closure (fundesc, a) ->
    Format.fprintf ppf "@[<2>function %s@ arity %i" fundesc.fun_label
      fundesc.fun_arity;
