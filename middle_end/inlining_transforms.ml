...
let copy_of_function's_body_with_freshened_params env
  ...
  if E.does_not_bind env param_vars && E.does_not_freshen env param_vars
  then params, function_body.body
  else
...
let inline_by_copying_function_body ~env ~r ~lhs_of_application
  ...
  let body =
    ...
    let default_specialise =
      Lambda.equal_specialise_attribute
  ...
  let bindings_for_vars_bound_by_closure_and_params_to_args =
    ...
    fold_over_projections_of_vars_bound_by_closure
      ...
      ~lhs_of_application
      ~bound_variables
      ~init:bindings_for_params_to_args
...
(* For arguments of specialised parameters:
   ...
   - Add them and their invariant aliases to [old_params_to_new_outside] *)
let register_arguments ~specialised_args ~invariant_params ~state ~params
    ~args ~args_approxs =
    ...
    | param :: params, arg :: args, arg_approx :: args_approxs ->
          ...
          | Some (spec : Flambda.specialised_to) ->
              let old_outside_to_new_outside =
                Variable.Map.add
                  spec.var
                  arg
                  state.old_outside_to_new_outside
              in
        ...
        let old_params_to_new_outside =
          ...
          then
            ...
            in
            match
              Variable.Map.find_opt param (Lazy.force invariant_params)
            with
            | Some set ->
                Variable.Set.fold
...
let add_param ~specialised_args ~state ~param =
  ...
  let state =
    { state with
      old_inside_to_new_inside; new_specialised_args_with_old_projections }
  in
...
let add_fun_var ~lhs_of_application ~closure_id_being_applied ~state
    ~fun_var =
  if Variable.Map.mem fun_var state.old_inside_to_new_inside
  then state
  else
...
let add_free_var ~free_vars ~state ~free_var =
  if Variable.Map.mem free_var state.old_inside_to_new_inside
  then state
  else
    let spec : Flambda.specialised_to =
      Variable.Map.find free_var free_vars
    in
...
let specialisable_call ~specialised_args ~state ~args ~params =
  List.for_all2
    (fun arg param ->
      let param = Parameter.var param in
      if Variable.Map.mem param specialised_args
      then true
      else
      ...
      (fun var state ->
        ...
        then
          add_fun_var
            ...
            ~state
            ~fun_var:var
        else if Variable.Map.mem var free_vars
        then add_free_var ~free_vars ~state ~free_var:var
        else state )
      function_body.free_variables
      state
  in
