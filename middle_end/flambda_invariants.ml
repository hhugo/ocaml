...
let variable_and_symbol_invariants (program : Flambda.program) =
  ...
  let declare_variable var =
    if Variable.Set.mem var !all_declared_variables
    then raise (Binding_occurrence_of_variable_already_bound var);
  ...
  let declare_mutable_variable mut_var =
    if Mutable_variable.Set.mem mut_var !all_declared_mutable_variables
    then
  ...
  let add_binding_occurrence (var_env, mut_var_env, sym_env) var =
    ...
    if not (Variable.in_compilation_unit var compilation_unit)
    then raise (Binding_occurrence_not_from_current_compilation_unit var);
  ...
  let add_mutable_binding_occurrence (var_env, mut_var_env, sym_env) mut_var
      =
    let compilation_unit = Compilation_unit.get_current_exn () in
    if not (Mutable_variable.in_compilation_unit mut_var compilation_unit)
    then
      raise
  ...
  let add_binding_occurrence_of_symbol (var_env, mut_var_env, sym_env) sym =
    if Symbol.Set.mem sym sym_env
    then raise (Binding_occurrence_of_symbol_already_bound sym)
    else var_env, mut_var_env, Symbol.Set.add sym sym_env
  in
  ...
  let check_mutable_variable_is_bound (_, mut_var_env, _) mut_var =
    if not (Mutable_variable.Set.mem mut_var mut_var_env)
    then raise (Unbound_mutable_variable mut_var)
  in
  ...
  and loop_set_of_closures env
    ...
    let variables_in_closure =
      Variable.Map.fold
        (fun var
             (var_in_closure : Flambda.specialised_to)
    ...
    let all_params, all_free_vars =
      Variable.Map.fold
        (fun fun_var function_decl acc ->
          ...
          if not (Variable.Set.subset free_variables' free_variables)
          then
            raise
          ...
          in
          if not (Variable.Set.is_empty bad)
          then
            raise
          ...
          if all_params_size <> old_all_params_size + params_size
          then raise (Function_decls_have_overlapping_parameters all_params);
...
*)
    ...
    Variable.Map.iter
      (fun _inner_var (specialised_to : Flambda.specialised_to) ->
        ...
        | Some projection ->
            ...
            then raise (Projection_must_be_a_free_var projection) )
      free_vars;
    Variable.Map.iter
      (fun being_specialised (specialised_to : Flambda.specialised_to) ->
        if not (Variable.Set.mem being_specialised all_params)
        then
        ...
        | Some projection ->
            ...
            then raise (Projection_must_be_a_specialised_arg projection) )
      specialised_args
  in
  let loop_constant_defining_value env
    ...
    | Flambda.Set_of_closures set_of_closures ->
        ...
        if not (Variable.Map.is_empty set_of_closures.free_vars)
        then assert false;
        (* TODO: correct error *)
        if not (Variable.Map.is_empty set_of_closures.specialised_args)
        then assert false (* TODO: correct error *)
  ...
  let rec loop_program_body env (program : Flambda.program_body) =
    ...
    | Let_rec_symbol (defs, program) ->
        ...
        in
        List.iter
          (fun (_, def) -> loop_constant_defining_value env def)
          defs;
...
let every_declared_closure_is_from_current_compilation_unit flam =
  ...
  Flambda_iterators.iter_on_sets_of_closures
    (fun {Flambda.function_decls; _} ->
      ...
      in
      if not
           (Compilation_unit.equal compilation_unit current_compilation_unit)
...
let used_closure_ids (program : Flambda.program) =
  ...
  let f (flam : Flambda.named) =
    ...
    | Project_var {closure = _; closure_id; var = _} ->
        used := Closure_id.Set.add closure_id !used
    | Set_of_closures _ | Symbol _ | Const _ | Allocated_const _ | Prim _
...
let every_static_exception_is_caught flam =
  ...
  let rec loop env (flam : Flambda.t) =
    ...
    | Static_catch (i, _, body, handler) ->
        ...
        loop env handler; loop env body
    | exp ->
        check env exp;
        Flambda_iterators.apply_on_subexpressions
          (loop env)
          (fun (_ : Flambda.named) -> ())
          exp
  in
...
let every_static_exception_is_caught_at_a_single_position flam =
  ...
  let f (flam : Flambda.t) =
    ...
    | Static_catch (i, _, _body, _handler) ->
        ...
        then raise (Static_exception_caught_in_multiple_places i);
...
let _every_move_within_set_of_closures_is_to_a_function_in_the_free_vars
  ...
  Flambda_iterators.iter_named_of_program program ~f:(function
      | Move_within_set_of_closures {start_from; move_to; _} ->
          let moved_to =
            try Closure_id.Map.find start_from !moves
            with Not_found -> Closure_id.Set.empty
          in
          moves :=
            Closure_id.Map.add
              start_from
        ...
        (fun fun_var {Flambda.free_variables; _} ->
          ...
          | moved_to ->
              let missing_dependencies =
                Variable.Set.diff
                  (Closure_id.unwrap_set moved_to)
                  free_variables
              in
              if not (Variable.Set.is_empty missing_dependencies)
              then
                raise
...
let check_exn ?(kind = Normal) (flam : Flambda.program) =
  ...
  with exn ->
    ...
    | Binding_occurrence_not_from_current_compilation_unit var ->
        Format.eprintf
          ">> Binding occurrence of variable marked as not being from the \
           current compilation unit: %a"
          Variable.print
          var
    | Mutable_binding_occurrence_not_from_current_compilation_unit mut_var
      ->
        Format.eprintf
          ">> Binding occurrence of mutable variable marked as not being \
    ...
    | Binding_occurrence_of_mutable_variable_already_bound mut_var ->
        Format.eprintf
          ">> Binding occurrence of mutable variable that was already \
           bound: %a"
          Mutable_variable.print
          mut_var
    ...
    | Vars_in_function_body_not_bound_by_closure_or_params
        ...
        Format.eprintf
          ">> Variable(s) (%a) in the body of a function declaration \
           (fun_var = %a) that is not bound by either the closure or the \
           function's parameter list.  Set of closures: %a"
          Variable.Set.print
          vars
          Variable.print
          fun_var
    ...
    | Function_decls_have_overlapping_parameters vars ->
        Format.eprintf
          ">> Function declarations whose parameters overlap: %a"
          Variable.Set.print
          vars
    | Specialised_arg_that_is_not_a_parameter var ->
        Format.eprintf
          ">> Variable in [specialised_args] that is not a parameter of \
           any of the function(s) in the corresponding declaration(s): %a"
          Variable.print
          var
    | Projection_must_be_a_free_var var ->
        Format.eprintf
          ">> Projection %a in [free_vars] from a variable that is not a \
           (inner) free variable of the set of closures"
          Projection.print
          var
    | Projection_must_be_a_specialised_arg var ->
        Format.eprintf
          ">> Projection %a in [specialised_args] from a variable that is \
    ...
    | Free_variables_set_is_lying (var, claimed, calculated, function_decl)
      ->
        Format.eprintf
          ">> Function declaration whose [free_variables] set (%a) is not \
           a superset of the result of [Flambda.free_variables] applied to \
           the body of the function (%a).  Declaration: %a"
          Variable.Set.print
          claimed
          Variable.Set.print
          calculated
    ...
    | Set_of_closures_free_vars_map_has_wrong_range vars ->
        Format.eprintf
          ">> [free_vars] map in set of closures has in its range \
    ...
    | Unbound_vars_within_closures vars_within_closures ->
        Format.eprintf
          ...
          vars_within_closures
    | Static_exception_not_caught static_exn ->
        Format.eprintf
          ">> Uncaught static exception: %a"
          Static_exception.print
          static_exn
    | Static_exception_caught_in_multiple_places static_exn ->
        Format.eprintf
          ">> Static exception caught in multiple places: %a"
          Static_exception.print
          static_exn
    | Move_to_a_closure_not_in_the_free_variables (start_from, move_to) ->
        Format.eprintf
          ">> A Move_within_set_of_closures from the closure %a to \
           closures that are not parts of its free variables: %a"
          Variable.print
          start_from
          Variable.Set.print
          move_to
