        ...
        | exception Not_found ->
            Misc.fatal_errorf
              "(UFV) find_outer_var: expected %a to be in \
               [existing_inner_to_outer_vars], but it is not.  (The \
               projection was: %a)"
              Variable.print
              inner_var
              Projection.print
...
let run ~env ~(set_of_closures : Flambda.set_of_closures) =
  if not !Clflags.unbox_free_vars_of_closures
  then None
  else
    let definitions_indexed_by_new_inner_vars, _, free_vars, done_something
      ...
      Flambda_iterators.fold_function_decls_ignoring_stubs
        ...
        ~f:(fun ~fun_var:_ ~function_decl result ->
          let extracted =
            Extract_projections.from_function_decl
              ~env
              ~function_decl
    ...
    then None
      ...
      if num_free_vars_after > 2 * num_free_vars_before
      then None
      else
        let set_of_closures =
