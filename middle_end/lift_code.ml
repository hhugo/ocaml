...
let rebuild_let
  ...
  List.fold_left
    (fun body (var, def) -> W.create_let_reusing_defining_expr var def body)
    body
    defs
...
and extract acc (expr : Flambda.t Flambda.With_free_variables.t) =
  ...
  | Let let_expr -> extract_lets acc let_expr
  | _ -> acc, expr
