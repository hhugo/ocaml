...
module Env = struct
  ...
  let inlining_level_up env =
    ...
    in
    if env.inlining_level + 1 > max_level
    then Misc.fatal_error "Inlining level increased above maximum";
  ...
  let find_or_load_symbol t symbol =
    ...
    | exception Not_found ->
        ...
        then
          Misc.fatal_errorf
            "Symbol %a from the current compilation unit is unbound.  \
  ...
  let find_with_scope_exn t id =
    ...
    with Not_found ->
      Misc.fatal_errorf
        "Env.find_with_scope_exn: Unbound variable %a@.%s@. Environment: \
         %a@."
        Variable.print
        id
  ...
  let enter_closure t ~closure_id ~inline_inside ~dbg ~f =
    let t =
      if inline_inside && not t.never_inline_inside_closures
      then t
...
let keep_body_check ~is_classic_mode ~recursive =
  ...
  then fun _ _ -> true
    ...
    let can_inline_non_rec_function
    ...
    fun (var : Variable.t) (fun_decl : Flambda.function_declaration) ->
      if fun_decl.stub
      then true
      else if Variable.Set.mem var (Lazy.force recursive)
      then false
      else
...
let prepare_to_simplify_set_of_closures ~env
    ...
    Variable.Map.map
      (fun (external_var : Flambda.specialised_to) ->
        let var =
          ...
          in
          match
            A.simplify_var_to_var_using_env
              (E.find_exn env var)
  ...
  let free_vars, function_decls, sb, freshening =
    Freshening.apply_function_decls_and_free_vars
      ...
      free_vars
      function_decls
      ~only_freshen_parameters:(not freshen)
  in
  let env = E.set_freshening env sb in
  let free_vars =
    Freshening.freshen_projection_relation'
      free_vars
...
let prepare_to_simplify_closure
    ...
    ~specialised_args ~parameter_approximations ~set_of_closures_env =
  let closure_env =
    ...
    Variable.Map.fold
      (fun inner_var spec_arg env ->
        ...
        | Some projection ->
            ...
            if Variable.Set.mem from function_decl.free_variables
            then E.add_projection env ~projection ~bound_to:inner_var
            else env )
      which_variables
      closure_env
  in
  let closure_env =
    add_projections
      ~closure_env
      ~which_variables:specialised_args
      ~map:(fun spec_to -> spec_to )
  in
  add_projections
    ~closure_env
    ~which_variables:free_vars
