...
type env_element = Name.t * Name.t (** relative name * complete name *)
...
let full_module_or_module_type_name env n =
  try List.assoc n env.env_modules
  with
  | Not_found -> full_module_type_name env n
...
let full_class_or_class_type_name env n =
  try List.assoc n env.env_classes
  with
  | Not_found -> full_class_type_name env n
...
let subst_class_type env t =
  let rec iter t =
    ...
    | Cty_constr (p, texp_list, ct) ->
      let new_p = Odoc_name.to_path (full_type_name env (Odoc_name.from_path p))
      in
