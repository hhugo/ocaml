...
let rec add_signature env root ?rel signat =
  ...
  let f env item =
    ...
    | Types.Sig_type (ident, _, _, _) ->
        { env with
          env_types = (rel_name ident, qualify ident) :: env.env_types }
    | Types.Sig_typext (ident, _, _, _) ->
        { env with
          env_extensions =
            (rel_name ident, qualify ident) :: env.env_extensions }
    ...
    | Types.Sig_class_type (ident, _, _, _) ->
        { env with
          env_class_types =
            (rel_name ident, qualify ident) :: env.env_class_types }
  in
  ...
  let rec iter t =
    ...
    then ()
      ...
      | Types.Tpackage (p, n, l) ->
          let new_p =
            Odoc_name.to_path
              (full_module_type_name env (Odoc_name.from_path p))
          in
...
let subst_module_type env t =
  let rec iter t =
    ...
    | Types.Mty_alias _ | Types.Mty_signature _ -> t
