...
let string_of_variance t (co, cn) =
  if
    (t.Odoc_type.ty_kind = Odoc_type.Type_abstract ||
...
let string_of_type_list ?par sep type_list =
  ...
  in
  Printf.sprintf "%s%s%s" (if par then "(" else "")
...
let string_of_type_param_list t =
  ...
  Printf.sprintf "%s%s%s" (if par then "(" else "")
    (raw_string_of_type_list ", "
       (List.map (fun (typ, co, cn) -> string_of_variance t (co, cn), typ)
          t.Odoc_type.ty_parameters)) (if par then ")" else "")
...
let string_of_class_type_param_list l =
  ...
  in
  Printf.sprintf "%s%s%s" (if par then "[" else "")
    (raw_string_of_type_list ", " (List.map (fun typ -> "", typ) l))
    (if par then "]" else "")
  ...
  let rec iter =
    ...
    | Types.Cty_arrow (label, t, ctype) ->
      ...
      Printf.bprintf b "%s%s%s%s -> "
        ...
        end (if parent then "(" else "")
        (Odoc_print.string_of_type_expr
           (if Odoc_misc.is_optional label then Odoc_misc.remove_option t else t))
        (if parent then ")" else "");
      iter ctype
       ...
       (List.map
          (fun field ->
             P.sprintf "   %s%s : %s;%s"
               ...
               (field_doc_str field.M.rf_text)) l))
...
let string_of_type t =
           ...
           (List.map
              (fun field ->
                 P.sprintf "   %s : %s;%s" field.M.of_name
                   ...
                   (field_doc_str field.M.of_text)) fields))
    | Some (M.Other typ) ->
      "= " ^
        (if priv then "private " else "") ^
  ...
  P.sprintf "type %s %s %s%s%s" parameters_str (Name.simple t.M.ty_name)
    ...
    begin match t.M.ty_info with
    | None -> ""
    | Some info -> Odoc_misc.string_of_info info
    end

let string_of_type_extension te =
  let module M = Odoc_extension 
  in
  let module T = Odoc_type 
  in
  "type " ^
    String.concat ""
      (List.map (fun p -> Odoc_print.string_of_type_expr p ^ " ")
                   ...
                   (fun x ->
                      "  | " ^
                        Name.simple x.M.xt_name ^
                          begin match x.M.xt_args, x.M.xt_ret with
                          | T.Cstr_tuple [], None -> ""
                          | T.Cstr_tuple l, None ->
                            " of " ^
                              String.concat " * "
                          ...
                          | T.Cstr_record l, Some r ->
                            ...
                            begin match x.M.xt_alias with
                            | None -> ""
                            | Some xa ->
                              " = " ^
                                begin match xa.M.xa_xt with
                                | None -> xa.M.xa_name
                                | Some x2 -> x2.M.xt_name
                                end
                            end ^
                              begin match x.M.xt_text with
                              | None -> ""
                              | Some t ->
...
let string_of_exception e =
  let module T = Odoc_type 
  in
  let module M = Odoc_exception 
  in
  "exception " ^
    Name.simple e.M.ex_name ^
      begin match e.M.ex_args, e.M.ex_ret with
      | T.Cstr_tuple [], None -> ""
      | T.Cstr_tuple l, None ->
        " of " ^
          String.concat " * "
      ...
      | T.Cstr_record l, Some r ->
        ...
        begin match e.M.ex_alias with
        | None -> ""
        | Some ea ->
          " = " ^
            begin match ea.M.ea_ex with
            | None -> ea.M.ea_name
