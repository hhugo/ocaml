...
let string_of_variance t (co, cn) =
  ...
  then match (co, cn) with true, false -> "+" | false, true -> "-" | _ -> ""
  else ""
...
let rec is_arrow_type t =
  ...
  | Types.Ttuple _ | Types.Tconstr _ | Types.Tvar _ | Types.Tunivar _
  | Types.Tobject _ | Types.Tpoly _ | Types.Tfield _ | Types.Tnil
    ...
    | Types.Cty_arrow (label, t, ctype) ->
        ...
        Printf.bprintf b "%s%s%s%s -> "
          ...
          (Odoc_print.string_of_type_expr
             (if Odoc_misc.is_optional label then Odoc_misc.remove_option t
             else t))
          (if parent then ")" else "");
        iter ctype
             ...
             (List.map
                (fun cons ->
                  let comment =
                    ...
                    | None -> ""
                    | Some t ->
                        P.sprintf "(* %s *)" (Odoc_misc.string_of_info t)
                  in
