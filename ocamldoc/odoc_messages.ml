...
let latex_extension_prefix =
  ...
  ^ "\t\t(default is \""
  ^ default_latex_extension_prefix
  ^ "\")"
...
let texinfo_title r =
  ...
  ^ String.concat
      "\n\t\t"
      (List.map
         (fun (n, (t, h)) -> Printf.sprintf " %d -> %s, %s " n t h)
         !r)
...
let library_namespace =
  "<module> Name of the library namespace for a prefixed library.Note: \
   very experimental."
...
let should_escape_at_sign =
  "The character @ has a special meaning in ocamldoc comments, for \
   commands such as @raise or @since. If you want to write a single @, you \
   must escape it as \\@."
...
let cross_recfield_not_found n =
  Printf.sprintf "Record field %s not found" n
