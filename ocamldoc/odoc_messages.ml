...
let default_dot_colors =
  [ [ "darkturquoise"; "darkgoldenrod2"; "cyan"; "green" ];
    [ "magenta"; "yellow"; "burlywood1"; "aquamarine"; "floralwhite";
      "lightpink" ]; [ "lightblue"; "mediumturquoise"; "salmon"; "slategray3" ]
  ]
      ...
      latex_only ^
        "\n" ^
          "\t\tDefault sectioning is:\n\t\t" ^
            String.concat "\n\t\t"
              (List.map (fun (n, t) -> Printf.sprintf " %d -> %s" n t)
                 !ref_titles)
...
let texinfo_title r =
  "n,style Associate {n } to the given sectioning style\n" ^
    "\t\t(e.g. 'section') in the texInfo output " ^
      texi_only ^
        "\n" ^
          "\t\tDefault sectioning is:\n\t\t" ^
            String.concat "\n\t\t"
              (List.map
                 (fun (n, (t, h)) -> Printf.sprintf " %d -> %s, %s " n t h) !r)
...
let string_of_options_list l =
  List.fold_left (fun acc (c, m) -> acc ^ "\n\t\t" ^ String.make 1 c ^ "  " ^ m)
    "" l
    ...
    options_can_be ^
      string_of_options_list
        [ merge_description; merge_author; merge_version; merge_see; merge_since;
          ...
          merge_return_value; merge_custom; merge_all ]
...
let attribute_type_not_found cl att =
  "The type of the attribute " ^
    att ^ " could not be found in the signature of class " ^ cl ^ "."

let method_type_not_found cl met =
  "The type of the method " ^
    met ^ " could not be found in the signature of class " ^ cl ^ "."

let module_not_found m m2 =
  "The module " ^
    m2 ^ " could not be found in the signature of module " ^ m ^ "."

let module_type_not_found m mt =
  "The module type " ^
    mt ^ " could not be found in the signature of module " ^ m ^ "."
...
let extension_not_found m e =
  "The extension " ^
    e ^ " could not be found in the signature of module " ^ m ^ "."
...
let class_type_not_found m c =
  "The class type " ^
    c ^ " could not be found in the signature of module " ^ m ^ "."
...
let inherit_classexp_not_found_in_typedtree n =
  "Inheritance class expression number " ^
    Int.to_string n ^ " was not found in typed tree."
...
let code_could_be_cross_reference n parent =
  Printf.sprintf
    "Code element [%s] in %s corresponds to a known cross-referenceable \
     element, it might be worthwhile to replace it with {!%s}" n parent n
