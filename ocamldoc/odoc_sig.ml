...
module Analyser (My_ir : Info_retriever) = struct
    ...
    | Parsetree.Ptype_abstract -> (
        ...
        | Some core_ty -> (
          ...
          | Ptyp_object (fields, _) ->
              let fields =
                List.map (fun {pof_desc; _} -> pof_desc) fields
              in
              let rec f = function
                | [] -> []
                | Otag ({txt = ""}, _) :: _ ->
    ...
    | Types.Type_variant l ->
        let f {Types.cd_id = constructor_name; cd_args; cd_res = ret_type} =
          ...
          let vc_args =
            match cd_args with
            | Cstr_tuple l ->
                Cstr_tuple (List.map (Odoc_env.subst_type env) l)
            | Cstr_record l ->
      ...
      List.fold_right
        (fun sig_item acc ->
          ...
          | ( Parsetree.Psig_attribute _ | Parsetree.Psig_extension _
            ...
            | Parsetree.Psig_include _ | Parsetree.Psig_class _
          ...
          | Parsetree.Psig_type (rf, types) -> (
            match
              List.filter
                (fun td ->
                  not (is_erased td.Parsetree.ptype_name.txt erased) )
                types
            with
            | [] -> acc
          ...
          | Parsetree.Psig_module
              ({Parsetree.pmd_name = name; pmd_type = module_type} as r) as
            m -> (
            match Name.Map.find name.txt erased with
          ...
          | Parsetree.Psig_recmodule mods -> (
            match
              List.filter
                (fun pmd ->
                  not (is_erased pmd.Parsetree.pmd_name.txt erased) )
                mods
            with
            | [] -> acc
      ...
      | item :: q -> (
          ...
          | Parsetree.Pctf_val ({txt = name}, mutable_flag, virtual_flag, _)
            ->
              ...
              let typ =
                try
                  Signature_search.search_attribute_type
                    name
                    class_signature
                with Not_found ->
                  raise
          ...
          | Parsetree.Pctf_inherit class_type ->
              ...
              let inh =
                ...
                | Parsetree.Pcty_constr (longident, _) ->
                    ...
                    ; ic_class = None
                    ; ic_text = text_opt }
                | Parsetree.Pcty_open _
                (* one could also traverse the open *)
  ...
  (** Analyse of a .mli parse tree, to get the corresponding elements.
    ...
    *)
  let rec analyse_parsetree env signat current_module_name last_pos
      pos_limit sig_item_list =
  ...
  and analyse_signature_item_desc env _signat table current_module_name
    ...
    | Parsetree.Psig_typext tyext ->
        ...
        let ty_path, ty_params, priv =
          ...
          | None -> assert false
          | Some ext ->
              ext.ext_type_path, ext.ext_type_params, ext.ext_private
        in
        let new_te =
          { te_info = comment_opt
          ; te_type_name =
        ...
        let rec analyse_extension_constructors maybe_more exts_acc
          ...
          | (name, types_ext) :: q ->
              ...
              let new_x =
                ...
                ; xt_loc =
                    { loc_impl = None
                    ; loc_inter = Some types_ext.Types.ext_loc }
                ; xt_text = None }
              in
              let pos_limit2 =
                match q with
    ...
    | Parsetree.Psig_exception ext ->
        ...
        let e =
          ...
          ; ex_args
          ; ex_ret =
              may_map (Odoc_env.subst_type env) types_ext.ext_ret_type
          ; ex_alias = None
        ...
        let rec f ?(first = false) acc_maybe_more last_pos
          ...
          | type_decl :: q ->
              ...
              ((* DEBUG *)
               let comm =
                 (* DEBUG *)
                 match assoc_com with
                 (* DEBUG *)
                 | None -> "sans commentaire"
                 (* DEBUG *)
                 | Some c -> Odoc_misc.string_of_info c
                 (* DEBUG *)
               in
               (* DEBUG *)
               print_DEBUG ("Type " ^ name.txt ^ " : " ^ comm);
               (* DEBUG *)
               let f_DEBUG (name, c_opt) =
                 (* DEBUG *)
                 let comm =
                   (* DEBUG *)
                   match c_opt with
                 ...
                 in
                 (* DEBUG *)
                 print_DEBUG ("constructor/field " ^ name ^ ": " ^ comm)
                 (* DEBUG *)
               in
              ...
              let type_kind =
                get_type_kind
                  env
                  name_comment_list
                  sig_type_decl.Types.type_kind
              in
              let loc_start = Loc.start type_decl.Parsetree.ptype_loc in
              let new_end =
                Loc.end_ type_decl.Parsetree.ptype_loc + maybe_more
              in
              ...
              in
              ( new_maybe_more
              , (ele_comments @ [Element_type new_type]) @ eles )
        in
    ...
    | Parsetree.Psig_modsubst _ ->
        ...
        0, env, []
    | Parsetree.Psig_module
        {Parsetree.pmd_name = name; pmd_type = module_type} ->
    ...
    | Parsetree.Psig_recmodule decls ->
        ...
        let rec f ?(first = false) acc_maybe_more last_pos name_mtype_list =
          ...
          | {Parsetree.pmd_name = name; pmd_type = modtype} :: q ->
              ...
              let maybe_more2, eles =
                f maybe_more (loc_end + maybe_more) q
              in
              ( maybe_more2
        ...
        let rec f ?(first = false) acc_maybe_more last_pos
          ...
          | class_desc :: q ->
              ...
              let new_class =
                ...
                ; cl_virtual =
                    class_desc.Parsetree.pci_virt = Asttypes.Virtual
                ; cl_kind = class_kind
        ...
        let rec f ?(first = false) acc_maybe_more last_pos
          ...
          | ct_decl :: q ->
              ...
              let ct =
                ...
                ; clt_virtual =
                    ct_decl.Parsetree.pci_virt = Asttypes.Virtual
                ; clt_kind = kind
                ; clt_loc =
                    { loc_impl = None
  ...
  and analyse_module_kind ?(erased = Name.Map.empty) env current_module_name
    ...
    | Parsetree.Pmty_signature signature -> (
        let signature =
          filter_out_erased_items_from_signature erased signature
        in
        match sig_module_type with
        | Types.Mty_signature signat ->
            Module_struct
    ...
    (* of string * module_type * module_type *) -> (
      ...
      | _ ->
          (* if we're here something's wrong *)
          raise
            (Failure "Parsetree.Pmty_functor _ but not Types.Mty_functor _")
      )
    | Parsetree.Pmty_with (module_type2, constraints) ->
        (* of module_type * (Longident.t * with_constraint) list*)
        let loc_start = Loc.end_ module_type2.Parsetree.pmty_loc in
      ...
      (*of Path.t * type_expr list * class_type*) ) ->
        ...
        let k =
          Class_constr
            ...
            ; cco_class = None
            ; cco_type_parameters =
                List.map (Odoc_env.subst_type env) typ_list }
        in
        [], k
    ...
    | ( Parsetree.Pcty_arrow (parse_label, _, pclass_type)
      , Types.Cty_arrow (label, type_expr, class_type) ) ->
        ...
        then
          ...
          raise
            (Failure
               "Parsetree.Pcty_arrow (parse_label, _, pclass_type), labels \
                differents")
    | _ ->
        raise
          (Failure "analyse_class_kind pas de correspondance dans le match")

  (** Analyse of a Parsetree.class_type and a Types.class_type to return a class_type_kind.*)
  and analyse_class_type_kind env current_class_name last_pos
      parse_class_type sig_class_type =
    match parse_class_type.Parsetree.pcty_desc, sig_class_type with
    | ( Parsetree.Pcty_constr (_, _)
      (*of Longident.t * core_type list *)
      , Types.Cty_constr (p, typ_list, _)
...
*)
  ...
  let analyse_signature source_file input_file (ast : Parsetree.signature)
