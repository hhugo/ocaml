        ...
        | item :: q -> (
            ...
            | Parsetree.Pctf_val ({ txt = name }, mutable_flag, virtual_flag, _)
              ->
                ...
                let typ =
                  try
                    Signature_search.search_attribute_type name class_signature
                  with Not_found ->
                    raise
                      (Failure
      ...
      let rec f acc_eles acc_env last_pos = function
        ...
        | ele :: q ->
            ...
            let maybe_more, new_env, elements =
              analyse_signature_item_desc acc_env signat table
                current_module_name ele.Parsetree.psig_loc (Loc.psig_start ele)
                (Loc.psig_end ele)
                (match q with
                | [] -> pos_limit
                | ele2 :: _ -> Loc.psig_start ele2)
                assoc_com ele.Parsetree.psig_desc
            in
            let new_pos =
      ...
      | Parsetree.Psig_value value_desc ->
          ...
          let type_expr =
            ...
            with Not_found ->
              raise
                (Failure
                   (Odoc_messages.value_not_found current_module_name
                      name_pre.txt))
          in
          ...
          let e =
            {
              ...
              ex_args;
              ex_ret =
                Option.map (Odoc_env.subst_type env) types_ext.ext_ret_type;
              ex_alias = None;
      ...
      | Parsetree.Psig_type (rf, name_type_decl_list) ->
          ...
          let rec f ?(first = false) acc_maybe_more last_pos name_type_decl_list
            ...
            | type_decl :: q ->
                ...
                let type_kind =
                  get_type_kind env name_comment_list
          ...
          let rec f ?(first = false) acc_maybe_more last_pos name_mtype_list =
            ...
            | { Parsetree.pmd_name = { txt = Some name }; pmd_type = modtype }
              :: q ->
                ...
                (* associate the comments to each constructor and build the [Type.t_type] *)
                let module_kind =
                  analyse_module_kind new_env complete_name modtype
                    sig_module_type
                in
                let code_intf =
                  if !Odoc_global.keep_code then
    ...
    and analyse_module_type_kind ?(erased = Name.Map.empty) env
      ...
      | Parsetree.Pmty_signature ast -> (
          ...
          | Types.Mty_signature signat ->
              ...
              let elements =
                analyse_parsetree env signat current_module_name pos_start
                  pos_end ast
              in
