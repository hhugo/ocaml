...
module Analyser =
  ...
  struct
    ...
    let get_comments_in_class pos_start pos_end =
      My_ir.get_comments (fun t -> Class_comment t) !file_name
    ...
    let get_comments_in_module pos_start pos_end =
      My_ir.get_comments (fun t -> Element_module_comment t) !file_name
        (get_string_of_file pos_start pos_end)
    ...
    let get_field
        ...
        { Types.ld_id = field_name; ld_mutable = mutable_flag;
          ld_type = type_expr }
    =
    ...
    let get_type_kind env name_comment_list type_kind =
      ...
      | Types.Type_abstract -> Odoc_type.Type_abstract
    ...
    let get_cstr_args env pos_end =
      ...
      let open Typedtree in
      function
      | Cstr_tuple l -> Odoc_type.Cstr_tuple (List.map tuple l)
      | Cstr_record l ->
    ...
    let constraint_for_subitem =
      ...
      fun acc s rebuild_constraint ->
        ...
        | hd, Some p ->
          ...
          begin match Name.Map.find hd acc with
          | exception Not_found ->
        ...
        (fun constraint_ acc ->
           ...
           | Parsetree.Pwith_typesubst (s, typedecl) ->
             constraint_for_subitem acc s
               (fun s -> Parsetree.Pwith_typesubst (s, typedecl))
           | Parsetree.Pwith_modsubst (s, modpath) ->
             constraint_for_subitem acc s
               (fun s -> Parsetree.Pwith_modsubst (s, modpath))
           | Parsetree.Pwith_modtypesubst (s, modpath) ->
             constraint_for_subitem acc s
               (fun s -> Parsetree.Pwith_modtypesubst (s, modpath))) constraints
        acc
    ...
    let filter_out_erased_items_from_signature erased signature =
      if Name.Map.is_empty erased then
        signature
      else
        List.fold_right
          (fun sig_item acc ->
             ...
             | (Parsetree.Psig_attribute _
                ...
                | Parsetree.Psig_class_type _) as tp
               ->
               take_item tp
               ...
               begin match
                 List.filter
                   (fun pmd ->
                      ...
                      | None -> false
                      | Some name -> not (is_erased name erased)) mods
               with
               | [] -> acc
    ...
    let analyse_class_elements
        env current_class_name last_pos pos_limit class_type_field_list
        class_signature
    =
      let get_pos_limit2 q =
        ...
        | ele2 :: _ ->
          ...
          begin match ele2.Parsetree.pctf_desc with
      ...
      let rec f last_pos class_type_field_list =
        ...
        | item :: q ->
          ...
          begin match item.Parsetree.pctf_desc with
          ...
          | Parsetree.Pctf_method
              ({ txt = name }, private_flag, virtual_flag, _)
            ->
    ...
    *)
    ...
    =
      ...
      let rec f acc_eles acc_env last_pos =
        function
        | [] ->
          let s = get_string_of_file last_pos pos_limit in
          let (_, ele_coms) = My_ir.all_special !file_name s in
        ...
        | ele :: q ->
          ...
          let (maybe_more, new_env, elements) =
            analyse_signature_item_desc acc_env signat table current_module_name
              ele.Parsetree.psig_loc (Loc.psig_start ele) (Loc.psig_end ele)
              begin match q with
              | [] -> pos_limit
              | ele2 :: _ -> Loc.psig_start ele2
              end assoc_com ele.Parsetree.psig_desc
          in
    ...
    and analyse_signature_item_desc
        env _signat table current_module_name sig_item_loc pos_start_ele
        pos_end_ele pos_limit comment_opt sig_item_desc
    =
      ...
      | Parsetree.Psig_typext tyext ->
        let (new_env, types_ext_list, last_ext) =
          List.fold_left
            (fun (env_acc, exts_acc, _) { Parsetree.pext_name = { txt = name } }
             ->
        ...
        in
        let rec analyse_extension_constructors
            maybe_more exts_acc types_ext_list
        =
          ...
          | (name, types_ext) :: q ->
            ...
            let new_x =
              {
                ...
                xt_args;
                xt_ret =
                  Option.map (Odoc_env.subst_type new_env)
            ...
            let (maybe_more, comment_opt) =
              just_after_special ext_loc_end pos_limit2
            in
      ...
      | Parsetree.Psig_exception ext ->
        ...
        let types_ext =
          ...
          | Not_found ->
            raise
              (Failure
                 (Odoc_messages.exception_not_found current_module_name name.txt))
        in
        let ex_args =
        ...
        let extended_env =
          List.fold_left
            (fun acc_env td ->
               ...
               in
               Odoc_env.add_type acc_env complete_name) env name_type_decl_list
        in
        let env =
          match rf with
          ...
          | type_decl :: q ->
            ...
            let (maybe_more, name_comment_list) =
              name_comment_from_type_decl
                (Loc.end_ type_decl.Parsetree.ptype_loc) pos_limit2 type_decl
            in
      ...
      | Parsetree.Psig_module { Parsetree.pmd_name = { txt = None } } ->
        0, env, []
      | Parsetree.Psig_module
          { Parsetree.pmd_name = { txt = Some name }; pmd_type = module_type }
        ->
        let complete_name = Name.concat current_module_name name in
                     ...
                     raise
                       (Failure
                          (Odoc_messages.module_not_found current_module_name
                             name))
                 in
                 begin match sig_module_type with
                 (* FIXME : can this be a Tmty_ident? in this case, we wouldn't have the signature *)
                 | Types.Mty_signature s ->
      ...
      | Parsetree.Psig_modtype
          { Parsetree.pmtd_name = name; pmtd_type = pmodtype_decl }
      |
        ...
        let new_env =
          List.fold_left
            (fun acc_env class_desc ->
               ...
               in
               Odoc_env.add_class acc_env complete_name) env
            class_description_list
        in
        let rec f
            ?(first=false) acc_maybe_more last_pos class_description_list
        =
          ...
          | class_desc :: q ->
            ...
            let (new_maybe_more, eles) = f maybe_more (pos_end + maybe_more) q
            in
      ...
      | Parsetree.Psig_class_type class_type_declaration_list ->
        ...
        in
        let rec f
            ?(first=false) acc_maybe_more last_pos class_type_description_list
        =
          ...
          | ct_decl :: q ->
            ...
            let kind =
              analyse_class_type_kind new_env complete_name
                (Loc.start ct_decl.Parsetree.pci_loc) ct_decl.Parsetree.pci_expr
                sig_class_type
            in
            let ct =
              {
    ...
    and analyse_module_type_kind
        ?(erased=Name.Map.empty) env current_module_name module_type
        sig_module_type
    =
      match module_type.Parsetree.pmty_desc with
      ...
      | Parsetree.Pmty_signature ast ->
        ...
        | _ ->
          raise
            (Failure
               "Parsetree.Pmty_signature signature but not \
                Types.Mty_signature signat")
        end
      | Parsetree.Pmty_functor (param2, module_type2) ->
        let loc =
          ...
          | Parsetree.Named (_, pmty) -> pmty.Parsetree.pmty_loc
        in
        let loc_start = Loc.start loc in
        let loc_end = Loc.end_ loc in
    ...
    =
      ...
      | Parsetree.Pmty_signature signature ->
        let signature = filter_out_erased_items_from_signature erased signature
        in
        begin match sig_module_type with
        | Types.Mty_signature signat ->
          Module_struct
            (analyse_parsetree env signat current_module_name
               ...
               (Loc.end_ module_type.Parsetree.pmty_loc) signature)
        | _ ->
          (* if we're here something's wrong *)
          raise
            (Failure
      ...
      | Parsetree.Pcty_signature
        ...
        let (inher_l, ele) =
          analyse_class_elements env current_class_name last_pos
            (Loc.end_ parse_class_type.Parsetree.pcty_loc) class_type_field_list
            class_signature
        in
        [], Class_structure (inher_l, ele)
      | Parsetree.Pcty_arrow (parse_label, _, pclass_type),
...
*)
    ...
    let analyse_signature
        source_file input_file (ast : Parsetree.signature)
        (signat : Types.signature)
    =
      ...
      let mod_name =
        String.capitalize_ascii
          (Filename.basename
             (try Filename.chop_extension source_file with _ -> source_file))
      in
      let (len, info_opt) =
        preamble !file_name !file (fun x -> x.Parsetree.psig_loc) ast
      in
      let elements =
        analyse_parsetree Odoc_env.empty signat mod_name len
          (String.length !file) ast
      in
