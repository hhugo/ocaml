...
let new_span_id =
  let i = ref 0 in
  fun () -> incr i; span_id_of_int !i

let new_ghost_id =
  let i = ref 0 in
  fun () -> incr i; ghost_id_of_int !i
  ...
  then
    ...
    then
      ...
      | Some ((x1, c1) :: list, mpc, span) ->
          let list, mpc =
            if x1 = x.elem
            then (x1, c1 + 1) :: list, max mpc (c1 + 1)
    ...
    for j = 1 to Array.length t.table.(0) - 1 do
      ...
      | Ghost x, Ghost _ ->
          ...
          then (
            ...
            if i > 0
            then (t.table.(i - 1).(j)).span <- t.table.(i - 1).(j - 1).span )
      | _ -> ()
    done
  done
            ...
            then (
              for k = i downto i1 + 1 do
                for j = j to j2 do
                  ...
                  if k < i
                  then (t.table.(k).(j)).span <- t.table.(k - 1).(j).span
                done
              done;
              for l = j to j2 do
