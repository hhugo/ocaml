...
module Search =
  ...
  struct
    ...
    let search_type_extension te v =
      List.fold_left (fun acc xt -> acc @ search_extension_constructor xt v) []
    ...
    let search_class c v =
      ...
      let l =
        if go_deeper then
          let res_att =
            List.fold_left (fun acc att -> acc @ search_attribute att v) []
              (Odoc_class.class_attributes c)
          in
          let res_met =
            List.fold_left (fun acc m -> acc @ search_method m v) []
              (Odoc_class.class_methods c)
          in
          let res_sec =
            List.fold_left (fun acc t -> acc @ search_text c.cl_name t v) []
    ...
    let search_class_type ct v =
      ...
      let l =
        if go_deeper then
          ...
          in
          let res_sec =
            List.fold_left (fun acc t -> acc @ search_text ct.clt_name t v) []
    ...
    let rec search_module_type mt v =
      ...
      let l =
        if go_deeper then
          let res_val =
            List.fold_left (fun acc va -> acc @ search_value va v) []
              (Odoc_module.module_type_values mt)
          in
          let res_typ =
            List.fold_left (fun acc t -> acc @ search_type t v) []
              (Odoc_module.module_type_types mt)
          in
          let res_ext =
            List.fold_left (fun acc te -> acc @ search_type_extension te v) []
              (Odoc_module.module_type_type_extensions mt)
          in
          let res_exc =
            List.fold_left (fun acc e -> acc @ search_exception e v) []
              (Odoc_module.module_type_exceptions mt)
          in
          let res_mod = search (Odoc_module.module_type_modules mt) v in
          let res_modtyp =
            List.fold_left (fun acc mt -> acc @ search_module_type mt v) []
              (Odoc_module.module_type_module_types mt)
          in
          let res_cl =
            List.fold_left (fun acc cl -> acc @ search_class cl v) []
              (Odoc_module.module_type_classes mt)
          in
          let res_cltyp =
            List.fold_left (fun acc clt -> acc @ search_class_type clt v) []
              (Odoc_module.module_type_class_types mt)
          in
          let res_sec =
            List.fold_left (fun acc t -> acc @ search_text mt.mt_name t v) []
              (Odoc_module.module_type_comments mt)
          in
          res_val @
            res_typ @
              res_ext @
                res_exc @ res_mod @ res_modtyp @ res_cl @ res_cltyp @ res_sec
        else
          []
      in
    ...
    and search_module m v =
      ...
      let l =
        if go_deeper then
          ...
          in
          let res_sec =
            List.fold_left (fun acc t -> acc @ search_text m.m_name t v) []
