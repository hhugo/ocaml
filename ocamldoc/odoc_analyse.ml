...
let process_implementation_file sourcefile =
  ...
  | Syntaxerr.Error _ as exn ->
      ...
      | _ -> assert false );
      None, inputfile
  | Failure s -> prerr_endline s; incr Odoc_global.errors; None, inputfile
...
let process_interface_file sourcefile =
  ...
  Warnings.check_fatal (); ast, sg, inputfile
...
let process_file sourcefile =
  ...
  | Odoc_global.Impl_file file -> (
      ...
      try
        ...
        | Some (parsetree, typedtree) ->
            let file_module =
              Ast_analyser.analyse_typed_tree
                file
                input_file
                parsetree
                typedtree
      ...
      | Sys_error s | Failure s ->
          prerr_endline s; incr Odoc_global.errors; None
      | e -> process_error e; incr Odoc_global.errors; None )
  ...
  | Odoc_global.Text_file file -> (
      ...
      try
        ...
        let txt =
          try
            Odoc_text.Texter.text_of_string
...
let analyse_files ?(init = []) files =
  let modules_pre =
    ...
    @ List.fold_left
        (fun acc file ->
          try
            match process_file file with None -> acc | Some m -> acc @ [m]
          with Failure s -> prerr_endline s; incr Odoc_global.errors; acc )
        []
        files
  in
  ...
  then (
    ...
    print_newline () );
  let merged_modules =
    Odoc_merge.merge !Odoc_global.merge_options modules
  in
  if !Odoc_global.verbose
  then (
    print_string Odoc_messages.ok;
...
let dump_modules file (modules : Odoc_module.t_module list) =
  try
    ...
    output_value chanout dump; close_out chanout
