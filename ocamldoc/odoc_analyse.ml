...
let initial_env () =
  ...
  let initially_opened_module = if initial = current then None else Some initial
  in
  ...
  in
  Typemod.initial_env ~loc:(Location.in_file "ocamldoc command line")
...
let process_implementation_file sourcefile =
  ...
  try
    let parsetree =
      Pparse.file ~tool_name inputfile (no_docstring Parse.implementation)
        Pparse.Structure
    in
    let typedtree =
      Typemod.type_implementation sourcefile prefixname modulename env parsetree
    in
...
let process_interface_file sourcefile =
  ...
  let inputfile = preprocess sourcefile in
  let ast =
    Pparse.file ~tool_name inputfile (no_docstring Parse.interface)
      Pparse.Signature
  in
...
let rec remove_class_elements_between_stop_in_class_kind k =
  ...
  | Odoc_class.Class_structure (inher, l) ->
    Odoc_class.Class_structure
      (inher, remove_class_elements_between_stop true l)
    ...
    | Odoc_module.Element_class c ->
      if keep then
        (c.Odoc_class.cl_kind <-
           remove_class_elements_between_stop_in_class_kind c.Odoc_class.cl_kind;
...
let remove_elements_between_stop module_list =
  List.map
    (fun m ->
       m.Odoc_module.m_kind <-
         remove_module_elements_between_stop_in_module_kind m.Odoc_module.m_kind;
       m) module_list
...
let analyse_files ?(init=[]) files =
  ...
  if !Odoc_global.sort_modules then
    List.sort (fun m1 m2 -> compare m1.Odoc_module.m_name m2.Odoc_module.m_name)
      merged_modules
  else
    merged_modules
