  ...
  | Odoc_global.Impl_file file -> (
      ...
      try
        ...
        | Some (parsetree, Typedtree.{ structure; coercion; _ }) ->
            ...
            let file_module =
              Ast_analyser.analyse_typed_tree file input_file parsetree
                typedtree
            in
...
let rec remove_class_elements_between_stop keep eles =
  ...
  | ele :: q -> (
      ...
      | Odoc_class.Class_comment _ ->
          if keep then ele :: remove_class_elements_between_stop keep q
...
let analyse_files ?(init = []) files =
  ...
  let modules =
    if !Odoc_global.no_stop then modules_pre
