...
class scanner =
  object (self)
    ...
    method scan_extension_constructor
          (_ : Odoc_extension.t_extension_constructor) =
      ()
    ...
    method scan_type_extension_pre (_ : Odoc_extension.t_type_extension) =
      true

    (** This method scans the constructors of the given type extension. *)
    method scan_type_extension_constructors
          (x : Odoc_extension.t_type_extension) =
      List.iter
        self#scan_extension_constructor
        (Odoc_extension.extension_constructors x)
    ...
    method scan_type_extension (x : Odoc_extension.t_type_extension) =
      if self#scan_type_extension_pre x
      then self#scan_type_extension_constructors x
    ...
    method scan_class c =
      if self#scan_class_pre c then self#scan_class_elements c
    ...
    method scan_class_type ct =
      if self#scan_class_type_pre ct then self#scan_class_type_elements ct
    ...
    method scan_module_elements m =
      List.iter
        (fun ele ->
          ...
          | Odoc_module.Element_type_extension x ->
              self#scan_type_extension x
    ...
    method scan_module m =
      if self#scan_module_pre m then self#scan_module_elements m
