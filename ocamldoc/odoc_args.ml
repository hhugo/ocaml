...
let extend_html_generator f =
  ...
  let module Current = (val (current : (module Odoc_html.Html_generator))) 
  in
  let module F = (val (f : (module Odoc_gen.Html_functor))) 
  in
  let module M = F(Current) 
  in
  current_generator :=
...
let extend_latex_generator f =
  ...
  let module Current = (val (current : (module Odoc_latex.Latex_generator))) 
  in
  let module F = (val (f : (module Odoc_gen.Latex_functor))) 
  in
  let module M = F(Current) 
  in
  current_generator :=
...
let extend_texi_generator f =
  ...
  let module Current = (val (current : (module Odoc_texi.Texi_generator))) 
  in
  let module F = (val (f : (module Odoc_gen.Texi_functor))) 
  in
  let module M = F(Current) 
  in
  current_generator :=
...
let extend_man_generator f =
  ...
  let module Current = (val (current : (module Odoc_man.Man_generator))) 
  in
  let module F = (val (f : (module Odoc_gen.Man_functor))) 
  in
  let module M = F(Current) 
  in
...
let extend_dot_generator f =
  ...
  let module Current = (val (current : (module Odoc_dot.Dot_generator))) 
  in
  let module F = (val (f : (module Odoc_gen.Dot_functor))) 
  in
  let module M = F(Current) 
  in
...
let extend_base_generator f =
  ...
  let module Current = (val (current : (module Odoc_gen.Base))) 
  in
  let module F = (val (f : (module Odoc_gen.Base_functor))) 
  in
  let module M = F(Current) 
  in
...
let analyse_option_string l s =
  List.fold_left
    (fun acc ((c, _), v) -> if String.contains s c then acc @ v else acc) [] l
...
let anonymous f =
  let sf =
    ...
    else if Filename.check_suffix f !Config.interface_suffix then
      Odoc_global.Intf_file f
    else if Filename.check_suffix f "txt" then
...
let default_options =
  Options.list @
    [
      ...
      M.option_text; "-warn-error", Arg.Set Odoc_global.warn_error, M.werr;
      ...
      M.sort_modules; "-no-stop", Arg.Set Odoc_global.no_stop, M.no_stop;
      ...
      M.option_intro; "-hide", Arg.String add_hidden_modules, M.hide_modules;
      "-m",
      Arg.String
        (fun s ->
    ...
    (* html only options *)
      ...
      M.css_style; "-index-only", Arg.Set Odoc_html.index_only, M.index_only;
...
let parse () =
  ...
  (try
   ...
   with
   | Compenv.Exit_with_status n -> exit n);
