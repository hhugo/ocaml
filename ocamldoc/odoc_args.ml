...
let extend_man_generator f =
  ...
  let module M = F (Current) in
  current_generator :=
    Some (Odoc_gen.Man (module M : Odoc_man.Man_generator))
...
let extend_dot_generator f =
  ...
  let module M = F (Current) in
  current_generator :=
    Some (Odoc_gen.Dot (module M : Odoc_dot.Dot_generator))
...
let f_latex_title s =
  ...
  | [n; command] ->
      ...
      Odoc_latex.latex_titles :=
        List.remove_assoc n !Odoc_latex.latex_titles;
      ...
      | _ -> (
        ...
        | 'A' .. 'Z' ->
            Odoc_global.hidden_modules :=
              name :: !Odoc_global.hidden_modules
        | _ ->
            incr Odoc_global.errors;
...
let default_options =
  ...
  @ [ ( "-initially-opened-module"
    ...
    ; ( "-lib"
      , Arg.Set_string Odoc_global.library_namespace
      , M.library_namespace )
    ; ( "-text"
      , Arg.String
          (fun s ->
            Odoc_global.files :=
              !Odoc_global.files @ [Odoc_global.Text_file s] )
      , M.option_text )
    ...
    ; ( "-hide-warnings"
      , Arg.Clear Odoc_config.print_warnings
      , M.hide_warnings )
    ...
    ; ( "-no-custom-tags"
      , Arg.Set Odoc_global.no_custom_tags
      , M.no_custom_tags )
    ...
    ; ( "-load"
      ...
      , M.load ^ "\n" )
    ; ( "-t"
      , Arg.String (fun s -> Odoc_global.title := Some s)
      , M.option_title )
    ; ( "-intro"
    ...
    ; ( "-charset"
      , Arg.Set_string Odoc_html.charset
      , M.charset !Odoc_html.charset ^ "\n\n *** LaTeX options ***\n" )
    ; (* latex only options *)
      ( "-noheader"
    ...
    ; (* dot only options *)
      ...
      , Arg.String
          (fun s ->
            Odoc_dot.dot_colors := Str.split (Str.regexp_string ",") s )
      , M.dot_colors )
    ; ( "-dot-include-all"
      , Arg.Set Odoc_dot.dot_include_all
      , M.dot_include_all )
