...
let anonymous f =
  let sf =
    if Filename.check_suffix f "ml" then Odoc_global.Impl_file f
    else if Filename.check_suffix f !Config.interface_suffix then
      Odoc_global.Intf_file f
    else if Filename.check_suffix f "txt" then Odoc_global.Text_file f
...
let default_options =
  ...
  @ [
      ...
      ( "-dot-reduce",
        ...
        M.dot_reduce ^ "\n\n *** man pages options ***\n" );
      (* man only options *) ("-man-mini", Arg.Set Odoc_man.man_mini, M.man_mini);
      ( "-man-suffix",
