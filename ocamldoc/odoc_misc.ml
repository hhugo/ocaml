...
let string_of_info i =
  let module M = Odoc_types 
  in
  begin match i.M.i_deprecated with
  | None -> ""
  | Some d -> Odoc_messages.deprecated ^ ". " ^ string_of_text d ^ "\n"
  end ^
    begin match i.M.i_desc with
    | None -> ""
...
let rec text_no_title_no_list t =
  let iter t_ele =
    ...
    | Odoc_types.Module_list l ->
      list_concat (Odoc_types.Raw ", ")
        (List.map (fun s -> Odoc_types.Ref (s, Some Odoc_types.RK_module, None))
           l)
    | Odoc_types.Index_list -> []
...
let get_titles_in_text t =
  ...
  let rec iter_ele ele =
    ...
    | Odoc_types.Target _ -> ()
  and iter_text txt = List.iter iter_ele txt
  in
  iter_text t;
  List.rev !l
...
let rec get_before_dot s =
  try
    ...
    if n + 1 >= len then (* The dot is the last character *)
      true, s, ""
    else
      match s.[n + 1] with
      | ' ' | '\n' | '\r' | '\t' ->
...
let create_index_lists elements string_of_ele =
  let rec f current acc0 acc1 acc2 =
    function
    | [] -> acc0 :: acc1 @ [ acc2 ]
    | ele :: q ->
...
let remove_option typ =
  ...
  Transient_expr.type_expr
    (Transient_expr.create (trim (get_desc typ)) ~level:(get_level typ)
       ~scope:(get_scope typ) ~id:(get_id typ))
