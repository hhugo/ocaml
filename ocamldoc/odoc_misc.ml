...
let input_file_as_string nom =
  ...
  let rec iter () =
    ...
    with End_of_file -> ()
  in
  iter (); close_in chanin; Buffer.contents buf
...
let get_fields type_expr =
  ...
  List.fold_left
    (fun acc (label, field_kind, typ) ->
      match field_kind with
      | Types.Fabsent -> acc
      | _ -> if label = "*dummy method*" then acc else acc @ [label, typ] )
    []
    fields
...
let string_of_sees l =
  ...
  | _ ->
      ...
      ^ String.concat
          ""
          (List.map (fun see -> "- " ^ string_of_see see ^ "\n") l)
      ^ "\n"
...
let apply_opt f v_opt =
  match v_opt with None -> None | Some v -> Some (f v)
...
let rec text_no_title_no_list t =
  let iter t_ele =
    ...
    | Odoc_types.Emphasize t ->
        [Odoc_types.Emphasize (text_no_title_no_list t)]
    | Odoc_types.Latex s -> [Odoc_types.Latex s]
    | Odoc_types.Link (s, t) ->
        [Odoc_types.Link (s, text_no_title_no_list t)]
    | Odoc_types.Superscript t ->
        [Odoc_types.Superscript (text_no_title_no_list t)]
    | Odoc_types.Subscript t ->
        [Odoc_types.Subscript (text_no_title_no_list t)]
    | Odoc_types.Module_list l ->
        list_concat
          (Odoc_types.Raw ", ")
          (List.map
...
and first_sentence_text_ele text_ele =
  ...
  | Odoc_types.List _ | Odoc_types.Enum _ | Odoc_types.Latex _
   ...
   |Odoc_types.Subscript _ | Odoc_types.Module_list _
   |Odoc_types.Index_list ->
      false, text_ele, None
...
let remove_option typ =
  let rec iter t =
    ...
    | Types.Tconstr (path, [ty], _) when Path.same path Predef.path_option
      ->
        ty.Types.desc
    | Types.Tconstr _ | Types.Tvar _ | Types.Tunivar _ | Types.Tpoly _
     |Types.Tarrow _ | Types.Ttuple _ | Types.Tobject _ | Types.Tfield _
     |Types.Tnil | Types.Tvariant _ | Types.Tpackage _ ->
        t
    | Types.Tlink t2 | Types.Tsubst t2 -> iter t2.Types.desc
  in
