...
let rec first_sentence_text t =
  ...
  | ele :: q ->
      ...
      if stop then
        (stop, [ ele2 ], match ele3_opt with None -> q | Some e -> e :: q)
      else
  ...
  let rec f current acc0 acc1 acc2 = function
    ...
    | ele :: q -> (
        ...
        | _ -> (
            ...
            match first with
            | 'A' .. 'Z' ->
                if current = first then f current acc0 acc1 (acc2 @ [ ele ]) q
...
let remove_option typ =
  ...
  let rec trim t =
    ...
    | Tconstr (path, [ ty ], _) when Path.same path Predef.path_option ->
        get_desc ty
    | Tconstr _ | Tvar _ | Tunivar _ | Tpoly _ | Tarrow _ | Ttuple _ | Tobject _
    | Tfield _ | Tnil | Tvariant _ | Tpackage _ ->
        t
