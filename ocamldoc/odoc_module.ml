...
let modules l =
  List.fold_left
    (fun acc ele -> match ele with Element_module m -> acc @ [ m ] | _ -> acc)
    [] l
...
let rec module_type_elements ?(trans = true) mt =
  let rec iter_kind = function
    ...
    | Some (Module_type_with (k, _)) -> if trans then iter_kind (Some k) else []
  ...
  *)
  let rec module_elements visited ?(trans = true) m =
    let rec iter_kind = function
      ...
      | Module_functor (_, k) | Module_apply (k, _) -> iter_kind k
      | Module_with (tk, _) ->
...
let rec module_type_is_functor mt =
  let rec iter k =
    ...
    | Some (Module_type_struct _) | Some (Module_type_typeof _) | None -> false
  in
  iter mt.mt_kind
