...
and included_module =
  ...
  ; mutable im_module : mmt option
        (** the included module or module type *)
  ; mutable im_info : Odoc_types.info option
        (** comment associated to the include directive *) }
...
and module_type_kind =
  ...
  | Module_type_alias of module_type_alias
      (** complete name and corresponding module type if we found it *)
...
and t_module_type =
  ...
  ; mutable mt_kind : module_type_kind option
        (** [None] = abstract module type if mt_type = None ;
...
let values l =
  List.fold_left
    (fun acc ele -> match ele with Element_value v -> acc @ [v] | _ -> acc)
    []
    l
...
let types l =
  List.fold_left
    (fun acc ele -> match ele with Element_type t -> acc @ [t] | _ -> acc)
    []
    l
...
let type_extensions l =
  List.fold_left
    (fun acc ele ->
      match ele with Element_type_extension x -> acc @ [x] | _ -> acc )
    []
    l
...
let exceptions l =
  List.fold_left
    (fun acc ele ->
      match ele with Element_exception e -> acc @ [e] | _ -> acc )
    []
    l
...
let classes l =
  List.fold_left
    (fun acc ele -> match ele with Element_class c -> acc @ [c] | _ -> acc)
    []
    l
...
let class_types l =
  List.fold_left
    (fun acc ele ->
      match ele with Element_class_type ct -> acc @ [ct] | _ -> acc )
    []
    l
...
let modules l =
  List.fold_left
    (fun acc ele ->
      match ele with Element_module m -> acc @ [m] | _ -> acc )
    []
    l
...
let mod_types l =
  List.fold_left
    (fun acc ele ->
      match ele with Element_module_type mt -> acc @ [mt] | _ -> acc )
    []
    l
...
let comments l =
  List.fold_left
    (fun acc ele ->
      match ele with Element_module_comment t -> acc @ [t] | _ -> acc )
    []
    l
...
let included_modules l =
  List.fold_left
    (fun acc ele ->
      match ele with Element_included_module m -> acc @ [m] | _ -> acc )
    []
    l
...
let rec module_type_elements ?(trans = true) mt =
  let rec iter_kind = function
    ...
    | Some (Module_type_with (k, _)) ->
        if trans then iter_kind (Some k) else []
    ...
    let rec iter_kind = function
      ...
      | Module_alias ma ->
          ...
          then
            ...
            | None -> []
            | Some (Mod m') ->
                if String.Set.mem m'.m_name visited
                then []
                else module_elements (String.Set.add m'.m_name visited) m'
            | Some (Modtype mt) -> module_type_elements mt
          else []
      | Module_functor (_, k) | Module_apply (k, _) ->
          print_DEBUG
            "Odoc_module.module_elements: Module_functor ou Module_apply";
          iter_kind k
      | Module_with (tk, _) ->
...
(** Returns the list of type extensions of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_extensions ?(trans = true) m =
  type_extensions (module_elements ~trans m)

(** Returns the list of exceptions of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_exceptions ?(trans = true) m =
  exceptions (module_elements ~trans m)
...
(** Returns the list of class types of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_class_types ?(trans = true) m =
  class_types (module_elements ~trans m)
...
(** Returns the list of module types of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_module_types ?(trans = true) m =
  mod_types (module_elements ~trans m)

(** Returns the list of included module of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_included_modules ?(trans = true) m =
  included_modules (module_elements ~trans m)
...
let rec module_type_parameters ?(trans = true) mt =
  let rec iter k =
    ...
    | Some (Module_type_with (k, _)) -> if trans then iter (Some k) else []
  ...
  let rec iter = function
    ...
    | Module_constraint (_k, tk) ->
        module_type_parameters
          ...
          ; mt_kind = Some tk
          ; mt_loc = Odoc_types.dummy_loc }
    | Module_struct _ | Module_apply _ | Module_with _ | Module_typeof _
     |Module_unpack _ ->
        []
  in
  iter m.m_kind
...
let rec module_all_submodules ?(trans = true) m =
  ...
  List.fold_left (fun acc m -> acc @ module_all_submodules ~trans m) l l
...
let rec module_type_is_functor mt =
  let rec iter k =
    ...
    | Some (Module_type_with (k, _)) -> iter (Some k)
    | Some (Module_type_struct _) | Some (Module_type_typeof _) | None ->
        false
  in
  iter mt.mt_kind
...
let module_is_functor m =
  let rec iter visited = function
    ...
    | Module_alias ma -> (
        ...
        | None -> false
        | Some (Mod mo) ->
            iter (String.Set.add ma.ma_name visited) mo.m_kind
        | Some (Modtype mt) -> module_type_is_functor mt )
    | Module_constraint (k, _) -> iter visited k
    | _ -> false
  in
...
(** Returns the list of values of a module type.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_values ?(trans = true) m =
  values (module_type_elements ~trans m)

(** Returns the list of types of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_types ?(trans = true) m =
  types (module_type_elements ~trans m)

(** Returns the list of type extensions of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_type_extensions ?(trans = true) m =
  type_extensions (module_type_elements ~trans m)

(** Returns the list of exceptions of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_exceptions ?(trans = true) m =
  exceptions (module_type_elements ~trans m)

(** Returns the list of classes of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_classes ?(trans = true) m =
  classes (module_type_elements ~trans m)

(** Returns the list of class types of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_class_types ?(trans = true) m =
  class_types (module_type_elements ~trans m)

(** Returns the list of modules of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_modules ?(trans = true) m =
  modules (module_type_elements ~trans m)

(** Returns the list of module types of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_module_types ?(trans = true) m =
  mod_types (module_type_elements ~trans m)

(** Returns the list of included module of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_included_modules ?(trans = true) m =
  included_modules (module_type_elements ~trans m)

(** Returns the list of comments of a module.
  @param trans indicates if, for aliased modules, we must perform a transitive search.*)
let module_type_comments ?(trans = true) m =
  comments (module_type_elements ~trans m)
...
let rec module_all_classes ?(trans = true) m =
  List.fold_left
    (fun acc m -> acc @ module_all_classes ~trans m)
    (List.fold_left
       (fun acc mtyp -> acc @ module_type_all_classes ~trans mtyp)
