...
let (plugins, paths) =
  let rec iter (files, incs) =
    function
    | [] | _ :: [] -> List.rev files, List.rev incs
    | "-g" :: file :: q
      when
        Filename.check_suffix file "cmo" ||
          Filename.check_suffix file "cma" || Filename.check_suffix file "cmxs"
      ->
...
let get_real_filename name =
  if Filename.basename name <> name then
    ...
    let paths =
      Filename.current_dir_name :: paths @
    ...
    with
    | Not_found -> failwith (M.file_not_found_in_paths paths name)
