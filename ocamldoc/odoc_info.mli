...
exception
  Text_syntax of int * int * string
  (** Raised when parsing string to build a {!Odoc_info.text}
     structure. [(line, char, string)] *)

type see = see_ref * text (** The information in a \@see tag. *)
...
val dummy_loc : location (** A dummy location. *)
...
module Name : sig
  ...
  val simple : t -> t (** Access to the simple name. *)
...
module Parameter : sig
  ...
  val typ : parameter -> Types.type_expr (** Access to the complete type. *)
...
module Extension : sig
  type private_flag = Odoc_extension.private_flag = Private | Public
  
  type extension_alias =
    Odoc_extension.extension_alias
    =
    {
...
module Exception : sig
  type exception_alias =
    Odoc_exception.exception_alias
    =
    {
...
module Type : sig
  type private_flag = Odoc_type.private_flag = Private | Public
  ...
  type type_kind =
    ...
    | Type_variant of variant_constructor list (** constructors *)
    | Type_record of record_field list (** fields *)
    | Type_open (** Type is open *)
  
  type object_field =
    Odoc_type.object_field
    =
    {
      of_name : string;
...
module Class : sig
  ...
  val class_type_attributes
    : ?trans:bool -> t_class_type -> Value.t_attribute list
...
module Module : sig
  ...
  and mmt = Odoc_module.mmt = Mod of t_module | Modtype of t_module_type
  
  and included_module =
    Odoc_module.included_module
    =
    {
  ...
  val module_type_extensions
    : ?trans:bool -> t_module -> Extension.t_type_extension list
  ...
  val module_parameters
    : ?trans:bool -> t_module -> (module_parameter * text option) list
  ...
  val module_type_exceptions
    : ?trans:bool -> t_module_type -> Exception.t_exception list
  ...
  val module_type_simple_values
    : ?trans:bool -> t_module_type -> Value.t_value list
  ...
  val module_type_class_types
    : ?trans:bool -> t_module_type -> Class.t_class_type list
  (** Access to the class types of a module type. *)
  
  val module_type_all_classes
    : ?trans:bool -> t_module_type -> Class.t_class list
...
val string_of_text : text -> string (** Get a string from a text. *)

val string_of_info : info -> string (** Get a string from an info structure. *)
...
module Search : sig
  ...
  val extensions
    : Module.t_module list -> Extension.t_extension_constructor list
  ...
  class scanner :
    object
      ...
      method scan_type_extension_constructors :
        Extension.t_type_extension -> unit
