...
module Texter = struct
  ...
  let text_of_string s =
    ...
    with _ ->
      raise
        (Text_syntax
           (!Odoc_text_lexer.line_number, !Odoc_text_lexer.char_number, s))
  ...
  let escape_n s c n =
    ...
    for i = 0 to len - 1 do
      if s.[i] = c && !remain > 0
      then ( Printf.bprintf b "\\%c" c; decr remain )
  ...
  let escape_code s =
    ...
    then escape_n s '[' (open_brackets - close_brackets)
    else if close_brackets > open_brackets
    then escape_n s ']' (close_brackets - open_brackets)
    else s
  ...
  let rec p_text b t = List.iter (p_text_element b) t

  and p_list b l = List.iter (fun t -> p b "{- "; p_text b t; p b "}\n") l
  ...
  and p_text_element b = function
    ...
    | Block t -> p_text b t
    | Title (n, l_opt, t) ->
        p b "{%d%s " n (match l_opt with None -> "" | Some s -> ":" ^ s);
        p_text b t;
        p b "}"
    | Latex s -> p b "{%% %s%%}" s
    | Link (s, t) -> p b "{{:%s}" s; p_text b t; p b "}"
    | Ref (name, kind_opt, text_opt) -> (
        p
          ...
          | Some k ->
              ...
              s ^ ":" )
          name;
        match text_opt with None -> () | Some t -> p_text b t; p b "}" )
    ...
    | Module_list l ->
        ...
        p b "}"
    | Index_list -> p b "{!indexlist}"
    | Custom (s, t) -> p b "{%s " s; p_text b t; p b "}"
    | Target (target, code) -> p b "{%%%s: %s}" target (escape_raw code)

  let string_of_text s =
    let b = Buffer.create 256 in
    p_text b s; Buffer.contents b
end
