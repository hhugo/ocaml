...
module Texter = struct (* builds a text structure from a string. *)
  ...
  let count s c =
    let count = ref 0 in
    for i = 0 to String.length s - 1 do if s.[i] = c then incr count done;
    !count
  ...
  let escape_code s =
    ...
    else if close_brackets > open_brackets then
