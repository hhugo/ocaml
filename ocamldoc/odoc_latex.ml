...
let latex_extension_prefix =
  ref Odoc_messages.default_latex_extension_prefix

let latex_exception_prefix =
  ref Odoc_messages.default_latex_exception_prefix
...
let latex_class_type_prefix =
  ref Odoc_messages.default_latex_class_type_prefix

let latex_attribute_prefix =
  ref Odoc_messages.default_latex_attribute_prefix
...
class text =
  object (self)
    ...
    *)
    ...
    method value_label ?no_ name =
      !latex_value_prefix ^ self#label ?no_ name
    ...
    method method_label ?no_ name =
      !latex_method_prefix ^ self#label ?no_ name

    (** Make a correct label from a class name. *)
    method class_label ?no_ name =
      !latex_class_prefix ^ self#label ?no_ name
    ...
    method module_label ?no_ name =
      !latex_module_prefix ^ self#label ?no_ name
    ...
    method latex_of_Bold fmt t =
      ps fmt "{\\bf "; self#latex_of_text fmt t; ps fmt "}"

    method latex_of_Italic fmt t =
      ps fmt "{\\it "; self#latex_of_text fmt t; ps fmt "}"

    method latex_of_Emphasize fmt t =
      ps fmt "{\\em "; self#latex_of_text fmt t; ps fmt "}"
    ...
    method latex_of_List fmt tl =
      ...
      List.iter
        (fun t -> ps fmt "\\item "; self#latex_of_text fmt t; ps fmt "\n")
        tl;
      ps fmt "\\end{itemize}\n"
    ...
    method latex_of_Link fmt s t =
      self#latex_of_text fmt t; ps fmt "[\\url{"; ps fmt s; ps fmt "}]"
    ...
    method latex_of_Ref fmt name ref_opt text_opt =
      ...
      | Some (RK_section _) ->
          ...
          let label =
            self#make_ref (self#label ~no_:false (Name.simple name))
          in
    ...
    method latex_of_Superscript fmt t =
      ps fmt "$^{"; self#latex_of_text fmt t; ps fmt "}$"

    method latex_of_Subscript fmt t =
      ps fmt "$_{"; self#latex_of_text fmt t; ps fmt "}$"
  end
    ...
    *)
      ...
      method latex_of_attribute fmt a =
        self#latex_of_text
          fmt
          ( Latex
              (self#make_label (self#attribute_label a.att_value.val_name))
                ...
                let t =
                  ...
                  ; List
                      (List.map
                         (fun (p, text_opt) ->
                           let t =
                             match text_opt with None -> [] | Some t -> t
                           in
                           Raw p.mp_name :: Raw ": " :: t )
                         l) ]
                in
        ...
        then
          let f m =
            try
              let chanout =
                open_out
                  (Filename.concat dir (Name.simple m.m_name) ^ ".tex")
              in
