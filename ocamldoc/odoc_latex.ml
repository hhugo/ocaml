...
let latex_titles =
  ref
    [ 0, "section"; 1, "section"; 2, "subsection"; 3, "subsubsection";
...
class text =
  object (self)
    ...
    val subst_strings =
      List.map (fun (x, y) -> Str.regexp x, y)
        [ "\001", "\001\002"; "\\\\", "\001b"; "{", "\\\\{"; "}", "\\\\}";
          "\\$", "\\\\$"; "\\^", "{\\\\textasciicircum}"; "%", "\\\\%";
          "_", "\\\\_"; "~", "\\\\~{}"; "#", "{\\char35}";
          "->", "$\\\\rightarrow$"; "<-", "$\\\\leftarrow$"; ">=", "$\\\\geq$";
          "<=", "$\\\\leq$"; ">", "$>$"; "<", "$<$"; "=", "$=$";
          "|", "{\\\\textbar}"; "\\.\\.\\.", "$\\\\ldots$"; "&", "\\\\&";
          "\001b", "{\\\\char92}"; "\001\002", "\001" ]
    (** Associations of strings to substitute in latex code. *)
    
    val subst_strings_simple =
      List.map (fun (x, y) -> Str.regexp x, y)
        [ "\001", "\001\002"; "\\\\", "\001b"; "{", "\001l";
          "}", "{\\\\char125}"; "'", "{\\\\textquotesingle}";
          "`", "{\\\\textasciigrave}"; "\001b", "{\\\\char92}";
          "\001l", "{\\\\char123}"; "\001\002", "\001" ]
  ...
  class latex =
    object (self)
      ...
      method first_and_rest_of_info i_opt =
        ...
        | None -> [], []
        | Some i ->
          begin match i.Odoc_info.i_desc with
    ...
    *)
        ...
        let fields =
          List.map
            (fun r ->
               let s_field =
                 p fmt "@[<h 6>  %s%s :@ %s ;"
                   (if r.rf_mutable then "mutable " else "") r.rf_name
                   (self#normal_type mod_name r.rf_type);
                 flush ()
               in
               [ CodePre s_field ] @ self#entry_comment f r.rf_text) l
        in
      ...
      method latex_of_cstr_args (fmt, flush as f) mod_name (args, ret) =
        ...
        | (Cstr_tuple t as l), Some r ->
          ...
          (if t = [] then
             ...
             p fmt " :@ %s -> %s" (self#normal_cstr_args ~par:false mod_name l)
               res);
          [ CodePre (flush ()) ]
        | Cstr_record l, None ->
          p fmt " of@ ";
      ...
      method latex_of_type fmt t =
        ...
        let text =
          ...
          | Some (Other typ) ->
            p fmt2 " = %s%s" (if priv then "private " else "")
                  ...
                  List.map
                    (fun r ->
                       let s_field =
                         ...
                         flush2 ()
                       in
                       [ CodePre s_field ] @ self#entry_comment f r.of_text) l
                in
                List.flatten fields @ [ CodePre ">" ]
              | None | Some (Other _) -> []
              end
          ...
          merge_codepre defs2 @
            [
              Latex
                ("\\index{" ^
                   self#label s_name ^
                     "@\\verb`" ^ self#label ~no_:false s_name ^ "`}\n") ] @
      ...
      method latex_of_type_extension mod_name fmt te =
        let text =
          ...
          | l ->
            ps fmt2 "(";
            print_concat fmt2 ", "
              (fun p -> ps fmt2 (self#normal_type mod_name p)) l;
            ps fmt2 ") "
          end;
      ...
      method latex_of_module_type_kind fmt father kind =
        ...
        | Module_type_with (k, s) ->
          self#latex_of_module_type_kind fmt father k;
          self#latex_of_text fmt
            [ Code " "; Code (self#relative_idents father s) ]
      ...
      method latex_of_module fmt m =
        ...
        let t =
          [ Latex "\\begin{ocamldoccode}\n"; Code "module ";
            Code (Name.simple m.m_name); Code " : " ]
        in
        self#latex_of_text fmt t;
        ...
        begin match Module.module_is_functor m with
        | false -> ()
        | true ->
      ...
      method latex_of_module_type fmt mt =
        ...
        in
        self#latex_of_text fmt t;
        begin match mt.mt_type, mt.mt_kind with
        | Some _, Some kind ->
          self#latex_of_text fmt [ Code " = " ];
          self#latex_of_text fmt [ Latex "\\end{ocamldoccode}\n" ];
        ...
        begin match Module.module_type_is_functor mt with
        | false -> ()
        | true ->
          self#latex_of_text fmt [ Newline ];
          begin match
          ...
          [ Latex "\\begin{ocamldoccode}\n";
            Code
              (Printf.sprintf "class %s%s%s : "
                 (if c.cl_virtual then "virtual " else "") type_params
          ...
          [ Latex "\\begin{ocamldoccode}\n";
            Code
              (Printf.sprintf "class type %s%s%s = "
                 (if ct.clt_virtual then "virtual " else "") type_params
      ...
      method latex_of_class_element fmt class_name class_ele =
        ...
        | Class_comment t ->
          ...
          | _ ->
            self#latex_of_text fmt
              [ Title (Name.depth class_name + 2, None, t) ]
          end
          ...
          List.iter
            (fun m ->
               if !separate_files then
                 ...
                 self#generate_for_top_module fmt m) module_list;
