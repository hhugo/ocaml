          ...
          | None -> (
              ...
              | None ->
                  ( acc_b2,
                    ...
                    if ic.ic_name = Odoc_messages.object_end then acc_names2
        ...
        | None -> (
            ...
            | None ->
                ( acc_b,
                  ...
                  (* we don't want to output warning messages for "object ... end" classes not found *)
                  if capp.capp_name = Odoc_messages.object_end then acc_names
            ...
            | None -> (
                ...
                | None ->
                    ( acc_b,
                      Name.head c.cl_name :: acc_inc,
                      (* we don't want to output warning messages for "object ... end" classes not found *)
                      if cco.cco_name = Odoc_messages.object_end then acc_names
        ...
        | None -> (
            ...
            | None ->
                ( acc_b,
                  ...
                  (* we don't want to output warning messages for "object ... end" class types not found *)
                  if cta.cta_name = Odoc_messages.object_end then acc_names
...
let rec assoc_comments_text_elements parent_name module_list t_ele =
  ...
  | Ref (initial_name, None, text_option) ->
      ...
      search_within_ancestry finalize (query module_list) ~parent_name
        initial_name
  | Code s -> (
      if not !Odoc_global.show_missed_crossref then t_ele
      else
        ...
        with Not_found ->
          if name = "" then t_ele
            ...
            then t_ele
              ...
              if name = parent_name || is_path_suffix () then t_ele
              else
                let finalize = function
                  | None -> t_ele
                  | Some _ ->
  ...
  | Ref (initial_name, Some kind, text_option) ->
      let rec iter_parent ?parent_name name =
        ...
        if was_verified v then Ref (name, Some kind, text_option)
        else
          let res =
            match kind with
            | RK_section _ -> (
