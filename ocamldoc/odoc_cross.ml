...
let kind_name_exists kind =
  let pred =
    ...
    | RK_module -> (
        fun e ->
          match e with Odoc_search.Res_module _ -> true | _ -> false )
    | RK_module_type -> (
        fun e ->
          match e with Odoc_search.Res_module_type _ -> true | _ -> false )
    | RK_class -> (
        fun e -> match e with Odoc_search.Res_class _ -> true | _ -> false )
    | RK_class_type -> (
        fun e ->
          match e with Odoc_search.Res_class_type _ -> true | _ -> false )
    | RK_value -> (
    ...
    | RK_extension -> (
        fun e ->
          match e with Odoc_search.Res_extension _ -> true | _ -> false )
    | RK_exception -> (
        fun e ->
          match e with Odoc_search.Res_exception _ -> true | _ -> false )
    | RK_attribute -> (
        fun e ->
          match e with Odoc_search.Res_attribute _ -> true | _ -> false )
    | RK_method -> (
        fun e ->
          match e with Odoc_search.Res_method _ -> true | _ -> false )
    | RK_section _ -> assert false
    | RK_recfield -> (
        fun e ->
          match e with Odoc_search.Res_recfield _ -> true | _ -> false )
    | RK_const -> (
...
let lookup_extension name =
  match
    List.find
      (fun k ->
        match k with Odoc_search.Res_extension _ -> true | _ -> false )
      (get_known_elements name)
  with
...
let lookup_exception name =
  match
    List.find
      (fun k ->
        match k with Odoc_search.Res_exception _ -> true | _ -> false )
      (get_known_elements name)
  with
...
and associate_in_type_extension _module_list
  ...
  List.fold_left
    (fun (acc_b_modif, acc_incomplete_top_module_names, acc_names_not_found)
         xt ->
...
let query module_list name =
  ...
  | [] -> (
    try
      ...
      add_verified v2; name, Some (RK_section t)
...
let rec search_within_ancestry ((finalize, initial_name, query) as param)
  ...
  | _, None -> (
    ...
    | Some p ->
        ...
        in
        search_within_ancestry
          param
          ?parent_name
          (Name.concat p initial_name) )
...
let rec assoc_comments_text_elements parent_name module_list t_ele =
  ...
  | List l ->
      List (List.map (assoc_comments_text parent_name module_list) l)
  | Enum l ->
      Enum (List.map (assoc_comments_text parent_name module_list) l)
  | Newline -> Newline
  | Block t -> Block (assoc_comments_text parent_name module_list t)
  | Superscript t ->
      Superscript (assoc_comments_text parent_name module_list t)
            ...
            then t_ele
              ...
              then t_ele
                ...
                let finalize = function
                  ...
                  | Some _ ->
                      Odoc_global.pwarning
                      @@ Odoc_messages.code_could_be_cross_reference
                           name
                           parent_name;
                      t_ele
                in
        ...
        then Ref (name, Some kind, text_option)
          ...
          let res =
            ...
            | _ ->
                ...
                in
                if f name
                then ( add_verified v; name, Some kind )
                else name, None
          in
          match res with
  ...
  | Custom (s, t) ->
      Custom (s, assoc_comments_text parent_name module_list t)
...
and assoc_comments_module_kind parent_name module_list mk =
  ...
  | Module_with (mtk, s) ->
      Module_with
        (assoc_comments_module_type_kind parent_name module_list mtk, s)
...
and assoc_comments_module_type_kind parent_name module_list mtk =
  ...
  | Module_type_functor (params, mtk1) ->
      Module_type_functor
        ( params
        , assoc_comments_module_type_kind parent_name module_list mtk1 )
...
and assoc_comments_class_kind parent_name module_list ck =
  ...
  | Class_structure (inher, eles) ->
      ...
      Class_structure
        ( inher2
        , List.map
            (assoc_comments_class_element parent_name module_list)
            eles )
...
and assoc_comments_class_type module_list ct =
  ct.clt_info
  <- ao (assoc_comments_info ct.clt_name module_list) ct.clt_info;
  ct.clt_kind
...
and assoc_comments_parameter parent_name module_list p =
  ...
  | Simple_name sn ->
      sn.sn_text
      <- ao (assoc_comments_text parent_name module_list) sn.sn_text
  | Tuple (l, _) ->
...
and assoc_comments_type module_list t =
  ...
  | Type_variant vl ->
      List.iter
        (fun vc ->
          vc.vc_text
          <- ao (assoc_comments_info parent module_list) vc.vc_text )
        vl
  | Type_record fl ->
      List.iter
        (fun rf ->
          rf.rf_text
          <- ao (assoc_comments_info parent module_list) rf.rf_text )
        fl
  | Type_open -> () );
  t
      ...
      List.iter
        (fun nf ->
          Odoc_global.pwarning
            ...
            | NF_mmt n ->
                Odoc_messages.cross_module_or_module_type_not_found n
            | NF_c n -> Odoc_messages.cross_class_not_found n
            | NF_cct n ->
                Odoc_messages.cross_class_or_class_type_not_found n
