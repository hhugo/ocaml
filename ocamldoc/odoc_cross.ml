...
let add_known_element name k =
  ...
  with
  | Not_found -> known_elements := Ele_map.add name [ k ] !known_elements
...
let kind_name_exists kind =
  let pred =
    ...
    | RK_value ->
      (fun e ->
         match e with
         | Odoc_search.Res_value _ -> true
         | _ -> false)
    | RK_type ->
      (fun e ->
         match e with
         | Odoc_search.Res_type _ -> true
         | _ -> false)
    | RK_extension ->
      (fun e ->
         match e with
         | Odoc_search.Res_extension _ -> true
         | _ -> false)
    | RK_exception ->
      (fun e ->
         match e with
    ...
    | RK_attribute ->
      (fun e ->
         match e with
         | Odoc_search.Res_attribute _ -> true
         | _ -> false)
    | RK_method ->
      (fun e ->
         match e with
         | Odoc_search.Res_method _ -> true
         | _ -> false)
    | RK_section _ -> assert false
    | RK_recfield ->
      (fun e ->
         match e with
         | Odoc_search.Res_recfield _ -> true
         | _ -> false)
    | RK_const ->
      (fun e ->
         match e with
         | Odoc_search.Res_const _ -> true
         | _ -> false)
  in
  fun name ->
  ...
  match
    List.find
      (fun k ->
         match k with
         | Odoc_search.Res_module _ -> true
         | _ -> false) (get_known_elements name)
  with
...
class scan =
  object
    ...
    method! scan_type_recfield t f =
      add_known_element (Printf.sprintf "%s.%s" t.ty_name f.rf_name)
        (Odoc_search.Res_recfield (t, f))
    
    method! scan_type_const t f =
      add_known_element (Printf.sprintf "%s.%s" t.ty_name f.vc_name)
...
=
  let rec iter_kind (acc_b, acc_inc, acc_names) k =
    ...
    | Module_struct elements ->
      List.fold_left (associate_in_module_element module_list m.m_name)
        (acc_b, acc_inc, acc_names) elements
    | Module_alias ma ->
      begin match ma.ma_module with
      ...
      | None ->
        ...
        | None ->
          ...
          (if
             ma.ma_name = Odoc_messages.struct_end ||
      ...
      | None ->
        ...
        | None ->
          ...
          (* we don't want to output warning messages for
             "sig ... end" or "struct ... end" modules not found *)
          (if
             mta.mta_name = Odoc_messages.struct_end ||
  ...
  in
  iter_kind (acc_b_modif, acc_incomplete_top_module_names, acc_names_not_found)
    m.m_kind

and associate_in_module_type
    module_list
...
=
  let rec iter_kind (acc_b, acc_inc, acc_names) k =
    ...
    | Module_type_struct elements ->
      List.fold_left (associate_in_module_element module_list mt.mt_name)
        (acc_b, acc_inc, acc_names) elements
  ...
  | Some k ->
    iter_kind
      (acc_b_modif, acc_incomplete_top_module_names, acc_names_not_found) k

and associate_in_module_element
    module_list m_name
...
=
    ...
    | None ->
      ...
      | None ->
        ...
        (* we don't want to output warning messages for
            "sig ... end" or "struct ... end" modules not found *)
        (if
           im.im_name = Odoc_messages.struct_end ||
  ...
  | Element_class cl ->
    associate_in_class module_list
      (acc_b_modif, acc_incomplete_top_module_names, acc_names_not_found) cl
...
=
  let rec iter_kind (acc_b, acc_inc, acc_names) k =
    ...
    | Class_apply capp ->
      begin match capp.capp_class with
      | Some _ -> acc_b, acc_inc, acc_names
      | None ->
        let cl_opt =
        ...
        | None ->
          ...
          (* we don't want to output warning messages for "object ... end" classes not found *)
          (if capp.capp_name = Odoc_messages.object_end then
             acc_names
           else
             NF_c capp.capp_name :: acc_names)
        | Some c ->
    ...
    | Class_constr cco ->
      begin match cco.cco_class with
      | Some _ -> acc_b, acc_inc, acc_names
      | None ->
        let cl_opt =
        ...
        | None ->
          let clt_opt =
            ...
            | Not_found -> None
          in
          begin match clt_opt with
          | None ->
            acc_b,
            Name.head c.cl_name :: acc_inc,
            (* we don't want to output warning messages for "object ... end" classes not found *)
            (if cco.cco_name = Odoc_messages.object_end then
               acc_names
             else
               NF_cct cco.cco_name :: acc_names)
          | Some ct ->
...
=
  let iter_kind (acc_b, acc_inc, acc_names) k =
    ...
    | Class_signature (inher_l, _) ->
      ...
      List.fold_left f (acc_b, acc_inc, acc_names) inher_l
    | Class_type cta ->
      begin match cta.cta_class with
      | Some _ -> acc_b, acc_inc, acc_names
      | None ->
        let cct_opt =
...
and associate_in_type_extension
    _module_list
    (acc_b_modif, acc_incomplete_top_module_names, acc_names_not_found) te
=
  List.fold_left
      ...
      (try
       ...
       | Not_found ->
         if name = "" then
           ...
           if
             Char.uppercase_ascii first_char <> first_char ||
               Char.lowercase_ascii first_char = first_char
           then
             t_ele
           else (* Some path analysis auxiliary functions *)
             let path s = String.split_on_char '.' s in
             let filter =
               List.filter
                 (fun s -> s <> "" && s.[0] = Char.uppercase_ascii s.[0])
             in
             let rec is_prefix prefix full =
               ...
               | _ -> false
             in
             let p = filter @@ path name
             and parent_p = path parent_name
             in
             let is_path_suffix () =
...
and assoc_comments_type_extension parent_name module_list te =
  ...
  List.iter (assoc_comments_extension_constructor module_list)
    te.te_constructors;
  te
