          ...
          for i = 0 to len - 1 do
            ...
            | '.' when !j = 0 ->
                ...
                then
                  match s.[i + 1] with
                  | '(' -> j := 1
                  | _ -> Buffer.add_char buf.(!j) '.'
                else Buffer.add_char buf.(!j) s.[i]
            | c -> Buffer.add_char buf.(!j) c
          done;
          Buffer.contents buf.(0), Buffer.contents buf.(1)
      | _ -> (
        match List.rev (Str.split (Str.regexp_string ".") s) with
        | [] -> "", ""
        | h :: q -> String.concat "." (List.rev q), h ) )
...
let normalize_name name =
  ...
  let s =
    ...
    then parens_if_infix (strip_string (String.sub s 1 (len - 2)))
    else s
  in
  match p with "" -> s | p -> concat p s
  ...
  try
    ...
    then
      ...
      with Not_found ->
        ...
        if pos >= len - 1
        then h, ""
        else h, String.sub n (pos + 1) (len - pos - 1)
    else n, ""
  with Not_found -> n, ""
...
let depth name =
  try List.length (Str.split (Str.regexp "\\.") name) with _ -> 1
...
let prefix n1 n2 =
  ...
  try
    ...
    String.sub n2 0 len1 = n1 && n2.[len1] = '.'
  with _ -> false
...
let get_relative n1 n2 =
  ...
  then
    let len1 = String.length n1 in
    try String.sub n2 (len1 + 1) (String.length n2 - len1 - 1)
    with _ -> n2
  else n2
...
let alias_unprefix ln s =
  ...
  then s
    ...
    if k > n && String.sub s 0 n = p
    then String.(capitalize_ascii @@ sub s n (k - n))
    else s
