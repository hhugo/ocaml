  ...
  struct
          ...
          Some
            {
              ...
              i_before =
                Odoc_merge.merge_before_tags
                  (List.map (fun (n, s) -> n, MyTexter.text_of_string s)
                     !Odoc_comments_global.before);
              i_deprecated =
                begin match !Odoc_comments_global.deprecated with
                | None -> None
              ...
              i_return_value =
                begin match !Odoc_comments_global.return_value with
                | None -> None
                | Some s -> Some (MyTexter.text_of_string s)
                end;
              i_custom =
                List.map (fun (tag, s) -> tag, MyTexter.text_of_string s)
      ...
      let rec iter s2 =
        ...
        | len, Some _ ->
          (try
           ...
           with
           | Not_found -> (* we shouldn't get here *) false)
      in
      iter s
    ...
    let just_after_special file s =
      ...
      | len, Some d ->
        ...
        begin match retrieve_info_simple file (String.sub s 0 len) with
        | _, None ->
          (try
           (* if the special comment is the stop comment (**/**),
             ...
             then
               ...
               len, Some d
           with
           | Not_found -> (* should not occur *) 0, None)
        | _, Some _ -> 0, None
        end
    ...
    let get_comments f_create_ele file s =
      let (assoc_com, ele_coms) =
        ...
        | h :: q ->
          if
            blank_line_outside_simple file
      ...
      let ele_comments =
        List.fold_left
          (fun acc sc ->
             ...
             | None -> acc
             | Some t -> acc @ [ f_create_ele t ]) [] ele_coms
      in
