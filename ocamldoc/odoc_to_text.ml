...
class virtual info =
  object (self)
    ...
    method text_of_version_opt v_opt =
      ...
      | None -> []
      | Some v ->
        [ Bold [ Raw (Odoc_messages.version ^ ": ") ]; Raw v; Newline ]
    ...
    method text_of_raised_exceptions l =
      ...
      | (s, t) :: [] ->
        [ Bold [ Raw Odoc_messages.raises ]; Raw " "; Code s; Raw " " ] @
          t @ [ Newline ]
      | _ ->
        [ Bold [ Raw Odoc_messages.raises ]; Raw " ";
...
class virtual to_text =
  object (self)
    ...
    method text_of_attribute a =
      ...
      let s =
        Format.fprintf Format.str_formatter "@[<hov 2>val %s%s%s :@ %s"
          ...
          (if a.att_mutable then "mutable " else "") s_name
    ...
    method text_of_method m =
      ...
      let s =
        Format.fprintf Format.str_formatter "@[<hov 2>method %s%s%s :@ %s"
          ...
          (if m.met_virtual then "virtual " else "") s_name
    ...
    method text_of_parameter_list m_name l =
      ...
      | [] -> []
      | _ ->
        [ Bold [ Raw Odoc_messages.parameters ]; Raw ":";
          List
            (List.map
               (fun p ->
                  ...
                  | s -> Code s
                    ...
                    self#text_of_short_type_expr m_name (Parameter.typ p) @
                      [ Newline ] @ self#text_of_parameter_description p) l) ]
    ...
    method text_of_module_parameter_list l =
      ...
      | [] -> []
      | _ ->
        [ Newline; Bold [ Raw Odoc_messages.parameters ]; Raw ":";
          List
            (List.map
               (fun (p, desc_opt) ->
      ...
      | Class_constraint (ck, ctk) ->
        [ Code "( " ] @
          self#text_of_class_kind father ck @
            [ Code " : " ] @
              self#text_of_class_type_kind father ctk @ [ Code " )" ]
    ...
    method text_of_module_kind ?(with_def_syntax=true) k =
      ...
      | Module_alias m_alias ->
        begin match m_alias.ma_module with
        | None ->
      ...
      | Module_functor (_, k) ->
        (if with_def_syntax then [ Code " : " ] else []) @
          [ Code "functor ... " ] @
            [ Code " -> " ] @ self#text_of_module_kind ~with_def_syntax:false k
      | Module_typeof s ->
        let code =
    ...
    method text_of_module_type_kind ?(with_def_syntax=true) tk =
      ...
      | Module_type_functor (p, k) ->
        let t1 =
          [ Code ("(" ^ p.mp_name ^ " : ") ] @
            self#text_of_module_type_kind p.mp_kind @ [ Code ") -> " ]
        in
