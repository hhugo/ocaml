  ...
  object (self)
    ...
    method text_of_sees l =
      ...
      | [see] ->
          ...
          :: Raw " "
          :: self#text_of_see see )
          @ [Newline]
      | _ ->
          [ Bold [Raw Odoc_messages.see_also]
          ; List (List.map (fun see -> self#text_of_see see) l)
          ; Newline ]
  ...
  object (self)
    ...
    method normal_class_type_param_list m_name t =
      self#relative_idents
        m_name
        (Odoc_info.string_of_class_type_param_list t)
    ...
    method text_of_class_kind father ckind =
      ...
      | Class_constr cco ->
          ...
          | [] -> []
          | l ->
              (Code "[" :: self#text_of_type_expr_list father ", " l)
              @ [Code "] "] )
          @ [ Code
                ...
                | Some (Cltype (clt, _)) ->
                    Name.get_relative father clt.clt_name ) ]
    ...
    method text_of_module_kind ?(with_def_syntax = true) k =
      ...
      | Module_alias m_alias -> (
        ...
        | None ->
            [ Code
                ((if with_def_syntax then " = " else "") ^ m_alias.ma_name)
            ]
        | Some (Mod m) ->
    ...
    method text_of_module_type_kind ?(with_def_syntax = true) tk =
      ...
      | Module_type_with (tk2, code) ->
          let t =
            self#text_of_module_type_kind ~with_def_syntax:false tk2
          in
