...
let analyse_files
    ...
    ?(include_dirs=([] : string list)) ?(labels=false) ?(sort_modules=false)
    ?(no_stop=false) ?(init=[]) files
=
...
let info_string_of_info i =
  ...
  List.iter (fun s -> p b "\n@@author %s" (escape_arobas s)) i.i_authors;
  begin match i.i_version with
  | None -> ()
  ...
  (let f_see_ref =
   ...
   List.iter
     (fun (sref, t) ->
        p b "\n@@see %s %s" (escape_arobas (f_see_ref sref))
          (escape_arobas (text_string_of_text t))) i.i_sees);
  begin match i.i_since with
  | None -> ()
  | Some s -> p b "\n@@since %s" (escape_arobas s)
  end;
  begin match i.i_deprecated with
  | None -> ()
  | Some t -> p b "\n@@deprecated %s" (escape_arobas (text_string_of_text t))
  end;
  List.iter
    (fun (s, t) ->
       p b "\n@@param %s %s" (escape_arobas s)
         (escape_arobas (text_string_of_text t))) i.i_params;
  List.iter
    (fun (s, t) ->
       p b "\n@@raise %s %s" (escape_arobas s)
         (escape_arobas (text_string_of_text t))) i.i_raised_exceptions;
  begin match i.i_return_value with
  | None -> ()
  | Some t -> p b "\n@@return %s" (escape_arobas (text_string_of_text t))
  end;
  List.iter
    (fun (s, t) -> p b "\n@@%s %s" s (escape_arobas (text_string_of_text t)))
    i.i_custom;
  Buffer.contents b
