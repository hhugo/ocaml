...
let merge_before_tags l =
  let rec iter acc = function
    | [] -> List.rev acc
    | (v, text) :: q ->
        let l1, l2 = List.partition (fun (v2, _) -> v = v2) q in
        let acc =
          let text =
            List.fold_left
              (fun acc t -> acc @ [Raw " "] @ t)
...
let merge_info merge_options (m1 : info) (m2 : info) =
  let new_desc_opt =
    ...
    | Some d1, Some d2 ->
        if List.mem Merge_description merge_options
        then Some (d1 @ (Newline :: d2))
        else Some d1
  in
  let new_authors =
    ...
    | [], [] -> []
    | l, [] | [], l -> l
    | l1, l2 -> if List.mem Merge_author merge_options then l1 @ l2 else l1
  in
  let new_version =
    ...
    | Some v1, Some v2 ->
        if List.mem Merge_version merge_options
        then Some (v1 ^ " " ^ v2)
        else Some v1
  in
  let new_sees =
    ...
    | [], [] -> []
    | l, [] | [], l -> l
    | l1, l2 -> if List.mem Merge_see merge_options then l1 @ l2 else l1
  in
  let new_since =
    ...
    | Some v, None | None, Some v -> Some v
    | Some v1, Some v2 ->
        if List.mem Merge_since merge_options
        then Some (v1 ^ " " ^ v2)
        else Some v1
  in
  let new_before =
    ...
    | l, [] | [], l -> l
    | l1, _ ->
        if List.mem Merge_before merge_options
        then merge_before_tags (m1.i_before @ m2.i_before)
        else l1
  in
  let new_before =
    List.map (fun (v, t) -> Str.split version_separators v, v, t) new_before
  in
  ...
  let new_dep =
    ...
    | Some t1, Some t2 ->
        if List.mem Merge_deprecated merge_options
        then Some (t1 @ (Newline :: t2))
        else Some t1
  in
  let new_params =
    ...
    | l, [] | [], l -> l
    | l1, l2 ->
        if List.mem Merge_param merge_options
        then
          let l_in_m1_and_m2, l_in_m2_only =
            List.partition (fun (param2, _) -> List.mem_assoc param2 l1) l2
          in
  ...
  let new_raised_exceptions =
    ...
    | l, [] | [], l -> l
    | l1, l2 ->
        if List.mem Merge_raised_exception merge_options
        then
          let l_in_m1_and_m2, l_in_m2_only =
            List.partition (fun (exc2, _) -> List.mem_assoc exc2 l1) l2
          in
          let rec iter = function
            | [] -> []
            | (exc2, desc2) :: q ->
  ...
  let new_rv =
    ...
    | None, Some t | Some t, None -> Some t
    | Some t1, Some t2 ->
        if List.mem Merge_return_value merge_options
        then Some (t1 @ (Newline :: t2))
        else Some t1
  in
  let new_custom =
    ...
    | [], [] -> []
    | [], l | l, [] -> l
    | l1, l2 -> if List.mem Merge_custom merge_options then l1 @ l2 else l1
  in
...
let merge_types merge_options mli ml =
  ...
  mli.ty_code
  <- (match mli.ty_code with None -> ml.ty_code | _ -> mli.ty_code);
  match mli.ty_kind, ml.ty_kind with
  | Type_abstract, _ -> ()
  | Type_variant l1, Type_variant l2 ->
      let f cons =
        try
          let cons2 = List.find (fun c2 -> c2.vc_name = cons.vc_name) l2 in
          let new_desc =
            ...
            | Some d1, Some d2 ->
                if List.mem Merge_description merge_options
                then Some (merge_info merge_options d1 d2)
                else Some d1
          in
          cons.vc_text <- new_desc
        with Not_found ->
          if !Odoc_global.inverse_merge_ml_mli
          then ()
          else raise (Failure (Odoc_messages.different_types mli.ty_name))
      in
      List.iter f l1
  | Type_record l1, Type_record l2 ->
      let f record =
        try
          let record2 =
            List.find (fun r -> r.rf_name = record.rf_name) l2
          in
          let new_desc =
...
let merge_type_extension merge_options mli ml =
  ...
  mli.te_code
  <- (match mli.te_code with None -> ml.te_code | _ -> mli.te_code)
...
let rec merge_param_info pi_mli pi_ml =
  ...
  | Simple_name sn_mli, Simple_name sn_ml ->
      if sn_mli.sn_name = ""
      then Simple_name {sn_mli with sn_name = sn_ml.sn_name}
      else pi_mli
  | Simple_name _, Tuple _ -> pi_mli
        ...
        let _ =
          List.find
            (fun ele ->
              match ele with
              | Class_attribute a2 ->
                  if a2.att_value.val_name = a.att_value.val_name
                  then (
                    ...
                    then (a.att_value).val_code <- a2.att_value.val_code;
                    true )
                  else false
              | _ -> false )
        ...
        let _ =
          List.find
            (fun ele ->
              match ele with
              | Class_method m2 ->
                  if m2.met_value.val_name = m.met_value.val_name
                  then (
                    ...
                    if !Odoc_global.keep_code
                    then (m.met_value).val_code <- m2.met_value.val_code;
                    true )
                  else false
              | _ -> false )
        ...
        | _, Element_type_extension te2 :: rest ->
            let merge_ext xt =
              try
                let xt2 =
                  List.find
                    (fun xt2 -> xt.xt_name = xt2.xt_name)
                    te2.te_constructors
                in
          ...
          List.find
            (fun ele ->
              ...
              | Element_exception ex2 ->
                  if ex2.ex_name = ex.ex_name
                  then (
                    ex.ex_info
                    <- merge_info_opt merge_options ex.ex_info ex2.ex_info;
                    ex.ex_loc
                    <- {ex.ex_loc with loc_impl = ex2.ex_loc.loc_impl};
                    ex.ex_code
                    <- ( match ex.ex_code with
                       | None -> ex2.ex_code
                       | _ -> ex.ex_code );
                    true )
                  else false
        ...
        let _ =
          List.find
            (fun ele ->
              match ele with
              | Element_value v2 ->
                  if v2.val_name = v.val_name
                  then (
                    v.val_info
                    <- merge_info_opt merge_options v.val_info v2.val_info;
                    v.val_loc
                    <- {v.val_loc with loc_impl = v2.val_loc.loc_impl};
                    (* in the .mli we don't know any parameters so we add the ones in the .ml *)
                    v.val_parameters
                    <- merge_parameters v.val_parameters v2.val_parameters;
                    (* we must reassociate comments in @param to the corresponding
                        parameters because the associated comment of a parameter may have been changed by the merge.*)
                    Odoc_value.update_value_parameters_text v;
                    if !Odoc_global.keep_code then v.val_code <- v2.val_code;
                    true )
                  else false
              | _ -> false )
            ...
            (fun ele ->
              ...
              | Element_exception ex2 ->
                  ...
                  then (
                    ...
                    <- merge_info_opt merge_options ex.ex_info ex2.ex_info;
                    ex.ex_loc
                    <- {ex.ex_loc with loc_impl = ex.ex_loc.loc_impl};
                    ex.ex_code
                    <- ( match ex.ex_code with
                       | None -> ex2.ex_code
                       | _ -> ex.ex_code );
...
let merge merge_options modules_list =
  let rec iter = function
    ...
    | m :: q -> (
        ...
        let l_same, l_others =
          List.partition (fun m2 -> m.m_name = m2.m_name) q
        in
        match l_same with
        | [] ->
        ...
        | [m2] -> (
            (* we can merge m with m2 if there is an implementation
                and an interface.*)
            let f b =
              if !Odoc_global.inverse_merge_ml_mli then not b else b
            in
            match f m.m_is_interface, f m2.m_is_interface with
            | true, false ->
                merge_modules merge_options m m2 :: iter l_others
            | false, true ->
                merge_modules merge_options m2 m :: iter l_others
            | false, false ->
                if !Odoc_global.inverse_merge_ml_mli
                then
                  ...
                  raise
                    (Failure (Odoc_messages.two_implementations m.m_name))
            | true, true ->
                if !Odoc_global.inverse_merge_ml_mli
                then
                  (* two Module.t for the .ml ! *)
