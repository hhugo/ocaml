...
let merge_before_tags l =
  let rec iter acc =
    ...
    | (v, text) :: q ->
      ...
      let acc =
        let text =
          List.fold_left (fun acc t -> acc @ [ Raw " " ] @ t) text
            (List.map snd l1)
        in
        (v, text) :: acc
      in
...
let merge_types merge_options mli ml =
  ...
  | Type_record l1, Type_record l2 ->
    let f record =
      try
        let record2 = List.find (fun r -> r.rf_name = record.rf_name) l2 in
        let new_desc =
  ...
  List.iter
    (fun a ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_class.class_attributes mli);
  (* merge methods *)
  List.iter
    (fun m ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_class.class_methods mli)
  ...
  List.iter
    (fun a ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_class.class_type_attributes mli);
  (* merge methods *)
  List.iter
    (fun m ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_class.class_type_methods mli)
  ...
  List.iter
    (fun ex ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_type_exceptions mli);
  (* merge types *)
  List.iter
    (fun ty ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_type_types mli);
  (* merge submodules *)
  List.iter
    (fun m ->
       try
...
*)
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_type_modules mli);
  (* merge module types *)
  List.iter
    (fun m ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_type_module_types mli);
  ...
  List.iter
    (fun v ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_type_values mli);
  (* merge classes *)
  List.iter
    (fun c ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_type_classes mli);
  (* merge class types *)
  List.iter
    (fun c ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_type_class_types mli)
  ...
  List.iter
    (fun ex ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_exceptions mli);
  (* merge types *)
  List.iter
    (fun ty ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_types mli);
  (* merge submodules *)
  List.iter
    (fun m ->
       try
...
*)
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_modules mli);
  (* merge module types *)
  List.iter
    (fun m ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_module_types mli);
  (* A VOIR : merge included modules ? *)
  (* merge values *)
  List.iter
    (fun v ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_values mli);
  (* merge classes *)
  List.iter
    (fun c ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_classes mli);
  (* merge class types *)
  List.iter
    (fun c ->
       try
         ...
         ()
       with
       | Not_found -> ()) (Odoc_module.module_class_types mli);
  mli
...
let merge merge_options modules_list =
  let rec iter =
    ...
    | m :: q ->
      ...
      let (l_same, l_others) = List.partition (fun m2 -> m.m_name = m2.m_name) q
      in
      begin match l_same with
      | [] ->
      ...
      | m2 :: [] ->
        ...
        begin match f m.m_is_interface, f m2.m_is_interface with
