...
let merge_info merge_options (m1 : info) (m2 : info) =
  ...
  let new_version =
    ...
    | Some v1, Some v2 ->
        if List.mem Merge_version merge_options then Some (v1 ^ " " ^ v2)
        else Some v1
  in
  let new_sees =
  ...
  let new_since =
    ...
    | Some v, None | None, Some v -> Some v
    | Some v1, Some v2 ->
        if List.mem Merge_since merge_options then Some (v1 ^ " " ^ v2)
        else Some v1
  in
  let new_before =
...
let rec merge_param_info pi_mli pi_ml =
  ...
  | Tuple (l_mli, t_mli), Tuple (l_ml, _) ->
      ...
      if List.length l_mli <> List.length l_ml then pi_mli
      else
