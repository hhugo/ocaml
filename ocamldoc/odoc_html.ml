...
class virtual text =
  object (self)
    ...
    method html_of_text ?(with_p = false) b t =
      if not with_p then List.iter (self#html_of_text_element b) t
    ...
    method html_of_Code b s =
      if !colorize_code then self#html_of_code b ~with_pre:false s
      else (
        bs b "<code class=\"";
    ...
    method html_of_Title b n label_opt t =
      ...
      let tag_o, tag_c =
        if n > 6 then (Printf.sprintf "div class=\"h%d\"" (n + 1), "div")
        else
...
module Generator = struct
  ...
  class html =
    object (self)
      ...
      method index_prefix =
        if !Odoc_global.out_file = Odoc_messages.default_out_file then "index"
      ...
      method html_of_module_kind b father ?modu kind =
        ...
        | Module_alias a ->
            ...
            bs b "</code>"
        | Module_functor (p, k) ->
            if !html_short_functors then bs b " "
      ...
      method html_of_type b t =
        ...
        | Type_variant l ->
            ...
            let print_one constr =
              ...
              | l, None ->
                  bs b (" " ^ self#keyword "of" ^ " ");
                  self#html_of_cstr_args ~par:false b father constr.vc_name
                    " * " l
              | Cstr_tuple [], Some r ->
                  bs b (" " ^ self#keyword ":" ^ " ");
                  self#html_of_type_expr b father r
        ...
        | Type_record l ->
            ...
            let close_env =
              ...
              | Some _ -> "</pre>"
            in
            self#html_of_record ~father ~close_env (Naming.recfield_target t) b
              l
        | Type_open ->
            bs b "= ..";
            bs b "</pre>");
      ...
      method html_of_module b ?(info = true) ?(complete = true)
        ...
        bs b "</pre>";
        if info then
          (if complete then self#html_of_info ~cls:"module top" ~indent:true
      ...
      method html_of_modtype b ?(info = true) ?(complete = true)
        ...
        bs b "</pre>";
        if info then
          (if complete then self#html_of_info ~cls:"modtype top" ~indent:true
          else self#html_of_info_first_sentence)
            b mt.mt_info
        else ()
      ...
      method html_of_class b ?(complete = true) ?(with_link = true) c =
        ...
        bs b "</pre>";
        (if complete then self#html_of_info ~cls:"class top" ~indent:true
        else self#html_of_info_first_sentence)
          b c.cl_info
      ...
      method html_of_class_type b ?(complete = true) ?(with_link = true) ct =
        ...
        bs b "</pre>";
        (if complete then self#html_of_info ~cls:"classtype top" ~indent:true
        else self#html_of_info_first_sentence)
          b ct.clt_info
      ...
      method generate_for_module pre post modu =
        try
          ...
          bs b "<h1>";
          if modu.m_text_only then bs b modu.m_name
          else (
            bs b
              (if Module.module_is_functor modu then Odoc_messages.functo
