...
module Html = ( val match !Odoc_args.current_generator with
                    ...
                    | _ ->
                        failwith
                          "A non-html generator is already set. Cannot \
                           install the Todo-list html generator"
                  : Odoc_html.Html_generator )
  ...
  class html =
    object (self)
      ...
      method! private html_of_value b v =
        ...
        bs b "</pre>";
        if !Odoc_html.with_parameter_list
        then
      ...
      default_style_options <-
        ...
        ; "div.codeblock { "
          ...
          ^ "border-width: 1px 1px 1px 3px; border-style: solid; \
             border-color: grey; }"
        ; "span.code_expand { color: blue; text-decoration: underline; \
           cursor: pointer; "
          ^ "margin-left: 1em ; } " ]
    end
end
