...
module Html = (val match !Odoc_args.current_generator with
                   | None ->
  ...
  class html : Html.html =
    object (self)
      ...
      method! generate modules =
        ...
        let title =
          match !Odoc_info.Global.title with None -> "" | Some s -> s
        in
