...
module Html =
  ...
  | _ ->
    failwith
      "A non-html generator is already set. Cannot install \
       the Todo-list html generator"
  :
...
module Generator = struct
  class scanner html =
    object (self)
                 ...
                 | "todo" ->
                   ...
                   | Odoc_info.Code s :: q ->
                     (try
                        ...
                        let head = Odoc_info.Code (Printf.sprintf "[%d] " n) in
                        (Some n, head :: q) :: acc
                      with
                      | _ -> (None, text) :: acc)
                   | _ -> (None, text) :: acc
            ...
            let l =
              List.sort
                (fun a b ->
                   ...
                   | (None, _), _ -> (-1)
                   | _, (None, _) -> 1
                   | (Some n1, _), (Some n2, _) -> compare n1 n2) l
            in
            p b "<pre><a href=\"%s\">%s</a></pre><div class=\"info\">" target
              name;
            let col =
              function
              | None -> "#000000"
      ...
      method! scan_value v =
        self#gen_if_tag v.val_name (Odoc_html.Naming.complete_value_target v)
          v.val_info
      
      method! scan_type t =
        self#gen_if_tag t.ty_name (Odoc_html.Naming.complete_type_target t)
          t.ty_info
      
      method! scan_extension_constructor x =
        self#gen_if_tag x.xt_name (Odoc_html.Naming.complete_extension_target x)
          x.xt_type_extension.te_info
      
      method! scan_exception e =
        self#gen_if_tag e.ex_name (Odoc_html.Naming.complete_exception_target e)
          e.ex_info
      
      method! scan_attribute a =
        self#gen_if_tag a.att_value.val_name
          (Odoc_html.Naming.complete_attribute_target a) a.att_value.val_info
      
      method! scan_method m =
        self#gen_if_tag m.met_value.val_name
          (Odoc_html.Naming.complete_method_target m) m.met_value.val_info
      ...
      method! scan_module_elements m =
        List.iter
          (fun ele ->
             ...
             | Odoc_module.Element_included_module im ->
               self#scan_included_module im
             ...
             | Odoc_module.Element_type_extension te ->
               self#scan_type_extension te
      ...
      method! scan_class_pre c =
        self#gen_if_tag c.cl_name (fst (Odoc_html.Naming.html_files c.cl_name))
          c.cl_info;
        true
      
      method! scan_class_type_pre ct =
        self#gen_if_tag ct.clt_name
          (fst (Odoc_html.Naming.html_files ct.clt_name)) ct.clt_info;
        true
      
      method! scan_module_pre m =
        self#gen_if_tag m.m_name (fst (Odoc_html.Naming.html_files m.m_name))
          m.m_info;
        true
      
      method! scan_module_type_pre mt =
        self#gen_if_tag mt.mt_name
          (fst (Odoc_html.Naming.html_files mt.mt_name)) mt.mt_info;
        true
    end
  ...
  class html : Html.html =
    object (self)
      ...
      method! generate modules =
        ...
        let oc =
          open_out (Filename.concat !Odoc_info.Global.target_dir "todo.html")
        in
        Buffer.output_buffer oc b;
        close_out oc
      
      initializer scanner <- new scanner self
    end
end
