...
module Typedtree_search = struct
  ...
  let add_to_hashes table table_values tt =
    ...
    | Typedtree.Tstr_exception ext ->
        Hashtbl.add
          table
          (E (Name.from_ident ext.tyexn_constructor.ext_id))
          tt
  ...
  let search_attribute_type cls name =
    let rec iter = function
      | [] -> raise Not_found
      | { cf_desc =
            Typedtree.Tcf_val (_, _, ident, Tcfk_concrete (_, exp), _) }
        :: _
        when Name.from_ident ident = name ->
          exp.Typedtree.exp_type
      | {cf_desc = Typedtree.Tcf_val (_, _, ident, Tcfk_virtual typ, _)}
        :: _
        when Name.from_ident ident = name ->
...
module Analyser (My_ir : Odoc_sig.Info_retriever) = struct
  ...
  let tt_param_info_from_pattern env f_desc pat =
    let rec iter_pattern pat =
      ...
      | Typedtree.Tpat_construct (_, cons_desc, _)
        when (* we give a name to the parameter only if it unit *)
  ...
  let rec tt_analyse_function_parameters env current_comment_opt
      pat_exp_list =
    ...
    | [{c_lhs = pattern_param; c_rhs = func_body}] -> (
        ...
        let p, next_exp =
          ...
          | Simple_name {sn_name = "*opt*"} -> (
            ...
            | _ ->
                print_DEBUG3
                  "Pas le bon filtre pour le parametre optionnel avec \
                   valeur par defaut.";
                parameter, func_body )
...
*)
            ...
            then
              ...
              let current_param, next_exp =
                ...
                | Simple_name {sn_name = "*opt*"} -> (
                  ...
                  | _ ->
                      print_DEBUG3
                        "Pas le bon filtre pour le parametre optionnel \
    ...
    let rec iter acc_inher acc_fields last_pos = function
      ...
      | item :: q -> (
          ...
          | Parsetree.Pcf_inherit (_, p_clexp, _) ->
              ...
              let info_opt, ele_comments =
                get_comments_in_class
                  last_pos
                  p_clexp.Parsetree.pcl_loc.Location.loc_start
                    .Lexing.pos_cnum
              in
              let text_opt =
                match info_opt with
          ...
          | Parsetree.Pcf_method
              ...
              let met_type =
                try
                  Odoc_sig.Signature_search.search_method_type
                    label
                    tt_class_sig
                with Not_found ->
                  raise
  ...
  and analyse_structure_item env current_module_name loc pos_limit
      comment_opt parsetree_item_desc _typedtree table table_values =
    ...
    | Parsetree.Pstr_primitive val_desc ->
        ...
        print_DEBUG
          ...
          ^ name_pre
          ^ ", ["
          ^ String.concat ", " val_desc.Parsetree.pval_prim
          ^ "]" );
    ...
    | Parsetree.Pstr_type (rf, name_typedecl_list) ->
        ...
        in
        let rec f ?(first = false) maybe_more_acc last_pos
            name_type_decl_list =
          ...
          | type_decl :: q ->
              ...
              let pos_limit2 =
                ...
                | [] -> pos_limit
                | td :: _ ->
                    td.Parsetree.ptype_loc.Location.loc_start
                      .Lexing.pos_cnum
              in
              ...
              let t =
                ...
                ; ty_manifest =
                    ...
                    | Some t ->
                        Some
                          (Sig.manifest_structure env name_comment_list t)
                    )
    ...
    | Parsetree.Pstr_typext tyext ->
        ...
        let tt_tyext =
          ...
          | ext :: _ -> (
            try
              Typedtree_search.search_extension
                table
        ...
        let new_te =
          ...
          ; te_code =
              ( if !Odoc_global.keep_code
              then Some (get_string_of_file loc_start loc_end)
              else None ) }
        in
    ...
    | Parsetree.Pstr_module
        {Parsetree.pmb_name = name; pmb_expr = module_expr} -> (
      try
        (* of string * module_expr *)
        let tt_module_expr =
          Typedtree_search.search_module table name.txt
        in
        let new_module_pre =
          analyse_module
            env
    ...
    | Parsetree.Pstr_recmodule mods ->
        ...
        let new_env =
          List.fold_left
            (fun acc_env {Parsetree.pmb_name = name; pmb_expr = mod_exp} ->
              let complete_name =
                Name.concat current_module_name name.txt
              in
        ...
        let rec f ?(first = false) last_pos name_mod_exp_list =
          ...
          | {Parsetree.pmb_name = name; pmb_expr = mod_exp} :: q ->
              ...
              in
              let loc_start =
                mod_exp.Parsetree.pmod_loc.Location.loc_start
                  .Lexing.pos_cnum
              in
              let loc_end =
    ...
    | Parsetree.Pstr_class class_decl_list ->
        ...
        let rec f ?(first = false) last_pos class_decl_list =
          ...
          | class_decl :: q ->
              ...
              in
              let last_pos2 =
                class_decl.Parsetree.pci_loc.Location.loc_end
                  .Lexing.pos_cnum
              in
              let new_class =
                analyse_class
        ...
        let rec f ?(first = false) last_pos class_type_decl_list =
          ...
          | class_type_decl :: q ->
              ...
              let tt_cltype_declaration =
                try
                  Typedtree_search.search_class_type_declaration
                    table
                    name.txt
                with Not_found ->
                  raise
  ...
  and analyse_module env current_module_name module_name comment_opt
    ...
    let m_code_intf =
      ...
      | Parsetree.Pmod_constraint (_, pmodule_type) ->
          let loc_start =
            pmodule_type.Parsetree.pmty_loc.Location.loc_start
              .Lexing.pos_cnum
          in
          let loc_end =
    ...
    | ( Parsetree.Pmod_functor (_, pmodule_type, p_module_expr2)
      , Typedtree.Tmod_functor (ident, _, mtyp, tt_module_expr2) ) ->
        let loc =
          match pmodule_type with
          | None -> Location.none
          | Some pmty -> pmty.Parsetree.pmty_loc
        in
        let loc_start = loc.Location.loc_start.Lexing.pos_cnum in
        let loc_end = loc.Location.loc_end.Lexing.pos_cnum in
        ...
        let param =
          ...
          ; mp_type =
              Misc.may_map
                (fun m -> Odoc_env.subst_module_type env m.mty_type)
                mtyp
          ; mp_type_code
          ; mp_kind }
        in
    ...
    | ( Parsetree.Pmod_constraint (p_module_expr2, p_modtype)
      , Typedtree.Tmod_constraint (tt_module_expr2, tt_modtype, _, _) ) ->
        print_DEBUG
          ( "Odoc_ast: case Parsetree.Pmod_constraint + \
             Typedtree.Tmod_constraint "
          ^ module_name );
        let m_base2 =
          analyse_module
        ...
        if !Odoc_global.filter_with_module_constraints
        then filter_module_with_module_type_constraint m_base2 tt_modtype;
        { m_base with
          m_type = tt_modtype
    ...
    | ( Parsetree.Pmod_structure p_structure
      , Typedtree.Tmod_constraint
        ...
        print_DEBUG
          ( "Odoc_ast: case Parsetree.Pmod_structure + \
             Typedtree.Tmod_constraint "
          ^ module_name );
        let elements =
          analyse_structure
    ...
    | Parsetree.Pmod_unpack p_exp, Typedtree.Tmod_unpack (_t_exp, tt_modtype)
      ->
        print_DEBUG
          ( "Odoc_ast: case Parsetree.Pmod_unpack + Typedtree.Tmod_unpack "
          ^ module_name );
        let code =
          let loc = p_module_expr.Parsetree.pmod_loc in
          let loc_end = loc.Location.loc_end.Lexing.pos_cnum in
    ...
    | _parsetree, _typedtree ->
        ...
        let s_parse =
          ...
          (*DEBUG*)
          | Parsetree.Pmod_unpack _ -> "Pmod_unpack"
          (*DEBUG*)
          | Parsetree.Pmod_extension _ -> "Pmod_extension"
          (*DEBUG*)
        in
        (*DEBUG*)
        let s_typed =
          (*DEBUG*)
          match _typedtree with
          (*DEBUG*)
          ...
          (*DEBUG*)
          | Typedtree.Tmod_unpack _ -> "Tmod_unpack"
          (*DEBUG*)
        in
        (*DEBUG*)
        ...
        print_DEBUG
          (Printf.sprintf
             ...
             s_typed);
        raise
          (Failure "analyse_module: parsetree and typedtree don't match.")
