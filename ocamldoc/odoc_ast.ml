...
(*                                                                        *)
(**************************************************************************)
open Asttypes (** Analysis of implementation files. *)
...
*)
module Typedtree_search = struct
  type ele =
    | M of string
    | MT of string
    | T of string
  ...
  type tab_values =
    (Odoc_module.Name.t, Typedtree.pattern * Typedtree.expression) Hashtbl.t
  
  let iter_val_pattern =
    function
  ...
  let add_to_hashes table table_values tt =
    ...
    | Typedtree.Tstr_module mb ->
      Option.iter (fun id -> Hashtbl.add table (M (Name.from_ident id)) tt)
        mb.mb_id
  ...
  let search_attribute_type cls name =
    let rec iter =
      function
      | [] -> raise Not_found
      | { cf_desc = Typedtree.Tcf_val (_, _, ident, Tcfk_concrete (_, exp), _) } ::
          _
  ...
  let search_method_expression cls name =
    let rec iter =
      function
      | [] -> raise Not_found
      | { cf_desc = Typedtree.Tcf_method (label, _, Tcfk_concrete (_, exp)) } ::
          _
        when label.txt = name ->
        exp
      | _ :: q -> iter q
    ...
    *)
    let tt_param_info_from_pattern env f_desc pat =
      let rec iter_pattern pat =
        ...
        | Typedtree.Tpat_alias (pat, _, _) -> iter_pattern pat
        | Typedtree.Tpat_tuple patlist ->
          Tuple
            (List.map iter_pattern patlist, Odoc_env.subst_type env pat.pat_type)
        | Typedtree.Tpat_construct (_, cons_desc, _, _)
          when (* we give a name to the parameter only if it is unit *)
      ...
      | { c_lhs = pattern_param; c_rhs = func_body } :: [] ->
        ...
        let (p, next_exp) =
          ...
          | Simple_name { sn_name = "*opt*" } ->
            begin match func_body.exp_desc with
            | Typedtree.Texp_let
                (_,
                ...
                Simple_name
                  {
                    sn_name = name;
                    sn_text =
                      Odoc_parameter.desc_from_info_opt current_comment_opt name;
                    sn_type = Odoc_env.subst_type env exp.exp_type
                  }
              in
              new_param, func_body2
            | _ -> parameter, func_body
            end
          | _ -> parameter, func_body
        in
        (* continue if the body is still a function *)
        begin match next_exp.exp_desc with
        | Texp_function { cases = pat_exp_list; _ } ->
          p ::
            tt_analyse_function_parameters env current_comment_opt pat_exp_list
        | _ ->
    ...
    let tt_analyse_value
        env current_module_name comment_opt loc pat_exp rec_flag
    =
      let (pat, exp) = pat_exp in
      match pat.pat_desc, exp.exp_desc with
      | Typedtree.Tpat_var (ident, _),
        Typedtree.Texp_function { cases = pat_exp_list2; _ }
        ->
        ...
        let code =
          if !Odoc_global.keep_code then
            Some
              (get_string_of_file loc.Location.loc_start.Lexing.pos_cnum
        ...
        let new_value =
          {
            ...
            val_parameters =
              tt_analyse_function_parameters env comment_opt pat_exp_list2;
            val_code = code;
...
*)
      ...
      | Typedtree.Tcl_apply (class_expr, _)
        ->
        tt_name_of_class_expr class_expr
      (*
...
*)
    ...
    *)
    let rec tt_analyse_method_expression
        env current_method_name comment_opt ?(first=true) exp
    =
        ...
        | l ->
              ...
              Simple_name
                {
                  ...
                  sn_text = None;
                  sn_type =
                    Odoc_env.subst_type env pattern_param.Typedtree.pat_type
                }
            in
            [ new_param ]
          | { c_lhs = pattern_param; c_rhs = body } :: [] ->
            ...
            if not first then
              let parameter =
                tt_param_info_from_pattern env
                  (Odoc_parameter.desc_from_info_opt comment_opt) pattern_param
              in
              (* For optional parameters with a default value, a special treatment is required. *)
              (* We look if the name of the parameter we just add is "*opt*", which means
                 that there is a let param_name = ... in ... just right now. *)
                      ...
                      Simple_name
                        {
                          ...
                          sn_type =
                            Odoc_env.subst_type env exp.Typedtree.exp_type
                        }
                    in
                    new_param, body2
                  | _ -> parameter, body
              ...
              in
              current_param ::
                tt_analyse_method_expression env current_method_name comment_opt
                  ~first:false next_exp
            else
              tt_analyse_method_expression env current_method_name comment_opt
                ~first:false body
          end
    ...
    let analyse_class_structure
        env current_class_name tt_class_sig last_pos pos_limit p_cls tt_cls
        _table
    =
      let rec iter acc_inher acc_fields last_pos =
        function
        | [] ->
          ...
          let ele_comments =
            List.fold_left
              (fun acc sc ->
                 ...
                 | None -> acc
                 | Some t -> acc @ [ Class_comment t ]) [] ele_coms
          in
          acc_inher, acc_fields @ ele_comments
        | item :: q ->
          let loc = item.Parsetree.pcf_loc in
          begin match item.Parsetree.pcf_desc with
          ...
          | Parsetree.Pcf_val ({ txt = label }, mutable_flag, k) ->
            let virt =
              match k with
              | Parsetree.Cfk_virtual _ -> true
              | Parsetree.Cfk_concrete _ -> false
            in
            ...
            in
            iter acc_inher (acc_fields @ ele_comments @ [ Class_attribute att ])
              loc.Location.loc_end.Lexing.pos_cnum q
          | Parsetree.Pcf_method
              ({ txt = label }, private_flag, Parsetree.Cfk_virtual _)
            ->
            let complete_name = Name.concat current_class_name label in
            let (info_opt, ele_comments) =
              get_comments_in_class last_pos
            ...
            let met_type =
              try
                Odoc_sig.Signature_search.search_method_type label tt_class_sig
              with
              | Not_found ->
                raise
                  (Failure
                     (Odoc_messages.method_type_not_found current_class_name
                        label))
            in
            let real_type =
            ...
            iter acc_inher (acc_fields @ ele_comments @ [ Class_method met ])
              loc.Location.loc_end.Lexing.pos_cnum q
          | Parsetree.Pcf_method
              ({ txt = label }, private_flag, Parsetree.Cfk_concrete _)
            ->
            let complete_name = Name.concat current_class_name label in
            let (info_opt, ele_comments) =
              get_comments_in_class last_pos
            ...
            let exp =
              ...
              | Not_found ->
                raise
                  (Failure
                     (Odoc_messages.method_not_found_in_typedtree complete_name))
            in
            let real_type =
                ...
                met_value =
                  {
                    ...
                    val_recursive = false;
                    val_parameters =
                      tt_analyse_method_expression env complete_name info_opt
                        exp;
                    val_code = code;
                    val_loc = { loc_impl = Some loc; loc_inter = None }
                  };
          ...
          | Parsetree.Pcf_initializer exp ->
            iter acc_inher acc_fields
              exp.Parsetree.pexp_loc.Location.loc_end.Lexing.pos_cnum q
    ...
    let rec analyse_class_kind
        env current_class_name comment_opt last_pos p_class_expr tt_class_exp
        table
    =
      ...
      | Parsetree.Pcl_structure p_class_structure,
        Typedtree.Tcl_structure tt_class_structure
        ->
        ...
        let tt_class_sig =
          ...
          | _ ->
            raise
              (Failure
                 "analyse_class_kind: no class signature for a class \
                  structure.")
        in
        let (inherited_classes, class_elements) =
          analyse_class_structure env current_class_name tt_class_sig last_pos
            p_class_expr.Parsetree.pcl_loc.Location.loc_end.Lexing.pos_cnum
            p_class_structure tt_class_structure table
        in
      ...
      | Parsetree.Pcl_fun (_label, _expression_opt, _pattern, p_class_expr2),
        ...
        let (parameter, next_tt_class_exp) =
          ...
          | Typedtree.Tpat_var (ident, _)
            when Name.from_ident ident = "*opt*" ->
            (* there must be a Tcl_let just after *)
            begin match tt_class_expr2.Typedtree.cl_desc with
            | Typedtree.Tcl_let
                (_,
                 { vb_pat = { pat_desc = Typedtree.Tpat_var (id, _) };
                   vb_expr = exp } ::
          ...
          | _ ->
            ...
            let new_param =
              tt_param_info_from_pattern env
                (Odoc_parameter.desc_from_info_opt comment_opt) pat
            in
            new_param, tt_class_expr2
        in
        let (params, k) =
          analyse_class_kind env current_class_name comment_opt last_pos
            p_class_expr2 next_tt_class_exp table
        in
        parameter :: params, k
      | Parsetree.Pcl_apply (p_class_expr2, _),
        Tcl_apply (tt_class_expr2, exp_opt_optional_list)
        ->
        let applied_name =
          ...
          | Typedtree.Tcl_ident (p, _, _) -> Name.from_path p
          (* FIXME : obtain the full name *)
          | _ ->
            (* FIXME : Unfortunately, we don't have a Tclass_ident :-( even for a class tutu = toto *)
            begin match p_class_expr2.Parsetree.pcl_desc with
        ...
        let param_exps =
          List.fold_left
            (fun acc (_, exp_opt) ->
               match exp_opt with
               | None -> acc
               | Some e -> acc @ [ e ]) [] exp_opt_optional_list
        in
        let param_types = List.map (fun e -> e.Typedtree.exp_type) param_exps in
        let params_code =
          List.map
            (fun e ->
               get_string_of_file e.exp_loc.Location.loc_start.Lexing.pos_cnum
                 e.exp_loc.Location.loc_end.Lexing.pos_cnum) param_exps
        in
        [],
        Class_apply
          {
      ...
      | Parsetree.Pcl_let (_, _, p_class_expr2),
        Typedtree.Tcl_let (_, _, _, tt_class_expr2)
        ->
        (* we don't care about these lets *)
        analyse_class_kind env current_class_name comment_opt last_pos
          p_class_expr2 tt_class_expr2 table
      | Parsetree.Pcl_constraint (p_class_expr2, _p_class_type2),
        ...
        let class_type_kind =
          ...
          Class_type
            {
              cta_name = Odoc_messages.object_end;
              cta_class = None;
              cta_type_parameters = []
            }
        in
      ...
      | _ ->
        raise
          (Failure "analyse_class_kind: Parsetree and typedtree don't match.")
    
    (** Analysis of a [Parsetree.class_declaration] and a [Typedtree.class_expr] to return a [t_class].*)
    let analyse_class
        env current_module_name comment_opt p_class_decl tt_type_params
        tt_class_exp table
    =
      ...
      let cltype = Odoc_env.subst_class_type env tt_class_exp.Typedtree.cl_type
      in
      let (parameters, kind) =
        analyse_class_kind env complete_name comment_opt pos_start
          p_class_decl.Parsetree.pci_expr tt_class_exp table
      in
      {
      ...
      let rec f =
        ...
        | Element_included_module im :: q, im_repl :: im_q ->
          Element_included_module { im_repl with  im_info = im.im_info } ::
            f (q, im_q)
        | Element_included_module im :: q, [] -> Element_included_module im :: q
        | ele :: q, l -> ele :: f (q, l)
      in
    ...
    let rec filter_module_with_module_type_constraint m mt =
      ...
      | Module_struct l, Types.Mty_signature lsig ->
        m.m_kind <-
          Module_struct
            (filter_module_elements_with_module_type_constraint l lsig);
        m.m_type <- mt
      | _ -> ()
      ...
      let pred ele =
        let f =
          ...
          | Element_module m ->
            (function
             | Types.Sig_module (ident, _, md, _, _) ->
               ...
               and n2 = Ident.name ident
               in
               begin match n1 = n2 with
               | true ->
                 filter_module_with_module_type_constraint m md.md_type;
                 true
               | false -> false
               end
             | _ -> false)
          | Element_module_type mt ->
            (function
             | Types.Sig_modtype (ident, { Types.mtd_type = Some t }, _) ->
               ...
               begin match n1 = n2 with
               | true ->
                 filter_module_type_with_module_type_constraint mt t;
                 true
               | false -> false
               end
             | _ -> false)
          ...
          | Element_type_extension te ->
            ...
            if l <> [] then (fun _ -> true) else (fun _ -> false)
          | Element_exception e ->
            (function
    ...
    let rec analyse_structure
        env current_module_name last_pos pos_limit parsetree typedtree
    =
      let (table, table_values) = Typedtree_search.tables typedtree.str_items in
      let rec iter env last_pos =
        function
        | [] ->
          let s = get_string_of_file last_pos pos_limit in
          let (_, ele_coms) = My_ir.all_special !file_name s in
          List.fold_left
            (fun acc sc ->
               match sc.Odoc_types.i_desc with
               | None -> acc
               | Some t -> acc @ [ Element_module_comment t ]) [] ele_coms
        | item :: q ->
          let (comment_opt, ele_comments) =
            get_comments_in_module last_pos
              item.Parsetree.pstr_loc.Location.loc_start.Lexing.pos_cnum
          in
          let pos_limit2 =
            match q with
            | [] -> pos_limit
            | item2 :: _ ->
              item2.Parsetree.pstr_loc.Location.loc_start.Lexing.pos_cnum
          in
          let (maybe_more, new_env, elements) =
            analyse_structure_item env current_module_name
              item.Parsetree.pstr_loc pos_limit2 comment_opt
              item.Parsetree.pstr_desc typedtree table table_values
          in
          ele_comments @
            elements @
              iter new_env
                (item.Parsetree.pstr_loc.Location.loc_end.Lexing.pos_cnum +
                   maybe_more) q
      in
      iter env last_pos parsetree
    
    (** Analysis of a parse tree structure item to obtain a new environment and a list of elements.*)
    and analyse_structure_item
        env current_module_name loc pos_limit comment_opt parsetree_item_desc
    ...
    =
      ...
      | Parsetree.Pstr_eval _ ->
        (* don't care *)
        0, env, []
      | Parsetree.Pstr_attribute _ | Parsetree.Pstr_extension _ -> 0, env, []
      | Parsetree.Pstr_value (rec_flag, pat_exp_list) ->
        ...
        let rec iter_pat =
          ...
          | Parsetree.Ppat_tuple _ -> None
          (* FIXME when we will handle tuples *)
          | Parsetree.Ppat_constraint (pat, _) ->
            iter_pat pat.Parsetree.ppat_desc
          | _ -> None
        in
        let rec iter ?(first=false) last_pos acc_env acc p_e_list =
          ...
          | { Parsetree.pvb_pat = pat; pvb_expr = exp } :: q ->
            ...
            let new_last_pos =
              exp.Parsetree.pexp_loc.Location.loc_end.Lexing.pos_cnum
            in
            begin match value_name_opt with
            | None -> iter new_last_pos acc_env acc q
            | Some name ->
              (try
                 let pat_exp =
                   Typedtree_search.search_value table_values name.txt
                 in
                 let (info_opt, ele_comments) =
                 ...
                 in
                 let l_values =
                   tt_analyse_value env current_module_name info_opt loc pat_exp
                     rec_flag
                 in
                 let new_env =
                   List.fold_left (fun e v -> Odoc_env.add_value e v.val_name)
                     acc_env l_values
                 in
                 let l_ele = List.map (fun v -> Element_value v) l_values in
                 iter new_last_pos new_env (acc @ ele_comments @ l_ele) q
               with
               | Not_found -> iter new_last_pos acc_env acc q)
            end
        in
        let (new_env, l_ele) =
          iter ~first:true loc.Location.loc_start.Lexing.pos_cnum env []
            pat_exp_list
        in
        0, new_env, l_ele
        ...
        let extended_env =
          List.fold_left
            (fun acc_env { Parsetree.ptype_name = { txt = name } } ->
               let complete_name = Name.concat current_module_name name in
               Odoc_env.add_type acc_env complete_name) env name_typedecl_list
        in
        let env =
          match rf with
        ...
        let rec f ?(first=false) maybe_more_acc last_pos name_type_decl_list =
          ...
          | type_decl :: q ->
            ...
            let pos_limit2 =
              ...
              | [] -> pos_limit
              | td :: _ ->
                td.Parsetree.ptype_loc.Location.loc_start.Lexing.pos_cnum
            in
            ...
            let tt_type_decl =
              ...
              | Not_found ->
                raise
                  (Failure
                     (Odoc_messages.type_not_found_in_typedtree complete_name))
            in
            let tt_type_decl = tt_type_decl.Typedtree.typ_type in
            let (com_opt, ele_comments) =
              (* the comment for the first type was already retrieved *)
              if first then
                comment_opt, []
              else
            ...
            in
            let kind =
              Sig.get_type_kind env name_comment_list
            ...
            let t =
              {
                ...
                ty_manifest =
                  begin match tt_type_decl.Types.type_manifest with
                  | None -> None
                  | Some t ->
      ...
      | Parsetree.Pstr_typext tyext ->
        ...
        let tt_tyext =
          ...
          | ext :: _ ->
            (try
               Typedtree_search.search_extension table
                 ext.Parsetree.pext_name.txt
             with
             | Not_found ->
               raise
                 (Failure
        ...
        let new_env =
          List.fold_left
            (fun acc_env { Parsetree.pext_name = { txt = name } } ->
               let complete_name = Name.concat current_module_name name in
               Odoc_env.add_extension acc_env complete_name) env
        ...
        let new_te =
          {
            te_info = comment_opt;
            te_type_name =
              Odoc_env.full_type_name new_env
                (Name.from_path tt_tyext.tyext_path);
            te_type_parameters =
              List.map
                (fun (ctyp, _) -> Odoc_env.subst_type new_env ctyp.ctyp_type)
                tt_tyext.tyext_params;
        ...
        let rec analyse_extension_constructors maybe_more exts_acc tt_ext_list =
          ...
          | tt_ext :: q ->
              ...
              | Text_rebind (path, _) ->
                {
                  ...
                  xt_alias =
                    Some
                      {
                        xa_name =
                          Odoc_env.full_extension_constructor_name env
                            (Name.from_path path);
                        xa_xt = None
                      };
                  xt_loc = { loc_impl = Some tt_ext.ext_loc; loc_inter = None };
            ...
            let pos_limit2 =
              match q with
              | [] -> pos_limit
              | next :: _ -> next.ext_loc.Location.loc_start.Lexing.pos_cnum
            in
        ...
        in
        let (maybe_more, exts) =
          analyse_extension_constructors 0 [] tt_tyext.tyext_constructors
        in
      ...
      | Parsetree.Pstr_exception ext ->
        ...
        let tt_ext =
          ...
          | Not_found ->
            raise
              (Failure
                 (Odoc_messages.exception_not_found_in_typedtree complete_name))
        in
        ...
        let new_ext =
          ...
          | Text_decl (_, tt_args, tt_ret_type) ->
            let loc_start = loc.Location.loc_start.Lexing.pos_cnum in
            let loc_end = loc.Location.loc_end.Lexing.pos_cnum in
            let ex_args = Sig.get_cstr_args env loc_end tt_args in
            {
      ...
      | Parsetree.Pstr_module
          { Parsetree.pmb_name = { txt = Some name }; pmb_expr = module_expr }
        ->
        (* of string * module_expr *)
        (try
           let tt_module_expr = Typedtree_search.search_module table name in
           let new_module_pre =
             analyse_module env current_module_name name comment_opt module_expr
               tt_module_expr
           in
           let code =
             if !Odoc_global.keep_code then
               ...
               None
           in
           let new_module = { new_module_pre with  m_code = code } in
         ...
         | Not_found ->
           ...
           raise
             (Failure
                (Odoc_messages.module_not_found_in_typedtree complete_name)))
      | Parsetree.Pstr_recmodule mods ->
          ...
          List.fold_left
            (fun acc_env { Parsetree.pmb_name = name; pmb_expr = mod_exp } ->
               ...
               | Some name ->
                 ...
                 in
                 let new_module =
                   analyse_module e current_module_name name None mod_exp
                     tt_mod_exp
                 in
                 begin match new_module.m_type with
                 | Types.Mty_signature s ->
        ...
        let rec f ?(first=false) last_pos name_mod_exp_list =
          ...
          | { Parsetree.pmb_name = { txt = None }; pmb_expr = mod_exp } :: q ->
            let loc_start =
              mod_exp.Parsetree.pmod_loc.Location.loc_start.Lexing.pos_cnum
            in
            let loc_end =
              mod_exp.Parsetree.pmod_loc.Location.loc_end.Lexing.pos_cnum
            in
            let (_, ele_comments) =
              (* the comment for the first type was already retrieved *)
              if first then
            ...
            ele_comments @ eles
          | { Parsetree.pmb_name = { txt = Some name }; pmb_expr = mod_exp } ::
              q
            ->
            let complete_name = Name.concat current_module_name name in
            let loc_start =
            ...
            in
            let new_module =
              analyse_module new_env current_module_name name com_opt mod_exp
                tt_mod_exp
            in
            let eles = f loc_end q in
        ...
        0, new_env, eles
      | Parsetree.Pstr_modtype
          { Parsetree.pmtd_name = name; pmtd_type = modtype }
        ->
        let complete_name = Name.concat current_module_name name.txt in
        let tt_module_type =
          ...
          | Not_found ->
            raise
              (Failure
                 (Odoc_messages.module_type_not_found_in_typedtree complete_name))
        in
        let (kind, sig_mtype) =
          match modtype, tt_module_type.mtd_type with
          | Some modtype, Some mty_type ->
            Some
              (Sig.analyse_module_type_kind env complete_name modtype
                 mty_type.mty_type),
      ...
      | Parsetree.Pstr_class class_decl_list ->
        ...
        let new_env =
          List.fold_left
            (fun acc_env class_decl ->
               let complete_name =
                 Name.concat current_module_name
                   class_decl.Parsetree.pci_name.txt
               in
               Odoc_env.add_class acc_env complete_name) env class_decl_list
        in
        let rec f ?(first=false) last_pos class_decl_list =
          ...
          | class_decl :: q ->
            let (tt_class_exp, tt_type_params) =
              try
                Typedtree_search.search_class_exp table
                  class_decl.Parsetree.pci_name.txt
              with
              | Not_found ->
                let complete_name =
                ...
                raise
                  (Failure
                     (Odoc_messages.class_not_found_in_typedtree complete_name))
            in
            let (com_opt, ele_comments) =
              if first then
                comment_opt, []
                ...
                get_comments_in_module last_pos
                  class_decl.Parsetree.pci_loc.Location.loc_start.
                  Lexing.pos_cnum
            in
            let last_pos2 =
              class_decl.Parsetree.pci_loc.Location.loc_end.Lexing.pos_cnum
            in
            let new_class =
              analyse_class new_env current_module_name com_opt class_decl
                tt_type_params tt_class_exp table
            in
            ele_comments @ Element_class new_class :: f last_pos2 q
        in
        0,
        new_env,
        f ~first:true loc.Location.loc_start.Lexing.pos_cnum class_decl_list
      | Parsetree.Pstr_class_type class_type_decl_list ->
        ...
        let new_env =
          List.fold_left
            (fun acc_env class_type_decl ->
               ...
               in
               Odoc_env.add_class_type acc_env complete_name) env
        ...
        let rec f ?(first=false) last_pos class_type_decl_list =
          ...
          | class_type_decl :: q ->
            ...
            let kind =
              Sig.analyse_class_type_kind new_env complete_name
                class_type_decl.Parsetree.pci_loc.Location.loc_start.
                Lexing.pos_cnum class_type_decl.Parsetree.pci_expr
            ...
            in
            let last_pos2 =
              class_type_decl.Parsetree.pci_loc.Location.loc_end.Lexing.pos_cnum
            in
            let new_ele =
              Element_class_type
                {
                  ...
                  clt_type =
                    Odoc_env.subst_class_type env
                      tt_cltype_declaration.Types.clty_type;
                  clt_type_parameters =
                    List.map (Odoc_env.subst_type new_env) type_params;
                  clt_virtual = virt;
                  clt_kind = kind;
                  clt_loc = { loc_impl = Some loc; loc_inter = None }
                }
            in
        ...
        0,
        new_env,
        f ~first:true loc.Location.loc_start.Lexing.pos_cnum
          class_type_decl_list
      | Parsetree.Pstr_include _ ->
        ...
        let im = { im_name = "dummy"; im_module = None; im_info = comment_opt }
        in
        0, env, [ Element_included_module im ]
    
    (* FIXME: extend the environment? With what? *)
    (** Analysis of a [Parsetree.module_expr] and a name to return a [t_module].*)
    and analyse_module
        env current_module_name module_name comment_opt p_module_expr
        tt_module_expr
    =
      ...
      let m_code_intf =
        ...
        | Parsetree.Pmod_constraint (_, pmodule_type) ->
          let loc_start =
            pmodule_type.Parsetree.pmty_loc.Location.loc_start.Lexing.pos_cnum
          in
          let loc_end =
            pmodule_type.Parsetree.pmty_loc.Location.loc_end.Lexing.pos_cnum
          in
          Some (get_string_of_file loc_start loc_end)
        | _ -> None
      in
      let m_base =
        {
          ...
          m_top_deps = [];
          m_code = None;
          (* code is set by the caller, after the module is created *)
          m_code_intf;
          m_text_only = false
        }
      in
      match
        p_module_expr.Parsetree.pmod_desc, tt_module_expr.Typedtree.mod_desc
      with
      ...
      | Parsetree.Pmod_structure p_structure,
        Typedtree.Tmod_structure tt_structure
        ->
        let elements =
          analyse_structure env complete_name pos_start pos_end p_structure
            tt_structure
        in
        (* we must complete the included modules *)
        let included_modules_from_tt = tt_get_included_module_list tt_structure
        in
        let elements2 =
      ...
      | Parsetree.Pmod_functor (param2, p_module_expr2),
        ...
        let param = { mp_name; mp_type; mp_type_code; mp_kind } in
        let dummy_complete_name = (*Name.concat "__"*) param.mp_name in
        (* TODO: FIX THIS  __ *)
        let new_env = Odoc_env.add_module env dummy_complete_name in
        let m_base2 =
          analyse_module new_env current_module_name module_name None
            p_module_expr2 tt_module_expr2
        in
      ...
      Parsetree.Pmod_apply (p_module_expr1, p_module_expr2),
        ...
        ->
        let m1 =
          analyse_module env current_module_name module_name None p_module_expr1
            tt_module_expr1
        in
        let m2 =
          analyse_module env current_module_name module_name None p_module_expr2
            tt_module_expr2
        in
      ...
      | Parsetree.Pmod_constraint (p_module_expr2, p_modtype),
        ...
        in
        let mtkind =
          Sig.analyse_module_type_kind env
            (Name.concat current_module_name "??") p_modtype tt_modtype
        in
      ...
      | Parsetree.Pmod_unpack p_exp, Typedtree.Tmod_unpack (_t_exp, tt_modtype)
        ->
        let code =
          let loc = p_module_expr.Parsetree.pmod_loc in
          let loc_end = loc.Location.loc_end.Lexing.pos_cnum in
        ...
        let name =
          ...
          | Mty_ident p -> Odoc_env.full_module_type_name env (Name.from_path p)
          | _ -> ""
        in
    ...
    let analyse_typed_tree
        source_file input_file (parsetree : Parsetree.structure)
        (typedtree : typedtree)
    =
      ...
      in
      let (len, info_opt) =
        Sig.preamble !file_name !file (fun x -> x.Parsetree.pstr_loc) parsetree
      in
      (* we must complete the included modules *)
      let elements =
        analyse_structure Odoc_env.empty mod_name len (String.length !file)
          parsetree tree_structure
      in
      let included_modules_from_tt = tt_get_included_module_list tree_structure
      in
      let elements2 =
        replace_dummy_included_modules elements included_modules_from_tt
      in
      let kind = Module_struct elements2 in
      {
        ...
        m_kind = kind;
        m_loc =
          { loc_impl = Some (Location.in_file !file_name); loc_inter = None };
        m_top_deps = [];
