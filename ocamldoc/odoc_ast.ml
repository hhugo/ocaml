...
module Typedtree_search = struct
  ...
  let add_to_hashes table table_values tt =
    ...
    | Typedtree.Tstr_module mb ->
        Option.iter
          (fun id -> Hashtbl.add table (M (Name.from_ident id)) tt)
          mb.mb_id
    ...
    | Typedtree.Tstr_class info_list ->
        List.iter
          (fun (ci, s) ->
            Hashtbl.add table
              (C (Name.from_ident ci.ci_id_class))
...
functor
  ...
  struct
    ...
    *)
    ...
    let rec tt_analyse_function_parameters env current_comment_opt pat_exp_list
      ...
      | { c_lhs = pattern_param; c_rhs = func_body } :: [] -> (
          ...
          | Texp_function { cases = pat_exp_list; _ } ->
              p
              :: tt_analyse_function_parameters env current_comment_opt
                   pat_exp_list
          | _ ->
    ...
    let tt_analyse_value env current_module_name comment_opt loc pat_exp
        rec_flag =
...
*)
    ...
    *)
    let rec tt_analyse_method_expression env current_method_name comment_opt
        ?(first = true) exp =
                    ...
                    Simple_name
                      {
                        ...
                        sn_type =
                          Odoc_env.subst_type env
                            pattern_param.Typedtree.pat_type;
                      }
                  in
                  [ new_param ]
              | { c_lhs = pattern_param; c_rhs = body } :: [] ->
                  ...
                  if not first then
                      ...
                      | Simple_name { sn_name = "*opt*" } -> (
                          ...
                          | Typedtree.Texp_let
                              ( _,
                                {
                                  vb_pat =
                                    { pat_desc = Typedtree.Tpat_var (id, _) };
                                  vb_expr = exp;
                                }
                                :: _,
                                body2 ) ->
                    ...
                    :: tt_analyse_method_expression env current_method_name
                         comment_opt ~first:false next_exp
                  else
                    tt_analyse_method_expression env current_method_name
                      comment_opt ~first:false body))
      | _ ->
          (* no more parameter *)
          []
    ...
    let analyse_class_structure env current_class_name tt_class_sig last_pos
        pos_limit p_cls tt_cls _table =
      let rec iter acc_inher acc_fields last_pos = function
        ...
        | item :: q -> (
            ...
            | Parsetree.Pcf_val ({ txt = label }, mutable_flag, k) ->
                ...
                in
                iter acc_inher
                  (acc_fields @ ele_comments @ [ Class_attribute att ])
            ...
            | Parsetree.Pcf_method
                ...
                let met_type =
                  try
                    Odoc_sig.Signature_search.search_method_type label
                      tt_class_sig
                  with Not_found ->
                    raise
                      (Failure
                ...
                iter acc_inher
                  (acc_fields @ ele_comments @ [ Class_method met ])
                  loc.Location.loc_end.Lexing.pos_cnum q
            | Parsetree.Pcf_method
                    ...
                    met_value =
                      {
                        ...
                        val_recursive = false;
                        val_parameters =
                          tt_analyse_method_expression env complete_name
                            info_opt exp;
                        val_code = code;
                        val_loc = { loc_impl = Some loc; loc_inter = None };
                      };
    ...
    let rec analyse_class_kind env current_class_name comment_opt last_pos
        p_class_expr tt_class_exp table =
      match
        (p_class_expr.Parsetree.pcl_desc, tt_class_exp.Typedtree.cl_desc)
      with
      ...
      | ( Parsetree.Pcl_apply (p_class_expr2, _),
          ...
          let applied_name =
            ...
            match tt_class_expr2.Typedtree.cl_desc with
            | Typedtree.Tcl_ident (p, _, _) ->
                Name.from_path p (* FIXME : obtain the full name *)
            | _ -> (
                (* FIXME : Unfortunately, we don't have a Tclass_ident :-( even for a class tutu = toto *)
          ...
          let param_exps =
            List.fold_left
              (fun acc (_, exp_opt) ->
                match exp_opt with None -> acc | Some e -> acc @ [ e ])
              [] exp_opt_optional_list
          in
          let param_types =
            List.map (fun e -> e.Typedtree.exp_type) param_exps
          in
          let params_code =
            List.map
              (fun e ->
    ...
    let analyse_class env current_module_name comment_opt p_class_decl
      ...
      let cltype =
        Odoc_env.subst_class_type env tt_class_exp.Typedtree.cl_type
      in
      let parameters, kind =
    ...
    let rec tt_name_from_module_expr mod_expr =
      ...
      | Typedtree.Tmod_structure _ | Typedtree.Tmod_functor _
      | Typedtree.Tmod_apply _ | Typedtree.Tmod_unpack _ ->
        ...
        let f =
          ...
          | Element_module m -> (
              ...
              | Types.Sig_module (ident, _, md, _, _) -> (
                  let n1 = Name.simple m.m_name and n2 = Ident.name ident in
                  match n1 = n2 with
                  | true ->
          ...
          | Element_module_type mt -> (
              ...
              | Types.Sig_modtype (ident, { Types.mtd_type = Some t }, _) -> (
                  let n1 = Name.simple mt.mt_name and n2 = Ident.name ident in
                  match n1 = n2 with
                  | true ->
          ...
          | Element_value v -> (
              ...
              | Types.Sig_value (ident, _, _) ->
                  let n1 = Name.simple v.val_name and n2 = Ident.name ident in
                  n1 = n2
              | _ -> false)
          | Element_type t -> (
              function
              | Types.Sig_type (ident, _, _, _) ->
                  ...
                  let n1 = Name.simple t.ty_name and n2 = Ident.name ident in
                  n1 = n2
              | _ -> false)
          ...
          | Element_exception e -> (
              ...
              | Types.Sig_typext (ident, _, _, _) ->
                  let n1 = Name.simple e.ex_name and n2 = Ident.name ident in
                  n1 = n2
              | _ -> false)
          | Element_class c -> (
              function
              | Types.Sig_class (ident, _, _, _) ->
                  let n1 = Name.simple c.cl_name and n2 = Ident.name ident in
                  n1 = n2
              | _ -> false)
          | Element_class_type ct -> (
              function
              | Types.Sig_class_type (ident, _, _, _) ->
                  let n1 = Name.simple ct.clt_name and n2 = Ident.name ident in
                  n1 = n2
              | _ -> false)
    ...
    and filter_extension_constructors_with_module_type_constraint l lsig =
      let pred xt =
        List.exists
          (function
            | Types.Sig_typext (ident, _, _, _) ->
                let n1 = Name.simple xt.xt_name and n2 = Ident.name ident in
                n1 = n2
            | _ -> false)
          lsig
      in
    ...
    let rec analyse_structure env current_module_name last_pos pos_limit
        parsetree typedtree =
    ...
    and analyse_structure_item env current_module_name loc pos_limit comment_opt
        parsetree_item_desc _typedtree table table_values =
      ...
      | Parsetree.Pstr_value (rec_flag, pat_exp_list) ->
          ...
          let rec iter_pat = function
            ...
            | Parsetree.Ppat_tuple _ ->
                None (* FIXME when we will handle tuples *)
            ...
            | { Parsetree.pvb_pat = pat; pvb_expr = exp } :: q -> (
                ...
                | Some name -> (
                    try
                      ...
                      in
                      let l_values =
                        tt_analyse_value env current_module_name info_opt loc
                          pat_exp rec_flag
                      in
                      let new_env =
                        List.fold_left
                          (fun e v -> Odoc_env.add_value e v.val_name)
                          acc_env l_values
                      in
                      let l_ele =
                        List.map (fun v -> Element_value v) l_values
                      in
                      iter new_last_pos new_env (acc @ ele_comments @ l_ele) q
                    with Not_found -> iter new_last_pos acc_env acc q))
          in
          let new_env, l_ele =
      ...
      | Parsetree.Pstr_typext tyext ->
          ...
          let new_env =
            List.fold_left
              ...
              env tyext.Parsetree.ptyext_constructors
          in
          let loc_start = loc.Location.loc_start.Lexing.pos_cnum in
          let loc_end = loc.Location.loc_end.Lexing.pos_cnum in
          let new_te =
          ...
          let rec analyse_extension_constructors maybe_more exts_acc tt_ext_list
            ...
            | tt_ext :: q ->
                ...
                in
                let ext_loc_end =
                  tt_ext.ext_loc.Location.loc_end.Lexing.pos_cnum
                in
                let new_xt =
                  match tt_ext.ext_kind with
                  | Text_decl (_, args, ret_type) ->
                      let xt_args =
                        Sig.get_cstr_args new_env ext_loc_end args
                      in
                      {
                        ...
                        xt_alias = None;
                        xt_loc =
                          { loc_impl = Some tt_ext.ext_loc; loc_inter = None };
                        xt_text = None;
                      }
                  | Text_rebind (path, _) ->
                      {
      ...
      | Parsetree.Pstr_module
        ...
        -> (
          ...
          try
            ...
            let new_module_pre =
              analyse_module env current_module_name name comment_opt
                module_expr tt_module_expr
            in
            let code =
              if !Odoc_global.keep_code then
      ...
      | Parsetree.Pstr_recmodule mods ->
          ...
          let rec f ?(first = false) last_pos name_mod_exp_list =
            ...
            | { Parsetree.pmb_name = { txt = Some name }; pmb_expr = mod_exp }
              :: q ->
                ...
                in
                let new_module =
                  analyse_module new_env current_module_name name com_opt
                    mod_exp tt_mod_exp
                in
                let eles = f loc_end q in
          ...
          in
          let eles =
            f ~first:true loc.Location.loc_start.Lexing.pos_cnum mods
          in
          (0, new_env, eles)
      ...
      | Parsetree.Pstr_class_type class_type_decl_list ->
          ...
          let rec f ?(first = false) last_pos class_type_decl_list =
            ...
            | class_type_decl :: q ->
                ...
                let complete_name = Name.concat current_module_name name.txt in
                let virt =
                  class_type_decl.Parsetree.pci_virt = Asttypes.Virtual
                in
                let tt_cltype_declaration =
                  try
                    Typedtree_search.search_class_type_declaration table
                      name.txt
                  with Not_found ->
                    raise
                      (Failure
                ...
                in
                let tt_cltype_declaration =
                  tt_cltype_declaration.ci_type_decl
                in
                ...
                in
                let last_pos2 =
                  class_type_decl.Parsetree.pci_loc.Location.loc_end
                    .Lexing.pos_cnum
                in
                let new_ele =
                  Element_class_type
      ...
      | Parsetree.Pstr_include _ ->
          ...
          let im =
            { im_name = "dummy"; im_module = None; im_info = comment_opt }
          in
    ...
    and analyse_module env current_module_name module_name comment_opt
      ...
      | ( Parsetree.Pmod_ident _,
        ...
        ->
          let alias_name =
            Odoc_env.full_module_name env (Name.from_path path)
          in
          {
            m_base with
      ...
      | ( Parsetree.Pmod_functor (param2, p_module_expr2),
          ...
          let loc, mp_name, mp_kind, mp_type =
            ...
            | Parsetree.Named (_, pmty), Typedtree.Named (ident, _, mty) ->
                ...
                let mp_name =
                  Option.fold ~none:"*" ~some:Name.from_ident ident
                in
                let mp_kind =
      ...
      | ( Parsetree.Pmod_apply (p_module_expr1, p_module_expr2),
          Typedtree.Tmod_constraint
            ( {
              ...
              _ ) ) ->
          let m1 =
            analyse_module env current_module_name module_name None
              p_module_expr1 tt_module_expr1
          in
          let m2 =
            analyse_module env current_module_name module_name None
              p_module_expr2 tt_module_expr2
