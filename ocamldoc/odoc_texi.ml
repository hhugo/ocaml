...
let module_subparts =
  let rec iter acc =
    ...
    | [] -> List.rev acc
    (* skip aliases *)
    | Element_module { m_kind = Module_alias _ } :: n -> iter acc n
    | Element_module_type { mt_kind = Some (Module_type_alias _) } :: n ->
      iter acc n
    (* keep modules, module types, classes and class types *)
    | Element_module m :: n -> iter (`Module m :: acc) n
    | Element_module_type mt :: n -> iter (`Module_type mt :: acc) n
    | Element_class c :: n -> iter (`Class c :: acc) n
    | Element_class_type ct :: n -> iter (`Class_type ct :: acc) n
    (* forget the rest *)
    | _ :: n -> iter acc n
  in
  iter []
...
let indices_names =
  [ "Types", "ty"; "Extensions", "xt"; "Exceptions", "ex"; "Values", "va";
    "Class attributes", "ca"; "Methods", "me"; "Classes", "cl";
    "Class types", "ct"; "Modules", "mo"; "Module types", "mt" ]

(** Module for generating various Texinfo things (menus, xrefs, ...) *)
module Texi = struct
  (** Associations of strings to substitute in Texinfo code. *)
  let subst_strings =
    [ Str.regexp "@", "@@"; Str.regexp "{", "@{"; Str.regexp "}", "@}";
      Str.regexp "\\.\\.\\.", "@dots{}" ] @
      (if !esc_8bits then
         [ Str.regexp "\xE0", "@`a"; Str.regexp "\xE2", "@^a";
           Str.regexp "\xE9", "@'e"; Str.regexp "\xE8", "@`e";
           Str.regexp "\xEA", "@^e"; Str.regexp "\xEB", "@\"e";
           Str.regexp "\xF7", "@,{c}"; Str.regexp "\xF4", "@^o";
           Str.regexp "\xF6", "@\"o"; Str.regexp "\xEE", "@^i";
           Str.regexp "\xEF", "@\"i"; Str.regexp "\xF9", "@`u";
           Str.regexp "\xFB", "@^u"; Str.regexp "\xE6", "@ae{}";
           Str.regexp "\xC6", "@AE{}"; Str.regexp "\xDF", "@ss{}";
  ...
  let escape s =
    List.fold_left (fun acc (p, r) -> Str.global_replace p r acc) s
      subst_strings
  
  (** Removes dots (no good for a node name). *)
  let fix_nodename s = Str.global_replace (Str.regexp "\\.") "/" (escape s)
  
  (** Generates a Texinfo menu. *)
  let generate_menu chan subpart_list =
    if subpart_list <> [] then
      let menu_line part_qual name =
        let sname = Name.simple name in
        if sname = name then
          (puts chan (pad_to 35 ("* " ^ sname ^ ":: "));
           puts_nl chan part_qual)
        else
          (puts chan
             (pad_to 35 ("* " ^ sname ^ ": " ^ fix_nodename name ^ ". "));
           puts_nl chan part_qual)
      in
      (puts_nl chan "@menu";
       List.iter
         (function
          | `Module { m_name = name } -> menu_line Odoc_messages.modul name
          | `Module_type { mt_name = name } ->
            menu_line Odoc_messages.module_type name
          | `Class { cl_name = name } -> menu_line Odoc_messages.clas name
  ...
  let ifinfo s = String.concat "\n" [ "@ifinfo"; s; "@end ifinfo"; "" ]
  
  (** [install-info] information *)
  let dirsection sec = "@dircategory " ^ escape sec
  
  let direntry ent = [ "@direntry" ] @ List.map escape ent @ [ "@end direntry" ]
end
...
let titles_and_headings =
  ref
    [ 0, ("@chapter ", "@majorheading "); 1, ("@chapter ", "@majorheading ");
      2, ("@section ", "@heading "); 3, ("@subsection ", "@subheading ");
      4, ("@subsubsection ", "@subsubheading ") ]

let title = fst
let heading = snd
let fallback_title = "@unnumberedsubsubsec "
let fallback_heading = "@subsubheading "

(** This class generates Texinfo code from text structures *)
class text =
  object (self)
    method escape = Texi.escape
    
    method label ?no_:(_:bool option) (_ : string) : string = failwith "gni"
    (** this method is not used here but is virtual
        in a class we will inherit later *)
    
    method texi_of_text t =
      String.concat "" (List.map self#texi_of_text_element t)
    ...
    method texi_of_Title n t =
      let t_begin =
        ...
        with
        | Not_found -> fallback_title
      in
      t_begin ^ self#texi_of_text t ^ "\n"
    
    method texi_of_Link s t =
      String.concat "" [ "@uref{"; s; ","; self#texi_of_text t; "}" ]
    
    method texi_of_Ref name kind =
      let xname =
        match kind with
        | Some RK_module -> Odoc_messages.modul ^ " " ^ Name.simple name
        | Some RK_module_type ->
    ...
    method texi_of_Superscript t = "^@{" ^ self#texi_of_text t ^ "@}"
    
    method texi_of_Subscript t = "_@{" ^ self#texi_of_text t ^ "@}"
    
    method heading n t =
      let f =
        try heading @@ List.assoc n !titles_and_headings
        with
        | Not_found -> fallback_heading
      in
      f ^ self#texi_of_text t ^ "\n"
    
    method fixedblock t = Block (Verbatim "@t{" :: t @ [ Verbatim "}" ])
  end
  ...
  class texi =
    object (self)
      ...
      method node depth name =
        (if Hashtbl.mem node_tbl name then raise Aliased_node);
        Hashtbl.add node_tbl name ();
        if depth <= maxdepth then
          Verbatim ("@node " ^ Texi.fix_nodename name ^ ",\n")
        else
          nothing
      ...
      method index (ind : indices) ent =
        Verbatim
          (if !Global.with_index then
             (assert (List.mem ind indices_to_build);
              String.concat ""
                [ "@"; indices ind; "index "; Texi.escape (Name.simple ent);
                  "\n" ])
           else
             "")
      ...
      method private soft_fix_linebreaks =
        ...
        fun ind t ->
          let rep = "\n" ^ String.make ind ' ' in
          List.map
            (function Raw s -> Raw (Str.global_replace re rep s) | txt -> txt) t
      ...
      method text_of_sees_opt see_l =
        List.concat
          (List.map
             (function
              | See_url s, t ->
                [ linebreak; Bold [ Raw Odoc_messages.see_also ]; Raw " ";
                  Link (s, t); Newline ]
              | See_file s, t | See_doc s, t ->
                [ linebreak; Bold [ Raw Odoc_messages.see_also ]; Raw " "; Raw s
                ] @
                  t @ [ Newline ]) see_l)
      
      method! text_of_before l =
        List.flatten
          (List.map (fun x -> linebreak :: to_text#text_of_before [ x ]) l)
      
      method text_of_params params_list =
        List.concat
          (List.map
             (fun (s, t) ->
                [ linebreak; Bold [ Raw Odoc_messages.parameters ]; Raw " ";
                  Raw s; Raw ": " ] @
                  t @ [ Newline ]) params_list)
      
      method! text_of_raised_exceptions =
        function
        | [] -> []
        | (s, t) :: [] ->
          [ linebreak; Bold [ Raw Odoc_messages.raises ]; Raw " "; Code s;
            Raw " " ] @
            t @ [ Newline ]
        | l ->
          [ linebreak; Bold [ Raw Odoc_messages.raises ]; Raw " :";
      ...
      method! text_of_info ?(block=false) =
        function
        | None -> []
        | Some info ->
          let t =
            List.concat
              [
                begin match info.i_deprecated with
                | None -> []
                | Some t ->
                  Raw (Odoc_messages.deprecated ^ ". ") :: self#fix_linebreaks t @
                    [ Newline; Newline ]
                end; self#text_of_desc info.i_desc;
                if info.i_authors <> [] then
                  linebreak :: self#text_of_author_list info.i_authors
                else
                  [];
                if is info.i_version then
                  linebreak :: self#text_of_version_opt info.i_version
                else
                  []; self#text_of_sees_opt info.i_sees;
                self#text_of_before info.i_before;
                if is info.i_since then
                  linebreak :: self#text_of_since_opt info.i_since
                else
                  []; self#text_of_params info.i_params;
                self#text_of_raised_exceptions info.i_raised_exceptions;
                if is info.i_return_value then
                  linebreak :: self#text_of_return_opt info.i_return_value
                else
                  []; self#text_of_custom info.i_custom ]
          in
          if block then [ Block t ] else t @ [ Newline ]
      
      method texi_of_info i = self#texi_of_text (self#text_of_info i)
      ...
      method text_el_of_type_expr m_name typ =
        Raw
          (indent 5
             (self#relative_idents m_name (Odoc_info.string_of_type_expr typ)))
      ...
      method texi_of_value v =
        ...
        let t =
          [
            self#fixedblock
              [ Newline; minus; Raw ("val " ^ Name.simple v.val_name ^ " :\n");
                self#text_el_of_type_expr (Name.father v.val_name) v.val_type ];
      ...
      method texi_of_attribute a =
        ...
        let t =
          [
            self#fixedblock
              [ Newline; minus; Raw "val ";
                Raw (if a.att_virtual then "virtual " else "");
                Raw (if a.att_mutable then "mutable " else "");
                Raw (Name.simple a.att_value.val_name); Raw " :\n";
                self#text_el_of_type_expr (Name.father a.att_value.val_name)
          ...
          [
            self#fixedblock
              [ Newline; minus; Raw "method ";
                ...
                Raw (Name.simple m.met_value.val_name); Raw " :\n";
                self#text_el_of_type_expr (Name.father m.met_value.val_name)
      ...
      method string_of_type_parameters t =
        let f (tp, co, cn) =
          Printf.sprintf "%s%s" (Odoc_info.string_of_variance t (co, cn))
      ...
      method string_of_type_args
          (args : constructor_args) (ret : Types.type_expr option)
      =
        let f =
          function
        ...
        | args, Some r ->
          " : " ^ f args ^ " -> " ^ Odoc_info.string_of_type_expr r
          ...
          [
            self#fixedblock
              ([ Newline; minus; Raw "type ";
                 ...
                 begin match ty.ty_manifest with
                 | None -> []
                 | Some (Other typ) ->
                   Raw " = " ::
                          ...
                          (fun r ->
                             [ Raw ("  " ^ r.of_name ^ " : ") ] @
                               self#text_of_short_type_expr
                                 (Name.father r.of_name) r.of_type @
                                 [ Raw " ;" ] @ entry_doc r.of_text) l) @
                     [ Raw " }" ]
                 end @
                       ...
                       List.flatten
                         (List.map
                            (fun constr ->
                               Raw ("  | " ^ constr.vc_name) ::
                                 Raw
                                   (self#string_of_type_args constr.vc_args
                                      constr.vc_ret) ::
                                   entry_doc constr.vc_text) l)
                   | Type_record l ->
                     Raw (" = " ^ (if priv then "private " else "") ^ "{\n") ::
                       List.flatten
                         (List.map
                            (fun r ->
                               [ Raw ("  " ^ r.rf_name ^ " : ") ] @
                                 self#text_of_short_type_expr
                                   (Name.father r.rf_name) r.rf_type @
                                   [ Raw " ;" ] @ entry_doc r.rf_text) l) @
                       [ Raw " }" ]
                   | Type_open -> [ Raw " = .."; Newline ]);
            self#index `Type ty.ty_name; Newline ] @
            self#text_of_info ty.ty_info
        in
        self#texi_of_text t
      ...
      method texi_of_type_extension m_name te =
        ...
        let t =
          self#fixedblock
            ([ Newline; minus; Raw "type ";
               Raw
                 begin match te.te_type_parameters with
                 | [] -> ""
                 | [ tp ] ->
                   Printf.sprintf "%s " (Odoc_info.string_of_type_expr tp)
                 | l ->
                   Printf.sprintf "(%s) "
                     (String.concat ", "
                        (List.map Odoc_info.string_of_type_expr l))
                 end; Raw (self#relative_idents m_name te.te_type_name);
               Raw
                 (" +=" ^
                 ...
                 (List.map
                    (fun x ->
                       Raw ("  | " ^ Name.simple x.xt_name) ::
                         Raw (self#string_of_type_args x.xt_args x.xt_ret) ::
                           begin match x.xt_alias with
                           | None -> []
                           | Some xa ->
                             [ Raw " = ";
                               Raw
                                 begin match xa.xa_xt with
                                 | None -> xa.xa_name
                                 | Some x -> x.xt_name
                                 end ]
                           end @
                         begin match x.xt_text with
                         | None -> [ Newline ]
                         | Some t ->
                           Raw (indent 5 "\n(* ") ::
      ...
      method texi_of_exception e =
        ...
        let t =
          [
            self#fixedblock
              ([ Newline; minus; Raw "exception "; Raw (Name.simple e.ex_name);
                 ...
                 | Some ea ->
                   [ Raw " = ";
                     Raw
                       begin match ea.ea_ex with
                       | None -> ea.ea_name
                       | Some e -> e.ex_name
                       end ]); self#index `Exception e.ex_name; Newline ] @
      ...
      method texi_of_module m =
        ...
        let t =
          [
            [
              self#fixedblock
                [ Newline; minus; Raw "module "; Raw (Name.simple m.m_name);
                  Raw (if is_alias m then " = " ^ resolve_alias_name m else "")
                ] ];
            if is_alias_there m then
              [ Ref (resolve_alias_name m, Some RK_module, None); Newline ]
            else
              [];
            if is_alias m then
              [ self#index `Module m.m_name; Newline ]
            else
              [ Newline ]; self#text_of_info m.m_info ]
        in
      ...
      method texi_of_module_type mt =
        ...
        let is_alias_there =
          function
          | { mt_kind = Some (Module_type_alias { mta_module = None }) } ->
            false
          | _ -> true
        in
        let resolve_alias_name =
            ...
            [
              self#fixedblock
                [ Newline; minus; Raw "module type ";
                  ...
                  Raw
                    (if is_alias mt then " = " ^ resolve_alias_name mt else "")
                ] ];
            if is_alias_there mt then
              [ Ref (resolve_alias_name mt, Some RK_module_type, None); Newline
              ]
            else
              [];
            if is_alias mt then
              [ self#index `Module_type mt.mt_name; Newline ]
            else
              [ Newline ]; self#text_of_info mt.mt_info ]
        in
        self#texi_of_text (List.flatten t)
      ...
      method texi_of_class c =
        ...
        let t =
          [
            self#fixedblock
              [ Newline; minus; Raw "class "; Raw (Name.simple c.cl_name) ];
            Ref (c.cl_name, Some RK_class, None); Newline; Newline ] @
            self#text_of_info c.cl_info
        in
        self#texi_of_text t
      ...
      method texi_of_class_type ct =
        ...
        let t =
          [
            self#fixedblock
              [ Newline; minus; Raw "class type "; Raw (Name.simple ct.clt_name)
              ]; Ref (ct.clt_name, Some RK_class_type, None); Newline; Newline ] @
            self#text_of_info ct.clt_info
        in
        self#texi_of_text t
      ...
      method texi_of_module_element module_name module_ele =
        ...
        | Element_type_extension te ->
          self#texi_of_type_extension module_name te
      ...
      method generate_inheritance_info chanout inher_l =
        ...
        in
        let text =
          [ Bold [ Raw Odoc_messages.inherits ]; List (List.map f inher_l);
            Newline ]
        in
      ...
      method generate_class_inheritance_info chanout cl =
        let rec iter_kind =
          function
          | Class_structure ([], _) -> ()
          | Class_structure (l, _) -> self#generate_inheritance_info chanout l
      ...
      method generate_class_type_inheritance_info chanout clt =
        ...
        | Class_signature ([], _) -> ()
        | Class_signature (l, _) -> self#generate_inheritance_info chanout l
        | Class_type _ -> ()
      (** Generate the Texinfo code for the inherited classes
        ...
        try
          ...
          let title =
            [ self#node depth c.cl_name;
              Title
                (depth, None, [ Raw (Odoc_messages.clas ^ " "); Code c.cl_name ]);
              self#index `Class c.cl_name ]
          in
          puts chanout (self#texi_of_text title);
          (if is c.cl_info then
             let descr =
               [ Title (succ depth, None, [ Raw Odoc_messages.description ]) ]
             in
             (puts chanout (self#texi_of_text descr);
              puts chanout (self#texi_of_info c.cl_info)));
          let intf =
            [ Title (succ depth, None, [ Raw Odoc_messages.interface ]) ]
          in
          puts chanout (self#texi_of_text intf);
          self#generate_class_inheritance_info chanout c;
          List.iter
            (fun ele -> puts chanout (self#texi_of_class_element c.cl_name ele))
      ...
      method generate_for_class_type chanout ct =
        try
          ...
          let title =
            [ self#node depth ct.clt_name;
              Title
                (depth,
                 None,
                 [ Raw (Odoc_messages.class_type ^ " "); Code ct.clt_name ]);
              self#index `Class_type ct.clt_name ]
          in
          puts chanout (self#texi_of_text title);
          (if is ct.clt_info then
             let descr =
               [ Title (succ depth, None, [ Raw Odoc_messages.description ]) ]
             in
             (puts chanout (self#texi_of_text descr);
          ...
          List.iter
            (fun ele ->
               puts chanout (self#texi_of_class_element ct.clt_name ele))
      ...
      method generate_for_module_type chanout mt =
        try
          ...
          (if depth < maxdepth && subparts <> [] then
             let menu =
               Texi.ifinfo (self#heading (succ depth) [ Raw "Subparts" ])
             in
             (puts chanout menu;
          ...
          List.iter
            (fun ele ->
               puts chanout (self#texi_of_module_element mt.mt_name ele)) mt_ele;
      ...
      method generate_for_module chanout m =
        try
          ...
          puts chanout (self#texi_of_text intf);
          List.iter
            (fun ele -> puts chanout (self#texi_of_module_element m.m_name ele))
            m_ele;
      ...
      method generate_texi_header chan texi_filename m_list =
        let title =
          ...
          | None -> ""
          | Some s -> self#escape s
        in
        let filename =
          if texi_filename <> "ocamldoc.texi" then
            let fn = Filename.basename texi_filename in
            (if Filename.check_suffix fn ".texi" then
          ...
          (List.flatten
             [
               [ "\\input texinfo   @c -*-texinfo-*-"; "@c %**start of header";
                 "@setfilename " ^ filename; "@settitle " ^ title;
                 "@c %**end of header" ];
               if !Global.with_index then
                 List.map (fun ind -> "@defcodeindex " ^ indices ind)
                   indices_to_build
               else
               ...
               [ "@ifinfo";
                 "This file was generated by Ocamldoc using the Texinfo \
                  generator."; "@end ifinfo"; "@c no titlepage.";
                 "@node Top, , , (dir)"; "@top " ^ title ] ]);
        (* insert the intro file *)
        begin match !Odoc_info.Global.intro_file with
        | None when title <> "" ->
          puts_nl chan "@ifinfo";
          puts_nl chan ("Documentation for " ^ title);
          puts_nl chan "@end ifinfo"
        | None -> puts_nl chan "@c no title given"
        | Some f ->
          nl chan;
          puts_nl chan
            (self#texi_of_info (Some (Odoc_info.info_of_comment_file m_list f)))
        end;
        (* write a top menu *)
        Texi.generate_menu chan
          (List.map (fun m -> `Module m) m_list @
             (if !Global.with_index then
                let indices_names_to_build = List.map indices indices_to_build
                in
                List.rev
                  (List.fold_left
                     (fun acc ->
             ...
             (List.flatten
                (List.map
                   (fun (longname, shortname) ->
                      if List.mem shortname indices_names_to_build then
                        [ "@node " ^ longname ^ " index,";
                          "@unnumbered " ^ longname ^ " index";
                          "@printindex " ^ shortname ]
                      else
                        []) indices_names));
           (if !Global.with_toc then puts_nl chan "@contents");
           puts_nl chan "@bye")
      (** Writes the trailer of the TeXinfo document. *)
      
      method do_index it =
        if not (List.mem it indices_to_build) then
          indices_to_build <- it :: indices_to_build
      ...
      method scan_for_index_in_mod =
        ...
        | Element_included_module _ | Element_module_comment _ -> ()
        (* recursion *)
        | Element_module m ->
          self#do_index `Module;
          self#scan_for_index (`Module m)
        | Element_module_type mt ->
          self#do_index `Module_type;
          self#scan_for_index (`Module_type mt)
        | Element_class c ->
          self#do_index `Class;
          self#scan_for_index (`Class c)
        | Element_class_type ct ->
          self#do_index `Class_type;
      ...
      method generate module_list =
        ...
        let filename =
          if !Global.out_file = Odoc_messages.default_out_file then
            "ocamldoc.texi"
          else
            !Global.out_file
        in
        (if !Global.with_index then
           List.iter self#scan_for_index
             (List.map (fun m -> `Module m) module_list));
        try
          let chanout = open_out (Filename.concat !Global.target_dir filename)
          in
          (if !Global.with_header then
             self#generate_texi_header chanout filename module_list);
          List.iter (self#generate_for_module chanout) module_list;
          (if !Global.with_trailer then self#generate_texi_trailer chanout);
          close_out chanout
        with
