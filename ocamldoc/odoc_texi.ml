...
let puts_nl chan s = output_string chan s; output_char chan '\n'
    ...
    object (self)
      ...
      method private soft_fix_linebreaks =
        ...
        fun ind t ->
          ...
          List.map
            (function
              | Raw s -> Raw (Str.global_replace re rep s) | txt -> txt)
            t
      ...
      method! text_of_raised_exceptions =
        ...
        | l ->
            ...
            ; Raw " :"
            ; List
                (List.map (fun (ex, desc) -> Code ex :: Raw " " :: desc) l)
            ; Newline ]
        ...
        | Some info ->
            let t =
              List.concat
                ...
                ; ( if info.i_authors <> []
                  then linebreak :: self#text_of_author_list info.i_authors
                  else [] )
                ; ( if is info.i_version
                  then linebreak :: self#text_of_version_opt info.i_version
                  else [] )
                ; self#text_of_sees_opt info.i_sees
                ; self#text_of_before info.i_before
                ; ( if is info.i_since
                  then linebreak :: self#text_of_since_opt info.i_since
                  else [] )
      ...
      method texi_of_module m =
        ...
        let t =
          [ [ self#fixedblock
                ...
                ; Raw
                    (if is_alias m then " = " ^ resolve_alias_name m else "")
                ] ]
          ; ( if is_alias_there m
            then [Ref (resolve_alias_name m, Some RK_module, None); Newline]
            else [] )
          ; ( if is_alias m
      ...
      method texi_of_module_type mt =
        ...
        let t =
          ...
          ; ( if is_alias_there mt
            then
              [ Ref (resolve_alias_name mt, Some RK_module_type, None)
              ; Newline ]
            else [] )
          ; ( if is_alias mt
        ...
        try
          ...
          let title =
            ...
            ; Title
                ( depth
                , None
                , [Raw (Odoc_messages.clas ^ " "); Code c.cl_name] )
          ...
          List.iter
            (fun ele ->
              puts chanout (self#texi_of_class_element c.cl_name ele) )
      ...
      method generate_for_module chanout m =
        try
          ...
          List.iter
            (fun ele ->
              puts chanout (self#texi_of_module_element m.m_name ele) )
            m_ele;
      ...
      method generate_texi_header chan texi_filename m_list =
        ...
        Texi.generate_menu
          ...
          if !Global.with_index
          then
            let indices_names_to_build =
              List.map indices indices_to_build
            in
            List.rev
              (List.fold_left
                 (fun acc -> function
                   | longname, shortname
                     when List.mem shortname indices_names_to_build ->
                       `Index (longname ^ " index") :: acc | _ -> acc )
      ...
      method generate module_list =
        ...
        with Failure s | Sys_error s ->
          prerr_endline s; incr Odoc_info.errors
    end
end
