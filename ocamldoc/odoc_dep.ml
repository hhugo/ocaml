...
module Dep = struct
  ...
  let get_node graph s =
    ...
    with
    | Not_found -> make_node s []
  
  let rec trans_closure graph acc n =
    if String.Set.mem n.id acc then
      acc
    else (* potential optimisation: use far field if nonempty? *)
      String.Set.fold
  ...
  let node_trans_closure graph n =
    let far =
      List.map
        (fun child ->
           let set = trans_closure graph String.Set.empty (get_node graph child)
           in
           child, set) (set_to_list n.near)
    in
    n.far <- far
  ...
  let prune_node graph node =
    String.Set.iter
      (fun child ->
         ...
         if
           String.Set.exists
             (fun n2 -> String.Set.mem child (get_node graph n2).near) set
         then
  ...
  | T.Type_variant cl ->
    List.iter
      (fun c ->
         ...
         | T.Cstr_record l -> List.iter (fun r -> ty r.T.rf_type) l) cl
  | T.Type_record rl -> List.iter (fun r -> ty r.T.rf_type) rl
  | T.Type_open -> ()
  end;
  begin match t.T.ty_manifest with
  | None -> ()
...
let kernel_deps_of_modules modules =
  let graph =
    List.map (fun m -> Dep.make_node m.Module.m_name m.Module.m_top_deps)
      modules
  in
  ...
  List.iter
    (fun m ->
       ...
       m.Module.m_top_deps <-
         List.filter (fun m2 -> String.Set.mem m2 node.Dep.near)
           m.Module.m_top_deps) modules
  ...
  if kernel then
    ...
    List.map
      (fun t ->
         ...
         t, Dep.set_to_list node.Dep.near) types
  else
    deps_pre
