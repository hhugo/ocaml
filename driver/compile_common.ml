...
let with_info ~native ~tool_name ~source_file ~output_prefix ~dump_ext k =
  ...
  Compmisc.with_ppf_dump ~file_prefix:dump_file @@
    fun ppf_dump ->
      k
        {
          module_name;
...
let typecheck_intf info ast =
  Profile.(record_call typing) @@
    fun () ->
      let tsg =
        ast |> Typemod.type_interface info.env |>
         ...
         Printtyp.wrap_printing_env ~error:false info.env
           (fun () ->
              Format.(fprintf std_formatter) "%a@."
                (Printtyp.printed_signature info.source_file) sg));
...
let interface info =
  Profile.record_call info.source_file @@
    fun () ->
...
let implementation info ~backend =
  Profile.record_call info.source_file @@
    fun () ->
      ...
      in
      Misc.try_finally ?always:None ~exceptionally
        (fun () ->
