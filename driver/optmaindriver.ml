  ...
  match
    ...
    if !make_archive then
      (Compmisc.init_path ();
       ...
       Asmlibrarian.create_archive (Compenv.get_objfiles ~with_ocamlparam:false)
         target;
       Warnings.check_fatal ())
    else if !make_package then
      (Compmisc.init_path ();
       let target = Compenv.extract_output !output_name in
       Compmisc.with_ppf_dump ~file_prefix:target
         (fun ppf_dump ->
      ...
      not !Compenv.stop_early && (!objfiles <> [] || !Compenv.has_linker_inputs)
    then
      let target =
