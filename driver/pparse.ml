...
let call_external_preprocessor sourcefile pp =
  ...
  let comm = Printf.sprintf "%s %s > %s" pp (Filename.quote sourcefile) tmpfile
  in
...
let apply_rewriters_str ?(restore=true) ~tool_name ast =
  ...
  | ppxs ->
    let ast =
      ast |> Ast_mapper.add_ppx_context_str ~tool_name |> rewrite Structure ppxs |>
...
let apply_rewriters_sig ?(restore=true) ~tool_name ast =
  ...
  | ppxs ->
    let ast =
      ast |> Ast_mapper.add_ppx_context_sig ~tool_name |> rewrite Signature ppxs |>
...
let file_aux
    ~tool_name inputfile (type a) parse_fun invariant_fun (kind : a ast_kind)
  :
  a
=
  ...
  in
  close_in ic;
  Profile.record_call "-ppx"
    (fun () -> apply_rewriters ~restore:false ~tool_name kind ast)
...
let parse_file ~tool_name invariant_fun parse kind sourcefile =
  ...
  Misc.try_finally
    (fun () ->
       Profile.record_call "parsing" @@
         fun () -> file_aux ~tool_name inputfile parse invariant_fun kind)
