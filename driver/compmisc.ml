...
let init_path ?(dir="") () =
  let dirs =
    if !Clflags.use_threads then
      "+threads" :: !Clflags.include_dirs
    else
      !Clflags.include_dirs
  in
  let dirs =
    !Compenv.last_include_dirs @
      dirs @ Config.flexdll_dirs @ !Compenv.first_include_dirs
  in
  let exp_dirs = List.map (Misc.expand_directory Config.standard_library) dirs
  in
...
let initial_env () =
  ...
  in
  Typemod.initial_env ~loc:(Location.in_file "command line")
...
let read_clflags_from_env () =
  ...
  (if
     Option.is_none !Clflags.color && Option.is_some (Sys.getenv_opt "NO_COLOR")
   then
...
let with_ppf_dump ~file_prefix f =
  let (ppf_dump, finally) =
    if not !Clflags.dump_into_file then
      Format.err_formatter, ignore
    else
