...
let read_clflags_from_env () =
  ...
  if Option.is_none !Clflags.color && Option.is_some (Sys.getenv_opt "NO_COLOR")
  then Clflags.color := Some Misc.Color.Never;
  set_from_env Clflags.error_style Clflags.error_style_reader;
  ()

let with_ppf_dump ~file_prefix f =
  let ppf_dump, finally =
    if not !Clflags.dump_into_file then (Format.err_formatter, ignore)
    else
