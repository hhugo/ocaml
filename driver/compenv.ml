...
let parse_args s =
  ...
  let rec iter is_after args before after =
    match args with
    | [] ->
        if not is_after then raise (SyntaxError "no '_' separator found")
    ...
    | arg :: tail ->
        ...
        in
        if is_after then iter is_after tail before (binding :: after)
...
let get_objfiles ~with_ocamlparam =
  if with_ocamlparam then List.rev (!last_objfiles @ !objfiles @ !first_objfiles)
...
let action_of_file name =
  if Filename.check_suffix name ".ml" || Filename.check_suffix name ".mlt" then
    ...
    ProcessInterface name
  else if Filename.check_suffix name ".c" then ProcessCFile name
