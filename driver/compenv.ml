...
let is_unit_name name =
  try
    ...
    (match name.[0] with 'A' .. 'Z' -> () | _ -> raise Exit);
...
let parse_args s =
  ...
  let rec iter is_after args before after =
    ...
    | "_" :: _ when is_after ->
        raise (SyntaxError "too many '_' separators")
...
let read_one_param ppf position name v =
  ...
  | "open" ->
      open_modules :=
        List.rev_append (String.split_on_char ',' v) !open_modules
  ...
  | "inline" -> (
      ...
      | F.Parse_failed exn ->
          Printf.ksprintf
            (print_error ppf)
            "bad syntax %s for \"inline\": %s"
            v
            (Printexc.to_string exn) )
  | "inline-toplevel" ->
      Int_arg_helper.parse
        v
  ...
  | "O2" ->
      if check_bool ppf "O2" v
      then (
        default_simplify_rounds := 2;
        use_inlining_arguments_set o2_arguments;
        use_inlining_arguments_set ~round:0 o1_arguments )
  | "O3" ->
      if check_bool ppf "O3" v
      then (
  ...
  | "timings" | "profile" ->
      let if_on =
        if name = "timings" then [`Time] else Profile.all_columns
      in
  ...
  | "stop-after" -> (
      ...
      | Some pass -> (
          ...
          match pass with P.Parsing | P.Typing -> compile_only := true ) )
  | _ ->
      if not (List.mem name !can_discard)
      then (
...
let read_OCAMLPARAM ppf position =
  try
    ...
    let before, after =
      try parse_args s with SyntaxError s -> print_error ppf s; [], []
    in
    List.iter
...
let scan_line ic =
  Scanf.bscanf
    ic
    "%[0-9a-zA-Z_.*] : %[a-zA-Z_-] = %s "
    (fun pattern name value ->
      let pattern =
        match pattern with "*" -> Any | _ -> Filename pattern
      in
      {pattern; name; value} )
...
let process_action
  ...
  | ProcessOtherFile name ->
      ...
      then ccobjs := name :: !ccobjs
      else if (not !native_code)
              && Filename.check_suffix name Config.ext_dll
  ...
  | Some output_name ->
      ...
      then (
        if List.filter
             (function
               | ProcessCFile name ->
                   c_object_of_filename name <> output_name
               | _ -> false)
             !deferred_actions
           <> []
        then
        ...
        if List.length
             (List.filter
                (function
                  | ProcessImplementation _ | ProcessInterface _ | _ ->
                      false)
                !deferred_actions)
           > 1
        then
