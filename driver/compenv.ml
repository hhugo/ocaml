...
let parse_args s =
  let args =
    ...
    if len = 0 then
      []
    else (* allow first char to specify an alternative separator in ":|; ," *)
      match s.[0] with
  ...
  let rec iter is_after args before after =
    ...
    | arg :: tail ->
      let binding =
        try Misc.cut_at arg '='
        with
        | Not_found -> raise (SyntaxError ("missing '=' in " ^ arg))
      in
      if is_after then
...
let setter ppf f name options s =
  ...
  with
  | Not_found -> Printf.ksprintf (print_error ppf) "bad value %s for %s" s name
...
let decode_compiler_pass ppf v ~name ~filter =
  let module P = Clflags.Compiler_pass 
  in
...
let read_one_param ppf position name v =
  ...
  | "inline-indirect-cost" ->
    Int_arg_helper.parse v "Bad syntax in OCAMLPARAM for 'inline-indirect-cost'"
  ...
  | "inline-branch-factor" ->
    Float_arg_helper.parse v
      "Bad syntax in OCAMLPARAM for 'inline-branch-factor'" inline_branch_factor
  | "inline-max-depth" ->
    Int_arg_helper.parse v "Bad syntax in OCAMLPARAM for 'inline-max-depth'"
      inline_max_depth
  | "Oclassic" -> set "Oclassic" [ classic_inlining ] v
  | "O2" ->
    if check_bool ppf "O2" v then
      (default_simplify_rounds := 2;
       use_inlining_arguments_set o2_arguments;
       use_inlining_arguments_set ~round:0 o1_arguments)
  | "O3" ->
    if check_bool ppf "O3" v then
  ...
  | "error-style" ->
    ...
    | None ->
      Printf.ksprintf (print_error ppf) "bad value %s for \"error-style\", (%s)"
        v error_style_reader.usage
  ...
  | "save-ir-after" ->
    if !native_code then
      ...
      begin match decode_compiler_pass ppf v ~name ~filter with
      | None -> ()
      | Some pass -> set_save_ir_after pass true
      end
  | _ ->
    if not (List.mem name !can_discard) then
...
let load_config ppf filename =
  ...
  | exception e ->
    Location.errorf ~loc:(Location.in_file filename) "Cannot open file %s"
...
=
  ...
  | ProcessCFile name ->
    ...
    let obj_name =
      ...
      | Some n -> n
    in
    (if Ccomp.compile_file ?output:!output_name name <> 0 then
       raise (Exit_with_status 2));
  ...
  | ProcessOtherFile name ->
    if
      Filename.check_suffix name ocaml_mod_ext ||
    ...
    then
      ...
      begin match Compiler_pass.of_input_filename name with
      | Some start_from ->
        Location.input_name := name;
...
let process_deferred_actions env =
  ...
  begin match final_output_name with
  | None -> ()
  | Some _output_name ->
    if !compile_only then
      (if
...
let parse_arguments ?(current=ref 0) argv f program =
  try Arg.parse_and_expand_argv_dynamic current argv Clflags.arg_spec f "\000"
  with
  | Arg.Bad err_msg ->
