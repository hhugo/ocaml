...
let add_to_synonym_list synonyms suffix =
  ...
  then add_to_list synonyms suffix
  else (
...
let rec find_file_in_list = function
  | [] -> raise Not_found
  | x :: rem -> (
    try find_file x with Not_found -> find_file_in_list rem )
...
let find_dependency target_kind modname (byt_deps, opt_deps) =
  ...
  with Not_found -> (
    ...
    with Not_found -> byt_deps, opt_deps )
  ...
  String.Set.iter
    (fun dep ->
      ...
      if String.length dep > 0
         ...
         | 'A' .. 'Z' | '\128' .. '\255' -> true
         | _ -> false
      then ( print_char ' '; print_string dep ) )
    deps;
  print_char '\n'
...
let read_parse_and_extract parse_function extract_function def ast_kind
  ...
  try
    ...
    try
      ...
      let bound_vars =
        List.fold_left
          (fun bv modname -> Depend.open_module bv (Longident.parse modname))
          !module_map
...
let print_ml_dependencies source_file extracted_deps pp_deps =
  ...
  let init_deps, extra_targets =
    if List.exists
         (fun ext -> Sys.file_exists (basename ^ ext))
         !mli_synonyms
  ...
  in
  if not !native_only
  then print_dependencies (byte_targets @ extra_targets) (byt_deps @ pp_deps);
...
let print_file_dependencies (source_file, kind, extracted_deps, pp_deps) =
  if !raw_dependencies
  then print_raw_dependencies source_file extracted_deps
  else
    match kind with
...
let sort_files_by_dependencies files =
  ...
  then (
    ...
    let sorted_deps =
      ...
      List.sort
        (fun (file1, _) (file2, _) -> String.compare file1 file2)
        !li
    in
    List.iter
      (fun (file, deps) ->
...
let parse_map fname =
  ...
  let modname =
    String.capitalize_ascii
      (Filename.basename (Filename.chop_extension fname))
  in
...
let main () =
  ...
  Clflags.add_arguments
    ...
    ; ( "-impl"
      , Arg.String (file_dependencies_as ML)
      , "<f>  Process <f> as a .ml file" )
    ; ( "-intf"
      , Arg.String (file_dependencies_as MLI)
      , "<f>  Process <f> as a .mli file" )
    ; ( "-map"
      , Arg.String parse_map
    ...
    ; ( "-ppx"
      , Arg.String (add_to_list first_ppx)
    ...
    ; ( "-slash"
      ...
      , " (Windows) Use forward slash / instead of backslash \\ in file \
         paths" )
    ; ( "-sort"
      , Arg.Set sort_files
  ...
  if !sort_files
  then sort_files_by_dependencies !files
