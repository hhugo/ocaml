...
let find_module_in_load_path name =
  ...
  let rec find_in_array a pos =
    if pos >= Array.length a then None
      ...
      let s = a.(pos) in
      if List.mem s names || List.mem s unames then Some s
...
let find_dependency target_kind modname (byt_deps, opt_deps) =
  try
    ...
    if mli_exists then
      ...
      let bytenames =
        if !all_dependencies then
          match target_kind with MLI -> [ cmi_file ] | ML -> [ cmi_file ]
        else
...
let print_filename s =
  ...
  if not (String.contains s ' ') then print_string s
  else
    let rec count n i =
...
let print_file_dependencies (source_file, kind, extracted_deps, pp_deps) =
  if !raw_dependencies then print_raw_dependencies source_file extracted_deps
  else
    match kind with
