...
let find_module_in_load_path name =
  ...
  in
  let rec find_in_path =
    function
    | [] -> raise Not_found
    | (dir, contents) :: rem ->
      begin match find_in_array contents 0 with
      | Some truename ->
...
let print_dependencies target_files deps =
  ...
  let print_compact item =
    if !one_line || !pos + 1 + String.length item <= 77 then
      print_on_same_line item
    else
      print_on_new_line item
  in
  let print_dep item =
...
let read_parse_and_extract
    parse_function extract_function def ast_kind source_file
=
  Depend.pp_deps := [];
  Depend.free_structure_names := String.Set.empty;
  try
      ...
      let bound_vars =
        List.fold_left
          (fun bv modname ->
             ...
             in
             Depend.open_module bv lid) !module_map (* PR#7248 *)
...
let print_ml_dependencies source_file extracted_deps pp_deps =
  ...
  let (init_deps, extra_targets) =
    if
      List.exists (fun ext -> Sys.file_exists (basename ^ ext)) !mli_synonyms
    then
      (cmi_name :: init_deps, cmi_name :: init_deps), []
    else
      (init_deps, init_deps), (if !all_dependencies then [ cmi_name ] else [])
  in
...
let process_file_as process_fun def source_file =
  ...
  List.iter add_to_load_path
    (!Compenv.last_include_dirs @
       !Clflags.include_dirs @ !Compenv.first_include_dirs @ cwd);
  ...
  let files =
    List.map
      (fun (file, file_kind, deps, pp_deps) ->
         ...
         modname, file_kind, deps, new_deps, pp_deps) files
  in
       ...
       String.Set.iter
         (fun modname ->
            ...
            | MLI ->
              ...
              if Hashtbl.mem h (modname, MLI) then
                add_dep modname MLI
              else if Hashtbl.mem h (modname, ML) then
                add_dep modname ML) deps;
       if file_kind = ML then
         (* add dep from .ml to .mli *)
         (if Hashtbl.mem h (modname, MLI) then add_dep modname MLI)) files;
  ...
  while !printed && !worklist <> [] do
    ...
    List.iter
      (fun key ->
         ...
         let set = !deps in
         deps := [];
         List.iter (fun key -> if Hashtbl.mem h key then deps := key :: !deps)
           set;
         if !deps = [] then
           (printed := true;
           ...
           List.iter
             (fun (modname, kind) ->
                Format.fprintf Format.err_formatter "%s.%s " modname
                  (if kind = ML then "ml" else "mli")) !deps;
    ...
    (fun key (Node (s1, m')) ->
       ...
       if String.Set.is_empty s then
         Format.fprintf ppf "@ @[<hv2>module %s : sig%a@;<1 -2>end@]" key
           (dump_map (String.Set.union s1 s0)) m'
       else
         Format.fprintf ppf "@ module %s = %s" key (String.Set.choose s)) m
...
let parse_map fname =
  ...
  let modname =
    String.capitalize_ascii (Filename.basename (Filename.chop_extension fname))
  in
  ...
  try
    ...
    Clflags.add_arguments __LOC__
      ...
      (* "compiler uses -no-alias-deps, and no module is coerced"; *)
        ...
        "-I",
        Arg.String (add_to_list Clflags.include_dirs),
        ...
        "-open",
        Arg.String (add_to_list Clflags.open_modules),
        ...
        "-ppx",
        Arg.String (add_to_list Compenv.first_ppx),
    ...
    Compenv.parse_arguments (ref argv) (add_dep_arg (fun f -> Src (f, None)))
      program;
