...
let mk_stop_after f =
  ( "-stop-after"
  , Arg.Symbol (Clflags.Compiler_pass.pass_names, f)
...
let mk_inline_toplevel f =
  ...
  , Arg.String f
  , Printf.sprintf
      "<n>|<round>=<n>[,...]  Aggressiveness of inlining at toplevel \
       (higher numbers mean more aggressive)" )

let mk_inlining_report f =
  ( "-inlining-report"
  , Arg.Unit f
  , " Emit `.<round>.inlining' file(s) (one per round) showing the \
     inliner's decisions" )
...
let mk_classic_inlining f =
  ( "-Oclassic"
  , Arg.Unit f
  , " Make inlining decisions at function definition time rather than at \
     the call site (replicates previous behaviour of the compiler)" )
...
let mk_inline_lifting_benefit f =
  ...
  , Arg.String f
  , Printf.sprintf
      "<n>|<round>=<n>[,...]  The benefit of lifting definitions to \
       toplevel during inlining (default %d, higher numbers more \
       beneficial)"
...
let mk_inline_branch_factor f =
  ...
  , Arg.String f
  , Printf.sprintf
      "<n>|<round>=<n>[,...]  Estimate the probability of a branch being \
       cold as 1/(1+n) (used for inlining) (default %.2f)"
...
let mk_insn_sched f =
  ...
  , Arg.Unit f
  , Printf.sprintf
      " Run the instruction scheduling pass%s"
...
let mk_no_insn_sched f =
  ...
  , Arg.Unit f
  , Printf.sprintf
      " Do not run the instruction scheduling pass%s"
...
let mk_safe_string f =
  ...
  , if Config.safe_string
    then " (was set when configuring the compiler)"
    else if Config.default_safe_string
    then " Make strings immutable (default)"
...
let mk_unsafe_string f =
  ...
  then "-unsafe-string", Arg.Unit f, " Make strings mutable"
...
let mk_match_context_rows f =
  ...
  , let[@manual.ref "s:comp-options"] chapter, section = 9, 2 in
    Printf.sprintf
      "<n>  (advanced, see manual section %d.%d.)"
      chapter
      section )
...
let options_with_command_line_syntax options r =
  ...
  List.map
    (fun (name, spec, doc) ->
      ( name
      , options_with_command_line_syntax_inner
          r
          rest
          ~name_opt:(Some name)
          spec
