...
type string_constant = { str : string; tag : string }
...
let eval_expect_file _fname ~file_contents =
  ...
  let exec_phrases phrases =
    ...
    Buffer.add_char buf '\n';
    let (_ : bool) =
      List.fold_left phrases ~init:true
        ~f:(fun acc phrase ->
          acc &&
            let snap = Btype.snapshot () in
            try exec_phrase ppf phrase
            with
            | exn ->
...
let output_corrected oc ~file_contents correction =
  let output_body oc { str; tag } = Printf.fprintf oc "{%s|%s|%s}" tag str tag
  in
  let ofs =
    List.fold_left correction.corrected_expectations ~init:0
      ~f:(fun ofs c ->
        ...
        (if c.normal.str <> c.principal.str then
