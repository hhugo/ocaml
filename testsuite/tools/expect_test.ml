...
type expectation = {
  extid_loc : Location.t;
  (* Location of "expect" in "[%%expect ...]" *)
  payload_loc : Location.t;
  (* Location of the whole payload *)
  normal : string_constant;
...
let split_chunks phrases =
  let rec loop (phrases : Parsetree.toplevel_phrase list) code_acc acc =
    match phrases with
    | [] ->
        if code_acc = [] then (List.rev acc, None)
...
let eval_expectation expectation ~output =
  ...
  in
  if s.str = output then None
  else
    let s = { s with str = output } in
    Some
      (if !Clflags.principal then { expectation with principal = s }
