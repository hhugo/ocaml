...
let test_createprocess systemenv =
  let f_out =
    Unix.(
      openfile "./tmpout.txt" [ O_WRONLY; O_TRUNC; O_CREAT; O_CLOEXEC ] 0o600
    )
  in
  let f_err =
    Unix.(
      openfile "./tmperr.txt" [ O_WRONLY; O_TRUNC; O_CREAT; O_CLOEXEC ] 0o600
    )
  in
  ...
  let pid =
    Unix.create_process_env refl
      ...
      (Array.append [| "XVAR=xvar" |] systemenv) p_exit f_out f_err
  in
  out p_entrance "aaaa\n";
  out p_entrance "bbbb\n";
...
let test_2ampsup1 () =
  ...
  let pid =
    Unix.create_process refl [| refl; "-o"; "123"; "-e"; "456"; "-o"; "789" |]
...
let test_open_process_full systemenv =
  let (o, i, e as res) =
    Unix.open_process_full (refl ^ " -o 123 -i2o -e 456 -i2e -v XVAR")
      (Array.append [| "XVAR=xvar" |] systemenv)
  in
  output_string i "aa\nbbbb\n";
  close_out i;
  for _i = 1 to 3 do out Unix.stdout (input_line o ^ "\n") done;
  for _i = 1 to 2 do out Unix.stdout (input_line e ^ "\n") done;
