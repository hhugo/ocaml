let equal (x : float) (y : float) = x, "=", y, x = y [@@inline ;; never]
let not_equal (x : float) (y : float) = x, "!=", y, x <> y [@@inline ;; never]
let less_than (x : float) (y : float) = x, "<", y, x < y [@@inline ;; never]

let not_less_than (x : float) (y : float) =
  x, "!<", y, not (x < y) [@@inline ;; never]

let less_equal (x : float) (y : float) = x, "<=", y, x <= y [@@inline ;; never]

let not_less_equal (x : float) (y : float) =
  x, "!<=", y, not (x <= y) [@@inline ;; never]

let greater_than (x : float) (y : float) = x, ">", y, x > y [@@inline ;; never]

let not_greater_than (x : float) (y : float) =
  x, "!>", y, not (x > y) [@@inline ;; never]

let greater_equal (x : float) (y : float) =
  x, ">=", y, x >= y [@@inline ;; never]
