...
let obj_with_closure x = opaque (object method foo = x end)
let r = ref 42
...
let classes (x : int) =
  opaque @@
    let module M = struct
      class a = object method foo = x end
      class c = object inherit a end
    end
...
class c_global = object method foo = 42 end
...
let tests =
  ...
  (* ("state", state); *)(* this one should fail *)
   "classes", (fun () -> classes 42); "obj_ordering", obj_ordering;(* ("random", random); *)
   "laziness", laziness |]
