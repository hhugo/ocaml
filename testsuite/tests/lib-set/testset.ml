...
let checkbool msg b = if not b then Printf.printf "Test %s FAILED\n%!" msg

let normalize_cmp c = if c = 0 then 0 else if c > 0 then 1 else -1
...
let test x s1 s2 =
  ...
  checkbool "equal" (S.equal s1 s2 = (S.elements s1 = S.elements s2));
  ...
  checkbool "map" (S.elements (S.map succ s1) = List.map succ (S.elements s1));

  checkbool "map2" (S.map (fun x -> x) s1 == s1);
  ...
  checkbool "find_first"
    (let l, p, r = S.split x s1 in
     if (not p) && S.is_empty r then
       ...
       if p then e = x else e = S.min_elt r);

  checkbool "find_first_opt"
    (let l, p, r = S.split x s1 in
     let find_first_opt_result = S.find_first_opt (fun k -> k >= x) s1 in
     if (not p) && S.is_empty r then
       match find_first_opt_result with None -> true | _ -> false
     else
  ...
  checkbool "find_last"
    (let l, p, r = S.split x s1 in
     if (not p) && S.is_empty l then
       ...
       if p then e = x else e = S.max_elt l);

  checkbool "find_last_opt"
    (let l, p, r = S.split x s1 in
     let find_last_opt_result = S.find_last_opt (fun k -> k <= x) s1 in
     if (not p) && S.is_empty l then
       match find_last_opt_result with None -> true | _ -> false
     else
  ...
  checkbool "to_seq_of_seq" (S.equal s1 (S.of_seq @@ S.to_rev_seq s1));

  checkbool "to_seq_from"
    (let seq = S.to_seq_from x s1 in
     let ok1 = List.of_seq seq |> List.for_all (fun y -> y >= x) in
     let ok2 =
       S.elements s1 |> List.filter (fun y -> y >= x) = List.of_seq seq
     in
     ok1 && ok2);
