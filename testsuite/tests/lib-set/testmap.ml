...
let test x v s1 s2 =
  ...
  check "mem" (fun i -> M.mem i s1 = (img i s1 <> None));
  ...
  check "merge-inter"
    (let f _ o1 o2 =
       match (o1, o2) with Some v1, Some v2 -> Some (v1 -. v2) | _, _ -> None
     in
     let s = M.merge f s1 s2 in
     fun i -> img i s = f i (img i s1) (img i s2));
  ...
  checkbool "to_rev_seq_of_seq" (M.equal ( = ) s1 (M.of_seq @@ M.to_rev_seq s1));

  checkbool "to_seq_from"
    (let seq = M.to_seq_from x s1 in
     let ok1 = List.of_seq seq |> List.for_all (fun (y, _) -> y >= x) in
     let ok2 =
       M.to_seq s1 |> List.of_seq
       |> List.filter (fun (y, _) -> y >= x)
       = List.of_seq seq
     in
     ok1 && ok2);
