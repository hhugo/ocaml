...
let[@inline ;; never] fn_function =
  function
  | f -> f 42 + 1

let[@inline ;; never] fn_poly : 'a. 'a -> ('a -> int) -> int =
  fun x f -> f x + 1
...
and Rec2 : functor (X : sig end) -> sig val fn : (int -> int) -> int end =
  functor
  (X :
  sig end)
  ->
  struct let[@inline ;; never] fn f = f 42 + 1 end
...
and klass2 _v =
  object (self) method othermeth _ _ f _ = id (fun g -> g 42 + 1) f + 1 end
