...
module Format_list = struct
  ...
  let format box_type ppf items =
    fprintf ppf "[@;<0 2>@[<%s>%a@]%t]" box_type (list ~pp_sep string) items
...
module Format_statements = struct
  let pp_sep ppf () = custom_break ppf ~fits:(";", 1, "") ~breaks:("", 0, "")

  let rec format box_type ppf items =
    fprintf ppf "{@;<0 2>@[<%s>%a@]@,}" box_type (list ~pp_sep string) items
