...
module UText = struct
(* the internal representation is UCS4 with big endian*)(* The most significant digit appears first. *)
  ...
  let of_string s =
    ...
    for i = 0 to String.length s - 1 do buf.[4 * i] <- s.[i] done;
    new text_raw buf
