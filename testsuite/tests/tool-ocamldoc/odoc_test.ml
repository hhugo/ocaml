              ...
              List.iter
                (fun fd ->
                   Printf.bprintf b " %s: %s ;" fd.of_name
                     (Odoc_info.string_of_type_expr fd.of_type)) fields;
              Buffer.add_string b " >";
              Buffer.contents b
            end
      ...
      (try
         List.iter
           (fun m ->
              ...
              self#scan_module_list [ m ]) module_list
       with
...
let _ =
  let module My_generator = struct
    class generator =
      ...
      object method generate = inst#generate end
  end
