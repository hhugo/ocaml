  ...
  let rec mkreceive prev =
    ...
    | chan :: rem as chans ->
      wrap (receive chan)
        (fun n ->
           if n < 0 then
             consume (List.rev_append rem prev) sum
           else
             consume (List.rev_append chans prev) (sum + n)) ::
...
let _ =
  ...
  if n = sum_0_n 5000 + sum_0_n 2000 + sum_0_n 1000 then
    print_string "passed\n"
  else
    print_string "FAILED\n"
