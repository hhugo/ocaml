...
and amas =
  [| 6023600.0; 408523.5; 328900.5; 3098710.0; 1047.355; 3498.5; 22869.0;
    19314.0 |]
 ...
 *)
and a =
  [| [| 0.3870983098; 0.0; 0.0 |]; [| 0.7233298200; 0.0; 0.0 |];
    [| 1.0000010178; 0.0; 0.0 |]; [| 1.5236793419; 3e-10; 0.0 |];
...
and dinc =
  [| [| 7.00498625; (-214.25629); 0.28977 |];
    [| 3.39466189; (-30.84437); (-11.67836) |]; [| 0.0; 469.97289; (-3.35053) |];
...
and ca =
  [| [| 4.0; (-13.0); 11.0; (-9.0); (-9.0); (-3.0); (-1.0); 4.0; 0.0 |];
    ...
    [| (-23437.0); (-2634.0); 6601.0; 6259.0; (-1507.0); (-1821.0); 2620.0;
      (-2115.0); (-1489.0) |];
    [| 62911.0; (-119919.0); 79336.0; 17814.0; (-24241.0); 12068.0; 8306.0;
      (-4893.0); 8902.0 |];
...
and sa =
  [| [| (-29.0); (-1.0); 9.0; 6.0; (-6.0); 5.0; 4.0; 0.0; 0.0 |];
    ...
    [| (-14614.0); (-19828.0); (-5869.0); 1881.0; (-4372.0); (-2255.0); 782.0;
      930.0; 913.0 |];
    [| 139737.0; 0.0; 24667.0; 51123.0; (-5102.0); 7429.0; (-4095.0); (-1976.0);
      (-9566.0) |];
    [| (-138081.0); 0.0; 37205.0; (-49039.0); (-41901.0); (-33872.0); (-27037.0);
      (-12474.0); 18797.0 |];
    [| 0.0; 28492.0; 133236.0; 69654.0; 52322.0; (-49577.0); (-26430.0);
      (-3593.0); 0.0 |] |]
...
and cl =
  [| [| 21.0; (-95.0); (-157.0); 41.0; (-5.0); 42.0; 23.0; 30.0; 0.0; 0.0 |];
    [| (-160.0); (-313.0); (-235.0); 60.0; (-74.0); (-76.0); (-27.0); 34.0; 0.0;
      0.0 |];
    [| (-325.0); (-322.0); (-79.0); 232.0; (-52.0); 97.0; 55.0; (-41.0); 0.0;
      0.0 |];
    [| 2268.0; (-979.0); 802.0; 602.0; (-668.0); (-33.0); 345.0; 201.0; (-55.0);
      0.0 |];
    [| 7610.0; (-4997.0); (-7689.0); (-5841.0); (-2617.0); 1115.0; (-748.0);
      (-607.0); 6074.0; 354.0 |];
    [| (-18549.0); 30125.0; 20012.0; (-730.0); 824.0; 23.0; 1289.0; (-352.0);
      (-14767.0); (-2062.0) |];
    [| (-135245.0); (-14594.0); 4197.0; (-4030.0); (-5630.0); (-2898.0); 2540.0;
      (-306.0); 2939.0; 1986.0 |];
    [| 89948.0; 2103.0; 8963.0; 2695.0; 3682.0; 1648.0; 866.0; (-154.0);
      (-1963.0); (-283.0) |] |]
...
and sl =
  [|
    ...
    |];
    [| (-105.0); (-137.0); 258.0; 35.0; (-116.0); (-88.0); (-112.0); (-80.0);
      0.0; 0.0 |];
    [| 854.0; (-205.0); (-936.0); (-240.0); 140.0; (-341.0); (-97.0); (-232.0);
      536.0; 0.0 |];
    [| (-56980.0); 8016.0; 1012.0; 1448.0; (-3024.0); (-3710.0); 318.0; 503.0;
      3767.0; 577.0 |];
    [| 138606.0; (-13478.0); (-4964.0); 1441.0; (-1319.0); (-1482.0); 427.0;
      1236.0; (-9167.0); (-1918.0) |];
    [| 71234.0; (-41116.0); 5334.0; (-4935.0); (-1848.0); 66.0; 434.0; (-1748.0);
      3780.0; (-701.0) |];
    [| (-47645.0); 11647.0; 2166.0; 3194.0; 679.0; 0.0; (-244.0); (-419.0);
      (-2531.0); 48.0 |] |]
...
let planetpv epoch np pv =
  ...
  and dp =
    anpm
      ((3600.0 *. pi.(np).(0) +. (pi.(np).(1) +. pi.(np).(2) *. t) *. t) *. a2r)
  and di =
...
let radecdist state rdd =
  ...
  rdd.(2) <-
    sqrt
      (state.(0).(0) *. state.(0).(0) +. state.(0).(1) *. state.(0).(1) +.
         state.(0).(2) *. state.(0).(2));
  (* RA *)
