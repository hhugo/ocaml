...
module PR_6686 = struct
  type t = A of float | B of (int * int)
...
module PR_6770 = struct
  type t = Constant of float | Exponent of (float * float)
...
let unbox_compare_float () =
  let module M = struct
    type sf = { mutable x : float; y : float }
  end
...
type block = { mutable float : float; mutable int32 : int32 }
...
let unbox_only_if_useful () =
  for i = 1 to 1000 do
    ...
    r := x
  (* use [x] twice to avoid elimination of the let-binding *)
  done;
  ignore (Sys.opaque_identity !r)

let unbox_minor_words () =
  for i = 1 to 1000 do ignore (Gc.minor_words () = 0.) done
