...
type t5 = D of int * string [@@ocaml.unboxed]
...
type t8 = { h : bool; i : int } [@@ocaml.unboxed]
(* more than one field *)
...
;; let x = Array.make 10 (L 3.14)
(* represented as a flat array *)
and f (a : t11 array) = a.(0)
(* might wrongly assume an array of pointers *) in
assert (f x = L 3.14)
