...
(* VALID DECLARATIONS *)
module A = struct (* Abstract types can be immediate *)
  type t [@@immediate]
...
let test_foo () = for i = 0 to 100_000_000 do Foo.x := !Foo.x done
...
let test_bar () = for i = 0 to 100_000_000 do Bar.x := !Bar.x done
