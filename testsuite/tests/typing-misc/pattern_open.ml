...
let g = function M.[] -> [] | M.[ C ] -> M.[ C ] | _ -> []

let h = function M.[||] -> None | M.[| C |] -> Some M.C | _ -> None

let f2 = function M.(B (B C)) -> M.C
...
let () =
  ...
  let choose (type a) : a Exterior.Gadt.t * a Exterior.Gadt.t -> a -> a =
   fun (a, b) c ->
    match (a, b, c) with
    | Exterior.(Gadt.(Bool Boolean.{ b }), Gadt.Bool _, _) ->
        print ();
        true
    | Exterior.(Gadt.Bool Gadt.Boolean.{ b }), _, true ->
        print ();
        true
    | Exterior.(Gadt.Bool Gadt.Boolean.{ b }), _, false ->
        print ();
        b
    | Exterior.Gadt.(Int n, Int k, 0) ->
        print ();
        0
    | Exterior.(Gadt.(Int n, Gadt.Int k, l)) ->
        print ();
        k + n + l
    | Exterior.Gadt.(Eq (a, b)), _, true ->
        print ();
        true
    | Exterior.(Gadt.(Eq (a, b), _, false)) ->
        print ();
        eval a = eval b
  in
  let _ =
    choose
      Exterior.Gadt.(Bool Boolean.{ b = true }, Bool Boolean.{ b = false })
      false
  in
  print ()
...
let test_separation = function S.(Sep), (S.(Sep, Sep), Sep) -> ()
...
let test_separation_2 = function S.(Ex (a, b)), Ex (c, d) -> ()
...
let test_separation_3 = function S.(Sep) -> s
...
module PR6437 = struct
  ...
  module Var = struct
    type 'a t = O : ('a * unit) t | S : 'a t -> ('a * unit) t
  end
end

let rec f :
    type g1 g2. (g1, g2) PR6437.Ctx.t * g1 PR6437.Var.t -> g2 PR6437.Var.t =
  function
