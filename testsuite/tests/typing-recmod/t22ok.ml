...
= struct
  ...
  let rec merge x y =
    ...
    | (BE.H (e1, p1) as h1), (BE.H (e2, p2) as h2) ->
      if Elem.leq e1 e2 then
        BE.H (e1, PrimH.insert h2 p1)
      else
        BE.H (e2, PrimH.insert h1 p2)
  ...
  let rec merge h1 h2 =
    ...
    | T (_, x1, a1, b1), T (_, x2, a2, b2) ->
      if Elem.leq x1 x2 then
        make x1 a1 (merge b1 h2)
      else
        make x2 a2 (merge h1 b2)
...
module rec Class1 : sig
  ...
  class c = object method m x = if x <= 0 then x else new Class2.d#m x end
end
...
(* PR#4316 *)
module G (S : sig val x : int Lazy.t end) = struct
  include S
end
