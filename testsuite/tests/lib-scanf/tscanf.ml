...
let test0 () =
  (sscanf "" "" id) 1 + (sscanf "" " " id) 2 + (sscanf " " " " id) 3 +
    (sscanf "\t" " " id) 4 +
    (sscanf "\n" " " id) 5 +
    sscanf "\n\t 6" " %d" id
...
let test3 () =
  sscanf "0xff" "%3i" id + sscanf "0XEF" "%3i" id + sscanf "x=-245" " x = %d" id +
    sscanf " 2a " " %1da" id
        ...
        bscanf (Scanning.from_string "1.") "%f" (fun b0 -> b0 = 1.0) &&
          bscanf (Scanning.from_string ".1") "%f" (fun b0 -> b0 = 0.1) &&
            bscanf (Scanning.from_string "-.1") "%f" (fun b0 -> b0 = (-0.1)) &&
              bscanf (Scanning.from_string "+.1") "%f" (fun b0 -> b0 = 0.1) &&
                bscanf (Scanning.from_string "+1.") "%f" (fun b0 -> b0 = 1.0) &&
                  bscanf (Scanning.from_string "-1.") "%f"
                    (fun b0 -> b0 = (-1.0)) &&
                    bscanf (Scanning.from_string "0 1. 1.3") "%f %f %f"
                      (fun b0 b1 b2 -> b0 = 0.0 && b1 = 1.0 && b2 = 1.3) &&
                      bscanf (Scanning.from_string "0.113") "%4f"
                        (fun b0 -> b0 = 0.11) &&
                        bscanf (Scanning.from_string "0.113") "%5f"
                          (fun b0 -> b0 = 0.113) &&
                          bscanf (Scanning.from_string "000.113") "%15f"
                            (fun b0 -> b0 = 0.113) &&
                            bscanf (Scanning.from_string "+000.113") "%15f"
                              (fun b0 -> b0 = 0.113) &&
                              bscanf (Scanning.from_string "-000.113") "%15f"
      ...
      bscanf (Scanning.from_string "10e-1") "%e" (fun b -> b = 1.0) &&
        bscanf (Scanning.from_string "10.e-1") "%e" (fun b -> b = 1.0) &&
          bscanf (Scanning.from_string "1e1 1.e+1 1.3e-1") "%e %e %e"
            (fun b1 b2 b3 -> b1 = 10.0 && b2 = b1 && b3 = 0.13) (* g style. *)
          &&
            bscanf (Scanning.from_string "1 1.1 0e+1 1.3e-1") "%g %g %g %g"
              (fun b1 b2 b3 b4 -> b1 = 1.0 && b2 = 1.1 && b3 = 0.0 && b4 = 0.13) (* F style *)
            &&
              bscanf (Scanning.from_string "1.5 1.5e0 15e-1 0x1.8 0X1.8")
                "%F %F %f %F %F"
                (fun b1 b2 b3 b4 b5 -> b1 = b2 && b2 = b3 && b3 = b4 && b4 = b5) (* h style *)
              &&
                (let roundtrip x =
                   bscanf (Printf.ksprintf Scanning.from_string "%h" x) "%h"
                     (same_float x)
                 in
                 roundtrip 0. &&
                   roundtrip (-0.) &&
...
let test10 () =
  ...
  in
  res = "Unechaine:celle-cietcelle-la!" (* Testing the result of reading a %S string. *)
  &&
    unit "\"a\\\n  b\"" = "ab" &&
      unit "\"\\\n  ab\"" = "ab" &&
        unit "\"\n\\\n  ab\"" = "\nab" &&
  ...
  scan_int_list (Scanning.from_string "[]") = List.rev [] &&
    scan_int_list (Scanning.from_string "[1;2;3;4]") = List.rev [ 1; 2; 3; 4 ] &&
      scan_int_list (Scanning.from_string "[1;2;3;4; ]") =
        List.rev [ 1; 2; 3; 4 ] && (* Should fail but succeeds! *)
        scan_int_list (Scanning.from_string "[1;2;3;4") =
          List.rev [ 1; 2; 3; 4 ]
...
let rec scan_elems ib accu =
  bscanf ib "%1[];]"
    (function
     | "]" -> accu
     | ";" -> scan_rest ib accu
     | _ -> failwith (Printf.sprintf "scan_int_list"))
...
let rec scan_elems ib accu =
  try
    bscanf ib " %i %1[;]"
      (fun i s -> if s = "" then i :: accu else scan_elems ib (i :: accu))
  with
  | Scan_failure _ -> accu
...
let rec scan_elems ib scan_elem_fmt accu =
  kscanf ib (fun ib exc -> accu) scan_elem_fmt
    (fun i ->
       let accu = i :: accu in
       bscanf ib " %1[;] "
         (function "" -> accu | _ -> scan_elems ib scan_elem_fmt accu))
       ...
       scan_elem ib
         (function
          | elem ->
            let accu = elem :: accu in
            bscanf ib " %1[;] "
              (function "" -> accu | _ -> scan_elems scan_elem accu ib)))
    (function l -> l)
...
let test37 () =
  sscanf "" "" true &&
    sscanf "" "" (fun x -> x) 1 = 1 && sscanf "123" "" (fun x -> x) 1 = 1
...
let test42 () =
  ...
  let ib = Scanning.from_string s in
  bscanf ib "%[^abc]%[abc]%s%!"
    (fun s1 s2 s3 -> s1 = "def" && s2 = "cbaa" && s3 = "ghi") &&
    let ib = Scanning.from_string s in
...
let test48 () =
  ...
  test_meta_read s fmt fmt &&
    ...
    test_meta_read s fmt "%i" (* Complex test of scanning a meta format specified in the scanner input
          ...
          k "\" : %2f\": 987654321" = 98.0 &&
            k "\" : %3f\": 9.87654321" = 9.8 &&
              k "\" : %4f\": 9.87654321" = 9.87 &&
                let h s =
                  Scanf.sscanf s "Read integers with %(%i%)" (fun _fmt i -> i)
                in
                  ...
                  h "Read integers with \"%2d\"987654321" = 98 &&
                    h "Read integers with \"%3u\"987654321" = 987 &&
                      h "Read integers with \"%4x\"987654321" = 39030 &&
                        let i s =
                          Scanf.sscanf s "with %(%i %s%)"
                            (fun _fmt amount currency -> amount, currency)
                        in
                        i "with \" : %d %s\" :        21 euros" = (21, "euros") &&
                          i "with \" : %d %s\" : 987654321 dollars" =
                            (987654321, "dollars") &&
                            i "with \" : %u %s\" :     54321 pounds" =
                              (54321, "pounds") &&
                              i "with \" : %x %s\" :       321 yens" =
                                (801, "yens") &&
                                let j s =
                                  Scanf.sscanf s "with %(%i %_s %s%)"
                                    (fun _fmt amount currency ->
                                       amount, currency)
                                in
                                j "with \" : %1d %_s %s\" : 987654321 euros" =
                                  (9, "euros") &&
                                  j "with \" : %2d %_s %s\" : 987654321 dollars" =
                                    (98, "dollars") &&
                                    j
                                      "with \" : %3u %_s %s\" : 987654321 pounds" =
                                      (987, "pounds") &&
                                      j
                                        "with \" : %4x %_s %s\" : 987654321 yens" =
                                        (39030, "yens")
                  ...
                  sscanf "Hello \n" "%s%_1[ ]%s\n"
                    ...
                    sscanf "Hello\nWorld" "%s\n%s%!"
                      ...
                      sscanf "Hello\nWorld!" "%s\n%s%!"
                        (fun s1 s2 -> s1 = "Hello" && s2 = "World!") &&
                        sscanf "Hello\nWorld!" "%s\n%s@!%!"
                          (fun s1 s2 -> s1 = "Hello" && s2 = "World") (* PR#6791 *)
                        &&
                          sscanf "Hello{foo}" "%s@{%s"
                            (fun s1 s2 -> s1 = "Hello" && s2 = "foo}") &&
                            sscanf "Hello[foo]" "%s@[%s"
                              (fun s1 s2 -> s1 = "Hello" && s2 = "foo]")
...
let add_digest_ib ob ib =
  ...
  let output_line_digest s =
    ...
    Buffer.add_char ob '\n'
  in
  try while true do scan_line ib output_line_digest done
  with
  | End_of_file -> ()
    ...
    test_format_scan s fmt "%i" &&
      ...
      test_format_scan s fmt "Read an int %i then a string %s." &&
        let (s, fmt) =
          ...
          format_of_string "Spec%difi%Scation"
        in
        test_format_scan s fmt "Read an int %i then a string \"%s\"." &&
          let (s, fmt) =
            "Read an int %i then a string \"%s\".",
...
let test60 () =
  sscanf "abc" "%0c%0c%c%n"
    (fun c1 c2 c3 n -> c1 = 'a' && c2 = 'a' && c3 = 'a' && n = 1) &&
...
let test61 () =
  ...
  test "%s/%a" &&
    test "\\ " && test "\\x" && test "\\x25s" && test "\\\"%s" && test "\\"
