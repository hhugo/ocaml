...
let subst_lambda ~subst_rec ~free ~subst : _ lambda -> _ =
  function
  ...
  | `Abs (s, t) as l ->
    ...
    let used_expr =
      Subst.fold subst ~init:[]
        ~f:(fun ~key ~data acc -> if Names.mem s used then data :: acc else acc
        )
    in
...
let free_expr ~free_rec : _ expr -> _ =
  function
  | #var as x -> free_var x
  | `Num _ -> Names.empty
...
let subst_expr ~subst_rec ~subst : _ expr -> _ =
  function
  | #var as x -> subst_var ~subst x
