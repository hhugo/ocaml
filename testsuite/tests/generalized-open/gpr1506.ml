...
include struct
  ...
  let run () = raise Interrupt

  let () = match run () with exception Interrupt -> () | _ -> assert false
end
...
let print_list_of_int =
  let open Print_list (Print_int) in
  print
...
let f () =
  let open functor (X : sig end) -> struct end in
  ()
