...
let chkgen mke cmp rstate n a =
  ...
  try
    for i = 0 to n - 2 do
      if cmp a.(i) a.(i + 1) > 0 then raise Exit
    done;
    random_set_state rstate;
    for i = 0 to n - 1 do
      dicho (mke i) 0 (Array.length a - 1)
    done;
    true
...
let chkrand_dup rstate n a = chkgen (fun i -> Random.int n) compare rstate n a
...
let chkfloats rstate n a = chkgen (fun i -> Random.float 1.0) compare rstate n a

type record = { s1 : bytes; s2 : bytes; i1 : int; i2 : int }
...
let mkrec1 b i =
  { s1 = rand_string (); s2 = rand_string (); i1 = Random.int b; i2 = i }

let mkrecs b n = Array.init n (mkrec1 b)

let mkrec1_rev b i =
  { s1 = rand_string (); s2 = rand_string (); i1 = -i; i2 = i }
...
let test1 name f prepdata postdata cmp desc mk chk =
  ...
  List.iter
    (fun n ->
      ...
      (match output with
      | Value v ->
          if not (chk rstate n (postdata a v)) then (
            incr numfailed;
            printf "\n*** FAIL\n")
      | Exception e ->
          incr numfailed;
...
*)
  ...
  List.iter
    (fun m -> t cmp (sprintf "records (int[%d])" m) (mkrecs m) (chkint m))
    [ 1; 10; 100; 1000 ];
  if stable then
    List.iter
      (fun m ->
        t cmp (sprintf "records (int[%d]) [stable]" m) (mkrecs m) (chklex m))
      [ 1; 10; 100; 1000 ]
...
let lmerge_1a cmp l =
  ...
  in
  let rec merge rest accu2 accu l1 l2 =
    (* l1,l2,rest are forward;
       accu,accu2 are rev *)
    match (l1, l2) with
    | [], _ -> mergepairs ((l2 @@ accu) :: accu2) rest
    | _, [] -> mergepairs ((l1 @@ accu) :: accu2) rest
    | h1 :: t1, h2 :: t2 ->
        if cmp h1 h2 <= 0 then merge rest accu2 (h1 :: accu) t1 l2
        else merge rest accu2 (h2 :: accu) l1 t2
  and merge_rev rest accu2 accu l1 l2 =
    (* accu, accu2 are forward;
       l1,l2,rest are rev *)
    match (l1, l2) with
    | [], _ -> mergepairs_rev ((l2 @@ accu) :: accu2) rest
    | _, [] -> mergepairs_rev ((l1 @@ accu) :: accu2) rest
    | h1 :: t1, h2 :: t2 ->
        if cmp h2 h1 <= 0 then merge_rev rest accu2 (h1 :: accu) t1 l2
        else merge_rev rest accu2 (h2 :: accu) l1 t2
  and mergepairs accu = function
    (* accu is rev, arg is forward *)
    | [] -> mergeall_rev accu
    | [ l ] -> mergeall_rev (List.rev l :: accu)
    | l1 :: l2 :: rest -> merge rest accu [] l1 l2
  and mergepairs_rev accu = function
    (* accu is forward, arg is rev *)
    | [] -> mergeall accu
    | [ l ] -> mergeall (List.rev l :: accu)
    | l1 :: l2 :: rest -> merge_rev rest accu [] l1 l2
  and mergeall = function
    (* arg is forward *)
    | [] -> []
    | [ l ] -> l
    | llist -> mergepairs [] llist
  and mergeall_rev = function
    (* arg is rev *)
    | [] -> []
    | [ l ] -> List.rev l
...
let lmerge_1d cmp l =
  ...
  and merge_rev rest accu2 accu l1 l2 =
    ...
    in
    merge_rev_rest_accu2 accu l1 l2
  and mergepairs accu = function
    (* accu is rev, arg is forward *)
    | [] -> mergeall_rev accu
    | [ l ] -> mergeall_rev (List.rev l :: accu)
...
let rec rev_merge_append order l1 l2 acc =
  ...
  | h1 :: t1 -> (
      ...
      | h2 :: t2 ->
          if order h1 h2 then rev_merge_append order t1 l2 (h1 :: acc)
...
let rec rev_merge_append' order l1 l2 acc =
  ...
  | h1 :: t1 -> (
      ...
      | [] -> List.rev_append l1 acc
      | h2 :: t2 ->
          if order h2 h1 then rev_merge_append' order t1 l2 (h1 :: acc)
          else rev_merge_append' order l1 t2 (h2 :: acc))

let rev_merge' order l1 l2 = rev_merge_append' order l1 l2 []

let lmerge_3 order l =
  let rec initlist l acc =
    match l with
    | e1 :: e2 :: rest ->
        initlist rest ((if order e1 e2 then [ e1; e2 ] else [ e2; e1 ]) :: acc)
    | [ e ] -> [ e ] :: acc
    | [] -> acc
  in
  let rec merge2 ll acc =
    ...
    | [] -> acc
    | [ l ] -> [ List.rev l ] @ acc
    | l1 :: l2 :: rest -> merge2 rest (rev_merge order l1 l2 :: acc)
  in
  let rec merge2' ll acc =
    match ll with
    | [] -> acc
  ...
  let rec mergeall rev = function
    ...
    | llist -> mergeall (not rev) ((if rev then merge2' else merge2) llist [])
  in
...
let rec merge order l1 l2 =
  ...
  | h1 :: t1 -> (
      ...
      | [] -> l1
      | h2 :: t2 ->
          if order h1 h2 then h1 :: merge order t1 l2
...
let rec lmerge_4a order l =
  ...
  | _ ->
      let rec sort k l =
        (* k > 1 *)
        match (k, l) with
        | 2, x1 :: x2 :: _ -> if order x1 x2 then [ x1; x2 ] else [ x2; x1 ]
        | 3, x1 :: x2 :: x3 :: _ ->
            if order x1 x2 then
              if order x2 x3 then [ x1; x2; x3 ]
              else if order x1 x3 then [ x1; x3; x2 ]
              else [ x3; x1; x2 ]
            else if order x1 x3 then [ x2; x1; x3 ]
            else if order x2 x3 then [ x2; x3; x1 ]
            else [ x3; x2; x1 ]
        | _, _ ->
            let k1 = k / 2 in
            let k2 = k - k1 in
...
let rec merge order l1 l2 =
  ...
  | h1 :: t1 -> (
      ...
      | [] -> l1
      | h2 :: t2 ->
          if order h1 h2 <= 0 then h1 :: merge order t1 l2
          else h2 :: merge order l1 t2)
      ...
      let rec sort k l =
        ...
        | 3, x1 :: x2 :: x3 :: _ ->
            if order x1 x2 <= 0 then
              if order x2 x3 <= 0 then [ x1; x2; x3 ]
              else if order x1 x3 <= 0 then [ x1; x3; x2 ]
              else [ x3; x1; x2 ]
            else if order x1 x3 <= 0 then [ x2; x1; x3 ]
            else if order x2 x3 <= 0 then [ x2; x3; x1 ]
            else [ x3; x2; x1 ]
        | _, _ ->
            let k1 = k / 2 in
            let k2 = k - k1 in
...
let rec chop k l =
  if k = 0 then l
  else match l with x :: t -> chop (k - 1) t | _ -> assert false
...
let lmerge_4c cmp l =
  let rec merge1 h1 t1 l2 =
    ...
    | [] -> h1 :: t1
    | h2 :: t2 ->
        if cmp h1 h2 <= 0 then h1 :: merge2 t1 h2 t2 else h2 :: merge1 h1 t1 t2
  and merge2 l1 h2 t2 =
    match l1 with
    | [] -> h2 :: t2
    | h1 :: t1 ->
        if cmp h1 h2 <= 0 then h1 :: merge2 t1 h2 t2 else h2 :: merge1 h1 t1 t2
  in
  let merge l1 = function [] -> l1 | h2 :: t2 -> merge2 l1 h2 t2 in
  let rec sort n l =
    match (n, l) with
...
let lmerge_5a cmp l =
  ...
  let rec sort_prefix n =
    if n <= 1 then (
      ...
      | x :: y :: t ->
          rem := t;
          if cmp x y <= 0 then [ x; y ] else [ y; x ])
    else if !rem = [] then []
    else
...
let amerge_1b cmp a =
  ...
  let rec sortto srcofs dst dstofs len =
    ...
    if len = 1 then dst.(dstofs) <- a.(srcofs)
      ...
      if cmp a.(srcofs) a.(srcofs + 1) <= 0 then (
        dst.(dstofs) <- a.(srcofs);
...
let amerge_3a cmp a =
  let merge src1ofs src1len src2 src2ofs src2len dst dstofs =
    ...
    if !i1 < src1r then Array.blit a !i1 dst !d (src1r - !i1)
    else Array.blit src2 !i2 dst !d (src2r - !i2)
  in
  let rec sortto srcofs dst dstofs len =
    assert (len > 0);
    if len = 1 then dst.(dstofs) <- a.(srcofs)
...
let aquick_1a cmp a =
  let rec qsort l r =
    ...
    let pivot =
      if cmp al am <= 0 then
        if cmp am ar <= 0 then am else if cmp al ar <= 0 then ar else al
      else if cmp al ar <= 0 then al
...
let aheap_1 cmp a =
  ...
  let l3 = ref ((!l + 1) / 3) in
  (* l3 is the first element without sons *)
  let maxson i =
    (* ASSUMES i < !l3 *)
    let i31 = i + i + i + 1 in
    let x = ref i31 in
    if i31 + 2 < !l then (
      ...
      !x)
    else if i31 + 1 < !l && cmp a.(i31) a.(i31 + 1) < 0 then i31 + 1
    else i31
  in
  let rec trickledown i e =
    (* ASSUMES i < !l3 *)
    let j = maxson i in
    if cmp a.(j) e > 0 then (
      a.(i) <- a.(j);
...
let aheap_2 cmp a =
  let maxson l i e =
    let i21 = i + i + 1 in
    if i21 + 1 < l && cmp a.(i21) a.(i21 + 1) < 0 then i21 + 1
    else if i21 < l then i21
    else (
      a.(i) <- e;
      raise Exit)
  in
...
let ainsertion_1 cmp a =
  let rec dicho l r e =
    if l = r then l
      ...
      let m = (l + r) / 2 in
      if cmp a.(m) e <= 0 then dicho (m + 1) r e else dicho l m e
  in
...
let lold = [ ("lmerge_3", lmerge_3, false); ("lmerge_4a", lmerge_4a, true) ]
...
*)
  let benchonly f x y z t =
    match !only with [] -> f x y z t | l -> if List.mem y l then f x y z t
  in
  ...
  | Test_std ->
      ...
      testonly "Array.stable_sort" true Array.stable_sort Array.stable_sort ac
        ac;
  ...
  | Test ->
      ...
      printf "Number of tests failed: %d\n" !numfailed
  | Bench1 ->
      let ba x y z = benchonly bench1a !size x y z
      and bb x y z = benchonly bench1b !size x y z
      and bc x y z = benchonly bench1c !size x y z in
      for i = 0 to List.length lold - 1 do
        let name, f, stable = List.nth lold i in
        ba name f ll;
        let name, f, stable = List.nth lold i in
        bb name f ll;
        let name, f, stable = List.nth lold i in
  ...
  | Bench2 ->
      let b x y z = benchonly bench2 !size x y z in
      for i = 0 to List.length lold - 1 do
        let name, f, stable = List.nth lold i in
        b name f ll
      done;
  ...
  | Bench3 ->
      let ba x y z = benchonly bench3a !size x y z
      and bb x y z = benchonly bench3b !size x y z
      and bc x y z = benchonly bench3c !size x y z in
      for i = 0 to List.length lold - 1 do
        let name, f, stable = List.nth lold i in
        ba name f ll;
        let name, f, stable = List.nth lold i in
