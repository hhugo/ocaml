...
let resize newSize =
  ...
  in
  for n = 0 to !sz_1 do copyBucket arr.(n) done;
  htab := newArr;
  sz_1 := newSz_1
...
let rec and2 n1 n2 =
  ...
  | Node (l1, v1, i1, r1) ->
    ...
    | Node (l2, v2, i2, r2) ->
      let h = hash i1 i2 in
      if i1 = andslot1.(h) && i2 = andslot2.(h) then
        andslot3.(h)
      else
        let f =
...
let test_hwb bdd vars =
  ...
  let ntrue = ref 0 in
  for i = 0 to Array.length vars - 1 do if vars.(i) then incr ntrue done;
  eval bdd vars = (if !ntrue > 0 then vars.(!ntrue - 1) else false)

let main () =
  let n = if Array.length Sys.argv >= 2 then int_of_string Sys.argv.(1) else 20
  in
  let ntests =
