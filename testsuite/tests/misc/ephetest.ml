...
let is_data_value test eph (v : int) =
  ...
  | Some x ->
    if !x = v then
      printf "%s data set: OK\n" test
    else
      printf "%s data set: FAIL(bad value %i)\n" test !x
...
let is_key_value test eph (v : int) =
  ...
  | Some x ->
    if !x = v then
      ...
      printf "%s key set: FAIL(bad value %i)\n" test !x
  | None -> printf "%s key unset: FAIL\n" test

let is_key_unset test eph = is_false test "key unset" (K1.check_key eph)
let is_data_unset test eph = is_false test "data unset" (K1.check_data eph)
...
type t = No | Ephe of (int ref, t) K1.t
