...
module Test (A : S) : sig end = struct
  ...
  let test_blit_fill data initval ofs len =
    ...
    let rec check i = function
      | [] -> ()
      | hd :: tl ->
          assert (A.get b i = if i >= ofs && i < ofs + len then initval else hd);
          check (i + 1) tl
    in
    check 0 data
  in
  ...
  let check s =
    ...
    (* already sorted *)
    check_sort s (fun x y -> Stdlib.compare y x) a;
  ...
  check 0
    ...
    [ 0.0; 0.25; -4.0; 3.141592654; nan ];
  check (-1) [ 0.0; 0.25; nan ] [ 0.0; 0.25; 3.14 ];
  check (-1) [ 0.0; 0.25; -4.0 ] [ 0.0; 0.25; 3.14159 ];
  check 1 [ 0.0; 2.718; -4.0 ] [ 0.0; 0.25; 3.14159 ];
  check 1 [ 0.0; 2.718; -4.0 ] [ nan; 0.25; 3.14159 ];

  (* [unsafe_get] [unsafe_set] *)
  let a = A.create 3 in
  for i = 0 to 2 do
    A.unsafe_set a i (float i)
  done;
  for i = 0 to 2 do
    assert (A.unsafe_get a i = float i)
  done;
  ...
  in
  let l =
    [ 0.; 0.25; -4.; 3.14159265; nan; infinity; neg_infinity; neg_zero ]
  in
