...
let mult_ext order =
  ...
  | Term (_, sons1), Term (_, sons2) ->
    ...
    | l1, l2 ->
      if
        List.for_all (fun n -> List.exists (fun m -> gt_ord order (m, n)) l1) l2
      then
        Greater
      else
        NotGE
...
let lex_ext order =
  ...
  | (Term (_, sons1) as m), (Term (_, sons2) as n) ->
    let rec lexrec =
      ...
      | _, [] -> Greater
      | x1 :: l1, x2 :: l2 ->
        begin match order (x1, x2) with
        | Greater ->
          if List.for_all (fun n' -> gt_ord order (m, n')) l2 then
            Greater
          else
            NotGE
        | Equal -> lexrec (l1, l2)
        | NotGE ->
          if List.exists (fun m' -> ge_ord order (m', n)) l1 then
            Greater
          else
            NotGE
        end
  ...
  let rec rporec (m, n) =
    if m = n then
      ...
      | Term (op1, sons1) ->
        ...
        | Term (op2, sons2) ->
          begin match op_order op1 op2 with
          | Greater ->
            if List.for_all (fun n' -> gt_ord rporec (m, n')) sons2 then
              Greater
            else
              NotGE
          | Equal -> ext rporec (m, n)
          | NotGE ->
            if List.exists (fun m' -> ge_ord rporec (m', n)) sons1 then
              Greater
            else
              NotGE
          end
