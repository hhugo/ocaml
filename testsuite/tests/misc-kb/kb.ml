    ...
    let rec process failures (k, l) eqs =
      ...
      | (m, n) :: eqs ->
        ...
        (***
          ...
          ***)
        if m' = n' then
          process failures (k, l) eqs
        else if greater (m', n') then
          enter_rule (m', n')
        else if greater (n', m') then
          enter_rule (n', m')
        else
    ...
    and next_criticals failures (k, l) =
      ...
      try
        ...
        if k = l then
          ...
          with
          | Not_found -> next_criticals failures (k + 1, l)
      with
      | Not_found -> next_criticals failures (1, l + 1)
    in
    process
  in
  kbrec
...
let kb_complete greater complete_rules rules =
  ...
  and eqs = List.map (fun rule -> rule.lhs, rule.rhs) rules
  in
