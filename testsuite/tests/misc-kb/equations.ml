...
let mk_rule num m n =
  ...
  let subst =
    List.map
      (fun v ->
         incr counter;
         v, Var !counter) (List.rev all_vars)
  in
  {
    number = num;
    numvars = !counter;
...
let rec mreduce rules m =
  ...
  | [] -> failwith "mreduce"
  | rule :: rest ->
    (try reduce rule.lhs m rule.rhs
     with
     | Failure _ -> mreduce rest m)
...
and mrewrite1_sons rules =
  function
  | [] -> failwith "mrewrite1"
  | son :: rest ->
    (try mrewrite1 rules son :: rest
     with
     | Failure _ -> son :: mrewrite1_sons rules rest)

(* Iterating rewrite1. Returns a normal form. May loop forever *)
let rec mrewrite_all rules m =
  try mrewrite_all rules (mrewrite1 rules m)
  with
  | Failure _ -> m
