...
module Test (H : Hashtbl.S) (M : Map.S with type key = H.key) = struct
  ...
  let domain_hm h m =
    try
      H.iter (fun k d -> if not (M.mem k m) then raise Exit) h;
      true
    with
    | Exit -> false
  ...
  let incl_hm h m =
    try
      H.iter
        (fun k d ->
           let d' = M.find k m in
           if d <> d' then raise Exit) h;
      true
    with
    | Exit | Not_found -> false
  
  let to_list_ h : _ list =
    H.fold (fun k v acc -> (k, v) :: acc) h [] |> List.sort Stdlib.compare
...
module HofM (M : Map.S) : Hashtbl.S with type key = M.key = struct
  type key = M.key
...
let string_data =
  [| "Si", 0; "non", 1; "e", 2; "vero", 3; "e", 4; "ben", 5; "trovato", 6;
    ...
    "doctor", 16; "away", 17; "Pierre", 20; "qui", 21; "roule", 22;
    "n'amasse", 23; "pas", 24; "mousse", 25; "Asinus", 30; "asinum", 31;
    "fricat", 32 |]

(* Data set: random integers *)
let random_integers num range =
  let data = Array.make num (0, 0) in
  for i = 0 to num - 1 do data.(i) <- Random.int range, i done;
  data

(* Data set: pairs *)
let pair_data data = Array.map (fun (k, d) -> (k, k), d) data
...
let list_data data =
  ...
  let j = ref 0 in
  let rec mklist n =
    if n <= 0 || !j >= Array.length data then
      []
    else
...
let _ =
  ...
  let d =
    try file_data "../../LICENSE"
    with
    | Sys_error _ -> string_data
  in
...
let () =
  ...
  let () =
    ...
    assert (marshaled_before = marshaled_after)
  in
  Hashtbl.filter_map_inplace
    (fun k v ->
       if k mod 100 = 0 then (*Hashtbl.add h v v;*) Some (v / 100) else None) h;
