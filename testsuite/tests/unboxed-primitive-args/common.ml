...
let string_of : type a. a typ -> a -> string =
  function
  | Int -> Int.to_string
...
let rec arity : type a. a proto -> int =
  function
  | Ret _ -> 0
  | Abs (_, p) -> 1 + arity p
...
module Buffer = struct
  ...
  external init_c_side
    : ocaml_buffer:t -> c_buffer:t -> unit = "test_set_buffers"
  ...
  let get : type a. a typ -> t -> arg:int -> a =
    function
    | Int -> get_int
    | Int32 -> get_int32
    | Int64 -> get_int64
  ...
  let set : type a. a typ -> t -> arg:int -> a -> unit =
    function
    | Int -> set_int
    | Int32 -> set_int32
    | Int64 -> set_int64
...
let rec sizes : type a. a proto -> int list =
  function
...
let print_mismatch name proto ~ocaml_buffer ~c_buffer =
  ...
  let (o_args, c_args) =
    (* Align arguments *)
    List.map2 o_args c_args
      ~f:(fun a b ->
        let len_a = String.length a
        and len_b = String.length b
        in
  ...
  if !printed_mismatches >= 1000 then
...
let run_tests tests =
  ...
  let max_args =
    List.fold_left tests ~init:0
      ~f:(fun acc (Test (_, _, p)) -> max acc (arity p)
      )
  in
  let ocaml_buffer = Buffer.create ~arity:max_args
  and c_buffer = Buffer.create ~arity:max_args
  in
