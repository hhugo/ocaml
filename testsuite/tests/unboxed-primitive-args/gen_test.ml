...
let test_all_args_combination_of =
  [ Unboxed_float; Unboxed_integer Pint32; Unboxed_integer Pint64 ]
...
let manual_tests =
  [ "v_v"; "f_f"; "l_l"; "L_L"; "n_n"; "i_i"; "f_fffff"; "f_ffffff"; "f_fffffff";
    "f_fffffffffffffffff"; "v_iiiiiiiiiiiiiiiii"; "v_lllllllllllllllll";
    "v_LLLLLLLLLLLLLLLLL"; "v_iLiLiLiLiLiLiLiLi"; "v_LiLiLiLiLiLiLiLiL";
    "v_flflflflflflflflflflflflflflflflflfl";
    "v_fLfLfLfLfLfLfLfLfLfLfLfLfLfLfLfLfLfL" ]

let ocaml_type_of_repr =
  function
...
type proto = { params : native_repr list; return : native_repr }
...
let c_args_of_proto proto =
  String.concat ~sep:", "
    (List.mapi proto.params
       ~f:(fun i p -> Printf.sprintf "%s x%d" (c_type_of_repr p) i
       ))
...
let iter_protos ~f =
  let iter_for_arity arity =
    let rec loop params to_gen =
      List.iter test_all_args_combination_of
        ~f:(fun repr ->
          let params = repr :: params in
          let to_gen = to_gen - 1 in
          if to_gen = 0 then
            f { params = List.rev params; return = Same_as_ocaml_repr }
          else
            loop params to_gen
        )
    in
  ...
  let rec iter_arities arity =
    if arity <= test_all_combination_up_to_n_args then
...
let generate_ml () =
  ...
  iter_protos
    ~f:(fun proto ->
      ...
      pr "external %s : %s = \"\" %S [@@noalloc]" name
        (ocaml_type_of_proto proto) name
    );
  pr "";
  pr "let tests = []";
  iter_protos
    ~f:(fun proto ->
      ...
      if arity <= 6 then
        pr "let tests = T%d (%S, %s, %s, %s) :: tests" arity name name
          (List.map proto.params ~f:ocaml_type_gadt_of_repr |>
             String.concat ~sep:", ") (ocaml_type_gadt_of_repr proto.return)
      else
        pr "let tests = T (%S, %s, %s) :: tests" name name
