...
let bigstring_of_string s =
  ...
  for i = 0 to String.length s - 1 do a.{i} <- s.[i] done;
  a

let s = bigstring_of_string (String.make 10 '\000')
...
let () =
  ...
  Printf.printf "%x %x %x\n%!" (swap16 (caml_bigstring_get_16 s 0))
    (swap16 (caml_bigstring_get_16 s 1)) (swap16 (caml_bigstring_get_16 s 2));
...
let () =
  ...
  Printf.printf "%lx %lx %lx\n%!" (swap32 (caml_bigstring_get_32 s 0))
    (swap32 (caml_bigstring_get_32 s 1)) (swap32 (caml_bigstring_get_32 s 2));
...
let () =
  ...
  Printf.printf "%Lx %Lx %Lx\n%!" (swap64 (caml_bigstring_get_64 s 0))
    (swap64 (caml_bigstring_get_64 s 1)) (swap64 (caml_bigstring_get_64 s 2));
