...
;; if String.escaped raw_string <> ref_string then
  failwith "test:String.escaped"
...
let () =
  let s = " abc def " in
  for i = 0 to String.length s do check_split ' ' (String.sub s 0 i) done
