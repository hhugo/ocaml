...
module Typeable = struct
  ...
  let rec check_eq : type t t'. t ty -> t' ty -> (t, t') eq =
    fun t t' ->
      match t, t' with
      | Int, Int -> Eq
      | String, String -> Eq
      | List t, List t' ->
        begin match check_eq t t' with
        | Eq -> Eq
        end
      | Pair (t1, t2), Pair (t1', t2') ->
        begin match check_eq t1 t1', check_eq t2 t2' with
        | Eq, Eq -> Eq
        end
      | Fun (t1, t2), Fun (t1', t2') ->
        begin match check_eq t1 t1', check_eq t2 t2' with
  ...
  let gcast : type t t'. t ty -> t' ty -> t -> t' =
    fun t t' x ->
      match check_eq t t' with
      | Eq -> x
end
...
module DeBruijn = struct
  ...
  let rec to_int : type env t. (env, t) ix -> int =
    function
    | ZeroIx -> 0
    | SuccIx n -> to_int n + 1
  ...
  let rec prj : type env t. (env, t) ix -> env stack -> t =
    fun i s ->
      match i, s with
      | ZeroIx, Push (s, v) -> v
      | SuccIx i, Push (s, _) -> prj i s
  
  let rec intp : type env t. (env, t) term -> env stack -> t =
    fun t s ->
      match t with
      | Var ix -> prj ix s
      | Con v -> v
...
module Convert = struct
  ...
  let rec size : type env env'. (env, env') layout -> int =
    function
    | EmptyLayout -> 0
  ...
  =
    fun t n ->
      ...
      | PushLayout (t', l, ix) ->
        if n = 0 then
          begin match Typeable.check_eq t t' with
          | Typeable.Eq -> ix
          end
        else
          prj t (n - 1) l
  ...
  =
    fun lyt ->
      ...
      | HOAS.App (f, a) -> DeBruijn.App (cvt lyt f, cvt lyt a)
...
module Main = struct
  ...
  let one t =
    Lam (Typeable.Fun (t, t), (fun f -> Lam (t, (fun x -> App (f, x)))))
  
  let two t =
    Lam
      (Typeable.Fun (t, t), (fun f -> Lam (t, (fun x -> App (f, App (f, x))))))
  ...
  let plus t =
    let t1 = Typeable.Fun (t, t) in
    let t2 = Typeable.Fun (t1, t1) in
    Lam
      (t2,
       (fun m ->
          Lam
            (t2,
             (fun n ->
                Lam
                  (t1,
                   (fun f ->
                      Lam (t, (fun x -> App (App (m, f), App (App (n, f), x))))))))))
