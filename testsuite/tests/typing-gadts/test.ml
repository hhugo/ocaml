...
module List = struct
  ...
  let head = function Cons (a, b) -> a

  let tail = function Cons (a, b) -> b
...
module Nonexhaustive = struct
  type 'a u = C1 : int -> int u | C2 : bool -> bool u

  type 'a v = C1 : int -> int v

  let unexhaustive : type s. s u -> s = function C2 x -> x
...
module Exhaustive = struct
  ...
  let same_type : type s. s v * s v -> bool = function
    | Foo _, Foo _ -> true
    | Bar _, Bar _ -> true
end
...
module PR6801 = struct
  ...
  let print_string_value (x : string value) =
    match x with String s -> print_endline s
  (* warn : Any *)
end
...
module Existential_escape = struct
...
module Rectype = struct
  type (_, _) t = C : ('a, 'a) t

  let f : type s. (s, s * s) t -> unit = fun C -> () (* here s = s*s! *)
end
...
module Or_patterns = struct
  type _ t = IntLit : int -> int t | BoolLit : bool -> bool t

  let rec eval : type s. s t -> unit = function IntLit _ | BoolLit _ -> ()
end
...
module Propagation = struct
  ...
  let check : type s. s t -> s = function IntLit n -> n | BoolLit b -> b

  let check : type s. s t -> s =
   fun x ->
    let r = match x with IntLit n -> (n : s) | BoolLit b -> b in
    r
end
...
let test : type a. a t -> a = function Int -> ky (1 : a) 1
...
let test : type a. a t -> _ = function Int -> 1 (* ok *)

[%%expect {|
val test : 'a t -> int = <fun>
|}]

let test : type a. a t -> _ = function Int -> ky (1 : a) 1 (* fails *)
...
let test : type a. a t -> a =
 fun x ->
  let r = match x with Int -> ky (1 : a) 1 (* fails *) in
  r
...
let test : type a. a t -> a =
 fun x ->
  let r = match x with Int -> ky 1 (1 : a) (* fails *) in
  r
...
let test (type a) x =
  let r = match (x : a t) with Int -> ky 1 1 in
  r
...
let test : type a. a t -> a =
 fun x ->
  let r = match x with Int -> (1 : a) (* ok! *) in
  r
...
let test : type a. a t -> _ =
 fun x ->
  let r = match x with Int -> 1 (* ok! *) in
  r
...
let f (type a) (x : a t) (y : a) = match x with Int -> y (* returns 'a *)
...
let f : type a. a j -> a = function Has_A -> `A 1 | Has_B -> `B true
...
let f : type a b. (a, b) eq -> < m : a ; .. > -> < m : b ; .. > = fun Eq o -> o
...
let f (type a b) (eq : (a, b) eq) (o : < m : a ; .. >) : < m : b ; .. > =
  match eq with Eq -> o
...
let f : type a b. (a, b) eq -> < m : a ; .. > -> < m : b > =
 fun eq o ->
  ignore (o : < m : a >);
  let r : < m : b > = match eq with Eq -> o in
  (* fail with principal *)
  r
...
let f : type a b. (a, b) eq -> [> `A of a ] -> [> `A of b ] = fun Eq o -> o
...
let f : type a b. (a, b) eq -> [> `A of a | `B ] -> [ `A of b | `B ] =
 fun eq o ->
  ignore (o : [< `A of a | `B ]);
  let r : [ `A of b | `B ] = match eq with Eq -> o in
  (* fail with principal *)
  r
...
let f : type a b. (a M.t, b M.t) eq -> (a, b) eq = function Eq -> Eq
(* fail *)
...
let f : type a b. (a M.t * a, b M.t * b) eq -> (a, b) eq = function Eq -> Eq
(* ok *)
...
let f : type a b. (a * a M.t, b * b M.t) eq -> (a, b) eq = function Eq -> Eq
(* ok *)
...
let f : type a. a t -> a = function V1 -> `A | V2 -> `C;;
...
let f : type a. a ty -> a = fun x -> match x with Int y -> y
...
let f : type a b. (a, b) eq -> (a, int) eq -> a -> b -> _ =
 fun ab aint a b ->
  let Eq = ab in
  let x =
    let Eq = aint in
...
let f : type a b. (a, b) eq -> (b, int) eq -> a -> b -> _ =
 fun ab bint a b ->
  let Eq = ab in
  let x =
    let Eq = bint in
...
let f (type a b c) (b : bool) (w1 : (a, b) eq) (w2 : (a, int) eq) (x : a)
    (y : b) =
  let Eq = w1 in
  let Eq = w2 in
  if b then x else y
