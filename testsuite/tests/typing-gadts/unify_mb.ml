...
let fin_succ : type n. n fin -> n is_succ =
  function
  | FZ -> IS
  | FS _ -> IS
...
let rec check : type n. n succ fin -> n succ term -> n term option =
  fun x t ->
    ...
    | Fork (t1, t2) ->
      bind (check x t1)
        (fun t1 -> bind (check x t2) (fun t2 -> Some (Fork (t1, t2))))
...
let rec sub : type m n. (m, n) alist -> m fin -> n term =
  function
  | Anil -> var
...
let rec weaken_fin : type n. n fin -> n succ fin =
  function
  | FZ -> FZ
...
let rec sub' : type m. m ealist -> m fin -> m term =
  function
  | EAlist Anil -> var
  | EAlist (Asnoc (s, t, x)) ->
