...
let test_sheep () =
  match
    if Dynlink.is_native then Dynlink.loadfile_private "plugin1/sheep.cmxs"
...
let test_cow_repeated () =
  if Dynlink.is_native then Dynlink.loadfile_private "plugin2/cow.cmxs"
  else Dynlink.loadfile_private "plugin2/cow.cmo"
...
let test_cow_same_name_same_mli () =
  if Dynlink.is_native then Dynlink.loadfile_private "plugin2b/cow.cmxs"
...
let test_cow_same_name_different_mli () =
  if Dynlink.is_native then Dynlink.loadfile_private "plugin2c/cow.cmxs"
...
let test_pig () =
  match
    if Dynlink.is_native then Dynlink.loadfile_private "plugin3/pig.cmxs"
...
(* Test that a privately loaded module can recursively load a module of
   the same name. *)
let test_chicken () =
  if Dynlink.is_native then Dynlink.loadfile_private "plugin4/chicken.cmxs"
...
let test_pheasant () =
  if Dynlink.is_native then Dynlink.loadfile_private "plugin6/pheasant.cmxs"
  else Dynlink.loadfile_private "plugin6/pheasant.cmo";
  match
    if Dynlink.is_native then Dynlink.loadfile "plugin6/partridge.cmxs"
