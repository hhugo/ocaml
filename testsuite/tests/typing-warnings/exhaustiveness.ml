...
let f : (int t box pair * bool) option -> unit =
  function
  | None -> ()
...
let f : (string t box pair * bool) option -> unit =
  function
  | None -> ()
...
module Single_row_optim = struct
  ...
  (* This synthetic program is representative of user-written programs
     ...
     of the or-patterns.
  *)
  let non_exhaustive : (t * t * t * t) -> unit =
    function
    | A, A, A, A -> ()
