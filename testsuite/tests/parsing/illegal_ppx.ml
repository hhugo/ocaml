...
let empty_apply loc f = H.Exp.apply ~loc f []
...
let functor_id loc =
  Location.mkloc Longident.(Lapply (Lident "F", Lident "X")) loc
...
let expr mapper e =
  ...
  | Pexp_extension
      ({ txt = "no_args"; loc }, PStr [ { pstr_desc = Pstr_eval (e, _); _ } ])
    ->
    empty_apply loc e
...
let structure_item mapper stri =
  ...
  | Pstr_extension (({ Location.txt = "empty_let"; loc }, _), _) ->
    empty_let loc
  | Pstr_extension (({ Location.txt = "empty_type"; loc }, _), _) ->
    empty_type loc
  | _ -> super.structure_item mapper stri

let signature_item mapper stri =
  match stri.psig_desc with
  | Psig_extension (({ Location.txt = "missing_rhs"; loc }, _), _) ->
    missing_rhs loc
  | _ -> super.signature_item mapper stri

let () =
  M.register "illegal ppx"
    (fun _ -> { super with  expr; pat; structure_item; signature_item })
