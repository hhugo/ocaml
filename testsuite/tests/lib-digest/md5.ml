...
let string_to_data s =
  ...
  for i = 0 to 15 do
    ...
    data.(i) <-
      ...
      let open Int32 in
      byte 0 |> logor (shift_left (byte 1) 8) |> logor (shift_left (byte 2) 16) |>
...
let finish ctx =
  let padding = String.init 64 (function 0 -> '\128' | _ -> '\000') in
