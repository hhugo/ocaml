  ...
  { default_mapper with
    ...
    attributes =
      (fun mapper attrs ->
         ...
         List.filter
           (fun a -> a.Parsetree.attr_name.Location.txt <> "#punning#") attrs)
  }

(* this is to accommodate a LexiFi custom extension *)
...
let test parse_fun pprint print map filename =
  ...
  | ast ->
    ...
    begin match from_string parse_fun str with
    | exception exn ->
    ...
    | ast2 ->
      ...
      if ast <> ast2 then
        (Printf.printf "%s:  FAIL, REPARSED AST IS DIFFERENT\n%!" filename;
         ...
         let cmd =
           Printf.sprintf "%s %s %s" diff (Filename.quote f1)
             (Filename.quote f2)
         in
...
let rec process path =
  if Sys.is_directory path then
    ...
    test Parse.implementation Pprintast.structure Printast.implementation
      (fun mapper -> mapper.Ast_mapper.structure) path
  else if Filename.check_suffix path ".mli" then
    test Parse.interface Pprintast.signature Printast.interface
      (fun mapper -> mapper.Ast_mapper.signature) path

let () = process "source.ml"
