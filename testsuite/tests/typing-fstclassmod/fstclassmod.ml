...
let sort (type s) set l =
  let module Set = (val (set : (module Set.S with type elt = s))) 
  in
...
let forget (type s) x =
  let module M = (val (x : (module S with type t = s)))  in (module M : S)

let print x =
  let module M = (val (x : (module S)))  in print_endline (M.to_string M.x)

let apply x =
  let module M = (val (x : (module S))) 
  in
  let module N = struct
    include M
...
module rec Print : sig
  ...
  let to_string (type s) t x =
    ...
    | Pair p ->
      let module P = (val (p : (module PAIR with type t = s))) 
      in
