...
let alloc_callback alloc =
  if !stopped then None
    ...
    if t == main_thread then (
      ...
      if not do_stop then assert (Unix.read rd1 (Bytes.make 1 'a') 0 1 = 1))
    else (
      ...
      if not do_stop then assert (Unix.read rd2 (Bytes.make 1 'a') 0 1 = 1));
    Some ()
...
let rec go alloc_num tid =
  Mutex.lock mut;
  Mutex.unlock mut;
  if alloc_num < alloc_thread then (
    ...
    if !cnt < 2 then (
      ...
      Thread.yield ();
      go alloc_num tid)
    else Gc.minor () (* check for callbacks *))
