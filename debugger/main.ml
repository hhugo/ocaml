...
let rec protect ppf restart loop =
  ...
  | Toplevel ->
      protect ppf restart (function ppf ->
          pp_print_flush ppf (); stop_user_input (); restart ppf )
  | Sys.Break ->
      protect ppf restart (function ppf ->
          fprintf ppf "Interrupted.@.";
          Exec.protect (function () ->
              stop_user_input ();
              if !loaded then ( try_select_frame 0; show_current_event ppf ) );
          restart ppf )
      ...
      protect ppf restart (function ppf ->
          let b =
            ...
            then (
              let msg =
                sprintf
                  "Restart from time %Ld and try to get closer of the \
                   problem"
                  time
              in
              stop_user_input ();
          ...
          then (
            ...
            then
              ...
              show_current_event ppf;
              restart ppf ) )
          else ( recover (); show_current_event ppf; restart ppf ) )
...
let set_topdirs_path s = topdirs_path := s
...
let speclist =
  ...
  ; ( "-topdirs-path"
    , Arg.String set_topdirs_path
    , " Set path to the directory containing topdirs.cmi" ) ]

let function_placeholder () = raise Not_found
