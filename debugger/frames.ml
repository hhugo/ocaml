...
let selected_point () =
  ...
  | Some ev ->
      ...
      , (Events.get_pos ev).Lexing.pos_cnum
        - (Events.get_pos ev).Lexing.pos_bol )

let selected_event_is_before () =
  match !selected_event with
  | None -> raise Not_found
  | Some {ev_kind = Event_before} -> true
  | _ -> false
  ...
  try
    ...
    | Some curr_event -> (
      ...
      | _ ->
          ...
          current_frame := frame_number )
  with Not_found -> set_frame initial_sp; raise Not_found
