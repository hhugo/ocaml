    ...
    (function
     | () ->
       ...
       let summary =
         ...
         | 'D' ->
           Debug_info
             (input_value !conn.io_in : Instruct.debug_event list array)
...
module Remote_value = struct
  ...
  let obj =
    ...
    | Remote v ->
      ...
      (try input_value !conn.io_in
       with
       | End_of_file | Failure _ -> raise Marshalling_error)
  ...
  let size =
    ...
    | Remote v ->
      ...
      flush !conn.io_out;
      let header = input_binary_int !conn.io_in in
      if header land 0xFF = Obj.double_array_tag && Sys.word_size = 32 then
        header lsr 11
      else
        header lsr 10
