...
let show_current_event ppf =
  (if !Parameters.time then
     (fprintf ppf "Time: %Li" (current_time ());
      ...
      | Some pc -> fprintf ppf " - pc: %i:%i" pc.frag pc.pos
      | _ -> ()));
  ...
  | Some { rep_type = (Event | Breakpoint); rep_program_pointer = pc } ->
    ...
    begin match breakpoints_at_pc pc with
    | [] -> ()
    | [ breakpoint ] -> fprintf ppf "Breakpoint: %i@." breakpoint
    | breakpoints ->
      fprintf ppf "Breakpoints: %a@."
        (fun ppf l -> List.iter (function x -> fprintf ppf "%i " x) l)
  ...
  | Some { rep_type = Uncaught_exc } ->
    (if !Parameters.time then fprintf ppf "@.");
    fprintf ppf "Program end.@.@[Uncaught exception:@ %a@]@."
...
let show_one_frame framenum ppf ev =
  ...
  let cnum =
    ...
    with
    | _ -> pos.Lexing.pos_cnum
  in
  if !machine_readable then
    fprintf ppf "#%i  Pc: %i:%i  %s char %i@." framenum ev.ev_frag
      ev.ev_ev.ev_pos ev.ev_ev.ev_module cnum
  else
    fprintf ppf "#%i %s %s:%i:%i@." framenum ev.ev_ev.ev_module
...
let show_current_frame ppf selected =
  ...
  | None -> fprintf ppf "@.No frame selected.@."
  | Some sel_ev ->
    show_one_frame !current_frame ppf sel_ev;
    begin match
      breakpoints_at_pc { frag = sel_ev.ev_frag; pos = sel_ev.ev_ev.ev_pos }
    with
    | [] -> ()
    | [ breakpoint ] -> fprintf ppf "Breakpoint: %i@." breakpoint
    | breakpoints ->
