...
let rec pattern_matching pattern obj ty =
  ...
  | _ -> (
    ...
    | Ttuple ty_list -> (
      ...
      | P_nth (n, patt) ->
          if n >= List.length ty_list
          then (
            prerr_endline "Out of range.";
            raise Toplevel );
          pattern_matching
            patt
            (Debugcom.get_field obj n)
            (List.nth ty_list n)
      | _ -> error_matching () )
    | Tconstr (cstr, [ty_arg], _)
      ...
      | P_nth (n, patt) ->
          let rec find k current =
            if value_tag current = 0
            then (
              prerr_endline "Out of range.";
              raise Toplevel );
            if k = 0
            then pattern_matching patt (Debugcom.get_field current 0) ty_arg
...
and match_concrete_type pattern obj cstr ty ty_list =
  ...
  | Record_type label_list -> (
      let match_field (label, patt) =
        let lbl =
          try
            primitives__find (function l -> same_name l label) label_list
          with Not_found ->
        ...
        pattern_matching
          patt
          (Debugcom.get_field obj lbl.info.lbl_pos)
          ty_arg
      in
      match pattern with
