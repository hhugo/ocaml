...
let rec pattern_matching pattern obj ty =
  match pattern with
  | P_dummy -> []
  | P_variable var -> [ var, obj, ty ]
  | _ ->
    begin match (Ctype.repr ty).desc with
    | Tvar | Tarrow _ -> error_matching ()
    | Ttuple ty_list ->
      begin match pattern with
      | P_tuple pattern_list -> pattern_matching_list pattern_list obj ty_list
      | P_nth (n, patt) ->
    ...
    | Tconstr (cstr, [ ty_arg ], _)
      when same_type_constr cstr constr_type_list ->
      begin match pattern with
      | P_list pattern_list ->
        let (last, list) =
          it_list
            (fun (current, list) pattern ->
               ...
               (pattern, Debugcom.get_field current 0) :: list) (obj, [])
            pattern_list
        in
        (if value_tag last <> 0 then error_matching ());
        flat_map (function x, y -> pattern_matching x y ty_arg) (rev list)
      | P_nth (n, patt) ->
    ...
    | Tconstr (cstr, ty_list) ->
      ...
      | _ -> match_concrete_type pattern obj cstr ty ty_list
      end
    end

and match_concrete_type pattern obj cstr ty ty_list =
  let typ_descr = type_descr_of_type_constr cstr in
  match typ_descr.info.ty_desc with
  | Abstract_type -> error_matching ()
  | Variant_type constr_list ->
    ...
    (try
       ...
       | Constr_constant -> error_matching ()
       | Constr_regular ->
         begin match pattern with
       ...
       | Constr_superfluous n ->
         ...
         | P_constr (constr2, patt) ->
           ...
           | P_tuple pattern_list ->
             pattern_matching_list pattern_list obj (filter_product n ty_arg)
           | P_nth (n2, patt) ->
             let ty_list = filter_product n ty_arg in
             (if n2 >= n then
                (prerr_endline "Out of range.";
                 raise Toplevel));
             pattern_matching patt (Debugcom.get_field obj n2)
               (List.nth ty_list n2)
           | P_variable var ->
             [
               var,
     ...
     with
     | Not_found -> error_matching ()
     | Unify -> fatal_error "pattern_matching: types should match")
  | Record_type label_list ->
    let match_field (label, patt) =
      let lbl =
        try primitives__find (function l -> same_name l label) label_list
        with
        | Not_found ->
      ...
      in
      (try filter (ty_res, ty)
       with
       | Unify -> fatal_error "pattern_matching: types should match");
      pattern_matching patt (Debugcom.get_field obj lbl.info.lbl_pos) ty_arg
    in
    begin match pattern with
    | P_record pattern_label_list -> flat_map match_field pattern_label_list
    | _ -> error_matching ()
    end
  | Abbrev_type (_, _) -> fatal_error "pattern_matching: abbrev type"

and pattern_matching_list pattern_list obj ty_list =
  let val_list =
    try pair__combine (pattern_list, ty_list)
    with
    | Invalid_argument _ -> error_matching ()
  in
  flat_map (function x, y, z -> pattern_matching x y z)
    (rev
       (snd
          (it_list
