        ...
        when same_type_constr cstr constr_type_list -> (
          ...
          | P_nth (n, patt) ->
              let rec find k current =
                ...
                else find (k - 1) (Debugcom.get_field current 1)
              in
              find n obj
...
and match_concrete_type pattern obj cstr ty ty_list =
  ...
  | Variant_type constr_list -> (
      ...
      try
        let constr =
          if same_type_constr cstr constr_type_exn then find_exception tag
          else find_constr tag constr_list
        in
        let ty_res, ty_arg =
        ...
        | Constr_superfluous n -> (
            ...
            | P_constr (constr2, patt) -> (
                ...
                match patt with
                | P_tuple pattern_list ->
                    pattern_matching_list pattern_list obj
  ...
  | Record_type label_list -> (
      let match_field (label, patt) =
        ...
        (try filter (ty_res, ty)
         with Unify -> fatal_error "pattern_matching: types should match");
...
and pattern_matching_list pattern_list obj ty_list =
  let val_list =
    ...
    with Invalid_argument _ -> error_matching ()
  in
  flat_map
