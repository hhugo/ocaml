...
let rec eval_address = function
  | Env.Aident id -> (
      ...
      | None ->
          raise
            (Symtable.Error
               (Symtable.Undefined_global bytecode_or_asm_symbol))
      | Some obj -> obj )
...
let match_printer_type desc typename =
  let printer_type =
    try Env.lookup_type (Ldot (Lident "Topdirs", typename)) Env.empty
    with Not_found ->
...
let find_printer_type lid =
  try
    ...
    let ty_arg, is_old_style =
      try match_printer_type desc "printer_type_new", false
      with Ctype.Unify _ ->
        match_printer_type desc "printer_type_old", true
    in
