...
let rec loadfiles ppf name =
  try
    ...
    (if d <> Filename.current_dir_name then
       (if not (List.mem d (Load_path.get_paths ())) then Load_path.add_dir d));
...
let init () =
  let topdirs = Filename.concat !Parameters.topdirs_path "topdirs.cmi" in
  ignore (Env.read_signature "Topdirs" topdirs)

let match_printer_type desc typename =
  let printer_type =
    match
      Env.find_type_by_name (Ldot (Lident "Topdirs", typename)) Env.empty
    with
    | path, _ -> path
    | exception Not_found ->
  ...
  Ctype.unify Env.empty (Ctype.newconstr printer_type [ ty_arg ])
...
let find_printer_type lid =
  ...
  | path, desc ->
    begin match match_printer_type desc "printer_type_new" with
    | ty_arg -> ty_arg, path, false
    | exception (Ctype.Unify _) ->
      begin match match_printer_type desc "printer_type_old" with
...
let remove_printer lid =
  ...
  with
  | Not_found -> raise (Error (No_active_printer lid))
  ...
  | Unavailable_module (md, lid) ->
    fprintf ppf
      "@[The debugger does not contain the code for@ %a.@ Please load an \
       implementation of %s first.@]@." Printtyp.longident lid md
  | Wrong_type lid ->
