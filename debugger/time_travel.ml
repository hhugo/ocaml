...
let wait_for_connection checkpoint =
  try
    Exec.unprotect (function () ->
        let old_controller =
          Input_handling.current_controller !connection
        in
        execute_with_other_controller
          (function
            | fd ->
                old_controller fd;
                if checkpoint.c_valid = true then exit_main_loop ())
          !connection
          main_loop )
  with Sys.Break ->
...
let set_current_checkpoint checkpoint =
  if !debug_time_travel
  then prerr_endline ("Select: " ^ Int.to_string checkpoint.c_pid);
  if not checkpoint.c_valid then wait_for_connection checkpoint;
...
let kill_checkpoint checkpoint =
  if !debug_time_travel
  then prerr_endline ("Kill: " ^ Int.to_string checkpoint.c_pid);
  if checkpoint.c_pid > 0
  then (
    (* Ghosts don't have to be killed ! *)
    if not checkpoint.c_valid then wait_for_connection checkpoint;
    stop checkpoint.c_fd;
    if checkpoint.c_parent.c_pid > 0
    then wait_child checkpoint.c_parent.c_fd;
    checkpoint.c_parent <- root;
...
let chk_merge2 cont =
  let rec chk_merge2_cont = function
    | [] -> cont
    | [a] ->
...
let rec chk_merge = function
  | [] -> [], []
  | l :: tail -> chk_merge2 (chk_merge tail) l
...
let new_checkpoint_list checkpoint_count accepted rejected =
  ...
  then
    ...
    in
    ( List.merge
        (fun {c_time = t1} {c_time = t2} -> compare t2 t1)
        accepted
        k
    , l )
...
let clean_checkpoints time checkpoint_count =
  ...
  in
  let kept, lost = new_checkpoint_list checkpoint_count accepted after in
...
let find_checkpoint_before time =
  let rec find = function
    | [] ->
        print_string "Can't go that far in the past !";
        print_newline ();
        if yes_or_no "Reload program"
        then ( load_program (); find !checkpoints )
        else raise Toplevel
    | ({c_time = t} as a) :: l -> if t > time then find l else a
  in
  find !checkpoints
...
let duplicate_current_checkpoint () =
  ...
  if new_checkpoint.c_pid = 0
  then
    (* The ghost has not been killed *)
    match do_checkpoint () with
    (* Duplicate checkpoint *)
    | Checkpoint_done pid ->
        new_checkpoint.c_pid <- pid;
        if !debug_time_travel
        then prerr_endline ("Waiting for connection: " ^ Int.to_string pid)
    | Checkpoint_failed ->
        prerr_endline
...
let rec stop_on_event report =
  match report with
  | {rep_type = Breakpoint; rep_program_pointer = pc; rep_stack_pointer = sp}
    -> (
      last_breakpoint := Some (pc, sp);
      Symbols.update_current_event ();
      match !current_event with None -> find_event () | Some _ -> () )
...
let internal_step duration =
  ...
  | _ ->
      Exec.protect (function () ->
          ( if !make_checkpoints
            then duplicate_current_checkpoint ()
            else remove_checkpoint !current_checkpoint;
            update_breakpoints ();
            update_trap_barrier ();
            ...
            then (
              ...
              <- !current_checkpoint.c_time
                 ++ duration
                 -- Int64.of_int report.rep_event_count
                 ++ _1;
              interrupted := true;
              last_breakpoint := None;
              stop_on_event report );
            try insert_checkpoint !current_checkpoint
            with Exit ->
      ...
      if !debug_time_travel
      then (
        print_string "Checkpoints: pid(time)";
        print_newline ();
        List.iter
          (function
            | {c_time = time; c_pid = pid; c_valid = valid} ->
                Printf.printf
                  "%d(%Ld)%s "
                  pid
                  time
...
let set_file_descriptor pid fd =
  ...
  in
  if !debug_time_travel
  then prerr_endline ("New connection: " ^ Int.to_string pid);
...
let recover () =
  set_current_checkpoint (find_checkpoint_before (current_time ()))

(*** Simple movements. ***)

(* Forward stepping.  Requires `duration >= 0'. *)
let rec step_forward duration =
  if duration > !checkpoint_small_step
  then (
    let first_step =
      if duration > !checkpoint_big_step
      then !checkpoint_big_step
      else !checkpoint_small_step
    in
    internal_step first_step;
    if not !interrupted then step_forward (duration -- first_step) )
  else if duration != _0
  then internal_step duration

(* Go to time `time' from current checkpoint (internal). *)
let internal_go_to time =
  let duration = time -- current_time () in
  if duration > _0
  then execute_without_breakpoints (function () -> step_forward duration)
...
let find_last_breakpoint max_time =
  let rec find break =
    ...
    | Some _, _ when current_time () < max_time -> find !last_breakpoint
    | Some (pc, _), Some pc' when pc = pc' -> max_time, !last_breakpoint
    | _ -> time, break
  in
  find
...
let rec back_to time time_max =
  let {c_time = t} = find_checkpoint_before (pre64 time_max) in
  go_to (max time t);
  let new_time, break = find_last_breakpoint time_max in
  if break <> None || new_time <= time
  then (
    go_to new_time;
    interrupted := break <> None;
    last_breakpoint := break )
  else back_to time new_time

(* Backward stepping. *)
(* --- Assume duration > 1 *)
let step_backward duration =
  let time = current_time () in
  if time > _0 then back_to (max _0 (time -- duration)) time
...
let step duration =
  if duration >= _0
  then step_forward duration
  else step_backward (_0 -- duration)
  ...
  | Some curr_event ->
      ...
      ( try ignore (Symbols.any_event_at_pc pc)
        with Not_found ->
          ...
          raise Toplevel );
      exec_with_trap_barrier frame (fun () ->
  ...
  | Some event1 ->
      ...
      then (
        ...
        | Some event2 ->
            ...
            if frame1 >= 0
               && frame2 >= 0
               && frame2 - event2.ev_stacksize
                  > frame1 - event1.ev_stacksize
            then finish () )
...
let rec next = function
  ...
  | n ->
      next_1 ();
      if not !interrupted then next (n - 1)
...
let start () =
  ...
  | Some curr_event ->
      ...
      let nargs =
        match
          try Symbols.any_event_at_pc pc
          with Not_found ->
            prerr_endline "Calling function has no debugging information.";
            raise Toplevel
        with
...
let rec previous = function
  ...
  | n ->
      previous_1 ();
      if not !interrupted then previous (n - 1)
