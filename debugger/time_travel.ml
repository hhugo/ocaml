...
let insert_checkpoint ({ c_time = time } as checkpoint) =
  let rec traverse = function
    ...
    | ({ c_time = t } as a) :: l as l' ->
        if t > time then a :: traverse l
        else if t = time then raise Exit
        else checkpoint :: l'
  in
...
let set_current_checkpoint checkpoint =
  ...
  let dead_frags =
    List.filter
      (fun frag -> not (List.mem frag checkpoint.c_code_fragments))
...
let kill_checkpoint checkpoint =
  ...
  if checkpoint.c_pid > 0 then (
    (* Ghosts don't have to be killed ! *)
    if not checkpoint.c_valid then wait_for_connection checkpoint;
    stop checkpoint.c_fd;
    if checkpoint.c_parent.c_pid > 0 then wait_child checkpoint.c_parent.c_fd;
    checkpoint.c_parent <- root;
...
let new_checkpoint_list checkpoint_count accepted rejected =
  if List.length accepted >= checkpoint_count then
    ...
    in
    (List.merge (fun t1 t2 -> compare t2.c_time t1.c_time) accepted k, l)
...
let duplicate_current_checkpoint () =
  ...
  if new_checkpoint.c_pid = 0 then
    ...
    | Checkpoint_failed ->
        prerr_endline "A fork failed. Reducing maximum number of checkpoints.";
...
let rec do_go_dynlink steps =
  ...
  | { rep_type = Code_loaded frag; rep_event_count = steps } as report ->
      (match !last_debug_info with
      | Some di ->
          Symbols.add_symbols frag di;
          Symbols.set_all_events frag;
          last_debug_info := None
      | None -> assert false);
      if !break_on_load then report else do_go_dynlink steps
...
let internal_step duration =
  ...
  | _ ->
      Exec.protect (function () ->
          (if !make_checkpoints then duplicate_current_checkpoint ()
           ...
           if report.rep_type = Event then (
             ...
             !current_checkpoint.c_time <-
               !current_checkpoint.c_time ++ duration -- report.rep_event_count
               ++ _1;
             interrupted := true;
             last_breakpoint := None;
             stop_on_event report);
...
let rec back_to time time_max =
  ...
  if break <> None || new_time <= time then (
    go_to new_time;
...
let step_backward duration =
  let time = current_time () in
  if time > _0 then back_to (Int64.max _0 (time -- duration)) time
...
let step duration =
  if duration >= _0 then step_forward duration
...
let start () =
  ...
  | Some { ev_ev = { ev_stacksize } } ->
      ...
      while
        ...
        | Some (pc', frame') when pc = pc' ->
            step _minus1;
            (not !interrupted) && frame' - nargs > frame - ev_stacksize
        | _ -> false
      do
        ()
      done
