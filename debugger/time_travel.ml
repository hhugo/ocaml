       ...
       | () ->
         ...
         execute_with_other_controller
           (function
            | fd ->
              old_controller fd;
              if checkpoint.c_valid = true then exit_main_loop ()) !connection
           main_loop)
  with
  | Sys.Break ->
...
let set_current_checkpoint checkpoint =
  ...
  let dead_frags =
    List.filter (fun frag -> not (List.mem frag checkpoint.c_code_fragments))
...
let clean_checkpoints time checkpoint_count =
  ...
  let (accepted, rejected) = chk_merge (cut2 time !checkpoint_small_step before)
  in
         ...
         (function
          | { c_time = time; c_pid = pid; c_valid = valid } ->
            Printf.printf "%d(%Ld)%s " pid time
              (if valid then "" else "(invalid)")) !checkpoints;
       print_newline ())
...
let forget_process fd pid =
  let checkpoint =
    List.find (function c -> c.c_pid = pid)
...
let find_last_breakpoint max_time =
  ...
  in
  find
    begin match current_pc_sp () with
  ...
  | Some { ev_ev = { ev_stacksize } } ->
    ...
    (try ignore (Symbols.any_event_at_pc pc)
     ...
     | Not_found ->
       ...
       raise Toplevel);
    exec_with_trap_barrier frame
      (fun () ->
         exec_with_temporary_breakpoint pc
           (fun () ->
              while
                run ();
    ...
    if not !interrupted then
      (Symbols.update_current_event ();
       ...
       | Some { ev_ev = { ev_stacksize = ev_stacksize2 } } ->
         ...
         if
           frame1 >= 0 &&
             frame2 >= 0 && frame2 - ev_stacksize2 > frame1 - ev_stacksize1
         then
           finish ())
