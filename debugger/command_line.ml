...
let add_breakpoint_after_pc pc =
  let rec try_add n =
    if n < 3
    then
      try new_breakpoint (any_event_at_pc (pc + (n * 4)))
...
let convert_module mdle =
  ...
  | Some m ->
      ...
      String.capitalize_ascii
        ( if Filename.check_suffix m ".ml"
        then Filename.chop_suffix m ".ml"
        else m )
  | None -> (
    try (get_current_event ()).ev_module
...
let interprete_line ppf line =
  ...
  try
    ...
    | Some x -> (
      ...
      | [i] ->
          i.instr_action ppf lexbuf; resume_user_input (); i.instr_repeat
      | _ -> error "Ambiguous command." )
    | None -> resume_user_input (); false
  with
...
let instr_dir ppf lexbuf =
  ...
  let print_dirs ppf l =
    List.iter (function x -> fprintf ppf "@ %s" x) l
  in
...
let instr_kill _ppf lexbuf =
  ...
  then ( kill_program (); show_no_point () )
...
let instr_help ppf lexbuf =
  let pr_instrs ppf =
    List.iter (fun i -> fprintf ppf "%s@ " i.instr_name)
  in
  match identifier_or_eol Lexer.lexeme lexbuf with
  | Some x -> (
...
let print_expr depth ev env ppf expr =
  ...
  with Eval.Error msg -> Eval.report_error ppf msg; raise Toplevel
        ...
        ( try
            ...
            let ev_pos =
              { Lexing.dummy_pos with
                pos_fname = (Events.get_pos ev).pos_fname }
            in
            let buffer =
...
let instr_list _ppf lexbuf =
  ...
  let beginning =
    ...
    | None -> ( try max 1 (line - 10) with Out_of_range -> 1 )
    | Some x -> x
  in
  ...
  , function
    | ppf ->
        let rec find = function
          | [] -> ()
          | (name, funct) :: l ->
              if funct == !launching_func
              then fprintf ppf "%s" name
              else find l
        in
        find loading_modes; fprintf ppf "@." )
...
let info_modules ppf lexbuf =
  eol lexbuf; ensure_loaded (); pr_modules ppf !modules
...
let info_events _ppf lexbuf =
  ...
  let mdle =
    convert_module
      (module_of_longident (opt_longident_eol Lexer.lexeme lexbuf))
  in
