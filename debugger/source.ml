...
let source_of_module pos mdle =
  ...
  if Sys.file_exists pos_fname then pos_fname
  else
    ...
    let path =
      Hashtbl.fold
        ...
        Debugger_config.load_path_for (Load_path.get_paths ())
    in
    ...
    else if Filename.is_relative fname then find_in_path_rel path fname
...
let insert_pos buffer ((position, line) as pair) =
  let rec new_list = function
    ...
    | ((_pos, lin) as a) :: l as l' ->
        if lin < line then pair :: l'
        else if lin = line then l'
        else a :: new_list l
  in
...
let next_linefeed (buffer, _) pos =
  ...
  if pos >= len then raise Out_of_range
  else
    let rec search p =
