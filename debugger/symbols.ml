...
let partition_modules evl =
  let rec partition_modules' ev evl =
    ...
    | ev' :: evl ->
        ...
        if ev.ev_module = ev'.ev_module then (ev :: evl, evll)
        else ([ ev ], evl :: evll)
  in
  match evl with
  | [] -> []
...
let add_symbols frag all_events =
  List.iter
    (fun evl ->
      List.iter
        (fun ev -> Hashtbl.add events_by_pc { frag; pos = ev.ev_pos } ev)
        evl)
    all_events;
...
let erase_symbols frag =
  let pcs =
    Hashtbl.fold
      (fun pc _ pcs -> if pc.frag = frag then pc :: pcs else pcs)
      events_by_pc []
  in
...
let modules_in_code_fragment frag' =
  Hashtbl.fold
    (fun md (frag, _) l -> if frag' = frag then md :: l else l)
...
let events_in_module mdle =
  try Hashtbl.find all_events_by_module mdle with Not_found -> (0, [])

(* Binary search of event at or just after char *)
let find_event ev char =
  let rec bsearch lo hi =
    if lo >= hi then
      if (Events.get_pos ev.(hi)).Lexing.pos_cnum < char then raise Not_found
      else hi
    else
      let pivot = (lo + hi) / 2 in
      let e = ev.(pivot) in
      if char <= (Events.get_pos e).Lexing.pos_cnum then bsearch lo pivot
      else bsearch (pivot + 1) hi
  in
  if Array.length ev = 0 then raise Not_found
