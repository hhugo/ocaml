...
let events_by_pc = (Hashtbl.create 257 : (pc, debug_event) Hashtbl.t)
...
let partition_modules evl =
  let rec partition_modules' ev evl =
    ...
    | ev' :: evl ->
      ...
      if ev.ev_module = ev'.ev_module then
        ev :: evl, evll
      else
        [ ev ], evl :: evll
  in
...
let read_symbols' bytecode_file =
  ...
  (try
   ...
   with
   | Bytesections.Bad_magic_number | Not_found ->
...
let add_symbols frag all_events =
  List.iter
    (fun evl ->
       List.iter
         (fun ev -> Hashtbl.add events_by_pc { frag; pos = ev.ev_pos } ev) evl)
    all_events;
  List.iter
    (function
     | [] -> ()
     | ev :: _ as evl ->
       ...
       let cmp ev1 ev2 =
         compare (Events.get_pos ev1).Lexing.pos_cnum
       ...
       let real_evl =
         List.filter (function { ev_kind = Event_pseudo } -> false | _ -> true)
           sorted_evl
       in
...
let erase_symbols frag =
  let pcs =
    Hashtbl.fold (fun pc _ pcs -> if pc.frag = frag then pc :: pcs else pcs)
      events_by_pc []
  in
  List.iter (Hashtbl.remove events_by_pc) pcs;
  let mds =
    Hashtbl.fold
      (fun md (frag', _) mds -> if frag' = frag then md :: mds else mds)
      events_by_module []
  in
...
let code_fragments () =
  let frags =
    Hashtbl.fold (fun _ (frag, _) l -> frag :: l) all_events_by_module []
  in
  List.sort_uniq compare frags

let modules_in_code_fragment frag' =
  Hashtbl.fold (fun md (frag, _) l -> if frag' = frag then md :: l else l)
...
let events_in_module mdle =
  try Hashtbl.find all_events_by_module mdle
  with
  | Not_found -> 0, []
...
let find_event ev char =
  let rec bsearch lo hi =
    if lo >= hi then
      (if (Events.get_pos ev.(hi)).Lexing.pos_cnum < char then
         raise Not_found
       else
         hi)
    else
      ...
      let e = ev.(pivot) in
      if char <= (Events.get_pos e).Lexing.pos_cnum then
        bsearch lo pivot
      else
        bsearch (pivot + 1) hi
  in
