...
let rec address path event =
  function
  | Env.Aident id ->
    if Ident.global id then
      (try Debugcom.Remote_value.global (Symtable.get_global_position id)
       with
       | Symtable.Error _ -> raise (Error (Unbound_identifier id)))
    else
        ...
        (try
         ...
         | Not_found ->
           (try
            ...
            with
            | Not_found -> raise (Error (Unbound_identifier id))))
      | None -> raise (Error (Unbound_identifier id))
      end
  | Env.Adot (root, pos) ->
...
let rec expression event env =
  function
  | E_ident lid ->
    begin match Env.find_value_by_name lid env with
    | p, valdesc ->
      let v =
  ...
  | E_item (arg, n) ->
    ...
    | Tconstr (path, [], _) when Path.same path Predef.path_string ->
      ...
      if n >= String.length s then
        raise (Error (String_index (s, String.length s, n)))
      else
        Debugcom.Remote_value.of_int (Char.code s.[n]), Predef.type_char
...
and find_label lbl env ty path tydesc pos =
  function
  | [] -> raise (Error (Wrong_label (ty, lbl)))
  | { ld_id; ld_type } :: rem ->
    if Ident.name ld_id = lbl then
      let ty_res = Btype.newgenty (Tconstr (path, tydesc.type_params, ref Mnil))
      in
      pos,
  ...
  | Not_initialized_yet path ->
    fprintf ppf
      "@[The module path %a is not yet initialized.@ Please run program \
       forward@ until its initialization code is executed.@]@." Printtyp.path
      path
