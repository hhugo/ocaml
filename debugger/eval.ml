...
let abstract_type =
  Btype.newgenty
    (Tconstr (Pident (Ident.create_local "<abstr>"), [], ref Mnil))
...
let rec expression event env = function
  | E_ident lid -> (
    ...
    with Not_found -> raise (Error (Unbound_long_identifier lid)) )
  | E_result -> (
    match event with
  ...
  | E_item (arg, n) -> (
      ...
      match (Ctype.repr (Ctype.expand_head_opt env ty)).desc with
      | Ttuple ty_list ->
  ...
  | E_field (arg, lbl) -> (
      ...
      | Tconstr (path, _, _) -> (
          ...
          | Type_record (lbl_list, _repr) ->
              let pos, ty_res =
                find_label lbl env ty path tydesc 0 lbl_list
              in
...
let report_error ppf = function
  ...
  | String_index (s, len, pos) ->
      fprintf
        ppf
        "@[Cannot extract character number %i@ from the following string \
         of length %i:@ %S@]@."
        pos
        len
        s
