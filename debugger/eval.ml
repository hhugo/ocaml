      ...
      if Ident.global id then
        ...
        | Some { ev_ev = ev } -> (
            ...
            with Not_found -> (
              ...
              with Not_found -> raise (Error (Unbound_identifier id))))
...
let rec expression event env = function
  ...
  | E_item (arg, n) -> (
      ...
      | Tconstr (path, [ ty_arg ], _) when Path.same path Predef.path_array ->
          ...
          if n >= size then raise (Error (Array_index (size, n)))
      ...
      | Tconstr (path, [ ty_arg ], _) when Path.same path Predef.path_list ->
          let rec nth pos v =
            ...
            else if pos = n then (Debugcom.Remote_value.field v 0, ty_arg)
...
let report_error ppf = function
  ...
  | Tuple_index (ty, len, pos) ->
      fprintf ppf
        "@[Cannot extract field number %i from a %i-tuple of type@ %a@]@." pos
        len Printtyp.type_expr ty
  | Array_index (len, pos) ->
      fprintf ppf
  ...
  | List_index (len, pos) ->
      fprintf ppf
        "@[Cannot extract element number %i from a list of length %i@]@." pos
        len
  ...
  | Not_a_record ty ->
      fprintf ppf "@[The type@ %a@ is not a record type@]@." Printtyp.type_expr
        ty
  | No_result ->
