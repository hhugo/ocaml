...
let convert_address address =
  try
    ...
    and port = String.sub address (n + 1) (String.length address - n - 1) in
    ( PF_INET,
      ADDR_INET
...
let search_in_path name =
  ...
  if not (Filename.is_implicit name) then check name
    ...
    let rec traverse pointer =
      if pointer >= length || path.[pointer] = ':' then pointer
      else traverse (pointer + 1)
    in
    let rec find pos =
      let pos2 = traverse pos in
      let directory = String.sub path pos (pos2 - pos) in
      let fullname = if directory = "" then name else directory ^ "/" ^ name in
      try check fullname
      with Not_found ->
        if pos2 < length then find (pos2 + 1) else raise Not_found
    in
    find 0
  ...
  let rec subst_variable ch =
    try
      ...
      if pos + 1 < String.length ch && ch.[pos + 1] = '$' then
        ...
        ^ subst_variable (String.sub ch (pos + 2) (String.length ch - pos - 2))
      else
        String.sub ch 0 pos
  ...
  if ch.[0] = '~' then
    try
      ...
      | 1 -> (
          ...
          with Not_found -> concat_root (Sys.getenv "LOGNAME") tail)
      | n ->
          concat_root
            (String.sub ch 1 (n - 1))
            (String.sub ch (n + 1) (String.length ch - n - 1))
    with Not_found -> expand_path (ch ^ "/")
  else ch

let make_absolute name =
  if Filename.is_relative name then Filename.concat (getcwd ()) name else name
