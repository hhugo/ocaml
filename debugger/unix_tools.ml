...
let search_in_path name =
  ...
  if not (Filename.is_implicit name) then
    ...
    let rec find pos =
      ...
      try check fullname
      with
      | Not_found -> if pos2 < length then find (pos2 + 1) else raise Not_found
    in
    find 0
(* Expand a path. *)
    ...
    try
      ...
      | 1 ->
        ...
        (try Filename.concat (Sys.getenv "HOME") tail
         with
         | Not_found -> concat_root (Sys.getenv "LOGNAME") tail)
      | n ->
        concat_root (String.sub ch 1 (n - 1))
          (String.sub ch (n + 1) (String.length ch - n - 1))
    with
    | Not_found -> expand_path (ch ^ "/")
  else
    ch
