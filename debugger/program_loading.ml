...
let get_unix_environment () =
  let f (vname, vvalue) = Printf.sprintf "%s=%s " vname (Filename.quote vvalue)
  in
...
let generic_exec_unix cmdline =
  function
  | () ->
    (if !debug_loading then prerr_endline "Launching program...");
    let child =
      try fork ()
      with
      | x ->
...
let generic_exec_win cmdline =
  function
  | () ->
    (if !debug_loading then prerr_endline "Launching program...");
    (try
       ignore
         (create_process "cmd.exe" [| "/C"; (cmdline ()) |] stdin stdout stderr)
     with
     | x ->
       Unix_tools.report_error x;
       raise Toplevel)
     ...
     | () ->
       ...
       | "Win32" ->
         ...
         Printf.sprintf "%sset CAML_DEBUG_SOCKET=%s& %s %s %s"
           (get_win32_environment ()) !socket_name runtime_program !program_name
           !arguments
       | _ ->
         Printf.sprintf "%sCAML_DEBUG_SOCKET=%s %s %s %s"
           (get_unix_environment ()) !socket_name
           (Filename.quote runtime_program) (Filename.quote !program_name)
           !arguments
       end)
  ...
  generic_exec
    (function
     | () ->
       ...
       | _ ->
         Printf.sprintf "%sCAML_DEBUG_SOCKET=%s %s %s" (get_unix_environment ())
           !socket_name (Filename.quote !program_name) !arguments
       end)
...
let loading_modes =
  [ "direct", exec_direct; "runtime", exec_with_runtime; "manual", exec_manual ]

let set_launching_function func = launching_func := func
(* Initialization *)
let _ = set_launching_function exec_direct
(*** Connection. ***)
