  ...
  | () -> (
      ...
      | 0 -> (
          ...
          with x ->
            Unix_tools.report_error x;
            exit 1)
      | _ -> ( match wait () with _, WEXITED 0 -> () | _ -> raise Toplevel))
...
let generic_exec =
  match Sys.os_type with "Win32" -> generic_exec_win | _ -> generic_exec_unix
  ...
  generic_exec (function () ->
      ...
      | "Win32" ->
          ...
          Printf.sprintf "%sset CAML_DEBUG_SOCKET=%s& %s %s %s"
            (get_win32_environment ()) !socket_name runtime_program
