...
let load_program () = !launching_func (); main_loop ()
...
let generic_exec_unix cmdline = function
  | () -> (
      ...
      let child =
        try fork () with x -> Unix_tools.report_error x; raise Toplevel
      in
      match child with
      | 0 -> (
        try
          ...
          | 0 ->
              ...
              (try ignore (setsid ()) with Invalid_argument _ -> ());
              execv shell [|shell; "-c"; cmdline ()|]
          | _ -> exit 0
        with x -> Unix_tools.report_error x; exit 1 )
      | _ -> (
        match wait () with _, WEXITED 0 -> () | _ -> raise Toplevel ) )
...
let generic_exec_win cmdline = function
  | () -> (
      ...
      with x -> Unix_tools.report_error x; raise Toplevel )
