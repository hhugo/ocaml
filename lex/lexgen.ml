...
let rec do_remove_nested to_remove = function
  | Bind (e, x) ->
      if IdSet.mem x to_remove then do_remove_nested to_remove e
...
let opt_regexp all_vars char_vars optional_vars double_vars r =
  ...
  let rec simple_forward pos r =
    match r with
    | Tag n ->
        if mem_name n.id double_vars then (r, Some pos)
        else (
  ...
  let rec alloc_exp pos r =
    match r with
    | Tag n -> (
        if mem_name n.id double_vars then (r, pos)
        else
          match pos with
          | Some a ->
    ...
    TransSet.fold
      (fun (t, tags) st ->
        ...
        | ToAction n ->
            ...
            if n < on then { st with final = (n, (0, create_mem_map tags gen)) }
            else st
        | OnChars n -> (
            try
...
let sort_mvs mvs =
  let rec do_rec r mvs =
    ...
    | _ -> (
        ...
        in
        let rem, here =
          List.partition (fun mv -> Ints.mem (orig mv) dests) mvs
        in
        match here with
        | [] -> (
            match rem with
            | Copy (d, _) :: _ ->
                ...
                :: do_rec r
                     (List.map
                        (fun mv ->
                          if orig mv = d then Copy (dest mv, d') else mv)
                        rem)
            | _ -> assert false)
        | _ -> do_rec (here @ r) rem)
  in
...
let goto_state st =
  if dfa_state_is_empty st then (Backtrack, [])
  else
...
let rec split_env gen follow pos m s = function
  ...
  | ((s1, st1) as p) :: rem ->
      ...
      if Cset.is_empty here then p :: split_env gen follow pos m s rem
      else
        let rest = Cset.diff s here in
        let rem =
          if Cset.is_empty rest then rem
        ...
        if Cset.is_empty stay then (here, new_st) :: rem
...
let translate_state shortest_match tags chars follow st =
  ...
  if MemMap.empty = st.others then Perform (n, do_tag_actions n tags m)
  else if shortest_match then
    if n = no_action then Shift (No_remember, reachs chars follow st.others)
    else Perform (n, do_tag_actions n tags m)
  else
    Shift
      ( (if n = no_action then No_remember
