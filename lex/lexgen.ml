...
and automata_trans = No_remember | Remember of int * tag_action list
...
let rec do_find_double =
  ...
  | Sequence (e1, e2) ->
    ...
    and (dbl2, all2) = do_find_double e2
    in
...
let rec do_find_chars sz =
  ...
  | Alternative (e1, e2) ->
    ...
    and (c2, s2, sz2) = do_find_chars sz e2
    in
  ...
  let m =
    IdSet.fold
      (fun (name, _ as x) r ->
         let v =
           if IdSet.mem x char_vars then
             Ident_char (IdSet.mem x optional_vars, get_tag_addr (name, true))
           else
             Ident_string
  ...
  let r =
    List.fold_left
      (fun (reg, actions, count, ntags) (expr, act) ->
         ...
         and (double_vars, all_vars) = find_double expr
         in
         let (m, r, loc_ntags) =
         ...
         (count, m, act) :: actions,
         succ count,
         Int.max loc_ntags ntags) (Empty, [], 0, 0) casedef
  in
  r
...
let addtags transs tags =
  TransSet.fold
    (fun (t, tags_t) r -> TransSet.add (t, Tags.union tags tags_t) r) transs
    TransSet.empty
      ...
      (fun _ (tag, s) r ->
         TagMap.update tag
           (function
            ...
            | Some ss -> Some (StateSetSet.add s ss)) r) m TagMap.empty
  in
    ...
    (fun tag addr r ->
       MemMap.update addr
         (function
          ...
          | Some (otag, s) ->
            assert (tag = otag);
            Some (tag, StateSet.add trans s)) r) m r
  ...
  let env =
    MemMap.fold inverse_mem_map_other o
      (if act = no_action then
         ...
         inverse_mem_map (ToAction act) m_act MemMap.empty)
  in
  let state_key =
    MemMap.fold (fun n _ r -> StateSet.add (OnChars n) r) o
      (if act = no_action then
         ...
         StateSet.add (ToAction act) StateSet.empty)
  in
...
let do_alloc_cell used t =
  let available =
    try Hashtbl.find tag_cells t
    with
    | Not_found -> Ints.empty
  in
...
let old_in_map m r =
  TagMap.fold (fun _ addr r -> if is_old_addr addr then Ints.add addr r else r)
    m r
  ...
  let (new_o, mvs) =
    MemMap.fold
      (fun k (x, m) (r, mvs) ->
         ...
         MemMap.add k (x, m) r, mvs) o (MemMap.empty, mvs)
  in
...
let sort_mvs mvs =
  let rec do_rec r mvs =
    ...
    | _ ->
      ...
      in
      let (rem, here) = List.partition (fun mv -> Ints.mem (orig mv) dests) mvs
      in
      ...
      | [] ->
        ...
        | Copy (d, _) :: _ ->
          ...
          Copy (d', d) ::
            do_rec r
              (List.map
                 (fun mv -> if orig mv = d then Copy (dest mv, d') else mv) rem)
        | _ -> assert false
        end
      | _ -> do_rec (here @ r) rem
      end
         ...
         StateSetSet.fold
           (fun s r ->
              try
                ...
                and tgt = TagMap.find tag (get_map t tgt)
                in
                if src <> tgt then
                  ...
                  r
              with
              | Not_found -> assert false) m r) mem_key []
  in
...
let rec split_env gen follow pos m s =
  ...
  | (s1, st1 as p) :: rem ->
    ...
    if Cset.is_empty here then
      ...
      and new_st = apply_transitions gen st1 pos m follow
      in
...
let comp_shift gen chars follow st =
  MemMap.fold
    (fun pos (_, m) env -> split_env gen follow.(pos) pos m chars.(pos) env) st
  ...
  let (used, r) =
    TagMap.fold
      (fun t m (used, r) ->
         ...
         Ints.add a used, SetTag (a, m) :: r) m (Ints.empty, [])
  in
  let (_, r) =
    TagMap.fold
      (fun tag m (used, r) ->
