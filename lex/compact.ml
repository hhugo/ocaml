...
let do_pack state_num orig compact =
  ...
  let rec pack_from b =
    while b + 257 > Array.length compact.c_trans do grow_compact compact done;
    let rec try_pack =
      function
      | [] -> b
      | (pos, _v) :: rem ->
  ...
  List.iter
    (fun (pos, v) ->
       ...
       compact.c_check.(base + pos) <- state_num) nondef;
...
let pack_moves state_num move_t =
  ...
  and pk_moves = do_pack state_num move_m moves
  in
  pk_trans, pk_moves
...
type lex_tables =
  {
    ...
    tbl_check : int array;
    (* Check (compacted) *)(* code addresses are managed in a similar fashion as transitions *)
    
    tbl_base_code : int array; (* code ptr / base for Shift *)
    tbl_backtrk_code : int array;
    (* nothing / code when Remember *)(* moves to execute before transitions (compacted) *)
    
    tbl_default_code : int array;
    tbl_trans_code : int array;
    tbl_check_code : int array; (* byte code itself *)
    tbl_code : int array
  }
...
let compact_tables state_v =
  ...
  and default_code = Array.make n 0
  in
  for i = 0 to n - 1 do
