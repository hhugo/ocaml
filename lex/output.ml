...
let output_entry some_mem_code ic oc has_refill oci e =
  ...
  fprintf
    ...
    (fun oc x ->
      if some_mem_code
      then fprintf oc "lexbuf.Lexing.lex_mem <- Array.make %d (-1);" x )
    e.auto_mem_size
...
let output_lexdef ic oc oci header rh tables entry_points trailer =
  ...
  then
    Printf.printf
      ...
      ( 2
      * ( Array.length tables.tbl_base
        + Array.length tables.tbl_backtrk
        + Array.length tables.tbl_default
        + Array.length tables.tbl_trans
        + Array.length tables.tbl_check ) );
  let size_groups =
    ( 2
    * ( Array.length tables.tbl_base_code
  ...
  in
  if size_groups > 0 && not !Common.quiet_mode
  then Printf.printf "%d additional bytes used for bindings\n" size_groups;
  flush stdout;
