...
let output_tables oc tbl =
  ...
  fprintf oc "  Lexing.lex_backtrk_code =\n%a;\n" output_array
    tbl.tbl_backtrk_code;
  fprintf oc "  Lexing.lex_default_code =\n%a;\n" output_array
    tbl.tbl_default_code;
...
let output_entry some_mem_code ic oc has_refill oci e =
  ...
  fprintf oc "%s %alexbuf =\n  %a%a __ocaml_lex_%s_rec %alexbuf %d\n"
    ...
    e.auto_mem_size (output_memory_actions "  ") init_moves e.auto_name
    output_args e.auto_args init_num;
  fprintf oc "and __ocaml_lex_%s_rec %alexbuf __ocaml_lex_state =\n" e.auto_name
    output_args e.auto_args;
  fprintf oc "  match Lexing.%sengine" (if some_mem_code then "new_" else "");
...
let output_lexdef ic oc oci header rh tables entry_points trailer =
  (if not !Common.quiet_mode then
     Printf.printf "%d states, %d transitions, table size %d bytes\n"
       (Array.length tables.tbl_base) (Array.length tables.tbl_trans)
       (2 *
          (Array.length tables.tbl_base + Array.length tables.tbl_backtrk +
             Array.length tables.tbl_default +
             Array.length tables.tbl_trans +
             Array.length tables.tbl_check)));
  let size_groups =
    2 *
      (Array.length tables.tbl_base_code + Array.length tables.tbl_backtrk_code +
         Array.length tables.tbl_default_code +
         Array.length tables.tbl_trans_code +
         Array.length tables.tbl_check_code) +
      Array.length tables.tbl_code
  in
  ...
  | entry1 :: entries ->
    ...
    List.iter
      (fun e ->
         ...
         output_entry some_mem_code ic oc has_refill oci e) entries;
    output_string oc ";;\n\n"
  end;
