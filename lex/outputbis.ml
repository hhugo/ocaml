...
let output_auto_defs ctx =
  if ctx.has_refill then
    ...
    (pr ctx "\n";
     ...
     pr ctx "    if _curr < _len then\n";
     pr ctx
       "      Char.code (Bytes.unsafe_get _buf _curr), _buf, _len, (_curr + 1), \
        _last\n";
     pr ctx "    else\n";
  ...
  | mvs ->
    ...
    List.iter
      (fun mv ->
         ...
         | Set tgt -> fprintf oc "[%d] <- p ; " tgt) mvs;
    output_string oc " *)\n";
    List.iter
      (fun mv ->
         match mv with
...
let output_pat ctx i =
  if i >= 256 then pr ctx "|eof" else pr ctx "|'%s'" (Char.escaped (Char.chr i))
...
let output_moves ctx pref moves =
  ...
  Hashtbl.iter
    (fun m (mems, pats) ->
       if m <> !most_frequent then output_clause ctx pref (List.rev pats) mems m)
    t;
...
let output_tag_actions pref ctx mvs =
  ...
  List.iter
    (fun i ->
       ...
       | EraseTag t -> pr ctx "%s%a <- -1 ;\n" pref output_mem_access t) mvs

let output_trans_body pref ctx =
  function
  | Perform (n, mvs) ->
       ...
       Array.iter
         (function
          | Goto i, _ when not (Hashtbl.mem tbl i) ->
            ...
            count i
          | _ -> ()) a
     | Perform _ -> ()) transitions;
  Array.mapi
    (fun i ->
       function
       | Perform _ -> true
       | Shift _ -> counters.(i) = 1) transitions
...
let goto_state inline transitions ctx pref n =
  if inline.(n) then
    ...
    pr ctx "%s__ocaml_lex_state%d lexbuf %s _buf _len _curr _last%s\n" pref n
      (last_action ctx) (if ctx.has_refill then " k" else "")
  ...
  | entry1 :: entries ->
    ...
    List.iter
      (fun e ->
         ...
         output_entry ic ctx tr e) entries;
    output_string oc ";;\n\n"
  end;
