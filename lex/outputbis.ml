...
let output_auto_defs ctx =
  ...
  then (
    ...
    pr
      ctx
      "      Char.code (Bytes.unsafe_get _buf _curr), _buf, _len, (_curr + \
       1), _last\n";
    pr ctx "    else\n";
...
let output_action ctx pref mems r =
  ...
  | Backtrack ->
      pr
        ...
        pref;
      if ctx.has_refill
      then pr ctx "%sk lexbuf %s\n" pref (last_action ctx)
      else pr ctx "%s%s\n" pref (last_action ctx)
  | Goto n -> ctx.goto_state ctx pref n

let output_pat ctx i =
  if i >= 256
  then pr ctx "|eof"
  else pr ctx "|'%s'" (Char.escaped (Char.chr i))
...
let output_moves ctx pref moves =
  ...
  Hashtbl.iter
    (fun m (mems, pats) ->
      if m <> !most_frequent
      then output_clause ctx pref (List.rev pats) mems m )
    t;
...
let output_trans_body pref ctx = function
  ...
  | Shift (trans, move) ->
      ...
      then (
        ...
        pr
          ctx
          "%slet state lexbuf _last_action _buf _len _curr _last k = \
           function\n"
          pref;
...
let output_automata ctx auto inline =
  ...
  for i = 0 to n - 1 do
    ...
    then (
      pr
        ctx
        "%s __ocaml_lex_state%d lexbuf _last_action _buf _len _curr _last \
         %s=\n"
...
let output_init ctx pref e init_moves =
  ...
  then
    pr
      ctx
      "%slexbuf.Lexing.lex_mem <- Array.make %d (-1);\n"
      pref
      e.auto_mem_size;
...
let choose_inlining entry_points transitions =
  ...
  Array.iter
    ...
    transitions;
  Array.mapi
    (fun i -> function Perform _ -> true | Shift _ -> counters.(i) = 1)
    transitions

let goto_state inline transitions ctx pref n =
  if inline.(n)
  then output_trans_body pref ctx transitions.(n)
  else
    pr
      ctx
      "%s__ocaml_lex_state%d lexbuf %s _buf _len _curr _last%s\n"
      pref
      n
      (last_action ctx)
      (if ctx.has_refill then " k" else "")
...
let output_lexdef ic oc tr header rh entry_points transitions trailer =
  ...
  | entry1 :: entries ->
      ...
      List.iter
        (fun e -> output_string oc "and "; output_entry ic ctx tr e)
        entries;
      output_string oc ";;\n\n" );
