...
let update_tracker tr =
  ...
  try
    while true do
      ...
      | '\n' when not !cr_seen -> tr.cur_line <- tr.cur_line + 1
      | '\r' ->
        cr_seen := true;
...
let copy_chunk ic oc trl loc add_parens =
  if loc.start_pos < loc.end_pos || add_parens then
...
let output_env ic oc tr env =
  ...
  | _ ->
    ...
    let env =
      List.sort
        (fun ((_, p1), _) ((_, p2), _) ->
           Stdlib.compare p1.start_pos p2.start_pos) env
    in
    List.iter
      (fun ((_, pos), v) ->
         fprintf oc "%s\n" !pref;
...
let output_refill_handler ic oc oci =
  function
  | None -> false
  | Some location ->
    output_string oc
      "let __ocaml_lex_refill : (Lexing.lexbuf -> 'a) -> (Lexing.lexbuf -> \
       'a) =\n";
