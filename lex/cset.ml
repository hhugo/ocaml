...
let rec union s1 s2 =
  ...
  | ((c1, d1) as p1) :: r1, (c2, d2) :: r2 ->
      if c1 > c2 then union s2 s1
              ...
              d1 + 1 < c2 then p1 :: union r1 s2
      else if d1 < d2 then union ((c1, d2) :: r2) r1
      else union s1 r2
...
let rec inter l l' =
  ...
  | [], _ -> []
  | (c1, c2) :: r, (c1', c2') :: r' ->
      if c2 < c1' then inter r l'
      else if c2' < c1 then inter l r'
      else if c2 < c2' then (Int.max c1 c1', c2) :: inter r l'
      else (Int.max c1 c1', c2') :: inter l r'
...
let rec diff l l' =
  ...
  | [], _ -> []
  | (c1, c2) :: r, (c1', c2') :: r' ->
      if c2 < c1' then (c1, c2) :: diff r l'
      else if c2' < c1 then diff l r'
      else
