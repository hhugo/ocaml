...
let _ =
  Hashtbl.add
    ...
    "show_dirs"
    (Directive_none
       (fun () -> List.iter print_endline (Load_path.get_paths ())))
...
let load_file ppf name0 =
  ...
  | Some name ->
      ...
      let success =
        ...
        try Dynlink.loadfile fn; true with
        | Dynlink.Error err ->
            fprintf
              ppf
...
let match_printer_type ppf desc typename =
  let printer_type =
    try Env.lookup_type (Ldot (Lident "Opttopdirs", typename)) !toplevel_env
    with Not_found ->
...
let find_printer_type ppf lid =
  try
    ...
    let ty_arg, is_old_style =
      try match_printer_type ppf desc "printer_type_new", false
      with Ctype.Unify _ ->
        match_printer_type ppf desc "printer_type_old", true
    in
