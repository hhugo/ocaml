...
module EvalBase = struct
  let eval_ident id =
    try
      if Ident.persistent id || Ident.global id then global_symbol id
      else toplevel_value id
    with _ -> raise (Undefined_global (Ident.name id))
end
...
let load_lambda ppf ~module_ident ~required_globals lam size =
  ...
  match
    Fun.protect
      ~finally:(fun () -> try Sys.remove dll with Sys_error _ -> ())
