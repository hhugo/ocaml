...
external ndl_run_toplevel
  : string -> string -> res = "caml_natdynlink_run_toplevel"
...
let global_symbol id =
  ...
  | None -> fatal_error ("Toploop.global_symbol " ^ Ident.unique_name id)
  | Some obj -> obj
...
let rec remember phrase_name i =
  function
  | [] -> ()
...
let toplevel_value id =
  ...
  with
  | _ -> Misc.fatal_error @@ "Unknown ident: " ^ Ident.unique_name id
...
let close_phrase lam =
  ...
  Ident.Set.fold
    (fun id l ->
       ...
       let glob =
         Lprim
           (Pfield pos, [ Lprim (Pgetglobal glb, [], Loc_unknown) ], Loc_unknown)
       in
       Llet (Strict, Pgenval, id, glob, l)) (free_variables lam) lam
  ...
  let eval_ident id =
    try
      if Ident.persistent id || Ident.global id then
        ...
        toplevel_value id
    with
    | _ -> raise (Undefined_global (Ident.name id))
end
...
let load_lambda ppf ~module_ident ~required_globals lam size =
  ...
  let dll =
    if !Clflags.keep_asm_file then
      ...
      Filename.temp_file ("caml" ^ !phrase_name) ext_dll
  in
  ...
  let dll =
    if Filename.is_implicit dll then
      Filename.concat (Sys.getcwd ()) dll
    else
      dll
  in
  match
    Fun.protect
      ~finally:(fun () ->
        ...
        | Sys_error _ -> ()
          ...
          remove files) *)  (fun () -> dll_run dll !phrase_name)
  with
  | res -> res
  | exception x ->
    record_backtrace ();
...
let execute_phrase print_outcome ppf phr =
  ...
  | Ptop_def sstr ->
            ...
            pstr_desc =
              Pstr_value
                (Asttypes.Nonrecursive,
                 [
                   { pvb_expr = e; pvb_pat = { ppat_desc = Ppat_any; _ };
                     pvb_attributes = attrs; _ } ]); pstr_loc = loc } ]
        ->
    ...
    let (module_ident, res, required_globals, size) =
      if Config.flambda then
        let
          { Lambda.module_ident; main_module_block_size = size; required_globals;
            code = res } =
...
let load_file _ ppf name0 =
  ...
  | Some name ->
    let (fn, tmp) =
      if
        Filename.check_suffix name ".cmx" || Filename.check_suffix name ".cmxa"
      then
