...
let dir_directory s =
  ...
  toplevel_env :=
    Stdlib.String.Set.fold
      ...
      (Env.persistent_structures_of_dir dir) !toplevel_env
...
let dir_show_dirs () = List.iter print_endline (Load_path.get_paths ())
...
let _ =
  add_directive "load" (Directive_string (with_error_fmt dir_load))
    {
      section = section_run;
...
let _ =
  add_directive "load_rec" (Directive_string (with_error_fmt dir_load_rec))
    {
      section = section_run;
...
let _ =
  add_directive "use" (Directive_string (with_error_fmt dir_use))
    {
      section = section_run;
...
let _ =
  add_directive "use_output" (Directive_string (with_error_fmt dir_use_output))
    {
      section = section_run;
      doc =
...
let _ =
  add_directive "mod_use" (Directive_string (with_error_fmt dir_mod_use))
    {
      section = section_run;
      doc =
...
let match_simple_printer_type desc printer_type =
  ...
  (try
     Ctype.unify !toplevel_env (Ctype.newconstr printer_type [ ty_arg ])
       (Ctype.instance desc.val_type)
   with
   | Ctype.Unify _ -> raise Bad_printing_function);
  Ctype.end_def ();
...
let match_generic_printer_type desc path args printer_type =
  ...
  let ty_expected =
    List.fold_right
      (fun ty_arg ty ->
         Ctype.newty (Tarrow (Asttypes.Nolabel, ty_arg, ty, Cunknown))) ty_args
      (Ctype.newconstr printer_type [ ty_target ])
  in
  (try Ctype.unify !toplevel_env ty_expected (Ctype.instance desc.val_type)
   with
   | Ctype.Unify _ -> raise Bad_printing_function);
  Ctype.end_def ();
...
let match_printer_type ppf desc =
  ...
  | Bad_printing_function ->
    (try match_simple_printer_type desc printer_type_old, true
     with
     | Bad_printing_function as exn ->
       begin match extract_target_parameters desc.val_type with
       | None -> raise exn
       | Some (path, args) ->
...
let find_printer_type ppf lid =
  ...
  | path, desc ->
    begin match match_printer_type ppf desc with
    ...
    | Some (ty_path, ty_args) ->
      let rec build v =
        ...
        | [] ->
          ...
          in
          Zero print_function
        | _ :: args -> Succ (fun fn -> build ((Obj.obj v : _ -> Obj.t) fn) args)
      in
...
let dir_remove_printer ppf lid =
  try
    ...
    with
    | Not_found -> fprintf ppf "No printer named %a.@." Printtyp.longident lid
  with
  | Exit -> ()
...
let reg_show_prim name to_sig doc =
  ...
  add_directive name (Directive_ident (show_prim to_sig std_formatter))
...
let () =
  reg_show_prim "show_module"
    (fun env loc id lid ->
       ...
       in
       accum_aliases md []) "Print the signature of the corresponding module."
...
let _ =
  add_directive "print_depth" (Directive_int (fun n -> max_printer_depth := n))
    {
      section = section_print;
...
let _ =
  add_directive "print_length" (Directive_int (fun n -> max_printer_steps := n))
    {
      section = section_print;
...
let _ =
  add_directive "labels" (Directive_bool (fun b -> Clflags.classic := not b))
    {
...
let _ =
  add_directive "principal" (Directive_bool (fun b -> Clflags.principal := b))
    {
      section = section_options;
...
let directive_sections () =
  ...
  let add_dir name =
    ...
    Hashtbl.replace sections section
      ((name, dir, doc) ::
         try Hashtbl.find sections section
         with
         | Not_found -> [])
  in
  List.iter add_dir (all_directive_names ());
  let take_section section =
    if not (Hashtbl.mem sections section) then
      section, []
    else
      let section_dirs =
...
let _ =
  add_directive "help" (Directive_none (print_directives std_formatter))
    {
