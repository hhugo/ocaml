...
let rec extract_last_arrow desc =
  ...
  | Some ((_, r) as res) -> (
      try extract_last_arrow r with Bad_printing_function -> res)
...
let match_simple_printer_type desc printer_type =
  ...
  (try
   ...
   with Ctype.Unify _ -> raise Bad_printing_function);
  Ctype.end_def ();
  ...
  *)
  ...
  let it_path = function
    | Path.Pident id' when Ident.name id' = Ident.name id ->
        if Ident.same id id' then recursive_use := true
...
let is_extension_constructor = function Cstr_extension _ -> true | _ -> false
