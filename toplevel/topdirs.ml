...
let load_compunit ic filename ppf compunit =
  ...
  LongString.blit_string
    "\000\000\000\001\000\000\000"
    0
    code
    (compunit.cu_codesize + 1)
    7;
  let initial_symtable = Symtable.current_state () in
  Symtable.patch_object code compunit.cu_reloc;
  Symtable.update_global_table ();
  let events =
    if compunit.cu_debug = 0
    then [||]
    else (
          ...
          (function
            | Reloc_getglobal id, _ when not (Symtable.is_global_defined id)
              -> (
                ...
                | exception Not_found -> ()
                | file ->
                    if not (load_file recursive ppf file)
...
let printer_type ppf typename =
  let printer_type =
    try Env.lookup_type (Ldot (Lident "Topdirs", typename)) !toplevel_env
    with Not_found ->
...
let match_generic_printer_type desc path args printer_type =
  ...
  if not (Ctype.all_distinct_vars !toplevel_env args)
  then raise (Ctype.Unify []);
  ...
  try
    ...
    | _ ->
        ...
        if Obj.is_block clos
           && ( Obj.tag clos = Obj.closure_tag
              || Obj.tag clos = Obj.infix_tag )
           &&
...
let () =
  reg_show_prim
    ...
    (fun env loc id lid ->
      ...
      if not (Ctype.equal env true [desc.cstr_res] [Predef.type_exn])
...
let () =
  add_directive
    ...
    { section = section_env
    ; doc =
        "Print the signatures of components from any of the categories \
         below." }
