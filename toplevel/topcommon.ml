...
let parse_mod_use_file name lb =
  ...
  let items =
    List.concat
      (List.map (function Ptop_def s -> s | Ptop_dir _ -> [])
         (!parse_use_file lb))
  in
  [
    Ptop_def
  ...
  module Printer =
    ...
    (struct
      ...
      let eval_address addr =
        ...
        with
        | Undefined_global _ -> raise Error
...
let preprocess_phrase ppf phr =
  let phr =
    match phr with
    | Ptop_def str ->
      let str = Pparse.apply_rewriters_str ~restore:true ~tool_name:"ocaml" str
      in
      Ptop_def str
    | phr -> phr
  in
  ...
  let load_path =
    List.concat
      [ [ "" ]; List.map expand (List.rev !Compenv.first_include_dirs);
        ...
        List.map expand (List.rev !Compenv.last_include_dirs); current_load_path;
        [ expand "+camlp4" ] ]
  in
...
let try_run_directive ppf dir_name pdir_arg =
  ...
  | Some d ->
    ...
    | Directive_int f, Some { pdira_desc = Pdir_int (n, None) } ->
      ...
      | exception _ ->
        fprintf ppf
          "Integer literal exceeds the range of representable integers \
           for directive `%s'.@." dir_name;
        false
      end
    ...
    | _ ->
      ...
      in
      fprintf ppf "Directive `%s' expects %s, got %s.@." dir_name dir_type
        arg_type;
      false
    end
