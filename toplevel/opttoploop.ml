...
let global_symbol id =
  ...
  | None -> fatal_error ("Opttoploop.global_symbol " ^ Ident.unique_name id)
  | Some obj -> obj
...
let toplevel_value id =
  let glob, pos =
    if Config.flambda
    then toplevel_value id
    else Translmod.nat_toplevel_name id
  in
...
let parse_mod_use_file name lb =
  ...
  in
  [ Ptop_def
      [Str.module_ (Mb.mk (Location.mknoloc modname) (Mod.structure items))]
  ]
...
let execute_phrase print_outcome ppf phr =
  ...
  | Ptop_def sstr -> (
      ...
      try
        toplevel_env := newenv;
        let res =
          load_lambda ppf ~required_globals ~module_ident res size
        in
        let out_phr =
          match res with
          | Result _ ->
              if Config.flambda
              then (* CR-someday trefis: *)
                ()
                    ...
                    | _ ->
                        ...
                        then
                          ...
                          | [Sig_value (id, vd, _)] ->
                              ...
                              in
                              let ty =
                                Printtyp.tree_of_type_scheme vd.val_type
                              in
                              Ophr_eval (outv, ty)
                          | _ -> assert false
          ...
          try
            List.iter
              (fun ph ->
                ...
                if not (execute_phrase !use_print_results ppf ph)
                then raise Exit )
              ( if wrap_mod
...
let load_ocamlinit ppf =
  ...
  then ()
    ...
    | None -> (
        ...
        then ignore (use_silently ppf ".ocamlinit")
        else
          try
            let home_init =
            ...
            then ignore (use_silently ppf home_init)
          with Not_found -> () )
...
let loop ppf =
  ...
  then
    fprintf
      ppf
      "        OCaml version %s - native toplevel@.@."
      Config.version;
...
let override_sys_argv args =
  ...
  if Array.length Sys.argv < len
  then invalid_arg "Toploop.override_sys_argv";
