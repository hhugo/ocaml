...
let use_input ppf ~wrap_in_module input =
  ...
  | File name ->
    begin match Load_path.find name with
    | filename ->
...
let mod_use_input ppf name = use_input ppf ~wrap_in_module:true name
let use_input ppf name = use_input ppf ~wrap_in_module:false name
let use_file ppf name = use_input ppf (File name)

let use_silently ppf name =
  Misc.protect_refs [ R (use_print_results, false) ]
...
let run_script ppf name args =
  ...
  let explicit_name =
    ...
    | File name as filename ->
      if (* Prevent use_silently from searching in the path. *)
        name <> "" && Filename.is_implicit name
      then
        File (Filename.concat Filename.current_dir_name name)
      else
        filename
  ...
  if !Clflags.noinit then
    ...
    | Some f ->
      if Sys.file_exists f then
        ignore (use_silently ppf (File f))
      else
        fprintf ppf "Init file not found: \"%s\".@." f
    | None ->
...
let loop ppf =
  ...
  (if not !Clflags.noversion then
     fprintf ppf "        OCaml version %s%s%s@.@." Config.version
