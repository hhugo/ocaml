...
type traced_function =
  { path : Path.t
  ; (* Name under which it is traced *)
    closure : Obj.t
  ; (* Its function closure (patched) *)
    actual_code : codeptr
  ; (* Its original code pointer *)
...
let is_traced clos =
  let rec is_traced = function
    | [] -> None
    | tf :: rem ->
        if tf.closure == clos then Some tf.path else is_traced rem
  in
...
let print_label ppf l =
  if l <> Asttypes.Nolabel
  then fprintf ppf "%s:" (Printtyp.string_of_label l)
