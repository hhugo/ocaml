  ...
  let outval_of_untyped_exception bucket =
    if O.tag bucket <> 0 then
      ...
      let args =
        if
          (name = "Match_failure" ||
             name = "Assert_failure" || name = "Undefined_recursive_module") &&
            O.size bucket = 2 && O.tag (O.field bucket 1) = 0
        then
          outval_of_untyped_exception_args (O.field bucket 1) 0
        else
          outval_of_untyped_exception_args bucket 1
      in
    ...
    let rec build gp depth =
      ...
      | Zero fn ->
        let out_printer obj =
          let printer ppf =
            try fn ppf obj
            with
            | exn -> exn_printer ppf function_path exn
          in
      ...
      | Succ fn ->
        let print_val fn_arg =
          let print_arg ppf o = !Oprint.out_value ppf (fn_arg (depth + 1) o) in
  ...
  let tree_of_qualified find env ty_path name =
    ...
    | Pdot (p, _s) ->
      ...
      then
        ...
        Oide_dot (Printtyp.tree_of_path p, Out_name.print name)
    | Papply _ -> Printtyp.tree_of_path ty_path
  
  let tree_of_constr =
    tree_of_qualified
      (fun lid env -> (Env.find_constructor_by_name lid env).cstr_res)
  ...
  let outval_of_value max_steps max_depth check_depth env obj ty =
    ...
    let nest_gen err f depth obj ty =
      ...
      if not (O.is_block repr) then
        f depth obj ty
      else if ObjTbl.mem nested_values repr then
        err
      else
          ...
          | Tconstr (path, ty_list, _) ->
            (try
               ...
               | { type_kind = Type_variant (constr_list, rep) } ->
                 ...
                 | Cstr_tuple l ->
                   let ty_args = instantiate_types env type_params ty_list l in
                   tree_of_constr_with_args (tree_of_constr env path)
                     (Ident.name cd_id) false 0 depth obj ty_args unbx
                 | Cstr_record lbls ->
                   let r =
                     tree_of_record_fields depth env path type_params ty_list
                       lbls 0 obj unbx
                   in
                   Oval_constr
               ...
               | { type_kind = Type_record (lbl_list, rep) } ->
                 ...
                 | None ->
                   ...
                   let unbx =
                     ...
                     | _ -> false
                   in
                   tree_of_record_fields depth env path decl.type_params ty_list
              ...
              let rec find =
                ...
                | (l, f) :: fields ->
                  if Btype.hash_variant l = tag then
                    begin match Btype.row_field_repr f with
                    | Rpresent (Some ty) | Reither (_, [ ty ], _, _) ->
                      let args = nest tree_of_val (depth - 1) (O.field obj 1) ty
                      in
    ...
    =
      let rec tree_of_fields pos =
        ...
        | { ld_id; ld_type } :: remainder ->
          ...
          let lid =
            if pos = 0 then
              tree_of_label env path (Out_name.create name)
            else
              Oide_ident (Out_name.create name)
          and v =
            if unboxed then
    ...
    and tree_of_extension type_path ty_list depth bucket =
      ...
      (* Attempt to recover the constructor description for the exn
        ...
        let lid =
          ...
          | Syntaxerr.Error _ | Lexer.Error _ -> raise Not_found
        in
        ...
        tree_of_constr_with_args (fun x -> Oide_ident x) name
          (cstr.cstr_inlined <> None) 1 depth bucket args false
      with
      | Not_found | EVP.Error ->
        begin match check_depth depth bucket ty with
        | Some x -> x
    ...
    and find_printer depth env ty =
      let rec find =
        ...
        | (_name, Simple (sch, printer)) :: remainder ->
          if Ctype.is_moregeneral env false sch ty then
            printer
          else
            find remainder
