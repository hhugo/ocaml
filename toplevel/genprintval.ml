...
module Make (O : OBJ) (EVP : EVALPATH with type valu = O.t) = struct
  ...
  let outval_of_untyped_exception_args obj start_offset =
    if O.size obj > start_offset
    then (
      let list = ref [] in
      for i = start_offset to O.size obj - 1 do
        let arg = O.field obj i in
        if not (O.is_block arg)
        then
          list := Oval_int (O.obj arg : int) :: !list
          (* Note: this could be a char or a constant constructor... *)
        else if O.tag arg = Obj.string_tag
        then
          list :=
            Oval_string ((O.obj arg : string), max_int, Ostr_string)
            :: !list
        else if O.tag arg = Obj.double_tag
        then list := Oval_float (O.obj arg : float) :: !list
        else
          list :=
            Oval_constr (Oide_ident (Out_name.create "_"), []) :: !list
      done;
      List.rev !list )
    else []
  ...
  let tree_of_constr =
    tree_of_qualified (fun lid env ->
        (Env.lookup_constructor lid env).cstr_res )
  ...
  let outval_of_value max_steps max_depth check_depth env obj ty =
    ...
    let nest_gen err f depth obj ty =
      let repr = obj in
      if not (O.is_block repr)
      then f depth obj ty
      else if ObjTbl.mem nested_values repr
      then err
      else (
              ...
              | {type_kind = Type_record (lbl_list, rep)} -> (
                ...
                | None ->
                    ...
                    tree_of_record_fields
                      ...
                      obj
                      unbx )
              | {type_kind = Type_open} -> tree_of_extension path depth obj
            with
            | Not_found ->
          ...
          | Tvariant row ->
              ...
              then
                ...
                let rec find = function
                  | (l, _) :: fields ->
                      if Btype.hash_variant l = tag
                      then Oval_variant (l, None)
                      else find fields
                  | [] -> Oval_stuff "<variant>"
                in
                find row.row_fields
          | Tobject (_, _) -> Oval_stuff "<obj>"
          | Tsubst ty -> tree_of_val (depth - 1) obj ty
          | Tfield (_, _, _, _) | Tnil | Tlink _ ->
              fatal_error "Printval.outval_of_value"
          | Tpoly (ty, _) -> tree_of_val (depth - 1) obj ty
          | Tpackage _ -> Oval_stuff "<module>" )
    and tree_of_record_fields depth env path type_params ty_list lbl_list
        pos obj unboxed =
      let rec tree_of_fields pos = function
        | [] -> []
        | {ld_id; ld_type} :: remainder ->
            let ty_arg =
              try Ctype.apply env type_params ld_type ty_list
              with Ctype.Cannot_apply -> abstract_type
            in
            ...
            let lid =
              if pos = 0
              then tree_of_label env path (Out_name.create name)
            ...
            and v =
              ...
              then tree_of_val (depth - 1) obj ty_arg
                ...
                let fld =
                  if O.tag obj = O.double_array_tag
                  then O.repr (O.double_field obj pos)
    ...
    and tree_of_constr_with_args tree_of_cstr cstr_name inlined start depth
        obj ty_args unboxed =
    ...
    and tree_of_extension type_path depth bucket =
      ...
      let name : string = O.obj (O.field slot 0) in
    ...
    and apply_generic_printer path printer args =
      ...
      | _ ->
          fun _obj ->
            ...
            Oval_printer printer
    in
    nest tree_of_val max_depth obj ty
end
