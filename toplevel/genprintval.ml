...
module Make (O : OBJ) (EVP : EVALPATH with type valu = O.t) = struct
  ...
  module ObjTbl = Hashtbl.Make (struct
    ...
    let hash x = try Hashtbl.hash x with _exn -> 0
  end)
  ...
  let outval_of_untyped_exception_args obj start_offset =
    if O.size obj > start_offset then (
      let list = ref [] in
      for i = start_offset to O.size obj - 1 do
        let arg = O.field obj i in
        if not (O.is_block arg) then list := Oval_int (O.obj arg : int) :: !list
        ...
        else list := Oval_constr (Oide_ident (Out_name.create "_"), []) :: !list
      done;
      List.rev !list)
    else []
    ...
    ref
      ([
         ( Pident (Ident.create_local "print_int"),
           Simple (Predef.type_int, fun x -> Oval_int (O.obj x : int)) );
         ( Pident (Ident.create_local "print_float"),
           Simple (Predef.type_float, fun x -> Oval_float (O.obj x : float)) );
         ( Pident (Ident.create_local "print_char"),
           Simple (Predef.type_char, fun x -> Oval_char (O.obj x : char)) );
         ( Pident (Ident.create_local "print_int32"),
           Simple (Predef.type_int32, fun x -> Oval_int32 (O.obj x : int32)) );
         ...
         ( Pident (Ident.create_local "print_int64"),
           Simple (Predef.type_int64, fun x -> Oval_int64 (O.obj x : int64)) );
       ]
        : (Path.t * printer) list)
  ...
  let out_exn path exn = Oval_printer (fun ppf -> exn_printer ppf path exn)

  let install_printer path ty fn =
    let print_val ppf obj =
      try fn ppf obj with exn -> exn_printer ppf path exn
    in
  ...
  let install_generic_printer' function_path ty_path fn =
    let rec build gp depth =
      ...
      | Zero fn ->
          let out_printer obj =
            let printer ppf =
              try fn ppf obj with exn -> exn_printer ppf function_path exn
            in
      ...
      | Succ fn ->
          let print_val fn_arg =
            let print_arg ppf o =
              !Oprint.out_value ppf (fn_arg (depth + 1) o)
            in
            build (fn print_arg) depth
          in
          Succ print_val
    in
  ...
  let remove_printer path =
    let rec remove = function
      | [] -> raise Not_found
      | ((p, _) as printer) :: rem ->
          if Path.same p path then rem else printer :: remove rem
    in
  ...
  let tree_of_qualified find env ty_path name =
    ...
    | Papply _ -> Printtyp.tree_of_path ty_path
  ...
  and tree_of_label =
    tree_of_qualified (fun lid env -> (Env.find_label_by_name lid env).lbl_res)
  ...
  let outval_of_value max_steps max_depth check_depth env obj ty =
    ...
    let nest_gen err f depth obj ty =
      let repr = obj in
      if not (O.is_block repr) then f depth obj ty
      else if ObjTbl.mem nested_values repr then err
      else (
    ...
    let rec tree_of_val depth obj ty =
      ...
      if !printer_steps < 0 || depth < 0 then Oval_ellipsis
        ...
        try find_printer depth env ty obj
        with Not_found -> (
          match get_desc ty with
          | Tvar _ | Tunivar _ -> Oval_stuff "<poly>"
          | Tarrow _ -> Oval_stuff "<fun>"
          | Ttuple ty_list -> Oval_tuple (tree_of_val_list 0 depth obj ty_list)
          ...
          | Tconstr (path, [], _) when Path.same path Predef.path_bytes ->
              let s = Bytes.to_string (O.obj obj : bytes) in
              Oval_string (s, !printer_steps, Ostr_bytes)
          | Tconstr (path, [ ty_arg ], _) when Path.same path Predef.path_lazy_t
            ->
              ...
              *)
              if obj_tag = Obj.lazy_tag then Oval_stuff "<lazy>"
                ...
                *)
                let v =
                  if obj_tag = Obj.forward_tag then
              ...
              try
                ...
                | { type_kind = Type_variant (constr_list, rep) } -> (
                    ...
                    let tag =
                      ...
                      else if O.is_block obj then Cstr_block (O.tag obj)
                    ...
                    | Cstr_tuple l ->
                        let ty_args =
                          instantiate_types env type_params ty_list l
                        in
                        tree_of_constr_with_args (tree_of_constr env path)
                          (Ident.name cd_id) false 0 depth obj ty_args unbx
                    | Cstr_record lbls ->
                        let r =
                          tree_of_record_fields depth env path type_params
                            ty_list lbls 0 obj unbx
                        in
                        Oval_constr
                ...
                | { type_kind = Type_record (lbl_list, rep) } -> (
                    ...
                    | None ->
                        let pos =
                          match rep with Record_extension _ -> 1 | _ -> 0
                        in
                        let unbx =
                          match rep with Record_unboxed _ -> true | _ -> false
                        in
                        tree_of_record_fields depth env path decl.type_params
                          ty_list lbl_list pos obj unbx)
                | { type_kind = Type_open } ->
          ...
          | Tvariant row ->
              ...
              if O.is_block obj then
                ...
                let rec find = function
                  | (l, _) :: fields ->
                      if Btype.hash_variant l = tag then Oval_variant (l, None)
                      else find fields
                  | [] -> Oval_stuff "<variant>"
                in
                find row.row_fields
          | Tobject (_, _) -> Oval_stuff "<obj>"
          | Tsubst _ | Tfield (_, _, _, _) | Tnil | Tlink _ ->
              fatal_error "Printval.outval_of_value"
          | Tpoly (ty, _) -> tree_of_val (depth - 1) obj ty
          | Tpackage _ -> Oval_stuff "<module>")
    and tree_of_record_fields depth env path type_params ty_list lbl_list pos
        obj unboxed =
      let rec tree_of_fields pos = function
        ...
        | { ld_id; ld_type } :: remainder ->
            ...
            and v =
              if unboxed then tree_of_val (depth - 1) obj ty_arg
              else
                let fld =
                  if O.tag obj = O.double_array_tag then
                    O.repr (O.double_field obj pos)
                  else O.field obj pos
                in
    ...
    and tree_of_constr_with_args tree_of_cstr cstr_name inlined start depth obj
        ty_args unboxed =
      ...
      let args =
        if inlined || unboxed then
          ...
          | _ -> assert false
        else tree_of_val_list start depth obj ty_args
      in
      Oval_constr (lid, args)
    and tree_of_extension type_path ty_list depth bucket =
      let slot = if O.tag bucket <> 0 then bucket else O.field bucket 0 in
      let name = (O.obj (O.field slot 0) : string) in
      try
        (* Attempt to recover the constructor description for the exn
           from its name *)
        let lid =
          try Parse.longident (Lexing.from_string name)
          with
        ...
        if not (EVP.same_value slot (EVP.eval_address addr)) then
          raise Not_found;
        let type_params =
        ...
        tree_of_constr_with_args
          (fun x -> Oide_ident x)
          name
          (cstr.cstr_inlined <> None)
          1 depth bucket args false
    ...
    and find_printer depth env ty =
      let rec find = function
        ...
        | (_name, Simple (sch, printer)) :: remainder ->
            if Ctype.is_moregeneral env false sch ty then printer
            else find remainder
        | (_name, Generic (path, fn)) :: remainder -> (
            match get_desc (Ctype.expand_head env ty) with
            | Tconstr (p, args, _) when Path.same p path -> (
                try apply_generic_printer path (fn depth) args
    ...
    nest tree_of_val max_depth obj (Ctype.correct_levels ty)
end
