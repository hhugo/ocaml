...
let getvalue name =
  ...
  with
  | Not_found -> fatal_error (name ^ " unbound at toplevel")
...
module EvalBase = struct
  let eval_ident id =
    if Ident.persistent id || Ident.global id then
      ...
      with
      | Symtable.Error (Undefined_global name) -> raise (Undefined_global name)
    else
      let name = Translmod.toplevel_name id in
      try String.Map.find name !toplevel_value_bindings
      with
      | Not_found -> raise (Undefined_global name)
end
...
let load_lambda ppf lam =
  ...
  (if !Clflags.dump_instr then
     fprintf ppf "%a%a@." Printinstr.instrlist init_code Printinstr.instrlist
       fun_code);
  ...
  Printtyp.print_items
    (fun env ->
       ...
       | Sig_value (id, { val_kind = Val_reg; val_type }, _) ->
         Some
           (outval_of_value env (getvalue (Translmod.toplevel_name id)) val_type)
       | _ -> None)
         ...
         (fun dllib ->
            ...
            | Failure reason ->
              fprintf ppf
                ...
                reason;
              raise Load_failed) lib.lib_dllibs;
