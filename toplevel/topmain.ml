...
let prepare ppf =
  ...
  try
    let res =
      let objects = List.rev (!preload_objects @ !first_objfiles) in
...
let file_argument name =
  ...
  then (
    ...
    *)
    Printf.eprintf
      "For implementation reasons, the toplevel does not support having \
       script files (here %S) inside expanded arguments passed through the \
