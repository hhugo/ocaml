...
let parse_id_payload txt loc ~default ~empty cases payload =
  ...
  | None -> warn ()
  | Some id -> (
    match List.assoc_opt id cases with Some r -> r | None -> warn () )
      ...
      then (
        ...
        | PStr [{pstr_desc = Pstr_eval ({pexp_desc}, [])}] -> (
          ...
          | Pexp_constant (Pconst_integer (s, None)) -> (
            try Unroll (Misc.Int_literal_converter.int s)
            with Failure _ ->
        ...
        parse_id_payload
          ...
          ~empty:Always_inline
          ["never", Never_inline; "always", Always_inline]
          payload
...
let parse_specialise_attribute attr =
  ...
  | Some {Parsetree.attr_name = {txt; loc}; attr_payload = payload} ->
      parse_id_payload
        ...
        ["never", Never_specialise; "always", Always_specialise]
        payload
