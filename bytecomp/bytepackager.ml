  ...
  let rel' =
    ...
    | Reloc_getglobal id -> (
      ...
      with Not_found ->
        ...
        then
          Reloc_getglobal
            (Ident.create_persistent (packagename ^ "." ^ name))
        else rel )
    | Reloc_setglobal id -> (
      try
        let id' = List.assoc id mapping in
...
let build_global_target ~ppf_dump oc target_name members mapping pos
    coercion =
  let components =
    List.map2
      (fun m (_id1, id2) ->
...
let report_error ppf = function
  ...
  | Illegal_renaming (name, file, id) ->
      fprintf
        ppf
        "Wrong file naming: %a@ contains the code for@ %s when %s was \
         expected"
        Location.print_filename
        file
        name
