...
let read_member_info file =
  let name = String.capitalize_ascii (Filename.basename (chop_extensions file))
  in
  let kind =
    ...
    if Filename.check_suffix file ".cmi" then
      ...
      try
        ...
        (if compunit.cu_name <> name then
           raise (Error (Illegal_renaming (name, file, compunit.cu_name))));
        close_in ic;
        PM_impl compunit
      with
      | x ->
...
=
  ...
  try
    ...
    List.iter (rename_relocation packagename objfile mapping defined ofs)
      compunit.cu_reloc;
    ...
    (if !Clflags.debug && compunit.cu_debug > 0 then
       (seek_in ic compunit.cu_debug;
        ...
        debug_dirs :=
          List.fold_left (fun s e -> String.Set.add e s) !debug_dirs
            (input_value ic)));
    close_in ic;
...
let rec rename_append_bytecode_list
    packagename oc mapping defined ofs prefix subst
=
  function
  | [] -> ofs
  | m :: rem ->
    begin match m.pm_kind with
    | PM_intf ->
      rename_append_bytecode_list packagename oc mapping defined ofs prefix
        subst rem
    | PM_impl compunit ->
      let size =
      ...
      rename_append_bytecode_list packagename oc mapping (id :: defined)
        (ofs + size) prefix
        (Subst.add_module id (Path.Pdot (root, Ident.name id)) subst) rem
    end
...
let build_global_target ~ppf_dump oc target_name members mapping pos coercion =
  let components =
    List.map2
      (fun m (_id1, id2) ->
         ...
         | PM_intf -> None
         | PM_impl _ -> Some id2) members mapping
  in
  let lam =
    Translmod.transl_package components (Ident.create_persistent target_name)
      coercion
  in
  ...
  let rel = Emitcode.to_packed_file oc instrs in
...
let package_object_files ~ppf_dump files targetfile targetname coercion =
  ...
  let required_globals =
    List.fold_right
      (fun compunit required_globals ->
         match compunit with
         | { pm_kind = PM_intf } -> required_globals
         | { pm_kind = PM_impl { cu_required_globals; cu_reloc } } ->
           let remove_required (rel, _pos) required_globals =
             match rel with
             | Reloc_setglobal id -> Ident.Set.remove id required_globals
             | _ -> required_globals
           in
           let required_globals =
  ...
  let mapping =
    List.map
      (fun name ->
         ...
         Ident.create_persistent (targetname ^ "." ^ name)) unit_names
  in
  ...
  try
    ...
    let ofs =
      rename_append_bytecode_list targetname oc mapping [] 0 targetname
        Subst.identity members
    in
    ...
    let compunit =
      {
        ...
        cu_imports = (targetname, Some (Env.crc_of_unit targetname)) :: imports;
    ...
    in
    Emitcode.marshal_to_channel_with_possibly_32bit_compat ~filename:targetfile
      ~kind:"bytecode unit" oc compunit;
...
let package_files ~ppf_dump initial_env files targetfile =
  let files =
    List.map
      (fun f ->
         ...
         with
         | Not_found -> raise (Error (File_not_found f))) files
  in
  ...
  Misc.try_finally
    (fun () ->
       let coercion =
         Typemod.package_units initial_env files targetcmi targetname
       in
...
let report_error ppf =
  ...
  | Multiple_definition (file, ident) ->
    fprintf ppf "File %a redefines %s" Location.print_filename file
      (Ident.name ident)
  | Not_an_object_file file ->
    fprintf ppf "%a is not a bytecode object file" Location.print_filename file
  | Illegal_renaming (name, file, id) ->
    fprintf ppf
      "Wrong file naming: %a@ contains the code for@ %s when %s was expected"
      Location.print_filename file name id
  | File_not_found file -> fprintf ppf "File %s not found" file
