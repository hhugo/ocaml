  ...
  let rel' =
    ...
    | Reloc_getglobal id -> (
        try
          ...
          if List.mem id defined then Reloc_getglobal id'
...
let read_member_info file =
  ...
  let kind =
    ...
    if Filename.check_suffix file ".cmi" then PM_intf
    else
...
let rec rename_append_bytecode_list packagename oc mapping defined ofs prefix
    subst = function
  | [] -> ofs
  | m :: rem -> (
      ...
      | PM_impl compunit ->
          let size =
            rename_append_bytecode packagename oc mapping defined ofs prefix
              subst m.pm_file compunit
          in
          ...
          rename_append_bytecode_list packagename oc mapping (id :: defined)
            (ofs + size) prefix
            (Subst.add_module id (Path.Pdot (root, Ident.name id)) subst)
            rem)
...
let build_global_target ~ppf_dump oc target_name members mapping pos coercion =
  let components =
    List.map2
      (fun m (_id1, id2) ->
        match m.pm_kind with PM_intf -> None | PM_impl _ -> Some id2)
      members mapping
  in
  let lam =
    Translmod.transl_package components
      (Ident.create_persistent target_name)
      coercion
  in
...
let report_error ppf = function
  ...
  | Not_an_object_file file ->
      fprintf ppf "%a is not a bytecode object file" Location.print_filename
        file
