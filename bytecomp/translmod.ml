...
let rec compose_coercions c1 c2 =
  ...
  | Tcoerce_functor (arg1, res1), Tcoerce_functor (arg2, res2) ->
      Tcoerce_functor
        (compose_coercions arg2 arg1, compose_coercions res1 res2)
...
let init_shape id modl =
    ...
    | Mty_ident _ | Mty_alias _ ->
        raise
          (Initialization_failure
             {reason = Unsafe_module_binding; loc; subid})
    | Mty_signature sg ->
  ...
  and init_shape_struct env sg =
    ...
    | [] -> []
    | Sig_value
        (subid, {val_kind = Val_reg; val_type = ty; val_loc = loc}, _)
      :: rem ->
        let init_v =
          ...
          | _ ->
              let not_a_function =
                {reason = Unsafe_non_function; loc; subid}
              in
...
let extract_unsafe_cycle id status init cycle_start =
  let info i =
    match init.(i) with Result.Error r -> id.(i), r | Ok _ -> assert false
  in
...
and transl_module cc rootpath mexp =
  ...
  | Tmod_functor _ ->
      oo_wrap
        mexp.mod_env
        true
        (fun () -> compile_functor mexp cc rootpath loc)
        ()
  ...
  | [] ->
      let body, size =
        ...
        | Tcoerce_structure (pos_cc_list, id_pos_list) ->
            ...
            let get_field pos =
              if pos < 0 then lambda_unit else Lvar v.(pos)
            in
            let ids =
...
let required_globals ~flambda body =
  ...
  let add_global id req =
    if (not flambda) && Ident.Set.mem id globals
    then req
    else Ident.Set.add id req
  in
  let required =
    List.fold_left
...
let rec defined_idents = function
  ...
  | item :: rem -> (
    ...
    | Tstr_module {mb_id; mb_presence = Mp_present} ->
        mb_id :: defined_idents rem
    ...
    | Tstr_class cl_list ->
        List.map (fun (ci, _) -> ci.ci_id_class) cl_list
        @ defined_idents rem
  ...
  let rec transl_store rootpath subst cont = function
    ...
    | item :: rem -> (
      ...
      | Tstr_eval (expr, _attrs) ->
          Lsequence
            ( Lambda.subst no_env_update subst (transl_exp expr)
      ...
      | Tstr_primitive descr ->
          record_primitive descr.val_val;
          transl_store rootpath subst cont rem
      | Tstr_type _ -> transl_store rootpath subst cont rem
      | Tstr_typext tyext ->
          let ids =
            List.map (fun ext -> ext.ext_id) tyext.tyext_constructors
          in
      ...
      | Tstr_exception ext ->
          ...
          Lsequence
            ...
            , transl_store rootpath (add_ident false id subst) cont rem )
      | Tstr_module
          { mb_id = id
          ; mb_loc = loc
          ; mb_presence = Mp_present
      ...
      | Tstr_module
          ...
          Llet
            ...
            , Lsequence
                ( store_ident loc id
                , transl_store rootpath (add_ident true id subst) cont rem
                ) )
      | Tstr_module {mb_presence = Mp_absent} ->
          transl_store rootpath subst cont rem
      | Tstr_recmodule bindings ->
          ...
          let rec loop ids args =
            ...
            | id :: ids, arg :: args ->
                Llet
                  ...
                  , Lsequence (store_ident loc id, loop ids args) )
            | _ -> assert false
          in
          ...
          let rec store_idents pos = function
            ...
            | id :: idl ->
                Llet
                  ...
                  , Lsequence
                      (store_ident loc id, store_idents (pos + 1) idl) )
          in
          Llet
            ( Strict
            , Pgenval
...
let transl_toplevel_item item =
  ...
  | Tstr_modtype _
   |Tstr_module {mb_presence = Mp_absent}
   |Tstr_type _ | Tstr_class_type _ | Tstr_attribute _ ->
      lambda_unit
  ...
  | Circular_dependency cycle ->
      ...
      Location.errorf
        ...
        "Cannot safely evaluate the definition of the following cycle@ of \
         recursively-defined modules:@ %a.@ There are no safe modules in \
         this cycle@ (see manual section %d.%d)."
        print_cycle
        cycle
        chapter
        section
