  ...
  with Not_found ->
    ...
    then PrimMap.enter c_prim_table name
      ...
      let symb =
        ...
        with Not_found -> raise (Error (Unavailable_primitive name))
      in
...
let init () =
  ...
  then set_prim_table_from_file !Clflags.use_prims
  else if String.length !Clflags.use_runtime > 0
  then
...
let update_global_table () =
  ...
  then Meta.realloc_global_data ng;
