...
module Num_tbl (M : Map.S) = struct
  ...
  let find nt key = M.find key nt.tbl
...
let is_global_defined id = Ident.Map.mem id !global_table.tbl

let slot_for_getglobal id =
  try GlobalMap.find !global_table id
  with Not_found -> raise (Error (Undefined_global (Ident.name id)))

let slot_for_setglobal id = GlobalMap.enter global_table id
...
let set_prim_table name = ignore (PrimMap.enter c_prim_table name)

let of_prim name =
  try PrimMap.find !c_prim_table name
  with Not_found -> (
    if
      !Clflags.custom_runtime
      || Config.host <> Config.target
      || !Clflags.no_check_prims
    then PrimMap.enter c_prim_table name
    else
      match Dll.find_primitive name with
      | None -> raise (Error (Unavailable_primitive name))
      | Some Prim_exists -> PrimMap.enter c_prim_table name
      | Some (Prim_loaded symb) ->
          let num = PrimMap.enter c_prim_table name in
          Dll.synchronize_primitive num symb;
          num)

let require_primitive name = if name.[0] <> '%' then ignore (of_prim name)
...
let init () =
  ...
  Array.iteri
    (fun i name ->
      let id =
        ...
        with Not_found -> fatal_error "Symtable.init"
      in
  ...
  if String.length !Clflags.use_prims > 0 then
  ...
  else if String.length !Clflags.use_runtime > 0 then
    ...
    Misc.try_finally
      ...
      (fun () ->
        if
          Sys.command
            (Printf.sprintf "%s -p > %s" !Clflags.use_runtime primfile)
          <> 0
        then raise (Error (Wrong_vm !Clflags.use_runtime));
        set_prim_table_from_file primfile)
  else Array.iter set_prim_table Runtimedef.builtin_primitives
...
let patch_object buff patchlist =
  List.iter
    (function
      | Reloc_literal sc, pos -> patch_int buff pos (slot_for_literal sc)
      | Reloc_getglobal id, pos -> patch_int buff pos (slot_for_getglobal id)
      | Reloc_setglobal id, pos -> patch_int buff pos (slot_for_setglobal id)
      | Reloc_primitive name, pos -> patch_int buff pos (of_prim name))
    patchlist
...
let rec transl_const = function
  ...
  | Const_block (tag, fields) ->
      ...
      List.iter
        (fun c ->
          Obj.set_field block !pos (transl_const c);
          incr pos)
        fields;
      block
  | Const_float_array fields ->
      ...
      List.iteri
        (fun i f -> Array.Floatarray.set res i (float_of_string f))
        fields;
      Obj.repr res
...
let initial_global_table () =
  ...
  List.iter (fun (slot, cst) -> glob.(slot) <- transl_const cst) !literal_table;
  literal_table := [];
  glob

(* Save the table of globals *)

let output_global_map oc = output_value oc !global_table

let data_global_map () = Obj.repr !global_table
...
let read_sections () =
  try
    ...
    {
      read_string = (fun name : string -> Obj.magic (List.assoc name sections));
      read_struct = (fun name -> List.assoc name sections);
      close_reader = (fun () -> ());
    }
  with Not_found ->
...
let get_global_value id = (Meta.global_data ()).(slot_for_getglobal id)
...
let defined_globals patchlist =
  List.fold_left
    (fun accu rel ->
      match rel with Reloc_setglobal id, _pos -> id :: accu | _ -> accu)
    [] patchlist

let required_globals patchlist =
  List.fold_left
    (fun accu rel ->
      match rel with Reloc_getglobal id, _pos -> id :: accu | _ -> accu)
    [] patchlist
...
let check_global_initialized patchlist =
  ...
  let check_reference = function
    | Reloc_getglobal id, _pos ->
        if
          (not (List.mem id defined_globals))
...
let hide_additions (st : global_map) =
  if st.cnt > !global_table.cnt then fatal_error "Symtable.hide_additions";
  global_table := { GlobalMap.cnt = !global_table.cnt; tbl = st.tbl }
...
let iter_global_map f (gmap : global_map) = Ident.Map.iter f gmap.tbl

let is_defined_in_global_map (gmap : global_map) id = Ident.Map.mem id gmap.tbl
...
let report_error ppf = function
  | Undefined_global s -> fprintf ppf "Reference to undefined global `%s'" s
  | Unavailable_primitive s ->
      fprintf ppf "The external function `%s' is not available" s
  | Wrong_vm s -> fprintf ppf "Cannot find or execute the runtime system %s" s
