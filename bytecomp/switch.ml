...
*)
    ...
    with Not_found ->
      let r =
        ...
        | _ ->
            if lcases < !cut
            then enum top cases
            else if lcases < !more_cut
            then heuristic cases
            else divide cases
      in
      Hashtbl.add t key r; r
  ...
  let make_if_in ctx l d mk_ifso mk_ifno =
    match l with
    | 0 ->
        do_make_if_in (Arg.make_const d) ctx.arg (mk_ifso ctx) (mk_ifno ctx)
    | _ ->
        Arg.bind (Arg.make_offset ctx.arg (-l)) (fun arg ->
            let ctx = {off = -l + ctx.off; arg} in
  ...
  *)
      ...
      | Sep i ->
          ...
          then make_if_ne ctx.arg 0 (c_test ctx right) (c_test ctx left)
  ...
  *)
  ...
  let make_switch loc {cases; actions} i j =
    ...
    let get_index act =
      ...
      with Not_found ->
        let i = !index in
        incr index; Hashtbl.add t act i; i
    in
    for k = i to j do
