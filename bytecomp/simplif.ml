...
let rec eliminate_ref id = function
  ...
  | Lletrec (idel, e2) ->
      Lletrec
        ...
        , eliminate_ref id e2 )
  | Lprim (Pfield 0, [Lvar v], _) when Ident.same v id -> Lvar id
  ...
  | Lswitch (e, sw, loc) ->
      Lswitch
        ...
        , { sw_numconsts = sw.sw_numconsts
          ...
          ; sw_failaction = Misc.may_map (eliminate_ref id) sw.sw_failaction
          }
        , loc )
  ...
  | Lifthenelse (e1, e2, e3) ->
      Lifthenelse
        (eliminate_ref id e1, eliminate_ref id e2, eliminate_ref id e3)
  | Lsequence (e1, e2) ->
...
let simplify_exits lam =
  ...
  let rec count = function
    ...
    | Lapply ap ->
        count ap.ap_func;
        List.iter count ap.ap_args
    | Lfunction {body} -> count body
    | Llet (_str, _kind, _v, l1, l2) -> count l2; count l1
    ...
    | Lstringswitch (l, sw, d, _) -> (
        ...
        | Some d -> (
          ...
          | _ ->
              count d;
              count d (* default will get replicated *) ) )
    ...
    | Lstaticcatch (l1, (i, _), l2) ->
        ...
        if (get_exit i).count > 0 then count l2
    | Ltrywith (l1, _v, l2) ->
  ...
  *)
  ...
  let rec simplif = function
    ...
    | Lprim (p, ll, loc) -> (
        ...
        | Prevapply, [x; Lapply ap] | Prevapply, [x; Levent (Lapply ap, _)]
          ->
        ...
        | Pdirapply, [Lapply ap; x] | Pdirapply, [Levent (Lapply ap, _); x]
          ->
            Lapply {ap with ap_args = ap.ap_args @ [x]; ap_loc = loc}
        | Pdirapply, [f; x] ->
            Lapply
    ...
    | Lstaticcatch (l1, (i, xs), l2) ->
        ...
        if count = 0
        then (* Discard staticcatch: not matching exit *)
          simplif l1
...
let beta_reduce params body args =
  List.fold_left2
    (fun l (param, kind) arg -> Llet (Strict, kind, param, arg, l))
    body
    params
    args
...
let simplify_lets lam =
  ...
  and bind_var bv v =
    let r = ref 0 in
    Hashtbl.add occ v r; Ident.Map.add v r bv
  ...
  let rec count bv = function
    ...
    | Lapply
        {ap_func = Lfunction {kind = Curried; params; body}; ap_args = args}
      when optimize && List.length params = List.length args ->
        count bv (beta_reduce params body args)
    | Lapply
        { ap_func = Lfunction {kind = Tupled; params; body}
  ...
  let rec simplif = function
    ...
    | Lfunction {kind; params; return = return1; body = l; attr; loc} -> (
      ...
      | Lfunction
        ...
        when kind = Curried && optimize ->
          ...
          Lfunction
            {kind; params = params @ params'; return; body; attr; loc}
    ...
    | Llet (str, kind, v, l1, l2) ->
        mklet str kind v (simplif l1) (simplif l2)
    | Lletrec (bindings, body) ->
        Lletrec
          (List.map (fun (v, l) -> v, simplif l) bindings, simplif body)
...
let is_tail_native_heuristic : (int -> bool) ref = ref (fun _ -> true)
...
let rec emit_tail_infos is_tail lambda =
  ...
  | Lapply ap ->
      ...
      then Stypes.record (Stypes.An_call (ap.ap_loc, call_kind ap.ap_args))
  ...
  | Lletrec (bindings, body) ->
      ...
      emit_tail_infos is_tail body
  | Lprim (Pidentity, [arg], _) -> emit_tail_infos is_tail arg
...
let split_default_wrapper ~id:fun_id ~kind ~params ~return ~body ~attr ~loc
  ...
  let rec aux map = function
    | Llet (Strict, k, id, (Lifthenelse (Lvar optparam, _, _) as def), rest)
      when Ident.name optparam = "*opt*"
           && List.mem_assoc optparam params
    ...
    | body ->
        ...
        List.iter
          (fun (id, _) -> if Ident.Set.mem id fv then raise Exit)
          map;
        ...
        let subst =
          List.fold_left2
            (fun s id new_id -> Ident.Map.add id new_id s)
            Ident.Map.empty
            inner_params
            new_ids
        in
  ...
  with Exit -> [fun_id, Lfunction {kind; params; return; body; attr; loc}]
