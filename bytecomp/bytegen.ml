...
let preserve_tailcall_for_prim = function
  ...
  | Pcompare_ints | Pcompare_floats | Pcompare_bints _ | Pbyteslength
  | Pbytesrefu | Pbytessetu | Pbytesrefs | Pbytessets | Pmakearray _
  | Pduparray _ | Parraylength _ | Parrayrefu _ | Parraysetu _ | Parrayrefs _
  | Parraysets _ | Pisint | Pisout | Pbintofint _ | Pintofbint _ | Pcvtbint _
...
let rec size_of_lambda env = function
  | Lvar id -> ( try Ident.find_same id env with Not_found -> RHS_nonrec)
  ...
  | Lletrec (bindings, body)
    when List.for_all (function _, Lfunction _ -> true | _ -> false) bindings ->
      ...
      let env =
        List.fold_right
          (fun (id, offset) env ->
            Ident.add id (RHS_infix { blocksize; offset }) env)
          offsets env
      in
...
let rec comp_expr env exp sz cont =
  ...
  | Lapply { ap_func = func; ap_args = args } ->
      ...
      if is_tailcall cont then
        ...
        :: comp_args env args (sz + 3)
             (Kpush
             :: comp_expr env func (sz + 3 + nargs) (Kapply nargs :: cont1))
  | Lsend (kind, met, obj, args, _) ->
      assert (kind <> Cached);
      let nargs = List.length args + 1 in
      let getmethod, args' =
        if kind = Self then (Kgetmethod, met :: obj :: args)
        else
          match met with
  ...
  | Lletrec (decl, body) ->
      ...
      if List.for_all (function _, Lfunction _ -> true | _ -> false) decl then
        ...
        let decl_size =
          List.map
            (fun (id, exp) -> (id, exp, size_of_lambda Ident.empty exp))
            decl
        in
        let rec comp_init new_env sz = function
          ...
          | (id, _exp, RHS_infix { blocksize; offset }) :: rem ->
              Kconst (Const_base (Const_int offset)) :: Kpush
              :: Kconst (Const_base (Const_int blocksize))
              :: Kccall ("caml_alloc_dummy_infix", 2)
              :: Kpush
              :: comp_init (add_var id (sz + 1) new_env) (sz + 1) rem
          | (id, _exp, RHS_function (blocksize, arity)) :: rem ->
              Kconst (Const_base (Const_int arity)) :: Kpush
              :: Kconst (Const_base (Const_int blocksize))
        ...
        and comp_nonrec new_env sz i = function
          ...
          | ( _id,
              _exp,
              (RHS_block _ | RHS_infix _ | RHS_floatblock _ | RHS_function _) )
            :: rem ->
  ...
  | Lprim (Psequor, [ exp1; exp2 ], _) -> (
      ...
      | _ ->
          let lbl, cont1 = label_code cont in
          comp_expr env exp1 sz
            (Kstrictbranchif lbl :: comp_expr env exp2 sz cont1))
  ...
  | Lprim (Pmakearray (kind, _), args, loc) -> (
      ...
      | Pgenarray ->
          if args = [] then Kmakeblock (0, 0) :: cont
          else
            comp_args env args sz
...
let comp_function tc cont =
  ...
  if arity > 1 then Krestart :: Klabel tc.label :: Kgrab (arity - 1) :: cont
