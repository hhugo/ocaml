...
let preserve_tailcall_for_prim =
  function
  | Popaque | Psequor | Psequand -> true
  | Pbytes_to_string
  | Pbytes_of_string
  | Pignore
  | Pgetglobal _
...
let rec check_recordwith_updates id e =
  match e with
  | Lsequence
      (Lprim ((Psetfield _ | Psetfloatfield _), [ Lvar id2; _ ], _), cont)
    ->
    id2 = id && check_recordwith_updates id cont
  | Lvar id2 -> id2 = id
  | _ -> false

let rec size_of_lambda env =
  function
  | Lvar id ->
  ...
  | Lfunction { params } as funct ->
    RHS_function
      (2 + Ident.Set.cardinal (free_variables funct), List.length params)
  ...
  | Lletrec (bindings, body)
    when List.for_all (function _, Lfunction _ -> true | _ -> false) bindings ->
    (* let rec of functions *)
    let fv =
      Ident.Set.elements (free_variables (Lletrec (bindings, lambda_unit)))
    in
    ...
    let env =
      List.fold_right
        (fun (id, offset) env ->
           Ident.add id (RHS_infix { blocksize; offset }) env) offsets env
    in
    size_of_lambda env body
  | Lletrec (bindings, body) ->
    let env =
      List.fold_right
        (fun (id, e) env -> Ident.add id (size_of_lambda env e) env) bindings
        env
    in
    size_of_lambda env body
  ...
  | Lprim (Pduprecord ((Record_regular | Record_inlined _), size), _, _) ->
    RHS_block size
  | Lprim (Pduprecord (Record_unboxed _, _), _, _) -> assert false
  | Lprim (Pduprecord (Record_extension _, size), _, _) -> RHS_block (size + 1)
  ...
  | Event_after _ ->
    ...
    | Kpush :: Kevent ({ ev_repr = Event_none } as ev') :: c ->
      ...
      | Event_return _ ->
        ...
        and ev' = copy_event ev' ev'.ev_kind ev'.ev_info (Event_child repr)
        in
...
let find_raise_label i =
  try List.assoc i !sz_static_raises
  with
  | Not_found ->
    Misc.fatal_error
...
let code_as_jump l sz =
  match l with
  | Lstaticraise (i, []) ->
    let (label, size, tb) = find_raise_label i in
    if sz = size && tb == !try_blocks then Some label else None
  | _ -> None
...
let rec comp_expr env exp sz cont =
    ...
    if is_tailcall cont then
      ...
      Kpush_retaddr lbl ::
        comp_args env args (sz + 3)
          (Kpush :: comp_expr env func (sz + 3 + nargs) (Kapply nargs :: cont1))
  | Lsend (kind, met, obj, args, _) ->
    assert (kind <> Cached);
    let nargs = List.length args + 1 in
    let (getmethod, args') =
      if kind = Self then
        Kgetmethod, met :: obj :: args
      else
        match met with
    ...
    if is_tailcall cont then
      ...
      comp_args env args' sz (getmethod :: Kapply nargs :: cont)
    else
      let (lbl, cont1) = label_code cont in
      Kpush_retaddr lbl ::
        comp_args env args' (sz + 3) (getmethod :: Kapply nargs :: cont1)
  ...
  | Llet (_, _k, id, arg, body) | Lmutlet (_k, id, arg, body) ->
    comp_expr env arg sz
      (Kpush ::
         comp_expr (add_var id (sz + 1) env) body (sz + 1) (add_pop 1 cont))
  | Lletrec (decl, body) ->
    let ndecl = List.length decl in
    if List.for_all (function _, Lfunction _ -> true | _ -> false) decl then
      (* let rec of functions *)
      let fv = Ident.Set.elements (free_variables (Lletrec (decl, lambda_unit)))
      in
      let rec_idents = List.map (fun (id, _lam) -> id) decl in
      let rec comp_fun pos =
        function
        | [] -> []
      ...
      and comp_rec new_env sz i =
        ...
        | (_id,
          ...
          comp_expr new_env exp sz
            (Kpush ::
               Kacc i ::
                 Kccall ("caml_update_dummy", 2) ::
                   comp_rec new_env sz (i - 1) rem)
        | (_id, _exp, RHS_nonrec) :: rem -> comp_rec new_env sz (i - 1) rem
      in
      comp_init env sz decl_size
  | Lprim (Popaque, [ arg ], _) -> comp_expr env arg sz cont
  | Lprim (Pignore, [ arg ], _) -> comp_expr env arg sz (add_const_unit cont)
  | Lprim (Pnot, [ arg ], _) ->
    let newcont =
      match cont with
  ...
  | Lprim (Psequand, [ exp1; exp2 ], _) ->
    begin match cont with
    | Kbranchifnot lbl :: _ ->
      comp_expr env exp1 sz (Kbranchifnot lbl :: comp_expr env exp2 sz cont)
    | Kbranchif lbl :: cont1 ->
      let (lbl2, cont2) = label_code cont1 in
      comp_expr env exp1 sz
        (Kbranchifnot lbl2 :: comp_expr env exp2 sz (Kbranchif lbl :: cont2))
    | _ ->
      let (lbl, cont1) = label_code cont in
      comp_expr env exp1 sz
        (Kstrictbranchifnot lbl :: comp_expr env exp2 sz cont1)
    end
  | Lprim (Psequor, [ exp1; exp2 ], _) ->
    begin match cont with
    | Kbranchif lbl :: _ ->
      comp_expr env exp1 sz (Kbranchif lbl :: comp_expr env exp2 sz cont)
    | Kbranchifnot lbl :: cont1 ->
      let (lbl2, cont2) = label_code cont1 in
      comp_expr env exp1 sz
        (Kbranchif lbl2 :: comp_expr env exp2 sz (Kbranchifnot lbl :: cont2))
    | _ ->
      let (lbl, cont1) = label_code cont in
      comp_expr env exp1 sz (Kstrictbranchif lbl :: comp_expr env exp2 sz cont1)
    end
  ...
  | Lprim (p, args, _) -> comp_args env args sz (comp_primitive p args :: cont)
    ...
    let r =
      if nvars <> 1 then
        ...
        let (lbl_handler, cont2) =
          label_code
            (comp_expr (add_vars vars (sz + 1) env) handler (sz + nvars)
               (add_pop nvars cont1))
        in
        (push_static_raise i lbl_handler (sz + nvars);
         push_dummies nvars
           (comp_expr env body (sz + nvars) (add_pop nvars (branch1 :: cont2))))
      else (* small optimization for nvars = 1 *)
        let var =
          match vars with
          | [ var ] -> var
          | _ -> assert false
        ...
        let (lbl_handler, cont2) =
          label_code
            (Kpush ::
               comp_expr (add_var var (sz + 1) env) handler (sz + 1)
                 (add_pop 1 cont1))
        in
    ...
    let body_cont =
      Kpoptrap ::
        branch1 ::
          Klabel lbl_handler ::
            Kpush ::
              comp_expr (add_var id (sz + 1) env) handler (sz + 1)
                (add_pop 1 cont1)
    in
  ...
  | Lsequence (exp1, exp2) -> comp_expr env exp1 sz (comp_expr env exp2 sz cont)
  | Lwhile (cond, body) ->
  ...
  | Lswitch (arg, sw, _loc) ->
    ...
    and act_blocks = Array.make sw.sw_numblocks 0
    in
    ...
    List.iter (fun (n, act) -> act_consts.(n) <- store.act_store () act)
      sw.sw_consts;
    List.iter (fun (n, act) -> act_blocks.(n) <- store.act_store () act)
      sw.sw_blocks;
...
*)
  ...
  | Lassign (id, expr) ->
    (try
     ...
     with
     | Not_found -> fatal_error "Bytegen.comp_expr: assign")
  | Levent (lam, lev) ->
    let ev_defname =
      match lev.lev_loc with
      | Loc_unknown -> "??"
    ...
    | Lev_module_definition _ -> comp_expr env lam sz cont
    end
  | Lifused (_, exp) -> comp_expr env exp sz cont

(* Compile a list of arguments [e1; ...; eN] to a primitive operation.
   The values of eN ... e2 are pushed on the stack, e2 at top of stack,
   then e3, then ... The value of e1 is left in the accumulator. *)
and comp_args env argl sz cont = comp_expr_list env (List.rev argl) sz cont
...
and comp_expr_list_assign env exprl sz pos cont =
  match exprl with
  | [] -> cont
  | exp :: rem ->
...
and comp_binary_test env cond ifso ifnot sz cont =
  let cont_cond =
    if ifnot = Lconst const_unit then
      ...
      | _ ->
        begin match code_as_jump ifnot sz with
        | Some label ->
...
let comp_block env exp sz cont =
  ...
  if used_safe > Config.stack_threshold then
    Kconst (Const_base (Const_int used_safe)) ::
      Kccall ("caml_ensure_stack_capacity", 1) :: code
  else
    code
...
let comp_function tc cont =
  ...
  let rec positions pos delta =
    function
    | [] -> Ident.empty
    | id :: rem -> Ident.add id pos (positions (pos + delta) delta rem)
  in
  let env =
    {
      ce_stack = positions arity (-1) tc.params;
      ce_heap = positions (3 * (tc.num_defs - tc.rec_pos) - 1) 1 tc.free_vars;
      ce_rec = positions ((-3) * tc.rec_pos) 3 tc.rec_vars
    }
  in
...
let comp_remainder cont =
  let c = ref cont in
  (try while true do c := comp_function (Stack.pop functions_to_compile) !c done
   with
   | Stack.Empty -> ());
  !c
...
let compile_implementation modulename expr =
  ...
  if Stack.length functions_to_compile > 0 then
