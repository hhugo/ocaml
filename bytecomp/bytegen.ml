...
let add_const_unit = function
  | (Kacc _ | Kconst _ | Kgetglobal _ | Kpush_retaddr _) :: _ as cont ->
      cont
...
let rec size_of_lambda env = function
  ...
  | Lfunction {params} as funct ->
      RHS_function
        (1 + Ident.Set.cardinal (free_variables funct), List.length params)
...
let push_static_raise i lbl_handler sz =
  sz_static_raises :=
    (i, (lbl_handler, sz, !try_blocks)) :: !sz_static_raises
...
let rec comp_expr env exp sz cont =
  ...
  | Lsend (kind, met, obj, args, _) ->
      ...
      then comp_args env args' sz (getmethod :: Kapply nargs :: cont)
      else
        let lbl, cont1 = label_code cont in
        Kpush_retaddr lbl
  ...
  | Lletrec (decl, body) ->
      ...
      if List.for_all (function _, Lfunction _ -> true | _ -> false) decl
      then
  ...
  | Lprim ((Pidentity | Popaque), [arg], _) -> comp_expr env arg sz cont
  | Lprim (Pignore, [arg], _) -> comp_expr env arg sz (add_const_unit cont)
  | Lprim (Pdirapply, [func; arg], loc) | Lprim (Prevapply, [arg; func], loc)
    ->
      let exp =
        Lapply
  ...
  | Lprim (Psequand, [exp1; exp2], _) -> (
    ...
    | Kbranchifnot lbl :: _ ->
        comp_expr
          ...
          exp1
          sz
          (Kbranchifnot lbl :: comp_expr env exp2 sz cont)
    | Kbranchif lbl :: cont1 ->
  ...
  | Lprim
      ...
      comp_expr
        env
        (Lprim (Pmakearray (kind, mutability), args, loc))
        sz
        cont
...
*)
      ...
      for i = Array.length acts - 1 downto 0 do
        let lbl, c1 =
          label_code (comp_expr env acts.(i) sz (branch :: !c))
        in
        lbls.(i) <- lbl;
  ...
  | Levent (lam, lev) -> (
      ...
      | Lev_pseudo ->
          ...
          add_event ev c
      | Lev_after _ when is_tailcall cont ->
...
and comp_expr_list_assign env exprl sz pos cont =
  ...
  | exp :: rem ->
      comp_expr
        ...
        sz
        ( Kassign (sz - pos)
        :: comp_expr_list_assign env rem sz (pos - 1) cont )
    ...
    then
      ...
      | _ -> (
        ...
        | _ ->
            ...
            let lbl_not, cont2 =
              label_code (comp_expr env ifnot sz cont1)
            in
...
let comp_function tc cont =
  ...
  let env =
    ...
    ; ce_heap =
        positions ((2 * (tc.num_defs - tc.rec_pos)) - 1) 1 tc.free_vars
