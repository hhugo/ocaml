...
let primitives_table =
  create_hashtable
    ...
    ; ( "%caml_ba_ref_1"
      , Primitive
          ( Pbigarrayref
              (false, 1, Pbigarray_unknown, Pbigarray_unknown_layout)
          , 2 ) )
    ; ( "%caml_ba_ref_2"
      , Primitive
          ( Pbigarrayref
              (false, 2, Pbigarray_unknown, Pbigarray_unknown_layout)
          , 3 ) )
    ; ( "%caml_ba_ref_3"
      , Primitive
          ( Pbigarrayref
              (false, 3, Pbigarray_unknown, Pbigarray_unknown_layout)
          , 4 ) )
    ; ( "%caml_ba_set_1"
      , Primitive
          ( Pbigarrayset
    ...
    ; ( "%caml_ba_unsafe_ref_1"
      , Primitive
          ( Pbigarrayref
              (true, 1, Pbigarray_unknown, Pbigarray_unknown_layout)
          , 2 ) )
    ; ( "%caml_ba_unsafe_ref_2"
      , Primitive
          ( Pbigarrayref
              (true, 2, Pbigarray_unknown, Pbigarray_unknown_layout)
          , 3 ) )
    ; ( "%caml_ba_unsafe_ref_3"
      , Primitive
          ( Pbigarrayref
              (true, 3, Pbigarray_unknown, Pbigarray_unknown_layout)
          , 4 ) )
...
let lookup_primitive loc p =
  ...
  | prim -> prim
  | exception Not_found ->
      if String.length p.prim_name > 0 && p.prim_name.[0] = '%'
      then raise (Error (loc, Unknown_builtin_primitive p.prim_name));
      External p
...
let specialize_primitive env ty ~has_constant_constructor prim =
  ...
  | ( Primitive
        ( Pbigarrayref
            (unsafe, n, Pbigarray_unknown, Pbigarray_unknown_layout)
        , arity )
    , p1 :: _ ) -> (
  ...
  | Primitive (Pmakeblock (tag, mut, None), arity), fields ->
      ...
      if useful
      then Some (Primitive (Pmakeblock (tag, mut, Some shape), arity))
      else None
...
let lambda_of_loc kind loc =
  ...
  | Loc_MODULE ->
      ...
      let module_name =
        if name = "" then "//" ^ filename ^ "//" else name
      in
...
let lambda_of_prim prim_name prim loc args arg_exps =
  ...
  | Lazy_force, [arg] -> Matching.inline_lazy_force arg Location.none
  ...
  | ( ( Raise _ | Raise_with_backtrace | Lazy_force | Loc _ | Primitive _
      | Comparison _ | Send | Send_self | Send_cache )
    , _ ) ->
...
let check_primitive_arity loc p =
  ...
  in
  if not ok
  then raise (Error (loc, Wrong_arity_builtin_primitive p.prim_name))
...
let transl_primitive_application loc p env ty path exp args arg_exps =
  ...
  let lam =
    ...
    then match exp with None -> lam | Some exp -> event_after exp lam
    else lam
  in
  lam
