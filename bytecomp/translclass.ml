...
let bind_super tbl (vals, meths) cl_init =
  transl_vals
    ...
    vals
    (List.fold_right
       (fun (_nm, id, def) rem -> Llet (StrictOpt, Pgenval, id, def, rem))
       meths
       cl_init)
            ...
            List.fold_right
              (fun field (inh_init, obj_init, has_init) ->
                ...
                | Tcf_val (_, _, id, Tcfk_concrete (_, exp), _) ->
                    ( inh_init
                    , lsequence (set_inst_var obj id exp) obj_init
                    , has_init )
...
let rec build_object_init_0 cl_table params cl copy_env subst_env top ids =
  ...
  | _ ->
      ...
      let (_, inh_init), obj_init =
        build_object_init cl_table obj params (envs, []) copy_env cl
      in
      let obj_init =
  ...
  then transl_vals tbl true Strict vals cl_init
    ...
    Llet
      ...
      , List.fold_right
          (fun (_lab, id) lam ->
            decr i;
            Llet (StrictOpt, Pgenval, id, lfield ids !i, lam) )
          (methl @ vals)
          cl_init )
  ...
  | _ ->
      lsequence
        (mkappl
           ...
           , [ Lvar tbl
             ; Lprim
                 (Pmakeblock (0, Immutable, None), methods, Location.none)
             ] ))
        lam
...
let rec build_class_init cla cstr super inh_init cl_init msubst top cl =
  ...
  | Tcl_ident _ -> (
    ...
    | (_, path_lam, obj_init) :: inh_init ->
        ...
        , Llet
            ...
            , obj_init
            , mkappl
                ( Lprim (Pfield 1, [path_lam], Location.none)
                , Lvar cla
                  ::
                  ( if top
                  then [Lprim (Pfield 3, [path_lam], Location.none)]
                  else [] ) )
        ...
        List.fold_right
          (fun field (inh_init, cl_init, methods, values) ->
            ...
            | Tcf_method (name, _, Tcfk_concrete (_, exp)) ->
                ...
                ( inh_init
                , cl_init
                , (Lvar (Meths.find name.txt str.cstr_meths) :: met_code)
                  @ methods
                , values )
            | Tcf_initializer exp ->
                ( inh_init
  ...
  | Tcl_constraint (cl, _, vals, meths, concr_meths) -> (
      let virt_meths =
        List.filter (fun lab -> not (Concr.mem lab concr_meths)) meths
      in
      ...
      | _ ->
          ...
          then core cl_init
            ...
            let inh_init, cl_init =
              core
                (Lsequence (mkappl (oo_prim "widen", [Lvar cla]), cl_init))
            in
            ( inh_init
            , Lsequence (mkappl (oo_prim "narrow", narrow_args), cl_init) )
      )
  | Tcl_open (_, cl) ->
      build_class_init cla cstr super inh_init cl_init msubst top cl
...
let rec transl_class_rebind obj_init cl vf =
  ...
  | Tcl_ident (path, _, _) ->
      ...
      then
        try
          if (Env.find_class path cl.cl_env).cty_new = None then raise Exit
...
let transl_class_rebind cl vf =
  try
    ...
    let _, path_lam, obj_init' =
      transl_class_rebind_0 self obj_init0 cl vf
    in
...
let free_methods l =
  ...
  let rec free l =
    ...
    | Lfunction {params} ->
        List.iter
          (fun (param, _) -> fv := Ident.Set.remove param !fv)
          params
    | Llet (_str, _k, id, _arg, _body) -> fv := Ident.Set.remove id !fv
    | Lletrec (decl, _body) ->
    ...
    | Lassign _ | Lvar _ | Lconst _ | Lapply _ | Lprim _ | Lswitch _
     |Lstringswitch _ | Lstaticraise _ | Lifthenelse _ | Lsequence _
     |Lwhile _ | Levent _ | Lifused _ ->
        ()
  in
  free l; !fv
...
let transl_class ids cl_id pub_meths cl vflag =
  ...
  then rebind
        ...
        -> (
          ...
          let body' =
            ...
            then body
              ...
              Lambda.subst
                no_env_update
                (subst env body 0 new_ids_meths)
                body
          in
          try
    ...
    and env1' = Ident.create_local "env'" in
    let copy_env self =
      if top
      then lambda_unit
      else
    ...
    let inh_init, obj_init =
      build_object_init_0 cla [] cl copy_env subst_env top ids
    in
    ...
    if top && ids = []
    then llets (ltable cla (ldirect obj_init))
    else
      ...
      then llets (lbody_virt lambda_unit)
      else
        ...
        and linh_envs =
          List.map
            (fun (_, path_lam, _) ->
              Lprim (Pfield 3, [path_lam], Location.none) )
            (List.rev inh_init)
        in
        let make_envs lam =
          Llet
        ...
        let lcheck_cache =
         ...
         so that the program's behaviour does not change between runs *)
            lupdate_cache
          else Lifthenelse (lfield cached 0, lambda_unit, lupdate_cache)
        in
        llets
          (lcache
             (Lsequence
                ...
                , make_envs
                    ( if ids = []
                    then mkappl (lfield cached 0, [lenvs])
                    else
                      Lprim
                        ( Pmakeblock (0, Immutable, None)
                        , ( if concrete
