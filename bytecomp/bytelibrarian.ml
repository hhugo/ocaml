...
let copy_object_file oc name =
  let file_name =
    ...
    with
    | Not_found -> raise (Error (File_not_found name))
  in
...
let create_archive file_list lib_name =
  ...
  Misc.try_finally ~always:(fun () -> close_out outchan)
    ...
    (fun () ->
       ...
       let units = List.flatten (List.map (copy_object_file outchan) file_list)
       in
       let toc =
         {
           lib_units = units;
           ...
           lib_dllibs = !Clflags.dllibs @ !lib_dllibs
         }
       in
       let pos_toc = pos_out outchan in
       Emitcode.marshal_to_channel_with_possibly_32bit_compat ~filename:lib_name
         ~kind:"bytecode library" outchan toc;
