...
let primitive ppf = function
  ...
  | Pmakearray (k, Immutable) ->
      fprintf ppf "makearray_imm[%s]" (array_kind k)
...
let apply_tailcall_attribute ppf tailcall =
  if tailcall then fprintf ppf " @@tailcall"
...
let rec lam ppf = function
  ...
  | Lapply ap ->
      ...
      fprintf
        ...
        lams
        ap.ap_args
        apply_tailcall_attribute
        ap.ap_should_be_tailcall
        apply_inlined_attribute
        ap.ap_inlined
  ...
  | Lfunction {kind; params; return; body; attr} ->
      let pr_params ppf params =
        ...
        | Curried ->
            List.iter
              (fun (param, k) ->
                fprintf ppf "@ %a%a" Ident.print param value_kind k )
              params
        | Tupled ->
            fprintf ppf " (";
