  ...
  | Text_decl _ ->
      Lprim
        ...
        , [ Lconst (Const_base (Const_string (name, None)))
          ; Lprim
              (prim_fresh_oo_id, [Lconst (Const_base (Const_int 0))], loc)
          ]
        , loc )
...
let rec push_defaults loc bindings cases partial =
  ...
  | [ { c_lhs = pat
      ; c_guard = None
      ; c_rhs =
          {exp_desc = Texp_function {arg_label; param; cases; partial}} as
          exp } ] ->
  ...
  | [ { c_lhs = pat
      ...
      ; c_rhs =
          ...
          ; exp_desc =
              Texp_letmodule
                (id, name, pres, mexpr, ({exp_desc = Texp_function _} as e2))
          } } ] ->
      push_defaults
        loc
        (Bind_module (id, name, pres, mexpr) :: bindings)
        [{c_lhs = pat; c_guard = None; c_rhs = e2}]
        partial
  | [case] ->
      let exp =
...
and transl_exp0 e =
  ...
  | Texp_function {arg_label = _; param; cases; partial} ->
      let (kind, params, return), body =
        event_function e (function repr ->
            let pl = push_defaults e.exp_loc [] cases partial in
            let return_kind =
              function_return_value_kind e.exp_env e.exp_type
            in
            transl_function
              e.exp_loc
              return_kind
  ...
  | Texp_construct (_, cstr, args) -> (
      ...
      then match ll with [x] -> x | _ -> assert false
        ...
        | Cstr_constant n -> Lconst (Const_pointer n)
  ...
  | Texp_variant (l, arg) -> (
      ...
      | Some arg -> (
          ...
          try
            Lconst
              (Const_block
                 (0, [Const_base (Const_int tag); extract_constant lam]))
          with Not_constant ->
            Lprim
              ( Pmakeblock (0, Immutable, None)
  ...
  | Texp_ifthenelse (cond, ifso, None) ->
      Lifthenelse
        (transl_exp cond, event_before ifso (transl_exp ifso), lambda_unit)
      ...
      Llet
        ...
        , List.fold_right
            (fun (path, _, expr) rem ->
              let var = transl_value_path e.exp_loc e.exp_env path in
              Lsequence
                (transl_setinstvar Location.none (Lvar cpy) var expr, rem)
              )
            modifs
            (Lvar cpy) )
  ...
  | Texp_letmodule (_, _, Mp_absent, _, body) -> transl_exp body
            ...
            List.fold_left
              (fun (body, pos) id ->
                ( Llet
                    ...
                    , Pgenval
                    , id
                    , Lprim (Pfield pos, [Lvar oid], od.open_loc)
                    , body )
                , pos + 1 ) )
              (transl_exp e, 0)
...
and transl_tupled_cases patl_expr_list =
  let patl_expr_list =
    List.filter
      (fun (_, _, e) -> e.exp_desc <> Texp_unreachable)
      patl_expr_list
  in
  List.map
    (fun (patl, guard, expr) -> patl, transl_guard guard expr)
    patl_expr_list
...
and transl_apply ?(should_be_tailcall = false) ?(inlined = Default_inline)
  ...
  let lapply funct args =
    ...
    | lexp ->
        Lapply
          { ap_should_be_tailcall = should_be_tailcall
          ; ap_loc = loc
          ; ap_func = lexp
          ; ap_args = args
          ; ap_inlined = inlined
  ...
  let rec build_apply lam args = function
    | (None, optional) :: l ->
        ...
        in
        let args, args' =
          if List.for_all (fun (_, opt) -> opt) args
          then [], args
          else args, []
        in
        let lam =
...
and transl_function loc return untuplify_fn repr partial (param : Ident.t)
    cases =
  match cases with
  | [ { c_lhs = pat
      ; c_guard = None
...
and transl_let rec_flag pat_expr_list =
  ...
  | Nonrecursive ->
      let rec transl = function
        | [] -> fun body -> body
        | {vb_pat = pat; vb_expr = expr; vb_attributes = attr; vb_loc}
          :: rem ->
    ...
    let lam =
      ...
      with Not_constant -> (
        ...
        | Record_extension path ->
            ...
            Lprim
              (Pmakeblock (0, mut, Some (Pgenval :: shape)), slot :: ll, loc) )
    in
  ...
  let rewrite_case ((val_cases, exn_cases, static_handlers) as acc)
    ...
    then acc
      ...
      | Some pv, Some pe ->
          ...
          let ids_kinds =
            List.map
              (fun (id, _, ty) -> id, Typeopt.value_kind pv.pat_env ty)
              ids_full
          in
...
and transl_letop loc env let_ ands param case partial =
  ...
  let func =
    ...
    let (kind, params, return), body =
      event_function case.c_rhs (function repr ->
          transl_function
            case.c_rhs.exp_loc
            return_kind
            !Clflags.native_code
