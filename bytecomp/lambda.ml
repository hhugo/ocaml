...
let equal_inline_attribute x y =
  ...
  | (Always_inline | Never_inline | Unroll _ | Default_inline), _ -> false
...
let make_key e =
  ...
  let rec tr_rec env e =
    ...
    | Lsend (m, e1, e2, es, _loc) ->
        Lsend
          (m, tr_rec env e1, tr_rec env e2, tr_recs env es, Location.none)
...
let shallow_iter ~tail ~non_tail:f = function
  | Lvar _ | Lconst _ -> ()
  | Lapply {ap_func = fn; ap_args = args} -> f fn; List.iter f args
  | Lfunction {body} -> f body
  | Llet (_str, _k, _id, arg, body) -> f arg; tail body
  | Lletrec (decl, body) ->
      tail body;
      List.iter (fun (_id, exp) -> f exp) decl
  | Lprim (Pidentity, [l], _) -> tail l
  ...
  | Lstaticcatch (e1, _, e2) -> tail e1; tail e2
  | Ltrywith (e1, _, e2) -> f e1; tail e2
  | Lifthenelse (e1, e2, e3) -> f e1; tail e2; tail e3
  | Lsequence (e1, e2) -> f e1; tail e2
...
let rec free_variables = function
  ...
  | Lstringswitch (arg, cases, default, _) -> (
      let set =
        free_variables_list (free_variables arg) (List.map snd cases)
      in
      match default with
      | None -> set
...
let subst update_env s lam =
  let rec subst s lam =
    ...
    match lam with
    | Lvar id as l -> ( try Ident.Map.find id s with Not_found -> l )
    | Lconst _ as l -> l
    | Lapply ap ->
        Lapply
          { ap with
            ap_func = subst s ap.ap_func; ap_args = subst_list s ap.ap_args
          }
    | Lfunction lf ->
        let s =
          List.fold_right
    ...
    | Lletrec (decl, body) ->
        let s =
          List.fold_left (fun s (id, _) -> Ident.Map.remove id s) s decl
        in
    ...
    | Lswitch (arg, sw, loc) ->
        Lswitch
          ( subst s arg
          , { sw with
              sw_consts = List.map (subst_case s) sw.sw_consts
  ...
  and subst_opt s = function None -> None | Some e -> Some (subst s e) in
  subst s lam
