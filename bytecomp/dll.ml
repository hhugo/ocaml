...
external dll_sym
  : dll_handle -> string -> dll_address = "caml_dynlink_lookup_symbol"

(* returned dll_address may be Val_unit *)
external add_primitive : dll_address -> int = "caml_dynlink_add_primitive"

external get_current_dlls
  : unit -> dll_handle array = "caml_dynlink_get_current_libs"
...
let open_dll mode name =
  ...
  let fullname =
    ...
    | Not_found -> name
  in
  if not (List.mem fullname !names_of_opened_dlls) then
    let dll =
      match mode with
      | For_checking ->
        begin match Binutils.read fullname with
        | Ok t -> Checking t
        | Error err -> failwith (fullname ^ ": " ^ Binutils.error_to_string err)
        end
      | For_execution ->
        begin match dll_open mode fullname with
        | dll -> Execution dll
        | exception (Failure msg) -> failwith (fullname ^ ": " ^ msg)
        end
    in
...
let ld_conf_contents () =
  ...
  (try
     ...
     (try while true do path := input_line ic :: !path done
      with
      | End_of_file -> ());
     close_in ic
   with
...
let init_compile nostdlib =
  search_path :=
    ld_library_path_contents () @ (if nostdlib then [] else ld_conf_contents ())

(* Initialization for linking in core (dynlink or toplevel) *)
let init_toplevel dllpath =
  search_path :=
    ld_library_path_contents () @ split_dll_path dllpath @ ld_conf_contents ();
  opened_dlls :=
    List.map (fun dll -> Execution dll) (Array.to_list (get_current_dlls ()));
  names_of_opened_dlls := [];
