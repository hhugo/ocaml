...
let extract_dll_name file =
  ...
  then Filename.chop_suffix file Config.ext_dll
  else if String.length file >= 2 && String.sub file 0 2 = "-l"
  then "dll" ^ String.sub file 2 (String.length file - 2)
  else file
...
let open_dll mode name =
  ...
  let fullname =
    try
      ...
      then Filename.concat Filename.current_dir_name fullname
      else fullname
    with Not_found -> name
  in
  if not (List.mem fullname !names_of_opened_dlls)
  then
    try
      let dll = dll_open mode fullname in
      names_of_opened_dlls := fullname :: !names_of_opened_dlls;
      opened_dlls := dll :: !opened_dlls
    with Failure msg -> failwith (fullname ^ ": " ^ msg)
...
let ld_conf_contents () =
  ...
  ( try
      let ic =
        open_in (Filename.concat Config.standard_library "ld.conf")
      in
      ( try
          while true do
