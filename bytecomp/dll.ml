...
let open_dll mode name =
  ...
  if not (List.mem fullname !names_of_opened_dlls) then (
    let dll =
      match mode with
      | For_checking -> (
          match Binutils.read fullname with
          | Ok t -> Checking t
          | Error err ->
              failwith (fullname ^ ": " ^ Binutils.error_to_string err))
      | For_execution -> (
          match dll_open mode fullname with
          | dll -> Execution dll
...
let find_primitive prim_name =
  let rec find seen = function
    ...
    | (Execution dll as curr) :: rem ->
        ...
        if addr == Obj.magic () then find (curr :: seen) rem
        else (
          if seen <> [] then opened_dlls := curr :: List.rev_append seen rem;
          Some (Prim_loaded addr))
    | (Checking t as curr) :: rem ->
        if Binutils.defines_symbol t prim_name then Some Prim_exists
        else find (curr :: seen) rem
  in
  find [] !opened_dlls
