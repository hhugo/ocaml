...
let marshal_to_channel_with_possibly_32bit_compat ~filename ~kind outchan
    obj =
  try
    Marshal.to_channel
      outchan
...
let out_word b1 b2 b3 b4 =
  ...
  if p >= LongString.length !out_buffer
  then (
...
let slot_for_literal sc = enter (Reloc_literal sc); out_int 0
...
let emit_instr = function
  ...
  | Kacc n -> if n < 8 then out (opACC0 + n) else ( out opACC; out_int n )
  | Kenvacc n ->
      if n >= 1 && n <= 4
      then out (opENVACC1 + n - 1)
      else ( out opENVACC; out_int n )
  | Kpush -> out opPUSH
  | Kpop n -> out opPOP; out_int n
  | Kassign n -> out opASSIGN; out_int n
  ...
  | Kmakeblock (n, t) ->
      if n = 0
      then if t = 0 then out opATOM0 else ( out opATOM; out_int t )
      else if n < 4
      then (
...
let rec emit = function
  ...
  | Kpush :: Kconst k :: Kintcomp c :: Kbranchif lbl :: rem
    when is_immed_const k ->
      emit_branch_comp c; out_const k; out_label lbl; emit rem
  ...
  | Kpush :: Kconst k :: Kisout :: Kbranchif lbl :: rem
    when is_immed_const k ->
      out opBULTINT; out_const k; out_label lbl; emit rem
  | Kpush :: Kconst k :: Kisout :: Kbranchifnot lbl :: rem
    when is_immed_const k ->
      out opBUGEINT; out_const k; out_label lbl; emit rem
  ...
  | Kpush
    :: Kevent ({ev_kind = Event_before} as ev)
       :: ( (Kacc _ | Kenvacc _ | Koffsetclosure _ | Kgetglobal _ | Kconst _)
          as instr )
          :: c ->
  ...
  (* Default case *)
  | instr :: c -> emit_instr instr; emit c
