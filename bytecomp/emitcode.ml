...
let marshal_to_channel_with_possibly_32bit_compat ~filename ~kind outchan obj =
  try
    Marshal.to_channel outchan obj
      (if !Clflags.bytecode_compatible_32 then [ Marshal.Compat_32 ] else [])
  with Failure _ -> raise (Error (Not_compatible_32 (filename, kind)))

let report_error ppf (file, kind) =
  Format.fprintf ppf "Generated %s %S cannot be used on a 32-bit platform" kind
    file
...
let out_word b1 b2 b3 b4 =
  ...
  if p >= LongString.length !out_buffer then (
...
let out_const c =
  try out_int (const_as_int c)
  with AsInt -> Misc.fatal_error "Emitcode.const_as_int"
...
let define_label lbl =
  ...
  | Label_defined _ -> fatal_error "Emitcode.define_label"
...
let out_label_with_orig orig lbl =
  if lbl >= Array.length !label_table then extend_label_table lbl;
  match !label_table.(lbl) with
  | Label_defined def -> out_int ((def - orig) asr 2)
  | Label_undefined patchlist ->
      !label_table.(lbl) <- Label_undefined ((!out_position, orig) :: patchlist);
      out_int 0
...
let emit_instr = function
  ...
  | Kacc n ->
      if n < 8 then out (opACC0 + n)
      else (
        out opACC;
        out_int n)
  | Kenvacc n ->
      if n >= 1 && n <= 4 then out (opENVACC1 + n - 1)
      else (
        out opENVACC;
        out_int n)
  | Kpush -> out opPUSH
  ...
  | Koffsetclosure ofs ->
      if ofs = -3 || ofs = 0 || ofs = 3 then out (opOFFSETCLOSURE0 + (ofs / 3))
      else (
        out opOFFSETCLOSURE;
        out_int ofs)
  | Kgetglobal q ->
  ...
  | Kmakeblock (n, t) ->
      if n = 0 then
        ...
        out_int t)
  | Kgetfield n ->
      if n < 4 then out (opGETFIELD0 + n)
      else (
        out opGETFIELD;
        out_int n)
  | Ksetfield n ->
      if n < 4 then out (opSETFIELD0 + n)
      else (
        out opSETFIELD;
        out_int n)
  | Kmakefloatblock n ->
  ...
  | Kccall (name, n) ->
      if n <= 5 then (
        ...
        out_int n;
        slot_for_c_prim name)
  | Knegint -> out opNEGINT
  | Kaddint -> out opADDINT
  | Ksubint -> out opSUBINT
  | Kmulint -> out opMULINT
  | Kdivint -> out opDIVINT
  | Kmodint -> out opMODINT
  | Kandint -> out opANDINT
  | Korint -> out opORINT
  | Kxorint -> out opXORINT
  | Klslint -> out opLSLINT
  | Klsrint -> out opLSRINT
  | Kasrint -> out opASRINT
...
let remerge_events ev1 = function
  | Kevent ev2 :: c -> Kevent (Bytegen.merge_events ev1 ev2) :: c
  | c -> Kevent ev1 :: c
...
let rec emit = function
  ...
  | Kpush :: Kconst k :: Kisout :: Kbranchif lbl :: rem when is_immed_const k ->
      out opBULTINT;
      out_const k;
      out_label lbl;
      emit rem
  | Kpush :: Kconst k :: Kisout :: Kbranchifnot lbl :: rem when is_immed_const k
    ->
      out opBUGEINT;
      out_const k;
      out_label lbl;
  ...
  | Kpush :: Kacc 0 :: Kreturn m :: c -> emit (Kreturn (m - 1) :: c)
  (* General push then access scheme *)
  | Kpush :: Kacc n :: c ->
      if n < 8 then out (opPUSHACC0 + n)
      else (
        out opPUSHACC;
        out_int n);
      emit c
  | Kpush :: Kenvacc n :: c ->
      if n >= 1 && n < 4 then out (opPUSHENVACC1 + n - 1)
      else (
        out opPUSHENVACC;
        out_int n);
      emit c
  | Kpush :: Koffsetclosure ofs :: c ->
      if ofs = -3 || ofs = 0 || ofs = 3 then
        out (opPUSHOFFSETCLOSURE0 + (ofs / 3))
      else (
        out opPUSHOFFSETCLOSURE;
        out_int ofs);
      emit c
  | Kpush :: Kgetglobal id :: Kgetfield n :: c ->
      out opPUSHGETGLOBALFIELD;
...
let to_file outchan unit_name objfile ~required_globals code =
  ...
  in
  init ();
  (* Free out_buffer and reloc_info *)
  Btype.cleanup_abbrev ();
  (* Remove any cached abbreviation
     expansion before saving *)
  let pos_compunit = pos_out outchan in
  marshal_to_channel_with_possibly_32bit_compat ~filename:objfile
    ~kind:"bytecode unit" outchan compunit;
