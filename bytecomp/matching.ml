...
let ctx_lshift ctx =
  if small_enough (!Clflags.match_context_rows - 1) ctx
  then List.map lshift ctx
    ...
    (fun ps r ->
      List.fold_right
        (fun {left; right} r ->
          ...
          try {left = lubs transfert ps @ left; right} :: r
          with Empty -> r )
        ctx
        r )
    pss
...
and jumps_is_empty = function [] -> true | _ -> false

let jumps_singleton i = function [] -> [] | ctx -> [i, ctx]
...
let rec jumps_union (env1 : (int * ctx list) list) env2 =
  ...
  | ((i1, pss1) as x1) :: rem1, ((i2, pss2) as x2) :: rem2 ->
      if i1 = i2
      then (i1, get_mins le_ctx (pss1 @ pss2)) :: jumps_union rem1 rem2
      else if i1 > i2
      then x1 :: jumps_union rem1 env2
...
let rec jumps_unions envs =
  match envs with [] -> [] | [env] -> env | _ -> jumps_unions (merge envs)
...
let tr_raw act =
  match make_key act with Some act -> act | None -> raise Exit
  ...
  | (_, act0) :: rem -> (
    try
      ...
      let rec s_rec = function
        | [] -> Some act0
        | (_, act) :: rem ->
            if raw_act0 = tr_raw act then s_rec rem else None
      in
      s_rec rem
    with Exit -> None )
...
let cons_default matrix raise_num default =
  match matrix with [] -> default | _ -> (matrix, raise_num) :: default
  ...
  List.map
    (fun id ->
      ...
      , if List.mem id aliases
        then match arg with Some v -> v | _ -> raise Cannot_flatten
        else Ident.create_local (Ident.name id) ) )
    ids
...
and group_var = function {pat_desc = Tpat_any} -> true | _ -> false
...
and group_array = function {pat_desc = Tpat_array _} -> true | _ -> false

and group_lazy = function {pat_desc = Tpat_lazy _} -> true | _ -> false
  ...
  let rec attempt seen = function
    | ((q :: qs, act_q) as cl) :: rem ->
        ...
        then
          if may_compat p q
          then
            if Typedtree.pat_bound_idents p = []
  ...
  let rec do_split before ors no = function
    ...
    | ((p :: ps, act) as cl) :: rem ->
        ...
        then
          ...
          else if up_ok cl ors
          then do_split (cl :: before) ors no rem
          else if or_ok p ps ors
          then do_split before (cl :: ors) no rem
...
and split_naive cls args def k =
  let rec split_exc cstr0 yes = function
    ...
    | ((p :: _, _) as cl) :: rem ->
        ...
        then
          ...
          if cstr = cstr0
          then split_exc cstr0 (cl :: yes) rem
          else
            let yes = List.rev yes in
            let {me = next; matrix; top_default = def}, nexts =
  ...
  | ((p :: _, _) as cl) :: rem ->
      if group_constructor p
      then split_exc (pat_as_constr p) [cl] rem
...
and split_constr cls args def k =
  ...
  | _ -> (
        ...
        | [] -> (
            ...
            | cl :: rem -> (
              ...
              | _ ->
                  ...
                  , (idef, next) :: nexts ) ) )
        | ((p :: _, _) as cl) :: rem ->
            if group p && up_ok cl no
            then split_ex (cl :: yes) no rem
      ...
      and split_noex yes no = function
        ...
        | ((p :: _, _) as cl) :: rem ->
            if (not (group p)) && up_ok cl no
            then split_noex (cl :: yes) no rem
  ...
  | _ :: (((Lvar v as av), _) as arg) :: rargs -> (
    ...
    | _ -> (
        ...
        let var_cls =
          List.map
            (fun (ps, act) ->
              match ps with _ :: ps -> ps, act | _ -> assert false )
            cls
        ...
        | (({pat_desc = Tpat_or _} as orp) :: patl, action) :: rem ->
            ...
            let orpm =
              { cases =
                  ...
                  :: List.map
                       (function
                         | _ :: ps, action -> ps, action | _ -> assert false)
                       others
  ...
  | 1 ->
      let rec matcher_rec q rem =
        ...
        | Tpat_or (p1, p2, _) -> (
            ...
            | Some (a1 :: _), Some (a2 :: _) ->
                { a1 with
                  pat_loc = Location.none; pat_desc = Tpat_or (a1, a2, None)
                }
                :: rem
            | _, _ -> assert false )
...
let make_tuple_matching loc arity def = function
  ...
  | (arg, _mut) :: argl ->
      let rec make_args pos =
        if pos >= arity
        then argl
        else (Lprim (Pfield pos, [arg], loc), Alias) :: make_args (pos + 1)
      in
      { cases = []
      ; args = make_args 0
...
let make_record_matching loc all_labels def = function
  ...
  | (arg, _mut) :: argl ->
      let rec make_args pos =
        if pos >= Array.length all_labels
        then argl
        else
...
let make_string_test_sequence loc arg sw d =
  let d, sw =
    match d with
    | None -> ( match sw with (_, d) :: sw -> d, sw | [] -> assert false )
    | Some d -> d, sw
  in
  bind_sw arg (fun arg ->
      List.fold_right
...
let rec do_make_string_test_tree loc arg sw delta d =
  ...
  then make_string_test_sequence loc arg sw d
  else
...
let expand_stringswitch loc arg sw d =
  match d with
  | None ->
      bind_sw arg (fun arg -> do_make_string_test_tree loc arg sw 0 None)
  | Some e ->
      bind_sw arg (fun arg ->
          make_catch e (fun d ->
...
let share_actions_tree sw d =
  ...
  let d = match d with None -> None | Some d -> Some acts.(d) in
...
let rec uniq_lambda_list sw =
  ...
  | ((c1, _) as p1) :: ((c2, _) :: sw2 as sw1) ->
      if const_compare c1 c2 = 0
      then uniq_lambda_list (p1 :: sw2)
...
let make_test_sequence loc fail tst lt_tst arg const_lambda_list =
  ...
  let rec make_test_sequence const_lambda_list =
    if List.length const_lambda_list >= 4 && lt_tst <> Pignore
...
module SArg = struct
  ...
  let make_offset arg n =
    match n with 0 -> arg | _ -> Lprim (Poffsetint n, [arg], Location.none)
...
let share_actions_sw sw =
  ...
  let acts = Array.map handle_shared acts in
  let fail =
    match fail with None -> None | Some fail -> Some acts.(fail)
  in
  ( !hs
  , { sw with
...
let get_edges low high l =
  match l with [] -> low, high | (x, _) :: _ -> x, last high l
...
*)
  ...
  let rec nofail_rec cur_low cur_high cur_act = function
    ...
    | (i, act_i) :: rem as all ->
        ...
        then
          ...
          then nofail_rec cur_low i cur_act rem
          else if act_index = 0
          then (cur_low, i - 1, cur_act) :: fail_rec i i rem
  ...
  let init_rec = function
    ...
    | (i, act_i) :: rem ->
        ...
        if index = 0
        then fail_rec low i rem
        else if low < i
        then (low, i - 1, 0) :: nofail_rec i i index rem
...
let as_interval_nofail l =
  ...
  let rec i_rec cur_low cur_high cur_act = function
    ...
    | (i, act) :: rem ->
        ...
        if act_index = cur_act
        then i_rec cur_low i cur_act rem
  ...
  let inters =
    ...
    | (i, act) :: rem ->
        let act_index =
          ...
          if some_hole rem
          then store.act_store_shared () act
...
let mk_failaction_pos partial seen ctx defs =
          ...
          (fun (klist, jumps) (pats, i) ->
            ...
            let klist =
              List.fold_right
                ...
                klist
            and jumps =
              jumps_add i (ctx_lub (list_as_pat pats) ctx) jumps
            in
            klist, jumps )
          ([], jumps_empty)
          env
  ...
  if List.length fail_pats < !Clflags.match_context_rows
  then (
    let fail, jmps =
      scan_def
        []
        (List.map (fun pat -> pat, ctx_lub pat ctx) fail_pats)
        defs
    in
    if dbg
    then (
    ...
    then
      eprintf
        "FAIL: %s\n"
        (match fail with None -> "<none>" | Some lam -> string_of_lam lam);
    fail, [], jumps )
...
let combine_constant loc arg cst partial ctx def
  ...
  let lambda1 =
    ...
    | Const_int _ ->
        let int_lambda_list =
          List.map
            (function Const_int n, l -> n, l | _ -> assert false)
            const_lambda_list
        in
    ...
    | Const_char _ ->
        let int_lambda_list =
          List.map
            (function
              | Const_char c, l -> Char.code c, l | _ -> assert false)
            const_lambda_list
        in
      ...
      | _ -> (
        ...
        | n, _, _, _ -> (
            let act0 =
              ...
              | Some a, [] -> Some a
              | Some _, _ ->
                  if List.length nonconsts = cstr.cstr_nonconsts
                  then same_actions nonconsts
                  else None
...
and do_compile_matching repr partial ctx arg pmh =
  ...
  | Pm pm -> (
      ...
      match pat.pat_desc with
      | Tpat_any ->
          compile_no_test divide_var ctx_rshift repr partial ctx pm
      | Tpat_tuple patl ->
          compile_no_test
...
let find_in_pat pred =
  let rec find_rec p =
    ...
    | Tpat_record (lpats, _) ->
        List.exists (fun (_, _, p) -> find_rec p) lpats
...
let is_lazy_pat = function
  ...
  | Tpat_alias _ | Tpat_variant _ | Tpat_record _ | Tpat_tuple _
   |Tpat_construct _ | Tpat_array _ | Tpat_or _ | Tpat_constant _
   |Tpat_var _ | Tpat_any ->
      false
  | Tpat_exception _ -> assert false
...
let for_let loc param pat body =
  ...
  | _ ->
      ...
      if !opt
      then Lstaticcatch (bind, (nraise, ids_with_kinds), body)
...
let bind_opt (v, eo) k =
  match eo with None -> k | Some e -> Lambda.bind Strict v e k
