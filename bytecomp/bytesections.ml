...
let write_toc_and_trailer outchan =
  List.iter
    (fun (name, len) ->
       ...
       output_binary_int outchan len) (List.rev !section_table);
...
let read_toc ic =
  ...
  let header = really_input_string ic (String.length Config.exec_magic_number)
  in
