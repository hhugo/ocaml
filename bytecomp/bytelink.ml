...
let is_required (rel, _pos) =
  match rel with
  | Reloc_setglobal id -> Ident.Set.mem id !missing_globals
  | _ -> false

let add_required compunit =
  let add id = missing_globals := Ident.Set.add id !missing_globals in
...
let debug_info =
  ref ([] : (int * Instruct.debug_event list * string list) list)
    ...
    (fun () ->
      ...
      then
        try
          ...
          copy_file inchan outchan; close_in inchan
...
let build_custom_runtime prim_name exec_name =
  ...
  let debug_prefix_map =
    ...
    then [Printf.sprintf "-fdebug-prefix-map=%s=camlprim.c" prim_name]
    else []
  in
  Ccomp.call_linker
    Ccomp.Exe
...
let append_bytecode bytecode_name exec_name =
  let oc =
    open_out_gen [Open_wronly; Open_append; Open_binary] 0 exec_name
  in
  let ic = open_in_bin bytecode_name in
  copy_file ic oc; close_in ic; close_out oc
  ...
  then
    ...
    Misc.try_finally
      ...
      (fun () ->
        ...
        if not (Filename.check_suffix output_name ".c")
        then (
          temps := c_file :: !temps;
