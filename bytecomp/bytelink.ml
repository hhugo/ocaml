...
let add_ccobjs origin l =
  if not !Clflags.no_auto_link then
    ((if
...
let is_required (rel, _pos) =
  match rel with
  | Reloc_setglobal id -> Ident.Map.mem id !missing_globals
  | _ -> false
...
let scan_file obj_name tolink =
  let file_name =
    ...
    with
    | Not_found -> raise (Error (File_not_found obj_name))
  in
       ...
       (fun (name, crco) ->
          ...
          | Some crc ->
            if name = cu.cu_name then
              ...
              Consistbl.check crc_interfaces name crc file_name) cu.cu_imports
   with
...
let link_compunit output_fun currpos_fun inchan file_name compunit =
  ...
  (if !Clflags.debug && compunit.cu_debug > 0 then
     (seek_in inchan compunit.cu_debug;
      ...
      let debug_dirs =
        if List.mem file_path debug_dirs then
          debug_dirs
        else
          file_path :: debug_dirs
      in
      debug_info :=
        (currpos_fun (), debug_event_list, debug_dirs) :: !debug_info));
  ...
  try
    List.iter
      (fun cu ->
         ...
         with
         | Symtable.Error msg -> raise (Error (Symbol_error (name, msg))))
      units_required;
    close_in inchan
  with
  | x ->
...
let link_file output_fun currpos_fun =
  function
...
let output_debug_info oc =
  ...
  List.iter
    (fun (ofs, evl, debug_dirs) ->
       ...
       output_value oc evl;
       output_value oc debug_dirs) !debug_info;
  debug_info := []
...
let make_absolute file =
  if not (Filename.is_relative file) then
    ...
    Location.rewrite_absolute_path (Filename.concat (Sys.getcwd ()) file)
...
let link_bytecode ?final_name tolink exec_name standalone =
  ...
  (* avoid permission problems, cf PR#8354 *)
  let outperm = if !Clflags.with_runtime then 0o777 else 0o666 in
  let outchan =
    open_out_gen [ Open_wronly; Open_trunc; Open_creat; Open_binary ] outperm
      exec_name
  in
  Misc.try_finally ~always:(fun () -> close_out outchan)
    ...
    (fun () ->
       ...
       (if check_dlls then
          ...
          (Dll.init_compile !Clflags.no_std_include;
           ...
           with
           | Failure reason -> raise (Error (Cannot_open_dll reason))));
       let output_fun = output_bytes outchan
       and currpos_fun () = pos_out outchan - start_code
       in
...
let output_code_string outchan code =
  ...
  while !pos < len do
    ...
    if !output_code_string_counter >= 6 then
      (output_char outchan '\n';
...
let build_custom_runtime prim_name exec_name =
  let runtime_lib =
    if not !Clflags.with_runtime then
      ...
      "-lcamlrun" ^ !Clflags.runtime_variant
  in
  let debug_prefix_map =
    if Config.c_has_debug_prefix_map && not !Clflags.keep_camlprimc_file then
      let flag = [ Printf.sprintf "-fdebug-prefix-map=%s=camlprim.c" prim_name ]
      in
  ...
  Ccomp.call_linker Ccomp.Exe exec_name
    (debug_prefix_map @
       [ prim_name ] @ List.rev !Clflags.ccobjs @ [ runtime_lib ]) exitcode =
    0
...
let link objfiles output_name =
  ...
  in
  begin match Ident.Map.bindings missing_modules with
  | [] -> ()
  | (id, cu_name) :: _ ->
  ...
  if not !Clflags.custom_runtime then
  ...
  else if not !Clflags.output_c_object then
    let bytecode_name = Filename.temp_file "camlcode" "" in
    let prim_name =
      if !Clflags.keep_camlprimc_file then
        output_name ^ ".camlprim.c"
      else
        Filename.temp_file "camlprim" ".c"
    in
    Misc.try_finally
      ~always:(fun () ->
      ...
      (fun () ->
         ...
         (if not (build_custom_runtime prim_name exec_name) then
    ...
    let (c_file, stable_name) =
      ...
      then
        Filename.temp_file "camlobj" ".c", Some "camlobj.c"
      else
        let f = basename ^ ".c" in
    ...
    let obj_file =
      if !Clflags.output_complete_object then
        Filename.chop_extension c_file ^ Config.ext_obj
      else
        basename ^ Config.ext_obj
    in
    let temps = ref [] in
    Misc.try_finally ~always:(fun () -> List.iter remove_file !temps)
      (fun () ->
         ...
         if !Clflags.output_complete_executable then
           (temps := c_file :: !temps;
            if not (build_custom_runtime c_file output_name) then
              raise (Error Custom_runtime))
         else if not (Filename.check_suffix output_name ".c") then
           (temps := c_file :: !temps;
            (if
               Ccomp.compile_file ~output:obj_file ?stable_name c_file <> 0
             then
               raise (Error Custom_runtime));
            if
              not (Filename.check_suffix output_name Config.ext_obj) ||
                !Clflags.output_complete_object
            then
              (temps := obj_file :: !temps;
               let (mode, c_libs) =
                 if Filename.check_suffix output_name Config.ext_obj then
                   Ccomp.Partial, ""
                 else
                   Ccomp.MainDll, Config.bytecomp_c_libraries
               in
               if
                 not
                   (let runtime_lib =
...
let report_error ppf =
  ...
  | Wrong_object_name name ->
    fprintf ppf
      "The output file %s has the wrong name. The extension implies an \
       object file but the link step was requested" name
  ...
  | Inconsistent_import (intf, file1, file2) ->
    fprintf ppf
      "@[<hov>Files %a@ and %a@ make inconsistent assumptions over interface \
       %s@]" Location.print_filename file1 Location.print_filename file2 intf
  | Custom_runtime -> fprintf ppf "Error while building custom runtime system"
  | File_exists file ->
    fprintf ppf "Cannot overwrite existing file %a" Location.print_filename file
