...
let link_compunit output_fun currpos_fun inchan file_name compunit =
  ...
  if !Clflags.debug && compunit.cu_debug > 0 then (
    ...
    let debug_dirs =
      if List.mem file_path debug_dirs then debug_dirs
  ...
  Misc.try_finally
    ...
    (fun () ->
      (if standalone && !Clflags.with_runtime then
       (* Copy the header *)
       let header =
         if String.length !Clflags.use_runtime > 0 then "camlheader_ur"
         else "camlheader" ^ !Clflags.runtime_variant
       in
       try
...
let build_custom_runtime prim_name exec_name =
  ...
  Ccomp.call_linker Ccomp.Exe exec_name
    (debug_prefix_map @ [ prim_name ] @ List.rev !Clflags.ccobjs
   @ [ runtime_lib ])
    exitcode
  = 0
...
let link objfiles output_name =
  ...
  if not !Clflags.custom_runtime then link_bytecode tolink output_name true
  else if not !Clflags.output_c_object then
    let bytecode_name = Filename.temp_file "camlcode" "" in
    let prim_name =
      if !Clflags.keep_camlprimc_file then output_name ^ ".camlprim.c"
...
let report_error ppf = function
  ...
  | Wrong_object_name name ->
      fprintf ppf
        "The output file %s has the wrong name. The extension implies an \
         object file but the link step was requested"
        name
  ...
  | File_exists file ->
      fprintf ppf "Cannot overwrite existing file %a" Location.print_filename
        file
