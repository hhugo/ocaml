...
(* Lists. *)
  ...
  let rec uniq1 cmp x ys =
    match ys with
    | [] -> []
    | y :: ys -> if cmp x y = 0 then uniq1 cmp x ys else y :: uniq1 cmp y ys
  
  let uniq cmp xs =
    match xs with
    | [] -> []
    | x :: xs -> x :: uniq1 cmp x xs
...
(* This traditional API is convenient when used with ocamllex, but
  ...
  module Simplified = struct
    ...
    let revised2traditional parser =
      revised2traditional (fun token startp endp -> token, startp, endp) parser
  end
...
(******************************************************************************)
  ...
  module type EVERYTHING =
    sig
      ...
      include
      INSPECTION with type 'a lr1state := 'a lr1state
...
(* A stack is a linked list of cells. A sentinel cell -- which is its own
  ...
  module type ENGINE =
    sig
      ...
      include
      INCREMENTAL_ENGINE_START with type state := state
...
(******************************************************************************)
  ...
  (* This propagates type and exception definitions. The functions [number],
    ...
    and check_for_default_reduction env =
      ...
      T.default_reduction env.current announce_reduce(* there is a default reduction; perform it *)
         check_for_error_token (* there is none; continue below *) env
    ...
    and check_for_error_token env =
      ...
      if env.error then
        ...
        T.action env.current (* determines a row *) (T.token2terminal token)(* determines a column *)
           (T.token2value token) shift (* shift continuation *) announce_reduce
          (* reduce continuation *) initiate (* failure continuation *) env
    (* ------------------------------------------------------------------------ *)
    ...
    and shift
        env (please_discard : bool) (terminal : terminal)
    ...
    and error ~strategy env =
      ...
      (* Consult the column associated with the [error] pseudo-token in the
         action table. *)
      T.action env.current (* determines a row *) T.error_terminal
        (* determines a column *) T.error_value (error_shift ~strategy)
        (* shift continuation *) (error_reduce ~strategy)
        (* reduce continuation *) (error_fail ~strategy)(* failure continuation *)
         env
    ...
    and error_fail ~strategy env =
      ...
      | `Legacy ->
        ...
        (* The stack is nonempty. Pop a cell, updating the current state
           ...
           error handling there. *)(* I note that if the new state [cell.state] has a default reduction,
    ...
    let lexer_lexbuf_to_supplier
        (lexer : Lexing.lexbuf -> token) (lexbuf : Lexing.lexbuf)
      :
      supplier
    =
      fun () ->
        ...
        and endp = lexbuf.Lexing.lex_curr_p
        in
        token, startp, endp
    (* ------------------------------------------------------------------------ *)
    ...
    let pop (env : 'a env) : 'a env option =
      ...
      if next == cell then (* The stack is empty. *)
        None
      else (* The stack is nonempty. Pop off one cell. *)
  ...
  *)
  ...
  let shorten k text =
    ...
    if n <= 2 * k + 3 then
      ...
      String.sub text 0 k ^ "..." ^ String.sub text (n - k) k
...
(* The ocaml programming language treats string literals and array literals
  ...
  let pack (a : int array) : t =
    ...
    (* k > 8 *)(* [w] is the number of characters that we use to encode an array entry. *)
...
(* A compressed table is represented as a pair of arrays. The
  ...
  let compress
      (equal : 'a -> 'a -> bool) (insignificant : 'a -> bool) (dummy : 'a)
      (m : int) (n : int) (t : 'a array array)
    :
    'a table
  =
    (* Be defensive. *)
    ...
    Array.fast_sort (fun (_, rank1, _) (_, rank2, _) -> compare rank2 rank1)
      rows;
...
(* This signature defines the format of the parse tables. It is used as
  ...
  module type TABLES =
    sig
      ...
      val error : int (* width of the bitmap *) * string
...
(* This signature defines the format of the tables that are produced (in
  ...
  module type TABLES =
    sig
      ...
      val first : int (* width of the bitmap *) * string
    (* second component of [PackedIntArray.t] *)
    end
end
...
(* The type functor. *)
  ...
  module Make
      ...
      (ET :
        EngineTypes.TABLE with type terminal = int
  ...
  (* Including [IT] is an easy way of inheriting the definitions of the types
    ...
    let export t : item = t lsr low_bits, t mod low_limit
    ...
    let foreach_terminal_but_error f accu =
      ...
      foldij 0 n
        (fun i accu ->
           if i = TT.error_terminal then accu else f (IT.terminal i) accu) accu
    (* ------------------------------------------------------------------------ *)
    ...
    =
      ...
      ET.action source terminal semv
        (fun env _please_discard _terminal semv target ->
           ...
           let stack = { state = source; semv; startp; endp; next = env.stack }
           in
           { env with  stack; current = target }) reduce initiate env
  ...
  module MakeEngineTable (T : TableFormat.TABLES) = struct
    ...
    let action state terminal value shift reduce fail env =
      ...
      | 1 ->
        ...
        (* 0b01 : reduce *)(* 0b00 : cannot happen *)
    ...
    let may_reduce state prod =
      ...
      default_reduction state (fun () prod' -> prod = prod')
        (fun () ->
           (* If not, then for each terminal [t], ... *)
           foreach_terminal
             (fun t accu ->
                accu || (* ... test if there is a reduction of [prod] on [t]. *)
                  action state t ((* shift:  *) ) (fun () _ _ () _ -> false)
                    (* reduce: *) (fun () prod' -> prod = prod') (* fail:   *)
                    (fun () -> false) ()) false) ()
