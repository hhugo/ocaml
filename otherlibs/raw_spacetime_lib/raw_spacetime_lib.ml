...
module Shape_table = struct
  ...
  let demarshal chn ~call_counts : t =
    ...
    let shapes =
      List.fold_left
        (fun map (key, data) -> Int64_map.add key data map)
        Int64_map.empty
        raw
    in
    {shapes; call_counts}
...
module Trace = struct
  ...
  external unmarshal : in_channel -> 'a = "caml_spacetime_unmarshal_trie"

  let unmarshal in_channel =
    let trace = unmarshal in_channel in
    if trace = () then None else Some (Obj.magic trace : node)

  let foreign_node_is_null (node : foreign_node) =
    (Obj.magic node : unit) == ()

  external node_num_header_words :
    unit -> int
    = "caml_spacetime_node_num_header_words"
    [@@noalloc]
  ...
  module OCaml = struct
    ...
    module Direct_call_point = struct
      ...
      external callee_node :
        ocaml_node -> int -> 'target
      ...
      let call_count t =
        if Shape_table.call_counts t.shape_table
        then Some (call_count t.node t.offset)
        else None
    end
    ...
    module Indirect_call_point = struct
      ...
      module Callee = struct
        ...
        external callee :
          foreign_node -> Function_entry_point.t
        ...
        external callee_node :
          foreign_node -> node
          = "caml_spacetime_c_node_callee_node"
          [@@noalloc]
        ...
        let call_count t =
          if t.call_counts then Some (call_count t.node) else None

        external next :
          foreign_node -> foreign_node
          = "caml_spacetime_c_node_next"
          [@@noalloc]

        let next t =
          let next = {t with node = next t.node} in
          if foreign_node_is_null next.node then None else Some next
      end

      external callees :
        ocaml_node -> int -> foreign_node
        = "caml_spacetime_ocaml_indirect_call_point_callees"
        [@@noalloc]

      let callees t =
        let callees =
          { Callee.node = callees t.node t.offset
          ; call_counts = Shape_table.call_counts t.shape_table }
        in
        if Callee.is_null callees then None else Some callees
    end
    ...
    module Field = struct
      ...
      external classify_direct_call_point :
        ocaml_node -> int -> int
      ...
      and skip_uninitialised t =
        if not (is_uninitialised t) then Some t else next t
    end

    module Node = struct
      type t = ocaml_node

      external function_identifier :
        t -> Function_identifier.t
        = "caml_spacetime_ocaml_function_identifier"

      external next_in_tail_call_chain :
        t -> t
        = "caml_spacetime_ocaml_tail_chain"
        [@@noalloc]

      external compare : t -> t -> int = "caml_spacetime_compare_node"
        [@@noalloc]
  ...
  module Foreign = struct
    module Node = struct
      ...
      let fields t = if foreign_node_is_null t then None else Some t
    end

    module Allocation_point = struct
      type t = foreign_node

      external program_counter :
           t
        -> Program_counter.Foreign.t
           (* This is not a mistake; the same C function works. *)
        = "caml_spacetime_c_node_call_site"

      external annotation :
        t -> Annotation.t
        = "caml_spacetime_c_node_profinfo"
        [@@noalloc]

      external num_words_including_headers :
        t -> int
        = "caml_spacetime_c_node_allocation_count"
        [@@noalloc]
    end
    ...
    module Call_point = struct
      ...
      external call_site :
        t -> Program_counter.Foreign.t
        = "caml_spacetime_c_node_call_site"

      (* May return a null node.  See comment above and the C code. *)
      external callee_node : t -> node = "caml_spacetime_c_node_callee_node"
        [@@noalloc]
    end
    ...
    module Field = struct
      ...
      external is_call : t -> bool = "caml_spacetime_c_node_is_call"
        [@@noalloc]

      let classify t = if is_call t then Call t else Allocation t

      external next : t -> t = "caml_spacetime_c_node_next" [@@noalloc]

      let next t =
        let next = next t in
        if foreign_node_is_null next then None else Some next
    end
  end
  ...
  module Node = struct
    ...
    external is_ocaml_node : t -> bool = "caml_spacetime_is_ocaml_node"
      [@@noalloc]

    let classify t =
      if is_ocaml_node t
      then OCaml (Obj.magic t : ocaml_node)
...
module Heap_snapshot = struct
  ...
  module Series = struct
    ...
    let rec read_snapshots_and_events chn snapshots events =
      ...
      | Traces -> Array.of_list (List.rev snapshots), List.rev events
    ...
    let read ~path =
      ...
      if magic_number_base <> 0xace00ace
      then failwith "Raw_spacetime_lib: not a Spacetime profiling file"
      else
        ...
        | 0 ->
            let call_counts =
              ...
              | _ ->
                  failwith
                    "Raw_spacetime_lib: unknown Spacetime profiling file \
                     feature set"
            in
        ...
        | _ ->
            failwith
              "Raw_spacetime_lib: unknown Spacetime profiling file version \
               number"
    ...
    let trace t ~kind ~thread_index =
      if thread_index < 0 || thread_index >= num_threads t
      then None
      else
        match kind with
