...
let digest_interface unit loadpath =
  let filename =
    ...
    try Misc.find_in_path_uncap loadpath shortname
    with Not_found ->
      failwith
        (Printf.sprintf "Cannot find interface %s in load path" shortname)
  in
  ...
  try
    ...
    then ( close_in ic; raise Corrupted_interface );
    ...
    in
    crc
  with End_of_file | Failure _ -> close_in ic; raise Corrupted_interface
...
let print_crc unit =
  try
    ...
    print_string "  \"";
    print_string (String.capitalize_ascii unit);
    print_string "\",\n    \"";
