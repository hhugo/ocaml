...
module Make (P : Dynlink_platform_intf.S) = struct
  ...
  let default_available_units () =
    ...
    let (ifaces, implems, defined_symbols) =
      P.fold_initial_units
        ...
        ~f:(
        fun
          (ifaces, implems, defined_symbols) ~comp_unit ~interface
          ~implementation ~defined_symbols:defined_symbols_this_unit
        ->
          let ifaces =
         ...
         | old_crc, _old_src ->
           ...
           | Contents old_crc, Some crc ->
             if old_crc <> crc then
               ...
               ifaces
           end) ifaces (UH.interface_imports ui)
  
  let check_implementation_imports ~allowed_units filename ui implems =
    List.iter
      (fun (name, crc) ->
         (if not (String.Set.mem name allowed_units) then
         ...
         | (old_crc, _old_src, unit_state : implem) ->
           ...
           | Check_inited i ->
             if P.num_globals_inited () < i then
               raise
                 (DT.Error (Linking_error (filename, Uninitialized_global name)))
           | Loaded -> ()
           end) (UH.implementation_imports ui)
  
  let check_name filename ui priv ifaces implems =
    let name = UH.name ui in
    (if String.Map.mem name implems then
       raise (DT.Error (Module_already_loaded name)));
    (if priv && String.Map.mem name ifaces then
  ...
  let check_unsafe_module ui =
    if not !unsafe_allowed && UH.unsafe_module ui then
  ...
  let check filename (units : UH.t list) (state : State.t) ~priv =
    ...
    let new_units = String.Set.of_list (List.map (fun ui -> UH.name ui) units)
    in
    let implems =
      List.fold_left
    ...
    let (_ : implem String.Map.t) =
      List.fold_left
        (fun acc ui ->
           ...
           set_loaded_implem filename ui acc) implems units
    in
    let defined_symbols =
      List.fold_left
             ...
             lazy
               (Printf.sprintf
                  "between the executable file (and any existing \
                   ...
                   dynamically loaded from %s" (UH.name ui) filename)
           in
           let symbols = String.Set.of_list (UH.defined_symbols ui) in
           check_symbols_disjoint ~descr symbols defined_symbols;
           String.Set.union symbols defined_symbols) state.defined_symbols units
    in
    if priv then
      state
    else
