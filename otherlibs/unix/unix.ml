...
let _ =
  Callback.register_exception "Unix.Unix_error" (Unix_error (E2BIG, "", ""))
...
let handle_unix_error f arg =
  ...
  with Unix_error (err, fun_name, arg) ->
    ...
    then ( prerr_string " on \""; prerr_string arg; prerr_string "\"" );
    prerr_string ": ";
  ...
  let rec scan_dir eacces = function
    ...
    | dir :: rem -> (
        ...
        with Unix_error (err, _, _) as exn -> (
          ...
          | EACCES -> scan_dir true rem
          | EISDIR | ELOOP | ENAMETOOLONG | ENODEV | ENOENT | ENOTDIR
           |ETIMEDOUT ->
              scan_dir eacces rem
...
let execvpe name args env =
  try execvpe_c name args env
  with Unix_error (ENOSYS, _, _) -> execvpe_ml name args env
...
external in_channel_of_descr :
  file_descr -> in_channel
  = "unix_inchannel_of_filedescr"

external out_channel_of_descr :
  file_descr -> out_channel
  = "unix_outchannel_of_filedescr"

external descr_of_in_channel :
  in_channel -> file_descr
  = "caml_channel_descriptor"

external descr_of_out_channel :
  out_channel -> file_descr
  = "caml_channel_descriptor"
...
module LargeFile = struct
  external lseek :
    file_descr -> int64 -> seek_command -> int64
    = "unix_lseek_64"
...
let pause () =
  let sigs = sigprocmask SIG_BLOCK [] in
  sigsuspend sigs
...
module SO : sig
  ...
  external get :
    ('opt, 'v) t -> file_descr -> 'opt -> 'v
    = "unix_getsockopt"

  external set :
    ('opt, 'v) t -> file_descr -> 'opt -> 'v -> unit
    = "unix_setsockopt"
end
...
external getprotobyname : string -> protocol_entry = "unix_getprotobyname"

external getprotobynumber : int -> protocol_entry = "unix_getprotobynumber"
...
let system cmd =
  match fork () with
  | 0 -> ( try execv shell [|shell; "-c"; cmd|] with _ -> sys_exit 127 )
...
let safe_close fd = try close fd with Unix_error (_, _, _) -> ()
...
let open_process_args_in prog args =
  ...
  ( try open_proc prog args None (Process_in inchan) stdin in_write stderr
    with e -> close_in inchan; close in_write; raise e );
  close in_write; inchan

let open_process_args_out prog args =
  let out_read, out_write = pipe ~cloexec:true () in
  let outchan = out_channel_of_descr out_write in
  ( try open_proc prog args None (Process_out outchan) out_read stdout stderr
    with e -> close_out outchan; close out_read; raise e );
  close out_read; outchan

let open_process_args prog args =
  let in_read, in_write = pipe ~cloexec:true () in
  let out_read, out_write =
    try pipe ~cloexec:true ()
    with e -> close in_read; close in_write; raise e
  in
  let inchan = in_channel_of_descr in_read in
  let outchan = out_channel_of_descr out_write in
  ( try
...
let open_process_args_full prog args env =
  ...
  ( try
    ...
    with e ->
      ...
      close err_write;
      raise e );
  close out_read; close in_write; close err_write; inchan, outchan, errchan
...
let find_proc_id fun_name proc =
  try Hashtbl.find popen_processes proc
