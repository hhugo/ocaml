...
external spawn :
  string -> string array -> string array option -> bool -> int array -> int
  = "unix_spawn"
...
let create_process_gen cmd args optenv new_stdin new_stdout new_stderr =
  ...
  let redirections =
    ...
    |]
  in
  Fun.protect ~finally:close_after (fun () ->
      spawn cmd args optenv true (* usepath *) redirections)
...
let open_proc prog args envopt proc input output error =
  let pid = create_process_gen prog args envopt input output error in
