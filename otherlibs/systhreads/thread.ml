...
external thread_uncaught_exception
  : exn -> unit = "caml_thread_uncaught_exception"
  ...
  thread_new
    (fun () ->
       ...
       | exn ->
         ...
         flush stdout;
         flush stderr;
         (try !uncaught_exception_handler exn
          with
          | exn' ->
            ...
            Printf.eprintf "Thread %d uncaught exception handler raised %s\n"
              (id (self ())) (Printexc.to_string exn');
...
let () =
  ...
  thread_initialize ();
  Callback.register "Thread.at_shutdown"
    (fun () ->
       thread_cleanup ();
...
external sigmask
  : Unix.sigprocmask_command -> int list -> int list = "caml_thread_sigmask"
