...
and 'a communication =
  {
    performed : int ref;
    (* -1 if not performed yet, set to the number *)(* of the matching communication after rendez-vous. *)
...
let do_aborts abort_env genev performed =
  if abort_env <> [] then
    (if performed >= 0 then
       ...
       List.iter (fun (id, f) -> if not (List.mem id ids_done) then f ())
         abort_env
     else
...
let basic_sync abort_env genev =
  ...
  let rec poll_events i =
    if i >= Array.length bev then
      false
    else
      bev.(i).poll () || poll_events (i + 1)
  in
...
let rec flatten_event
    (abort_list : int list) (accu : ('a behavior * int list) list)
...
=
  ...
  | Choose evl ->
    let rec flatten_list accu' accu_abort' =
      function
      | ev :: l ->
        let (accu'', accu_abort'') =
...
let basic_poll abort_env genev =
  ...
  if ready then
    ...
    (Mutex.unlock masterlock;
     ...
     result)
  else (* Cancel the communication offers *)
    (performed := 0;
     Mutex.unlock masterlock;
...
let receive channel =
  Communication
    (fun performed condition evnum ->
       let rcomm = { performed; condition; data = None; event_number = evnum }
       in
       {
         poll =
           (fun () ->
