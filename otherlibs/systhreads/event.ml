...
and 'a communication = {
  performed : int ref;
  (* -1 if not performed yet, set to the number *)
  (* of the matching communication after rendez-vous. *)
  condition : Condition.t;
  (* To restart the blocked thread. *)
  mutable data : 'a option;
  (* The data sent or received. *)
  event_number : int;
}
(* Event number in select *)

(* Create a channel *)

let new_channel () =
  { writes_pending = Queue.create (); reads_pending = Queue.create () }
  ...
  if abort_env <> [] then
    if performed >= 0 then
      ...
      List.iter
        ...
        abort_env
    else List.iter (fun (_, f) -> f ()) abort_env

let basic_sync abort_env genev =
  let performed = ref (-1) in
  let condition = Condition.create () in
  let bev =
    Array.make (Array.length genev) (fst genev.(0) performed condition 0)
  in
  ...
  let rec poll_events i =
    if i >= Array.length bev then false
    else bev.(i).poll () || poll_events (i + 1)
  in
  Mutex.lock masterlock;
  if not (poll_events 0) then (
    (* Suspend on all events *)
    for i = 0 to Array.length bev - 1 do
      bev.(i).suspend ()
    done;
    (* Wait until the condition is signalled *)
    Condition.wait condition masterlock;
    (* PR#7013: protect against spurious wake-up *)
    while !performed < 0 do
      Condition.wait condition masterlock
    done);
  Mutex.unlock masterlock;
  (* Extract the result *)
  if abort_env = [] then (* Preserve tail recursion *)
...
let rec flatten_event (abort_list : int list)
    (accu : ('a behavior * int list) list)
    (accu_abort : (int * (unit -> unit)) list) ev =
  match ev with
  ...
  | Choose evl ->
      let rec flatten_list accu' accu_abort' = function
        | ev :: l ->
            let accu'', accu_abort'' =
              flatten_event abort_list accu' accu_abort' ev
            in
...
let send channel data =
  Communication
    (fun performed condition evnum ->
      let wcomm =
        { performed; condition; data = Some data; event_number = evnum }
      in
      {
        poll =
          (fun () ->
            let rec poll () =
              ...
              if !(rcomm.performed) >= 0 then poll ()
              else (
...
let receive channel =
  Communication
    (fun performed condition evnum ->
      let rcomm = { performed; condition; data = None; event_number = evnum } in
      {
        poll =
          (fun () ->
            let rec poll () =
              let wcomm = Queue.take channel.writes_pending in
              if !(wcomm.performed) >= 0 then poll ()
              else (
                rcomm.data <- wcomm.data;
                performed := evnum;
...
let rec wrap ev fn =
  ...
  | WrapAbort (ev, f') -> WrapAbort (wrap ev fn, f')
