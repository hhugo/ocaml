    ...
    (fun performed condition evnum ->
      ...
      { poll =
          (fun () ->
            ...
            in
            try poll (); true with Queue.Empty -> false )
      ; suspend =
          (fun () ->
...
let receive channel =
  Communication
    (fun performed condition evnum ->
      let rcomm =
        {performed; condition; data = None; event_number = evnum}
      in
      { poll =
          (fun () ->
            let rec poll () =
              let wcomm = Queue.take channel.writes_pending in
              if !(wcomm.performed) >= 0
              then poll ()
              else (
                rcomm.data <- wcomm.data;
                performed := evnum;
