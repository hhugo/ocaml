...
external waitpid : wait_flag list -> int -> int * process_status = "win_waitpid"
...
let realpath p =
  let cleanup p =
    (* Remove any \\?\ prefix. *)
    if String.length p <= 4 then p
    else if p.[0] = '\\' && p.[1] = '\\' && p.[2] = '?' && p.[3] = '\\' then
...
let closedir d =
  match d.entry_read with Dir_empty -> () | _ -> win_findclose d.handle
...
let normalize_slashes path =
  ...
  then path
    ...
    String.init (String.length path) (fun i ->
        match path.[i] with '/' -> '\\' | c -> c)
...
let kill pid signo =
  if signo <> Sys.sigkill then invalid_arg "Unix.kill"
  else if not (terminate_process pid) then
...
external time : unit -> (float[@unboxed]) = "unix_time" "unix_time_unboxed"
  [@@noalloc]

external gettimeofday : unit -> (float[@unboxed])
  = "unix_gettimeofday" "unix_gettimeofday_unboxed"
  [@@noalloc]
...
external inet_addr_of_string : string -> inet_addr = "unix_inet_addr_of_string"

external string_of_inet_addr : inet_addr -> string = "unix_string_of_inet_addr"
...
external socketpair :
  ...
  socket_type ->
  int ->
  file_descr * file_descr = "unix_socketpair"
...
module SO : sig
  ...
  external get : ('opt, 'v) t -> file_descr -> 'opt -> 'v = "unix_getsockopt"
...
external getservbyport : int -> string -> service_entry = "unix_getservbyport"
...
let getaddrinfo_emulation node service opts =
  ...
  let addresses =
    if node = "" then
      if List.mem AI_PASSIVE opts then [ (inet_addr_any, "0.0.0.0") ]
...
external win_create_process :
  ...
  file_descr ->
  file_descr ->
  int = "win_create_process" "win_create_process_native"
...
let open_proc prog cmdline optenv proc input output error =
  let pid = win_create_process prog cmdline optenv input output error in
