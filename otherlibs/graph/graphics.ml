...
external raw_draw_rect :
  int -> int -> int -> int -> unit
  = "caml_gr_draw_rect"

let draw_rect x y w h =
  if w < 0 || h < 0
  then raise (Invalid_argument "draw_rect")
