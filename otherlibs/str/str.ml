...
module Charset = struct
  ...
  let add_range s c1 c2 =
    for i = Char.code c1 to Char.code c2 do
      add s (Char.chr i)
    done
  ...
  *)
  ...
  let union s1 s2 =
    ...
    for i = 0 to 31 do
      Bytes.set r i
        (Char.chr (Char.code (Bytes.get s1 i) lor Char.code (Bytes.get s2 i)))
    done;
    r
  ...
  let disjoint s1 s2 =
    try
      for i = 0 to 31 do
        if Char.code (Bytes.get s1 i) land Char.code (Bytes.get s2 i) <> 0 then
          raise Exit
      done;
      true
    with Exit -> false
...
type regexp = {
  ...
  normtable : string;
  (* case folding table (if any) *)
  numgroups : int;
  (* number of \(...\) groups *)
  numregisters : int;
  (* number of nullable Star or Plus *)
...
let compile fold_case re =
  ...
  let emit_instr opc arg =
    if !progpos >= Array.length !prog then (
      let newlen = ref (Array.length !prog) in
      while !progpos >= !newlen do
        newlen := !newlen * 2
      done;
  ...
  let patch_instr pos opc dest = !prog.(pos) <- instr opc (displ dest pos) in
  ...
  let rec emit_code = function
    | Char c ->
        if fold_case then
          emit_instr op_CHARNORM
            (Char.code (Char.lowercase c)) [@ocaml.warning "-3"]
        else emit_instr op_CHAR (Char.code c)
    | String s -> (
        ...
        | 1 ->
            if fold_case then
              emit_instr op_CHARNORM
                (Char.code (Char.lowercase s.[0])) [@ocaml.warning "-3"]
            else emit_instr op_CHAR (Char.code s.[0])
        | _ -> (
            try
            ...
            with Not_found ->
              if fold_case then
                emit_instr op_STRINGNORM
                  (cpool_index (String.lowercase s)) [@ocaml.warning "-3"]
              else emit_instr op_STRING (cpool_index s)))
    | CharClass (cl, compl) ->
  ...
  let start_pos =
    if start = Charset.full then -1
    else cpool_index (Bytes.to_string (Charset.expand start'))
  in
  ...
  and regexpbackslash i =
    if i >= len then (Char '\\', i)
      ...
      | '(' ->
          ...
          else failwith "\\( group not closed by \\)"
      | '1' .. '9' as c -> (Refgroup (Char.code c - 48), i + 1)
      | 'b' -> (Wordboundary, i + 1)
      | c -> (Char c, i + 1)
  and regexpclass0 i =
    if i < len && s.[i] = '^' then
      let c, j = regexpclass1 (i + 1) in
      (c, true, j)
    else
      let c, j = regexpclass1 i in
      (c, false, j)
  and regexpclass1 i =
...
let matched_group n txt =
  ...
  if n < 0 || n2 >= Array.length !last_search_result then
    ...
    let b = !last_search_result.(n2) and e = !last_search_result.(n2 + 1) in
...
let global_substitute expr repl_fun text =
  let rec replace accu start last_was_empty =
    ...
    if startpos > String.length text then string_after text start :: accu
    else
...
let opt_search_forward_progress expr text start =
  ...
  | None -> None
  | Some pos ->
      if match_end () > start then Some pos
...
let bounded_split expr text num =
  ...
  let rec split accu start n =
    if start >= String.length text then accu
    else if n = 1 then string_after text start :: accu
    else
...
let bounded_full_split expr text num =
  let rec split accu start n =
    if start >= String.length text then accu
    else if n = 1 then Text (string_after text start) :: accu
    else
      match opt_search_forward_progress expr text start with
