...
let compile fold_case re =
  ...
  let rec emit_code = function
    | Char c ->
        if fold_case
        then (
          emit_instr op_CHARNORM (Char.code (Char.lowercase c)) [@ocaml.warning
                                                                  "-3"] )
    ...
    | String s -> (
      ...
      | 1 ->
          if fold_case
          then (
            emit_instr op_CHARNORM (Char.code (Char.lowercase s.[0])) [@ocaml.warning
                                                                        "-3"] )
      ...
      | _ -> (
        ...
        with Not_found ->
          if fold_case
          then (
            emit_instr op_STRINGNORM (cpool_index (String.lowercase s)) [@ocaml.warning
                                                                          "-3"] )
    ...
    | Option ((Char _ | CharClass _) as r) :: rl
      when disjoint_modulo_case (first r) (first_seq rl) ->
        emit_instr
          ...
          (cpool_index (charclass_of_regexp fold_case r));
        emit_seq_code rl
    | r :: rl -> emit_code r; emit_seq_code rl
...
module SeqBuffer = struct
  type t =
    { sb_chars : Buffer.t
    ; mutable sb_next : re_syntax list }
...
let parse s =
  ...
  and regexp1cont sb i =
    ...
    then SeqBuffer.extract sb, i
      ...
      SeqBuffer.add sb r; regexp1cont sb j
  and regexp2 i =
...
external re_string_match :
  regexp -> string -> int -> int array
  = "re_string_match"

external re_partial_match :
  regexp -> string -> int -> int array
  = "re_partial_match"

external re_search_forward :
  regexp -> string -> int -> int array
  = "re_search_forward"

external re_search_backward :
  regexp -> string -> int -> int array
  = "re_search_backward"
...
let group_beginning n =
  ...
  then invalid_arg "Str.group_beginning"
  else
...
let group_end n =
  let n2 = n + n in
  if n < 0 || n2 >= Array.length !last_search_result
  then invalid_arg "Str.group_end"
  else
...
let matched_group n txt =
  let n2 = n + n in
  if n < 0 || n2 >= Array.length !last_search_result
  then invalid_arg "Str.matched_group"
  else
