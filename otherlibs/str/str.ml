...
type regexp =
  ...
  (* index of set of starting chars, or -1 if none *)
...
external re_string_match
  : regexp -> string -> int -> int array = "re_string_match"

external re_partial_match
  : regexp -> string -> int -> int array = "re_partial_match"

external re_search_forward
  : regexp -> string -> int -> int array = "re_search_forward"

external re_search_backward
  : regexp -> string -> int -> int array = "re_search_backward"
...
external re_replacement_text
  : string -> int array -> string -> string = "re_replacement_text"
...
let bounded_split expr text num =
  ...
  let rec split accu start n =
    if start >= String.length text then
      ...
      | None -> string_after text start :: accu
      | Some pos ->
        split (String.sub text start (pos - start) :: accu) (match_end ())
          (n - 1)
  in
