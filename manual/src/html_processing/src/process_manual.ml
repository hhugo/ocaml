...
let preg_anyspace =
  String.concat "\\|"
    [ "\u{00a0}";(* NO-BREAK SPACE *)  "\u{2000}";(* EN QUAD *)  "\u{2001}";(* EM QUAD *)
     "\u{2002}";(* EN SPACE *)  "\u{2003}"; (* EM SPACE *) "\u{2004}";(* THREE-PER-EM SPACE *)
...
let toc_get_title li =
  let a = li $ "a[href]" in
  let title = trimmed_texts a |> String.concat " " |> remove_number in
  let file = R.attribute "href" a |> String.split_on_char '#' |> List.hd in
  file, title

let register_toc_entry toc_table name li =
  let (file, title) = toc_get_title li in
  dbg "%s : %s" name title;
  (if not (Hashtbl.mem toc_table file) then
           ...
           let chapters =
             li $$ ">ul >li.li-toc" (* Chapters *) |>
               fold
                 (fun chapters li ->
                    let (file, title) =
                      register_toc_entry toc_table "  Chapters" li
                    in
                    li $$ ">ul >li.li-toc" (* Sections *) |>
                      iter
                        (ignore << register_toc_entry toc_table "    Section");
                    (file, title) :: chapters) [] |>
               List.rev
           in
           if chapters = [] then
...
let copyright () =
  "<div class=\"copyright\">" ^ !copyright_text ^ "</div>" |> parse
...
let load_html file =
  ...
  let html =
    ...
    (* I think it would be good to replace "chapter" by "tutorial" for part
       I. The problem of course is how we number chapters in the other parts. *) (* |> Re.Str.global_replace (Re.Str.regexp_string "chapter") "tutorial"
    ...
    |>
      Re.Str.(
          global_replace (regexp (">[0-9]+\\.\\([0-9]+\\)" ^ preg_anyspace))
    ...
    (* The API (libref and compilerlibref directories) should be separate
      ...
      Re.Str.(
        global_replace (regexp_string "\"compilerlibref/")
  ...
  let charset_regexp = Re.Str.regexp "charset=\\([-A-Za-z0-9]+\\)\\(\\b\\|;\\)"
  in
  match Re.Str.search_forward charset_regexp html 0 with
  | exception Not_found ->
    dbg "Warning, no charset found in html.";
    html
  | _ ->
    begin match String.lowercase_ascii (Re.Str.matched_group 1 html) with
    | "utf-8" ->
      dbg "Charset is UTF-8; good.";
      html
    | "us-ascii" ->
      dbg "Charset is US-ASCII. We change it to UTF-8";
      Re.Str.global_replace charset_regexp "charset=UTF-8\\2" html
    | _ ->
      dbg "Warning, charset not recognized.";
      html
    end
...
let file_title file toc =
  if file = "index.html" then Some index_title else Hashtbl.find_opt toc file
...
let nav_replace_img_by_text toc alt a img =
  ...
  let title =
    match file_title file toc with
    | Some f ->
    ...
    | None ->
      dbg "Unknown title for file %s" file;
      file
  in
  ...
  if links <> [] then
    ...
    let (a1, a2) =
      ...
      | [ prev; up; next ] ->
        ...
        prev, next
      | [ a; b ] -> a, b
      | _ -> failwith "Navigation link should have at least 2 elements"
    in
...
let insert_xfile_content soup (title, header) toc xfile =
  ...
  create_element ~id:"start-section" "a" |> insert_after title;
  insert_after title header;
  next_siblings xbody |> iter delete;
...
let extract_date maintitle =
  ...
  in
  let txts = texts maintitle |> List.map String.trim in
...
let convert_index version soup =
  ...
  let title_selector =
    if float_of_string version < 4.07 then "div.center" else "div.maintitle"
  in
  let maintitle = soup $ title_selector in
  sprintf
    "<div class=\"maintitle\"><h1><span>The OCaml system</span>  release %s </h1><h3>%s</h3></div>"
...
let make_toc_sidebar ~version ~title file body =
  let toc =
    ...
    | Some t ->
      if classes t <> [] (* as in libthreads.html or parsing.html *) then
        (dbg "We don't promote <UL> to TOC for file %s" file;
         None)
      else
        Some t
  in
  let () =
    match body $? "h2.section", toc with
    | None, Some toc ->
      ...
      original_toc $ "ul" |> add_class "ul-content";
  ...
  | Some toc ->
    ...
    if file = "index.html" then
        ...
        body $$ "ul" |>
          fold
            (fun key ul ->
               match key with
               | None ->
                 begin match ul $$ "li" |> last with
                 | None -> None
                 | Some l ->
                   begin match l $ "a" |> leaf_text with
                   | Some text ->
                     dbg "[%s]" text;
                     if text = "Index of keywords" then
                       l $ "a" |> attribute "href"
                     else
                       None
                   | None -> None
      ...
      (begin match keywords with
       ...
       | Some keywords ->
         let a =
           create_element "a" ~inner_text:"Index of keywords"
             ~attributes:[ "href", keywords ]
         in
       ...
       (* Link to APIs *)
       let a =
         create_element "a" ~inner_text:"OCaml API"
           ~attributes:[ "href", api_page_url ^ "/index.html" ]
       in
       let li = create_element "li" in
       (append_child li a;
        append_child toc li);
       let a =
         create_element "a" ~inner_text:"OCaml Compiler API"
           ~attributes:[ "href", api_page_url ^ "/compilerlibref/index.html" ]
       in
       let li = create_element "li" in
       append_child li a;
       append_child toc li)
  ...
  (if file <> "index.html" then
     ...
     let a =
       create_element "a" ~inner_text:"< The OCaml Manual"
...
let make_part_menu ~part_title chapters file body =
  ...
  List.iter
    (fun (href, title) ->
       let a = create_element "a" ~inner_text:title ~attributes:[ "href", href ]
       in
       let li =
         if href = file then
           create_element "li" ~class_:"active"
         else
           create_element "li"
       in
       append_child li a;
  ...
  if part_title <> "" then
    ...
    (create_element "span" ~inner_text:"☰" |> prepend_child nav;
  ...
  body $? writtenby_css |>
    Option.iter
      (fun authors ->
         ...
         | Some s ->
           begin match
             Re.Str.(search_forward (regexp "(.+written by.+)") s 0)
           with
           | exception Not_found -> ()
           | _ ->
             dbg "Moving authors";
    ...
    toc $$ "li" |>
      fold
        (fun list li ->
           ...
           if
             not (String.contains rf '#') &&
               not (starts_with ".." rf) && not (starts_with "http" rf)
           then
