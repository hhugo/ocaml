...
let starts_with substring s =
  ...
  l <= String.length s && String.sub s 0 l = substring
(**** html processing ****)
...
let logo_html url =
  "<nav class=\"toc brand\"><a class=\"brand\" href=\"" ^
    url ^
...
let add_version_link nav text url =
  ...
  let a =
    create_element "a" ~inner_text:text
      ~attributes:[ "href", url; "id", "version-select" ]
  in
  append_child vnum a;
...
let add_sidebar_button body =
  ...
  create_element "span" ~inner_text:"☰" |> prepend_child btn;
...
let find_version () =
  ...
  let (major, minor) =
    Scanf.bscanf (Scanf.Scanning.from_file version_file) "%u.%u"
      (fun x y -> x, y)
  in
