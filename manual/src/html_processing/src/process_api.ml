...
type config = { src_dir : string; dst_dir : string; title : string }
...
let parse_file ?(original=false) file =
  ...
  | Some soup ->
    if original then
      failwith (sprintf "File %s was already processed" file)
    else
      soup
  | None ->
...
let make_toc ~version ~search file config title body =
  ...
  (let li_of_h ul h =
     ...
     let () =
       ...
       | Some id ->
         let href = "#" ^ id in
         let a =
           create_element "a" ~inner_text:(texts h |> String.concat "")
     ...
     li_current
   in
   descendants body |> elements |>
     fold
       (fun (li_current, h3_current) h ->
          match name h with
          | "h2" -> li_of_h ul h, None
          | "h3" ->
            begin match h3_current with
            | Some h3 -> li_of_h h3 h, h3_current
            | None ->
  ...
  let href =
    ...
    if String.sub base 0 5 = "type_" then
  ...
  let () =
    match body $? "ul.indexlist" with
    | Some uli ->
      delete uli;
      append_child ul uli;
      unwrap uli;
      (if search then search_widget true |> prepend_child body);
      create_element "h1" ~inner_text:(sprintf "The OCaml %sAPI" config.title) |>
        prepend_child body
    | None ->
...
let process ?(search=true) ~version config file out =
  ...
  body $? "div.navbar" |> Option.iter delete;
...
let process ?(overwrite=false) ~version config file out =
  if overwrite || not (Sys.file_exists out) then
    Ok (process ~version config file out)
  else
    Error (sprintf "File %s already exists." out)
...
module Index = struct
(* Generate the index.js file for searching with the quick search widget *)(* The idea is to parse the file "index_values.html" to extract, for each
  ...
  *)
  ...
  type item = { html : string; txt : string }
  ...
  *)
  let get_sig ?mod_name ~id_name config file =
    ...
    let span =
      soup $$ "pre span" |> filter (fun s -> id s = Some id_name) |> first |>
        require
    in
    let pre =
    ...
    let sig_txt = texts code |> String.concat "" |> String.escaped in
    (* We now replace anonymous "t"'s by the qualified "Module.t" *)
    let sig_txt =
      match mod_name with
      | None -> sig_txt
      | Some mod_name ->
        Re.Str.global_replace anon_t_regexp (mod_name ^ ".t") sig_txt
    in
    ...
    soup $ "table" |> select "tr" |>
      fold
        (fun index_list tr ->
           ...
           | [ td_val; td_info ] ->
             let (mdule, value) =
               match td_val $$ ">a" |> to_list with
               | [ a_val; a_mod ] ->
                 { txt = R.leaf_text a_mod; html = R.attribute "href" a_mod },
                 { txt = R.leaf_text a_val; html = R.attribute "href" a_val }
               | _ -> failwith "Cannot parse value"
             in
             let info =
               match td_info $? "div.info" with
               | Some info ->
                 {
                   html = to_string info |> one_line |> String.escaped;
                   txt =
                     texts info |> String.concat "" |> one_line |>
                       String.escaped
                 }
               | None -> { html = ""; txt = "" }
             in
...
let process_html config overwrite version =
  ...
  all_html_files config |>
    List.iter
      (fun file ->
         match
           process config ~overwrite ~version (config.src_dir // file)
             (config.dst_dir // file)
         with
         | Ok () -> incr processed
         | Error s -> dbg "%s" s);
...
let () =
  ...
  let config =
    if List.mem "compiler" args then
      {
        src_dir = html_maindir // "compilerlibref";
        dst_dir = api_dir // "compilerlibref";
        title = "Compiler "
      }
    else
      { src_dir = html_maindir // "libref"; dst_dir = api_dir; title = "" }
  in
