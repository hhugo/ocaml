...
let is_prefix prefix str =
  ...
  if length_prefix > length_str then
    false
  else
    String.sub str 0 length_prefix = prefix
  ...
  while !i < l do
      ...
      | '\\' ->
        ...
        then
          ...
          (print_char '\\';
           incr i)
      | _ ->
        (if !in_quotes && escape line.[!i] <> "" then
           print_string (escape line.[!i])
         else
           print_char line.[!i]);
        incr i
      end
    | Some mark ->
      (if line.[!i] = mark then
         ...
         Printf.eprintf "Warning: %c found in \\verb\n" line.[!i]);
      print_char line.[!i];
      incr i
  done
...
let process_line line =
  function
  | Normal ->
    if is_prefix "\\begin{caml_" line || is_prefix "\\begin{rawhtml}" line then
      (print_string line;
       Verbatim_like)
    else if is_prefix "\\begin{camlexample}" line then
      (print_endline line;
       Caml)
    else if is_prefix "\\begin{verbatim}" line then
      (print_string "\\begin{machineenv}";
       Verbatim ("\\end{verbatim}", "\\end{machineenv}"))
    else if is_prefix "\\begin{ocamldoccode}" line then
  ...
  | Verbatim (verbatim_end_in, verbatim_end_out) as env ->
    if is_prefix verbatim_end_in line then
  ...
  | Verbatim_like ->
    print_endline line;
    if is_prefix "\\end{caml_" line || is_prefix "\\end{rawhtml}" line then
      Normal
    else
      Verbatim_like
