...
let print_error error =
  Location.print_report Format.std_formatter @@
    ...
    | Unknown_label (loc, label) ->
      Location.errorf ~loc "@[<hov>Unknown manual label:@ %s@]@." label
    | Reference_mismatch r ->
      Location.errorf ~loc:r.loc
        "@[<v 2>References for label %S do not match:@,\
         OCaml side %a,@,\
         manual     %a@]@." r.label pp_ref r.ocaml pp_ref r.tex
    | No_aux_file -> Location.errorf "No aux file provided@."
    | Wrong_attribute_payload loc ->
      Location.errorf ~loc "Wrong payload for \"@manual.ref\"@."

(** {1 Main types} *)

module Refs = Map.Make(String) (** Maps of ocaml reference to manual labels *)
...
let check_consistency (ref : tex_reference) { loc; pos; _ } =
  if ref.pos = pos then
    ...
    (print_error @@
       Reference_mismatch { loc; label = ref.label; tex = ref.pos; ocaml = pos };
     { loc; pos; status = Bad })

let rec check_final_status label error =
  function
  | { status = Ok; _ } -> error
  | { status = Bad; _ } -> true
...
module TeX = struct
    ...
    try
      Scanf.sscanf s "\\newlabel{%s@}{{%s@}{%_d}{%_s@}{%s@.%_s@}{%_s@}}"
        (fun label position_string level ->
           let pos =
             List.map int_of_string (String.split_on_char '.' position_string)
           in
  ...
  let check_line refs line =
    ...
    | None -> refs
    | Some ref ->
      begin match Refs.find_opt ref.label refs with
      | None -> refs
      | Some l -> Refs.add ref.label (List.map (check_consistency ref) l) refs
      end
  ...
  let check_all aux refs =
    ...
    let rec lines refs =
      ...
      match s with
      | None -> refs
      | Some line -> lines @@ check_line refs line
    in
    let refs = lines refs in
    close_in chan;
    let error =
      Refs.fold
        (fun label ocaml_refs error ->
           List.fold_left (check_final_status label) error ocaml_refs) refs
        false
    in
...
module OCaml_refs = struct
  ...
  let manual_reference_attribute attr =
    ...
    if attr.attr_name.Location.txt <> "manual.ref" then
      None
    else
      ...
      | PStr
        ...
        Some s
      | _ ->
        print_error (Wrong_attribute_payload attr.attr_loc);
        Some ""
  (* triggers an error *)
  
  let rec label_from_attributes =
    function
    | [] -> None
    | a :: q ->
      begin match manual_reference_attribute a with
      | Some _ as x -> x
  ...
  let add_ref label ref refs =
    let l =
      match Refs.find_opt label refs with
      | None -> [ ref ]
      | Some l -> ref :: l
    in
  ...
  let inner_expr loc e =
    ...
    | Parsetree.Pexp_constant (Pconst_integer (n, _)) -> [ int_of_string n ]
    | _ ->
      tuple_expected ();
      []
  ...
  let from_file m f = from_ast m @@ parse f
end
...
let usage =
  "cross-reference-check -auxfile [file.aux] file_1 ... file_n checks that the \
   cross reference annotated with [@manual_cross_reference] are consistent \
...
let args =
  [
    "-auxfile",
    Arg.String (fun s -> aux_file := Some s),
