...
let check_consistency (ref : tex_reference) { loc; pos; _ } =
  if ref.pos = pos then { loc; pos; status = Ok }
  else (
    print_error
...
module TeX = struct
  ...
  let check_all aux refs =
    ...
    let rec lines refs =
      ...
      match s with None -> refs | Some line -> lines @@ check_line refs line
    in
...
module OCaml_refs = struct
  ...
  let manual_reference_attribute attr =
    ...
    if attr.attr_name.Location.txt <> "manual.ref" then None
    else
  ...
  let rec label_from_attributes = function
    | [] -> None
    | a :: q -> (
        match manual_reference_attribute a with
        | Some _ as x -> x
  ...
  let int_list l = try Some (List.map int l) with Exit -> None

  (** We keep a list of OCaml-side references to the same label *)
  let add_ref label ref refs =
    let l =
      match Refs.find_opt label refs with None -> [ ref ] | Some l -> ref :: l
    in
  ...
  let value_binding m iterator vb =
    let open Parsetree in
    (match label_from_attributes vb.pvb_attributes with
    | None -> ()
    | Some label ->
  ...
  let expr m iterator e =
    let open Parsetree in
    (match label_from_attributes e.pexp_attributes with
    | None -> ()
    | Some label ->
