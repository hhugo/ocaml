...
let print_error error =
  ...
  | Reference_mismatch r ->
      Location.errorf
        ~loc:r.loc
        "@[<v 2>References for label %S do not match:@,OCaml side \
         %a,@,manual     %a@]@."
        r.label
        pp_ref
        r.ocaml
...
let check_consistency (ref : tex_reference) {loc; pos; _} =
  if ref.pos = pos
  then {loc; pos; status = Ok}
  else (
    print_error
    @@ Reference_mismatch
         {loc; label = ref.label; tex = ref.pos; ocaml = pos};
    {loc; pos; status = Bad} )
...
module TeX = struct
      ...
      Scanf.sscanf
        ...
        (fun label position_string level ->
          let pos =
            List.map
              int_of_string
              (String.split_on_char '.' position_string)
          in
  ...
  let check_line refs line =
    ...
    | Some ref -> (
      ...
      | None -> refs
      | Some l ->
          Refs.add ref.label (List.map (check_consistency ref) l) refs )
...
module OCaml_refs = struct
  let parse sourcefile =
    Pparse.parse_implementation
      ~tool_name:"manual_cross_reference_check"
      sourcefile
...
let usage =
  "cross-reference-check -auxfile [file.aux] file_1 ... file_n checks that \
   the cross reference annotated with [@manual_cross_reference] are \
   consistent with the provided auxiliary TeX file"
