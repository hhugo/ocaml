...
module Dir = struct
  type t = { path : string; files : string list }
...
let add dir =
  ...
  List.iter
    (fun base ->
      let fn = Filename.concat dir.Dir.path base in
      if not (STbl.mem !files base) then STbl.replace !files base fn;
...
let find fn =
  assert ((not Config.merlin) || Local_store.is_bound ());
  if is_basename fn && not !Sys.interactive then STbl.find !files fn
