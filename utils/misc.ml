  ...
  module Array = struct
    let exists2 p a1 a2 =
      ...
      let rec loop i =
        if i = n then
          false
        else if p (Array.unsafe_get a1 i) (Array.unsafe_get a2 i) then
          true
        else
          loop (succ i)
      in
    ...
    let for_alli p a =
      ...
      let rec loop i =
        if i = n then
          true
        else if p i (Array.unsafe_get a i) then
          loop (succ i)
        else
          false
      in
    ...
    let all_somes a =
      try Some (Array.map (function None -> raise_notrace Exit | Some x -> x) a)
      with
      | Exit -> None
  end
...
let find_in_path_uncap path name =
  ...
  let rec try_dir =
    ...
    | dir :: rem ->
      ...
      in
      if Sys.file_exists ufullname then
        ufullname
      else if Sys.file_exists fullname then
        fullname
      else
        try_dir rem
  in
...
let no_overflow_mul a b = not (a = min_int && b < 0 || b <> 0 && a * b / b <> a)
...
let chop_extensions file =
  ...
  and basename = Filename.basename file
  in
  try
...
let search_substring pat str start =
  let rec search i j =
    if j >= String.length pat then
      i
    else if i + j >= String.length str then
      raise Not_found
    else if str.[i + j] = pat.[j] then
      search i (j + 1)
    else
      search (i + 1) 0
  in
...
let did_you_mean ppf get_choices =
  ...
  | choices ->
    ...
    Format.fprintf ppf "@\nHint: Did you mean %s%s%s?@?"
      (String.concat ", " rest) (if rest = [] then "" else " or ") last
  ...
  fun () ->
    (if not !init then
       (init := true;
        ...
        | exception Not_found -> ()
        | encoded_map ->
          begin match Build_path_prefix_map.decode_map encoded_map with
          | Error err ->
            fatal_errorf
...
module Magic_number = struct
  ...
  let parse s : (info, parse_error) result =
    ...
    (* a header is "truncated" if it starts like a valid magic number,
      ...
      in
      if List.exists starts_as all_kinds then
        Error (Truncated s)
      else
        Error (Not_a_magic_number s)
  ...
  let current_raw kind =
    ...
    | Cmx config ->
      ...
      if config = native_obj_config then
        reference
      else
  ...
  let check_current expected_kind { kind; version } : _ result =
    if kind <> expected_kind then
      ...
      if actual <> expected then
        Error (Version (kind, { actual; expected }))
      else
        Ok ()
  
  type error = Parse_error of parse_error | Unexpected_error of unexpected_error
  ...
  let read_current_info ~expected_kind ic =
    ...
    | Ok info ->
      ...
      begin match check_current kind info with
