...
let try_finally ?(always = fun () -> ()) ?(exceptionally = fun () -> ())
    work =
  match work () with
  | result -> (
    match always () with
...
module Stdlib = struct
  module List = struct
    ...
    let rec find_map f = function
      | x :: xs -> (
        match f x with None -> find_map f xs | Some _ as y -> y )
      | [] -> None
    ...
    let find_and_chop_longest_common_prefix ~equal ~first ~second =
      let rec find_prefix ~longest_common_prefix_rev l1 l2 =
        ...
        | elt1 :: l1, elt2 :: l2 when equal elt1 elt2 ->
            let longest_common_prefix_rev =
              elt1 :: longest_common_prefix_rev
            in
...
module LongString = struct
  ...
  let length tbl =
    ...
    (Sys.max_string_length * (tbl_size - 1))
    + Bytes.length tbl.(tbl_size - 1)
