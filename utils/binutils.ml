...
let name_at ?max_len buf start =
  ...
  in
  let rec loop pos =
    if pos >= max_pos || Bytes.get buf pos = '\000' then
...
type decoder = { ic : in_channel; endianness : endianness; bitness : bitness }
...
(* Reference: http://man7.org/linux/man-pages/man5/elf.5.html *)
  ...
  let read_sections d { e_shoff; e_shnum; e_shentsize; e_shstrndx; _ } =
    ...
    let mk i =
      ...
      in
      let sh_entsize =
        uint64_to_int "sh_entsize" (get_word d buf (base + 16 + 5 * word_size))
      in
      {
        sh_name;
        sh_type;
    ...
    if e_shstrndx = 0 then (* no string table *)
      sections
    else
  ...
  type symbol = { st_name : string; st_value : int64; st_shndx : int }
...
(* Reference:
  ...
  type header = { ncmds : int; sizeofcmds : int }
  ...
  type load_command = LC_SYMTAB of lc_symtab | OTHER
  ...
  type symbol = { n_name : string; n_type : int; n_value : int64 }
  
  let size_nlist d = 8 + word_size d
  
  let read_symbols d load_commands =
    match (* Can it happen there be more than one LC_SYMTAB? *)
      array_find_map (function LC_SYMTAB symtab -> Some symtab | _ -> None)
        load_commands
    with
    | None -> [| |]
  ...
  type magic = MH_MAGIC | MH_CIGAM | MH_MAGIC_64 | MH_CIGAM_64
...
(* Reference:
  ...
  type optional_header_magic = PE32 | PE32PLUS
  type optional_header = { _magic : optional_header_magic; image_base : int64 }
  ...
  let read_sections
      d { e_lfanew; number_of_sections; size_of_optional_header; _ }
  =
    let buf =
      load_bytes d
    ...
    let mk i =
      ...
      let pointer_to_raw_data = uint64_of_uint32 (get_uint32 d buf (base + 20))
      in
      {
        name;
        _virtual_size;
  ...
  type symbol = { name : string; address : int64 }
