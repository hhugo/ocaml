...
let command cmdline =
  if !Clflags.verbose
  then ( prerr_string "+ "; prerr_string cmdline; prerr_newline () );
...
let display_msvc_output file name =
  ...
  with _ -> close_in c; Sys.remove file

let compile_file ?output ?(opt = "") ?stable_name name =
  let pipe, file =
    if Config.ccomp_type = "msvc" && not !Clflags.verbose
    then
      try
      ...
      (Printf.sprintf
         ...
         | None ->
             let cflags, cppflags =
               ...
               else Config.ocamlc_cflags, Config.ocamlc_cppflags
             in
...
let macos_create_empty_archive ~quoted_archive =
  ...
  then result
    ...
    let result =
      command
        (Printf.sprintf "%s %s 2> /dev/null" Config.ranlib quoted_archive)
    in
    if result <> 0
    then result
    else
...
let create_archive archive file_list =
  ...
  | _ ->
      ...
      let is_macosx =
        match Config.system with "macosx" -> true | _ -> false
      in
      if is_macosx && file_list = []
      then (* PR#6550 *)
...
let expand_libname name =
  ...
  then name
    ...
    in
    try Load_path.find libname with Not_found -> libname
...
let remove_Wl cclibs =
  ...
  |> List.map (fun cclib ->
         (* -Wl,-foo,bar -> -foo bar *)
         if String.length cclib >= 4 && "-Wl," = String.sub cclib 0 4
         then
           String.map
             (function ',' -> ' ' | c -> c)
