...
let quote_files lst =
  ...
  if
    String.length s >= 65536 || String.length s >= 4096 && Sys.os_type = "Win32"
  then
...
let display_msvc_output file name =
  ...
  try
    ...
    (if first <> Filename.basename name then print_endline first);
    while true do print_endline (input_line c) done
  with
  | _ ->
    close_in c;
    Sys.remove file
  ...
  let exit =
    command
      (Printf.sprintf "%s%s %s %s -c %s %s %s %s %s%s"
         begin match !Clflags.c_compiler with
         | Some cc -> cc
         | None ->
           ...
           String.concat " "
             [ Config.c_compiler; Config.ocamlc_cflags; Config.ocamlc_cppflags ]
         end debug_prefix_map
         ...
         (Clflags.std_include_flag "-I") (Filename.quote name)
      ...
      let r1 =
        command
          (Printf.sprintf "%s rc %s %s" Config.ar quoted_archive
             (quote_files file_list))
      in
      if r1 <> 0 || String.length Config.ranlib = 0 then
        r1
      else
        command (Config.ranlib ^ " " ^ quoted_archive)
    ...
    List.map
      (fun cclib ->
         if String.starts_with ~prefix:"-l" cclib then
           let libname =
             "lib" ^
               String.sub cclib 2 (String.length cclib - 2) ^ Config.ext_lib
           in
