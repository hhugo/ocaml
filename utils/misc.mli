...
val show_config_and_exit : unit -> unit (** configuration variables *)
...
module Magic_number : sig
  ...
  *)
  ...
  val parse : raw -> (info, parse_error) result (** Parses a raw magic number *)
  ...
  val magic_length : int (** all magic numbers take the same number of bytes *)
  ...
  val current_raw : kind -> raw (** the current magic number of each kind *)
  
  val current_version : kind -> version (** the current version of each kind *)
  ...
  val parse_kind : raw_kind -> kind option (** parse a raw kind into a kind *)
