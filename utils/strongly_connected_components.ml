...
module Kosaraju : sig
  ...
  let component_graph graph =
    ...
    let add_component_dep node set =
      ...
      List.fold_left (fun set dep -> Int.Set.add components.(dep) set) set
        node_deps
    in
    Array.iteri
      (fun node component ->
         ...
         component_graph.(component) <-
           add_component_dep node component_graph.(component)) components;
    {
              ...
              if not (Id.Map.mem v dependencies) then
                Misc.fatal_errorf
                  "Strongly_connected_components.check: the graph has external \
                   dependencies (%a -> %a)" Id.print id Id.print v) set)
      dependencies
    ...
    Array.mapi
      (fun component nodes ->
         ...
         | _ :: _ ->
           ...
           component_edges.(component)) sorted_connected_components
