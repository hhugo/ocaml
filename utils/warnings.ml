...
let descriptions =
  [ 1, "Suspicious-looking start-of-comment mark.", [ "comment-start" ];
    ...
    9, "Missing fields in a record pattern.", [ "missing-record-field-pattern" ];
    10,
    ...
    [ "bad-module-name" ]; 25, "Ignored: now part of warning 8.", [];
    26,
    ...
    [ "redefining-unit" ]; 66, "Unused open! statement.", [ "unused-open-bang" ];
    ...
    69, "Unused record field.", [ "unused-field" ];
    70, "Missing interface file.", [ "missing-mli" ] ]

let name_to_number =
  let h = Hashtbl.create last_warning_number in
  List.iter
    (fun (num, _, names) -> List.iter (fun name -> Hashtbl.add h name num) names)
    descriptions;
...
let set_alert ~error ~enable s =
  let upd =
    ...
    | s ->
      let (set, pos) = if error then !current.alert_errors else !current.alerts
      in
      let f =
        if enable = pos then
...
let parse_alert_option s =
  ...
  let rec scan i =
    if i = n then
      ...
      raise (Arg.Bad "Ill-formed list of alert settings")
    else
...
type modifier = Set (** +a *) | Clear (** -a *) | Set_all (** @a *)
type token = Letter of char * modifier option | Num of int * int * modifier
...
let letter_alert tokens =
  ...
  let consecutive_letters =
    ...
    in
    let (l, on_going) = List.fold_left group_consecutive_letters ([], []) tokens
    in
    ...
    let message =
      Format.asprintf
        "@[<v>@[Setting a warning with a sequence of lowercase or uppercase \
         ...
         form:@ %t.@]@ @[Hint: Enabling or disabling a warning by its \
         mnemonic name requires a + or - prefix.@]%t@?@]"
        Format.(pp_print_list ~pp_sep:(fun _ -> ignore) pp_print_char) example
        (fun ppf -> List.iter (print_token ppf) tokens) spelling_hint
    in
    Some
...
let parse_opt error active errflag s =
  ...
  let action modifier i =
    ...
    | Set ->
      if i = 3 then
        set_alert ~error:errflag ~enable:true "deprecated"
      else
        flags.(i) <- true
    | Clear ->
      if i = 3 then
        set_alert ~error:errflag ~enable:false "deprecated"
      else
        flags.(i) <- false
    | Set_all ->
      if i = 3 then
  ...
  | None ->
    if s = "" then
      ...
      begin match s.[0], name_to_number rest with
      | '+', Some n ->
        action Set n;
        None
      | '-', Some n ->
