...
let parse_opt error active errflag s =
  ...
  let set_all i =
    ...
    else ( active.(i) <- true; error.(i) <- true )
  in
...
let message = function
  ...
  | Open_shadow_identifier (kind, s) ->
      Printf.sprintf
        "this open statement shadows the %s identifier %s (which is later \
         used)"
        kind
        s
  ...
  | No_cmx_file name ->
      Printf.sprintf
        "no cmx file was found in path for module %s, and its interface \
