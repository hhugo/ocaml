...
let prefix ppf x =
  ...
  | Swap { pos = left, right; _ } -> style kind ppf "%i<->%i. " left right
  | Move { got; expected; _ } -> style kind ppf "%i->%i. " expected got
...
module Two_cycle : sig
  ...
  let create kx ky = if kx <= ky then kx, ky else ky, kx
end
  ...
  module Simple (Impl : Parameters) = struct
    ...
    let swap swaps x y =
      ...
      | Some (Both (state, (ll, lr), (rl, rr))) ->
        begin match test state ll rr, test state rl lr with
        | Ok _, Ok _ ->
    ...
    let move moves x =
      ...
      | Some (Both (state, got, expected)) ->
        begin match test state got expected with
        | Ok _ -> Some (Move { name; got = got.pos; expected = expected.pos })
        | Error _ -> None
        end
    
    let refine state patch =
      let (_, (swaps, moves)) = two_cycles state patch in
      let filter : change -> composite_change option =
        function
        | Keep _ -> None
        | Insert x ->
        ...
        | Change (x, y, reason) ->
          begin match swap swaps x y with
    ...
    let diff state left right =
      ...
      let module Raw = Diff.Simple(Impl) 
      in
