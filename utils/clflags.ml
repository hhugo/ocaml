...
let profile_columns : Profile.column list ref = ref []

(* -dprofile/-dtimings *)

let debug_runavail = ref false

(* -drunavail *)

let native_code = ref false

(* set to true under ocamlopt *)
...
let afl_instrument = ref Config.afl_instrument

(* -afl-instrument *)

let afl_inst_ratio = ref 100
...
let inline_threshold =
  ref (Float_arg_helper.default default_inline_threshold)
...
let inline_alloc_cost =
  ref (Int_arg_helper.default default_inline_alloc_cost)
...
let set_int_arg round (arg : Int_arg_helper.parsed ref) default value =
  let value : int =
    match value with None -> default | Some value -> value
  in
  match round with
  | None ->
      arg :=
        Int_arg_helper.set_base_default
          value
...
let set_float_arg round (arg : Float_arg_helper.parsed ref) default value =
  ...
  | None ->
      arg :=
        Float_arg_helper.set_base_default
          value
...
module Compiler_pass = struct
  ...
  *)
  ...
  let to_string = function Parsing -> "parsing" | Typing -> "typing"
  ...
  let rank = function Parsing -> 0 | Typing -> 1

  let passes = [Parsing; Typing]
