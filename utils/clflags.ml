...
(* -clambda-checks *)
let cmm_invariants = ref Config.with_cmm_invariants
(* -dcmm-invariants *)
...
let pic_code =
  ref
    begin match Config.architecture with (* -fPIC *)
    | "amd64" -> true
    | _ -> false
    end
...
(* If you add a new pass, the following must be updated:
  ...
  *)
  ...
  let can_save_ir_after =
    function
    | Scheduling -> true
    | _ -> false
  
  let available_pass_names ~filter ~native =
    passes |> List.filter (enabled native) |> List.filter filter |>
      List.map to_string
...
let should_stop_after pass =
  if Compiler_pass.(rank Typing <= rank pass) && !print_types then
    true
  else
     ...
     | arg_name, _, _ as arg ->
       (try
        ...
        | Not_found ->
          ...
          arg_names := String.Map.add arg_name loc !arg_names)) args
...
let print_arguments program = Arg.usage !arg_spec (create_usage_msg program)
