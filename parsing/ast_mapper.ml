...
type mapper =
  ...
  ; include_declaration :
      mapper -> include_declaration -> include_declaration
  ; include_description :
...
module T = struct
  ...
  let map_extension_constructor_kind sub = function
    | Pext_decl (ctl, cto) ->
        Pext_decl
          (map_constructor_arguments sub ctl, map_opt (sub.typ sub) cto)
...
let extension_of_error {kind; main; sub} =
  ...
  then
    raise
      (Invalid_argument "extension_of_error: expected kind Report_error");
  let str_of_pp pp_msg = Format.asprintf "%t" pp_msg in
  let extension_of_sub sub =
    ( {loc = sub.loc; txt = "ocaml.error"}
    , PStr
        [Str.eval (Exp.constant (Pconst_string (str_of_pp sub.txt, None)))]
    )
  in
  ...
  let restore fields =
    let field name payload =
      let rec get_string = function
        ...
        | _ ->
            raise_errorf
              "Internal error: invalid [@@@ocaml.ppx.context { %s }] \
               string syntax"
              name
      and get_bool pexp =
        match pexp with
...
let apply_lazy ~source ~target mapper =
  ...
  then rewrite (implem : structure -> structure)
  else if magic = Config.ast_intf_magic_number
  then rewrite (iface : signature -> signature)
  else fail ()
