...
(* Type expressions for the core language *)
  ...
  =
    ...
    Type.mk ~loc ~attrs (map_loc sub ptype_name)
      ...
      ~priv:ptype_private
      ~cstrs:
      (List.map (map_tuple3 (sub.typ sub) (sub.typ sub) (sub.location sub))
         ptype_cstrs) ~kind:(sub.type_kind sub ptype_kind)
...
(* Type expressions for the module language *)
  let map sub { pmty_desc = desc; pmty_loc = loc; pmty_attributes = attrs } =
    ...
    | Pmty_functor (param, mt) ->
      functor_ ~loc ~attrs (map_functor_param sub param)
...
(* Value expressions for the core language *)
  let map sub { pexp_loc = loc; pexp_desc = desc; pexp_attributes = attrs } =
    ...
    | Pexp_letexception (cd, e) ->
      letexception ~loc ~attrs (sub.extension_constructor sub cd)
        (sub.expr sub e)
  ...
  let map sub { ppat_desc = desc; ppat_loc = loc; ppat_attributes = attrs } =
    ...
    | Ppat_construct (l, p) ->
      construct ~loc ~attrs (map_loc sub l)
        (map_opt (fun (vl, p) -> List.map (map_loc sub) vl, sub.pat sub p) p)
...
(* Value expressions for the class language *)
  let map sub { pcl_loc = loc; pcl_desc = desc; pcl_attributes = attrs } =
    ...
    | Pcl_fun (lab, e, p, ce) ->
      fun_ ~loc ~attrs lab (map_opt (sub.expr sub) e) (sub.pat sub p)
  ...
  =
    ...
    Ci.mk ~loc ~attrs ~virt:pci_virt
      ~params:(List.map (map_fst (sub.typ sub)) pl) (map_loc sub pci_name)
      (f pci_expr)
end
...
let default_mapper =
  {
      ...
      (fun this { pval_name; pval_type; pval_prim; pval_loc; pval_attributes }
       ->
         Val.mk (map_loc this pval_name) (this.typ this pval_type)
           ...
           ~loc:(this.location this pval_loc) ~prim:pval_prim);
    pat = P.map;
    expr = E.map;
    binding_op = E.map_binding_op;
    module_declaration =
      (fun this { pmd_name; pmd_type; pmd_attributes; pmd_loc } ->
         Md.mk (map_loc this pmd_name) (this.module_type this pmd_type)
           ~attrs:(this.attributes this pmd_attributes)
           ~loc:(this.location this pmd_loc));
    module_substitution =
      (fun this { pms_name; pms_manifest; pms_attributes; pms_loc } ->
         Ms.mk (map_loc this pms_name) (map_loc this pms_manifest)
    ...
    open_declaration =
      (fun this { popen_expr; popen_override; popen_attributes; popen_loc } ->
         Opn.mk (this.module_expr this popen_expr) ~override:popen_override
    ...
    constructor_declaration =
      (fun
         this { pcd_name; pcd_vars; pcd_args; pcd_res; pcd_loc; pcd_attributes }
       ->
    ...
    label_declaration =
      (fun this { pld_name; pld_type; pld_loc; pld_mutable; pld_attributes } ->
         Type.field (map_loc this pld_name) (this.typ this pld_type)
           ~mut:pld_mutable ~loc:(this.location this pld_loc)
...
let attribute_of_warning loc s =
  Attr.mk { loc; txt = "ocaml.ppwarning" }
  ...
  let make ~tool_name () =
    let fields =
      [ lid "tool_name", make_string tool_name;
        ...
        lid "unsafe_string", make_bool !Clflags.unsafe_string; get_cookies () ]
    in
    mk fields
  ...
  let restore fields =
    let field name payload =
      ...
      | "cookies" ->
        ...
        cookies :=
          List.fold_left (fun s (k, v) -> String.Map.add k v s) String.Map.empty
            l
      | _ -> ()
    in
    List.iter (function { txt = Lident name }, x -> field name x | _ -> ())
      fields
  
  let update_cookies fields =
    let fields =
      List.filter (function { txt = Lident "cookies" }, _ -> false | _ -> true)
        fields
    in
    fields @ [ get_cookies () ]
end
...
let drop_ppx_context_str ~restore =
  ...
  | {
      pstr_desc =
        Pstr_attribute
          { attr_name = { Location.txt = "ocaml.ppx.context" }; attr_payload = a;
            attr_loc = _ } } ::
      items
    ->
