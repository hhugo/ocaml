...
module Mtd = struct
  let mk
      ?(loc= !default_loc) ?(attrs=[]) ?(docs=empty_docs) ?(text=[]) ?typ name
  =
    {
      pmtd_name = name;
      pmtd_type = typ;
...
module Opn = struct
  let mk
      ?(loc= !default_loc) ?(attrs=[]) ?(docs=empty_docs) ?(override=Fresh) expr
  =
    {
      popen_expr = expr;
...
module Ci = struct
  let mk
      ?(loc= !default_loc) ?(attrs=[]) ?(docs=empty_docs) ?(text=[])
...
module Type = struct
  let mk
      ?(loc= !default_loc) ?(attrs=[]) ?(docs=empty_docs) ?(text=[])
      ?(params=[]) ?(cstrs=[]) ?(kind=Ptype_abstract) ?(priv=Public) ?manifest
      name
  =
    {
      ptype_name = name;
  ...
  let constructor
      ?(loc= !default_loc) ?(attrs=[]) ?(info=empty_info) ?(vars=[])
  ...
  let field
      ?(loc= !default_loc) ?(attrs=[]) ?(info=empty_info) ?(mut=Immutable) name
      typ
  =
    {
      pld_name = name;
...
module Te = struct
  let mk
      ?(loc= !default_loc) ?(attrs=[]) ?(docs=empty_docs) ?(params=[])
  ...
  let constructor
      ?(loc= !default_loc) ?(attrs=[]) ?(docs=empty_docs) ?(info=empty_info)
      name kind
  =
    {
      pext_name = name;
