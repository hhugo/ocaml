...
let fmt_position with_name f l =
  ...
  if l.pos_lnum = (-1) then
    fprintf f "%s[%d]" fname l.pos_cnum
  else
...
and attributes i ppf l =
  ...
  List.iter
    (fun a ->
       ...
       payload (i + 1) ppf a.attr_payload) l
...
and label_decl
    i ppf { pld_name; pld_mutable; pld_type; pld_loc; pld_attributes }
=
