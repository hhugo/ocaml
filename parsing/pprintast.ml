...
let letop s =
  String.length s > 3 &&
    s.[0] = 'l' && s.[1] = 'e' && s.[2] = 't' && List.mem s.[3] infix_symbols

let andop s =
  String.length s > 3 &&
    s.[0] = 'a' && s.[1] = 'n' && s.[2] = 'd' && List.mem s.[3] infix_symbols
...
let needs_parens txt =
  ...
  is_infix fix ||
    is_mixfix fix || is_kwdop fix || first_is_in prefix_symbols txt
...
(* c ['a,'b] *)
let rec class_params_def ctxt f =
  function
  | [] -> ()
        ...
        (fun f l ->
           pp f "%a"
             (fun f l ->
                match l with
                | [] -> ()
                | _ -> pp f "%a@;.@;" (list tyvar_loc ~sep:"@;") l) l) sl
        (core_type ctxt) ct
...
and core_type1 ctxt f x =
  if x.ptyp_attributes <> [] then
    ...
    | Ptyp_variant (l, closed, low) ->
        ...
        | Rtag (l, _, ctl) ->
          pp f "@[<2>%a%a@;%a@]" (iter_loc string_quot) l
            (fun f l ->
               match l with
               | [] -> ()
               | _ -> pp f "@;of@;%a" (list (core_type ctxt) ~sep:"&") ctl) ctl
      ...
      pp f "@[<2>[%a%a]@]"
        (fun f l ->
           ...
           | _ ->
             pp f "%s@;%a"
               begin match closed, low with
               | Closed, None -> if first_is_inherit then " |" else ""
               | Closed, Some _ -> "<" (* FIXME desugar the syntax sugar*)
               | Open, _ -> ">"
               end (list type_variant_helper ~sep:"@;<1 -2>| ") l) l
        (fun f low ->
           match low with
           | Some [] | None -> ()
           | Some xs -> pp f ">@ %a" (list string_quot) xs) low
    | Ptyp_object (l, o) ->
    ...
    | Ptyp_class (li, l) ->
      (*FIXME*)
      pp f "@[<hov2>%a#%a@]"
        (list (core_type ctxt) ~sep:"," ~first:"(" ~last:")") l longident_loc li
...
and pattern1 ctxt (f : Format.formatter) (x : pattern) : unit =
  ...
  if x.ppat_attributes <> [] then
    ...
    | Ppat_construct (({ txt; _ } as li), po) ->
      if(* FIXME The third field always false *)  txt = Lident "::" then
        pp f "%a" pattern_list_helper x
      else
        begin match po with
        | Some ([], x) -> pp f "%a@;%a" longident_loc li (simple_pattern ctxt) x
        | Some (vl, x) ->
          pp f "%a@ (type %a)@;%a" longident_loc li (list ~sep:"@ " string_loc)
...
and simple_pattern ctxt (f : Format.formatter) (x : pattern) : unit =
  if x.ppat_attributes <> [] then
    ...
    | Ppat_tuple l -> pp f "@[<1>(%a)@]" (list ~sep:",@;" (pattern1 ctxt)) l
    (* level1*)
    ...
    | Ppat_open (lid, p) ->
      let with_paren =
        ...
        | _ -> true
      in
      pp f "@[<2>%a.%a @]" longident_loc lid (paren with_paren @@ pattern1 ctxt)
        p
  ...
  if e.pexp_attributes <> [] then
    ...
    | Pexp_apply
      ...
      =
        ...
        | _ -> false
      in
      begin match id, List.map snd args with
      | Lident "!", [ e ] ->
      ...
      | Ldot (path, (("get" | "set") as func)), a :: other_args ->
        ...
        begin match path, other_args with
        | Lident "Array", i :: rest ->
      ...
      | (Lident s | Ldot (_, s)), a :: i :: rest when first_is '.' s ->
        ...
        print_indexop a path_prefix assign left ";" right
          (if multi_indices then expression ctxt else simple_expr ctxt) i rest
      | _ -> false
      end
    | _ -> false
...
and expression ctxt f x =
  if x.pexp_attributes <> [] then
    ...
    | Pexp_letop _
      ...
      paren true (expression reset_ctxt) f x
    | Pexp_fun (l, e0, p, e) ->
      pp f "@[<2>fun@;%a->@;%a@]" (label_exp ctxt) (l, e0, p) (expression ctxt)
        e
    | Pexp_newtype (lid, e) ->
      ...
      if not (sugar_expr ctxt f x) then
        ...
        | `Infix s ->
          ...
          | [ Nolabel, _ as arg1; Nolabel, _ as arg2 ] ->
            ...
            pp f "@[<2>%a@;%s@;%a@]" (label_x_expression_param reset_ctxt) arg1
              s (label_x_expression_param ctxt) arg2
          | _ ->
    ...
    | Pexp_ifthenelse (e1, e2, eo) ->
      (* @;@[<2>else@ %a@]@] *)
      let fmt : (_, _, _) format = "@[<hv0>@[<2>if@ %a@]@;@[<2>then@ %a@]%a@]"
      in
    ...
    | Pexp_letmodule (s, me, e) ->
      pp f "@[<hov2>let@ module@ %s@ =@ %a@ in@ %a@]"
        (Option.value s.txt ~default:"_") (module_expr reset_ctxt) me
        (expression ctxt) e
    | Pexp_letexception (cd, e) ->
      pp f "@[<hov2>let@ exception@ %a@ in@ %a@]" (extension_constructor ctxt)
    ...
    | Pexp_letop { let_; ands; body } ->
      pp f "@[<2>@[<v>%a@,%a@] in@;<1 -2>%a@]" (binding_op ctxt) let_
        (list ~sep:"@," (binding_op ctxt)) ands (expression ctxt) body
...
and value_description ctxt f x =
  ...
  pp f "@[<hov2>%a%a@]" (core_type ctxt) x.pval_type
    (fun f x ->
       if x.pval_prim <> [] then
         pp f "@ =@ %a" (list constant_string) x.pval_prim) x
...
and exception_declaration ctxt f x =
  pp f "@[<hov2>exception@ %a@]%a" (extension_constructor ctxt)
...
and class_type ctxt f x =
  ...
  | Pcty_constr (li, l) ->
    pp f "%a%a%a"
      ...
      longident_loc li (attributes ctxt) x.pcty_attributes
  | Pcty_arrow (l, co, cl) ->
    pp f "@[<2>%a@;->@;%a@]" (* FIXME remove parens later *)
...
and class_type_declaration_list ctxt f l =
  ...
  | x :: xs ->
    pp f "@[<v>%a@,%a@]" (class_type_declaration "class type") x
  ...
  | Pcf_inherit (ovf, ce, so) ->
    pp f "@[<2>inherit@ %s@ %a%a@]%a" (override ovf) (class_expr ctxt) ce
      (fun f so ->
         match so with
         | None -> ()
         | Some s -> pp f "@ as %s" s.txt) so (item_attributes ctxt)
      x.pcf_attributes
    ...
    pp f "@[<2>method%s %a%a@]%a" (override ovf) private_flag pf
      (fun f ->
         ...
         | { pexp_desc = Pexp_poly (e, None); pexp_attributes = []; _ } ->
           bind e
         | _ -> bind e) e (item_attributes ctxt) x.pcf_attributes
...
and class_structure ctxt f { pcstr_self = p; pcstr_fields = l } =
  pp f "@[<hv0>@[<hv2>object%a@;%a@]@;end@]"
    (fun f p ->
       ...
       | _ -> pp f " (%a)" (pattern ctxt) p) p (list (class_field ctxt)) l
...
and class_expr ctxt f x =
  if x.pcl_attributes <> [] then
    ...
    | Pcl_apply (ce, l) ->
      pp f "((%a)@ %a)" (* Cf: #7200 *) (class_expr ctxt) ce
        (list (label_x_expression_param ctxt)) l
    | Pcl_constr (li, l) ->
      pp f "%a%a"
        (fun f l ->
           if l <> [] then pp f "[%a]@ " (list (core_type ctxt) ~sep:",") l) l
        longident_loc li
...
and module_type ctxt f x =
  if x.pmty_attributes <> [] then
    ...
    | Pmty_functor (Named (s, mt1), mt2) ->
      ...
      | None ->
        pp f "@[<hov2>%a@ ->@ %a@]" (module_type1 ctxt) mt1 (module_type ctxt)
          mt2
      | Some name ->
        pp f "@[<hov2>functor@ (%s@ :@ %a)@ ->@ %a@]" name (module_type ctxt)
...
and with_constraint ctxt f =
  function
  | Pwith_type (li, ({ ptype_params = ls; _ } as td)) ->
    let ls = List.map fst ls in
    pp f "type@ %a %a =@ %a"
      (list (core_type ctxt) ~sep:"," ~first:"(" ~last:")") ls longident_loc li
...
and signature_item ctxt f x : unit =
  ...
  | Psig_module
    ...
    pp f "@[<hov>module@ %s@ =@ %a@]%a"
      (Option.value pmd.pmd_name.txt ~default:"_") longident_loc alias
      (item_attributes ctxt) pmd.pmd_attributes
  | Psig_module pmd ->
    pp f "@[<hov>module@ %s@ :@ %a@]%a"
      (Option.value pmd.pmd_name.txt ~default:"_") (module_type ctxt)
  ...
  | Psig_open od ->
    pp f "@[<hov2>open%s@ %a@]%a" (override od.popen_override) longident_loc
      od.popen_expr (item_attributes ctxt) od.popen_attributes
  | Psig_include incl ->
  ...
  | Psig_modtype { pmtd_name = s; pmtd_type = md; pmtd_attributes = attrs } ->
    pp f "@[<hov2>module@ type@ %s%a@]%a" s.txt
      (fun f md ->
         ...
         | Some mt ->
           pp_print_space f ();
           pp f "@ =@ %a" (module_type ctxt) mt) md (item_attributes ctxt) attrs
  | Psig_modtypesubst { pmtd_name = s; pmtd_type = md; pmtd_attributes = attrs }
    ->
    let md =
      match md with
      | None -> assert false
      (* ast invariant *) | Some mt -> mt
    in
  ...
  | Psig_recmodule decls ->
    let rec string_x_module_type_list f ?(first=true) l =
      ...
      | pmd :: tl ->
        (if not first then
           pp f "@ @[<hov2>and@ %s:@ %a@]%a"
             (Option.value pmd.pmd_name.txt ~default:"_") (module_type1 ctxt)
             pmd.pmd_type (item_attributes ctxt) pmd.pmd_attributes
         else
...
and module_expr ctxt f x =
  if x.pmod_attributes <> [] then
    ...
    | Pmod_functor (Named (s, mt), me) ->
      pp f "functor@ (%s@ :@ %a)@;->@;%a" (Option.value s.txt ~default:"_")
  ...
  if x.pexp_attributes <> [] then
        ...
        ppat_desc =
          Ppat_constraint
            (({ ppat_desc = Ppat_var _; _ } as pat),
             ({ ptyp_desc = Ptyp_poly _; _ } as typ)); ppat_attributes = []; _ }
      ->
    ...
    | Some (p, tyvars, ct, e) ->
      pp f "%a@;: type@;%a.@;%a@;=@;%a" (simple_pattern ctxt) p
        (list pp_print_string ~sep:"@;") (tyvars_str tyvars) (core_type ctxt) ct
        (expression ctxt) e
    | None ->
      begin match p with
...
and structure_item ctxt f x =
  ...
  | Pstr_module x ->
    let rec module_helper =
      ...
      | me -> me
    in
    pp f "@[<hov2>module %s%a@]%a" (Option.value x.pmb_name.txt ~default:"_")
      (fun f me ->
         ...
         | _ -> pp f " =@ %a" (module_expr ctxt) me) x.pmb_expr
      (item_attributes ctxt) x.pmb_attributes
  | Pstr_open od ->
    pp f "@[<2>open%s@;%a@]%a" (override od.popen_override) (module_expr ctxt)
      od.popen_expr (item_attributes ctxt) od.popen_attributes
  ...
  | Pstr_class l ->
    ...
    =
      ...
      pp f "@[<2>%s %a%a%s %a%a=@;%a@]%a" kwd virtual_flag x.pci_virt
        (class_params_def ctxt) ls txt (list (label_exp ctxt)) args
        (option class_constraint) constr (class_expr ctxt) cl
        (item_attributes ctxt) x.pci_attributes
    in
    begin match l with
    | [] -> ()
    ...
    let aux f =
      ...
      | { pmb_expr = { pmod_desc = Pmod_constraint (expr, typ) } } as pmb ->
        pp f "@[<hov2>@ and@ %s:%a@ =@ %a@]%a"
          (Option.value pmb.pmb_name.txt ~default:"_") (module_type ctxt) typ
          (module_expr ctxt) expr (item_attributes ctxt) pmb.pmb_attributes
      | pmb ->
        pp f "@[<hov2>@ and@ %s@ =@ %a@]%a"
          (Option.value pmb.pmb_name.txt ~default:"_") (module_expr ctxt)
          pmb.pmb_expr (item_attributes ctxt) pmb.pmb_attributes
    in
    ...
    List.iter
      (fun (ct1, ct2, _) ->
         pp f "@[<hov2>@ constraint@ %a@ =@ %a@]" (core_type ctxt) ct1
           (core_type ctxt) ct2) x.ptype_cstrs
  in
...
and type_extension ctxt f x =
  ...
  pp f "@[<2>type %a%a += %a@ %a@]%a"
    ...
    x.ptyext_params longident_loc x.ptyext_path private_flag x.ptyext_private(* Cf: #7200 *)
     (list ~sep:"" extension_constructor) x.ptyext_constructors
  ...
  | None ->
    pp f "%s%a@;%a" name
      (fun f ->
         ...
         | Pcstr_record l -> pp f "@;of@;%a" (record_declaration ctxt) l) args
  ...
  | Some r ->
    pp f "%s:@;%a%a@;%a" name pp_vars vars
      (fun f ->
         ...
         | Pcstr_tuple l ->
           pp f "%a@;->@;%a" (list (core_type1 ctxt) ~sep:"@;*@;") l
             (core_type1 ctxt) r
         | Pcstr_record l ->
...
and case_list ctxt f l : unit =
  let aux f { pc_lhs; pc_guard; pc_rhs } =
    pp f "@;| @[<2>%a%a@;->@;%a@]" (pattern ctxt) pc_lhs
      (option (expression ctxt) ~first:"@;when@;") pc_guard
