      ...
      let rec loop exp acc =
        ...
        | { pexp_desc =
              Pexp_construct
                ( {txt = Lident "::"; _}
                , Some
                    {pexp_desc = Pexp_tuple [e1; e2]; pexp_attributes = []}
                )
          ; pexp_attributes = [] } ->
            loop e2 (e1 :: acc)
...
let list :
      'a.    ?sep:space_formatter -> ?first:space_formatter
...
let option :
      'a.    ?first:space_formatter -> ?last:space_formatter
      -> (Format.formatter -> 'a -> unit) -> Format.formatter -> 'a option
      -> unit =
 fun ?first ?last fu f a ->
  let first = match first with Some x -> x | None -> ("" : _ format6)
  and last = match last with Some x -> x | None -> ("" : _ format6) in
  match a with None -> () | Some x -> pp f first; fu f x; pp f last

let paren :
      'a.    ?first:space_formatter -> ?last:space_formatter -> bool
      -> (Format.formatter -> 'a -> unit) -> Format.formatter -> 'a -> unit
    =
 fun ?(first = ("" : _ format6)) ?(last = ("" : _ format6)) b fu f x ->
  if b
  then ( pp f "("; pp f first; fu f x; pp f last; pp f ")" )
  else fu f x
...
and core_type ctxt f x =
  ...
  then
    ...
    | Ptyp_arrow (l, ct1, ct2) ->
        pp
          f
          "@[<2>%a@;->@;%a@]" (* FIXME remove parens later *)
          (type_with_label ctxt)
          (l, ct1)
          (core_type ctxt)
          ct2
    | Ptyp_alias (ct, s) ->
    ...
    | Ptyp_constr (li, l) ->
        pp
          ...
          (fun f l ->
            ...
            | _ ->
                list ~first:"(" ~last:")@;" (core_type ctxt) ~sep:",@;" f l
            )
          l
          longident_loc
          li
        ...
        let type_variant_helper f x =
          ...
          | Rtag (l, _, ctl) ->
              pp
                f
                "@[<2>%a%a@;%a@]"
                (iter_loc string_quot)
                l
                (fun f l ->
                  match l with
                  | [] -> ()
                  | _ ->
                      pp f "@;of@;%a" (list (core_type ctxt) ~sep:"&") ctl
                  )
                ctl
                (attributes ctxt)
                x.prf_attributes
...
and pattern1 ctxt (f : Format.formatter) (x : pattern) : unit =
  let rec pattern_list_helper f = function
    | { ppat_desc =
      ...
      ; ppat_attributes = [] } ->
        pp f "%a::%a" (simple_pattern ctxt) pat1 pattern_list_helper pat2
        (*RA*)
    | p -> pattern1 ctxt f p
  in
  if x.ppat_attributes <> []
  ...
  then false
    ...
    | Pexp_apply
      ...
      when List.for_all (fun (lab, _) -> lab = Nolabel) args -> (
        let print_indexop a path_prefix assign left right print_index
            indices rem_args =
...
and expression ctxt f x =
  ...
  then
    ...
    | Pexp_ifthenelse (e1, e2, eo) ->
        ...
        in
        let expression_under_ifthenelse =
          expression (under_ifthenelse ctxt)
        in
        pp
          f
          fmt
...
and module_type1 ctxt f x =
  ...
  then module_type ctxt f x
    ...
    | Pmty_signature s ->
        pp
          f
          "@[<hv0>@[<hv2>sig@ %a@]@ end@]" (* "@[<hov>sig@ %a@ end@]" *)
...
and signature_item ctxt f x : unit =
  ...
  | Psig_extension (e, a) ->
      item_extension ctxt f e; item_attributes ctxt f a
...
and payload ctxt f = function
  ...
  | PPat (x, Some e) ->
      pp f "?"; pattern ctxt f x; pp f " when "; expression ctxt f e
    ...
    (fun f -> function [] -> ()
      | l ->
          pp
            f
            "%a@;"
            (list (type_param ctxt) ~first:"(" ~last:")" ~sep:",")
            l )
    x.ptyext_params
    longident_loc
    x.ptyext_path
...
and constructor_declaration ctxt f (name, args, res, attrs) =
  ...
  | Some r ->
      pp
        ...
        "%s:@;%a@;%a"
        name
        (fun f -> function Pcstr_tuple [] -> core_type1 ctxt f r
          | Pcstr_tuple l ->
              pp
                f
                "%a@;->@;%a"
