...
let rec add_path bv ?(p = []) = function
  | Lident s ->
      let free =
        try lookup_free (s :: p) bv
        with Not_found -> String.Set.singleton s
      in
...
let open_module bv lid =
  ...
  | exception Not_found -> add_path bv lid; bv
...
and add_module_alias bv l =
  (* If we are in delayed dependencies mode, we delay the dependencies
       induced by "Lident s" *)
  (if !Clflags.transparent_modules then add_parent else add_module_path)
    bv
    l;
...
and add_sig_item (bv, m) item =
  ...
  | Psig_value vd -> add_type bv vd.pval_type; bv, m
  | Psig_type (_, dcls) | Psig_typesubst dcls ->
      List.iter (add_type_declaration bv) dcls;
      bv, m
  | Psig_typext te -> add_type_extension bv te; bv, m
  | Psig_exception te -> add_type_exception bv te; bv, m
  | Psig_module pmd ->
  ...
  | Psig_recmodule decls ->
      let add =
        List.fold_right
          (fun pmd -> String.Map.add pmd.pmd_name.txt bound)
          decls
      in
  ...
  | Psig_extension (e, _) -> handle_extension e; bv, m
...
and add_class_type bv cty =
  ...
  | Pcty_arrow (_, ty1, cty2) -> add_type bv ty1; add_class_type bv cty2
...
and add_struct_item (bv, m) item : _ String.Map.t * _ String.Map.t =
  match item.pstr_desc with
  | Pstr_eval (e, _attrs) -> add_expr bv e; bv, m
  | Pstr_value (rf, pel) ->
      let bv = add_bindings rf bv pel in
      bv, m
  | Pstr_primitive vd -> add_type bv vd.pval_type; bv, m
  | Pstr_type (_, dcls) ->
      List.iter (add_type_declaration bv) dcls;
      bv, m
  | Pstr_typext te -> add_type_extension bv te; bv, m
  | Pstr_exception te -> add_type_exception bv te; bv, m
  | Pstr_module x ->
  ...
  | Pstr_recmodule bindings ->
      let add =
        List.fold_right
          (fun x -> String.Map.add x.pmb_name.txt bound)
          bindings
      in
      let bv' = add bv and m = add m in
  ...
  | Pstr_extension (e, _) -> handle_extension e; bv, m
...
and add_class_expr bv ce =
  ...
  | Pcl_let (rf, pel, ce) ->
      let bv = add_bindings rf bv pel in
      add_class_expr bv ce
  | Pcl_constraint (ce, ct) -> add_class_expr bv ce; add_class_type bv ct
