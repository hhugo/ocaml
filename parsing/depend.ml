...
let rec add_path bv ?(p=[]) =
  function
  | Lident s ->
    let free =
      try lookup_free (s :: p) bv
      with
      | Not_found -> String.Set.singleton s
    in
...
let add_constructor_arguments bv =
  function
...
let add_type_declaration bv td =
  List.iter
    (fun (ty1, ty2, _) ->
       add_type bv ty1;
       add_type bv ty2) td.ptype_cstrs;
  ...
  | Pmty_with (mty, cstrl) ->
    ...
    List.iter
      (function
       ...
       | Pwith_modtypesubst (_, mty) -> add_modtype bv mty) cstrl
    ...
    let add =
      List.fold_right
        (fun pmd map ->
           match pmd.pmd_name.txt with
           | None -> map
           | Some name -> String.Map.add name bound map) decls
    in
    let bv' = add bv
    and m' = add m
    in
    ...
    let add =
      List.fold_right
        (fun x map ->
           match x.pmb_name.txt with
           | None -> map
           | Some name -> String.Map.add name bound map) bindings
    in
    let bv' = add bv
    and m = add m
    in
