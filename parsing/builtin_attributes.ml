  ...
  List.iter
    (fun (a, _, _) ->
       Location.prerr_warning a.attr_loc
         (Warnings.Misplaced_attribute a.attr_name.txt)) (alert_attrs attrs)
  ...
  let process loc txt errflag payload =
    ...
    | Some s ->
      (try
       ...
       with
       | Arg.Bad msg -> warn_payload loc txt msg)
  ...
  let process_alert loc txt =
    ...
    | PStr
      ...
      (try Warnings.parse_alert_option s
       ...
       | Arg.Bad msg -> warn_payload loc txt msg)
    | k ->
      begin match kind_and_message k with
      | Some ("all", _) ->
