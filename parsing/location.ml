...
module ISet : sig
    ...
    let (nesting, acc) =
      List.fold_left
        (fun (nesting, acc) (a, kind) ->
           ...
           | `E, `Inside (s, n) -> `Inside (s, n - 1), acc) (`Outside, []) pos
    in
  ...
  let find_bound_in iset ~range:(start, end_) =
    List.find_map
      (fun ((a, x), (b, y)) ->
         if start <= x && x <= end_ then
           Some (a, x)
         else if start <= y && y <= end_ then
           Some (b, y)
         else
           None) iset
  ...
  let extrema iset =
    if iset = [] then
      ...
      Some (fst (List.hd iset), snd (List.hd (List.rev iset)))
end
...
*)
let highlight_quote
    ppf ~(get_lines:start_pos:position -> end_pos:position -> input_line list)
    ...
    ISet.of_intervals @@
      List.filter_map
        (fun loc ->
           ...
           if s.pos_cnum = (-1) || e.pos_cnum = (-1) then
             None
           else
             Some ((s, s.pos_cnum), (e, e.pos_cnum - 1))) locs
  in
        ...
        List.map
          (fun (line, line_nb, line_start_cnum) ->
             let line =
               String.mapi
                 (fun i car ->
                    if ISet.mem iset ~pos:(line_start_cnum + i) then
                      car
                    else
                      '.') line
             in
...
let lines_around
    ~(start_pos:position) ~(end_pos:position) ~(seek:int -> unit)
...
let lines_around_from_lexbuf
    ~(start_pos:position) ~(end_pos:position) (lb : lexbuf)
  :
  input_line list
=
...
let lines_around_from_phrasebuf
    ~(start_pos:position) ~(end_pos:position) (pb : Buffer.t)
  :
  input_line list
=
  let pos = ref 0 in
...
let lines_around_from_file
    ~(start_pos:position) ~(end_pos:position) (filename : string)
  :
  input_line list
=
  try
    ...
    let read_char () =
      try Some (input_char cin)
      with
      | End_of_file -> None
    in
    let lines = lines_around ~start_pos ~end_pos ~seek:(seek_in cin) ~read_char
    in
    close_in cin;
    lines
  with
...
let is_quotable_loc loc =
  not (is_dummy_loc loc) &&
    loc.loc_start.pos_fname = !input_name && loc.loc_end.pos_fname = !input_name
  ...
  let pp self ppf report =
    ...
    *)
    print_updating_num_loc_lines ppf
      (fun ppf () ->
         Format.fprintf ppf "@[<v>%a%a%a: %a%a%a%a@]@." Format.pp_open_tbox ()
           (self.pp_main_loc self report) report.main.loc
           (self.pp_report_kind self report) report.kind Format.pp_set_tab ()
           (self.pp_main_txt self report) report.main.txt
           (self.pp_submsgs self report) report.sub Format.pp_close_tbox ()) ()
  in
