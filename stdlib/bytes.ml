...
let rec sum_lengths acc seplen = function
  ...
  | [hd] -> length hd + acc
  | hd :: tl ->
      sum_lengths (ensure_ge (length hd + seplen + acc) acc) seplen tl
...
let cat s1 s2 =
  ...
  unsafe_blit s1 0 r 0 l1; unsafe_blit s2 0 r l1 l2; r
...
let set_int16_be b i x =
  if not Sys.big_endian
  then set_int16_ne b i (swap16 x)
...
let set_int32_be b i x =
  if not Sys.big_endian
  then set_int32_ne b i (swap32 x)
...
let set_int64_be b i x =
  if not Sys.big_endian
  then set_int64_ne b i (swap64 x)
