...
let rec other_fields x i =
  if i >= Obj.size x then
    ...
    sprintf ", %s%s" (field x i) (other_fields x (i + 1))
  ...
  | Known_location l ->
    Some
      (sprintf "%s %s in file \"%s\"%s, line %d, characters %d-%d"
         ...
         (if l.is_inline then " (inlined)" else "") l.line_number l.start_char
         l.end_char)
...
let errors =
  ...
  (* WRONG_MAGIC *)
    "(Cannot print locations:\n\
    \ bytecode executable program file has wrong magic number)"; (* NO_FDS *)
    "(Cannot print locations:\n\
...
let try_get_raw_backtrace () =
  try get_raw_backtrace ()
  with
  | _ (* Out_of_memory? *) -> empty_backtrace
...
let handle_uncaught_exception exn debugger_in_use =
  ...
  with
  | _ -> (* There is not much we can do at this point *) ()

external register_named_value
  : string -> 'a -> unit = "caml_register_named_value"
