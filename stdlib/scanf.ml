...
let character_mismatch_err c ci = Printf.sprintf "looking for %C, found %C" c ci
...
let hexadecimal_value_of_char c =
  ...
  *)
  if d >= int_of_char 'a' then d - 87 (* 10 + int_of_char c - int_of_char 'a' *)
...
let char_for_hexadecimal_code c1 c2 =
  ...
  if c < 0 || c > 255 then
    bad_input (Printf.sprintf "bad character hexadecimal encoding \\%c%c" c1 c2)
  else char_of_int c
...
let rec make_scanf :
 ...
 fun ib fmt readers ->
  ...
  | Float
      ((_, (Float_f | Float_e | Float_E | Float_g | Float_G)), pad, prec, rest)
    ->
