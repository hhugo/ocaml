...
module Scanning : SCANNING = struct
  ...
  *)
  ...
  let stdin =
    from_ic scan_raise_at_end (From_file ("-", Stdlib.stdin)) Stdlib.stdin
  ...
  let close_in ib =
    match ib.ic_input_name with
    | From_channel ic -> Stdlib.close_in ic
...
let bad_end_of_input message =
  bad_input
    (Printf.sprintf
       "scanning of %s failed: premature end of file occurred before end \
        of token"
       message)
...
let token_int_literal conv ib =
  let tok =
    ...
    | B_conversion -> "0b" ^ Scanning.token ib
  in
...
let token_nativeint conv ib =
  nativeint_of_string (token_int_literal conv ib)
...
let scan_decimal_digit_plus width ib =
  if width = 0
  then bad_token_length "decimal digits"
  else
...
let scan_digit_plus basis digitp width ib =
  ...
  then bad_token_length "digits"
    ...
    then
      ...
      bad_input
        (Printf.sprintf "character %C is not a valid %s digit" c basis)
  ...
  then width, precision
    ...
    then width, precision
      ...
      | '.' ->
          ...
          let width =
            width - (precision - scan_fractional_part precision ib)
          in
...
let scan_hex_float width precision ib =
  ...
  | '0' as c -> (
      ...
      if width = 0 || Scanning.end_of_input ib then bad_hex_float ();
      let width =
        check_case_insensitive_string width ib bad_hex_float "x"
      in
      if width = 0 || Scanning.end_of_input ib
      then width
      else
        let width =
          match Scanning.peek_char ib with
          | '.' | 'p' | 'P' -> width
          | _ -> scan_hexadecimal_int width ib
        in
        if width = 0 || Scanning.end_of_input ib
        then width
        else
...
let hexadecimal_value_of_char c =
  ...
  *)
  if d >= int_of_char 'a'
  then d - 87 (* 10 + int_of_char c - int_of_char 'a' *)
  else if d >= int_of_char 'A'
  then d - 55 (* 10 + int_of_char c - int_of_char 'A' *)
  else d - int_of_char '0'

let char_for_hexadecimal_code c1 c2 =
  let c =
    (16 * hexadecimal_value_of_char c1) + hexadecimal_value_of_char c2
  in
  if c < 0 || c > 255
  then
    bad_input
...
let scan_backslash_char width ib =
  ...
  | '0' .. '9' as c ->
      let get_digit () =
        ...
        match c with '0' .. '9' as c -> c | c -> bad_input_escape c
      in
      let c0 = c in
      let c1 = get_digit () in
      let c2 = get_digit () in
...
let scan_bool ib =
  ...
  let m =
    ...
    | c ->
        bad_input
          (Printf.sprintf "the character %C cannot start a boolean" c)
  in
...
let scanf_bad_input ib = function
  | Scan_failure s | Failure s ->
      ...
      bad_input
        (Printf.sprintf "scanf: bad input at char number %i: %s" i s)
  | x -> raise x
...
let rec make_scanf : type a c d e f.
 ...
 fun ib fmt readers ->
  ...
  | Float
      ( (_, (Float_f | Float_e | Float_E | Float_g | Float_G))
      , pad
      , prec
      , rest ) ->
  ...
  | Flush rest ->
      if Scanning.end_of_input ib
      then make_scanf ib rest readers
...
type 'a kscanf_result =
  | Args of 'a
  | Exc of exn

let kscanf ib ef (Format (fmt, str)) =
  let rec apply : type a b. a -> (a, b) heter_list -> b =
   fun f args -> match args with Cons (x, r) -> apply (f x) r | Nil -> f
  in
  let k readers f =
...
let fscanf ic fmt =
  kscanf (Scanning.memo_from_channel ic) scanf_bad_input fmt
