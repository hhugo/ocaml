...
module Closure = struct
  ...
  let info_of_raw (info : nativeint) =
    ...
    let start_env =
      ...
      to_int (shift_right_logical (shift_left info 8) 9)
    in
    { arity; start_env }
...
module Extension_constructor = struct
  ...
  let of_val x =
    ...
    in
    let name =
      if is_block slot && tag slot = object_tag then
        field slot 0
      else
        invalid_arg "Obj.extension_constructor"
    in
    if tag name = string_tag then
      (obj slot : t)
    else
      invalid_arg "Obj.extension_constructor"
...
module Ephemeron = struct
  ...
  let blit_key e1 o1 e2 o2 l =
    if
      l < 0 || o1 < 0 || o1 > length e1 - l || o2 < 0 || o2 > length e2 - l
    then
      invalid_arg "Obj.Ephemeron.blit_key"
    else if l <> 0 then
