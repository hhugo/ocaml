...
let init l f =
  ...
  (* See #6575. We could also check for maximum array size, but this depends
    ...
    (for i = 1 to pred l do unsafe_set res i (f i) done;
     res)

let make_matrix sx sy init =
  let res = create sx [| |] in
  for x = 0 to pred sx do unsafe_set res x (create sy init) done;
  res
...
let blit a1 ofs1 a2 ofs2 len =
  if
    len < 0 ||
      ofs1 < 0 || ofs1 > length a1 - len || ofs2 < 0 || ofs2 > length a2 - len
  then
...
let map f a =
  ...
  if l = 0 then
    ...
    (for i = 1 to l - 1 do unsafe_set r i (f (unsafe_get a i)) done;
     r)
...
let mapi f a =
  ...
  if l = 0 then
    ...
    (for i = 1 to l - 1 do unsafe_set r i (f i (unsafe_get a i)) done;
     r)
...
let for_all2 p l1 l2 =
  ...
  if n1 <> n2 then
    ...
    let rec loop i =
      if i = n1 then
        true
      else if p (unsafe_get l1 i) (unsafe_get l2 i) then
        loop (succ i)
      else
        false
    in
...
let mem x a =
  ...
  let rec loop i =
    if i = n then
      false
    else if compare (unsafe_get a i) x = 0 then
      true
    else
      loop (succ i)
  in
...
let combine a b =
  ...
  if na = 0 then
    ...
    (for i = 1 to na - 1 do unsafe_set x i (unsafe_get a i, unsafe_get b i) done;
     x)
