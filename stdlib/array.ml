...
let fill a ofs len v =
  if ofs < 0 || len < 0 || ofs > length a - len then invalid_arg "Array.fill"
...
let map2 f a b =
  let la = length a in
  let lb = length b in
  if la <> lb then invalid_arg "Array.map2: arrays must have the same length"
  else if la = 0 then [||]
  else
...
let find_map f a =
  ...
  let rec loop i =
    if i = n then None
      ...
      match f (unsafe_get a i) with None -> loop (succ i) | Some _ as r -> r
  in
  loop 0
