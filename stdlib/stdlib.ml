...
let valid_float_lexem s =
  ...
  let rec loop i =
    ...
    then s ^ "."
      ...
      match string_get s i with '0' .. '9' | '-' -> loop (i + 1) | _ -> s
  in
  loop 0
...
external output_binary_int :
  out_channel -> int -> unit
  = "caml_ml_output_int"

external marshal_to_channel :
  out_channel -> 'a -> unit list -> unit
  = "caml_output_value"
