...
module GenHashTable = struct
  ...
  = struct
    ...
    let clear h =
      ...
      for i = 0 to len - 1 do h.data.(i) <- Empty done
    ...
    let clean h =
      ...
      in
      let d = h.data in
      for i = 0 to Array.length d - 1 do d.(i) <- do_bucket d.(i) done
    ...
    *)
    ...
    let remove h key =
      ...
      let rec remove_bucket =
        ...
        | Cons (hk, c, next) -> Cons (hk, c, remove_bucket next)
      in
      let i = key_index h hkey in
    ...
    *)
    ...
    let find_all h key =
      ...
      let rec find_in_bucket =
        ...
        | Cons (_, _, rest) -> find_in_bucket rest
      in
      find_in_bucket h.data.(key_index h hkey)
    
    let replace h key info =
      let hkey = H.hash h.seed key in
      let rec replace_bucket =
        function
        | Empty -> raise Not_found
        | Cons (hk, c, next) when hkey = hk ->
          begin match H.equal c key with
    ...
    let mem h key =
      ...
      let rec mem_in_bucket =
        ...
        | Cons (_hk, _c, rest) -> mem_in_bucket rest
      in
    ...
    let fold f h init =
      ...
      for i = 0 to Array.length d - 1 do accu := do_bucket d.(i) !accu done;
      !accu
    ...
    let rec bucket_length_alive accu =
      function
      | Empty -> accu
    ...
    let stats_alive h =
      let size = ref 0 in
      let mbl =
        Array.fold_left (fun m b -> Int.max m (bucket_length_alive 0 b)) 0
          h.data
      in
      let histo = Array.make (mbl + 1) 0 in
      Array.iter
    ...
    let to_seq tbl =
      ...
      let rec aux i buck () =
        match buck with
        | Empty ->
          if i = Array.length tbl_data then
            Seq.Nil
          else
            aux (i + 1) tbl_data.(i) ()
        | Cons (_, c, next) ->
          ...
          | Some key, Some data -> Seq.Cons ((key, data), aux i next)
          end
      in
      aux 0 Empty
  ...
  =
    ...
    (struct
      ...
      let equal c (k1, k2) =
        ...
        | Some k1', Some k2' ->
          if H1.equal k1 k1' && H2.equal k2 k2' then
            GenHashTable.ETrue
          else
            GenHashTable.EFalse
...
module Kn = struct
  ...
  let blit_key
      (t1 : ('k, 'd) t) (o1 : int) (t2 : ('k, 'd) t) (o2 : int) (l : int)
    :
    unit
  =
  ...
  module MakeSeeded (H : Hashtbl.SeededHashedType) =
    ...
    (struct
      ...
      let create k d =
        ...
        set_data c d;
        for i = 0 to Array.length k - 1 do set_key c i k.(i) done;
        c
