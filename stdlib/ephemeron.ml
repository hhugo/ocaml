  ...
  module MakeSeeded (H : sig
    ...
    *)
    let resize h =
      ...
      clean h;
      if nsize < Sys.max_array_length && h.size >= osize lsr 1
      then (
    ...
    *)
    ...
    let stats_alive h =
      ...
      let mbl =
        Array.fold_left
          (fun m b -> max m (bucket_length_alive 0 b))
          0
          h.data
      in
      let histo = Array.make (mbl + 1) 0 in
...
module K2 = struct
  ...
  let get_data (t : ('k1, 'k2, 'd) t) : 'd option =
    obj_opt (ObjEph.get_data t)
