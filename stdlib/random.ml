...
module State = struct
  ...
  let full_init s seed =
    ...
    for i = 0 to 54 do s.st.(i) <- i done;
    let accu = ref "x" in
  ...
  let rec int63aux s n =
    let max_int_32 = 1 lsl 30 + 0x3FFFFFFF in
    (* 0x7FFFFFFF *)
    let b1 = bits s in
    let b2 = bits s in
    let (r, max_int) =
      if n <= max_int_32 then
        ...
        let bpos = (b2 land 0x3FFFC000) lsl 1 lor b1 lsr 15 in
        bpos, max_int_32
      else
        let b3 = bits s in
        ...
        let bpos =
          ((b3 land 0x3FFFFE00) lsl 12 lor b2 lsr 9) lsl 20 lor b1 lsr 10
        in
        bpos, max_int
    in
    let v = r mod n in
  ...
  let nativeint =
    if Nativeint.size = 32 then
      (fun s bound -> Nativeint.of_int32 (int32 s (Nativeint.to_int32 bound)))
    else
      (fun s bound -> Int64.to_nativeint (int64 s (Int64.of_nativeint bound)))
  ...
  let bits32 s =
    let b1 = Int32.(shift_right_logical (of_int (bits s)) 14) in
    (* 16 bits *)
    let b2 = Int32.(shift_right_logical (of_int (bits s)) 14) in
    (* 16 bits *)
    Int32.(logor b1 (shift_left b2 16))
  
  let bits64 s =
    let b1 = Int64.(shift_right_logical (of_int (bits s)) 9) in
    (* 21 bits *)
    let b2 = Int64.(shift_right_logical (of_int (bits s)) 9) in
    (* 21 bits *)
    let b3 = Int64.(shift_right_logical (of_int (bits s)) 8) in
    (* 22 bits *)
    Int64.(logor b1 (logor (shift_left b2 21) (shift_left b3 42)))
  
  let nativebits =
    if Nativeint.size = 32 then
      (fun s -> Nativeint.of_int32 (bits32 s))
    else
      (fun s -> Int64.to_nativeint (bits64 s))
end
