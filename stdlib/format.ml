...
type formatter =
  (* The pretty-printer scanning stack. *)
  {
    ...
    pp_tbox_stack : tbox Stack.t; (* The pretty-printer semantics tag stack. *)
    pp_tag_stack : stag Stack.t;
    pp_mark_stack : stag Stack.t; (* Value of right margin. *)
    mutable pp_margin : int;
    (* Minimal space left before margin, when opening a box. *)
    mutable pp_min_space_left : int;
    (* Maximum value of indentation:
       no box can be opened further. *)
    mutable pp_max_indent : int; (* Space remaining on the current line. *)
    mutable pp_space_left : int; (* Current value of indentation. *)
    mutable pp_current_indent : int;
    (* True when the line has been broken by the pretty-printer. *)
    mutable pp_is_new_line : bool; (* Total width of tokens already printed. *)
    mutable pp_left_total : int; (* Total width of tokens ever put in queue. *)
    mutable pp_right_total : int; (* Current number of open boxes. *)
    mutable pp_curr_depth : int;
    (* Maximum number of boxes which can be simultaneously open. *)
    mutable pp_max_boxes : int; (* Ellipsis string. *)
    mutable pp_ellipsis : string; (* Output function. *)
    mutable pp_out_string : string -> int -> int -> unit;
    (* Flushing function. *)
    mutable pp_out_flush : unit -> unit; (* Output of new lines. *)
    mutable pp_out_newline : unit -> unit; (* Output of break hints spaces. *)
    mutable pp_out_spaces : int -> unit;
    (* Output of indentation of new lines. *)
    mutable pp_out_indent : int -> unit; (* Are tags printed ? *)
    mutable pp_print_tags : bool; (* Are tags marked ? *)
    mutable pp_mark_tags : bool; (* Find opening and closing markers of tags. *)
    mutable pp_mark_open_tag : stag -> string;
    mutable pp_mark_close_tag : stag -> string;
    mutable pp_print_open_tag : stag -> unit;
    mutable pp_print_close_tag : stag -> unit; (* The pretty-printer queue. *)
    pp_queue : pp_queue
  }
...
let format_pp_token state size =
  ...
  | Pp_break { fits; breaks } ->
    ...
    | Some { box_type; width } ->
      begin match box_type with
      | Pp_hovbox ->
        if size + String.length before > state.pp_space_left then
      ...
      | Pp_box ->
        ...
        if state.pp_is_new_line then
          break_same_line state fits
        else if size + String.length before > state.pp_space_left then
          break_new_line state breaks width
...
let rec advance_left state =
  ...
  | Some { size; token; length } ->
    ...
    if Size.is_known size || pending_count >= state.pp_space_left then
      (Queue.take state.pp_queue |> ignore;
       (* Not empty: we peek into it *)
       let size = if Size.is_known size then Size.to_int size else pp_infinity
       in
...
let set_size state ty =
  ...
  | Some { left_total; queue_elem } ->
    ...
    if left_total < state.pp_left_total then
      ...
      begin match queue_elem.token with
...
let pp_set_formatter_stag_functions
    state
    { mark_open_stag = mot; mark_close_stag = mct; print_open_stag = pot;
    ...
    (state.pp_margin <- n;
     let new_max_indent =
       ...
       (* If possible maintain pp_min_space_left to its actual value,
         ...
         Int.max
           (Int.max (state.pp_margin - state.pp_min_space_left)
              (state.pp_margin / 2)) 1
     in
     (* Rebuild invariants. *)
...
let pp_set_formatter_out_functions
    state
    { out_string = f; out_flush = g; out_newline = h; out_spaces = i;
      out_indent = j }
=
...
*)
let default_pp_mark_open_tag =
  function
  | String_tag s -> "<" ^ s ^ ">"
  | _ -> ""

let default_pp_mark_close_tag =
  function
  | String_tag s -> "</" ^ s ^ ">"
  | _ -> ""
...
and set_formatter_stag_functions = pp_set_formatter_stag_functions std_formatter

and get_formatter_stag_functions = pp_get_formatter_stag_functions std_formatter
...
(* To format a list *)
let rec pp_print_list ?(pp_sep=pp_print_cut) pp_v ppf =
  function
  | [] -> ()
  | [ v ] -> pp_v ppf v
  | v :: vs ->
...
let pp_print_option ?(none=(fun _ () -> ())) pp_v ppf =
  function
  | None -> none ppf ()
  | Some v -> pp_v ppf v

let pp_print_result ~ok ~error ppf =
  function
  | Ok v -> ok ppf v
  | Error e -> error ppf e

let pp_print_either ~left ~right ppf =
  function
...
let pp_set_all_formatter_output_functions
    state ~out:f ~flush:g ~newline:h ~spaces:i
=
...
let pp_set_formatter_tag_functions
    state
    { mark_open_tag = mot; mark_close_tag = mct; print_open_tag = pot;
      print_close_tag = pct }
=
  let stringify f e =
    function
...
let set_formatter_tag_functions = pp_set_formatter_tag_functions std_formatter
