...
let generic_basename is_dir_sep current_dir_name name =
  let rec find_end n =
    if n < 0
    then String.sub name 0 1
    else if is_dir_sep name n
    then find_end (n - 1)
    else find_beg n (n + 1)
  and find_beg n p =
    if n < 0
    then String.sub name 0 p
    else if is_dir_sep name n
    then String.sub name (n + 1) (p - n - 1)
    else find_beg (n - 1) p
  in
...
let generic_dirname is_dir_sep current_dir_name name =
  let rec trailing_sep n =
    ...
    then trailing_sep (n - 1)
    else base n
  and base n =
    if n < 0
    then current_dir_name
    else if is_dir_sep name n
    then intermediate_sep n
    else base (n - 1)
  and intermediate_sep n =
    if n < 0
    then String.sub name 0 1
    else if is_dir_sep name n
    then intermediate_sep (n - 1)
...
module Unix = struct
  ...
  let chop_suffix_opt ~suffix filename =
    ...
    then
      ...
      if r = suffix
      then Some (String.sub filename 0 (len_f - len_s))
      else None
    else None

  let temp_dir_name = try Sys.getenv "TMPDIR" with Not_found -> "/tmp"
...
module Win32 = struct
  ...
  let chop_suffix_opt ~suffix filename =
    ...
    then
      let r = String.sub filename (len_f - len_s) len_s in
      if String.lowercase_ascii r = String.lowercase_ascii suffix
      then Some (String.sub filename 0 (len_f - len_s))
      else None
    else None

  let temp_dir_name = try Sys.getenv "TEMP" with Not_found -> "."
  ...
  let quote s =
    ...
    and loop_bs n i =
      if i = l
      then ( Buffer.add_char b '\"'; add_bs n )
      else
        match s.[i] with
        | '\"' ->
            add_bs ((2 * n) + 1);
...
let chop_extension name =
  let l = extension_len name in
  if l = 0
  then invalid_arg "Filename.chop_extension"
...
let open_temp_file ?(mode = [Open_text]) ?(perms = 0o600)
  ...
  let rec try_name counter =
    ...
    try
      ( name
      , open_out_gen
          (Open_wronly :: Open_creat :: Open_excl :: mode)
          perms
          name )
    with Sys_error _ as e ->
      if counter >= 1000 then raise e else try_name (counter + 1)
