...
let generic_basename is_dir_sep current_dir_name name =
  let rec find_end n =
    if n < 0 then
      String.sub name 0 1
    else if is_dir_sep name n then
      find_end (n - 1)
    else
      find_beg n (n + 1)
  and find_beg n p =
...
let generic_dirname is_dir_sep current_dir_name name =
  ...
  in
  if name = "" then current_dir_name else trailing_sep (String.length name - 1)
...
module Unix : SYSDEPS = struct
  ...
  let check_suffix name suff = String.ends_with ~suffix:suff name
  
  let chop_suffix_opt ~suffix filename =
    let len_s = String.length suffix
    and len_f = String.length filename
    in
    if len_f >= len_s then
      let r = String.sub filename (len_f - len_s) len_s in
      (if r = suffix then Some (String.sub filename 0 (len_f - len_s)) else None)
    else
      None
  ...
  let quote_command cmd ?stdin ?stdout ?stderr args =
    String.concat ""
      [ "\""; quote_cmd_filename cmd; " ";
        ...
        | None -> ""
        | Some f ->
          if stderr = stdout then " 2>&1" else " 2>" ^ quote_cmd_filename f
        end; "\"" ]
...
let concat dirname filename =
  ...
  if l = 0 || is_dir_sep dirname (l - 1) then
    dirname ^ filename
  else
...
let open_temp_file
    ?(mode=[ Open_text ]) ?(perms=0o600) ?(temp_dir= !current_temp_dir_name)
    prefix suffix
=
  let rec try_name counter =
    let name = temp_file_name temp_dir prefix suffix in
