...
let bool_of_string_opt x =
  try Some (bool_of_string x) with Invalid_argument _ -> None

let int_of_string_opt x = try Some (int_of_string x) with Failure _ -> None

let float_of_string_opt x =
  try Some (float_of_string x) with Failure _ -> None
...
let parse_and_expand_argv_dynamic_aux allow_expand current argv speclist anonfun
  ...
  let convert_error error =
    ...
    *)
    let b = Buffer.create 200 in
    let progname =
      if initpos < Array.length !argv then !argv.(initpos) else "(?)"
    in
    (match error with
    | Unknown "-help" -> ()
    | Unknown "--help" -> ()
    | Unknown s -> bprintf b "%s: unknown option '%s'.\n" progname s
    | Missing s -> bprintf b "%s: option '%s' needs an argument.\n" progname s
    ...
    | Message s ->
        ...
        bprintf b "%s: %s.\n" progname s);
    usage_b b !speclist errmsg;
    if error = Unknown "-help" || error = Unknown "--help" then
  ...
  while !current < Array.length !argv do
    (try
       ...
       if String.length s >= 1 && s.[0] = '-' then
         ...
         in
         let consume_arg () =
           match follow with None -> incr current | Some _ -> ()
         in
         let rec treat_action = function
           ...
           | Bool f ->
               let arg = get_arg () in
               (match bool_of_string_opt arg with
           ...
           | Symbol (symb, f) ->
               let arg = get_arg () in
               if List.mem arg symb then (
                 f arg;
                 consume_arg ())
               else
                 raise
                   (Stop
                      (Wrong (s, arg, "one of: " ^ make_symlist "" " " "" symb)))
           | Set_string r ->
               r := get_arg ();
               consume_arg ()
           | Int f ->
           ...
           | Expand f ->
               if not allow_expand then
                 raise
                   (Invalid_argument
                      "Arg.Expand is is only allowed with \
...
let parse l f msg =
  try parse_argv Sys.argv l f msg with
  | Bad msg ->
      eprintf "%s" msg;
      exit 2
  | Help msg ->
...
let parse_dynamic l f msg =
  try parse_argv_dynamic Sys.argv l f msg with
  | Bad msg ->
      eprintf "%s" msg;
      exit 2
  | Help msg ->
...
let add_padding len ksd =
  ...
  | kwd, spec, msg ->
      ...
      if diff <= 0 then (kwd, spec, replace_leading_tab msg)
      else
...
let trim_cr s =
  let len = String.length s in
  if len > 0 && String.get s (len - 1) = '\r' then String.sub s 0 (len - 1)
  else s
