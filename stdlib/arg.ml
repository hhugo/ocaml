...
let usage_string speclist errmsg =
  ...
  usage_b b speclist errmsg; Buffer.contents b
...
let parse_and_expand_argv_dynamic_aux allow_expand current argv speclist
    anonfun errmsg =
    ...
    ( try
        ...
        then
          ...
          let get_arg () =
            match follow with
            | None ->
                if !current + 1 < Array.length !argv
                then !argv.(!current + 1)
          ...
          let rec treat_action = function
            ...
            | Symbol (symb, f) ->
                ...
                then ( f arg; consume_arg () )
                else
                  raise
                    (Stop
                       (Wrong
                          (s, arg, "one of: " ^ make_symlist "" " " "" symb)))
            | Set_string r ->
                r := get_arg ();
                consume_arg ()
            | Int f ->
...
let add_padding len ksd =
  ...
  | kwd, spec, msg ->
      ...
      if diff <= 0
      then kwd, spec, replace_leading_tab msg
      else
