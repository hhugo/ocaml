...
let bprint_char_set buf char_set =
  ...
  buffer_add_char buf '[';
  print_start
    (if is_in_char_set char_set '\000' then (
     buffer_add_char buf '^';
...
let bprint_fconv_flag buf fconv =
  ...
  | Float_f | Float_e | Float_E | Float_g | Float_G | Float_F | Float_h
  | Float_H ->
      ()
...
and type_ignored_format_substitution :
 ...
 fun sub_fmtty fmt fmtty ->
  ...
  | ( Format_subst_ty (sub1_fmtty, sub2_fmtty, sub_fmtty_rest),
      ...
      let Refl = f4 Refl in
      let (Fmtty_fmt_EBB (sub_fmtty_rest', fmt')) =
        type_ignored_format_substitution (erase_rel sub_fmtty_rest) fmt
          fmtty_rest
      in
      Fmtty_fmt_EBB
...
let convert_float fconv prec x =
  let hex () =
    let sign =
      ...
      | _ -> '-'
    in
    hexstring_of_float x prec sign
  in
...
let fmt_ebb_of_string ?legacy_behavior str =
  ...
  and parse_literal : type e f. int -> int -> int -> (_, _, e, f) fmt_ebb =
   fun lit_start str_ind end_ind ->
    if str_ind = end_ind then add_literal lit_start str_ind End_of_format
    else
  ...
  and parse_precision :
   ...
   fun pct_ind str_ind end_ind minus plus hash space ign pad ->
    ...
    | '*' ->
        parse_after_precision pct_ind (str_ind + 1) end_ind minus plus hash
          space ign pad Arg_precision
    | _ ->
        if legacy_behavior then
          (* note that legacy implementation did not ignore '.' without
             ...
             interprets it as '.0' *)
          parse_after_precision pct_ind str_ind end_ind minus plus hash space
            ign pad (Lit_precision 0)
    ...
    let fmt_result =
      ...
      | 'c' -> (
          ...
          let scan_format fmt_rest =
            (* %0c *)
            if get_ign () then
              Fmt_EBB (Ignored_param (Ignored_scan_next_char, fmt_rest))
          ...
          | Some _n ->
              if not legacy_behavior then invalid_nonnull_char_width str_ind
              else (* legacy ignores %c widths *) char_format fmt_rest)
      | 'C' ->
          let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
          if get_ign () then
            Fmt_EBB (Ignored_param (Ignored_caml_char, fmt_rest))
      ...
      | 'f' | 'e' | 'E' | 'g' | 'G' | 'F' | 'h' | 'H' ->
          let fconv =
            compute_float_conv pct_ind str_ind (get_plus ()) (get_hash ())
              (get_space ()) symb
          in
          let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
          if get_ign () then
  ...
  and search_subformat_end str_ind end_ind c =
    ...
    | '%' -> (
        if str_ind + 1 = end_ind then unexpected_end_of_format end_ind;
        if str.[str_ind + 1] = c then (* End of format found *) str_ind
        else
          match str.[str_ind + 1] with
          | '_' -> (
              (* Search for "%_(" or "%_{". *)
