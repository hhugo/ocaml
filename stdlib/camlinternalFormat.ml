...
let rev_char_set char_set =
  ...
  for i = 0 to 31 do
    Bytes.set char_set' i (char_of_int (int_of_char char_set.[i] lxor 0xFF))
  done;
...
let param_format_of_ignored_format : type a b c d e f x y.
 ...
 fun ign fmt ->
  ...
  | Ignored_int (iconv, pad_opt) ->
      Param_format_EBB
        (Int (iconv, pad_of_pad_opt pad_opt, No_precision, fmt))
...
let bprint_char_set buf char_set =
  ...
  and print_second set i =
    ...
    then
      match char_of_int i with
      | '\255' -> print_char buf 254; print_char buf 255
...
let bprint_padding : type a b. buffer -> (a, b) padding -> unit =
 fun buf pad ->
  ...
  | Arg_padding padty -> bprint_padty buf padty; buffer_add_char buf '*'
  ...
  let rec fmtiter : type a b c d e f. (a, b, c, d, e, f) fmt -> bool -> unit
   ...
   fun fmt ign_flag ->
    ...
    | Ignored_param (ign, rest) ->
        let (Param_format_EBB fmt') =
          param_format_of_ignored_format ign rest
        in
        fmtiter fmt' true
...
let string_of_fmt fmt =
  ...
  bprint_fmt buf fmt; buffer_contents buf
...
and trans : type a1 b1 c1 d1 e1 f1 a2 b2 c2 d2 e2 f2 a3 b3 c3 d3 e3 f3.
 ...
 fun ty1 ty2 ->
  ...
  | Nativeint_ty rest1, Nativeint_ty rest2 ->
      Nativeint_ty (trans rest1 rest2)
...
let type_padding : type a b c d e f x y.
 ...
 fun pad fmtty ->
  ...
  | Arg_padding padty, Int_ty rest ->
      Padding_fmtty_EBB (Arg_padding padty, rest)
 ...
 fun fmt fmtty ->
  ...
  | ( Format_arg (pad_opt, sub_fmtty, fmt_rest)
    , Format_arg_ty (sub_fmtty', fmtty_rest) ) ->
      if Fmtty_EBB sub_fmtty <> Fmtty_EBB sub_fmtty'
...
and type_formatting_gen : type a1 a3 b1 b3 c1 c3 d1 d3 e1 e2 e3 f1 f2 f3.
 ...
 fun formatting_gen fmt0 fmtty0 ->
  ...
  | Open_tag (Format (fmt1, str)) ->
      ...
      Fmt_fmtty_EBB
        (Formatting_gen (Open_tag (Format (fmt2, str)), fmt3), fmtty3)
  | Open_box (Format (fmt1, str)) ->
      let (Fmt_fmtty_EBB (fmt2, fmtty2)) = type_format_gen fmt1 fmtty0 in
      let (Fmt_fmtty_EBB (fmt3, fmtty3)) = type_format_gen fmt0 fmtty2 in
      Fmt_fmtty_EBB
        (Formatting_gen (Open_box (Format (fmt2, str)), fmt3), fmtty3)
...
and type_ignored_param : type p q x y z t u v a b c d e f.
 ...
 fun ign fmt fmtty ->
  ...
  | Ignored_scan_get_counter _ as ign' ->
      type_ignored_param_one ign' fmt fmtty
 ...
 fun sub_fmtty fmt fmtty ->
  ...
  | ( Format_arg_ty (sub2_fmtty, sub_fmtty_rest)
    , Format_arg_ty (sub2_fmtty', fmtty_rest) ) ->
      if Fmtty_EBB sub2_fmtty <> Fmtty_EBB sub2_fmtty'
      then raise Type_mismatch;
      let (Fmtty_fmt_EBB (sub_fmtty_rest', fmt')) =
        type_ignored_format_substitution sub_fmtty_rest fmt fmtty_rest
      in
  ...
  | ( Format_subst_ty (sub1_fmtty, sub2_fmtty, sub_fmtty_rest)
    , Format_subst_ty (sub1_fmtty', sub2_fmtty', fmtty_rest) ) ->
      ...
      if Fmtty_EBB (erase_rel sub1_fmtty)
         <> Fmtty_EBB (erase_rel sub1_fmtty')
      then raise Type_mismatch;
      if Fmtty_EBB (erase_rel sub2_fmtty)
         <> Fmtty_EBB (erase_rel sub2_fmtty')
      then raise Type_mismatch;
      ...
      let (Fmtty_fmt_EBB (sub_fmtty_rest', fmt')) =
        type_ignored_format_substitution
          (erase_rel sub_fmtty_rest)
          fmt
          fmtty_rest
      in
      Fmtty_fmt_EBB
        ( Format_subst_ty (sub1_fmtty', sub2_fmtty', symm sub_fmtty_rest')
        , fmt' )
  | End_of_fmtty, fmtty ->
...
let string_of_fmtty fmtty =
  let buf = buffer_create 16 in
  bprint_fmtty buf fmtty; buffer_contents buf
...
and make_padding : type x z a b c d e f.
 ...
 fun k acc fmt pad trans ->
  ...
  | Lit_padding (padty, width) ->
      fun x ->
        let new_acc =
          Acc_data_string (acc, fix_padding padty width (trans x))
        in
        make_printf k new_acc fmt
  | Arg_padding padty ->
      fun w x ->
        let new_acc =
          Acc_data_string (acc, fix_padding padty w (trans x))
        in
        make_printf k new_acc fmt
...
let rec output_acc o acc =
  ...
  | Acc_char_literal (p, c) | Acc_data_char (p, c) ->
      output_acc o p; output_char o c
...
let rec bufput_acc b acc =
  ...
  | Acc_string_literal (p, s) | Acc_data_string (p, s) ->
      bufput_acc b p; Buffer.add_string b s
  | Acc_char_literal (p, c) | Acc_data_char (p, c) ->
      bufput_acc b p; Buffer.add_char b c
...
let rec strput_acc b acc =
  ...
  | Acc_string_literal (p, s) | Acc_data_string (p, s) ->
      strput_acc b p; Buffer.add_string b s
  | Acc_char_literal (p, c) | Acc_data_char (p, c) ->
      strput_acc b p; Buffer.add_char b c
  | Acc_delay (p, f) ->
      strput_acc b p;
...
let open_box_of_string str =
  ...
  then 0, Pp_box
    ...
    let len = String.length str in
    let invalid_box () =
      failwith_message "invalid box description %S" str
    in
...
let fmt_ebb_of_string ?legacy_behavior str =
  ...
  *)
  ...
  and parse_flags : type e f.
      int -> int -> int -> bool -> (_, _, e, f) fmt_ebb =
   ...
   fun pct_ind str_ind end_ind plus hash space ign pad prec padprec symb ->
    ...
    let fmt_result =
      ...
      | 'f' | 'e' | 'E' | 'g' | 'G' | 'F' | 'h' | 'H' ->
          ...
          then
            let ignored =
              Ignored_float (get_pad_opt '_', get_prec_opt ())
            in
            Fmt_EBB (Ignored_param (ignored, fmt_rest))
          else
            let (Padprec_fmt_EBB (pad', prec', fmt_rest')) =
      ...
      | '(' ->
          ...
          then
            let ignored =
              Ignored_format_subst (get_pad_opt '_', sub_fmtty)
            in
            Fmt_EBB (Ignored_param (ignored, fmt_rest))
      ...
      | '[' ->
          ...
          then
            let ignored =
              Ignored_scan_char_set (get_pad_opt '_', char_set)
            in
            Fmt_EBB (Ignored_param (ignored, fmt_rest))
          else Fmt_EBB (Scan_char_set (get_pad_opt '[', char_set, fmt_rest))
      | '-' | '+' | '#' | ' ' | '_' ->
          failwith_message
            "invalid format %S: at character number %d, flag %C is only \
             allowed after the '%%', before padding and precision"
            str
            pct_ind
            symb
      | _ ->
    ...
    then (
      ...
      if (not !prec_used)
         && Precision_EBB prec <> Precision_EBB No_precision
      then
        incompatible_flag
          pct_ind
          str_ind
    ...
    try
      ...
      | '<' ->
          ...
          let formatting =
            ...
            else ( check_open_box sub_fmt; Open_box sub_format )
          in
    ...
    let next_ind, formatting_lit =
      try
        ...
        | '0' .. '9' | '-' -> (
            ...
            | '>' ->
                let s =
                  String.sub str (str_ind - 2) (str_ind_3 - str_ind + 3)
                in
            ...
            | '0' .. '9' | '-' ->
                ...
                let s =
                  String.sub str (str_ind - 2) (str_ind_5 - str_ind + 3)
                in
  ...
  and parse_spaces str_ind end_ind =
    if str_ind = end_ind then unexpected_end_of_format end_ind;
    if str.[str_ind] = ' '
    then parse_spaces (str_ind + 1) end_ind
    else str_ind
  ...
  and add_literal : type a d e f.
      int -> int -> (a, _, _, d, e, f) fmt -> (_, _, e, f) fmt_ebb =
    ...
    | '%' -> (
        ...
        then (* End of format found *) str_ind
          ...
          | '_' -> (
              ...
              | '{' ->
                  let sub_end =
                    search_subformat_end (str_ind + 3) end_ind '}'
                  in
          ...
          | '{' ->
              (* %{...%} sub-format found. *)
              let sub_end =
                search_subformat_end (str_ind + 2) end_ind '}'
              in
              search_subformat_end (sub_end + 2) end_ind c
          | '(' ->
              (* %(...%) sub-format found. *)
  ...
  and incompatible_flag : type a. int -> int -> char -> string -> a =
   fun pct_ind str_ind symb option ->
    ...
    failwith_message
      "invalid format %S: at character number %d, %s is incompatible with \
       '%c' in sub-format %S"
      str
      pct_ind
      option
