...
module Array2 = struct
  ...
  let of_array (type t) kind (layout : t layout) data =
    ...
    for i = 0 to dim1 - 1 do
      let row = data.(i) in
      if Array.length row <> dim2
      then invalid_arg "Bigarray.Array2.of_array: non-rectangular data";
...
module Array3 = struct
  ...
  let of_array (type t) kind (layout : t layout) data =
    ...
    for i = 0 to dim1 - 1 do
      let row = data.(i) in
      if Array.length row <> dim2
      then invalid_arg "Bigarray.Array3.of_array: non-cubic data";
      for j = 0 to dim2 - 1 do
        let col = row.(j) in
        if Array.length col <> dim3
        then invalid_arg "Bigarray.Array3.of_array: non-cubic data";
