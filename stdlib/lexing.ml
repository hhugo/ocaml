...
external c_new_engine
  : lex_tables -> int -> lexbuf -> int = "caml_new_lex_engine"

let engine tbl state buf =
  let result = c_engine tbl state buf in
  (if result >= 0 && buf.lex_curr_p != dummy_pos then
  ...
  (if lexbuf.lex_buffer_len + n > Bytes.length lexbuf.lex_buffer then
     ...
     ((if
       ...
       (* We must grow the buffer.  Doubling its size will provide enough
         ...
         let newlen =
           Int.min (2 * Bytes.length lexbuf.lex_buffer) Sys.max_string_length
         in
...
let from_string ?(with_positions=true) s =
  {
    ...
    (* have to make a copy for compatibility
