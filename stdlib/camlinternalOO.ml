    ...
    (fun met label ->
       ...
       by_label :=
         Labs.add label
           ...
           !by_label) concr_meths concr_meth_labs;
  List.iter2
    (fun met label ->
       by_name := Meths.add met label !by_name;
       by_label := Labs.add label false !by_label) virt_meths virt_meth_labs;
...
let widen table =
  ...
  table.vars <-
    List.fold_left (fun s v -> Vars.add v (Vars.find v table.vars) s) saved_vars
      vars;
  table.methods_by_name <- by_name;
  table.methods_by_label <- by_label;
  table.hidden_meths <-
    List.fold_right
      (fun (lab, _ as met) hm ->
         if List.mem lab virt_meths then hm else met :: hm) table.hidden_meths
      saved_hidden_meths
...
let new_methods_variables table meths vals =
  ...
  for i = 0 to nmeths - 1 do res.(i) <- get_method_label table meths.(i) done;
  for i = 0 to nvals - 1 do res.(i + nmeths) <- new_variable table vals.(i) done;
  res
...
let create_table public_methods =
  if public_methods == magic 0 then
    new_table [| |]
  else (* [public_methods] must be in ascending order for bytecode *)
...
let create_object_opt obj_0 table =
  if (Obj.magic obj_0 : bool) then
    obj_0
  else (* XXX Appel de [obj_block] | Call to [obj_block]  *)
    let obj = Obj.new_block Obj.object_tag table.size in
    (* XXX Appel de [caml_modify] | Call to [caml_modify] *)
    (Obj.set_field obj 0 (Obj.repr table.methods);
     Obj.obj (set_id obj))
...
let build_path n keys tables =
  ...
  let r = ref res in
  for i = 0 to n do r := Cons { key = keys.(i); data = !r; next = Empty } done;
  set_data tables !r;
  res
...
let get_env e n =
  ret (fun obj -> Array.unsafe_get (Obj.magic (Array.unsafe_get obj e) : obj) n)
...
let send_const m x c = ret (fun obj -> sendcache x m (Array.unsafe_get obj 0) c)
  ...
  ret
    (fun obj ->
       sendcache
         ...
         :
         obj) m (Array.unsafe_get obj 0) c)
...
let new_cache table =
  ...
  let n =
    if n mod 2 = 0 || n > 2 + magic table.methods.(1) * 16 / Sys.word_size then
      n
    else
      new_method table
  in
