    ...
    | Asttypes.Nonrecursive ->
        ...
        | Ptype_variant scds ->
            List.iter
              (fun cd ->
                if cd.pcd_res <> None
                then raise (Error (cd.pcd_loc, Nonrec_gadt)) )
              scds
        | _ -> () );
...
let set_fixed_row env loc p decl =
  ...
  in
  if not (Btype.is_Tvar rv)
  then raise (Error (loc, Bad_fixed_type "has no row variable"));
...
let check_type_var loc univ id =
  ...
  if not (List.exists f univ)
  then raise (Error (loc, Wrong_unboxed_type_float))
...
let rec check_unboxed_abstract_arg loc univ ty =
  ...
  | Ttuple args | Tconstr (_, args, _) | Tpackage (_, _, args) ->
...
let rec check_unboxed_gadt_arg loc univ env ty =
  ...
  | Some {desc = Tconstr (p, args, _); _} ->
      ...
      if tydecl.type_kind = Type_abstract
      then List.iter (check_unboxed_abstract_arg loc univ) args
...
let transl_declaration env sdecl id =
  ...
  ( if raw_status.unboxed && not raw_status.default
  then
    match sdecl.ptype_kind with
    | Ptype_abstract ->
        raise
          (Error (sdecl.ptype_loc, Bad_unboxed_attribute "it is abstract"))
    | Ptype_variant [{pcd_args = Pcstr_tuple []; _}] ->
        raise
          (Error
             ( sdecl.ptype_loc
             , Bad_unboxed_attribute "its constructor has no argument" ))
    ...
    | Ptype_variant
        ...
        ()
    | Ptype_variant
        [{pcd_args = Pcstr_record [{pld_mutable = Mutable; _}]; _}] ->
        raise
          (Error (sdecl.ptype_loc, Bad_unboxed_attribute "it is mutable"))
    ...
    | Ptype_variant _ ->
        raise
          (Error
             ( sdecl.ptype_loc
             , Bad_unboxed_attribute "it has more than one constructor" ))
    ...
    | Ptype_record _ ->
        raise
          (Error
             ( sdecl.ptype_loc
             , Bad_unboxed_attribute "it has more than one field" ))
    | Ptype_open ->
        raise
          (Error
             ( sdecl.ptype_loc
        ...
        let make_cstr scstr =
          ...
          let targs, tret_type, args, ret_type, cstr_params =
            make_constructor
              ...
              scstr.pcd_res
          in
          ( if Config.flat_float_array && unbox
          then
  ...
  | Some ty ->
      ...
      then
        raise
          (Error (sdecl.ptype_loc, Recursive_abbrev sdecl.ptype_name.txt))
  );
  { typ_id = id
...
let rec check_constraints_rec env loc visited ty =
  ...
  then ()
    ...
    | Tconstr (path, args, _) ->
        ...
        if not (Ctype.matches env ty ty')
        then raise (Error (loc, Constraint_failed (ty, ty')));
  ...
  | { type_kind = Type_variant _ | Type_record _ | Type_open
    ...
    | Tconstr (path, args, _) -> (
      try
        ...
        in
        if err <> None
        then raise (Error (loc, Definition_mismatch (ty, err)))
      with Not_found ->
        raise (Error (loc, Unavailable_type_constructor path)) )
    ...
    let rec check_regular cpath args prev_exp ty =
      ...
      then (
        ...
        | Tconstr (path', args', _) ->
            ...
            then (
              if not (Ctype.equal env false args args')
              then
                raise
                  (Error
                     ( loc
                     , Parameters_differ
                         (cpath, ty, Ctype.newconstr path args) ))
            ...
            else if to_check path' && not (List.mem path' prev_exp)
            then
              try
                (* Attempt expansion *)
...
let transl_type_decl env rec_flag sdecl_list =
  ...
  let id_list =
    List.map
      (fun sdecl -> Ident.create_scoped ~scope sdecl.ptype_name.txt)
      sdecl_list
  in
  Ctype.begin_def ();
  (* Enter types. *)
  ...
  let id_slots id =
    ...
    | Asttypes.Recursive when warn_unused ->
        ...
        Env.set_type_used_callback name td (fun old_callback ->
            ...
            | None ->
                List.iter
                  (fun (name, d) -> Env.mark_type_used name d)
                  (get_ref slot);
                old_callback () );
        id, Some slot
...
let transl_extension_constructor env type_path type_params typext_params
    priv sext =
  let scope = Ctype.create_scope () in
  ...
  let args, ret_type, kind =
    ...
    | Pext_rebind lid ->
        ...
        let ext_types =
          ...
          :: type_params
        in
        if not (Ctype.equal env true cstr_types ext_types)
        then
          raise
            (Error
               ( lid.loc
...
let transl_type_extension extend env loc styext =
  ...
  List.iter
    (fun ext ->
      ...
      | Some ty ->
          raise
            (Error (ext.ext_loc, Unbound_type_var_ext (ty, ext.ext_type)))
      | None -> () )
    constructors;
  ...
  List.iter
    (fun ext ->
      ...
      try
        Typedecl_variance.check_variance_extension
          env
          type_decl
          ext
          (type_variance, loc)
      ...
      typ =
        (fun iterator core_type ->
          ...
          | Native_repr_attr_present kind ->
              raise
                (Error
                   (core_type.ptyp_loc, Deep_unbox_or_untag_attribute kind))
          | Native_repr_attr_absent -> () );
...
let check_unboxable env loc ty =
  ...
  let all_unboxable_types =
    Btype.fold_type_expr check_type Path.Set.empty ty
  in
  Path.Set.fold
    (fun p () ->
      Location.prerr_warning
...
let transl_value_decl env loc valdecl =
  ...
  let v =
    ...
    | _ ->
        ...
        then
          raise
            (Error (valdecl.pval_type.ptyp_loc, Missing_native_external));
...
let transl_with_constraint env id row_path orig_decl sdecl =
  ...
  if arity_ok
  then List.iter2 (Ctype.unify_var env) params orig_decl.type_params;
  let constraints =
    List.map
      (function
        | ty, ty', loc -> (
          try
            ...
            let ty = cty.ctyp_type in
            let ty' = cty'.ctyp_type in
            Ctype.unify env ty ty'; cty, cty', loc
          with Ctype.Unify tr ->
            raise (Error (loc, Inconsistent_constraint (env, tr))) ))
      sdecl.ptype_cstrs
  in
  ...
  let priv =
    ...
    then Private
    else if arity_ok && orig_decl.type_kind <> Type_abstract
    then orig_decl.type_private
  ...
  let type_kind, type_unboxed =
    if arity_ok && man <> None
    then orig_decl.type_kind, orig_decl.type_unboxed
  ...
  let type_variance =
    try
      Typedecl_variance.compute_decl
        env
        ~check:true
        decl
        (Typedecl_variance.variance_of_sdecl sdecl)
    with Typedecl_variance.Error (loc, err) ->
      raise (Error (loc, Variance err))
  in
  let type_immediate =
    (* Typedecl_immediacy.compute_decl never raises *)
    Typedecl_immediacy.compute_decl env decl
  in
...
let abstract_type_decl arity =
  ...
  let decl =
    ...
    ; type_unboxed = unboxed_false_default_false }
  in
  Ctype.end_def (); generalize_decl decl; decl
...
let report_error ppf = function
          ...
          explain_unbound_gen
            ...
            (fun ppf c ->
              fprintf
                ...
                Printtyp.ident
                c.Types.cd_id
                Printtyp.constructor_arguments
                c.Types.cd_args )
      | Type_record (tl, _), _ ->
          explain_unbound
            ppf
  ...
  | Cannot_unbox_or_untag_type Unboxed ->
      fprintf
        ppf
        "@[Don't know how to unbox this type.@ Only float, int32, int64 \
         and nativeint can be unboxed.@]"
  ...
  | Deep_unbox_or_untag_attribute kind ->
      fprintf
        ...
        "@[The attribute '%s' should be attached to@ a direct argument or \
         result of the primitive,@ it should not occur deeply into its \
         type.@]"
  ...
  | Wrong_unboxed_type_float ->
      fprintf
        ppf
        "@[This type cannot be unboxed because@ it might contain both \
         float and non-float values.@ You should annotate it with \
         [%@%@ocaml.boxed].@]"
  | Boxed_and_unboxed ->
