...
let update_type temp_env env id loc =
  ...
  | Some ty ->
    ...
    (try Ctype.unify env (Ctype.newconstr path params) ty
     with
     | Ctype.Unify err -> raise (Error (loc, Type_clash (env, err))))
...
let set_private_row env loc p decl =
  ...
  let rv =
    ...
    | Tvariant row ->
      ...
      set_type_desc tm (Tvariant { row with  row_fixed = Some Fixed_private });
...
let transl_labels env univars closed lbls =
  ...
  List.iter
    (fun { pld_name = { txt = name; loc } } ->
       ...
       all_labels := String.Set.add name !all_labels) lbls;
  let mk
      { pld_name = name; pld_mutable = mut; pld_type = arg; pld_loc = loc;
...
let transl_constructor_arguments env univars closed =
  function
  | Pcstr_tuple l ->
...
let make_constructor env loc type_path type_params svars sargs sret_type =
  ...
  | None ->
    let (args, targs) = transl_constructor_arguments env None true sargs in
  ...
  | Some sret_type ->
    ...
    in
    let (args, targs) = transl_constructor_arguments env univars closed sargs in
...
let transl_declaration env sdecl (id, uid) =
  ...
  let (tkind, kind) =
    ...
    | Ptype_variant scstrs ->
      ...
      List.iter
        (fun { pcd_name = { txt = name } } ->
           ...
           all_constrs := String.Set.add name !all_constrs) scstrs;
      (if
         List.length
    ...
    | Ptype_record lbls ->
      ...
      let rep =
        ...
        else if List.for_all (fun l -> is_float env l.Types.ld_type) lbls' then
          Record_float
        else
          Record_regular
      in
  ...
  List.iter
    (fun (cty, cty', loc) ->
       ...
       with
       | Ctype.Unify err ->
         raise (Error (loc, Inconsistent_constraint (env, err)))) cstrs;
  Ctype.end_def ();
  (* Add abstract row *)
...
let check_constraints_labels env visited l pl =
  ...
  List.iter
    (fun { Types.ld_id = name; ld_type = ty } ->
       check_constraints_rec env (get_loc (Ident.name name) pl) visited ty) l
...
let check_well_founded env loc path to_check ty =
  ...
  let rec check ty0 parents ty =
    (if TypeSet.mem ty parents then
       (if (*Format.eprintf "@[%a@]@." Printtyp.raw_type_expr ty;*)
    ...
    let (fini, parents) =
      try
        ...
        if TypeSet.subset parents prev then
          true, parents
        else
    ...
    if fini then
      ...
      let arg_exn =
        try
          visited := visited';
          let parents = if rec_ok then TypeSet.empty else TypeSet.add ty parents
          in
  ...
  if decl.type_params = [] then
    ...
    Option.iter
      (fun body ->
         ...
         check_regular path args [] [] body) decl.type_manifest
       ...
       | Ptype_variant cl ->
         List.iter
           (fun pcd ->
              ...
              with
              | Not_found ->
                Hashtbl.add constrs pcd.pcd_name.txt sdecl.ptype_name.txt) cl
       | Ptype_record fl ->
         List.iter
           (fun { pld_name = cname; pld_loc = loc } ->
              ...
              with
              | Not_found -> Hashtbl.add labels cname.txt sdecl.ptype_name.txt)
           fl
       | Ptype_abstract -> ()
       | Ptype_open -> ()) sdecl_list
...
let name_recursion_decls sdecls decls =
  List.map2 (fun sdecl (id, decl) -> id, name_recursion sdecl id decl) sdecls
    decls
...
let check_redefined_unit (td : Parsetree.type_declaration) =
  ...
  match td with
  | { ptype_name = { txt = name }; ptype_manifest = None;
...
let add_types_to_env decls env =
  List.fold_right (fun (id, decl) env -> add_type ~check:true id decl env) decls
    env
...
let transl_type_decl env rec_flag sdecl_list =
  ...
  let ids_list =
    List.map
      (fun sdecl ->
         ...
         Uid.mk ~current_unit:(Env.get_unit_name ())) sdecl_list
  in
  Ctype.begin_def ();
  (* Enter types. *)
  let temp_env = List.fold_left2 (enter_type rec_flag) env sdecl_list ids_list
  in
  ...
  List.iter
    (fun (id, decl) ->
       check_well_founded_decl new_env (List.assoc id id_loc_list)
         (Path.Pident id) decl to_check) decls;
  List.iter (check_abbrev_recursion ~orig_env:env new_env id_loc_list to_check)
    tdecls;
  ...
  let final_decls =
    List.map2 (fun tdecl (_id2, decl) -> { tdecl with  typ_type = decl }) tdecls
      decls
  in
  (* Done *)
...
let transl_extension_constructor
    ~scope env type_path type_params typext_params priv sext
=
  ...
  let (args, ret_type, kind) =
    ...
    | Pext_decl (svars, sargs, sret_type) ->
      let (targs, tret_type, args, ret_type) =
        make_constructor env sext.pext_loc type_path typext_params svars sargs
          sret_type
      in
    ...
    | Pext_rebind lid ->
      ...
      let (res, ret_type) =
        if cdescr.cstr_generalized then
          ...
          Ctype.newconstr type_path typext_params, None
      in
      ...
      (if not cdescr.cstr_generalized then
         ...
         List.iter
           (fun ty ->
              if
                get_desc ty = Tvar (Some "_") && List.exists (eq_type ty) vars
              then
                set_type_desc ty (Tvar None)) typext_params);
      ...
      begin match cdescr.cstr_private, priv with
...
let transl_type_extension extend env loc styext =
  ...
  | Type_open ->
    begin match type_decl.type_private with
    | Private when extend ->
      begin match
        List.find
          (function
           | { pext_kind = Pext_decl _ } -> true
           | { pext_kind = Pext_rebind _ } -> false) styext.ptyext_constructors
      with
      | { pext_loc } ->
  ...
  let type_variance =
    List.map
      (fun v ->
         ...
         not cn, not co, false) type_decl.type_variance
  in
  let err =
  ...
  List.iter2 (Ctype.unify_var env) (Ctype.instance_list type_decl.type_params)
    type_params;
  let constructors =
    List.map
      (transl_extension_constructor ~scope env type_path type_decl.type_params
         type_params styext.ptyext_private) styext.ptyext_constructors
  in
  Ctype.end_def ();
  (* Generalize types *)
  ...
  List.iter
    (fun ext ->
       ...
       Option.iter Ctype.generalize ext.ext_type.ext_ret_type) constructors;
  (* Check that all type variables are closed *)
  List.iter
    (fun ext ->
  ...
  let newenv =
    List.fold_left
      (fun env ext ->
         ...
         Env.add_extension ~check:true ~rebind ext.ext_id ext.ext_type env) env
      constructors
  in
  let tyext =
    {
...
let transl_exception env sext =
  ...
  let rebind = is_rebind ext in
  let newenv = Env.add_extension ~check:true ~rebind ext.ext_id ext.ext_type env
  in
  ext, newenv
...
let transl_type_exception env t =
  ...
  let (contructor, newenv) =
    Builtin_attributes.warning_scope t.ptyexn_attributes
      (fun () -> transl_exception env t.ptyexn_constructor)
  in
  {
...
let rec parse_native_repr_attributes env core_type ty ~global_repr =
  ...
  | Ptyp_poly (_, t), _, _ -> parse_native_repr_attributes env t ty ~global_repr
...
let transl_value_decl env loc valdecl =
  ...
  let v =
    ...
    | _ ->
      ...
      (if
         !Clflags.native_code &&
           prim.prim_arity > 5 && prim.prim_native_name = ""
       then
         raise (Error (valdecl.pval_type.ptyp_loc, Missing_native_external)));
...
let transl_with_constraint
    id ?fixed_row_path ~sig_env ~sig_decl ~outer_env sdecl
=
  ...
  List.iter
    (fun (cty, cty', loc) ->
       ...
       with
       | Ctype.Unify err ->
         raise (Error (loc, Inconsistent_constraint (env, err)))) constraints;
  let priv =
    if sdecl.ptype_private = Private then
      Private
  ...
  (if
     arity_ok &&
       sig_decl.type_kind <> Type_abstract && sdecl.ptype_private = Private
   then
...
let explain_unbound_single ppf tv ty =
  ...
  | Tobject (fi, _) ->
    ...
    if eq_type rv tv then
      trivial ty
    else
      explain_unbound ppf tv tl (fun (_, _, t) -> t) "method"
        (fun (lab, _, _) -> lab ^ ": ")
  | Tvariant row ->
    let row = Btype.row_repr row in
    if eq_type row.row_more tv then
      trivial ty
    else
...
let report_error ppf =
  ...
  | Non_regular { definition; used_as; defined_as; expansions } ->
    let pp_expansion ppf (ty, body) =
      Format.fprintf ppf "%a = %a" Printtyp.type_expr ty Printtyp.type_expr body
    in
    ...
    | [] ->
      fprintf ppf
        "@[<hv>This recursive type is not regular.@ The type constructor \
         ...
         type to be regular.@]" (Path.name definition) !Oprint.out_type
        (Printtyp.tree_of_typexp Type defined_as) !Oprint.out_type
        (Printtyp.tree_of_typexp Type used_as)
    | _ :: _ ->
      fprintf ppf
        "@[<hv>This recursive type is not regular.@ The type constructor \
         %s is defined as@;\
         ...
         type to be regular.@]" (Path.name definition) !Oprint.out_type
        (Printtyp.tree_of_typexp Type defined_as) !Oprint.out_type
        (Printtyp.tree_of_typexp Type used_as) pp_expansions expansions
    end
  ...
  | Separability (Typedecl_separability.Non_separable_evar evar) ->
    ...
    fprintf ppf
      "@[This type cannot be unboxed because@ it might contain both float \
       ...
       should annotate it with [%@%@ocaml.boxed].@]" pp_evar evar
  | Boxed_and_unboxed ->
  ...
  | Invalid_private_row_declaration ty ->
    Format.fprintf ppf
      "@[<hv>This private row type declaration is invalid.@ The type \
       ...
       write explicitly@]@;\
       <1 2>private %a@]" Printtyp.type_expr ty Printtyp.type_expr ty
