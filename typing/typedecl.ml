...
let transl_declaration env sdecl (id, uid) =
  ...
  | Some true -> (
      let bad msg =
        raise (Error (sdecl.ptype_loc, Bad_unboxed_attribute msg))
      in
      match sdecl.ptype_kind with
              ...
              if List.length args <> List.length decl.type_params then
                ...
                | () ->
                    Includecore.type_declarations ~loc ~equality:true env
                      ~mark:true (Path.last path) decl' dpath
...
let explain_unbound_single ppf tv ty =
  ...
  | Tobject (fi, _) ->
      ...
      if eq_type rv tv then trivial ty
      else
  ...
  | Tvariant row ->
      let row = Btype.row_repr row in
      if eq_type row.row_more tv then trivial ty
      else
  ...
  | Non_regular { definition; used_as; defined_as; expansions } -> (
      ...
      | [] ->
          fprintf ppf
            ...
            (Printtyp.tree_of_typexp Type defined_as)
            !Oprint.out_type
  ...
  | Variance (Typedecl_variance.Bad_variance (n, v1, v2)) -> (
      ...
      | Variance_not_satisfied n ->
          fprintf ppf "@[%s@ %s@ The %d%s type parameter"
            ...
            "variances are not satisfied." n (Misc.ordinal_suffix n));
      match n with
      | No_variable -> ()
      | _ ->
