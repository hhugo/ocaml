...
type error =
  ...
  | Pattern_type_clash
      : Errortrace.unification_error * _ pattern_desc option
      -> error
...
let unify_pat_types_return_equated_pairs ?(refine=None) loc env ty ty' =
  try
    ...
    | Some allow_recursive ->
      unify_gadt ~equations_level:(get_gadt_equations_level ()) ~allow_recursive
        env ty ty'
    | None ->
      unify !env ty ty';
      nothing_equated
...
let maybe_add_pattern_variables_ghost loc_let env pv =
  List.fold_right
    (fun { pv_id; _ } env ->
       ...
       if Env.bound_value name env then
         env
       else
...
let enter_variable
    ?(is_module=false) ?(is_as_variable=false) loc name ty attrs
=
  (if
     List.exists (fun { pv_id; _ } -> Ident.name pv_id = name.txt)
       !pattern_variables
   then
     raise (Error (loc, Env.empty, Multiply_bound_variable name.txt)));
...
let sort_pattern_variables vs =
  List.sort
    (fun { pv_id = x; _ } { pv_id = y; _ } ->
       Stdlib.compare (Ident.name x) (Ident.name y)) vs
...
let enter_orpat_variables loc env p1_vs p2_vs =
  ...
  let rec unify_vars p1_vs p2_vs =
    ...
    | { pv_id = x; _ } :: _, { pv_id = y; _ } :: _ ->
      let err =
        if Ident.name x < Ident.name y then
          Orpat_vars (x, vars p2_vs)
        else
          Orpat_vars (y, vars p1_vs)
      in
...
and build_as_type_aux env p =
  ...
  | Tpat_construct (_, cstr, pl, vto) ->
    let keep =
      cstr.cstr_private = Private || cstr.cstr_existentials <> [] || vto <> None
    (* be lazy and keep the type for node constraints *)
    in
    if keep then
      p.pat_type
    else
  ...
  | Tpat_record (lpl, _) ->
    ...
    if lbl.lbl_private = Private then
      p.pat_type
    else
      let ty = newvar () in
...
let solve_constructor_annotation env name_list sty ty_args ty_ex =
  ...
  let ids =
    List.map
      (fun name ->
         ...
         let (id, new_env) =
           Env.enter_type ~scope:expansion_scope name.txt decl !env
         in
         env := new_env;
         { name with  txt = id }) name_list
  in
  begin_def ();
  let (cty, ty, force) = Typetexp.transl_simple_type_delayed !env sty in
  end_def ();
  ...
  let ty_args =
    ...
    | _ ->
      ...
      begin match get_desc (expand_head !env ty2) with
      | Ttuple tyl -> tyl
      | _ -> assert false
      end
  in
        ...
        let rem =
          List.fold_left
            (fun rem tv ->
               ...
               | _ ->
                 raise
                   (Error (cty.ctyp_loc, !env, Unbound_existential (ids, ty))))
            ids ty_ex
        in
        if rem <> [] then
          raise (Error (cty.ctyp_loc, !env, Unbound_existential (ids, ty)))));
  ty_args, Some (ids, cty)

let solve_Ppat_construct
    ~refine env loc constr no_existentials existential_styp expected_ty
=
  ...
  let (ty_args, ty_res, equated_types, existential_ctyp) =
    ...
    | Some (name_list, sty) ->
      let in_pattern =
        if name_list = [] then Some (env, expansion_scope) else None
      in
      let (ty_args, ty_res, ty_ex) = instance_constructor ?in_pattern constr in
      let equated_types = unify_res ty_res in
      let (ty_args, existential_ctyp) =
        solve_constructor_annotation env name_list sty ty_args ty_ex
      in
         ...
         (fun (t1, t2) () ->
            ...
            if not (fully_generic t1 && fully_generic t2) then
              ...
              (Location.prerr_warning loc (Warnings.Not_principal msg);
               raise Warn_only_once)) equated_types
     with
     | Warn_only_once -> ());
...
let solve_Ppat_record_field ~refine loc env label label_lid record_ty =
  ...
  (try unify_pat_types ~refine loc env ty_res (instance record_ty)
   ...
   | Error (_loc, _env, Pattern_type_clash (err, _)) ->
     raise (Error (label_lid.loc, !env, Label_mismatch (label_lid.txt, err))));
  end_def ();
  generalize_structure ty_res;
...
let solve_Ppat_variant ~refine loc env tag constant expected_ty =
  ...
  let row =
    {
      row_fields = [ tag, Reither (constant, arg_type, true, ref None) ];
      row_bound = ();
      row_closed = false;
      row_more = newgenvar ();
      row_fixed = None;
  ...
  let (pats, fields) =
    List.fold_left
      (fun (pats, fields) (l, f) ->
         ...
         | _ -> pats, fields) ([], []) (row_repr row0).row_fields
  in
  let row =
    {
         ...
         raise
           (Error
              (c_lhs.pat_loc,
               env,
               Mixed_value_and_exception_patterns_under_guard))
       | vp, ep -> add_case vals case vp, add_case exns case ep) cases ([], [])
...
= struct
  ...
  let lookup_from_type env type_path usage lid =
    ...
    | Longident.Lident name ->
      begin match List.find (fun (nd, _) -> get_name nd = name) descrs with
      | descr, use ->
        use ();
        descr
      | exception Not_found ->
        ...
        raise
          (Wrong_name_disambiguation
             (env,
              { type_path; name = { lid with  txt = name }; kind; valid_names }))
      end
    | _ -> raise Not_found
  ...
  let warn_if_ambiguous warn lid env lbl rest =
    if Warnings.is_active (Ambiguous_name ([], [], false, "")) then
  ...
  let warn_out_of_scope warn lid env tpath =
    if Warnings.is_active (Name_out_of_scope ("", [], false)) then
      let path_s =
        Printtyp.wrap_printing_env ~error:true env
          (fun () -> Printtyp.string_of_path tpath)
      in
      warn lid.loc
  ...
  *)
  ...
  let disambiguate
      ...
      ?(filter:nonempty_candidate_filter=Result.ok) usage lid env expected_type
      candidates_in_scope
  =
    let lbl =
      ...
      | Some (tpath0, tpath, principal) ->
        ...
        | exception Not_found ->
          ...
          begin match lookup_from_type env tpath usage lid with
          | lbl ->
          ...
          | exception Not_found ->
            ...
            | Ok lbls | Error lbls ->
              ...
              raise
                (Error
                   (lid.loc, env, Name_type_mismatch (kind, lid.txt, tp, tpl)))
            end
          end
        end
    in
...
let disambiguate_label_by_ids closed ids labels : (_, _) result =
  ...
  | [] -> Error labels
  | labels ->
    begin match List.filter check_closed labels with
    | [] -> Error labels
    | labels -> Ok labels
    end
     ...
     | (_, types, ex) :: _ as amb ->
       ...
       if List.for_all (compare_type_path env path) (List.tl paths) then
         ...
         List.iter
           (fun (s, l, ex) ->
              Location.prerr_warning loc
                (Warnings.Ambiguous_name ([ s ], l, false, ex))) amb
     | _ -> ());
  (if !w_scope <> [] then
     Location.prerr_warning loc
...
let map_fold_cont f xs k =
  List.fold_right (fun x k ys -> f x (fun y -> k (y :: ys))) xs
...
type state = { snapshot : snapshot; levels : Ctype.levels; env : Env.t }
...
=
  fun category ~no_existentials ~mode ~env sp expected_ty k ->
    ...
    and rcp k x = k (rp (only_impure category x))
    in
    ...
    | Ppat_constraint
      ...
      assert construction_not_used_in_counterexamples;
      let (cty, ty, ty') =
        solve_Ppat_poly_constraint ~refine env lloc sty expected_ty
      in
    ...
    | Ppat_interval (Pconst_char c1, Pconst_char c2) ->
      ...
      let rec loop c1 c2 =
        if c1 = c2 then
          ...
          or_ ~loc:gloc (constant ~loc:gloc (Pconst_char c1))
    ...
    | Ppat_tuple spl ->
      ...
      map_fold_cont (fun (p, t) -> type_pat Value p t) spl_ann
        (fun pl ->
           rvp k
             {
    ...
    | Ppat_construct (lid, sarg) ->
      ...
      map_fold_cont (fun (p, t) -> type_pat Value p t)
    ...
    | Ppat_variant (tag, sarg) ->
      (if tag = Parmatch.some_private_tag then
         assert
           begin match mode with
           | Normal -> false
           | Counter_example _ -> true
           end);
      let constant = sarg = None in
      let (arg_type, row, pat_type) =
        solve_Ppat_variant ~refine loc env tag constant expected_ty
      in
      let k arg =
        rvp k
          {
            pat_desc =
              Tpat_variant (tag, arg, ref { row with  row_more = newvar () });
            pat_loc = loc;
            pat_extra = [];
            pat_type;
            pat_attributes = sp.ppat_attributes;
      ...
      begin match sarg, arg_type with
    ...
    | Ppat_record (lid_sp_list, closed) ->
      ...
      let type_label_pat (label_lid, label, sarg) k =
        let ty_arg =
          solve_Ppat_record_field ~refine loc env label label_lid record_ty
        in
      ...
      | Normal ->
        k'
          (wrap_disambiguate "This record pattern is expected to have"
             (mk_expected expected_ty)
             (type_label_a_list loc false !env Env.Projection type_label_pat
                expected_type lid_sp_list) make_record_pat)
      | Counter_example { labels; _ } ->
        type_label_a_list ~labels loc false !env Env.Projection type_label_pat
    ...
    | Ppat_array spl ->
      ...
      map_fold_cont (fun p -> type_pat Value p ty_elt) spl
        (fun pl ->
           rvp k
             {
    ...
    | Ppat_or (sp1, sp2) ->
      ...
      | Normal ->
        ...
        let type_pat_rec env sp =
          type_pat category sp expected_ty ~env (fun x -> x)
        in
        let env1 = ref !env in
        ...
        List.iter
          (fun { pv_type; pv_loc; _ } ->
             check_scope_escape pv_loc !env1 outter_lev pv_type) p1_variables;
        List.iter
          (fun { pv_type; pv_loc; _ } ->
             check_scope_escape pv_loc !env2 outter_lev pv_type) p2_variables;
        let alpha_env = enter_orpat_variables loc !env p1_variables p2_variables
        in
    ...
    | Ppat_lazy sp1 ->
      ...
      type_pat Value ~mode:(no_explosion mode) sp1 nv
        (fun p1 ->
           rvp k
             {
    ...
    | Ppat_constraint (sp, sty) ->
      ...
      let (cty, ty, expected_ty') =
        solve_Ppat_constraint ~refine loc env sty expected_ty
      in
      type_pat category sp expected_ty'
        (fun p ->
           (*Format.printf "%a@.%a@."
    ...
    | Ppat_open (lid, p) ->
      ...
      env := new_env;
      type_pat category ~env p expected_ty
        (fun p ->
           let new_env = !env in
    ...
    | Ppat_exception p ->
      type_pat Value p Predef.type_exn
        (fun p_exn ->
           rcp k
             {
...
let type_pat
    category ?no_existentials ?(mode=Normal) ?(lev=get_current_level ()) env sp
    expected_ty
=
...
let partial_pred
    ~lev ~splitting_mode ?(explode=0) env expected_ty constrs labels p
=
  let env = ref env in
  let state = save_state env in
  let mode =
...
let type_pattern_list category no_existentials env spatl expected_tys allow =
  ...
  let type_pat (attrs, pat) ty =
    Builtin_attributes.warning_scope ~ppwarning:false attrs
      (fun () -> type_pat category ~no_existentials new_env pat ty)
  in
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  reset_pattern false;
  let nv = newvar () in
  let pat = type_pat Value ~no_existentials:In_class_args (ref val_env) spat nv
  in
  ...
  let (pv, val_env, met_env) =
    List.fold_right
      (fun
         { pv_id; pv_type; pv_loc; pv_as_var; pv_attributes }
         (pv, val_env, met_env)
       ->
         let check s =
           if pv_as_var then
         ...
         in
         (id', pv_id, pv_type) :: pv, val_env, met_env) !pattern_variables
      ([], val_env, met_env)
  in
...
let type_self_pattern env spat =
  ...
  reset_pattern false;
  let nv = newvar () in
  let pat = type_pat Value ~no_existentials:In_self_pattern (ref env) spat nv in
  List.iter (fun f -> f ()) (get_ref pattern_force);
      ...
      List.for_all
        (fun { c_lhs; c_guard; c_rhs } ->
           is_nonexpansive_opt c_guard &&
             is_nonexpansive c_rhs && not (contains_exception_pat c_lhs)) cases
         ...
         | Tstr_typext te ->
           List.for_all
             (function
              ...
              | { ext_kind = Text_rebind _ } -> true) te.tyext_constructors
         | Tstr_class _ -> false (* could be more precise *)
         | Tstr_attribute _ -> true) str.str_items
  | Tmod_apply _ -> false

and is_nonexpansive_opt =
  function
  | None -> true
  ...
  List.iter
    (fun { vb_expr } ->
       if not (Rec_check.is_valid_recursive_expression ids vb_expr) then
         raise (Error (vb_expr.exp_loc, env, Illegal_letrec_expr))) valbinds

let check_recursive_class_bindings env ids exprs =
  List.iter
    (fun expr ->
       if not (Rec_check.is_valid_class_expr ids expr) then
         raise (Error (expr.cl_loc, env, Illegal_class_expr))) exprs
      ...
      (Error
         (exp.exp_loc,
          ...
          Less_general
            (kind,
             Errortrace.unification_error
               ~trace:[ Ctype.expanded_diff env ~got:ty ~expected:ty_expected ])))
    ...
    let rec loop { exp_loc; exp_desc; exp_extra; _ } =
      ...
      | _ ->
        let loc =
          match
            List.find_opt
              (function Texp_constraint _, _, _ -> true | _ -> false) exp_extra
          with
          | Some (_, loc, _) -> loc
          | None -> exp_loc
        in
  ...
  let doit () =
    ...
    | Tarrow _ ->
      let rec check { exp_desc; exp_loc; exp_extra; _ } =
        if
          List.exists (function Texp_constraint _, _, _ -> true | _ -> false)
            exp_extra
        then
          check_statement ()
        else
...
let may_contain_gadts p =
  exists_ppat (function { ppat_desc = Ppat_construct _ } -> true | _ -> false) p
...
*)
type apply_prim = Apply | Revapply
    ...
    | Tarrow (Nolabel, c, d, _) ->
      ...
      | Tarrow (Nolabel, fl, fr, _) ->
        is_Tvar fl &&
          is_Tvar fr &&
            is_Tvar x &&
              is_Tvar res && Types.eq_type fl x && Types.eq_type fr res
      | _ -> false
      end
    | _ -> false
    end
...
=
  ...
  | Pexp_let (rec_flag, spat_sexp_list, sbody) ->
    let existential_context =
      if rec_flag = Recursive then
        In_rec
      else if List.compare_length_with spat_sexp_list 1 > 0 then
        In_group
      else
        With_attributes
    in
  ...
  | Pexp_fun (l, Some default, spat, sbody) ->
    ...
    let scases =
      [
        ...
        Exp.case
          (Pat.construct ~loc:default_loc
             (mknoloc Longident.(Ldot (Lident "*predef*", "None"))) None)
          default ]
    in
    let sloc =
      {
    ...
    let body =
      Exp.let_ ~loc Nonrecursive
        ~attrs:[ Attr.mk (mknoloc "#default") (PStr []) ] [ Vb.mk spat smatch ]
        sbody
    in
  ...
  | Pexp_apply (sfunct, sargs) ->
    ...
    let (funct, sargs) =
      ...
      | Texp_ident
          (_, _, { val_kind = Val_prim { prim_name = "%revapply" }; val_type }),
        [ Nolabel, sarg; Nolabel, actual_sfunct ]
        when
          is_inferred actual_sfunct && check_apply_prim_type Revapply val_type
        ->
        type_sfunct actual_sfunct, [ Nolabel, sarg ]
      | Texp_ident
          (_, _, { val_kind = Val_prim { prim_name = "%apply" }; val_type }),
    ...
    (try rue exp
     with
     | Error (_, _, Expr_type_clash _) as err ->
       Misc.reraise_preserving_backtrace err
         (fun () -> check_partial_application ~statement:false exp))
  ...
  | Pexp_tuple sexpl ->
    ...
    with_explanation
      (fun () -> unify_exp_types loc env to_unify (generic_instance ty_expected));
    let expl =
      List.map2 (fun body ty -> type_expect env body (mk_expected ty)) sexpl
        subtypes
    in
    re
      {
  ...
  | Pexp_variant (l, sarg) ->
    ...
    (try
       match
         sarg,
         get_desc (expand_head env ty_expected),
         get_desc (expand_head env ty_expected0)
       with
       | Some sarg, Tvariant row, Tvariant row0 ->
         let row = row_repr row
         and row0 = row_repr row0
         in
         begin match
  ...
  | Pexp_record (lid_sexp_list, opt_sexp) ->
    ...
    let (ty_record, expected_type) =
      ...
      let opt_exp_opath =
        ...
        | None -> None
        | Some exp ->
          begin match extract_concrete_record env exp.exp_type with
    ...
    let lbl_exp_list =
      wrap_disambiguate "This record expression is expected to have"
        ...
        (type_label_a_list loc closed env Env.Construct
           ...
           expected_type lid_sexp_list) (fun x -> x)
    in
    with_explanation
      (fun () ->
          ...
          Array.map
            (fun lbl ->
               ...
               | exception Not_found ->
                 ...
                 raise (Error (loc, env, Label_missing missing))) lbl.lbl_all
        in
      ...
      | Some exp ->
        ...
        let unify_kept lbl =
          ...
          | exception Not_found ->
            ...
            with_explanation
              (fun () -> unify_exp_types loc env (instance ty_expected) ty_res2);
            Kept ty_arg1
        in
    ...
    in
    let fields =
      Array.map2 (fun descr def -> descr, def) label_descriptions
        label_definitions
    in
    re
      {
  ...
  | Pexp_field (srecord, lid) ->
    let (record, label, _) = type_label_access env srecord Env.Projection lid in
    let (_, ty_arg, ty_res) = instance_label false label in
  ...
  | Pexp_setfield (srecord, lid, snewval) ->
    let (record, label, expected_type) =
      type_label_access env srecord Env.Mutation lid
    in
    let ty_record = if expected_type = None then newvar () else record.exp_type
    in
  ...
  | Pexp_sequence (sexp1, sexp2) ->
    let exp1 = type_statement ~explanation:Sequence_left_hand_side env sexp1 in
  ...
  | Pexp_coerce (sarg, sty, sty') ->
    ...
    let (arg, ty', cty, cty') =
      ...
      | None ->
        ...
        | _
          when
            free_variables ~env arg.exp_type = [] &&
      ...
      | Some sty ->
        ...
        and (cty', ty', force') = Typetexp.transl_simple_type_delayed env sty'
        in
        (try
        ...
        generalize_structure ty;
        generalize_structure ty';
        type_argument env sarg ty (instance ty), instance ty', Some cty, cty'
    in
    rue
      {
        exp_desc = arg.exp_desc;
      ...
      | Texp_ident (_, _, { val_kind = Val_self (sign, meths, _, _) }) ->
        let (id, typ) =
          ...
          | Self_virtual meths_ref ->
            begin match Meths.find met !meths_ref with
      ...
      | Texp_ident (_, _, { val_kind = Val_anc (sign, meths, cl_num) }) ->
        let id =
          ...
          | exception Not_found ->
            ...
            raise
              (Error
                 (e.pexp_loc, env, Undefined_self_method (met, valid_methods)))
        in
        let typ = Btype.method_type met sign in
        let (self_path, _) =
          Env.find_value_by_name (Longident.Lident ("self-" ^ cl_num)) env
  ...
  | Pexp_setinstvar (lab, snewval) ->
    let (path, mut, cl_num, ty) = Env.lookup_instance_variable ~loc lab.txt env
    in
  ...
  | Pexp_poly (sbody, sty) ->
    ...
    (if sty <> None then
       with_explanation
         (fun () -> unify_exp_types loc env (instance ty) (instance ty_expected)));
    let exp =
      match get_desc (expand_head env ty) with
      | Tpoly (ty', []) ->
  ...
  | Pexp_pack m ->
    let (p, fl) =
      ...
      | Tpackage (p, fl) ->
        (if
           !Clflags.principal &&
             get_level (Ctype.expand_head env ty_expected) < Btype.generic_level
         then
           Location.prerr_warning loc
  ...
  | Pexp_letop { let_ = slet; ands = sands; body = sbody } ->
    ...
    let (cases, partial) =
      type_cases Value env ty_params (mk_expected ty_func_result) true loc
        [ scase ]
    in
    let body =
      match cases with
  ...
  | Pexp_extension
    ...
    | PStr
      ...
      ->
      let path =
        let cd = Env.lookup_constructor Env.Positive ~loc:lid.loc lid.txt env in
...
and type_function
    ?(in_function:(Location.t * type_expr) option) loc attrs env
    ty_expected_explained arg_label caselist
=
  let { ty = ty_expected; explanation } = ty_expected_explained in
  let (loc_fun, ty_fun) =
...
and type_label_access env srecord usage lid =
  ...
  let expected_type =
    ...
    | Record_type (p0, p, _) -> Some (p0, p, is_principal ty_exp)
    | Maybe_a_record_type -> None
    | Not_a_record_type ->
  ...
  let label =
    wrap_disambiguate "This expression has" (mk_expected ty_exp)
      (Label.disambiguate usage lid env expected_type) labels
  in
  ...
  try
    CamlinternalFormatBasics.(
      CamlinternalFormat.(
        ...
        and mk_lid_loc lid = { txt = lid; loc }
        in
        let mk_constr name args =
          let lid = Longident.(Ldot (Lident "CamlinternalFormatBasics", name))
          in
          let arg =
            match args with
            | [] -> None
        ...
        and mk_char chr = mk_cst (Pconst_char chr)
        in
...
and type_label_exp create env loc ty_expected (lid, label, sarg) =
  ...
  (try unify env (instance ty_res) (instance ty_expected)
   with
   | Unify err -> raise (Error (lid.loc, env, Label_mismatch (lid.txt, err))));
  ...
  let arg =
    ...
    | exn when maybe_expansive arg ->
      (try (* Try to retype without propagating ty_arg, cf PR#4862 *)
...
and type_argument ?explanation ?recarg env sarg ty_expected' ty_expected =
  ...
  | Some (safe_expect, lv) ->
    ...
    in
    if not (simple_res || safe_expect) then
      ...
      (unify_exp env { texp with  exp_type = ty_fun } ty_expected;
       if args = [] then
         texp
       else (* eta-expand to avoid side effects *)
  ...
  let type_unknown_arg (ty_fun, typed_args) (lbl, sarg) =
    let (ty_arg, ty_res) =
      ...
      | Tvar _ ->
        ...
        t1, t2
      | Tarrow (l, t1, t2, _)
        when l = lbl || !Clflags.classic && lbl = Nolabel && not (is_optional l)
        ->
        t1, t2
      | td ->
        let ty_fun =
        ...
        let ty_res =
          result_type (!omitted_parameters @ !eliminated_optional_arguments)
            ty_fun
        in
        begin match get_desc ty_res with
        | Tarrow _ ->
        ...
        Some
          (if not optional || is_optional l' then
             ...
             (may_warn sarg.pexp_loc
              ...
              fun () ->
                option_some env
                  (type_argument env sarg (extract_option_type env ty)
        ...
        if ignore_labels then
          ...
          | (l', sarg) :: remaining_sargs ->
            if name = label_name l' || not optional && l' = Nolabel then
              ...
              optional &&
                not
                  (List.exists (fun (l, _) -> name = label_name l)
                     remaining_sargs) &&
                  List.exists (function Nolabel, _ -> true | _ -> false) sargs
            then
    ...
    | _ ->
      ...
      let args =
        (* Force typing of arguments.
           ...
           incorrect. *)
        List.map (function l, None -> l, None | l, Some f -> l, Some (f ()))
          (List.rev typed_args)
      in
...
and type_unpacks
    ?(in_function:(Location.t * type_expr) option) env
    (unpacks : to_unpack list) sbody expected_ty
=
  if unpacks = [] then
    ...
    let (extended_env, tunpacks) =
      List.fold_left
        (fun (env, tunpacks) unpack ->
           ...
           in
           Typetexp.widen context;
           env, (id, unpack.tu_name, pres, modl) :: tunpacks) (env, []) unpacks
    in
    (* ideally, we should catch Expr_type_clash errors
       in type_expect triggered by escaping identifiers from the local module
       and refine them into Scoping_let_module errors
    *)
    ...
    let exp_attributes = [ Ast_helper.Attr.mk (mknoloc "#modulepat") (PStr []) ]
    in
    List.fold_left
...
=
  fun
    category ?in_function env ty_arg ty_res_explained partial_flag loc caselist
  ->
    ...
    let unify_pats ty =
      List.iter
        (fun { typed_pat = pat; pat_type_for_unif = pat_ty; _ } ->
           unify_pat_types pat.pat_loc (ref env) pat_ty ty) half_typed_cases
    in
    unify_pats ty_arg';
    ...
    let cases =
      List.map
        (fun
           { typed_pat = pat; branch_env = ext_env; pat_vars = pvs; unpacks;
             untyped_case = { pc_lhs = _; pc_guard; pc_rhs }; contains_gadt; _ }
         ->
           let ext_env =
...
=
          ...
          pvb_expr =
            {
              pexp_desc =
                Pexp_match
                  ({ pexp_desc = Pexp_ident { txt = Longident.Lident "*opt*" } },
                   _) } } ]
      ->
  ...
  let spatl =
    List.map
      (fun { pvb_pat = spat; pvb_expr = sexp; pvb_attributes = attrs } ->
         attrs,
         begin match spat.ppat_desc, sexp.pexp_desc with
         ...
         | _, Pexp_constraint (_, sty)
           ...
           Pat.constraint_
             ~loc:{ spat.ppat_loc with  Location.loc_ghost = true } spat sty
         | _ -> spat
         end) spat_sexp_list
  in
     ...
     List.iter2
       (fun pat binding ->
          let pat =
            ...
            | _ -> pat
          in
          unify_pat (ref env) pat (type_approx env binding.pvb_expr)) pat_list
       spat_sexp_list);
      ...
      (end_def ();
       ...
       List.map
         (fun pat ->
            ...
            { pat with  pat_type = instance pat.pat_type }) pat_list)
    else
      pat_list
  in
           ...
           (fun () ->
              if not warn_about_unused_bindings then
                ...
                (List.iter
                 ...
                 pat, Some slot))) attrs_list pat_list
  in
  let exp_list =
    List.map2
      (fun { pvb_expr = sexp; pvb_attributes; _ } (pat, slot) ->
         ...
         | _ ->
           ...
           in
           exp, None) spat_sexp_list pat_slot_list
  in
  current_slot := None;
  ...
  List.iter2
    ...
    pat_list (List.map2 (fun (attrs, _) (e, _) -> attrs, e) spatl exp_list);
  let pvs = List.map (fun pv -> { pv with  pv_type = instance pv.pv_type }) pvs
  in
  end_def ();
  List.iter2
    (fun pat (exp, _) ->
  ...
  (if is_recursive then
     List.iter
       (fun { vb_pat = pat } ->
          ...
          | _ -> raise (Error (pat.pat_loc, env, Illegal_letrec_pat))) l);
  List.iter
    (function
...
let type_binding env rec_flag spat_sexp_list =
  ...
  let (pat_exp_list, new_env, _unpacks) =
    type_let ~check:(fun s -> Warnings.Unused_value_declaration s)
...
let report_literal_type_constraint const =
  function
  | Some tr ->
...
let report_unification_error
    ~loc ?sub env err ?type_expected_explanation txt1 txt2
=
...
let report_error ~loc env =
  ...
  | Constructor_arity_mismatch (lid, expected, provided) ->
    Location.errorf ~loc
      "@[The constructor %a@ expects %i argument(s),@ but is applied here to \
       %i argument(s)@]" longident lid expected provided
    ...
    | Tarrow _ ->
      Location.errorf ~loc
        "@[<v>@[<2>This function has type@ %a@]@ @[It is applied to too \
         many arguments;@ %s@]@]" Printtyp.type_expr typ
        "maybe you forgot a `;'."
    | _ ->
      Location.errorf ~loc "@[<v>@[<2>This expression has type@ %a@]@ %s@]"
        Printtyp.type_expr typ "This is not a function; it cannot be applied."
    end
  | Apply_wrong_label (l, ty, extra_info) ->
    ...
    Location.errorf ~loc ~sub:extra_info
      "@[<v>@[<2>The function applied to this argument has type@ %a@]@.This \
       argument cannot be applied %a@]" Printtyp.type_expr ty print_label l
              ...
              (if Path.is_constructor_typath type_path then
                 fprintf ppf
                   "@[The field %s is not part of the record argument for the \
                    %a constructor@]" name.txt Printtyp.type_path type_path
               else
                 fprintf ppf
                   "@[@[<2>%s type@ %a%t@]@ There is no %s %s within type %a@]"
                   eorp Printtyp.type_expr ty
                   (report_type_expected_explanation_opt explanation)
                   (Datatype_kind.label_name kind) name.txt (*kind*)
  ...
  | Not_an_object (ty, explanation) ->
    Location.error_of_printer ~loc
      (fun ppf () ->
         fprintf ppf "This expression is not an object;@ it has type %a"
  ...
  | Virtual_class cl ->
    Location.errorf ~loc "Cannot instantiate the virtual class %a" longident cl
  ...
  | Value_multiply_overridden v ->
    Location.errorf ~loc "The instance variable %s is overridden several times"
      v
            ...
            | ppf ->
              ...
              fprintf ppf
                "This expression cannot be coerced to type@;\
                 <1 2>%a;@ it has type" (Printtyp.type_expansion Type) ty_exp)
  ...
  | Not_a_function (ty, explanation) ->
    Location.errorf ~loc
      ...
      Printtyp.type_expr ty (report_type_expected_explanation_opt explanation)
  ...
  | Abstract_wrong_label { got; expected; expected_type; explanation } ->
    ...
    Location.errorf ~loc
      "@[<v>@[<2>This function should have type@ %a%t@]@,\
       @[but its first argument is %s@ instead of %s%s@]@]" Printtyp.type_expr
      expected_type (report_type_expected_explanation_opt explanation)
      (label ~long:true got) (if second_long then "being " else "")
      (label ~long:second_long expected)
  | Scoping_let_module (id, ty) ->
    Location.errorf ~loc
      "This `let module' expression has type@ %a@ In this type, the locally \
       bound module name %s escapes its scope" Printtyp.type_expr ty id
  | Private_type ty ->
  ...
  | Unexpected_existential (reason, name, types) ->
    let reason_str =
      match reason with
      | In_class_args -> "Existential types are not allowed in class arguments"
      | In_class_def ->
        "Existential types are not allowed in bindings inside class \
         definition"
      | In_self_pattern -> "Existential types are not allowed in self patterns"
      | At_toplevel -> "Existential types are not allowed in toplevel bindings"
      | In_group ->
    ...
    | example ->
      Location.errorf ~loc
        "%s,@ but this pattern introduces the existential type %s." reason_str
        example
    | exception Not_found ->
      Location.errorf ~loc
        "%s,@ but the constructor %s introduces existential types." reason_str
        name
    end
  | Invalid_interval ->
    Location.errorf ~loc
  ...
  | Unbound_existential (ids, ty) ->
    Location.errorf ~loc "@[<2>%s:@ @[type %s.@ %a@]@]"
      "This type does not bind all existentials in the constructor"
      (String.concat " " (List.map Ident.name ids)) Printtyp.type_expr ty
  | Missing_type_constraint ->
