...
type error =
  ...
  | Abstract_wrong_label of
      arg_label * type_expr * type_forcing_context option
...
let iter_expression f e =
  let rec expr e =
    ...
    | Pexp_match (e, pel) | Pexp_try (e, pel) -> expr e; List.iter case pel
    ...
    | Pexp_unreachable -> ()
  and case {pc_lhs = _; pc_guard; pc_rhs} = may expr pc_guard; expr pc_rhs
  and binding_op {pbop_exp; _} = expr pbop_exp
  ...
  and class_expr ce =
    ...
    | Pcl_apply (ce, lel) ->
        class_expr ce;
        List.iter (fun (_, e) -> expr e) lel
    | Pcl_let (_, pel, ce) -> List.iter binding pel; class_expr ce
    | Pcl_open (_, ce) | Pcl_constraint (ce, _) -> class_expr ce
    | Pcl_extension _ -> ()
  and class_field cf =
...
let enter_variable ?(is_module = false) ?(is_as_variable = false) loc name
    ty attrs =
  if List.exists
  ...
  then (
    ...
    then raise (Error (loc, Env.empty, Modules_not_allowed));
      ...
      then p.pat_type
        ...
        let do_label lbl =
          ...
          then
            ...
            let _, ty_arg', ty_res' = instance_label false lbl in
            unify env ty_arg ty_arg'; unify_pat env p ty_res'
        in
...
struct
  ...
  let disambiguate ?(warn = Location.prerr_warning) ?scope lid env opath
      lbls =
    ...
    let lbl =
      ...
      | Some (tpath0, tpath, pr) -> (
          ...
          try
            ...
            then
              ...
              | (lbl', _use') :: rest ->
                  ...
                  then warn_pr ()
                  else Printtyp.Conflicts.reset ();
                  let paths = ambiguous_types env lbl rest in
                  let expansion =
                    Format.asprintf "%t" Printtyp.Conflicts.print
                  in
                  if paths <> []
                  then
                    warn
          ...
          with Not_found -> (
            ...
            with Not_found ->
              if lbls = []
              then unbound_name_error env lid
              else
...
let wrap_disambiguate kind ty f x =
  ...
  with Error (loc, env, Wrong_name ("", _, tk, tp, name, valid_names)) ->
    raise
      (Error (loc, env, Wrong_name (kind, ty, tk, tp, name, valid_names)))
...
let disambiguate_lid_a_list loc closed env opath lid_a_list =
  ...
  let process_label lid =
    ...
    let ok, labels =
      match opath with
      | Some (_, _, true) ->
          true, scope (* disambiguate only checks scope *)
      | _ -> disambiguate_label_by_ids (opath = None) closed ids scope
    in
    if ok
    then Label.disambiguate lid env opath labels ~warn ~scope
...
let check_recordpat_labels loc lbl_pat_list closed =
  ...
  | (_, label1, _) :: _ ->
      ...
      let check_defined (_, label, _) =
        ...
        then
          raise
            (Error (loc, Env.empty, Label_multiply_defined label.lbl_name))
      ...
      then (
        ...
        for i = 0 to Array.length all - 1 do
          if not defined.(i)
          then undefined := all.(i).lbl_name :: !undefined
        done;
        if !undefined <> []
        then
...
let rec type_pat ?(exception_allowed = false) ~constrs ~labels
    ~no_existentials ~mode ~explode ~env sp expected_ty k =
...
and type_pat_aux ~exception_allowed ~constrs ~labels ~no_existentials ~mode
  ...
  let type_pat ?(exception_allowed = false) ?(constrs = constrs)
      ?(labels = labels) ?(mode = mode') ?(explode = explode) ?(env = env) =
    type_pat
      ~exception_allowed
      ~constrs
      ~labels
  ...
  | Ppat_any ->
      ...
      then
        let sp, constrs, labels =
          try Parmatch.ppat_of_type !env expected_ty
          with Parmatch.Empty -> raise (Error (loc, !env, Empty_pattern))
        in
        if sp.ppat_desc = Parsetree.Ppat_any
        then k' Tpat_any
  ...
  | Ppat_var name ->
      ...
      let id =
        ...
        then Ident.create_local name.txt
  ...
  | Ppat_unpack name ->
      ...
      let id =
        enter_variable loc name t ~is_module:true sp.ppat_attributes
      in
      rp
        k
        { pat_desc = Tpat_var (id, name)
  ...
  | Ppat_interval (Pconst_char c1, Pconst_char c2) ->
      ...
      let rec loop c1 c2 =
        if c1 = c2
        then constant ~loc:gloc (Pconst_char c1)
        else
          or_
            ~loc:gloc
  ...
  | Ppat_construct (lid, sarg) ->
      ...
      Builtin_attributes.check_alerts
        loc
      ...
      then unify_head_only loc !env (instance expected_ty) constr;
      let sargs =
        match sarg with
        | None -> []
      ...
      let rec check_non_escaping p =
        match p.ppat_desc with
        | Ppat_or (p1, p2) -> check_non_escaping p1; check_non_escaping p2
  ...
  | Ppat_record (lid_sp_list, closed) ->
      ...
      let type_label_pat (label_lid, label, sarg) k =
        ...
        ( try unify_pat_types loc !env ty_res (instance record_ty)
          with Error (_loc, _env, Pattern_type_clash (trace, _)) ->
            raise
              (Error
                 (label_lid.loc, !env, Label_mismatch (label_lid.txt, trace)))
        );
        end_def ();
        generalize_structure ty_res;
  ...
  | Ppat_array spl ->
      ...
      end_def ();
      generalize_structure expected_ty;
      unify_pat_types loc !env (Predef.type_array ty_elt) expected_ty;
      map_fold_cont
        (fun p -> type_pat p ty_elt)
        spl
        (fun pl ->
          rp
            k
  ...
  | Ppat_exception p ->
      ...
      then raise (Error (loc, !env, Exception_pattern_disallowed))
      else
...
let type_pat ?exception_allowed ?no_existentials ?constrs ?labels
    ?(mode = Normal) ?(explode = 0) ?(lev = get_current_level ()) env sp
    expected_ty =
  Misc.protect_refs
...
let partial_pred ~lev ?mode ?explode env expected_ty constrs labels p =
  ...
  try
    ...
    Some typed_p
  with Error _ -> set_state state env; None
...
let type_self_pattern cl_num privty val_env met_env par_env spat =
  ...
  let nv = newvar () in
  let pat =
    type_pat ~no_existentials:In_self_pattern (ref val_env) spat nv
  in
  List.iter (fun f -> f ()) (get_ref pattern_force);
...
let rec is_nonexpansive exp =
  ...
  | Texp_new (_, _, cl_decl)
    when Ctype.class_type_arity cl_decl.cty_type > 0 ->
      true
      ...
      List.for_all
        (fun item ->
          ...
          | Tstr_value (_, pat_exp_list) ->
              List.for_all
                (fun vb -> is_nonexpansive vb.vb_expr)
                pat_exp_list
...
let check_recursive_bindings env valbinds =
  ...
  List.iter
    (fun {vb_expr} ->
      if not (Rec_check.is_valid_recursive_expression ids vb_expr)
...
let rec type_approx env sexp =
  ...
  | Pexp_constraint (e, sty) ->
      ...
      ( try unify env ty ty1
        with Unify trace ->
          raise
            (Error (sexp.pexp_loc, env, Expr_type_clash (trace, None, None)))
      );
      ty1
...
let check_univars env expans kind exp ty_expected vars =
  if expans && not (is_nonexpansive exp)
  then generalize_expansive env exp.exp_type;
        ...
        let rec check {exp_desc; exp_loc; exp_extra; _} =
          ...
          then check_statement ()
            ...
            | Texp_ident _ | Texp_constant _ | Texp_tuple _
             ...
             |Texp_field _ | Texp_setfield _ | Texp_array _ | Texp_while _
             |Texp_for _ | Texp_instvar _ | Texp_setinstvar _
             |Texp_override _ | Texp_assert _ | Texp_lazy _
            ...
            | Texp_try (e, cases) ->
                check e;
                List.iter (fun {c_rhs; _} -> check c_rhs) cases
            | Texp_ifthenelse (_, e1, Some e2) -> check e1; check e2
            | Texp_let (_, _, e)
             |Texp_sequence (_, e)
             |Texp_open (_, e)
             |Texp_letexception (_, e)
...
let generalizable level ty =
  let rec check ty =
    ...
    then raise Exit
    else ( mark_type_node ty; iter_type_expr check ty )
  in
  try check ty; unmark_type ty; true with Exit -> unmark_type ty; false
...
let contains_variant_either ty =
  ...
  in
  try loop ty; unmark_type ty; false with Exit -> unmark_type ty; true
...
let contains_gadt p =
  let check p =
    match p.pat_desc with
    | Tpat_construct (_, cd, _) when cd.cstr_generalized -> raise Exit
    | _ -> ()
  in
...
let duplicate_ident_types half_typed_cases env =
  let caselist =
    List.filter
      (fun {typed_pat; _} -> contains_gadt typed_pat)
      half_typed_cases
  in
...
let unify_exp env exp expected_ty =
  ...
  with Error (loc, env, Expr_type_clash (trace, tfc, None)) ->
    raise
      (Error (loc, env, Expr_type_clash (trace, tfc, Some exp.exp_desc)))
...
and type_expect_ ?in_function ?(recarg = Rejected) env sexp
  ...
  | Pexp_ident lid ->
      ...
      let exp_desc =
        ...
        | Val_unbound Val_unbound_ghost_recursive ->
            ...
            else raise Typetexp.(Error (loc, env, Unbound_value lid.txt))
  ...
  | Pexp_constant (Pconst_string (str, _) as cst) ->
      ...
      let is_format =
        ...
        | Tconstr (path, _, _) when Path.same path fmt6_path ->
            if !Clflags.principal && ty_exp.level <> generic_level
            then
              Location.prerr_warning
                loc
  ...
  | Pexp_let (rec_flag, spat_sexp_list, sbody) ->
      ...
      let pat_exp_list, new_env, unpacks =
        type_let existential_context env rec_flag spat_sexp_list scp true
      in
      let body =
        type_expect
          new_env
          (wrap_unpacks sbody unpacks)
          ty_expected_explained
      in
      let () =
        if rec_flag = Recursive
        then check_recursive_bindings env pat_exp_list
      in
      re
  ...
  | Pexp_match (sarg, caselist) ->
      ...
      end_def ();
      if not (is_nonexpansive arg)
      then generalize_expansive env arg.exp_type;
        ...
        | None -> (
          ...
          | Some exp -> (
            ...
            | Some (_, p', _) as op ->
                ...
                end_def (); generalize_structure ty; ty, op ) )
        | op -> ty_expected, op
      in
              ...
              Array.map
                (fun lbl ->
                  ...
                  | exception Not_found ->
                      let present_indices =
                        List.map
                          (fun (_, lbl, _) -> lbl.lbl_pos)
                          lbl_exp_list
                      in
                      let label_names =
                        extract_label_names env ty_expected
                      in
  ...
  | Pexp_setfield (srecord, lid, snewval) ->
      ...
      then raise (Error (loc, env, Label_not_mutable lid.txt));
      Builtin_attributes.check_deprecated_mutable
        lid.loc
  ...
  | Pexp_constraint (sarg, sty) ->
      ...
      rue
        ...
        ; exp_env = env
        ; exp_extra =
            (Texp_constraint cty, loc, sexp.pexp_attributes)
            :: arg.exp_extra }
  ...
  | Pexp_send (e, {txt = met}) -> (
      ...
      try
        let meth, exp, typ =
          ...
          | Texp_ident
              (_path, _, {val_kind = Val_self (meths, _, _, privty)}) ->
                ...
                , (path, _) ) ->
                  ...
                  let exp =
                    Texp_apply
                      ( { exp_desc =
                            Texp_ident
                              (Path.Pident method_id, lid, method_desc)
                        ; exp_loc = loc
                        ; exp_extra = []
        ...
        in
        if !Clflags.principal then ( end_def (); generalize_structure typ );
        let typ =
          match repr typ with
          | {desc = Tpoly (ty, [])} -> instance ty
          | {desc = Tpoly (ty, tl); level = l} ->
              if !Clflags.principal && l <> generic_level
              then
                Location.prerr_warning
                  loc
  ...
  | Pexp_poly (sbody, sty) ->
      ...
      in
      if !Clflags.principal then ( end_def (); generalize_structure ty );
      if sty <> None
      then
        with_explanation (fun () ->
      ...
      let exp =
        ...
        | Tpoly (ty', tl) ->
            ...
            if !Clflags.principal
            then ( end_def (); generalize_structure ty'' );
            let exp = type_expect env sbody (mk_expected ty'') in
            end_def ();
  ...
  | Pexp_letop {let_ = slet; ands = sands; body = sbody} ->
      ...
      let ty_func =
        newty (Tarrow (Nolabel, ty_params, ty_func_result, Cok))
      in
  ...
  | Pexp_extension
    ...
    | PStr
        ...
        let path =
          match
            (Typetexp.find_constructor env lid.loc lid.txt).cstr_tag
          with
...
and type_ident env ?(recarg = Rejected) lid =
  ...
  then
    ...
    let annot =
      if dloc.Location.loc_ghost
      then Annot.Iref_external
...
and type_binding_op_ident env s =
  ...
  let path =
    ...
    | Val_unbound Val_unbound_ghost_recursive ->
        let desc_loc = desc.Types.val_loc in
        (* Only display the "missing rec" hint for non-ghost code *)
        if (not loc.Location.loc_ghost) && not desc_loc.Location.loc_ghost
        then
          raise
            Typetexp.(
              Error (loc, env, Unbound_value_missing_rec (lid.txt, desc_loc)))
        else raise Typetexp.(Error (loc, env, Unbound_value lid.txt))
    | _ -> path
  in
  path, desc

and type_function ?in_function loc attrs env ty_expected_explained l
    caselist =
  let {ty = ty_expected; explanation} = ty_expected_explained in
  let loc_fun, ty_fun =
  ...
  let ty_arg, ty_res =
    ...
    with Unify _ -> (
      ...
      | {desc = Tarrow _} as ty ->
          raise
            (Error (loc, env, Abstract_wrong_label (l, ty, explanation)))
      | _ ->
          raise
            (Error
               ( loc_fun
               , env
               , Too_many_arguments
                   (in_function <> None, ty_fun, explanation) )) )
  in
  let ty_arg =
    if is_optional l
    then (
  ...
  try
    CamlinternalFormatBasics.(
      CamlinternalFormat.(
        ...
        and mk_precision : type x y.
            (x, y) precision -> Parsetree.expression =
         fun prec ->
          match prec with
...
and type_argument ?explanation ?recarg env sarg ty_expected' ty_expected =
  ...
  | {desc = Tarrow (Nolabel, ty_arg, ty_res, _); level = lv}
    when is_inferred sarg ->
      ...
      if not (simple_res || no_labels ty_res)
      then (
...
and type_application env funct sargs =
  ...
  let rec type_unknown_args
    ...
    | (l1, sarg1) :: sargl ->
        let ty1, ty2 =
          ...
          | td -> (
              let ty_fun =
                match td with Tarrow _ -> newty td | _ -> ty_fun
              in
              let ty_res = result_type (omitted @ !ignored) ty_fun in
              match ty_res.desc with
              | Tarrow _ ->
                  if !Clflags.classic || not (has_label l1 ty_fun)
                  then
                    raise
                      (Error
                         ( sarg1.pexp_loc
    ...
    | ( ({desc = Tarrow (l, ty, ty_fun, com); level = lv} as ty_fun')
      ...
      when (sargs <> [] || more_sargs <> []) && commu_repr com = Cok ->
        ...
        let sargs, more_sargs, arg =
          if ignore_labels && not (is_optional l)
          then
            ...
            | (l', sarg0) :: _, _ ->
                raise
                  (Error
                     (sarg0.pexp_loc, env, Apply_wrong_label (l', ty_old)))
            | _, (l', sarg0) :: more_sargs ->
                if l <> l' && l' <> Nolabel
                then
                  raise
                    (Error
                       (sarg0.pexp_loc, env, Apply_wrong_label (l', ty_fun')))
                else
                  ( []
                  , more_sargs
                  , Some
                      (fun () -> type_argument ~explanation env sarg0 ty ty0)
                  )
            | _ -> assert false
          else
            try
        ...
        in
        let omitted =
          if arg = None then (l, ty, lv) :: omitted else omitted
        in
  ...
  | _ ->
      ...
      if ignore_labels
      then type_args [] [] ty (instance ty) ty [] sargs
      else type_args [] [] ty (instance ty) ty sargs []
...
and type_construct env loc lid sarg ty_expected_explained attrs =
  ...
  then raise (Error (loc, env, Private_type ty_res));
...
and type_cases ?exception_allowed ?in_function env ty_arg ty_res
  ...
  let ty_res, duplicated_ident_types =
    ...
    then correct_levels ty_res, duplicate_ident_types half_typed_cases env
  ...
  (* type bodies *)
  let in_function =
    if List.length caselist = 1 then in_function else None
  in
  let cases =
    List.map
      (fun { typed_pat = pat
        ...
        let ext_env =
          if contains_gadt
          then Env.do_copy_types duplicated_ident_types ext_env
          else ext_env
        in
        let ext_env =
        ...
        let ty_res' =
          ...
          then (
            ...
            end_def (); generalize_structure ty; ty )
          else if contains_gadt
          then
      ...
      half_typed_cases
  in
  ( if !Clflags.principal || does_contain_gadt
  then
...
and type_let ?(check = fun s -> Warnings.Unused_var s)
    ...
    existential_context env rec_flag spat_sexp_list scope allow =
  let open Ast_helper in
  begin_def ();
  if !Clflags.principal then begin_def ();
  let is_fake_let =
...
and type_andops env sarg sands expected_ty =
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | {pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _} :: rest ->
        ...
        let ty_result = newvar () in
        let ty_rest_fun =
          newty (Tarrow (Nolabel, ty_arg, ty_result, Cok))
        in
...
let type_let existential_ctx env rec_flag spat_sexp_list scope =
  let pat_exp_list, new_env, _unpacks =
    type_let existential_ctx env rec_flag spat_sexp_list scope false
  in
  pat_exp_list, new_env
...
let report_literal_type_constraint ppf const = function
  | Some Unification_trace.{expected = {t = {desc = Tconstr (typ, [], _)}}}
    ->
...
let report_expr_type_clash_hints ppf exp diff =
  match exp with
  | Some (Texp_constant const) ->
      report_literal_type_constraint ppf const diff
  | _ -> ()

let report_pattern_type_clash_hints ppf pat diff =
  match pat with
  | Some (Tpat_constant const) ->
      report_literal_type_constraint ppf const diff
  | _ -> ()
...
let report_numeric_operator_clash_hints actual_type operator =
  ...
  let stdlib_qualified mod_ val_ =
    Path.Pdot (Path.Pdot (stdlib, mod_), val_)
  in
  ...
  let hint_std () =
    ...
    hint
      ...
      ~mul:(qualified "*.")
      ~div:(qualified "/.")
      ~mod_:(stdlib_qualified "Float" "rem")
      ()
  in
  let expecting = Path.same actual_type in
  if expecting Predef.path_int32
  then hint_qualified "Int32"
  else if expecting Predef.path_int64
  then hint_qualified "Int64"
  else if expecting Predef.path_nativeint
  then hint_qualified "Nativeint"
  else if expecting Predef.path_float
  then hint_std ()
  else None
...
let report_application_clash_hints diff expl =
  ...
  | ( Some (Application {exp_desc = Texp_ident (p, _, _); exp_loc = loc; _})
    ...
    -> (
    match report_numeric_operator_clash_hints typ p with
    | Some txt -> [{txt; loc}]
    | None -> [] )
  | _ -> []
...
let report_unification_error ~loc ?sub env trace ?type_expected_explanation
    txt1 txt2 =
...
let report_error ~loc env = function
  | Constructor_arity_mismatch (lid, expected, provided) ->
      Location.errorf
        ~loc
        "@[The constructor %a@ expects %i argument(s),@ but is applied \
         here to %i argument(s)@]"
        longident
        lid
        expected
        provided
      ...
      report_unification_error
        ...
        (function
          | ppf ->
              fprintf
                ppf
                "The variable %s on the left-hand side of this or-pattern \
                 has type"
                (Ident.name id))
        (function
  ...
  | Apply_non_function typ -> (
      ...
      | Tarrow _ ->
          Location.errorf
            ~loc
            "@[<v>@[<2>This function has type@ %a@]@ @[It is applied to \
  ...
  | Apply_wrong_label (l, ty) ->
      ...
      Location.errorf
        ~loc
        "@[<v>@[<2>The function applied to this argument has type@ \
         %a@]@.This argument cannot be applied %a@]"
        type_expr
        ty
        print_label
        l
  ...
  | Wrong_name (eorp, ty_expected, kind, p, name, valid_names) ->
      Location.error_of_printer
        ...
        (fun ppf () ->
          ...
          reset_and_mark_loops ty;
          if Path.is_constructor_typath p
          then
            fprintf
              ppf
  ...
  | Undefined_method (ty, me, valid_methods) ->
      ...
      Location.error_of_printer
        ...
        (fun ppf () ->
          fprintf
            ppf
            "@[<v>@[This expression has type@;<1 2>%a@]@,It has no method \
             %s@]"
            type_expr
            ty
            me;
  ...
  | Instance_variable_not_mutable (b, v) ->
      if b
      then Location.errorf ~loc "The instance variable %s is not mutable" v
      else Location.errorf ~loc "The value %s is not an instance variable" v
  | Not_subtype (tr1, tr2) ->
      Location.error_of_printer
        ~loc
        (fun ppf () ->
  ...
  | Too_many_arguments (in_function, ty, explanation) ->
      ...
      then
        ...
        Location.errorf
          ~loc
          "This expression should not be a function,@ the expected type \
           is@ %a%t"
          type_expr
          ty
          (report_type_expected_explanation_opt explanation)
  ...
  | Scoping_let_module (id, ty) ->
      ...
      Location.errorf
        ~loc
        "This `let module' expression has type@ %a@ In this type, the \
         locally bound module name %s escapes its scope"
        type_expr
        ty
        id
  | Masked_instance_variable lid ->
      Location.errorf
        ~loc
        "The instance variable %a@ cannot be accessed from the definition \
         of another instance variable"
        longident
        lid
  | Private_type ty ->
      Location.errorf
  ...
  | Unexpected_existential (reason, name, types) -> (
      let reason_str =
        ...
        | In_group ->
            "Existential types are not allowed in \"let ... and ...\" \
             bindings"
        | In_rec ->
  ...
  | Invalid_extension_constructor_payload ->
      Location.errorf
        ~loc
        "Invalid [%%extension_constructor] payload, a constructor is \
         expected."
  | Not_an_extension_constructor ->
      Location.errorf
        ~loc
  ...
  | Literal_overflow ty ->
      Location.errorf
        ~loc
        "Integer literal exceeds the range of representable integers of \
         type %s"
        ty
  | Unknown_literal (n, m) ->
