      ...
      if lbl.lbl_private = Private then p.pat_type
        ...
        let do_label lbl =
          ...
          let refinable =
            ...
            && match get_desc lbl.lbl_arg with Tpoly _ -> false | _ -> true
          in
          if refinable then
...
let solve_Ppat_construct ~refine env loc constr no_existentials existential_styp
    expected_ty =
...
let get_constr_type_path ty =
  match get_desc ty with Tconstr (p, _, _) -> p | _ -> assert false
...
let type_label_a_list ?labels loc closed env usage type_lbl_a expected_type
    lid_a_list k =
  let lbl_a_list =
  ...
  | Ppat_record (lid_sp_list, closed) -> (
      ...
      | Counter_example { labels; _ } ->
          type_label_a_list ~labels loc false !env Env.Projection type_label_pat
            expected_type lid_sp_list (fun lbl_pat_list ->
              k' (make_record_pat lbl_pat_list)))
  | Ppat_array spl ->
      ...
      | Counter_example { splitting_mode; _ } -> (
          ...
          if must_split then split_or sp
            ...
            let type_pat_result env sp : (_, abort_reason) result =
              ...
              match
                type_pat category ~mode sp expected_ty ~env (fun x -> x)
              with
              | res -> Ok res
...
let rec list_labels_aux env visited ls ty_fun =
  ...
  if TypeSet.mem ty visited then (List.rev ls, false)
  else
    match get_desc ty with
    | Tarrow (l, _, ty_res, _) ->
         ...
         ( exp.exp_loc,
           ...
           Less_general
             ( kind,
               Errortrace.unification_error
                 ~trace:
                   [ Ctype.expanded_diff env ~got:ty ~expected:ty_expected ] )
         ))
...
let may_contain_gadts p =
  exists_ppat
    (function { ppat_desc = Ppat_construct _ } -> true | _ -> false)
    p
...
let check_apply_prim_type prim typ =
  ...
  | Tarrow (Nolabel, a, b, _) -> (
      ...
      | Tarrow (Nolabel, c, d, _) -> (
          let f, x, res =
            match prim with Apply -> (a, c, d) | Revapply -> (c, a, d)
          in
  ...
  | Pexp_coerce (sarg, sty, sty') ->
      ...
      let arg, ty', cty, cty' =
        ...
        | None ->
            ...
            (match (arg.exp_desc, !self_coercion, get_desc ty') with
        ...
        | Some sty ->
            ...
            (try
               ...
               force' ();
               force'' ()
             with Subtype err -> raise (Error (loc, env, Not_subtype err)));
            end_def ();
            generalize_structure ty;
            generalize_structure ty';
...
and type_function ?(in_function : (Location.t * type_expr) option) loc attrs env
...
and type_argument ?explanation ?recarg env sarg ty_expected' ty_expected =
  ...
  in
  let may_coerce =
    if not (is_inferred sarg) then None
    else
      let work () =
...
and type_unpacks ?(in_function : (Location.t * type_expr) option) env
    (unpacks : to_unpack list) sbody expected_ty =
  if unpacks = [] then type_expect ?in_function env sbody expected_ty
  else
    let ty = newvar () in
    (* remember original level *)
          ...
          if
            not
              (List.exists
                 ...
                 pat_extra)
          then check_partial_application ~statement:false vb_expr
      | _ -> ())
    l;
  (l, new_env, unpacks)
  ...
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | { pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _ } :: rest ->
        ...
        (try unify env (instance ty_result) (instance expected_ty)
         with Unify err -> raise (Error (loc, env, Bindings_type_clash err)));
        let andop =
          {
            bop_op_name = sop;
            bop_op_path = op_path;
...
let report_unification_error ~loc ?sub env err ?type_expected_explanation txt1
    txt2 =
