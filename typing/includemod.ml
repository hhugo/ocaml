...
module Error = struct
  type functor_arg_descr = Anonymous | Named of Path.t | Unit
...
let value_descriptions ~loc env ~mark subst id vd1 vd2 =
  ...
  try Ok (Includecore.value_descriptions ~loc env (Ident.name id) vd1 vd2)
  with
...
let class_declarations ~old_env:_ env subst decl1 decl2 =
  ...
  | [] -> Ok Tcoerce_none
  | reason -> Error Error.(Core (Class_declarations (diff decl1 decl2 reason)))
...
let expand_module_alias ~strengthen env path =
  match
    if strengthen then
      Env.find_strengthened_module ~aliasable:true path env
    else
...
let rec print_coercion ppf c =
  ...
  | Tcoerce_structure (fl, nl) ->
    pr "@[<2>struct@ %a@ %a@]" (print_list print_coercion2) fl
...
let simplify_structure_coercion cc id_pos_list =
  let rec is_identity_coercion pos =
    function
    | [] -> true
    | (n, c) :: rem ->
      n = pos && c = Tcoerce_none && is_identity_coercion (pos + 1) rem
  in
  if is_identity_coercion 0 cc then
    Tcoerce_none
  else
...
and try_modtypes ~in_eq ~loc env ~mark subst mty1 mty2 =
  ...
  | Mty_alias p1, Mty_alias p2 ->
    if Env.is_functor_arg p2 env then
      ...
      Ok Tcoerce_none
  | Mty_alias p1, _ ->
    begin match Env.normalize_module_path (Some Location.none) env p1 with
    ...
    | p1 ->
      ...
      | Ok mty1 ->
        begin match
          strengthened_modtypes ~in_eq ~loc ~aliasable:true env ~mark subst mty1
            p1 mty2
        with
        | Ok _ as x -> x
  ...
  | Mty_ident p1, _ ->
    ...
    | Some p1 -> try_modtypes ~in_eq ~loc env ~mark subst p1 mty2
  ...
  | Mty_functor (param1, res1), Mty_functor (param2, res2) ->
    let (cc_arg, env, subst) =
      functor_param ~in_eq ~loc env ~mark:(negate_mark mark) subst param1 param2
    in
    ...
    | _, Error { Error.symptom = Error.Functor (Error.Params res); _ } ->
      ...
      let d =
        Error.sdiff (param1 :: got_params, got_res)
          (param2 :: expected_params, expected_res)
      in
      Error Error.(Functor (Params d))
    | Error _, _ ->
...
and functor_param ~in_eq ~loc env ~mark subst param1 param2 =
  ...
  | Named (name1, arg1), Named (name2, arg2) ->
    ...
    let (env, subst) =
      ...
      | None, Some id2 -> Env.add_module id2 Mp_present arg2' env, subst
      | Some id1, None -> Env.add_module id1 Mp_present arg2' env, subst
      | None, None -> env, subst
    in
    cc_arg, env, subst
  | _, _ -> Error (Error.Incompatible_params (param1, param2)), env, subst

and strengthened_modtypes
    ~in_eq ~loc ~aliasable env ~mark subst mty1 path1 mty2
=
  match mty1, mty2 with
...
and signatures ~in_eq ~loc env ~mark subst sig1 sig2 =
  ...
  let rec build_component_table pos tbl =
    ...
    | item :: rem ->
      ...
      in
      build_component_table nextpos (FieldMap.add name (id, item, pos) tbl) rem
  in
  let (len1, comps1) = build_component_table 0 FieldMap.empty sig1 in
  let len2 =
    List.fold_left (fun n i -> if is_runtime_component i then n + 1 else n) 0
      sig2
  in
  ...
  let rec pair_components subst paired unpaired =
    ...
    | item2 :: rem ->
      ...
      let (name2, report) =
        ...
        | Sig_type (_, { type_manifest = None }, _, _),
          { name = s; kind = Field_type }
  ...
  | (sigi1, sigi2, pos) :: rem ->
    let (id, item, present_at_runtime) =
      ...
      | Sig_value (id1, valdecl1, _), Sig_value (_id2, valdecl2, _) ->
        let item = value_descriptions ~loc env ~mark subst id1 valdecl1 valdecl2
        in
      ...
      | Sig_typext (id1, ext1, _, _), Sig_typext (_id2, ext2, _, _) ->
        let item = extension_constructors ~loc env ~mark subst id1 ext1 ext2 in
        id1, item, true
      | Sig_module (id1, pres1, mty1, _, _), Sig_module (_, pres2, mty2, _, _)
        ->
        let item = module_declarations ~in_eq ~loc env ~mark subst id1 mty1 mty2
        in
        let item = Result.map_error (fun diff -> Error.Module_type diff) item in
      ...
      | Sig_modtype (id1, info1, _), Sig_modtype (_id2, info2, _) ->
        let item = modtype_infos ~in_eq ~loc env ~mark subst id1 info1 info2 in
        id1, item, false
      | Sig_class (id1, decl1, _, _), Sig_class (_id2, decl2, _, _) ->
        let item = class_declarations ~old_env env subst decl1 decl2 in
        id1, item, true
      | Sig_class_type (id1, info1, _, _), Sig_class_type (_id2, info2, _, _) ->
        let item = class_type_declarations ~loc ~old_env env subst info1 info2
        in
        id1, item, false
      | _ -> assert false
    in
...
and module_declarations ~in_eq ~loc env ~mark subst id1 md1 md2 =
  ...
  strengthened_modtypes ~in_eq ~loc ~aliasable:true env ~mark subst md1.md_type
    p1 md2.md_type
...
let compunit env ~mark impl_name impl_sig intf_name intf_sig =
  ...
  | Result.Error reasons ->
    let cdiff =
      Error.In_Compilation_unit (Error.diff impl_name intf_name reasons)
    in
...
module Functor_inclusion_diff = struct
  module Defs = struct
    ...
    type state = { res : module_type option; env : Env.t; subst : Subst.t }
  end
  ...
  let weight : Diff.change -> _ =
    ...
    | Keep (param1, param2, _) ->
      begin match param_name param1, param_name param2 with
      | None, None -> 0
  ...
  let lookup_expansion { env; res; _ } =
    ...
    | None -> None
    | Some res ->
      begin match retrieve_functor_params env res with
      | [], _ -> None
      | params, res ->
  ...
  let update (d : Diff.change) st =
    ...
    | Change (Named (name1, _), Named (name2, arg2), _)
      ->
      let arg' = Subst.modtype Keep st.subst arg2 in
      begin match name1, name2 with
      | Some id1, Some id2 ->
    ...
    let module Compute =
      ...
      (struct
        let test st mty1 mty2 =
          ...
          let (res, _, _) =
            functor_param ~in_eq:false ~loc st.env ~mark:Mark_neither st.subst
              mty1 mty2
          in
          res
...
module Functor_app_diff = struct
  ...
  let weight : Diff.change -> _ =
    ...
    | Keep (param1, param2, _) ->
      ...
      begin match desc1, I.param_name param2 with
      | (Unit | Anonymous), None -> 0
  ...
  let update (d : Diff.change) (st : Defs.state) =
        ...
        let res =
          Option.map
            (fun res ->
               ...
               Subst.modtype (Rescope scope) subst res) st.res
        in
    ...
    | Change ((Anonymous, mty), Named (param_name, _param), _)
      ->
      begin match param_name with
      | Some param ->
...
let signatures env ~mark sig1 sig2 =
  match
    signatures ~in_eq:false ~loc:Location.none env ~mark Subst.identity sig1
      sig2
  with
  | Ok x -> x
...
let strengthened_module_decl ~loc ~aliasable env ~mark md1 path1 md2 =
  ...
  with
  | Ok x -> x
  | Error mdiff -> raise (Error (env, Error.(In_Module_type mdiff)))
...
let check_modtype_equiv ~loc env id mty1 mty2 =
  ...
  | Error e ->
    raise
      (Error (env, Error.(In_Module_type_substitution (id, diff mty1 mty2 e))))
