...
let value_descriptions ~loc env ~mark cxt subst id vd1 vd2 =
  ...
  if mark_positive mark then Env.mark_value_used (Ident.name id) vd1;
...
let type_declarations ~loc env ~mark ?old_env:_ cxt subst id decl1 decl2 =
  ...
  if mark then Env.mark_type_used (Ident.name id) decl1;
...
let class_type_declarations ~loc ~old_env:_ env cxt subst id decl1 decl2 =
  ...
  | reason ->
      raise
        (Error [cxt, env, Class_type_declarations (id, decl1, decl2, reason)])
...
let rec print_coercion ppf c =
  ...
  | Tcoerce_primitive {pc_desc; pc_env = _; pc_type} ->
      pr
        "prim %s@ (%a)"
        pc_desc.Primitive.prim_name
        Printtyp.raw_type_expr
        pc_type
  | Tcoerce_alias (_, p, c) ->
...
and try_modtypes ~loc env ~mark cxt subst mty1 mty2 =
  ...
  | Mty_alias p1, Mty_alias p2 ->
      ...
      then raise (Error [cxt, env, Invalid_module_alias p2]);
      ( if not (Path.same p1 p2)
      then
  ...
  | ( Mty_functor (param1, Some arg1, res1)
    , Mty_functor (param2, Some arg2, res2) ) -> (
      ...
      let cc_res =
        modtypes
          ~loc
          (Env.add_module param1 Mp_present arg2' env)
          ~mark
          (Body param1 :: cxt)
...
and signature_components ~loc old_env ~mark env cxt subst paired =
  ...
  match paired with
  | [] -> []
  | (Sig_value (id1, valdecl1, _), Sig_value (_id2, valdecl2, _), pos)
    :: rem -> (
      let cc =
  ...
  | (Sig_type (id1, tydecl1, _, _), Sig_type (_id2, tydecl2, _, _), _pos)
    :: rem ->
      type_declarations
        ~loc
        ~old_env
...
and module_declarations ~loc env ~mark cxt subst id1 md1 md2 =
  ...
  if mark_positive mark
  then Env.mark_module_used (Ident.name id1) md1.md_loc;
  modtypes
    ~loc
    env
    ~mark
...
let include_err ppf = function
  ...
  | Modtype_infos (id, d1, d2) ->
      fprintf
        ppf
        "@[<hv 2>Module type declarations do not match:@ %a@;<1 -2>does \
         not match@ %a@]"
        !Oprint.out_sig_item
  ...
  | Class_type_declarations (id, d1, d2, reason) ->
      fprintf
        ppf
        "@[<hv 2>Class type declarations do not match:@ %a@;<1 -2>does not \
         match@ %a@]@ %a"
        !Oprint.out_sig_item
  ...
  | Class_declarations (id, d1, d2, reason) ->
      fprintf
        ppf
        "@[<hv 2>Class declarations do not match:@ %a@;<1 -2>does not \
         match@ %a@]@ %a"
        !Oprint.out_sig_item
...
let context ppf cxt =
  ...
  then fprintf ppf "In module %a:@ " Printtyp.path (path_of_context cxt)
...
let report_error ppf errs =
  ...
  then ()
    ...
    let include_err' ppf ((_, _, obj) as err) =
      if not (is_big obj)
      then fprintf ppf "%a@ " include_err err
      else if !pe
      then (
        fprintf ppf "...@ ";
        pe := false )
