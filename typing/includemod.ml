...
let item_ident_name = function
  ...
  | Sig_typext (id, d, _, _) ->
      let kind =
        if Path.same d.ext_type_path Predef.path_exn then Field_exception
        else Field_typext
      in
...
let retrieve_functor_params env mty =
  let rec retrieve_functor_params before env = function
    | Mty_ident p as res -> (
        match expand_modtype_path env p with
        | Some mty -> retrieve_functor_params before env mty
        | None -> (List.rev before, res))
    | Mty_alias p as res -> (
        match expand_module_alias ~strengthen:false env p with
...
and try_modtypes ~in_eq ~loc env ~mark subst mty1 mty2 =
  ...
  | Mty_alias p1, Mty_alias p2 ->
      if Env.is_functor_arg p2 env then Error (Error.Invalid_module_alias p2)
  ...
  | Mty_alias p1, _ -> (
      ...
      | p1 -> (
          match expand_module_alias ~strengthen:false env p1 with
          | Error e -> Error (Error.Mt_core e)
          | Ok mty1 -> (
              match
                strengthened_modtypes ~in_eq ~loc ~aliasable:true env ~mark
                  subst mty1 p1 mty2
              with
              | Ok _ as x -> x
  ...
  | Mty_signature sig1, Mty_signature sig2 -> (
      match signatures ~in_eq ~loc env ~mark subst sig1 sig2 with
      | Ok _ as ok -> ok
      | Error e -> Error (Error.Signature e))
  | Mty_functor (param1, res1), Mty_functor (param2, res2) -> (
      let cc_arg, env, subst =
        functor_param ~in_eq ~loc env ~mark:(negate_mark mark) subst param1
          param2
      in
      ...
      | Error _, _ ->
          ...
          let d =
            Error.sdiff (param1 :: params1, res1) (param2 :: params2, res2)
          in
          Error Error.(Functor (Params d))
...
and strengthened_modtypes ~in_eq ~loc ~aliasable env ~mark subst mty1 path1 mty2
    =
  match (mty1, mty2) with
  ...
  let rec pair_components subst paired unpaired = function
    | [] -> (
        let oks, errors =
          signature_components ~in_eq ~loc env ~mark new_env subst
            (List.rev paired)
        in
        match (unpaired, errors, oks) with
        | [], [], cc ->
            if len1 = len2 then
              (* see PR#5098 *)
              Ok (simplify_structure_coercion cc id_pos_list)
            else Ok (Tcoerce_structure (cc, id_pos_list))
    ...
    | item2 :: rem -> (
        ...
        let name2, report =
          ...
          | _ -> (name2, true)
        in
        match FieldMap.find name2 comps1 with
        | id1, item1, pos1 ->
            let new_subst =
              match item2 with
      ...
      let id, item, present_at_runtime =
        ...
        | Sig_value (id1, valdecl1, _), Sig_value (_id2, valdecl2, _) ->
            ...
            let present_at_runtime =
              match valdecl2.val_kind with Val_prim _ -> false | _ -> true
            in
...
let check_functor_application_in_path ~errors ~loc ~lid_whole_app ~f0_path ~args
...
module Functor_inclusion_diff = struct
  ...
  let param_name = function Named (x, _) -> x | Unit -> None
  ...
  let weight : Diff.change -> _ = function
    ...
    | Keep (param1, param2, _) -> (
        ...
        | None, None -> 0
        | Some n1, Some n2 when String.equal (Ident.name n1) (Ident.name n2) ->
            0
        | Some _, Some _ -> 1
  ...
  let update (d : Diff.change) (st : Defs.state) =
    ...
    | Change ((Anonymous, mty), Named (param_name, _param), _) -> (
        ...
        | Some param ->
            ...
            let res =
              Option.map (Mtype.nondep_supertype env [ param ]) st.res
            in
