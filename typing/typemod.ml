...
let initial_env
    ~loc ~safe_string ~initially_opened_module ~open_implicit_modules
=
  let env =
  ...
  let add_units env units =
    String.Set.fold
      (fun name env ->
         Env.add_persistent_structure (Ident.create_persistent name) env) units
      env
  in
...
let type_module_type_of_fwd :
  (Env.t -> Parsetree.module_expr -> Typedtree.module_expr * Types.module_type)
  ref
=
  ...
  List.iter
    (fun (id, md) ->
       List.iter
         ...
         (Mtype.type_paths env (Pident id) md.Types.md_type)) decls
...
let check_type_decl env loc id row_id newdecl decl rec_group =
  ...
  let env =
    ...
    in
    List.fold_left add_sigitem env rec_group
  in
...
let rec iter_path_apply p ~f =
  ...
  | Papply (p1, p2) ->
    ...
    f p1 p2
(* after recursing, so we know both paths are well typed *)
...
let check_usage_of_path_of_substituted_item paths ~loc ~lid env super =
  { super with
    ...
    Btype.it_path =
      (fun referenced_path ->
         iter_path_apply referenced_path
           ~f:(fun funct arg ->
             if
               List.exists (fun path -> path_is_strict_prefix path ~prefix:arg)
                 paths
             then
      ...
      List.iter2
        (fun x (y, _) ->
           ...
           | { ptyp_desc = Ptyp_var sx }, { ptyp_desc = Ptyp_var sy }
             when sx = sy ->
             ()
           | _, _ -> raise Exit) stl sdecl.ptype_params
    with
    | exception Exit -> None
    | () -> Some lid
    end
...
let merge_constraint initial_env loc sg lid constr =
  ...
  let rec patch_item constr namelist sig_env ~rec_group ~ghosts item =
    ...
    | Sig_type (id, decl, rs, priv),
      ...
      and id_row = Ident.create_local (s ^ "#row")
      in
      ...
      check_type_decl sig_env sdecl.ptype_loc id row_id newdecl decl rec_group;
      ...
      in
      return ~ghosts ~replace_by:(Some (Sig_type (id, newdecl, rs, priv)))
    ...
    | Sig_type (id, sig_decl, rs, priv),
      ...
      let tdecl =
        Typedecl.transl_with_constraint id ~sig_env ~sig_decl
          ~outer_env:initial_env sdecl
      in
      let newdecl = tdecl.typ_type
      and loc = sdecl.ptype_loc
      in
      let (before_ghosts, row_id, after_ghosts) = split_row_id s ghosts in
      ...
      | _ ->
        real_ids := [ Pident id ];
        return ~ghosts ~replace_by:None (Pident id, lid, Twith_typesubst tdecl)
      end
    | Sig_modtype (id, mtd, priv),
      ...
      let () =
        ...
        | None -> ()
        | Some previous_mty ->
          Includemod.check_modtype_equiv ~loc sig_env id previous_mty
            mty.mty_type
      in
      if not destructive_substitution then
        ...
        in
        return ~replace_by:(Some (Sig_modtype (id, mtd', priv)))
    ...
    | Sig_module (id, pres, md, rs, priv),
      ...
      ignore
        (Includemod.modtypes ~mark:Mark_both ~loc sig_env newmd.md_type
           md.md_type);
      return ~replace_by:(Some (Sig_module (id, pres, newmd, rs, priv)))
  ...
  try
    ...
    let sg =
      ...
      | _, _, Twith_modsubst (real_path, _) ->
        let sub = Subst.change_locs Subst.identity loc in
        let sub =
          List.fold_left (fun s path -> Subst.add_module_path path real_path s)
            sub !real_ids
        in
...
let rec approx_modtype env smty =
  ...
  | Pmty_ident lid ->
    let path = Env.lookup_modtype_path ~use:false ~loc:smty.pmty_loc lid.txt env
    in
    Mty_ident path
  | Pmty_alias lid ->
    let path =
      ...
      (fun sdecl ->
         ...
         | Pwith_module (_, lid') ->
           (* Lookup the module to make sure that it is not recursive.
              (GPR#1626) *)
           ignore
             (Env.lookup_module_path ~use:false ~load:false ~loc:lid'.loc
                lid'.txt env)
         | Pwith_modsubst (_, lid') ->
           ignore
             (Env.lookup_module_path ~use:false ~load:false ~loc:lid'.loc
                lid'.txt env)) constraints;
    body
  | Pmty_typeof smod ->
...
and approx_sig env ssg =
  ...
  | item :: srem ->
    ...
    | Psig_module pmd ->
      ...
      in
      let (id, newenv) =
        Env.enter_module_declaration ~scope (Option.get pmd.pmd_name.txt) pres
          md env
      in
          ...
          (fun pmd ->
             Option.map
               (fun name ->
                  ...
                  approx_module_declaration env pmd) pmd.pmd_name.txt) sdecls
      in
      let newenv =
        List.fold_left
          (fun env (id, md) ->
             Env.add_module_declaration ~check:false id Mp_present md env) env
          decls
      in
      map_rec (fun rs (id, md) -> Sig_module (id, Mp_present, md, rs, Exported))
...
module Signature_names : sig
  ...
  type bound_info = [ `Exported | `Shadowable of shadowable ]
  ...
  let check cl t loc id (info : info) =
    ...
    | #bound_info as bound_info ->
      ...
      begin match Hashtbl.find_opt tbl name with
...
and transl_modtype_aux env smty =
  ...
  | Pmty_functor (sarg_opt, sres) ->
    ...
    mkmty (Tmty_functor (t_arg, res)) (Mty_functor (ty_arg, res.mty_type)) env
      loc smty.pmty_attributes
  | Pmty_with (sbody, constraints) ->
...
and transl_signature env sg =
  ...
  let rec transl_sig env sg =
    ...
    | item :: srem ->
      ...
      begin match item.psig_desc with
      | Psig_value sdesc ->
        let (tdesc, newenv) = Typedecl.transl_value_decl env item.psig_loc sdesc
        in
      ...
      | Psig_type (rec_flag, sdecls) ->
        ...
        let sg =
          map_rec_type_with_row_types ~rec_flag
            (fun rs td -> Sig_type (td.typ_id, td.typ_type, rs, Exported)) decls
            rem
        in
        mksig (Tsig_type (rec_flag, decls)) env loc :: trem, sg, final_env
      | Psig_typesubst sdecls ->
        let (decls, newenv) = Typedecl.transl_type_decl env Nonrecursive sdecls
        in
        List.iter
          (fun td ->
             (if
             ...
             let info =
               let subst =
                 Subst.add_type_function (Pident td.typ_id) ~params
                   ~body:(Option.get td.typ_type.type_manifest) Subst.identity
               in
               Some (`Substituted_away subst)
             in
             Signature_names.check_type ?info names td.typ_loc td.typ_id) decls;
        let (trem, rem, final_env) = transl_sig newenv srem in
        let sg = rem in
      ...
      | Psig_modsubst pms ->
        ...
        let (id, newenv) =
          Env.enter_module_declaration ~scope pms.pms_name.txt pres md env
        in
        let info = `Substituted_away (Subst.add_module id path Subst.identity)
        in
      ...
      | Psig_recmodule sdecls ->
        ...
        let decls =
          List.filter_map
            (fun (md, uid) ->
               match md.md_id with
               | None -> None
               | Some id -> Some (id, md, uid)) tdecls
        in
        List.iter
          (fun (id, md, _) -> Signature_names.check_module names md.md_loc id)
          decls;
        let (trem, rem, final_env) = transl_sig newenv srem in
        ...
        map_rec
          (fun rs (id, md, uid) ->
             ...
             in
             Sig_module (id, Mp_present, d, rs, Exported)) decls rem,
        final_env
      | Psig_modtype pmtd ->
      ...
      | Psig_modtypesubst pmtd ->
        ...
        let (trem, rem, final_env) = transl_sig newenv srem in
        mksig (Tsig_modtypesubst mtd) env loc :: trem, rem, final_env
      | Psig_open sod ->
      ...
      | Psig_include sincl ->
        ...
        Signature_group.iter
          (Signature_names.check_sig_item names item.psig_loc) sg;
        let incl =
          {
            incl_mod = tmty;
            incl_type = sg;
      ...
      | Psig_class cl ->
        ...
        List.iter
          (fun cls ->
             ...
             Signature_names.check_type names loc cls.cls_typesharp_id) classes;
        let (trem, rem, final_env) = transl_sig newenv srem in
        let sg =
          map_rec
            (fun rs cls ->
      ...
      | Psig_class_type cl ->
        ...
        let typedtree =
          mksig
            (Tsig_class_type
               (List.map (fun decl -> decl.Typeclass.clsty_info) classes)) env
            loc ::
            trem
        in
        typedtree, sg, final_env
  ...
  Builtin_attributes.warning_scope []
    (fun () ->
       ...
       let sg = { sig_items = trem; sig_type = rem; sig_final_env = final_env }
       in
...
and transl_modtype_decl_aux
    env { pmtd_name; pmtd_type; pmtd_attributes; pmtd_loc }
=
  let tmty = Option.map (transl_modtype (Env.in_signature true env)) pmtd_type
  in
  let decl =
    {
  ...
  let transition env_c curr =
    List.map2
      (fun pmd (id, id_loc, md, _) ->
         ...
         id, id_loc, md, tmty) sdecls curr
  in
  let map_mtys curr =
    List.filter_map (fun (id, _, md, _) -> Option.map (fun id -> id, md) id)
      curr
  in
  let scope = Ctype.create_scope () in
  let ids =
  ...
  | Some p ->
    List.fold_left
      (fun e info ->
         ...
         let info' =
           Mtype.enrich_typedecl oldenv (Pdot (p, Ident.name id)) id
             info.typ_type
         in
         Env.add_type ~check:true id info' e) oldenv decls
  ...
  let rec check_incl first_time n env s =
             ...
             | Some (id, id') ->
               let mty_actual' =
                 if first_time then
                   ...
                   subst_and_strengthen env scope s (Some id) mty_actual
               in
               Env.add_module ~arg:false id' Mp_present mty_actual' env) env
          bindings1
      in
    ...
    (* Base case: check inclusion of s(mty_actual) in s(mty_decl)
      ...
      =
        ...
        and mty_actual' = subst_and_strengthen env scope s id mty_actual
        in
        let coercion =
          try
...
let package_subtype env p1 fl1 p2 fl2 =
  let mkmty p fl =
    let fl = List.filter (fun (_n, t) -> Ctype.free_variables t = []) fl in
  ...
  | mty1, mty2 ->
    ...
    begin match Includemod.modtypes ~loc ~mark:Mark_both env mty1 mty2 with
    | Tcoerce_none -> true
...
and type_module_aux ~alias sttn funct_body anchor env smod =
  ...
  | Pmod_structure sstr ->
    let (str, sg, names, _finalenv) = type_structure funct_body anchor env sstr
    in
    let md =
      {
    ...
    if List.length sg' = List.length sg then
      md
    else
      wrap_constraint env false md (Mty_signature sg') Tmodtype_implicit
  ...
  | Pmod_apply _ -> type_application smod.pmod_loc sttn funct_body env smod
  | Pmod_constraint (sarg, smty) ->
    let arg = type_module ~alias true funct_body anchor env sarg in
    let mty = transl_modtype env smty in
    let md = wrap_constraint env true arg mty.mty_type (Tmodtype_explicit mty)
    in
...
and type_one_application
    ~ctx:(apply_loc, md_f, args) funct_body env funct app_view
=
  ...
  | Mty_functor (Named (param, mty_param), mty_res) as mty_functor ->
    let coercion =
      try
        Includemod.modtypes ~loc:app_view.arg.mod_loc ~mark:Mark_both env
    ...
    let mty_appl =
      ...
      | None ->
        let (env, nondep_mty) =
          ...
          | Some param ->
            let env =
              Env.add_module ~arg:true param Mp_present app_view.arg.mod_type
                env
            in
        ...
        in
        begin match
          Includemod.modtypes ~loc:app_view.loc ~mark:Mark_neither env mty_res
            nondep_mty
        with
        | Tcoerce_none -> ()
        | _ ->
...
and type_open_decl ?used_slot ?toplevel funct_body names env sod =
  Builtin_attributes.warning_scope sod.popen_attributes
    (fun () -> type_open_decl_aux ?used_slot ?toplevel funct_body names env sod)
      ...
      List.map
        (function
         ...
         | Sig_class_type (id, ctd, rs, _) ->
           Sig_class_type (id, ctd, rs, visibility)) sg
    in
    let open_descr =
      {
        open_expr = md;
...
and type_structure ?(toplevel=false) funct_body anchor env sstr =
  ...
  let type_str_item env { pstr_loc = loc; pstr_desc = desc } =
    ...
    | Pstr_type (rec_flag, sdecls) ->
      let (decls, newenv) = Typedecl.transl_type_decl env rec_flag sdecls in
      List.iter
        Signature_names.(fun td -> check_type names td.typ_loc td.typ_id) decls;
    ...
    | Pstr_typext styext ->
      let (tyext, newenv) = Typedecl.transl_type_extension true env loc styext
      in
      let constructors = tyext.tyext_constructors in
      List.iter
        ...
        List.map
          (function
           | { pmb_name = name;
               ...
               pmb_attributes = attrs; pmb_loc = loc }
             ->
             ...
             raise
               (Error
                  (mb.pmb_expr.pmod_loc,
                   env,
                   Recursive_module_require_explicit_type))) sbind
      in
      let (decls, newenv) =
               ...
               Builtin_attributes.warning_scope attrs
                 (fun () ->
                    type_module true funct_body (anchor_recmodule id) newenv
                      smodl)
             in
             let mty' = enrich_module_type anchor name.txt modl.mod_type newenv
             in
             id, name, mty, modl, mty', attrs, loc, uid) decls sbind
      in
      let newenv =
    ...
    | Pstr_open sod ->
      let (od, sg, newenv) = type_open_decl ~toplevel funct_body names env sod
      in
    ...
    | Pstr_class_type cl ->
      ...
      List.iter
        (fun decl ->
           ...
           Signature_names.check_class_type names loc decl.clsty_ty_id;
           Signature_names.check_type names loc decl.clsty_obj_id;
           Signature_names.check_type names loc decl.clsty_typesharp_id) classes;
      Tstr_class_type
        (List.map
           (fun cl ->
    ...
    List.fold_left
      (fun acc item ->
         ...
         | Sig_module (id, _, _, _, _) ->
           ...
           types, modules
         | _ -> acc) (String.Map.empty, String.Map.empty) sg
  in
  let rec module_path =
    function
...
let type_package env m p fl =
  ...
  List.iter
    (fun (n, ty) ->
       ...
       with
       | Ctype.Unify _ ->
         raise (Error (modl.mod_loc, env, Scoping_pack (n, ty)))) fl';
    ...
    (fun () ->
       ...
       if !Clflags.print_types then
         (Typecore.force_delayed_checks ();
          ...
          { structure = str; coercion = Tcoerce_none; signature = simple_sg })
       (* result is ignored by Compile.implementation *)
         ...
         if Sys.file_exists sourceintf then
           ...
           (Typecore.force_delayed_checks ();
            ...
            { structure = str; coercion; signature = dclsig })
         else
           (Location.prerr_warning (Location.in_file sourcefile)
            ...
            (if not !Clflags.dont_write_files then
               ...
               (Cmt_format.save_cmt (outputprefix ^ ".cmt") modulename annots
                ...
                gen_annot outputprefix sourcefile annots));
            { structure = str; coercion; signature = simple_sg }))
    ~exceptionally:(fun () ->
      let annots =
...
let package_signatures units =
  ...
  List.map
    (fun (_, newid, sg) ->
       ...
       in
       Sig_module (newid, Mp_present, md, Trec_not, Exported)) units_with_ids
    ...
    List.map
      (fun f ->
         ...
         (if
          ...
          then
            raise
              (Error (Location.none, Env.empty, Implementation_is_required f)));
         modname, Env.read_signature modname (pref ^ ".cmi")) objfiles
  in
  ...
  if Sys.file_exists mlifile then
    ((if not (Sys.file_exists cmifile) then
     ...
     Includemod.compunit initial_env ~mark:Mark_both "(obtained by packing)" sg
       mlifile dclsig)
  else (* Determine imports *)
    let unit_names = List.map fst units in
    let imports =
      List.filter (fun (name, _crc) -> not (List.mem name unit_names))
        (Env.imports ())
    in
    ...
    ((if not !Clflags.dont_write_files then
        let cmi =
          Env.save_signature_with_imports ~alerts:Misc.Stdlib.String.Map.empty
            sg modulename (prefix ^ ".cmi") imports
        in
        Cmt_format.save_cmt (prefix ^ ".cmt") modulename
          (Cmt_format.Packed (cmi.Cmi_format.cmi_sign, objfiles)) None
          initial_env (Some cmi));
     Tcoerce_none)

(* Error report *)
open Printtyp

let report_error ~loc _env =
  function
  | Cannot_apply mty ->
  ...
  | With_mismatch (lid, explanation) ->
    ...
    Location.errorf ~loc
      "@[<v>@[In this `with' constraint, the new definition of %a@ does not \
       match its original definition@ in the constrained signature:@]@ %t@]"
      longident lid main
  | With_makes_applicative_functor_ill_typed (lid, path, explanation) ->
    let main = Includemod_errorprinter.err_msgs explanation in
    Location.errorf ~loc
      "@[<v>@[This `with' constraint on %a makes the applicative functor @ \
       type %s ill-typed in the constrained signature:@]@ %t@]" longident lid
      (Path.name path) main
  | With_changes_module_alias (lid, id, path) ->
    Location.errorf ~loc
      "@[<v>@[This `with' constraint on %a changes %s, which is aliased @ in \
       the constrained signature (as %s)@].@]" longident lid (Path.name path)
      (Ident.name id)
  | With_cannot_remove_constrained_type ->
    Location.errorf ~loc
  ...
  | Repeated_name (kind, name) ->
    Location.errorf ~loc
      "@[Multiple definition of the %s name %s.@ Names must be unique in a \
       given structure or signature.@]" (Sig_component_kind.to_string kind) name
  | Non_generalizable typ ->
    Location.errorf ~loc
      "@[The type of this expression,@ %a,@ contains type variables that \
       cannot be generalized@]" type_scheme typ
  | Non_generalizable_module mty ->
    Location.errorf ~loc
      "@[The type of this module,@ %a,@ contains type variables that cannot \
       be generalized@]" modtype mty
  | Implementation_is_required intf_name ->
    Location.errorf ~loc
      "@[The interface %a@ declares values, not just types.@ An \
       implementation must be provided.@]" Location.print_filename intf_name
  ...
  | Not_a_packed_module ty ->
    Location.errorf ~loc
      "This expression is not a packed module. It has type@ %a" type_expr ty
  | Incomplete_packed_module ty ->
    Location.errorf ~loc
      "The type of this packed module contains variables:@ %a" type_expr ty
  | Scoping_pack (lid, ty) ->
    Location.errorf ~loc
      "The type %a in this module cannot be exported.@ Its type contains \
       local dependencies:@ %a" longident lid type_expr ty
  | Recursive_module_require_explicit_type ->
  ...
  | Cannot_scrape_alias p ->
    Location.errorf ~loc "This is an alias for module %a, which is missing" path
      p
  ...
  | Cannot_hide_id
      (Illegal_shadowing
         { shadowed_item_kind; shadowed_item_id; shadowed_item_loc; shadower_id;
    ...
    Location.errorf ~loc
      "@[<v>Illegal shadowing of included %s %a by %a@ %a:@;\
       ...
       <1 2>The %s %s has no valid type if %a is shadowed@]" shadowed_item_kind
      Ident.print shadowed_item_id Ident.print shadower_id Location.print_loc
      shadowed_item_loc (String.capitalize_ascii shadowed_item_kind) Ident.print
      shadowed_item_id Location.print_loc user_loc
      (Sig_component_kind.to_string user_kind) (Ident.name user_id) Ident.print
      shadowed_item_id
  | Cannot_hide_id
      (Appears_in_signature
  ...
  | Unpackable_local_modtype_subst p ->
    Location.errorf ~loc
      "The module type@ %s@ is not a valid type for a packed module:@ it is \
       defined as a local substitution for a non-path module type."
      (Path.name p)
