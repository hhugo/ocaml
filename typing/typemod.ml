...
let initial_env ~loc ~safe_string ~initially_opened_module
  ...
  let env =
    if safe_string
    then Env.initial_safe_string
    else Env.initial_unsafe_string
  in
...
let rec iter_path_apply p ~f =
  ...
  | Papply (p1, p2) -> iter_path_apply p1 ~f; iter_path_apply p2 ~f; f p1 p2
      ...
      Btype.it_signature_item =
        (fun self -> function
          | Sig_module (id, _, {md_type = Mty_alias aliased_path; _}, _, _)
            when List.exists
                   (fun path ->
                     path_is_strict_prefix path ~prefix:aliased_path )
                   paths ->
              let e =
                With_changes_module_alias (lid.txt, id, aliased_path)
              in
...
let merge_constraint initial_env remove_aliases loc sg constr =
  ...
  let rec merge env sg namelist row_id =
    ...
    | ( (Sig_module (id, _, ({md_type = Mty_alias _} as md), _, _) as item)
        :: rem
      , s :: namelist
      , (Pwith_module _ | Pwith_type _) )
      when Ident.name id = s ->
        let (path, _, tcstr), _ =
    ...
    | item :: rem, _, _ ->
        let cstr, items =
          merge (Env.add_item item env) rem namelist row_id
        in
        cstr, item :: items
  in
  try
...
let rec map_rec_type_with_row_types ~rec_flag fn decls rem =
  ...
  | d1 :: dl ->
      ...
      then fn Trec_not d1 :: map_rec_type_with_row_types ~rec_flag fn dl rem
...
let rec approx_modtype env smty =
  ...
  | Pmty_functor (param, sarg, sres) ->
      ...
      let id, newenv =
        Env.enter_module ~scope ~arg:true param.txt Mp_present rarg env
      in
...
module Signature_names : sig
  ...
  let check_value ?info t loc id =
    let info =
      match info with Some i -> i | None -> `Shadowable (id, loc)
    in
    check
      Sig_component_kind.Value
      loc
      t.bound.values
      id
      info
  ...
  let check_modtype ?(info = `Exported) t loc id =
    check
      Sig_component_kind.Module_type
      loc
      t.bound.modtypes
      id
      info
  ...
  let check_typext ?(info = `Exported) t loc id =
    check
      Sig_component_kind.Extension_constructor
      loc
      t.bound.typexts
      id
      info
  ...
  *)
  ...
  let simplify env t sg =
    ...
    let ids_to_remove =
      Ident.Map.fold
        (fun id (kind, _, _) lst ->
          if Sig_component_kind.can_appear_in_types kind
          then id :: lst
          else lst )
        to_remove.hide
        []
    ...
    let aux component sg =
      ...
      then sg
        ...
        let component =
          ...
          then component
          else Subst.signature_item to_remove.subst component
        in
        let component =
...
and transl_modtype_aux env smty =
  ...
  | Pmty_with (sbody, constraints) ->
      ...
      let remove_aliases =
        has_remove_aliases_attribute smty.pmty_attributes
      in
  ...
  let rec transl_sig env sg =
    ...
    | item :: srem -> (
        ...
        | Psig_type (rec_flag, sdecls) ->
            ...
            let sg =
              map_rec_type_with_row_types
                ~rec_flag
                (fun rs td ->
                  Sig_type (td.typ_id, td.typ_type, rs, Exported) )
                decls
                rem
            in
        ...
        | Psig_modsubst pms ->
            ...
            let id = Ident.create_scoped ~scope pms.pms_name.txt in
            let path, md =
              Typetexp.find_module
                env
                pms.pms_manifest.loc
                pms.pms_manifest.txt
            in
            let aliasable = not (Env.is_functor_arg path env) in
            let md =
              if not aliasable
        ...
        | Psig_recmodule sdecls ->
            ...
            List.iter
              (fun md ->
                Signature_names.check_module names md.md_loc md.md_id )
              decls;
            let trem, rem, final_env = transl_sig newenv srem in
            ...
            let sg =
              map_rec
                (fun rs cls ->
                  ...
                  ; Sig_class_type
                      (cls.cls_ty_id, cls.cls_ty_decl, rs, Exported)
                  ; Sig_type (cls.cls_obj_id, cls.cls_obj_abbr, rs, Exported)
                  ; Sig_type
                      (cls.cls_typesharp_id, cls.cls_abbr, rs, Exported) ]
                  )
                classes
                [rem]
              |> List.flatten
...
let check_nongen_scheme env sig_item =
  match sig_item with
  | Sig_value (_id, vd, _) ->
      if not (Ctype.closed_schema env vd.val_type)
      then raise (Error (vd.val_loc, env, Non_generalizable vd.val_type))
  | Sig_module (_id, _, md, _, _) ->
      if not (closed_modtype env md.md_type)
      then
...
let check_recmodule_inclusion env bindings =
  ...
  let rec check_incl first_time n env s =
    ...
    then
      ...
      let check_inclusion
          (id, id_loc, mty_decl, modl, mty_actual, attrs, loc) =
...
let wrap_constraint env mark arg mty explicit =
  let mark =
    if mark then Includemod.Mark_both else Includemod.Mark_neither
  in
  let coercion =
...
and type_module_aux ~alias sttn funct_body anchor env smod =
      ...
      let md =
        ...
        then (
          ...
          | Mty_alias p1 when not alias ->
              let p1 =
                Env.normalize_module_path (Some smod.pmod_loc) env p1
              in
              ...
              ; mod_type =
                  ( if sttn
                  then Mtype.strengthen ~aliasable:true env mty p1
                  else mty ) }
          | mty ->
              let mty =
                if sttn
                then Mtype.strengthen ~aliasable env mty path
                else mty
              in
  ...
  | Pmod_apply (sfunct, sarg) -> (
      ...
      | Mty_functor (param, mty_param, mty_res) as mty_functor ->
          ...
          if generative
          then (
            if sarg.pmod_desc <> Pmod_structure []
            then raise (Error (sfunct.pmod_loc, env, Apply_generative));
            ...
            | None ->
                ...
                then mty_res
                  ...
                  | exception Includemod.Error _ ->
                      fatal_error
                        "nondep_supertype not included in original module \
                         type" );
                  nondep_mty
          in
          check_well_formed_module
  ...
  | Pmod_constraint (sarg, smty) ->
      ...
      rm
        { md with
          mod_loc = smod.pmod_loc; mod_attributes = smod.pmod_attributes }
  | Pmod_unpack sexp ->
      ...
      let mty =
        ...
        | {desc = Tpackage (p, nl, tl)} ->
            if List.exists (fun t -> Ctype.free_variables t <> []) tl
            then
              raise
                (Error
                   ( smod.pmod_loc
            ...
            if !Clflags.principal
               && not
                    (Typecore.generalizable
                       (Btype.generic_level - 1)
                       exp.exp_type)
            then
        ...
        | _ ->
            raise
              (Error (smod.pmod_loc, env, Not_a_packed_module exp.exp_type))
      in
...
and type_open_decl_aux ?used_slot ?toplevel funct_body names env od =
  ...
  | _ ->
      ...
      in
      List.iter
        (Signature_names.check_sig_item ~info:`From_open names loc)
        sg;
      let sg =
        List.map
          (function
  ...
  let type_str_item env srem {pstr_loc = loc; pstr_desc = desc} =
    ...
    | Pstr_value (rec_flag, sdefs) ->
        ...
        let () =
          ...
          then Typecore.check_recursive_bindings env defs
        in
    ...
    | Pstr_type (rec_flag, sdecls) ->
        ...
        , map_rec_type_with_row_types
            ~rec_flag
            (fun rs info ->
              Sig_type (info.typ_id, info.typ_type, rs, Exported) )
            decls
            []
    ...
    | Pstr_typext styext ->
        ...
        List.iter
          Signature_names.(
            fun ext -> check_typext names ext.ext_loc ext.ext_id)
          constructors;
        ( Tstr_typext tyext
        , map_ext
            (fun es ext ->
...
*)
  ...
  let run () =
    ...
    let str =
      {str_items = items; str_type = sg; str_final_env = final_env}
    in
    Cmt_format.set_saved_types
...
let type_module_type_of env smod =
  ...
  if not (closed_modtype env mty)
  then raise (Error (smod.pmod_loc, env, Non_generalizable_module mty));
  tmty, mty
...
let type_package env m p nl =
  ...
  Ctype.end_def ();
  if nl = []
  then wrap_constraint env true modl (Mty_ident p) Tmodtype_implicit, []
  else
...
let package_units initial_env objfiles cmifile modulename =
  ...
  if Sys.file_exists mlifile
  then (
    if not (Sys.file_exists cmifile)
    then
      raise
        (Error
...
let report_error ppf = function
  ...
  | Cannot_eliminate_dependency mty ->
      fprintf
        ppf
        "@[This functor has type@ %a@ The parameter cannot be eliminated \
         in the result type.@ Please bind the argument to a module \
         identifier.@]"
        modtype
        mty
  ...
  | With_mismatch (lid, explanation) ->
      fprintf
        ppf
        "@[<v>@[In this `with' constraint, the new definition of %a@ does \
         not match its original definition@ in the constrained \
         signature:@]@ %a@]"
        longident
        lid
        Includemod.report_error
  ...
  | With_changes_module_alias (lid, id, path) ->
      fprintf
        ppf
        "@[<v>@[This `with' constraint on %a changes %s, which is aliased \
  ...
  | With_cannot_remove_constrained_type ->
      fprintf
        ...
        "@[<v>Destructive substitutions are not supported for constrained \
         @ types (other than when replacing a type constructor with @ a \
         type constructor with the same arguments).@]"
  | Repeated_name (kind, name) ->
      fprintf
        ppf
        "@[Multiple definition of the %s name %s.@ Names must be unique in \
         a given structure or signature.@]"
  ...
  | Non_generalizable_class (id, desc) ->
      fprintf
        ppf
        "@[The type of this class,@ %a,@ contains type variables that \
         cannot be generalized@]"
  ...
  | Non_generalizable_module mty ->
      fprintf
        ppf
        "@[The type of this module,@ %a,@ contains type variables that \
         cannot be generalized@]"
        modtype
        mty
  ...
  | Cannot_hide_id
      ...
      fprintf
        ppf
        "@[<v>Illegal shadowing of included %s %a by %a@ %a:@;<1 2>%s %a \
         came from this include@ %a:@;<1 2>The %s %s has no valid type if \
         %a is shadowed@]"
        shadowed_item_kind
        Ident.print
        shadowed_item_id
  ...
  | Cannot_hide_id
      ...
      let opened_item_kind =
        Sig_component_kind.to_string opened_item_kind
      in
      fprintf
        ppf
