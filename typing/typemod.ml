...
let check_usage_of_module_types ~error ~paths ~loc env super =
  let it_do_type_expr it ty =
    ...
    | Tpackage (p, _) -> (
        match List.find_opt (Path.same p) paths with
...
let rec approx_modtype env smty =
  ...
  | Pmty_with (sbody, constraints) ->
      ...
      List.iter
        (fun sdecl ->
          match sdecl with
          | Pwith_type _ | Pwith_typesubst _ | Pwith_modtype _
  ...
  *)
  ...
  let simplify env t sg =
    ...
    let simplify_item (component : Types.signature_item) =
      ...
      in
      if Ident.Map.mem user_id to_remove.hide then None
      else
        let component =
...
and type_module_aux ~alias sttn funct_body anchor env smod =
  ...
  | Pmod_ident lid -> (
      ...
      if alias && aliasable then (
        ...
        let mty =
          if sttn then Env.find_strengthened_module ~aliasable path env
...
and type_application loc strengthen funct_body env smod =
  ...
  in
  List.fold_left
    (type_one_application ~ctx:(loc, funct, args) funct_body env)
    funct args
  ...
  | Mty_functor (Named (param, mty_param), mty_res) as mty_functor ->
      let coercion =
        ...
        with Includemod.Error _ ->
          ...
          let lid_app = None in
          raise
            (Includemod.Apply_error
               { loc = apply_loc; env; lid_app; mty_f; args })
      in
      let mty_appl =
        ...
        | None ->
            ...
            in
            (match
               Includemod.modtypes ~loc:app_view.loc ~mark:Mark_neither env
                 mty_res nondep_mty
             with
            | Tcoerce_none -> ()
            | _ ->
  ...
  | With_makes_applicative_functor_ill_typed (lid, path, explanation) ->
      ...
      Location.errorf ~loc
        "@[<v>@[This `with' constraint on %a makes the applicative functor @ \
         type %s ill-typed in the constrained signature:@]@ %t@]"
