...
let all_coherent column =
  ...
  let coherent_heads hp1 hp2 =
    ...
    | _, _ -> false
  in
  match List.find (function { pat_desc = Any } -> false | _ -> true) column with
  | exception Not_found ->
...
module SyntacticCompat =
  Compat
  (struct let equal c1 c2 = Types.equal_tag c1.cstr_tag c2.cstr_tag end)
...
let clean_copy ty =
  if get_level ty = Btype.generic_level then
    ty
  else
...
let do_set_args ~erase_mutable q r =
         ...
         (List.map2
            (fun (lid, lbl, _) arg ->
               ...
               then
                 ...
                 lid, lbl, arg) omegas args,
          closed)) q.pat_type q.pat_env ::
      rest
  ...
  | { pat_desc = (Tpat_constant _ | Tpat_any) } -> q :: r
...
let build_specialized_submatrices ~extend_row discr rows =
  ...
  (* insert a row of head [p] and rest [r] into the right group
     ...
     in source order.
  *)
  let rec insert_constr head args r =
    function
    | [] ->
    ...
    | (q0, rs as bd) :: env ->
      if simple_match q0 head then
        extend_group q0 head args r rs :: env
      else
        bd :: insert_constr head args r env
  in
  ...
  let rec form_groups constr_groups omega_tails =
    ...
    | ((head, args), tail) :: rest ->
      ...
      | _ ->
        form_groups (insert_constr head args tail constr_groups) omega_tails
          rest
      end
  in
    ...
    List.fold_left
      (fun nm (_tag, f) ->
         ...
         | Reither (_, _, false, e) ->
           ...
           None
         | Rabsent | Reither (_, _, true, _) | Rpresent _ -> nm) row.row_name
      row.row_fields
  in
      ...
      if closing && not (Btype.row_fixed row) then
        ...
        List.for_all
          (fun (tag, f) ->
             ...
             | Rpresent _
               ->
               List.mem tag fields) row.row_fields
      else
        row.row_closed &&
          List.for_all
            (fun (tag, f) ->
...
let pats_of_type ?(always=false) env ty =
  ...
  | Tconstr (path, _, _) ->
    ...
    | Type_record (labels, _) ->
      let fields =
        List.map (fun ld -> mknoloc (Longident.Lident ld.lbl_name), ld, omega)
          labels
      in
...
let rec get_variant_constructors env ty =
  ...
  | Tconstr (path, _, _) ->
    (try
       match Env.find_type path env, Env.find_type_descrs path env with
...
let complete_constrs constr used_constrs =
  ...
  let others =
    List.filter (fun cnstr -> not (ConstructorSet.mem cnstr used_constrs))
      constrs
  in
  ...
  | (d, _) :: _ ->
      ...
      make_pat
        (Tpat_var
           (Ident.create_local "*extension*",
            { txt = "*extension*"; loc = d.pat_loc })) Ctype.none Env.empty
    | Construct _ ->
      begin match ext with
      | Some ext ->
        if Path.same ext (get_constructor_type_path d.pat_type d.pat_env) then
          extra_pat
        else
    ...
    | Variant { cstr_row; type_row } ->
      ...
      | pat :: other_pats ->
        List.fold_left
          (fun p_res pat ->
             make_pat (Tpat_or (pat, p_res, None)) d.pat_type d.pat_env) pat
          other_pats
      end
    ...
    | Constant (Const_int _) ->
      build_other_constant
        ...
        (function i -> Tpat_constant (Const_int i)) 0 succ d env
    | Constant (Const_int32 _) ->
      build_other_constant
        (function Constant (Const_int32 i) -> i | _ -> assert false)
        (function i -> Tpat_constant (Const_int32 i)) 0l Int32.succ d env
    | Constant (Const_int64 _) ->
      build_other_constant
        (function Constant (Const_int64 i) -> i | _ -> assert false)
        (function i -> Tpat_constant (Const_int64 i)) 0L Int64.succ d env
    | Constant (Const_nativeint _) ->
      build_other_constant
        (function Constant (Const_nativeint i) -> i | _ -> assert false)
        (function i -> Tpat_constant (Const_nativeint i)) 0n Nativeint.succ d
        env
    | Constant (Const_string _) ->
      build_other_constant
        ...
        (function
         | i ->
           Tpat_constant (Const_string (String.make i '*', Location.none, None)))
        0 succ d env
    | Constant (Const_float _) ->
      build_other_constant
        (function
         ...
         | _ -> assert false)
        (function f -> Tpat_constant (Const_float (string_of_float f))) 0.0
        (fun f -> f +. 1.0) d env
    | Array _ ->
      let all_lengths =
        List.map
      ...
      let rec try_arrays l =
        if List.mem l all_lengths then
          try_arrays (l + 1)
        else
          make_pat (Tpat_array (omegas l)) d.pat_type d.pat_env
      in
      try_arrays 0
    | _ -> Patterns.omega
    end
          ...
          if not (full_match false constrs) then
            ...
            List.exists
              (fun (p, pss) ->
                 not (is_absent_pat p) &&
                   satisfiable pss
                     (simple_match_args p Patterns.Head.omega [] @ qs)) constrs
                ...
                (List.map
                   (fun (p, pss) ->
                      if is_absent_pat p then
                        ...
                        List.map (set_args p) witnesses) constrs)
            in
  ...
  | q :: qs ->
    ...
    | #Patterns.Simple.view as view ->
      ...
      (* [pss] will (or won't) match [q0 :: qs] regardless of the coherence of
         its first column. *)
      do_match (build_specialized_submatrix ~extend_row:(@) q0 pss) (qargs @ qs)
    end
(*
let print_pat pat =
        ...
        if full_match (tdefs = None) constrs then
          ...
          (begin match constrs, tdefs with
           ...
           | (d, _) :: _, Some env ->
             ...
             | Variant { type_row; _ } ->
               let row = type_row () in
               if Btype.row_fixed row || pressure_variants None default then
                 ()
               else
    ...
    | _ ->
      ...
      List.fold_right2
        (fun pss qs r ->
           ...
           | Unused -> Unused
           | _ ->
             begin match qs.active with
             | [ q ] ->
               let (q1, q2) = or_args q in
               let r_loc = every_both pss qs q1 q2 in
               union_res r r_loc
             | _ -> assert false
             end) (extract_columns pss qs) (extract_elements qs) Used
    end
  | q :: rem ->
    begin match Patterns.General.(view q |> strip_vars).pat_desc with
    | `Any ->
      ...
      else (* otherwise this is direct food for satisfiable *)
    ...
    | `Or (q1, q2, _) ->
      if q1.pat_loc.Location.loc_ghost && q2.pat_loc.Location.loc_ghost then
        ...
        every_satisfiables (push_no_or_column pss) (push_no_or qs)
      else (* this is a real or-pattern *)
...
and every_both pss qs q1 q2 =
  ...
  and qs2 = { qs with  active = [ q2 ] }
  in
  let r1 = every_satisfiables pss qs1
  and r2 = every_satisfiables (if compat q1 q2 then qs1 :: pss else pss) qs2
  in
  match r1 with
  | Unused ->
    begin match r2 with
...
let get_mins le ps =
  let rec select_rec r =
    ...
    | [] -> r
    | p :: ps ->
      if List.exists (fun p0 -> le p0 p) ps then
        select_rec r ps
      else
        select_rec (p :: r) ps
  in
...
and orlub p1 p2 q =
  try
    let r1 = lub p1 q in
    try { q with  pat_desc = Tpat_or (r1, lub p2 q, None) }
    with
    | Empty -> r1
  with
  | Empty -> lub p2 q
  ...
  let (val_pss, exn_pss) =
    List.fold_right
      (fun pat (vpss, epss) ->
         ...
         add_row vpss vp, add_row epss ep) patl ([], [])
  in
        ...
        let fields =
          List.map
            (fun (_, lbl, p) ->
               ...
               mknoloc (Longident.Lident id), loop p) subpatterns
        in
...
let do_check_partial ~pred loc casel pss =
  ...
  | ps :: _ ->
    ...
    in
    begin match counter_examples () with
    | Seq.Nil -> Total
    | Seq.Cons (v, _rest) ->
  ...
  not
    (Path.same path Predef.path_bool ||
       Path.same path Predef.path_list ||
         Path.same path Predef.path_unit || Path.same path Predef.path_option)

let rec collect_paths_from_pat r p =
  match p.pat_desc with
  | Tpat_construct
      (_, { cstr_tag = (Cstr_constant _ | Cstr_block _ | Cstr_unboxed) }, ps, _)
    ->
    let path = get_constructor_type_path p.pat_type p.pat_env in
    List.fold_left collect_paths_from_pat
      (if extendable_path path then add_path path r else r) ps
        ...
        (try
           ...
           if r = Unused && refute then
             ...
             let r =
               ...
               if skip then
                 r
               else (* Then look for empty patterns *)
...
let rec matrix_stable_vars m =
  ...
  | (Positive { row = []; _ } | Negative []) :: _ as empty_rows ->
    ...
    | rows_varsets ->
      let stables_in_varsets = reduce (List.map2 Ident.Set.inter) rows_varsets
      in
...
let all_rhs_idents exp =
  ...
  let is_unpack exp =
    List.exists (fun attr -> attr.Parsetree.attr_name.txt = "#modulepat")
      exp.exp_attributes
  in
          ...
          (if not (Ident.Set.is_empty all) then
             ...
             | Vars stable ->
               ...
               if not (Ident.Set.is_empty ambiguous) then
                 let pps = Ident.Set.elements ambiguous |> List.map Ident.name
                 in
