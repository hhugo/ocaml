...
let rec normalize_pat q =
  ...
  | Tpat_or _ | Tpat_exception _ -> fatal_error "Parmatch.normalize_pat"
...
let close_variant env row =
  ...
  let nm =
    List.fold_left
      ...
      row.row_fields
  in
  if (not row.row_closed) || nm != row.row_name
  then
...
let full_match closing env =
  ...
  | (({pat_desc = Tpat_variant _} as p), _) :: _ ->
      ...
      if closing && not (Btype.row_fixed row)
      then
...
let complete_tags nconsts nconstrs tags =
  ...
  for i = 0 to nconsts - 1 do
    if not seen_const.(i)
    then ConstructorTagHashtbl.add r (Cstr_constant i) ()
  done;
  for i = 0 to nconstrs - 1 do
    if not seen_constr.(i)
    then ConstructorTagHashtbl.add r (Cstr_block i) ()
  done;
  r
...
let build_other ext env =
  ...
  | ({pat_desc = Tpat_construct (lid, {cstr_tag = Cstr_extension _}, _)}, _)
    :: _ ->
      ...
      make_pat
        (Tpat_var
           (Ident.create_local "*extension*", {lid with txt = "*extension*"}))
        Ctype.none
        Env.empty
  ...
  | (({pat_desc = Tpat_constant (Const_nativeint _)} as p), _) :: _ ->
      build_other_constant
        (function
          | Tpat_constant (Const_nativeint i) -> i | _ -> assert false)
...
let rec satisfiable pss qs =
  ...
  | _ -> (
    ...
    | q :: qs ->
        let pss = simplify_first_col pss in
        if not (all_coherent (q :: first_column pss))
        then false
        else
  ...
  | q :: rem -> (
      ...
      | _ ->
          ...
          (* The handling of incoherent matrices is kept in line with
           [satisfiable] *)
          if not (all_coherent (uq :: first_column pss))
          then Unused
          else
            let q0 = discr_pat q pss in
            every_satisfiables
...
and every_both pss qs q1 q2 =
  ...
  and r2 =
    every_satisfiables (if compat q1 q2 then qs1 :: pss else pss) qs2
  in
  match r1 with
  | Unused -> (
    match r2 with
...
let rec lub p q =
  ...
  | _, Tpat_or (q1, q2, _) ->
      orlub q1 q2 p (* Thanks god, lub is commutative *)
  ...
  | Tpat_variant (l1, None, _row), Tpat_variant (l2, None, _) when l1 = l2
    ->
      p
...
let pressure_variants tdefs patl =
  let add_row pss p_opt =
    match p_opt with None -> pss | Some p -> [p; omega] :: pss
  in
  let val_pss, exn_pss =
    List.fold_right
      (fun pat (vpss, epss) ->
              ...
              then ()
                ...
                let r =
                  ...
                  let skip =
                    ...
                    || not
                         ( refute
                         || Warnings.is_active Warnings.Unreachable_case )
                  in
                  if skip
                  then r
                  else
    ...
    let leave_expression exp =
      ...
      then
        ...
        | Texp_letmodule
            ...
            if not (Ident.Set.mem id_mod !ids)
            then ids := Ident.Set.remove id_exp !ids
        | _ -> assert false
  end) in
