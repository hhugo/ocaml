...
let classify_expression : Typedtree.expression -> sd =
  ...
  *)
  let rec classify_expression env e =
    ...
    | Texp_for _ | Texp_constant _ | Texp_new _ | Texp_instvar _
     |Texp_tuple _ | Texp_array _ | Texp_variant _ | Texp_setfield _
     |Texp_while _ | Texp_setinstvar _ | Texp_pack _ | Texp_object _
     |Texp_function _ | Texp_lazy _ | Texp_unreachable
 ...
 fun exp ->
  ...
  | Texp_match (e, cases, _) ->
      ...
      fun mode ->
       ...
       in
       let env_e =
         expression e (List.fold_left Mode.join Ignore pat_modes)
       in
  ...
  | Texp_record {fields = es; extended_expression = eo; representation = rep}
    ->
      ...
      in
      join
        [array field es << field_mode; option expression eo << Dereference]
  ...
  | Texp_send (e1, _, eo) ->
      ...
      *)
      join
        [expression e1 << Dereference; option expression eo << Dereference]
  | Texp_field (e, _, _) ->
      (*
        G |- e: m[Dereference]
        -----------------------
  ...
  | Texp_lazy e ->
      ...
      *)
      let lazy_mode =
        ...
        | `Constant_or_function | `Identifier _
...
and modexp : Typedtree.module_expr -> term_judg =
 fun mexp ->
  ...
  | Tmod_functor (_, _, _, e) -> modexp e << Delay
  | Tmod_apply (f, p, _) ->
...
and structure : Typedtree.structure -> term_judg =
 ...
 fun s m ->
  List.fold_right
    (fun it env -> structure_item it m env)
    s.str_items
    Env.empty
        ...
        *)
        ...
        let rec transitive_closure env =
          let transitive_deps env_i mdef_i =
            ...
            Env.join
              env_i
              (Env.join_list (List.map2 Env.compose mdef_i env))
          in
...
*)
and pattern : pattern -> Env.t -> mode =
 fun pat env ->
  (*
