...
let is_ref : Types.value_description -> bool =
  function
  | {
      Types.val_kind =
...
let classify_expression : Typedtree.expression -> sd =
  ...
  *)
  let rec classify_expression env e =
    ...
    | Texp_record
        { representation = Record_unboxed _; fields = [| _, Overridden (_, e) |]
        }
      ->
      classify_expression env e
...
module Env : sig
  ...
  *)
  ...
  val empty : t (** An environment with no used identifiers. *)
  ...
  let join (x : t) (y : t) =
    M.fold
      (fun (id : Ident.t) (v : Mode.t) (tbl : t) ->
         ...
         M.add id (Mode.join v v') tbl) x y
...
let rec expression : Typedtree.expression -> term_judg =
  fun exp ->
    ...
    | Texp_for (_, _, low, high, _, body) ->
      ...
      *)
      join
        [ expression low << Dereference; expression high << Dereference;
    ...
    | Texp_ifthenelse (cond, ifso, ifnot) ->
      ...
      *)
      join
        [ expression cond << Dereference; expression ifso;
...
and structure_item : Typedtree.structure_item -> bind_judg =
  fun s m env ->
    ...
    | Tstr_class classes ->
      ...
      in
      Env.join (list class_declaration classes m)
      ...
      | Recursive ->
        ...
        *)
        ...
        let rec transitive_closure env =
          ...
          if List.for_all2 Env.equal env env' then
            env'
          else
            transitive_closure env'
        in
...
and pattern : type k. k general_pattern -> Env.t -> mode =
  ...
  *)
    ...
    let m_env =
      pat_bound_idents pat |> List.map (fun id -> Env.find id env) |>
...
let is_valid_recursive_expression idlist expr =
  ...
  | Texp_function _ ->
    ...
    true
  | _ ->
    begin match classify_expression expr with
    | Static ->
