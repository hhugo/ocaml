...
let rec generalize_structure ty =
  ...
  if level <> generic_level then
    if is_Tvar ty && level > !current_level then set_level ty !current_level
    else if
      level > !current_level
      &&
      match get_desc ty with
...
let rec copy ?partial ?keep_names scope ty =
  ...
  | desc ->
      let level = get_level ty in
      if level <> generic_level && partial = None then ty
      else
        (* We only forget types that are non generic and do not contain
           free univars *)
        let forget =
          if level = generic_level then generic_level
          else
            match partial with
            | None -> assert false
        ...
        in
        if forget <> generic_level then newty2 ~level:forget (Tvar None)
        else
          ...
          *)
            ...
            | Tvariant row0 -> (
                ...
                | _ ->
                    ...
                    let keep =
                      get_level more <> generic_level && partial = None
                    in
                    let more' =
                      match mored with
                      | Tsubst (ty, None) ->
                    ...
                    let more', row =
                      ...
                      | Some (free_univars, false) ->
                          let more' =
                            if not (eq_type more more') then more'
                              (* we've already made a copy *)
                            else newvar ()
                          in
                          let not_reither (_, f) =
                            match row_field_repr f with
...
let rec copy_sep ~cleanup_scope ~fixed ~free ~bound ~may_share
  ...
  if is_Tvar ty || (may_share && TypeSet.is_empty univars) then (
    if get_level ty <> generic_level then ty
    else
      let t = newstub ~scope:(get_scope ty) in
      delayed_copy :=
...
let rec extract_concrete_typedecl env ty =
  ...
  | Tarrow _ | Ttuple _ | Tobject _ | Tfield _ | Tnil | Tvariant _ | Tpackage _
    ->
      Has_no_typedecl
...
let occur_for tr_exn env t1 t2 =
  try occur env t1 t2 with Occur -> raise_for tr_exn (Rec_occur (t1, t2))
      ...
      | Tconstr (p1, [], _), Tconstr (p2, [], _)
        when Env.has_local_constraints !env
          ...
          try
            ...
            else unify env (try_expand_safe !env t1) t2
...
and make_rowvar level use1 rest1 use2 rest2 =
  let set_name ty name =
    match get_desc ty with Tvar None -> set_type_desc ty (Tvar name) | _ -> ()
  in
  let name =
...
let rec filter_method_row env name priv ty =
  ...
  | Tvar _ ->
      ...
      let kind =
        match priv with Private -> Fvar (ref None) | Public -> Fpresent
      in
...
let add_dummy_method env ~scope sign =
  let ty, row = filter_method_row env dummy_method Private sign.csig_self_row in
...
let add_method env label priv virt ty sign =
  ...
  let priv, virt =
    ...
    | priv', virt', ty' -> (
        let priv = match priv' with Public -> Public | Private -> priv in
...
let hide_private_methods env sign =
  ...
  List.iter
    (fun (_, k, _) ->
      match field_kind_repr k with Fvar r -> set_kind r Fabsent | _ -> ())
    fields
...
let moregen_occur env level ty =
  let rec occur ty =
    ...
    if lv <= level then ()
    else if is_Tvar ty && lv >= generic_level - 1 then raise Occur
    else if try_mark_node ty then iter_type_expr occur ty
  in
  (try
     occur ty;
     unmark_type ty
...
let all_distinct_vars env vars =
  ...
  List.for_all
    (fun ty ->
      let ty = expand_head env ty in
      if TypeSet.mem ty !tys then false
      else (
...
let matches ~expand_error_trace env ty ty' =
  ...
  | () ->
      if not (all_distinct_vars env vars) then (
        backtrack snap;
        let diff =
          if expand_error_trace then expanded_diff env ~got:ty ~expected:ty'
  ...
  if
    List.exists
      (fun (t, t') ->
        ...
        else if found1 || found2 then raise_unexplained_for Equality
        else false)
      !subst
  then ()
  else (
...
let rec build_subtype env (visited : transient_expr list)
  ...
  | Tarrow (l, t1, t2, _) ->
      ...
      if memq_warn tt visited then (t, Unchanged)
      else
    ...
    when level > 0 && generic_abbrev env p && safe_abbrev env t
      ...
      try
        ...
        | Tobject _ when posi && not (opened_object t') ->
            ...
            let ty =
              try
                subst env !current_level Public abbrev None cl_abbr.type_params
                  tl body
  ...
  | Tvariant row ->
      let row = row_repr row in
      let tt = Transient_expr.repr t in
      if memq_warn tt visited || not (static_row row) then (t, Unchanged)
      else
  ...
  | Tobject (t1, _) ->
      let tt = Transient_expr.repr t in
      if memq_warn tt visited || opened_object t1 then (t, Unchanged)
      else
        let level' = pred_enlarge level in
        let visited =
          tt :: (if level' < level then [] else filter_visited visited)
...
and subtype_fields env trace ty1 ty2 cstrs =
  ...
  let pairs, miss1, miss2 = associate_fields fields1 fields2 in
  let cstrs =
    if get_desc rest2 = Tnil then cstrs
    else if miss1 = [] then
      subtype_rec env
...
and subtype_row env trace row1 row2 cstrs =
  ...
  let more1 = row1.row_more and more2 = row2.row_more in
  ...
  if TypeSet.mem ty !visited then ()
    ...
    | Tconstr _ -> (
        ...
        with Nongen -> (
          ...
          with Cannot_expand -> raise Nongen))
    | Tfield (_, kind, t1, t2) ->
                        ...
                        List.fold_left
                          (fun tyl ty ->
                            if
                              List.exists
                                (fun ty' ->
                                  is_equal Env.empty false [ ty ] [ ty' ])
                                tyl
                            then tyl
                            else ty :: tyl)
                          [ ty ] tyl
