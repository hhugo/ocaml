...
let raise_trace_for
    (type variant) (tr_exn : variant trace_exn) (tr : variant trace)
  :
  'a
=
  match tr_exn with
         ...
         Location.(
           errorf ~loc:(in_file !input_name)
             "In this program,@ variant constructors@ `%s and `%s@ have the \
              same hash value.@ Change one of them." l l'
         )
     | _ -> None)
...
let proper_abbrevs path tl abbrev =
  if
    tl <> [] ||
      !trace_gadt_instances || !Clflags.principal || is_object_type path
  then
    abbrev
  else
    simple_abbrevs
...
let set_mode_pattern ~generate ~injective ~allow_recursive f =
  Misc.protect_refs
    [ Misc.R (umode, Pattern); Misc.R (equations_generation, generate);
      Misc.R (assume_injective, injective);
      Misc.R (allow_recursive_equation, allow_recursive) ] f
...
let rec merge_rf r1 r2 pairs fi1 fi2 =
  ...
  | (l1, f1 as p1) :: fi1', (l2, f2 as p2) :: fi2' ->
    if l1 = l2 then
      merge_rf r1 r2 ((l1, f1, f2) :: pairs) fi1' fi2'
    else if l1 < l2 then
...
let rec filter_row_fields erase =
  function
  | [] -> []
  | (_l, f as p) :: fi ->
    ...
    Meths.iter
      (fun lab (priv, _, ty) ->
         if priv = Public then
           try closed_type ty
           with
           | Non_closed (ty0, real) -> raise (CCFailure (ty0, real, lab, ty)))
      sign.csig_meths;
    List.iter unmark_type params;
...
let rec generalize ty =
  ...
  if level > !current_level && level <> generic_level then
...
let rec check_scope_escape env level ty =
  ...
  if try_logged_mark_node ty then
    ...
    ((if level < get_scope ty then raise_scope_escape_exn ty);
     ...
     | Tconstr (p, _tl, _abbrev) when level < Path.scope p ->
       ...
       (try
        ...
        with
        | Cannot_expand -> raise_escape_exn (Constructor p))
     | Tconstr (p, (_ :: _ as tl), _) ->
       let variance =
         try (Env.find_type p env).type_variance
         with
         | Not_found -> List.map (fun _ -> Variance.unknown) tl
       in
       let needs_expand =
         expand ||
           List.exists2
           ...
           List.iter2
             (fun v t ->
                if v = Variance.null then
                  ()
                else if Variance.(mem May_weak v) then
                  lower_rec true t
                else
                  lower_rec contra t) variance tyl
         in
...
let limited_generalize ty0 ty =
  ...
  let rec inverse pty ty =
    ...
    else if level < lowest_level then
    ...
    if idx <> generic_level then
      (set_level ty generic_level;
       ...
       | Tvariant row ->
         ...
         if
           (lv < lowest_level || lv > !current_level) && lv <> generic_level
         then
           set_level more generic_level
       | _ -> ())
  in
  inverse [] ty;
  (if get_level ty0 < lowest_level then iter_type_expr (inverse []) ty0);
  List.iter generalize_parents !roots;
  Hashtbl.iter
    (fun _ (ty, _) ->
       if get_level ty <> generic_level then set_level ty !current_level) graph
...
let rec copy ?partial ?keep_names scope ty =
  ...
  | desc ->
    let level = get_level ty in
    if level <> generic_level && partial = None then
      ty
    else
          ...
          *)
               Tconstr
                 (p,
                  List.map copy tl,
                  ref
                    begin match !(!abbreviations) with
           ...
           | Tvariant row0 ->
             ...
             | _ ->
               ...
               For_copy.redirect_desc scope more (Tsubst (more', Some t));
               (* Return a new copy *)
...
let get_new_abstract_name s =
  ...
  if index = 0 && s <> "" && s.[String.length s - 1] <> '$' then
    s
  else
      ...
      Cty_signature
        {
          ...
          csig_vars =
            Vars.map (function m, v, ty -> m, v, copy scope ty) sign.csig_vars;
          csig_meths =
            Meths.map (function p, v, ty -> p, v, copy scope ty) sign.csig_meths
        }
    | Cty_arrow (l, ty, cty) ->
...
let rec copy_sep
    ~cleanup_scope ~fixed ~free ~bound ~may_share
  ...
  if is_Tvar ty || may_share && TypeSet.is_empty univars then
    ...
    | Not_found ->
      ...
      let visited =
        ...
        | Tvar _ | Tfield _ | Tnil | Tpoly _ | Tunivar _ -> visited
      ...
      let desc' =
        ...
        | Tvariant row0 ->
          ...
          let row = copy_row (copy_rec ~may_share:true) fixed' row keep more' in
          Tvariant row
        | Tpoly (t1, tl) ->
          let tl' = List.map (fun t -> newty (get_desc t)) tl in
          let bound = tl @ bound in
          let visited =
            List.map2 (fun ty t -> get_id ty, (t, bound)) tl tl' @ visited
          in
          let body =
            copy_sep ~cleanup_scope ~fixed ~free ~bound ~may_share:true visited
              t1
          in
          Tpoly (body, tl')
        | Tfield (p, k, ty1, ty2) ->
  ...
  For_copy.with_scope
    (fun scope ->
       let (vars, ty_arg) =
         ...
         | Tpoly (ty, tl) -> instance_poly' scope ~keep_names:false fixed tl ty
...
let expand_abbrev_gen kind find_type_expansion env ty =
  ...
  | Tconstr (path, args, abbrev) ->
    ...
    | Some ty' ->
      ...
      ty'
    | None ->
      begin match find_type_expansion path env with
      | exception Not_found ->
        ...
        if Path.same path path' then
          ...
          newty2 ~level (Tconstr (path', args, abbrev))
      | params, body, lv ->
        (* prerr_endline
        ...
        let ty' =
          ...
          with
          | Cannot_subst -> raise_escape_exn Constraint
        in
...
let expand_head_once env ty =
  try expand_abbrev env ty
  with
  | Cannot_expand | Escape _ -> assert false
...
let rec extract_concrete_typedecl env ty =
  ...
  | Tconstr (p, _, _) ->
    ...
    if decl.type_kind <> Type_abstract then
      ...
      | ty ->
        begin match extract_concrete_typedecl env ty with
...
let expand_head_opt env ty =
  try try_expand_head try_expand_safe_opt env ty
  with
  | Cannot_expand -> ty
...
(* trace possible changes to the studied type *)
...
let occur_for tr_exn env t1 t2 =
  try occur env t1 t2
  with
  | Occur -> raise_for tr_exn (Rec_occur (t1, t2))
...
let rec local_non_recursive_abbrev ~allow_rec strict visited env p ty =
  ...
  if not (List.memq (get_id ty) visited) then
    match get_desc ty with
    | Tconstr (p', args, _abbrev) ->
      ...
      if allow_rec && not strict && is_contractive env p' then
        ...
        (try (* try expanding, since [p] could be hidden *)
    ...
    if not_marked_node ty then
      (if TypeSet.is_empty bound then
         ...
         try
           ...
           if not (TypeSet.subset bound' bound) then
      ...
      (try
         ...
         List.iter2
           (fun t v ->
              ...
              if Variance.(if inj_only then mem Inj v else not (eq v null)) then
                occur_rec bound t) tl td.type_variance
       with
       | Not_found -> if not inj_only then List.iter (occur_rec bound) tl)
      ...
      (visited := TypeSet.add t !visited;
       ...
       | Tconstr (p, tl, _) ->
         (try
            ...
            List.iter2
              (* see occur_univar *)
              (fun t v -> if not Variance.(eq v null) then occur t) tl
              td.type_variance
          with
...
let enter_poly env univar_pairs t1 tl1 t2 tl2 f =
  ...
  and cl2 = List.map (fun t -> t, ref None) tl2
  in
...
let deep_occur t0 ty =
  let rec occur_rec ty =
    if get_level ty >= get_level t0 && try_mark_node ty then
...
let is_newtype env p =
  try
    ...
    decl.type_expansion_scope <> Btype.lowest_level &&
      decl.type_kind = Type_abstract && decl.type_private = Public
  with
  | Not_found -> false
  ...
  try
    ...
    decl.type_kind = Type_abstract &&
      decl.type_private = Public &&
        decl.type_arity = 0 &&
          decl.type_manifest = None && not (non_aliasable p decl)
  with
  | Not_found -> false
...
and mcomp_fields type_pairs env ty1 ty2 =
  ...
  let has_present = List.exists (fun (_, k, _) -> field_kind_repr k = Fpresent)
  in
  mcomp type_pairs env rest1 rest2;
  (if
     has_present miss1 && get_desc (object_row ty2) = Tnil ||
  ...
  List.iter
    (function
     | _n, k1, t1, k2, t2 ->
       mcomp_kind k1 k2;
       mcomp type_pairs env t1 t2) pairs

and mcomp_kind k1 k2 =
  let k1 = field_kind_repr k1 in
  let k2 = field_kind_repr k2 in
  match k1, k2 with
  | Fpresent, Fabsent | Fabsent, Fpresent -> raise Incompatible
  | _ -> ()

and mcomp_row type_pairs env row1 row2 =
  let row1 = row_repr row1
  and row2 = row_repr row2
  in
  ...
  try
    ...
    if compatible_paths p1 p2 then
      let inj =
        ...
        with
        | Not_found -> List.map (fun _ -> false) tl1
      in
      List.iter2 (fun i (t1, t2) -> if i then mcomp type_pairs env t1 t2) inj
...
and mcomp_variant_description type_pairs env xs ys =
  let rec iter x y =
    ...
    | c1 :: xs, c2 :: ys ->
      ...
      if Ident.name c1.cd_id = Ident.name c2.cd_id then
        iter xs ys
      else
        raise Incompatible
    | [], [] -> ()
...
let mcomp_for tr_exn env t1 t2 =
  try mcomp env t1 t2
  with
  | Incompatible -> raise_unexplained_for tr_exn
...
let add_gadt_equation env source destination =
  ...
  else if local_non_recursive_abbrev !env source destination then
    let destination = duplicate_type destination in
    let expansion_scope =
      Int.max (Path.scope source) (get_gadt_equations_level ())
    in
    let decl =
      new_local_type ~manifest_and_scope:(destination, expansion_scope) ()
    in
...
let complete_type_list ?(allow_absent=false) env fl1 lv2 mty2 fl2 =
  ...
  let rec complete fl1 fl2 =
    ...
    | (n, _) :: nl, _ ->
      ...
      begin match Env.find_type_by_name lid env' with
      | _,
        { type_arity = 0; type_kind = Type_abstract; type_private = Public;
...
let unify_package env unify_list lv1 p1 fl1 lv2 p2 fl2 =
  ...
  and ntl1 = complete_type_list env fl2 lv1 (Mty_ident p1) fl1
  in
  unify_list (List.map snd ntl1) (List.map snd ntl2);
  if
    eq_package_path env p1 p2 ||
      !package_subtype env p1 fl1 p2 fl2 && !package_subtype env p2 fl2 p1 fl1
  then
    ()
  else
    raise Not_found
...
let unify1_var env t1 t2 =
  ...
  | () ->
    ...
    link_type t1 t2;
    true
  | exception (Unify_trace _) when !umode = Pattern -> false
...
let rec unify (env : Env.t ref) t1 t2 =
  ...
  if unify_eq t1 t2 then
    ...
    try
      ...
      | Tvar _, _ -> if unify1_var !env t1 t2 then () else unify2 env t1 t2
      | _, Tvar _ -> if unify1_var !env t2 t1 then () else unify2 env t1 t2
      | Tunivar _, Tunivar _ ->
      ...
      | Tconstr (p1, [], a1), Tconstr (p2, [], a2)
        when
          Path.same p1 p2 (* && actual_mode !env = Old *) (* This optimization assumes that t1 does not expand to t2
...
and unify2 env t1 t2 =
  ...
  if unify_eq t1' t2' then
    ...
    if unify_eq t1 t1' || not (unify_eq t2 t2') then
      ...
      with
      | Unify_trace trace -> raise_trace_for Unify (swap_trace trace)
...
and unify3 env t1 t1' t2 t2' =
  ...
  | Tunivar _, Tunivar _ ->
    unify_univar_for Unify t1' t2' !univar_pairs;
    link_type t1' t2'
  | Tvar _, _ -> unify3_var env t1' t2 t2'
  | _, Tvar _ -> unify3_var env t2' t1 t1'
  | Tfield _, Tfield _ ->
  ...
  | _ ->
    ...
    (try
                  ...
                  set_mode_pattern ~generate:Forbidden ~injective:false
                    ...
                    (fun () ->
                       ...
                       | Unify_trace _ ->
                         backtrack snap;
                         reify env t1;
                         reify env t2)) inj (List.combine tl1 tl2)
       | Tconstr (path, [], _), Tconstr (path', [], _)
         when
           is_instantiable !env path &&
             is_instantiable !env path' && can_generate_equations ()
         ->
       ...
       | Tpackage (p1, fl1), Tpackage (p2, fl2) ->
         (try
            unify_package !env (unify_list env) (get_level t1) p1 fl1
              (get_level t2) p2 fl2
          with
          | Not_found ->
...
and make_rowvar level use1 rest1 use2 rest2 =
  ...
  let name =
    ...
    | _ -> None
  in
  if use1 then rest1 else if use2 then rest2 else newty2 ~level (Tvar name)

and unify_fields env ty1 ty2 =
  (* Optimization *)
  let (fields1, rest1) = flatten_fields ty1
  and (fields2, rest2) = flatten_fields ty2
  in
  let (pairs, miss1, miss2) = associate_fields fields1 fields2 in
  let l1 = get_level ty1
  and l2 = get_level ty2
  in
  let va = make_rowvar (Int.min l1 l2) (miss2 = []) rest1 (miss1 = []) rest2 in
  let tr1 = Transient_expr.repr rest1
  and tr2 = Transient_expr.repr rest2
  in
  let d1 = tr1.desc
  and d2 = tr2.desc
  in
      ...
      (fun (name, k1, t1, k2, t2) ->
         ...
         | Unify_trace trace ->
           raise_trace_for Unify
             (incompatible_fields ~name ~got:t1 ~expected:t2 :: trace)) pairs
  with
  | exn ->
...
and unify_row env row1 row2 =
  ...
  and row2 = row_repr row2
  in
  let rm1 = row_more row1
  and rm2 = row_more row2
  in
  if unify_eq rm1 rm2 then
    ...
    ((if r1 <> [] && r2 <> [] then
        ...
        (List.iter (fun (l, _) -> Hashtbl.add ht (hash_variant l) l) r1;
         List.iter
           (fun (l, _) ->
              ...
              with
              | Not_found -> ()) r2));
     let fixed1 = fixed_explanation row1
     and fixed2 = fixed_explanation row2
     in
     let more =
     ...
     and closed = row1.row_closed || row2.row_closed
     in
     let keep switch =
       List.for_all
         (fun (_, f1, f2) ->
            let (f1, f2) = switch f1 f2 in
            row_field_repr f1 = Rabsent || row_field_repr f2 <> Rabsent) pairs
     in
     let empty fields =
     ...
     let set_more row rest =
       let rest = if closed then filter_row_fields row.row_closed rest else rest
       in
       ...
       if row_fixed row then
         (if eq_type more rm then
            ()
          else if is_Tvar rm then
            link_type rm more
          else
            unify env rm more)
       else
         ...
         (update_level_for Unify !env (get_level rm) ty;
          ...
          link_type rm ty)
     in
     let tm1 = Transient_expr.repr rm1
     and tm2 = Transient_expr.repr rm2
     in
     let md1 = tm1.desc
     and md2 = tm2.desc
     in
     try
       ...
       List.iter
         (fun (l, f1, f2) ->
            ...
            | Unify_trace trace ->
              raise_trace_for Unify
                (Variant (Incompatible_types_for l) :: trace)) pairs;
...
and unify_row_field env fixed1 fixed2 rm1 rm2 l f1 f2 =
  let f1 = row_field_repr f1
  and f2 = row_field_repr f2
  in
  ...
  in
  let first = First, fixed1
  and second = Second, fixed2
  in
  let either_fixed =
    match fixed1, fixed2 with
    | None, None -> false
  ...
  if f1 == f2 then
    ...
    | Reither (c1, tl1, m1, e1), Reither (c2, tl2, m2, e2) ->
      ...
      then
        ...
        in
        if redo then
          unify_row_field env fixed1 fixed2 rm1 rm2 l f1 f2
        else
          let remq tl =
            List.filter (fun ty -> not (List.exists (eq_type ty) tl))
          in
          let tl1' = remq tl2 tl1
          and tl2' = remq tl1 tl2
          in
          (* PR#6744 *)
          let (tlu1, tl1') = List.partition (has_free_univars !env) tl1'
          and (tlu2, tl2') = List.partition (has_free_univars !env) tl2'
          in
          (begin match tlu1, tlu2 with
           ...
           and f2' = Reither (c1 || c2, tl1', m1 || m2, e)
           in
           set_row_field e1 f1';
           set_row_field e2 f2')
...
let unify_gadt ~equations_level:lev ~allow_recursive (env : Env.t ref) ty1 ty2 =
  try
    ...
    set_mode_pattern ~generate:(Allowed { equated_types }) ~injective:true
      ~allow_recursive (fun () -> unify env ty1 ty2);
...
let filter_arrow env t l =
  let function_type level =
    let t1 = newvar2 level
    and t2 = newvar2 level
    in
  ...
  | Tvar _ ->
    ...
    t1, t2
  | Tarrow (l', t1, t2, _) ->
    if l = l' || !Clflags.classic && l = Nolabel && not (is_optional l') then
      t1, t2
    else
      raise
        (Filter_arrow_failed
...
let rec filter_method_field env name ty =
  let method_type ~level =
    let ty1 = newvar2 level
    and ty2 = newvar2 level
    in
...
let rec filter_method_row env name priv ty =
  ...
  | Tfield (n, kind, ty1, ty2) ->
    ...
    if n = name && kind <> Fabsent then
      ...
      field, row
  | Tnil ->
    if name = Btype.dummy_method then
...
let add_dummy_method env ~scope sign =
  let (ty, row) = filter_method_row env dummy_method Private sign.csig_self_row
  in
  ...
  Meths.iter
    (fun label (priv, virt, ty) ->
       ...
       | exception (Add_method_failed failure) ->
         ...
         raise (Inherit_class_signature_failed failure)) sign2.csig_meths;
  Vars.iter
    (fun label (mut, virt, ty) ->
       ...
       | exception (Add_instance_variable_failed failure) ->
         ...
         raise (Inherit_class_signature_failed failure)) sign2.csig_vars
...
let generalize_class_signature_spine env sign =
  ...
  Meths.iter (fun _ (_, _, ty) -> unify_var env (newvar ()) ty) meths;
...
let moregen_occur env level ty =
  let rec occur ty =
    ...
    if lv <= level then
      ()
    else if is_Tvar ty && lv >= generic_level - 1 then
      raise Occur
    else if try_mark_node ty then
           ...
           | Not_found ->
             ...
             | Tpackage (p1, fl1), Tpackage (p2, fl2) ->
               (try
                ...
                with
                | Not_found -> raise_unexplained_for Moregen)
...
and moregen_row inst_nongen type_pairs env row1 row2 =
  ...
  and row2 = row_repr row2
  in
  let rm1 = row1.row_more
  and rm2 = row2.row_more
  in
  if eq_type rm1 rm2 then
    ()
  else
    let may_inst =
    ...
    ((if r1 <> [] then raise_for Moregen (Variant (No_tags (Second, r1))));
     ...
     | Tunivar _, Tunivar _ -> unify_univar_for Moregen rm1 rm2 !univar_pairs
         ...
         (fun (l, f1, f2) ->
            ...
            if f1 == f2 then
              ...
              | ((Rpresent _ | Reither _) as r), Rabsent ->
                raise_for Moregen (Variant (No_tags (Second, [ l, r ])))) pairs
     with
     | exn ->
...
let moregeneral env inst_nongen pat_sch subj_sch =
  ...
  *)
  ...
  Misc.try_finally
    (fun () ->
       try moregen inst_nongen (TypePairs.create 13) env patt subj
       with
       | Moregen_trace trace ->
...
let rec rigidify_rec vars ty =
  if try_mark_node ty then
    match get_desc ty with
    | Tvar _ -> if not (TypeSet.mem ty !vars) then vars := TypeSet.add ty !vars
    | Tvariant row ->
      let row = row_repr row in
      let more = row.row_more in
...
let matches ~expand_error_trace env ty ty' =
  ...
  | () ->
    (if not (all_distinct_vars env vars) then
       (backtrack snap;
        let diff =
          if expand_error_trace then
            expanded_diff env ~got:ty ~expected:ty'
          else
            unexpanded_diff ~got:ty ~expected:ty'
        in
...
let rec eqtype rename type_pairs subst env t1 t2 =
  if eq_type t1 t2 then
    ...
    try
      match get_desc t1, get_desc t2 with
      | Tvar _, Tvar _ when rename -> eqtype_subst type_pairs subst t1 t2
      | Tconstr (p1, [], _), Tconstr (p2, [], _) when Path.same p1 p2 -> ()
      | _ ->
             ...
             | Tpackage (p1, fl1), Tpackage (p2, fl2) ->
               (try
                  unify_package env (eqtype_list rename type_pairs subst env)
                    (get_level t1') p1 fl1 (get_level t2') p2 fl2
                with
                | Not_found -> raise_unexplained_for Equality)
...
and eqtype_fields rename type_pairs subst env ty1 ty2 =
  ...
  let same_row = eq_type rest1 rest2 || TypePairs.mem type_pairs (rest1, rest2)
  in
  if same_row then
    ()
  else (* Try expansion, needed when called from Includecore.type_manifest *)
    match get_desc (expand_head_rigid env rest2) with
    | Tobject (ty2, _) -> eqtype_fields rename type_pairs subst env ty1 ty2
    | _ ->
      let (pairs, miss1, miss2) = associate_fields fields1 fields2 in
      eqtype rename type_pairs subst env rest1 rest2;
      begin match miss1, miss2 with
      | (n, _, _) :: _, _ ->
...
and eqtype_row rename type_pairs subst env row1 row2 =
  ...
  | _ ->
    ...
    in
    let (r1, r2, pairs) = merge_row_fields row1.row_fields row2.row_fields in
    (if row1.row_closed <> row2.row_closed then
       raise_for Equality
    ...
    (if not row1.row_closed then
       ...
       | _, _ -> ());
    begin match filter_row_fields false r1 with
    | [] -> ()
    | _ :: _ as r1 -> raise_for Equality (Variant (No_tags (Second, r1)))
    end;
    begin match filter_row_fields false r2 with
    | [] -> ()
    ...
    List.iter
      (fun (l, f1, f2) ->
         ...
         if f1 == f2 then
           ...
           | Reither (c1, t1 :: tl1, _, _), Reither (c2, t2 :: tl2, _, _)
             ...
             (try
                ...
                if List.length tl1 = List.length tl2 then
                  ...
                  List.iter2 (eqtype rename type_pairs subst env) tl1 tl2
                else (* otherwise everything must be equal *)
           ...
           | ((Rpresent _ | Reither _) as r), Rabsent ->
             raise_for Equality (Variant (No_tags (Second, [ l, r ])))) pairs

(* Must empty univar_pairs first *)
let eqtype_list rename type_pairs subst env tl1 tl2 =
  univar_pairs := [];
  let snap = Btype.snapshot () in
  Misc.try_finally ~always:(fun () -> backtrack snap)
...
let rec equal_private env params1 ty1 params2 ty2 =
  ...
  | exception (Equality _ as err) ->
    begin match try_expand_safe_opt env (expand_head env ty1) with
    ...
    Meths.fold
      (fun lab (priv, vr, _) err ->
         if Meths.mem lab sign2.csig_meths then
           ...
           | Concrete -> err) sign1.csig_meths errors
  in
  let errors =
    Vars.fold
      ...
      Meths.iter
        (fun lab (_, _, ty) ->
           ...
           | exception Not_found ->
             ...
             assert false
           | _, _, ty' ->
             begin match moregen true type_pairs env ty' ty with
             | () -> ()
    ...
    Meths.iter
      (fun lab (_, _, ty) ->
         ...
         | exception Not_found ->
           ...
           assert false
         | _, _, ty' ->
           begin match eqtype true type_pairs subst env ty' ty with
           | () -> ()
...
let match_class_declarations env patt_params patt_type subj_params subj_type =
  ...
  | [] ->
    (try
       ...
       in
       match_class_types ~trace:false env (clty_params patt_params patt_type)
...
let warn = ref false
(* whether double coercion might do better *)
...
let rec build_subtype
    env (visited : transient_expr list) (loops : (int * type_expr) list) posi
    level t
=
  match get_desc t with
  | Tvar _ ->
    if posi then
      (try
         ...
         t', Equiv
       with
       | Not_found -> t, Unchanged)
    else
      t, Unchanged
  | Tarrow (l, t1, t2, _) ->
    let tt = Transient_expr.repr t in
    if memq_warn tt visited then
      t, Unchanged
    else
  ...
  | Ttuple tlist ->
    ...
    if memq_warn tt visited then
      ...
      let visited = tt :: visited in
      let tlist' = List.map (build_subtype env visited loops posi level) tlist
      in
      let c = collect tlist' in
      if c > Unchanged then
        newty (Ttuple (List.map fst tlist')), c
      else
        t, Unchanged
  | Tconstr (p, tl, abbrev)
    when
      level > 0 &&
        generic_abbrev env p && safe_abbrev env t && not (has_constr_row' env t)
    ->
    ...
    (try
       ...
       | Tobject _ when posi && not (opened_object t') ->
         ...
         let (ty1', c) =
           build_subtype env [ Transient_expr.repr t' ] loops posi
             (pred_enlarge level') ty1
         in
         assert (is_Tvar t'');
         let nm =
         ...
         (if
            level = 0 &&
              generic_abbrev env p &&
                safe_abbrev env t && not (has_constr_row' env t)
          then
            warn := true);
         let tl' =
           List.map2
             (fun v t ->
                ...
                if cn then
                  (if co then
                     ...
                     build_subtype env visited loops (not posi) level t)
                else if co then
                  build_subtype env visited loops posi level t
                else
                  newvar (), Changed) decl.type_variance tl
         in
         let c = collect tl' in
         if c > Unchanged then
           newconstr p (List.map fst tl'), c
         else
           t, Unchanged
       with
       | Not_found -> t, Unchanged)
  | Tvariant row ->
    let row = row_repr row in
    let tt = Transient_expr.repr t in
    if memq_warn tt visited || not (static_row row) then
      t, Unchanged
    else
      let level' = pred_enlarge level in
      let visited = tt :: if level' < level then [] else filter_visited visited
      in
  ...
  | Tobject (t1, _) ->
    let tt = Transient_expr.repr t in
    if memq_warn tt visited || opened_object t1 then
      t, Unchanged
    else
      let level' = pred_enlarge level in
      let visited = tt :: if level' < level then [] else filter_visited visited
  ...
  | Tfield (s, _, t1, t2) (* Always present *) ->
    ...
    let c = max_change c1 c2 in
    if c > Unchanged then
      newty (Tfield (s, Fpresent, t1', t2')), c
    else
      t, Unchanged
  | Tnil ->
    if posi then
...
let subtype_error ~env ~trace ~unification_trace =
  raise
    (Subtype
       (Subtype.error ~trace:(expand_subtype_trace env (List.rev trace))
          ~unification_trace))
...
let rec subtype_rec env trace t1 t2 cstrs =
  if eq_type t1 t2 then
    ...
    | Not_found ->
      ...
      | Tarrow (l1, t1, u1, _), Tarrow (l2, t2, u2, _)
        when
          l1 = l2 || !Clflags.classic && not (is_optional l1 || is_optional l2)
        ->
        let cstrs =
          subtype_rec env (Subtype.Diff { got = t2; expected = t1 } :: trace) t2
            t1 cstrs
        in
        subtype_rec env (Subtype.Diff { got = u1; expected = u2 } :: trace) u1
          u2 cstrs
             ...
             (fun cstrs v (t1, t2) ->
                ...
                if co then
                  (if cn then
                     ...
                     subtype_rec env
                       (Subtype.Diff { got = t1; expected = t2 } :: trace) t1 t2
                       cstrs)
                else if cn then
                  subtype_rec env
                    (Subtype.Diff { got = t2; expected = t1 } :: trace) t2 t1
                    cstrs
                else
                  cstrs) cstrs decl.type_variance (List.combine tl1 tl2)
         with
      ...
      | Tvariant row1, Tvariant row2 ->
        (try subtype_row env trace row1 row2 cstrs
         with
         | Exit -> (trace, t1, t2, !univar_pairs) :: cstrs)
      ...
      | Tpoly (u1, tl1), Tpoly (u2, tl2) ->
        (try
         ...
         with
         | Escape _ -> (trace, t1, t2, !univar_pairs) :: cstrs)
      | Tpackage (p1, fl1), Tpackage (p2, fl2) ->
        (try
           let ntl1 =
           ...
           let cstrs' =
             List.map
               ...
               ntl2
           in
           if eq_package_path env p1 p2 then
             cstrs' @ cstrs
           else (* need to check module subtyping *)
             let snap = Btype.snapshot () in
...
and subtype_list env trace tl1 tl2 cstrs =
  ...
  List.fold_left2
    (fun cstrs t1 t2 ->
       subtype_rec env (Subtype.Diff { got = t1; expected = t2 } :: trace) t1 t2
         cstrs) cstrs tl1 tl2
...
and subtype_fields env trace ty1 ty2 cstrs =
  ...
  let (pairs, miss1, miss2) = associate_fields fields1 fields2 in
  let cstrs =
    if get_desc rest2 = Tnil then
      cstrs
    else if miss1 = [] then
      subtype_rec env (Subtype.Diff { got = rest1; expected = rest2 } :: trace)
        rest1 rest2 cstrs
    else
...
and subtype_row env trace row1 row2 cstrs =
  ...
  and more2 = row2.row_more
  in
  match get_desc more1, get_desc more2 with
  | Tconstr (p1, _, _), Tconstr (p2, _, _) when Path.same p1 p2 ->
    subtype_rec env (Subtype.Diff { got = more1; expected = more2 } :: trace)
      more1 more2 cstrs
...
let unalias ty =
  ...
  | Tvariant row ->
    ...
    newty2 ~level
      (Tvariant
         { row with  row_more = newty2 ~level:(get_level more) (get_desc more) })
...
let rec nongen_schema_rec env ty =
  if TypeSet.mem ty !visited then
    ...
    (visited := TypeSet.add ty !visited;
     match get_desc ty with
     | Tvar _ when get_level ty <> generic_level -> raise Nongen
     | Tconstr _ ->
       let old = !visited in
       (try iter_type_expr (nongen_schema_rec env) ty
        with
        | Nongen ->
          (try
             visited := old;
           ...
           with
           | Cannot_expand -> raise Nongen))
     | Tfield (_, kind, t1, t2) ->
       (if field_kind_repr kind = Fpresent then nongen_schema_rec env t1);
       nongen_schema_rec env t2
     | Tvariant row ->
       let row = row_repr row in
       iter_row (nongen_schema_rec env) row;
       if not (static_row row) then nongen_schema_rec env row.row_more
     | _ -> iter_type_expr (nongen_schema_rec env) ty)
...
let rec nongen_class_type =
  function
  | Cty_constr (_, params, _) -> List.exists (nongen_schema Env.empty) params
  | Cty_signature sign ->
    nongen_schema Env.empty sign.csig_self ||
      nongen_schema Env.empty sign.csig_self_row ||
        Meths.exists (fun _ (_, _, ty) -> nongen_schema Env.empty ty)
          sign.csig_meths ||
          Vars.exists (fun _ (_, _, ty) -> nongen_schema Env.empty ty)
            sign.csig_vars
  | Cty_arrow (_, ty, cty) ->
    nongen_schema Env.empty ty || nongen_class_type cty
                     ...
                     List.fold_left
                       (fun tyl ty ->
                          if
                            List.exists
                              (fun ty' ->
                                 is_equal Env.empty false [ ty ] [ ty' ]) tyl
                          then
                            tyl
                          else
                            ty :: tyl) [ ty ] tyl
     ...
     | Not_found ->
       ...
       let desc =
         ...
         | Tconstr (p, tl, _abbrev) as desc ->
           (try (* First, try keeping the same type constructor p *)
              ...
              ref
                ...
                | Some (p, tl) ->
                  if Path.exists_free ids p then
                    ...
                    Some (p, List.map (nondep_type_rec env ids) tl)
                end)
         | Tvariant row ->
           let row = row_repr row in
           let more = row.row_more in
           ...
           (try
            ...
            | Not_found ->
              ...
              in
              (* Return a new copy *)
              let row = copy_row (nondep_type_rec env ids) true row true more'
              in
              begin match row.row_name with
...
let nondep_type_decl env mid is_covariant decl =
  try
    ...
    let tk =
      ...
      with
      | Nondep_cannot_erase _ when is_covariant -> Type_abstract
    and (tm, priv) =
      ...
      | Some ty ->
        (try Some (nondep_type_rec env mid ty), decl.type_private
         with
         | Nondep_cannot_erase _ when is_covariant ->
           clear_hash ();
           (try Some (nondep_type_rec ~expand_private:true env mid ty), Private
            with
            | Nondep_cannot_erase _ -> None, decl.type_private))
    in
    clear_hash ();
    let priv =
      match tm with
...
let nondep_extension_constructor env ids ext =
  try
    let (type_path, type_params) =
      ...
      | None ->
        let type_params = List.map (nondep_type_rec env ids) ext.ext_type_params
        in
...
let immediacy env typ =
  ...
  | Tconstr (p, _args, _abbrev) ->
    (try
     ...
     with
     | Not_found -> Type_immediacy.Unknown)
