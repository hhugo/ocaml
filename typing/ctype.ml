...
module Unification_trace = struct
  ...
  let swap_elt = function
    ...
    | Obj (Missing_field (pos, s)) ->
        Obj (Missing_field (swap_position pos, s))
  ...
  Location.register_error_of_exn (function
      | Tags (l, l') ->
          Some
            Location.(
              errorf
                ~loc:(in_file !input_name)
                "In this program,@ variant constructors@ `%s and `%s@ have \
                 the same hash value.@ Change one of them."
                l
                l')
      | _ -> None )
  ...
  then (
    ...
    | Tconstr (path, tl, _), Some env ->
        ( try
            ...
            then free_variables := (ty, real) :: !free_variables
          with Not_found -> () );
...
let closed_parameterized_type params ty =
  ...
  let ok = try closed_type ty; true with Non_closed _ -> false in
...
let closed_type_decl decl =
  try
    ...
    unmark_type_decl decl; None
  with Non_closed (ty, _) -> unmark_type_decl decl; Some ty
...
let closed_class params sign =
  ...
  try
    ...
    List.iter
      (fun (lab, kind, ty) ->
        if field_kind_repr kind = Fpresent
        then
          try closed_type ty
  ...
  let rec loop ty =
    ...
    then (
      ...
      | Tpackage (p, nl, tl) when level < Path.scope p ->
          ...
          if Path.same p p'
          then raise Trace.(Unify [escape (Module_type p)]);
          aux {ty with desc = Tpackage (p', nl, tl)}
      | _ -> iter_type_expr loop ty )
  and aux ty = loop ty; unmark_type ty in
  try aux ty
...
let rec generalize_expansive env var_level visited ty =
  ...
  then ()
  else if not (Hashtbl.mem visited ty.id)
  then (
        ...
        let forget =
          ...
          then generic_level
            ...
            | Some (free_univars, keep) ->
                ...
                then if keep then ty.level else !current_level
                else generic_level
        in
        if forget <> generic_level
        then newty2 forget (Tvar None)
        else
          let desc = ty.desc in
          ...
          *)
             ...
             | Tvariant row0 -> (
                 ...
                 | _ ->
                     ...
                     let keep =
                       more.level <> generic_level && partial = None
                     in
                     let more' =
                       match more.desc with
                       | Tsubst ty -> ty
                     ...
                     let more', row =
                       ...
                       | Some (free_univars, false) ->
                           ...
                           if row.row_closed
                              ...
                              && not
                                   (List.for_all not_reither row.row_fields)
                           then
                             ( more'
                             , { row_fields =
             ...
             | Tfield (_p, k, _ty1, ty2) -> (
               ...
               | Fvar r ->
                   For_copy.dup_kind scope r; copy_type_desc copy desc )
...
let safe_abbrev env ty =
  ...
  try
    ...
    true
  with Cannot_expand | Unify _ -> Btype.backtrack snap; false
...
let try_expand_safe env ty =
  ...
  with Unify _ -> Btype.backtrack snap; raise Cannot_expand
...
let occur_in env ty0 t = try occur env ty0 t; false with Unify _ -> true
...
let rec local_non_recursive_abbrev strict visited env p ty =
  ...
  let ty = repr ty in
  if not (List.memq ty visited)
  then
    match ty.desc with
    | Tconstr (p', args, _abbrev) -> (
        if Path.same p p' then raise Occur;
        if (not strict) && is_contractive env p'
        then ()
        else
...
let local_non_recursive_abbrev env p ty =
  try
    ...
    wrap_trace_gadt_instances
      env
      (local_non_recursive_abbrev false [] env p)
      ty;
    true
  with Occur -> false
...
let occur_univar env ty =
  ...
  let rec occur_rec bound ty =
    ...
    if ty.level >= lowest_level
       ...
       then (
         ...
         try
           ...
           if TypeSet.exists (fun x -> not (TypeSet.mem x bound)) bound'
           then (
    ...
    then
      match ty.desc with
      | Tunivar _ ->
          if not (TypeSet.mem ty bound)
          then raise Trace.(Unify [escape (Univ ty)])
      | Tpoly (ty, tyl) ->
          let bound =
            List.fold_right TypeSet.add (List.map repr tyl) bound
          in
          occur_rec bound ty
      | Tconstr (_, [], _) -> ()
      | Tconstr (p, tl, _) -> (
...
let get_univar_family univar_pairs univars =
  ...
  then TypeSet.empty
    ...
    let insert s = function
      | cl1, (_ :: _ as cl2) ->
          if List.exists (fun (t1, _) -> TypeSet.mem (repr t1) s) cl1
...
let univars_escape env univar_pairs vl ty =
  ...
  let rec occur t =
    ...
    then ()
      ...
      match t.desc with
      | Tpoly (t, tl) ->
          if List.exists (fun t -> TypeSet.mem (repr t) family) tl
          then ()
          else occur t
      | Tunivar _ ->
  ...
  let rec subst_univar scope vars ty =
    ...
    then (
      ...
      | Tvar name ->
          ...
          vars := t :: !vars;
          ty.desc <- Tsubst t
      | _ -> iter_type_expr (subst_univar scope vars) ty )
  in
  let vars = ref [] in
  let ty =
    For_copy.with_scope (fun scope ->
...
let deep_occur t0 ty =
  ...
  with Occur -> unmark_type ty; true
  ...
  let rec iterator ty =
    ...
    then ()
      ...
      | Tvar o ->
          ...
          link_type ty t;
          if ty.level < fresh_constr_scope
          then raise Trace.(Unify [escape (Constructor path)])
      | Tvariant r ->
          let r = row_repr r in
          ( if not (static_row r)
          then
...
and mcomp_row type_pairs env row1 row2 =
  ...
  List.iter
    (fun (_, f1, f2) ->
      ...
      | ( Rpresent None
        , (Rpresent (Some _) | Reither (_, _ :: _, _, _) | Rabsent) )
       |( Rpresent (Some _)
        , (Rpresent None | Reither (true, _, _, _) | Rabsent) )
...
let add_gadt_equation env source destination =
  ...
  if local_non_recursive_abbrev !env source destination
  then (
...
and unify2 env t1 t2 =
  ...
  then ()
    ...
    in
    if unify_eq t1 t1' || not (unify_eq t2 t2')
    then unify3 env t1 t1' t2 t2'
    else
...
and unify3 env t1 t1' t2 t2' =
  ...
  | Tunivar _, Tunivar _ ->
      unify_univar t1' t2' !univar_pairs;
      link_type t1' t2'
  | Tvar _, _ -> occur !env t1' t2; occur_univar !env t2; link_type t1' t2
  | _, Tvar _ -> occur !env t2' t1; occur_univar !env t1; link_type t2' t1
  | Tfield _, Tfield _ ->
      ...
      try
                  ...
                  then unify env t1 t2
                    ...
                    set_mode_pattern
                      ...
                      (fun () ->
                        ...
                        try unify env t1 t2
                        with Unify _ ->
                          backtrack snap; reify env t1; reify env t2 ) )
                inj
                (List.combine tl1 tl2)
        | Tconstr (path, [], _), Tconstr (path', [], _)
          when is_instantiable !env path
        ...
        | Tfield (f, kind, _, rem), Tnil | Tnil, Tfield (f, kind, _, rem)
          ...
          | Fvar r when f <> dummy_method ->
              set_kind r Fabsent;
              if d2 = Tnil
              then unify env rem t2'
              else unify env (newty2 rem.level Tnil) rem
          | _ ->
              if f = dummy_method
              then raise (Unify Trace.[Obj Self_cannot_be_closed])
              else if d1 = Tnil
              then raise (Unify Trace.[Obj (Missing_field (First, f))])
        ...
        then
          ...
          | Tconstr (p, tl, abbrev) ->
              ...
              let t2'' = expand_head_unif !env t2 in
              if not (closed_parameterized_type tl t2'')
              then link_type (repr t2) (repr t2')
          | _ -> ()
...
and unify_row env row1 row2 =
  ...
  then ()
    ...
    let name =
      if row1.row_name <> None
         && (row1.row_closed || empty r2)
         && ( (not row2.row_closed)
            || (keep (fun f1 f2 -> f1, f2) && empty r1) )
      then row1.row_name
      else if row2.row_name <> None
              && (row2.row_closed || empty r1)
              && ( (not row1.row_closed)
                 || (keep (fun f1 f2 -> f2, f1) && empty r2) )
      then row2.row_name
      else None
    in
    let row0 =
    ...
    let set_more row rest =
      ...
      if !trace_gadt_instances
      then update_level !env rm.level (newgenty (Tvariant row));
      if row_fixed row
      then
        if more == rm
        then ()
    ...
    try
      ...
      List.iter
        (fun (l, f1, f2) ->
          ...
          with Unify trace ->
            raise
              Trace.(Unify (Variant (Incompatible_types_for l) :: trace)) )
        pairs;
      if static_row row1
      then
  ...
  then ()
    ...
    | Reither (c1, tl1, m1, e1), Reither (c2, tl2, m2, e2) ->
        ...
        then (
          ...
          in
          if redo
          then unify_row_field env fixed1 fixed2 more l f1 f2
          else
            let tl1 = List.map repr tl1 and tl2 = List.map repr tl2 in
            ...
            let split_univars =
              List.partition (fun ty ->
                  try occur_univar !env ty; true with Unify _ -> false )
            in
            ...
            | tu1 :: tlu1, _ :: _ ->
                ...
                if not !passive_variants
                then List.iter (unify env tu1) (tlu1 @ tlu2)
    ...
    | Reither (_, _, false, e1), Rabsent when not fixed1 ->
        set_row_field e1 f2
    | Rabsent, Reither (_, _, false, e2) when not fixed2 ->
        set_row_field e2 f1
    | Rabsent, Rabsent -> ()
...
let moregen_occur env level ty =
  ...
  in
  ( try occur ty; unmark_type ty
    with Occur -> unmark_type ty; raise (Unify []) );
  (* also check for free univars *)
  occur_univar env ty; update_level env level ty

let may_instantiate inst_nongen t1 =
  if inst_nongen
  then t1.level <> generic_level - 1
        ...
        then ()
          ...
          | Reither (c1, tl1, _, e1), Reither (c2, tl2, m2, e2) ->
              ...
              then (
                ...
                then List.iter2 (moregen inst_nongen type_pairs env) tl1 tl2
                else
                  match tl2 with
                  | t2 :: _ ->
                      List.iter
...
let rec rigidify_rec vars ty =
  ...
  then (
    ...
    | Tvariant row ->
        let row = row_repr row in
        let more = repr row.row_more in
        ( if is_Tvar more && not (row_fixed row)
        then
...
let rigidify ty =
  let vars = ref [] in
  rigidify_rec vars ty; unmark_type ty; !vars
...
and eqtype_row rename type_pairs subst env row1 row2 =
  ...
  | _ ->
      let row1 = row_repr row1 and row2 = row_repr row2 in
      let r1, r2, pairs =
        merge_row_fields row1.row_fields row2.row_fields
      in
      ...
      then eqtype rename type_pairs subst env row1.row_more row2.row_more;
      List.iter
        (fun (_, f1, f2) ->
          match row_field_repr f1, row_field_repr f2 with
          | Rpresent (Some t1), Rpresent (Some t2) ->
    ...
    let error =
      List.fold_right
        (fun (lab, k, _) err ->
          ...
          in
          if lab = dummy_method || Concr.mem lab sign1.csig_concr
          then err
      ...
      Vars.fold
        (fun lab (mut, vr, _ty) err ->
          try
            ...
            if mut = Mutable && mut' <> Mutable
            then CM_Non_mutable_value lab :: err
            else if vr = Concrete && vr' <> Concrete
            then CM_Non_concrete_value lab :: err
            else err
...
let match_class_declarations env patt_params patt_type subj_params subj_type
  ...
  let error =
    List.fold_right
      (fun (lab, k, _) err ->
        ...
        in
        if Concr.mem lab sign1.csig_concr
        then err
        else CM_Hide_virtual ("method", lab) :: err )
      miss1
      []
  ...
  | [] -> (
    try
      ...
      if lp <> ls
      then raise (Failure [CM_Parameter_arity_mismatch (lp, ls)]);
      List.iter2
        (fun p s ->
...
let rec lid_of_path ?(hash = "") = function
  ...
  | Path.Pdot (p1, s) -> Longident.Ldot (lid_of_path p1, hash ^ s)
  | Path.Papply (p1, p2) ->
...
let rec build_subtype env visited loops posi level t =
  ...
  | Tarrow (l, t1, t2, _) ->
      ...
      then t, Unchanged
        ...
        let c = max c1 c2 in
        if c > Unchanged
        then newty (Tarrow (l, t1', t2', Cok)), c
        else t, Unchanged
  | Ttuple tlist ->
      if memq_warn t visited
      then t, Unchanged
        ...
        if c > Unchanged
        then newty (Ttuple (List.map fst tlist')), c
        else t, Unchanged
  | Tconstr (p, tl, abbrev)
    when level > 0
  ...
  | Tconstr (p, tl, _abbrev) -> (
      if (* Must check recursion on constructors, since we do not always
         expand them *)
         memq_warn t visited
      then t, Unchanged
      else
  ...
  | Tvariant row ->
      let row = row_repr row in
      if memq_warn t visited || not (static_row row)
      then t, Unchanged
      else
        ...
        let fields =
          List.map
            (fun ((l, f) as orig) ->
              ...
              | Rpresent None ->
                  if posi
                  then (l, Reither (true, [], false, ref None)), Unchanged
                  else orig, Unchanged
              | Rpresent (Some t) ->
                  let t', c =
  ...
  | Tobject (t1, _) ->
      if memq_warn t visited || opened_object t1
      then t, Unchanged
      else
        let level' = pred_enlarge level in
        let visited =
        ...
        if c > Unchanged
        then newty (Tobject (t1', ref None)), c
        else t, Unchanged
  | Tfield (s, _, t1, t2) (* Always present *) ->
      ...
      let c = max c1 c2 in
      if c > Unchanged
      then newty (Tfield (s, Fpresent, t1', t2')), c
      else t, Unchanged
  | Tnil ->
      if posi
      then
        ...
        try
          ...
          List.fold_left2
            (fun cstrs v (t1, t2) ->
              ...
              then subtype_rec env (Trace.diff t2 t1 :: trace) t2 t1 cstrs
              else cstrs )
            cstrs
            decl.type_variance
            (List.combine tl1 tl2)
      ...
      | Tpackage (p1, nl1, tl1), Tpackage (p2, nl2, tl2) -> (
        try
          ...
          then cstrs' @ cstrs
            ...
            with Unify _ -> Btype.backtrack snap; raise Not_found
        with Not_found -> (trace, t1, t2, !univar_pairs) :: cstrs )
...
and subtype_fields env trace ty1 ty2 cstrs =
  ...
  let cstrs =
    ...
    then subtype_rec env (Trace.diff rest1 rest2 :: trace) rest1 rest2 cstrs
    else
      ( trace
  ...
  then (
    ...
    then
                  ...
                  | Reither (b, ty :: (_ :: _ as tyl), m, e) ->
                      let tyl' =
                        List.fold_left
                          ...
                          tyl
                      in
                      if f != f0 || List.length tyl' < List.length tyl
                      then Reither (b, List.rev tyl', m, e)
                      else f
                  | _ -> f ) )
              row.row_fields
          in
      ...
      | Tobject (fi, nm) ->
          ...
          | None -> ()
          | Some (n, v :: l) -> (
              if deep_occur ty (newgenty (Ttuple l))
              then
...
let nondep_type env id ty =
  try
    ...
    clear_hash (); ty'
  with Nondep_cannot_erase _ as exn -> clear_hash (); raise exn
    ...
    and tm, priv =
      ...
      | Some ty -> (
        ...
        with Nondep_cannot_erase _ when is_covariant -> (
          clear_hash ();
          try
            Some (nondep_type_rec ~expand_private:true env mid ty), Private
...
let nondep_extension_constructor env ids ext =
  try
    ...
    in
    let args =
      map_type_expr_cstr_args (nondep_type_rec env ids) ext.ext_args
    in
...
let rec nondep_class_type env ids = function
  ...
  | Cty_arrow (l, ty, cty) ->
      Cty_arrow
        (l, nondep_type_rec env ids ty, nondep_class_type env ids cty)
