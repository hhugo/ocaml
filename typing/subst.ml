...
let add_modtype_path p ty s =
  { s with  modtypes = Path.Map.add p ty s.modtypes }
...
let loc s x =
  match s.loc with
  | Some l -> l
  | None -> if s.for_saving && not !Clflags.keep_locs then Location.none else x
      ...
      if has_fixed_row then
        ...
        | Tobject (t1, name) ->
          ...
          let name' =
            ...
            | None -> None
            | Some (p, tl) ->
              if to_subst_by_type_function s p then
                None
              else
                Some (type_path s p, List.map (typexp copy_scope s) tl)
          in
          Tobject (t1', ref name')
        | Tvariant row ->
          ...
          | _ ->
            ...
            let more' =
              ...
              | Tunivar _ | Tvar _ ->
                if s.for_saving then
                  newpersty (norm mored)
                else if dup && is_Tvar more then
                  newgenty mored
                else
                  more
              | _ -> assert false
...
let constructor_arguments copy_scope s =
  function
...
let type_declaration' copy_scope s decl =
  {
    ...
    type_kind =
      ...
      | Type_variant (cstrs, rep) ->
        Type_variant
          (List.map (constructor_declaration copy_scope s) cstrs, rep)
...
let class_signature copy_scope s sign =
  {
    ...
    csig_vars =
      Vars.map (function m, v, t -> m, v, typexp copy_scope s t) sign.csig_vars;
    csig_meths =
      Meths.map (function p, v, t -> p, v, typexp copy_scope s t)
        sign.csig_meths
  }

let rec class_type copy_scope s =
  function
...
module Lazy_types = struct
  ...
  and signature' = S_eager of Types.signature | S_lazy of signature_item list
  
  and signature = (scoping * t * signature', signature') Lazy_backtrack.t
...
let rename_bound_idents scoping s sg =
  ...
  in
  let rec rename_bound_idents s sg =
    function
    | [] -> sg, s
    | SigL_type (id, td, rs, vis) :: rest ->
      let id' = rename id in
      rename_bound_idents (add_type id (Pident id') s)
        (SigL_type (id', td, rs, vis) :: sg) rest
    | SigL_module (id, pres, md, rs, vis) :: rest ->
      let id' = rename id in
      rename_bound_idents (add_module id (Pident id') s)
        (SigL_module (id', pres, md, rs, vis) :: sg) rest
    | SigL_modtype (id, mtd, vis) :: rest ->
      let id' = rename id in
      rename_bound_idents (add_modtype id (Mty_ident (Pident id')) s)
        (SigL_modtype (id', mtd, vis) :: sg) rest
    | SigL_class (id, cd, rs, vis) :: rest ->
      (* cheat and pretend they are types cf. PR#6650 *)
      let id' = rename id in
      rename_bound_idents (add_type id (Pident id') s)
        (SigL_class (id', cd, rs, vis) :: sg) rest
    | SigL_class_type (id, ctd, rs, vis) :: rest ->
      (* cheat and pretend they are types cf. PR#6650 *)
      let id' = rename id in
      rename_bound_idents (add_type id (Pident id') s)
        (SigL_class_type (id', ctd, rs, vis) :: sg) rest
...
and subst_lazy_modtype scoping s =
  function
  | MtyL_ident p ->
    ...
    | exception Not_found ->
      ...
      | Pdot (p, n) -> MtyL_ident (Pdot (module_path s p, n))
      | Papply _ -> fatal_error "Subst.modtype"
      end
    end
  | MtyL_signature sg -> MtyL_signature (subst_lazy_signature scoping s sg)
  ...
  | MtyL_alias p -> MtyL_alias (module_path s p)
...
and subst_lazy_signature scoping s sg =
  ...
  | Right sg -> Lazy_backtrack.create (scoping, s, sg)

and force_signature sg = List.map force_signature_item (force_signature_once sg)
...
and lazy_signature_item =
  function
  | Sig_value (id, d, vis) -> SigL_value (id, d, vis)
  | Sig_type (id, d, rs, vis) -> SigL_type (id, d, rs, vis)
  | Sig_typext (id, ext, es, vis) -> SigL_typext (id, ext, es, vis)
  | Sig_module (id, res, d, rs, vis) ->
    SigL_module (id, res, lazy_module_decl d, rs, vis)
  | Sig_modtype (id, d, vis) -> SigL_modtype (id, lazy_modtype_decl d, vis)
  | Sig_class (id, d, rs, vis) -> SigL_class (id, d, rs, vis)
  | Sig_class_type (id, d, rs, vis) -> SigL_class_type (id, d, rs, vis)
...
and force_signature_item =
  ...
  | SigL_modtype (id, d, vis) -> Sig_modtype (id, force_modtype_decl d, vis)
