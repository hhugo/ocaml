...
let attrs s x =
  let x =
    if s.for_saving && not !Clflags.keep_docs
...
let rec typexp scope s ty =
  ...
  | (Tvar _ | Tunivar _) as desc ->
      if s.for_saving || ty.id < 0
      then (
        let ty' =
          if s.for_saving
          then newpersty (norm desc)
...
*)
  | _ ->
      ...
      (* Make a stub *)
      let ty' =
        if s.for_saving then newpersty (Tvar None) else newgenvar ()
      in
      ty.desc <- Tsubst ty';
      ty'.desc
      <- ( if has_fixed_row
               ...
               | _ -> (
                   ...
                   let more' =
                     ...
                     | Tunivar _ | Tvar _ ->
                         ...
                         if s.for_saving
                         then newpersty (norm more.desc)
                         else if dup && is_Tvar more
                         then newgenty more.desc
                         else more
                     | _ -> assert false
                   in
                   ...
                   (* Return a new copy *)
                   let row =
                     copy_row (typexp scope s) true row (not dup) more'
                   in
...
let constructor_arguments scope s = function
  | Cstr_tuple l -> Cstr_tuple (List.map (typexp scope s) l)
  | Cstr_record l -> Cstr_record (List.map (label_declaration scope s) l)
...
let class_signature scope s sign =
  ...
  ; csig_inher =
      List.map
        (fun (p, tl) -> type_path s p, List.map (typexp scope s) tl)
        sign.csig_inher }
...
let rec class_type scope s = function
  | Cty_constr (p, tyl, cty) ->
      ...
      Cty_constr (p', tyl', cty')
  | Cty_signature sign -> Cty_signature (class_signature scope s sign)
  | Cty_arrow (l, ty, cty) ->
...
let rec modtype s = function
  | Mty_ident p as mty -> (
    match p with
    | Pident id -> (
      try Ident.Map.find id s.modtypes with Not_found -> mty )
