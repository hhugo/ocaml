...
let get_variance ty visited =
  try TypeMap.find ty !visited
  with
  | Not_found -> Variance.null
           ...
           (try
              ...
              List.iter2
                (fun ty v ->
                   ...
                   in
                   if strict then
                     compute_variance_rec full ty
                   else
                     let p1 = inter v vari
                     and n1 = inter v (conjugate vari)
                     in
                     let v1 =
                     ...
                     compute_variance_rec v2 ty) tl decl.type_variance
            with
            | Not_found -> List.iter (compute_variance_rec unknown) tl)
         ...
         List.iter
           (fun (_, f) ->
              ...
              | Reither (_, tyl, _, _) ->
                let open Variance in
                let upper =
                  List.fold_left (fun s f -> set f true s) null
...
let compute_variance_type env ~check (required, loc) decl tyl =
  ...
  let required =
    List.map
      (fun (c, n, i) ->
         ...
         if c || n then c, n, i else true, true, i) required
  in
  (* Prepare *)
  ...
  List.iter
    (fun (cn, ty) ->
       compute_variance env tvl (if cn then full else covariant) ty) tyl;
  ...
  (if check then
     ...
     (List.iter2
      ...
      let fvl = List.filter (fun v -> not (List.exists (eq_type v) params)) fvl
      in
        ...
        (List.iter2
           (fun ty (p, n, _) ->
              if Btype.is_Tvar ty then
                ...
                let v =
                  if p then
                    (if n then full else covariant)
                  else
                    conjugate covariant
                in
                compute_variance env tvl2 v ty) params required;
         let visited = ref TypeSet.empty in
         let rec check ty =
           if TypeSet.mem ty !visited then
             ()
             ...
             (visited := visited';
              ...
              let v2 =
                TypeMap.fold
                  (fun t vt v ->
                     if Ctype.is_equal env false [ ty ] [ t ] then
                       union vt v
                     else
                       v) !tvl2 null
              in
              Btype.backtrack snap;
              let (c1, n1) = get_upper v1
              and (c2, n2, _, i2) = get_lower v2
              in
              if c1 && not c2 || n1 && not n2 then
                (if List.exists (eq_type ty) fvl then
                   ...
                   raise
                     (Error
                        (loc,
                         Bad_variance (code, (c1, n1, false), (c2, n2, false))))
                 else
  ...
  List.map2
    (fun ty (p, n, i) ->
       ...
       let v =
         if not concr then
           v
         else if mem Pos v && mem Neg v then
           full
         else if Btype.is_Tvar ty then
           v
         else
           union v
             (if p then (if n then full else covariant) else conjugate covariant)
       in
       if decl.type_kind = Type_abstract && tr = Public then
         v
       else
         set May_weak (mem May_neg v) v) params required
...
let for_constr =
  ...
  | Types.Cstr_record l ->
    List.map
      (fun { Types.ld_mutable; ld_type } -> ld_mutable = Mutable, ld_type) l

let compute_variance_gadt
    env ~check (required, loc as rloc) decl (tl, ret_type_opt)
=
        ...
        List.fold_left2
          (fun (fv1, fv2) ty (c, n, _) ->
             ...
             | fv :: fv2 ->
               ...
               fv :: fv1, fv2) ([], fvl) tyl required
      in
      compute_variance_type env ~check rloc
        { decl with  type_params = tyl; type_private = Private } (for_constr tl)
    | _ -> assert false
    end
...
let compute_variance_decl env ~check decl (required, _ as rloc) =
  ...
  then
    List.map
      (fun (c, n, i) ->
         make (not n) (not c) (decl.type_kind <> Type_abstract || i)) required
  else
    let mn =
    ...
    | Type_variant (tll, _rep) ->
      if List.for_all (fun c -> c.Types.cd_res = None) tll then
        ...
        let tll = mn @ List.map (fun c -> c.Types.cd_args, c.Types.cd_res) tll
        in
        begin match
          List.map (compute_variance_gadt env ~check rloc decl) tll
        with
        | vari :: rem ->
          let varl = List.fold_left (List.map2 Variance.union) vari rem in
          List.map
            Variance.(fun v -> if mem Pos v && mem Neg v then full else v) varl
        | _ -> assert false
        end
    | Type_record (ftl, _) ->
      compute_variance_type env ~check rloc decl
...
let property : (prop, req) Typedecl_properties.property =
  ...
  let eq li1 li2 =
    try List.for_all2 Variance.eq li1 li2
    with
    | _ -> false
  in
  ...
  let check env id decl req = if is_hash id then () else check_decl env decl req
  in
  ...
  let (decls, required) =
    List.fold_right
      (fun (obj_id, obj_abbr, _cl_abbr, _clty, _cltydef, ci) (decls, req) ->
         ...
         variance_of_params ci.Typedtree.ci_params :: req) cldecls ([], [])
  in
  let decls = Typedecl_properties.compute_property property env decls required
  in
  List.map2
    (fun (_, decl) (_, _, cl_abbr, clty, cltydef, _) ->
       ...
       { cltydef with  clty_variance = variance }) decls cldecls
