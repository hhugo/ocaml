...
let compute_variance env visited vari ty =
  let rec compute_variance_rec vari ty =
    ...
    then ()
            ...
            try
              ...
              List.iter2
                (fun ty v ->
                  ...
                  let strict =
                    (cvari Inv && cv Inj)
                    || ((cvari Pos || cvari Neg) && cv Inv)
                  in
                  if strict
                  then compute_variance_rec full ty
                  else
                    let p1 = inter v vari
      ...
      | Tfield (_, _, ty1, ty2) -> compute_same ty1; compute_same ty2
      ...
      | Tpackage (_, _, tyl) ->
          let v =
            Variance.(
              if mem Pos vari || mem Neg vari then full else may_inv)
          in
          List.iter (compute_variance_rec v) tyl
  in
  ...
  then (
    ...
    List.iter2
      (fun ty (c, n, i) ->
        ...
        if Btype.is_Tvar ty
           && ((co && not c) || (cn && not n) || ((not ij) && i))
        then
          raise (Error (loc, Bad_variance (!pos, (co, cn, ij), (c, n, i))))
        )
      params
      required;
      ...
      List.iter2
        (fun ty (p, n, _) ->
          ...
          then ()
            ...
            let v =
              if p
              then if n then full else covariant
      ...
      let rec check ty =
        ...
        then ()
          ...
          then
            ...
            then
              let code =
                if not i2 then -2 else if c2 || n2 then -1 else -3
              in
              raise
                (Error
                   ( loc
  ...
  List.map2
    (fun ty (p, n, i) ->
      ...
      let v =
        if not concr
        then v
        else if mem Pos v && mem Neg v
        then full
        else if Btype.is_Tvar ty
        then v
        else
          union
            v
      ...
      in
      if decl.type_kind = Type_abstract && tr = Public
      then v
    ...
    | Type_variant tll -> (
        ...
        then
          compute_variance_type
            ...
            ( mn
            @ List.flatten
                (List.map (fun c -> for_constr c.Types.cd_args) tll) )
        else
          let mn =
          ...
          | vari :: rem ->
              ...
              List.map
                Variance.(
                  fun v -> if mem Pos v && mem Neg v then full else v)
                varl
          | _ -> assert false )
    | Type_record (ftl, _) ->
        compute_variance_type
...
let property : (prop, req) Typedecl_properties.property =
  ...
  let eq li1 li2 =
    try List.for_all2 Variance.eq li1 li2 with _ -> false
  in
