...
let all_ident_chars s =
  let rec loop s len i =
    if i < len then
      (if is_ident_char s.[i] then loop s len (i + 1) else false)
    else
      true
  in
...
let print_out_string ppf s =
  let not_escaped =
    ...
    | None -> true
    | Some x ->
      begin match bool_of_string_opt x with
      | None -> true
      | Some f -> f
      end
  in
...
and print_simple_out_type ppf =
  ...
  | Otyp_variant (non_gen, row_fields, closed, tags) ->
    ...
    let print_fields ppf =
      function
      | Ovar_fields fields ->
        print_list print_row_field (fun ppf -> fprintf ppf "@;<1 -2>| ") ppf
          fields
      ...
      (fun (s, t) ->
         let sep =
           if !first then
             ...
             "and"
         in
         fprintf ppf " %s type %s = %a" sep s print_out_type t) fl;
    fprintf ppf ")@]"
...
let rec print_out_class_type ppf =
  ...
  | Octy_signature (self_ty, csil) ->
    ...
    fprintf ppf "@[<hv 2>@[<2>object%a@]@ %a@;<1 -2>end@]" pr_param self_ty
      (print_list print_out_class_sig_item (fun ppf -> fprintf ppf "@ ")) csil
...
and print_out_class_sig_item ppf =
  ...
  | Ocsg_method (name, priv, virt, ty) ->
    fprintf ppf "@[<2>method %s%s%s :@ %a@]" (if priv then "private " else "")
      (if virt then "virtual " else "") name !out_type ty
  | Ocsg_value (name, mut, vr, ty) ->
    fprintf ppf "@[<2>val %s%s%s :@ %a@]" (if mut then "mutable " else "")
      (if vr then "virtual " else "") name !out_type ty
...
let rec collect_functor_args acc =
  function
...
and print_out_functor_parameters ppf l =
  let print_nonanon_arg ppf =
    function
    | None -> fprintf ppf "()"
    | Some (param, mty) ->
      fprintf ppf "(%s : %a)" (Option.value param ~default:"_")
        print_out_module_type mty
  in
  let rec print_args ppf =
    function
    | [] -> ()
    | Some (None, mty_arg) :: l ->
    ...
    | _ :: _ as non_anonymous_functor ->
      ...
      fprintf ppf "@[<2>functor@ %a@]@ ->@ %a"
        (pp_print_list ~pp_sep:pp_print_space print_nonanon_arg) args print_args
        anons
  in
  print_args ppf l
  ...
  | Osig_typext (ext, Oext_first) :: items ->
    ...
    let rec gather_extensions acc items =
      ...
      | Osig_typext (ext, Oext_next) :: items ->
        gather_extensions
          ((ext.oext_name, ext.oext_args, ext.oext_ret_type) :: acc) items
      | _ -> List.rev acc, items
    in
    let (exts, items) =
      gather_extensions [ ext.oext_name, ext.oext_args, ext.oext_ret_type ]
        items
    in
    let te =
      {
...
and print_out_sig_item ppf =
  ...
  | Osig_class (vir_flag, name, params, clt, rs) ->
    fprintf ppf "@[<2>%s%s@ %a%s@ :@ %a@]"
      ...
      (if vir_flag then " virtual" else "") print_out_class_params params name
      !out_class_type clt
  ...
  | Some ret_type ->
    ...
    | _ ->
      fprintf ppf "@[<2>%s :@ %a -> %a@]" name
        (print_typlist print_simple_out_type " *") tyl print_simple_out_type
        ret_type
    end
...
let print_out_exception ppf exn outv =
  ...
  | _ ->
    begin match Printexc.use_printers exn with
