...
let print_out_string ppf s =
  let not_escaped =
    ...
    | None -> true
    | Some x -> (
      match bool_of_string_opt x with None -> true | Some f -> f )
  in
  if not_escaped
...
let rec print_out_functor funct ppf = function
  | Omty_functor (_, None, mty_res) ->
      if funct
      then fprintf ppf "() %a" (print_out_functor true) mty_res
  ...
  | m ->
      if funct
      then fprintf ppf "->@ %a" print_out_module_type m
