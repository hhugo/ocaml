...
let fmt_position f l =
  if l.pos_lnum = (-1) then
    fprintf f "%s[%d]" l.pos_fname l.pos_cnum
  else
...
let attributes i ppf l =
  ...
  List.iter
    (fun a ->
       ...
       Printast.payload (i + 1) ppf a.Parsetree.attr_payload) l
    ...
    | [] ->
      ...
      | Tpat_construct (li, _, po, vto) ->
        ...
        option i
          (fun i ppf (vl, ct) ->
             let names =
               List.map (fun { txt } -> "\"" ^ Ident.name txt ^ "\"") vl
             in
             line i ppf "[%s]\n" (String.concat "; " names);
             core_type i ppf ct) ppf vto
      | Tpat_variant (l, po, _) ->
...
and constructor_arguments i ppf =
  function
  | Cstr_tuple l -> list i core_type ppf l
  | Cstr_record l -> list i label_decl ppf l

and label_decl
    i ppf { ld_id; ld_name = _; ld_mutable; ld_type; ld_loc; ld_attributes }
=
  line i ppf "%a\n" fmt_location ld_loc;
