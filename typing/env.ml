...
module IdTbl = struct
  ...
  and 'a opened =
    { root : Path.t
          ...
          environment. *)
    ; components : 'a NameMap.t  (** Components from the opened module. *)
    ; using : (string -> ('a * 'a) option -> unit) option
          (** A callback to be applied when a component is used from this
          "open".  This is used to detect unused "opens".  The
          arguments are used to detect shadowing. *)
    ...
    | Some {root; using = _; next; components} ->
        NameMap.iter
          (fun s x ->
            ...
            f (Ident.create_scoped ~scope:root_scope s) (Pdot (root, s), x)
            )
          components;
        iter f next
    | None -> ()
...
type t =
  ...
  ; modules :
      (module_declaration_lazy * address_lazy) value_or_persistent IdTbl.t
  ; modtypes : modtype_declaration IdTbl.t
  ; components :
      (module_components * address_lazy) value_or_persistent IdTbl.t
...
and structure_components =
  ...
  ; mutable comp_modules :
      (module_declaration_lazy * address_lazy) NameMap.t
...
and functor_components =
  { fcomp_param : Ident.t
  ; (* Formal parameter *)
    fcomp_arg : module_type option
  ; (* Argument signature *)
    fcomp_res : module_type
  ; (* Result signature *)
    fcomp_cache : (Path.t, module_components) Hashtbl.t
  ; (* For memoization *)
...
let components_of_module' =
  ref
    ( fun ~alerts:_ ~loc:_ _env _fsub _psub _path _addr _mty -> assert false
      :    alerts:alerts
        -> loc:Location.t
        -> t
        -> Subst.t option
...
module Current_unit_name : sig
  ...
  let is_name_of id = is (Ident.name id)
end
...
let add_persistent_structure id env =
  ...
  then invalid_arg "Env.add_persistent_structure";
  if not (Current_unit_name.is_name_of id)
  then
    { env with
...
let find_value_full = find (fun env -> env.values) (fun sc -> sc.comp_values)
...
let find_value p env = fst (find_value_full p env)

let find_class p env = fst (find_class_full p env)
...
let find_value_address p env = get_address (snd (find_value_full p env))

let find_class_address p env = get_address (snd (find_class_full p env))
...
and expand_module_path lax env path =
  try
    ...
    | {md_type = Mty_alias path1} ->
        ...
        if lax || !Clflags.transparent_modules
        then path'
        else
...
let is_uident s = match s.[0] with 'A' .. 'Z' -> true | _ -> false
...
let normalize_type_path oloc env path =
  ...
  | Pdot (p, s) ->
      let p2 =
        if is_uident s && not (is_uident (Path.last p))
        then (* Cstr M.t.C *)
...
let rec lookup_module_descr_aux ?loc ~mark lid env =
  ...
  | Lident s -> (
      ...
      match IdTbl.find_name ~mark s env.components with
...
and lookup_module ~load ?loc ~mark lid env : Path.t =
  match lid with
  | Lident s -> (
    match IdTbl.find_name ~mark s env.modules with
    | exception Not_found when !Clflags.transparent_modules && not load ->
        check_pers_mod s ~loc:(Option.value loc ~default:Location.none);
...
let lookup_all_simple proj1 proj2 shadow ?loc ~mark lid env =
  ...
  | Ldot (l, s) -> (
      ...
      match get_components desc with
      | Structure_comps c ->
          let comps = try NameMap.find s (proj2 c) with Not_found -> [] in
...
let lookup_type ?loc ~mark lid env =
  lookup (fun env -> env.types) (fun sc -> sc.comp_types) ?loc ~mark lid env
...
let set_type_used_callback name td callback =
  ...
  then ()
    ...
    let key = name, loc in
    let old =
      try Hashtbl.find type_declarations key with Not_found -> ignore
    in
...
let lookup_class ?loc ?(mark = true) lid env =
  ...
  if Path.name desc.cty_path = ""
  then ignore (lookup_type ?loc ~mark lid env)
  else if mark
  then mark_type_path env desc.cty_path;
  r

let lookup_cltype ?loc ?(mark = true) lid env =
  let ((_, desc) as r) = lookup_cltype ?loc ~mark lid env in
  if Path.name desc.clty_path = ""
  then ignore (lookup_type ?loc lid env)
  ...
  | Mty_alias path -> (
    ...
    | path -> (
      try
        ...
        scrape_alias_for_visit env sub (find_module path env).md_type
      with Not_found -> false ) )
  | _ -> true
...
let rec find_shadowed_comps path env =
  ...
  | Pident id ->
      List.filter_map
        (fun (p, data) ->
          match data with Value x -> Some (p, x) | Persistent -> None )
  ...
  let rec prefix_idents root items_and_paths freshening_sub prefixing_sub =
    ...
    | Sig_type (id, td, rs, vis) :: rem ->
        ...
        let id', freshening_sub =
          refresh id Subst.add_type freshening_sub
        in
        prefix_idents
          root
    ...
    | Sig_module (id, pres, md, rs, vis) :: rem ->
        let p = Pdot (root, Ident.name id) in
        let id', freshening_sub =
          refresh id Subst.add_module freshening_sub
        in
        prefix_idents
          root
...
and components_of_module_maker
    { cm_env
    ; cm_freshening_subst
    ; cm_prefixing_subst
    ; cm_path
      ...
      List.iter
        (fun (item, path) ->
          ...
          | Sig_type (id, decl, _, _) ->
              ...
              in
              Datarepr.set_row_name
                final_decl
                (Subst.type_path prefixing_sub (Path.Pident id));
              let constructors =
                List.map snd (Datarepr.constructors_of_type path final_decl)
              in
              let labels =
...
and check_usage loc id warn tbl =
  if (not loc.Location.loc_ghost) && Warnings.is_active (warn "")
  then (
...
and store_type ~check id info env =
  ...
  ( if check
       ...
       && Warnings.is_active
            (Warnings.Unused_constructor ("", false, false))
  then
    let ty = Ident.name id in
    List.iter
      (fun (_, {cstr_name = c; _}) ->
        let k = ty, loc, c in
        if not (Hashtbl.mem used_constructors k)
        then (
          ...
          then
            !add_delayed_check_forward (fun () ->
                if (not (is_in_signature env)) && not used.cu_positive
                then
...
and store_extension ~check id addr ext env =
  ...
  ( if check
       && (not loc.Location.loc_ghost)
       && Warnings.is_active
            (Warnings.Unused_extension ("", false, false, false))
  then
...
and add_module_declaration ?(arg = false) ~check id presence md env =
  ...
  let env =
    store_module ~freshening_sub:None ~check id addr presence md env
  in
...
let add_components slot root env0 comps =
  ...
  in
  let components =
    let components = NameMap.map (fun x -> Value x) comps.comp_components in
    add (fun x -> `Component x) components env0.components
  in
  let modules =
    let modules = NameMap.map (fun x -> Value x) comps.comp_modules in
...
let fold_modules f lid env acc =
  ...
  | Some l -> (
      ...
      | Structure_comps c ->
          NameMap.fold
            (fun s (data, _) acc ->
              f s (Pdot (p, s)) (EnvLazy.force subst_modtype_maker data) acc
              )
            c.comp_modules
            acc
      | Functor_comps _ -> acc )
...
let filter_non_loaded_persistent f env =
  ...
  let rec filter_summary summary ids =
    ...
    then summary
      ...
      | Env_persistent (s, id) ->
          if String.Set.mem (Ident.name id) ids
          then filter_summary s (String.Set.remove (Ident.name id) ids)
...
let report_error ppf = function
  | Missing_module (_, path1, path2) ->
      ...
      then fprintf ppf "Internal path@ %s@ is dangling." (Path.name path1)
      else
        fprintf
          ppf
