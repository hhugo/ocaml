...
=
  ...
  | Asttypes.Private, _ | _, true ->
    if cu.cu_positive || cu.cu_pattern || cu.cu_exported_private then
      None
    else
      Some Unused
  | Asttypes.Public, false ->
    begin match cu.cu_positive, cu.cu_pattern, cu.cu_exported_private with
    | true, _, _ -> None
...
let label_usage_complaint priv mut lu : Warnings.field_usage_warning option =
  ...
  | Asttypes.Public, Asttypes.Immutable ->
    begin match lu.lu_projection, lu.lu_construct with
    | true, _ -> None
    | false, false -> Some Unused
    | false, true -> Some Not_read
    end
  | Asttypes.Public, Asttypes.Mutable ->
    begin match lu.lu_projection, lu.lu_mutation, lu.lu_construct with
    | true, true, _ -> None
    | false, false, false -> Some Unused
...
module NameMap = String.Map (** Map indexed by the name of module components. *)
...
module TycompTbl = struct
  ...
  let rec find_all ~mark name tbl =
    List.map (fun (_id, desc) -> desc, nothing)
      ...
      | Some { using; next; components; root = _ } ->
        ...
        begin match NameMap.find name components with
  ...
  let rec fold_name f tbl acc =
    ...
    | Some { using = _; next; components; root = _ } ->
      acc |> NameMap.fold (fun _name -> List.fold_right f) components |>
        fold_name f next
    | None -> acc
      ...
      | Open { using; root; next; components } ->
        (try
           ...
           (if mark then
              ...
              | None -> ()
              | Some f ->
                begin match find_name wrap ~mark:false name next with
                | exception Not_found -> f name None
                | _, descr' -> f name (Some (descr', descr))
                end);
           res
         with
         | Not_found -> find_name wrap ~mark name next)
      | Map { f; next } ->
    ...
    List.map (fun (id, desc) -> Pident id, desc)
      ...
      | Open { root; using = _; next; components } ->
        (try
         ...
         with
         | Not_found -> find_all wrap name next)
      | Map { f; next } ->
        List.map (fun (p, desc) -> p, f desc) (find_all wrap name next)
  
  let rec fold_name wrap f tbl acc =
    let acc =
      Ident.fold_name (fun id d -> f (Ident.name id) (Pident id, d)) tbl.current
        acc
    in
    match tbl.layer with
    | Open { root; using = _; next; components } ->
      acc |>
        NameMap.fold (fun name desc -> f name (Pdot (root, name), wrap desc))
          components |>
...
type type_descr_kind = (label_description, constructor_description) type_kind
...
and functor_components =
  {
    fcomp_arg : functor_parameter; (* Formal parameter and argument signature *)
...
and value_entry = Val_bound of value_data | Val_unbound of value_unbound_reason
...
let in_signature b env =
  let flags =
    if b then
      env.flags lor in_signature_flag
    else
      env.flags land lnot in_signature_flag
  in
  { env with  flags }
...
let check_functor_application =
  ...
  ref
    (fun
      ~errors:_ ~loc:_ ~lid_whole_app:_ ~f0_path:_ ~args:_ ~arg_path:_
...
let add_persistent_structure id env =
  ...
  if Current_unit_name.is_ident id then
    env
  else
    let material =
...
let sign_of_cmi ~freshen { Persistent_env.Persistent_signature.cmi; _ } =
  ...
  let alerts =
    List.fold_left
      (fun acc ->
         function
         | Alerts s -> s
         | _ -> acc) Misc.Stdlib.String.Map.empty flags
  in
  let md =
    {
  ...
  let mda_components =
    ...
    components_of_module ~alerts ~uid:md.md_uid empty freshening_subst
      Subst.identity path mda_address
      (Subst.Lazy.of_modtype (Mty_signature sign))
  in
...
let check_functor_appl
    ~errors ~loc ~lid_whole_app ~f0_path ~args ~f_comp ~arg_path ~arg_mty
    ~param_mty env
=
...
let type_of_cstr path =
  ...
  | { cstr_inlined = Some decl; _ } ->
    ...
    | Type_record (_, repr) ->
      { tda_declaration = decl; tda_descriptions = Type_record (labels, repr) }
    | _ -> assert false
    end
  | _ -> assert false
...
let find_type_data path env =
  ...
  | Regular p ->
    begin match Path.Map.find p env.local_constraints with
    | decl -> { tda_declaration = decl; tda_descriptions = Type_abstract }
...
let find_type p env = (find_type_data p env).tda_declaration
let find_type_descrs p env = (find_type_data p env).tda_descriptions
...
let rec get_constrs_address =
  function
  | [] -> raise Not_found
  | cda :: rest ->
    begin match cda.cda_address with
...
let rec normalize_module_path lax env =
  function
  | Pident id as path when lax && Ident.persistent id -> path
  ...
  | Pdot (p, s) as path ->
    ...
    if p == p' then
      ...
      expand_module_path lax env (Pdot (p', s))
  | Papply (p1, p2) as path ->
    ...
    if p1 == p1' && p2 == p2' then
      ...
      expand_module_path lax env (Papply (p1', p2'))
...
and expand_module_path lax env path =
  try
    ...
    | { mdl_type = MtyL_alias path1 } ->
      ...
      if lax || !Clflags.transparent_modules then
        path'
      else
  ...
  | Some body
    when
      decl.type_private = Public ||
        decl.type_kind <> Type_abstract || Btype.has_constr_row body
    ->
    ...
    let cont () =
      ...
      if not visit then
        ...
        | Structure_comps comps ->
          NameMap.iter (fun s d -> f (Pdot (path, s)) (Pdot (path', s), d))
            (proj2 comps);
          NameMap.iter
            (fun s mda ->
               iter_components (Pdot (path, s)) (Pdot (path', s))
                 mda.mda_components) comps.comp_modules
        | Functor_comps _ -> ()
    in
  ...
  IdTbl.iter wrap_module
    (fun id (path, entry) ->
       ...
       | Mod_persistent ->
         ...
         begin match Persistent_env.find_in_cache !persistent_env modname with
         | None -> ()
...
let used_persistent () =
  Persistent_env.fold !persistent_env (fun s _m r -> String.Set.add s r)
    String.Set.empty
...
let rec find_shadowed_comps path env =
  ...
  | Pdot (p, s) ->
    ...
    let l' =
      List.map
        (find_all_comps wrap_identity (fun comps -> comps.comp_modules) s) l
    in
    List.flatten l'
  | Papply _ -> []
...
let find_shadowed_types path env =
  List.map fst
    (find_shadowed wrap_identity (fun env -> env.types)
       (fun comps -> comps.comp_types) path env)
...
let rec scrape_alias env sub ?path mty =
  ...
  | MtyL_ident _, _ ->
    ...
    in
    (try scrape_alias env sub (find_modtype_expansion_lazy p env) ?path
     with
     | Not_found -> mty)
  | MtyL_alias path, _ ->
    let path = may_subst Subst.module_path sub path in
    (try scrape_alias env sub (find_module_lazy path env).mdl_type ~path
     with
     | Not_found ->
...
let prefix_idents root freshening_sub prefixing_sub sg =
  ...
  let rec prefix_idents root items_and_paths freshening_sub prefixing_sub =
    ...
    | (SigL_value (id, _, _) as item) :: rem ->
      ...
      prefix_idents root ((item, p) :: items_and_paths) freshening_sub
        prefixing_sub rem
    | SigL_type (id, td, rs, vis) :: rem ->
      let p = Pdot (root, Ident.name id) in
      let (id', freshening_sub) = refresh id Subst.add_type freshening_sub in
      prefix_idents root ((SigL_type (id', td, rs, vis), p) :: items_and_paths)
        freshening_sub (Subst.add_type id' p prefixing_sub) rem
    | SigL_typext (id, ec, es, vis) :: rem ->
      let p = Pdot (root, Ident.name id) in
      let (id', freshening_sub) = refresh id Subst.add_type freshening_sub in
      (* we extend the substitution in case of an inlined record *)
      prefix_idents root
        ((SigL_typext (id', ec, es, vis), p) :: items_and_paths) freshening_sub
        (Subst.add_type id' p prefixing_sub) rem
    | SigL_module (id, pres, md, rs, vis) :: rem ->
      ...
      prefix_idents root
        ...
        freshening_sub (Subst.add_module id' p prefixing_sub) rem
    | SigL_modtype (id, mtd, vis) :: rem ->
      ...
      let (id', freshening_sub) =
        refresh id (fun i p s -> Subst.add_modtype i (Mty_ident p) s)
          freshening_sub
      in
      prefix_idents root ((SigL_modtype (id', mtd, vis), p) :: items_and_paths)
        freshening_sub (Subst.add_modtype id' (Mty_ident p) prefixing_sub) rem
    | SigL_class (id, cd, rs, vis) :: rem ->
      (* pretend this is a type, cf. PR#6650 *)
      let p = Pdot (root, Ident.name id) in
      let (id', freshening_sub) = refresh id Subst.add_type freshening_sub in
      prefix_idents root ((SigL_class (id', cd, rs, vis), p) :: items_and_paths)
        freshening_sub (Subst.add_type id' p prefixing_sub) rem
...
let module_declaration_address env id presence md =
  ...
  | Mp_present -> Lazy_backtrack.create_forced (Aident id)
...
let rec components_of_module_maker
    { cm_env; cm_freshening_subst; cm_prefixing_subst; cm_path; cm_addr; cm_mty
    }
  :
  _ result
=
  ...
  | MtyL_signature sg ->
    ...
    let pos = ref 0 in
    let next_address () =
      let addr : address_unforced = Projection { parent = cm_addr; pos = !pos }
      in
      incr pos;
    ...
    List.iter
      (fun ((item : Subst.Lazy.signature_item), path) ->
         ...
         | SigL_type (id, decl, _, _) ->
           let fresh_decl = may_subst Subst.type_declaration freshening_sub decl
           in
             ...
             | Type_variant (_, repr) ->
               ...
               List.iter
                 (fun descr ->
                    ...
                    c.comp_constrs <-
                      add_to_tbl descr.cstr_name cda c.comp_constrs) cstrs;
               Type_variant (cstrs, repr)
             | Type_record (_, repr) ->
               let lbls = List.map snd (Datarepr.labels_of_type path final_decl)
               in
               List.iter
                 (fun descr ->
                    c.comp_labels <-
                      add_to_tbl descr.lbl_name descr c.comp_labels) lbls;
               Type_record (lbls, repr)
             | Type_abstract -> Type_abstract
             | Type_open -> Type_open
           in
           let tda = { tda_declaration = final_decl; tda_descriptions = descrs }
           in
         ...
         | SigL_module (id, pres, md, _, _) ->
           ...
           env :=
             store_module ~update_summary:false ~freshening_sub ~check:None id
               addr pres md !env
         | SigL_modtype (id, decl, _) ->
           let fresh_decl =
             ...
             may_subst (Subst.Lazy.modtype_decl Make_local) freshening_sub decl
           in
           let final_decl =
             (* The prefixed items get the same scope as [cm_path], which is
                the prefix. *)
  ...
  | MtyL_functor (arg, ty_res) ->
    let sub = may_subst Subst.compose cm_freshening_subst cm_prefixing_subst in
...
and check_usage loc id uid warn tbl =
  if
    not loc.Location.loc_ghost &&
      Uid.for_actual_declaration uid && Warnings.is_active (warn "")
  then
    let name = Ident.name id in
...
and store_constructor ~check type_decl type_id cstr_id cstr env =
  ...
  { env with
    ...
    constrs =
      TycompTbl.add cstr_id { cda_description = cstr; cda_address = None }
        env.constrs
  }
   ...
   then
     ...
     (if not (Types.Uid.Tbl.mem !used_labels k) then
        ...
        (Types.Uid.Tbl.add !used_labels k (add_label_usage used);
         if not (ty_name = "" || ty_name.[0] = '_' || name.[0] = '_') then
           !add_delayed_check_forward
             (fun () ->
                Option.iter
                  (fun complaint ->
                     if not (is_in_signature env) then
                       Location.prerr_warning loc
                         (Warnings.Unused_field (name, complaint)))
...
and store_type ~check id info env =
  ...
  (if check then
     check_usage loc id info.type_uid
       (fun s -> Warnings.Unused_type_declaration s) !type_declarations);
  let (descrs, env) =
    let path = Pident id in
    ...
    | Type_variant (_, repr) ->
      ...
      List.fold_left
        (fun env (cstr_id, cstr) ->
           store_constructor ~check info id cstr_id cstr env) env constructors
    | Type_record (_, repr) ->
      let labels = Datarepr.labels_of_type path info in
      Type_record (List.map snd labels, repr),
      List.fold_left
        (fun env (lbl_id, lbl) -> store_label ~check info id lbl_id lbl env) env
        labels
...
and store_module
    ?(update_summary=true) ~check ~freshening_sub id addr presence md env
=
  let open Subst.Lazy in
  let loc = md.mdl_loc in
  Option.iter (fun f -> check_usage loc id md.mdl_uid f !module_declarations)
    check;
  let alerts = Builtin_attributes.alerts_of_attrs md.mdl_attributes in
  ...
  let comps =
    components_of_module ~alerts ~uid:md.mdl_uid env freshening_sub
      Subst.identity (Pident id) addr md.mdl_type
  in
  let mda =
    {
  ...
  in
  let summary =
    if not update_summary then
      env.summary
    else
      Env_module (env.summary, id, presence, force_module_decl md)
  in
  { env with  modules = IdTbl.add id (Mod_local mda) env.modules; summary }
...
and store_modtype ?(update_summary=true) id info env =
  let summary =
    if not update_summary then
      ...
      Env_modtype (env.summary, id, Subst.Lazy.force_modtype_decl info)
  in
  { env with  modtypes = IdTbl.add id info env.modtypes; summary }
  ...
  | Not_found ->
    ...
    let comps =
      components_of_module ~alerts:Misc.Stdlib.String.Map.empty
        ~uid:Uid.internal_not_actually_unique(*???*)  env None Subst.identity p
        addr (Subst.Lazy.of_modtype mty)
    in
...
and add_module_declaration_lazy ~update_summary id presence md env =
  let addr = module_declaration_address env id presence md in
  let env =
    store_module ~update_summary ~freshening_sub:None ~check:None id addr
      presence md env
  in
  env
...
let add_components slot root env0 comps =
  ...
  let constrs = add_l (fun x -> `Constructor x) comps.comp_constrs env0.constrs
  in
  ...
  let modtypes = add (fun x -> `Module_type x) comps.comp_modtypes env0.modtypes
  in
...
let remove_last_open root env0 =
  ...
  | summary ->
    ...
    and rem tbl = IdTbl.remove_last_open root tbl
    in
    Some
      { env0 with
...
let open_signature
    ?(used_slot=ref false) ?(loc=Location.none) ?(toplevel=false) ovf root env
=
  let unused =
    match ovf with
...
let persistent_structures_of_dir dir =
  Load_path.Dir.files dir |> List.to_seq |> Seq.filter_map unit_name_of_filename |>
    String.Set.of_seq
...
let lookup_ident_module (type a) (load : a load) ~errors ~use ~loc s env =
  ...
  | Mod_persistent ->
    ...
    | Don't_load ->
      ...
      path, (() : a)
    | Load ->
      begin match find_pers_mod s with
      | mda ->
        use_module ~use ~loc path mda;
        path, (mda : a)
      | exception Not_found ->
    ...
    List.map
      (fun (cda, use_fn) ->
         let use_fn () =
           ...
           use_fn ()
         in
         cda.cda_description, use_fn) cstrs
...
and get_functor_components ~errors ~loc lid env comps =
  ...
  | Ok (Functor_comps fcomps) ->
    begin match fcomps.fcomp_arg with
    | Unit ->
      (* PR#7611 *)
...
and lookup_apply ~errors ~use ~loc lid0 env =
  ...
  let (f0_path, f0_comp) = lookup_module_components ~errors ~use ~loc f0_lid env
  in
  let check_one_apply ~errors ~loc ~f_lid ~f_comp ~arg_path ~arg_mty env =
    ...
    arg_path, f_comp
  in
  let rec check_apply ~path:f_path ~comp:f_comp =
    function
...
let lookup_all_dot_constructors ~errors ~use ~loc usage l s env =
  ...
  | Longident.Lident "*predef*" ->
    ...
    lookup_all_ident_constructors ~errors ~use ~loc usage s initial_safe_string
  | _ ->
    let (_, comps) = lookup_structure_components ~errors ~use ~loc l env in
    begin match NameMap.find s comps.comp_constrs with
    | [] | exception Not_found ->
    ...
    | cstrs ->
      List.map
        (fun cda ->
           ...
           cda.cda_description, use_fun) cstrs
    end
...
let bound_module name env =
  ...
  | _ -> true
  | exception Not_found ->
    if Current_unit_name.is name then
      false
    else
      begin match find_pers_mod name with
      | _ -> true
...
let find_all wrap proj1 proj2 f lid env acc =
  match lid with
  | None ->
    IdTbl.fold_name wrap (fun name (p, data) acc -> f name p data acc)
      (proj1 env) acc
  | Some l ->
    let (p, desc) =
    ...
    | Structure_comps c ->
      NameMap.fold (fun s data acc -> f s (Pdot (p, s)) (wrap data) acc)
        (proj2 c) acc
    | Functor_comps _ -> acc
    end
    ...
    IdTbl.fold_name wrap_module
      (fun name (p, entry) acc ->
         ...
         | Mod_local mda ->
           ...
           f name p md acc
         | Mod_persistent ->
           begin match Persistent_env.find_in_cache !persistent_env name with
           | None -> acc
           | Some mda ->
             let md = Subst.Lazy.force_module_decl mda.mda_declaration in
             f name p md acc
  ...
  | Some l ->
    ...
    | Structure_comps c ->
      NameMap.fold
        (fun s mda acc ->
           let md = Subst.Lazy.force_module_decl mda.mda_declaration in
           f s (Pdot (p, s)) md acc) c.comp_modules acc
    | Functor_comps _ -> acc
    end
...
and fold_types f =
  find_all wrap_identity (fun env -> env.types) (fun sc -> sc.comp_types)
    (fun k p tda acc -> f k p tda.tda_declaration acc)

and fold_modtypes f =
  let f l path data acc = f l path (Subst.Lazy.force_modtype_decl data) acc in
  find_all wrap_identity (fun env -> env.modtypes) (fun sc -> sc.comp_modtypes)
    f
    ...
    IdTbl.fold_name wrap_module
      (fun name (_, entry) acc ->
         ...
         | Mod_persistent ->
           ...
           end) env.modules String.Set.empty
  in
  let remove_ids tbl ids =
    String.Set.fold
      (fun name tbl -> IdTbl.remove (Ident.create_persistent name) tbl) ids tbl
  in
...
let summary env =
  if Path.Map.is_empty env.local_constraints then
    env.summary
  else
...
let report_lookup_error _loc env ppf =
  function
  | Unbound_value (lid, hint) ->
    ...
    | Missing_rec def_loc ->
      ...
      fprintf ppf "@.@[%s@ %s %i@]" "Hint: If this is a recursive definition,"
  ...
  | Unbound_module lid ->
    ...
    begin match find_modtype_by_name lid env with
  ...
  | Unbound_class lid ->
    ...
    begin match find_cltype_by_name lid env with
  ...
  | Unbound_modtype lid ->
    ...
    begin match find_module_by_name lid env with
  ...
  | Functor_used_as_structure lid ->
    fprintf ppf "@[The module %a is a functor, it cannot have any components@]"
      !print_longident lid
  ...
  | Generative_used_as_applicative lid ->
    fprintf ppf
      "@[The functor %a is generative,@ it@ cannot@ be@ applied@ in@ type@ \
       expressions@]" !print_longident lid
  | Cannot_scrape_alias (lid, p) ->
    let cause =
      if Current_unit_name.is_path p then
...
let report_error ppf =
  ...
  | Missing_module (_, path1, path2) ->
    ...
    fprintf ppf "@]@ @[%s@ %s@ %s.@]@]" "The compiled interface for module"
      (Ident.name (Path.head path2)) "was not found"
     ...
     | Error err ->
       ...
       let error_of_printer =
         if loc = Location.none then
           ...
           Location.error_of_printer ~loc ?sub:None
       in
