...
=
  fun property env decls required ->
    ...
    let rec compute_fixpoint props =
      let new_decls =
        List.map2 (fun (id, decl) prop -> id, property.update_decl decl prop)
          decls props
      in
