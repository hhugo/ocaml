...
let record ti =
  if !Clflags.annotations && not (get_location ti).Location.loc_ghost
  then annotations := ti :: !annotations
...
let print_info pp prev_loc ti =
  ...
  | Ti_pat {pat_loc = loc; pat_type = typ; pat_env = env}
   |Ti_expr {exp_loc = loc; exp_type = typ; exp_env = env} ->
      if loc <> prev_loc then ( print_location pp loc; output_char pp '\n' );
      ...
      output_string pp s; output_string pp ")\n"; loc
  | An_call (loc, k) ->
      if loc <> prev_loc then ( print_location pp loc; output_char pp '\n' );
