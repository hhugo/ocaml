...
let acknowledge_pers_struct penv check modname pers_sig pm =
  ...
  then error (Illegal_renaming (modname, ps.ps_name, filename));
  List.iter
    (function
      | Rectypes ->
          if not !Clflags.recursive_types
          then error (Need_recursive_types ps.ps_name)
      | Unsafe_string ->
          if Config.safe_string
          then error (Depend_on_unsafe_string_unit ps.ps_name)
      | Alerts _ -> ()
      | Opaque -> add_imported_opaque penv modname)
    ps.ps_flags;
  ...
  then (
    ...
    then
      !add_delayed_check_forward (fun () ->
          check_pers_struct penv f ~loc name ) )

let crc_of_unit penv f name =
  let ps, _pm = find_pers_struct penv f true name in
  let crco =
    try List.assoc name ps.ps_crcs with Not_found -> assert false
  in
