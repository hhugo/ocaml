...
let native_repr_args nra1 nra2 =
  let rec loop i nra1 nra2 =
    ...
    | _ :: _, [] -> assert false
    | nr1 :: nra1, nr2 :: nra2 ->
      if not (Primitive.equal_native_repr nr1 nr2) then
        Some (Argument_repr i)
      else
        loop (i + 1) nra1 nra2
  in
  loop 1 nra1 nra2
...
let primitive_descriptions pd1 pd2 =
  ...
  if not (String.equal pd1.prim_name pd2.prim_name) then
    ...
    not
      (Primitive.equal_native_repr pd1.prim_native_repr_res
         pd2.prim_native_repr_res)
  then
    Some Result_repr
  else
...
let value_descriptions
    ~loc env name (vd1 : Types.value_description)
...
=
  ...
  | () ->
    begin match vd1.val_kind, vd2.val_kind with
    | Val_prim p1, Val_prim p2 ->
      begin match primitive_descriptions p1 p2 with
      | None -> Tcoerce_none
...
type label_mismatch = Type of Errortrace.equality_error | Mutability of position
...
let report_privacy_mismatch ppf err =
  ...
  Format.fprintf ppf "%s %s would be revealed."
    (if singular then "A private" else "Private") item
...
let pp_record_diff first second prefix decl env ppf (x : record_change) =
  match x with
  | Delete cd ->
    Format.fprintf ppf "%aAn extra field, %s, is provided in %s %s." prefix x
      (Ident.name cd.delete.ld_id) first decl
  | Insert cd ->
    Format.fprintf ppf "%aA field, %s, is missing in %s %s." prefix x
      (Ident.name cd.insert.ld_id) first decl
  | Change (Type { got = lbl1; expected = lbl2; reason }) ->
    Format.fprintf ppf
      "@[<hv>%aFields do not match:@;\
       <1 2>%a@ is not the same as:@;\
       <1 2>%a@ %a@]" prefix x Printtyp.label lbl1 Printtyp.label lbl2
      (report_label_mismatch first second env) reason
  | Change (Name n) ->
    Format.fprintf ppf "%aFields have different names, %s and %s." prefix x
      n.got n.expected
  | Swap sw ->
    Format.fprintf ppf "%aFields %s and %s have been swapped." prefix x sw.first
      sw.last
  | Move { name; got; expected } ->
    Format.fprintf ppf
      "@[<2>%aField %s has been moved@ from@ position %d@ to %d.@]" prefix x
      name expected got
...
let report_patch pr_diff first second decl env ppf patch =
  ...
  match patch with
  | [ elt ] ->
    Format.fprintf ppf "@[<hv>%a@]" (pr_diff first second no_prefix decl env)
      elt
  | _ ->
    let pp_diff = pr_diff first second Diffing_with_keys.prefix decl env in
    Format.fprintf ppf "@[<hv>%a@]" (Format.pp_print_list ~pp_sep:nl pp_diff)
      patch
...
let pp_variant_diff first second prefix decl env ppf (x : variant_change) =
  ...
  | Insert cd ->
    Format.fprintf ppf "%aA constructor, %s, is missing in %s %s." prefix x
      (Ident.name cd.insert.cd_id) first decl
  | Change (Type { got; expected; reason }) ->
    Format.fprintf ppf
      "@[<hv>%aConstructors do not match:@;\
       <1 2>%a@ is not the same as:@;\
       <1 2>%a@ %a@]" prefix x Printtyp.constructor got Printtyp.constructor
      expected (report_constructor_mismatch first second decl env) reason
  | Change (Name n) ->
    Format.fprintf ppf "%aConstructors have different names, %s and %s." prefix
      x n.got n.expected
  | Swap sw ->
    Format.fprintf ppf "%aConstructors %s and %s have been swapped." prefix x
      sw.first sw.last
  | Move { name; got; expected } ->
    Format.fprintf ppf
      "@[<2>%aConstructor %s has been moved@ from@ position %d@ to %d.@]" prefix
      x name expected got
  ...
  | Constructor_mismatch (id, ext1, ext2, err) ->
    pr
      "@[<hv>Constructors do not match:@;\
       <1 2>%a@ is not the same as:@;\
       <1 2>%a@ %a@]" (Printtyp.extension_only_constructor id) ext1
...
let report_private_variant_mismatch first second decl env ppf err =
  ...
  | Missing (ord, name) ->
    pr "The constructor %s is only present in %s %s." name
      (choose ord first second) decl
  | Presence s ->
    pr "The tag `%s is present in the %s %s,@ but might not be in the %s" s
      second decl first
...
let report_type_mismatch first second decl env ppf err =
  ...
  | Immediate violation ->
    ...
    | Type_immediacy.Violation.Not_always_immediate_on_64bits ->
      pr "%s is not a type that is always immediate on 64 bit platforms." first
    end

module Record_diffing = struct
  let compare_labels
      env params1 params2 (ld1 : Types.label_declaration)
  ...
  let rec equal
      ~loc env params1 params2 (labels1 : Types.label_declaration list)
  ...
  let test
      _loc env (params1, params2) ({ pos; data = lbl1 } : Diff.left)
...
module Variant_diffing = struct
  let compare_constructor_arguments ~loc env params1 params2 arg1 arg2 =
    ...
    | Types.Cstr_tuple arg1, Types.Cstr_tuple arg2 ->
      ...
      (* Ctype.equal must be called on all arguments at once, cf. PR#7378 *)
        begin match Ctype.equal env true (params1 @ arg1) (params2 @ arg2) with
        | exception (Ctype.Equality err) -> Some (Type err)
        | () -> None
        end
    | Types.Cstr_record l1, Types.Cstr_record l2 ->
      Option.map (fun rec_err -> Inline_record rec_err)
  ...
  let equal
      ~loc env params1 params2 (cstrs1 : Types.constructor_declaration list)
  ...
  let test
      loc env (params1, params2) ({ pos; data = cd1 } : D.left)
      ({ data = cd2; _ } : D.right)
  =
  ...
  let compare_with_representation
      ~loc env params1 params2 cstrs1 cstrs2 rep1 rep2
  =
...
let privacy_mismatch env decl1 decl2 =
  ...
  | Private, Public ->
    ...
    | Type_abstract, Type_abstract when Option.is_some decl2.type_manifest ->
      begin match decl1.type_manifest with
      | Some ty1 ->
        let ty1 = Ctype.expand_head env ty1 in
        begin match get_desc ty1 with
        | Tvariant row
          when Btype.is_constr_row ~allow_ident:true (Btype.row_more row) ->
...
let private_variant env row1 params1 row2 params2 =
  let (r1, r2, pairs) = Ctype.merge_row_fields row1.row_fields row2.row_fields
  in
  let err =
    if row2.row_closed && not row1.row_closed then
      Some Only_outer_closed
    else
  ...
  if err <> None then
    ...
    if err <> None then
      ...
      let rec loop tl1 tl2 pairs =
        match pairs with
        | [] ->
          begin match Ctype.equal env true tl1 tl2 with
          | exception (Ctype.Equality err) ->
            Some (Types err : private_variant_mismatch)
          | () -> None
          end
        | (s, f1, f2) :: pairs ->
          begin match Btype.row_field_repr f1, Btype.row_field_repr f2 with
          ...
          | Rpresent to1, Reither (const2, ts2, _, _) ->
            begin match to1, const2, ts2 with
...
let type_manifest env ty1 params1 ty2 params2 priv2 kind2 =
  ...
  | Tvariant row1, Tvariant row2 when is_absrow env (Btype.row_more row2) ->
    let row1 = Btype.row_repr row1
    and row2 = Btype.row_repr row2
    in
    assert (Ctype.is_equal env true (ty1 :: params1) (row2.row_more :: params2));
    begin match private_variant env row1 params1 row2 params2 with
    | None -> None
  ...
  | Tobject (fi1, _), Tobject (fi2, _)
    ...
    begin match private_object env fields1 params1 fields2 params2 with
    | None -> None
  ...
  | _ ->
    let is_private_abbrev_2 =
      ...
      | Private, Type_abstract ->
        (* Same checks as the [when] guards from above, inverted *)
        begin match get_desc ty2' with
  ...
  if decl1.type_arity <> decl2.type_arity then
    ...
    if err <> None then
      ...
      let err =
        ...
        | None, Some ty2 ->
          let ty1 = Btype.newgenty (Tconstr (path, decl2.type_params, ref Mnil))
          in
          begin match
            Ctype.equal env true decl1.type_params decl2.type_params
          with
      ...
      if err <> None then
        ...
        let err =
          ...
          | Type_variant (cstrs1, rep1), Type_variant (cstrs2, rep2) ->
            (if mark then
               ...
               let usage : Env.constructor_usage =
                 if decl2.type_private = Public then
                   Env.Exported
                 else
                   Env.Exported_private
               in
               (mark usage cstrs1;
          ...
          | Type_record (labels1, rep1), Type_record (labels2, rep2) ->
            (if mark then
               let mark usage lbls = List.iter (Env.mark_label_used usage) lbls
               in
...
let extension_constructors ~loc env ~mark id ext1 ext2 =
  ...
  | () ->
    ...
    | None ->
      begin match ext1.ext_private, ext2.ext_private with
