...
let primitive_descriptions pd1 pd2 =
  ...
  if not (String.equal pd1.prim_name pd2.prim_name) then Some Name
  else if not (Int.equal pd1.prim_arity pd2.prim_arity) then Some Arity
  else if (not pd1.prim_alloc) && pd2.prim_alloc then Some (No_alloc First)
  else if pd1.prim_alloc && not pd2.prim_alloc then Some (No_alloc Second)
...
let pp_record_diff first second prefix decl env ppf (x : record_change) =
  ...
  | Change (Name n) ->
      Format.fprintf ppf "%aFields have different names, %s and %s." prefix x
        n.got n.expected
  | Swap sw ->
      Format.fprintf ppf "%aFields %s and %s have been swapped." prefix x
        sw.first sw.last
...
let pp_variant_diff first second prefix decl env ppf (x : variant_change) =
  ...
  | Change (Type { got; expected; reason }) ->
      Format.fprintf ppf
        ...
        reason
  | Change (Name n) ->
      Format.fprintf ppf "%aConstructors have different names, %s and %s."
...
module Record_diffing = struct
  ...
  let rec equal ~loc env params1 params2
    ...
    | _ :: _, [] | [], _ :: _ -> false
    | ld1 :: rem1, ld2 :: rem2 ->
        if Ident.name ld1.ld_id <> Ident.name ld2.ld_id then false
        else (
          ...
          | None ->
              equal ~loc env (ld1.ld_type :: params1) (ld2.ld_type :: params2)
                rem1 rem2)
  ...
  let compare ~loc env params1 params2 l r =
    if equal ~loc env params1 params2 l r then None
...
let private_variant env row1 params1 row2 params2 =
  let r1, r2, pairs = Ctype.merge_row_fields row1.row_fields row2.row_fields in
  let err =
...
let type_manifest env ty1 params1 ty2 params2 priv2 kind2 =
  ...
  | _ -> (
      ...
      match
        ...
        else Ctype.equal env true (params1 @ [ ty1 ]) (params2 @ [ ty2 ])
      with
