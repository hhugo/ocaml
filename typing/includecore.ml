...
let value_descriptions ~loc env name (vd1 : Types.value_description)
  ...
  Builtin_attributes.check_alerts_inclusion
    ...
    vd2.val_attributes
    name;
  if Ctype.moregeneral env true vd1.val_type vd2.val_type
  then
...
let type_manifest env ty1 params1 ty2 params2 priv2 =
  ...
  and ty2' = Ctype.expand_head env ty2 in
...
and compare_variants ~loc env params1 params2 n
  ...
  | cd1 :: rem1, cd2 :: rem2 ->
      ...
      then Some (Field_names (n, cd1.cd_id, cd2.cd_id))
      else (
        Builtin_attributes.check_alerts_inclusion
          ~def:cd1.cd_loc
          ~use:cd2.cd_loc
        ...
        let r =
          ...
          | Some _, None | None, Some _ -> Some (Field_type cd1.cd_id)
          | _ ->
              compare_constructor_arguments
                ~loc
                env
...
and compare_records ~loc env params1 params2 n
  ...
  | ld1 :: rem1, ld2 :: rem2 ->
      ...
      then Some (Field_mutable ld1.ld_id)
      else (
        ...
        then
          ...
          compare_records
            ...
            rem1
            rem2
        else Some (Field_type ld1.ld_id) )
...
let type_declarations ?(equality = false) ~loc env ~mark name decl1 id decl2
  ...
  Builtin_attributes.check_alerts_inclusion
    ...
    ~use:decl2.type_loc
    loc
    decl1.type_attributes
    decl2.type_attributes
    name;
  if decl1.type_arity <> decl2.type_arity
  then Some Arity
  else if not (private_flags decl1 decl2)
  then Some Privacy
  else
    let err =
      ...
      | None, Some ty2 ->
          let ty1 =
            Btype.newgenty
              (Tconstr (Pident id, decl2.type_params, ref Mnil))
          in
          if Ctype.equal env true decl1.type_params decl2.type_params
          then
        ...
        let err =
          ...
          | Type_variant cstrs1, Type_variant cstrs2 ->
              ...
              then (
                ...
                in
                let usage =
                  if decl1.type_private = Private
                     || decl2.type_private = Public
                  then Env.Positive
                  else Env.Privatize
                in
                mark cstrs1 usage name decl1;
                if equality
                then mark cstrs2 Env.Positive (Ident.name id) decl2 );
              compare_variants
                ~loc
                env
                decl1.type_params
          ...
          | Type_record (labels1, rep1), Type_record (labels2, rep2) ->
              ...
              then err
              else Some (Record_representation (rep2 = Record_float))
  ...
  then Some (Field_type id)
    ...
    then r
      ...
      | Some r1, Some r2 when not (Ctype.equal env true [r1] [r2]) ->
          Some (Field_type id)
      | Some _, None | None, Some _ -> Some (Field_type id)
      | _ -> (
