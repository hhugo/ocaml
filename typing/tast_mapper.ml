...
type mapper =
  ...
  ; module_substitution :
      mapper -> module_substitution -> module_substitution
...
let type_kind sub = function
  ...
  | Ttype_variant list ->
      Ttype_variant (List.map (constructor_decl sub) list)
...
let expr sub x =
  ...
  let exp_desc =
    ...
    | Texp_function {arg_label; param; cases; partial} ->
        Texp_function
          {arg_label; param; cases = sub.cases sub cases; partial}
    | Texp_apply (exp, list) ->
        Texp_apply
          (sub.expr sub exp, List.map (tuple2 id (opt (sub.expr sub))) list)
    | Texp_match (exp, cases, p) ->
        Texp_match (sub.expr sub exp, sub.cases sub cases, p)
    | Texp_try (exp, cases) ->
        Texp_try (sub.expr sub exp, sub.cases sub cases)
    ...
    | Texp_letmodule (id, s, pres, mexpr, exp) ->
        Texp_letmodule
          (id, s, pres, sub.module_expr sub mexpr, sub.expr sub exp)
    | Texp_letexception (cd, exp) ->
        Texp_letexception
          (sub.extension_constructor sub cd, sub.expr sub exp)
...
let module_coercion sub = function
  ...
  | Tcoerce_functor (c1, c2) ->
      Tcoerce_functor
        (sub.module_coercion sub c1, sub.module_coercion sub c2)
