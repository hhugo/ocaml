...
module Context = struct
  ...
  let rec context ppf = function
    ...
    | Arg x :: rem ->
        Format.fprintf ppf "functor (%s : %a) -> ..." (argname x) context_mty
          rem
  ...
  let alt_pp ppf cxt =
    ...
    else Format.fprintf ppf "@[<hv 2>at position@ %a,@]" context cxt
...
module Illegal_permutation = struct
  ...
  let rec runtime_item k = function
    | [] -> raise Not_found
    | item :: q ->
        if not (Includemod.is_runtime_component item) then runtime_item k q
...
let dwith_context_and_elision ?loc ctx printer diff =
  if is_big (diff.got, diff.expected) then Location.msg ?loc "..."
  else dwith_context ?loc ctx (printer diff)
...
let core env id x =
  ...
  | Err.Class_declarations { got; expected; symptom } ->
      ...
      Format.dprintf
        ...
        (Includeclass.report_error Type_scheme)
        symptom Printtyp.Conflicts.print_explanations
...
let all env = function
  | In_Compilation_unit diff ->
      ...
      signature ~expansion_token:true ~env ~before:[ first ] ~ctx:[]
        diff.symptom
