...
module Illegal_permutation = struct
  ...
  let rec transposition_under path (coerc : Typedtree.module_coercion) =
    ...
    | Tcoerce_functor (arg, res) ->
      either (transposition_under (InArg :: path)) arg
  ...
  and first_non_id path pos =
    ...
    | (_, c) :: q ->
      either (transposition_under (Item pos :: path)) c
  ...
  let pp_item ppf (id, _, kind) =
    Format.fprintf ppf "%s %S" (Includemod.kind_of_field_desc kind)
      (Ident.name id)
...
module With_shorthand = struct
  type 'a named = { item : 'a; name : string }
  ...
  type functor_param = Unit | Named of (Ident.t option * Types.module_type t)
  
  (** Shorthand generation *)
  type kind = Got | Expected | Unneeded
  
  type variant = App | Inclusion
  
  let elide_if_app ctx s =
    match ctx with
    | App -> Unneeded
    | Inclusion -> s
  ...
  open Diffing (** Add shorthands to a patch *)
  ...
  let modtype (r : _ named) =
    match r.item with
    | Types.Mty_ident _ | Types.Mty_alias _ | Types.Mty_signature [] ->
      Original r.item
...
module Functor_suberror = struct
  ...
  let pretty_params sep proj printer patch =
    let elt (x, param) =
      ...
      Format.dprintf "%a%t%a" Format.pp_open_stag (Misc.Color.Style sty)
  ...
  module App = struct
    ...
    let ok x y =
      ...
      Format.dprintf "Module %t matches the expected module type%t"
        (With_shorthand.arg x) pp_orig_name
    ...
    *)
    let single_diff g e more =
      ...
      in
      Format.dprintf
        "Modules do not match:@ @[%t@]@;<1 -2>is not included in@ @[%t@]%t"
        (dmodtype mty) e (more ())
  ...
  let subcase sub ~expansion_token env (pos, diff) =
    Location.msg "%a%a%a%a@[<hv 2>%t@]%a" Format.pp_print_tab ()
      Format.pp_open_tbox () Diffing.prefix (pos, Diffing.classify diff)
      Format.pp_set_tab ()
      (Printtyp.wrap_printing_env env ~error:true
         (fun () -> sub ~expansion_token env diff)) Format.pp_close_tbox ()
    ...
    let rec aux subcases =
      ...
      | a :: q ->
        List.fold_left
          ...
          (subcase sub ~expansion_token env a :: subcases) q
    in
    match l with
...
let coalesce msgs =
  ...
  | before ->
    let ctx ppf =
      Format.pp_print_list ~pp_sep:space (fun ppf x -> x.Location.txt ppf) ppf
        before
    in
    ctx
...
let core env id x =
  ...
  | Err.Value_descriptions diff ->
    Format.dprintf "@[<v>@[<hv>%s:@;<1 2>%a@ %s@;<1 2>%a@]%a%a%t@]"
      ...
      (Printtyp.tree_of_value_description id diff.got) "is not included in"
      !Oprint.out_sig_item (Printtyp.tree_of_value_description id diff.expected)
      (Includecore.report_value_mismatch "the first" "the second" env)
      diff.symptom show_locs (diff.got.val_loc, diff.expected.val_loc)
  ...
  | Err.Type_declarations diff ->
    Format.dprintf "@[<v>@[<hv>%s:@;<1 2>%a@ %s@;<1 2>%a@]%a%a%t@]"
      ...
      (Includecore.report_type_mismatch "the first" "the second" "declaration"
         env) diff.symptom show_locs (diff.got.type_loc, diff.expected.type_loc)
      Printtyp.Conflicts.print_explanations
  | Err.Extension_constructors diff ->
    Format.dprintf "@[<v>@[<hv>%s:@;<1 2>%a@ %s@;<1 2>%a@]@ %a%a%t@]"
      ...
      (Includecore.report_extension_constructor_mismatch "the first"
         "the second" "declaration" env) diff.symptom show_locs
  ...
  | Err.Class_declarations { got; expected; symptom } ->
    ...
    Format.dprintf
      ...
      !Oprint.out_sig_item t2 (Includeclass.report_error Type_scheme) symptom
      Printtyp.Conflicts.print_explanations
...
let interface_mismatch ppf (diff : _ Err.diff) =
  Format.fprintf ppf "The implementation %s@ does not match the interface %s:@ "
...
and module_type_symptom ~eqmode ~expansion_token ~env ~before ~ctx =
  function
  | Mt_core core ->
...
and functor_symptom ~expansion_token ~env ~before ~ctx =
  function
  | Result res ->
...
and signature ~expansion_token ~env:_ ~before ~ctx sgs =
  Printtyp.wrap_printing_env ~error:true sgs.env
    (fun () ->
...
let module_type_subst ~env id diff =
  ...
  | Illegal_permutation c ->
    let mty = diff.got in
    let main =
      with_context [ Modtype id ] (Illegal_permutation.pp Context.alt_pp env)
        (mty, c)
    in
    [ main ]
...
let all env =
  ...
  | In_Expansion cmts ->
    begin match core_module_type_symptom cmts with
    | None -> assert false
...
let report_apply_error ~loc env (lid_app, mty_f, args) =
  ...
  | _ ->
    ...
    Location.errorf ~loc ~sub
      "@[<hv>The functor application %tis ill-typed.@ These arguments:@;\
       <1 2>@[%t@]@ do not match these parameters:@;\
       <1 2>@[functor@ %t@ -> ...@]@]" may_print_app actual expected
  ...
  Location.register_error_of_exn
    (function
     ...
     | Includemod.Apply_error { loc; env; lid_app; mty_f; args } ->
       Some
         (Printtyp.wrap_printing_env env ~error:true
            (fun () -> report_apply_error ~loc env (lid_app, mty_f, args)))
     | _ -> None)
