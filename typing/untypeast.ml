...
let structure_item sub item =
  ...
  let desc =
    ...
    | Tstr_class_type list ->
      Pstr_class_type
        (List.map (fun (_id, _name, ct) -> sub.class_type_declaration sub ct)
           list)
...
let type_declaration sub decl =
  ...
  Type.mk ~loc ~attrs ~params:(List.map (type_parameter sub) decl.typ_params)
    ...
    (List.map
       (fun (ct1, ct2, loc) ->
          sub.typ sub ct1, sub.typ sub ct2, sub.location sub loc) decl.typ_cstrs)
    ~kind:(sub.type_kind sub decl.typ_kind) ~priv:decl.typ_private
...
let constructor_declaration sub cd =
  ...
  Type.constructor ~loc ~attrs ~vars:cd.cd_vars
    ...
    ?res:(Option.map (sub.typ sub) cd.cd_res) (map_loc sub cd.cd_name)
...
let type_extension sub tyext =
  ...
  Te.mk ~attrs ~params:(List.map (type_parameter sub) tyext.tyext_params)
    ~priv:tyext.tyext_private (map_loc sub tyext.tyext_txt)
...
let pattern : type k. _ -> k T.general_pattern -> _ =
  fun sub pat ->
    ...
    let desc =
      ...
      | { pat_extra = [ Tpat_unpack, _, _attrs ]; pat_desc = Tpat_var (_, name);
          _ }
        ->
      ...
      | _ ->
        ...
        | Tpat_any -> Ppat_any
        | Tpat_var (id, name) ->
          begin match (Ident.name id).[0] with
        ...
        *)
        ...
        | Tpat_construct (lid, _, args, vto) ->
          let tyo =
            ...
            | None -> None
            | Some (vl, ty) ->
              let vl = List.map (fun x -> { x with  txt = Ident.name x.txt }) vl
              in
        ...
        | Tpat_record (list, closed) ->
          Ppat_record
            (List.map (fun (lid, _, pat) -> map_loc sub lid, sub.pat sub pat)
               list,
             closed)
...
let expression sub exp =
  ...
  let desc =
        ...
        (sub.expr sub exp,
         List.fold_right
           (fun (label, expo) list ->
              match expo with
              | None -> list
              | Some exp -> (label, sub.expr sub exp) :: list) list [])
    ...
    | Texp_override (_, list) ->
      Pexp_override
        (List.map (fun (_path, lid, exp) -> map_loc sub lid, sub.expr sub exp)
           list)
    ...
    | Texp_letop { let_; ands; body; _ } ->
      let (pat, and_pats) = extract_letop_patterns (List.length ands) body.c_lhs
      in
...
let module_type_declaration sub mtd =
  ...
  Mtd.mk ~loc ~attrs ?typ:(Option.map (sub.module_type sub) mtd.mtd_type)
...
let class_infos f sub ci =
  ...
  Ci.mk ~loc ~attrs ~virt:ci.ci_virt
    ...
    (map_loc sub ci.ci_id_name) (f sub ci.ci_expr)
        ...
        (sub.class_expr sub cl,
         List.fold_right
           (fun (label, expo) list ->
              match expo with
              | None -> list
              | Some exp -> (label, sub.expr sub exp) :: list) args [])
...
let untype_pattern ?(mapper=default_mapper) pattern = mapper.pat mapper pattern
