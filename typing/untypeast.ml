...
type mapper =
  ...
  ; module_declaration :
      mapper -> T.module_declaration -> module_declaration
  ; module_substitution :
...
let structure_item sub item =
  ...
  let desc =
    ...
    | Tstr_class list ->
        Pstr_class
          (List.map (fun (ci, _) -> sub.class_declaration sub ci) list)
...
let expression sub exp =
  ...
  let desc =
    ...
    | Texp_function
        {arg_label = (Labelled s | Optional s) as label; cases; _} ->
    ...
    | Texp_letexception (ext, exp) ->
        Pexp_letexception
          (sub.extension_constructor sub ext, sub.expr sub exp)
...
let class_field sub cf =
  ...
  let desc =
    ...
    | Tcf_inherit (ovf, cl, super, _vals, _meths) ->
        Pcf_inherit
          (ovf, sub.class_expr sub cl, map_opt (fun v -> mkloc v loc) super)
