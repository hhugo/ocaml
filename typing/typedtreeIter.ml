...
module MakeIterator (Iter : IteratorArgument) : sig
  ...
  and iter_type_declaration decl =
    ...
    List.iter
      (fun (ct1, ct2, _loc) -> iter_core_type ct1; iter_core_type ct2)
      decl.typ_cstrs;
  ...
  and iter_pattern pat =
    ...
    | Tpat_lazy p | Tpat_exception p -> iter_pattern p );
  ...
  and iter_expression exp =
    ...
    | Texp_apply (exp, list) ->
        ...
        List.iter
          ...
          list
    | Texp_match (exp, list, _) -> iter_expression exp; iter_cases list
    | Texp_try (exp, list) -> iter_expression exp; iter_cases list
    ...
    | Texp_setfield (exp1, _, _label, exp2) ->
        iter_expression exp1; iter_expression exp2
    ...
    | Texp_for (_id, _, exp1, exp2, _dir, exp3) ->
        iter_expression exp1; iter_expression exp2; iter_expression exp3
    ...
    | Texp_override (_, list) ->
        List.iter (fun (_path, _, exp) -> iter_expression exp) list
    | Texp_letmodule (_id, _, _, mexpr, exp) ->
        iter_module_expr mexpr; iter_expression exp
  ...
  and iter_binding_op bop = iter_expression bop.bop_exp
  ...
  and iter_signature_item item =
    ...
    | Tsig_typesubst list -> iter_type_declarations Nonrecursive list
    | Tsig_exception ext -> iter_type_exception ext
  ...
  and iter_module_expr mexpr =
    ...
    | Tmod_apply (mexp1, mexp2, _) ->
        iter_module_expr mexp1; iter_module_expr mexp2
    | Tmod_constraint (mexpr, _, Tmodtype_implicit, _) ->
        iter_module_expr mexpr
    | Tmod_constraint (mexpr, _, Tmodtype_explicit mtype, _) ->
        iter_module_expr mexpr; iter_module_type mtype
    | Tmod_unpack (exp, _mty) ->
        iter_expression exp (*          iter_module_type mty *) );
  ...
  and iter_class_expr cexpr =
    ...
    | Tcl_constraint (cl, Some clty, _vals, _meths, _concrs) ->
        iter_class_expr cl; iter_class_type clty
    | Tcl_ident (_, _, tyl) -> List.iter iter_core_type tyl
    | Tcl_open (_, e) -> iter_class_expr e );
  ...
  and iter_class_type ct =
    ...
    | Tcty_arrow (_label, ct, cl) -> iter_core_type ct; iter_class_type cl
    | Tcty_open (_, e) -> iter_class_type e );
  ...
  and iter_class_type_field ctf =
    ...
    | Tctf_constraint (ct1, ct2) -> iter_core_type ct1; iter_core_type ct2
    | Tctf_attribute _ -> () );
  ...
  and iter_core_type ct =
    ...
    | Ttyp_arrow (_label, ct1, ct2) ->
        iter_core_type ct1; iter_core_type ct2
  ...
  and iter_row_field rf =
    ...
    | Ttag (_label, _bool, list) -> List.iter iter_core_type list
  ...
  and iter_class_field cf =
    ...
    | Tcf_inherit (_ovf, cl, _super, _vals, _meths) -> iter_class_expr cl
    | Tcf_constraint (cty, cty') -> iter_core_type cty; iter_core_type cty'
    | Tcf_val (_lab, _, _, Tcfk_virtual cty, _) -> iter_core_type cty
    | Tcf_val (_lab, _, _, Tcfk_concrete (_, exp), _) -> iter_expression exp
    | Tcf_method (_lab, _, Tcfk_virtual cty) -> iter_core_type cty
    | Tcf_method (_lab, _, Tcfk_concrete (_, exp)) -> iter_expression exp
