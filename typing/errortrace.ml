...
let explain trace f =
  let rec explain =
    ...
    | [ h ] -> f ~prev:None h
    | h :: (prev :: _ as rem) ->
      begin match f ~prev:(Some prev) h with
      | Some _ as m -> m
      | None -> explain rem
      end
  in
...
let map_elt (type variety) f : ('a, variety) elt -> ('b, variety) elt =
  function
...
let swap_elt (type variety) : ('a, variety) elt -> ('a, variety) elt =
  function
