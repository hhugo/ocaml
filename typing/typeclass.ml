...
type error =
  ...
  | Unbound_type_var of
      (formatter -> unit)
      * (type_expr * bool * string * type_expr)
...
let update_class_signature loc env ~warn_implicit_public virt kind sign =
  ...
  if warn_implicit_public && implicit_public <> [] then
...
let check_virtual loc env virt kind sign =
  ...
  | Concrete ->
    begin match
      Btype.virtual_methods sign, Btype.virtual_instance_vars sign
    with
    | [], [] -> ()
...
let raise_inherit_class_signature_failure loc env sign =
  function
...
let add_instance_vars_met loc vars sign cl_num met_env =
  List.fold_left
    (fun met_env (label, id) ->
       add_instance_var_met loc label id sign cl_num [] met_env) met_env vars
...
let rec class_field_first_pass self_loc cl_num sign self_scope acc cf =
  let
    { rev_fields; val_env; par_env; concrete_meths; concrete_vals; local_meths;
      local_vals; vars; meths } =
    acc
  in
         ...
         let (val_env, par_env, inherited_vars, vars) =
           Vars.fold
             (fun label _ (val_env, par_env, inherited_vars, vars) ->
                ...
                val_env, par_env, inherited_vars, vars) parent_sign.csig_vars
             (val_env, par_env, [], vars)
         in
         let meths =
           Meths.fold
             (fun label _ meths ->
                if Meths.mem label meths then
                  meths
                else
  ...
  | Pcf_val (label, mut, Cfk_virtual styp) ->
    with_attrs
      (fun () ->
         ...
         add_instance_variable ~strict:true loc val_env label.txt mut Virtual ty
           sign;
  ...
  | Pcf_val (label, mut, Cfk_concrete (override, sdefinition)) ->
    with_attrs
      (fun () ->
         ...
         (if VarSet.mem label.txt concrete_vals then
            (if override = Fresh then
               Location.prerr_warning label.loc
                 (Warnings.Instance_variable_override [ label.txt ]))
          else if override = Override then
            raise
              (Error
                 (loc, val_env, No_overriding ("instance variable", label.txt))));
         (if !Clflags.principal then Ctype.begin_def ());
  ...
  | Pcf_method (label, priv, Cfk_virtual sty) ->
    with_attrs
      (fun () ->
         ...
         let meths =
           if Meths.mem label.txt meths then
             meths
           else
             Meths.add label.txt (Ident.create_local label.txt) meths
         in
  ...
  | Pcf_method (label, priv, Cfk_concrete (override, expr)) ->
    with_attrs
      (fun () ->
         ...
         (if MethSet.mem label.txt concrete_meths then
            (if override = Fresh then
               Location.prerr_warning loc
...
and class_fields_first_pass
    self_loc cl_num sign self_scope val_env par_env cfs
=
  let rev_fields = [] in
  ...
  let acc =
    Builtin_attributes.warning_scope []
      (fun () ->
         List.fold_left (class_field_first_pass self_loc cl_num sign self_scope)
           init_acc cfs)
  in
...
and class_field_second_pass cl_num sign met_env field =
  ...
  | Inherit
    ...
    ->
    let met_env = add_instance_vars_met loc inherited_vars sign cl_num met_env
    in
    let met_env =
      match super with
      | None -> met_env
      | Some name ->
        let meths =
          List.fold_left (fun acc (label, id) -> Meths.add label id acc)
  ...
  | Virtual_val { label; mut; id; cty; already_declared; loc; attributes } ->
    ...
    met_env, mkcf desc loc attributes
  | Concrete_val
      { label; mut; id; override; definition; already_declared; loc; attributes
      }
    ->
    let met_env =
      if already_declared then
  ...
  let (_, rev_cfs) =
    List.fold_left
      (fun (met_env, cfs) field ->
         let (met_env, cf) = class_field_second_pass cl_num sign met_env field
         in
         met_env, cf :: cfs) (met_env, []) fields
  in
  List.rev rev_cfs
...
and class_structure
    cl_num virt self_scope final val_env met_env loc
  ...
  let (val_env, par_env) =
    List.fold_right
      (fun { pv_id; _ } (val_env, par_env) ->
         ...
         val_env, par_env) self_pat_vars (val_env, par_env)
  in
    ...
    List.fold_right
      (fun { pv_id; pv_type; pv_loc; pv_as_var; pv_attributes } met_env ->
         add_self_met pv_loc pv_id sign self_var_kind vars cl_num pv_as_var
           pv_type pv_attributes met_env) self_pat_vars met_env
  in
...
and class_expr_aux cl_num val_env met_env virt self_scope scl =
  ...
  | Pcl_constr (lid, styl) ->
    ...
    let tyl = List.map (fun sty -> transl_simple_type val_env false sty) styl in
    ...
    List.iter2
      (fun cty' ty ->
         ...
         with
         | Ctype.Unify err ->
           raise (Error (cty'.ctyp_loc, val_env, Parameter_mismatch err))) tyl
      params;
    let cl =
      rc
        {
  ...
  | Pcl_fun (l, Some default, spat, sbody) ->
    ...
    in
    let sfun =
      Cl.fun_ ~loc:scl.pcl_loc l None (Pat.var ~loc (mknoloc "*opt*"))
  ...
  | Pcl_apply (scl', sargs) ->
    ...
    let rec nonopt_labels ls ty_fun =
      ...
      | Cty_arrow (l, _, ty_res) ->
        if Btype.is_optional l then
          nonopt_labels ls ty_res
        else
          nonopt_labels (l :: ls) ty_res
      | _ -> ls
    in
    let ignore_labels =
      !Clflags.classic ||
    ...
    let rec type_args args omitted ty_fun ty_fun0 sargs =
      ...
      | Cty_arrow (l, ty, ty_fun), Cty_arrow (_, ty0, ty_fun0)
        ...
        and optional = Btype.is_optional l
        in
        let use_arg sarg l' =
          Some
            (if not optional || Btype.is_optional l' then
               ...
               and ty0' = extract_option_type val_env ty0
               in
        ...
        let (remaining_sargs, arg) =
          if ignore_labels then
            ...
            | (l', sarg) :: remaining_sargs ->
              if name = Btype.label_name l' || not optional && l' = Nolabel then
                remaining_sargs, use_arg sarg l'
              else if
                optional &&
                  not
                    (List.exists (fun (l, _) -> name = Btype.label_name l)
                       remaining_sargs)
              then
      ...
      | _ ->
        ...
        | [] ->
          List.rev args,
          List.fold_left (fun ty_fun (l, ty) -> Cty_arrow (l, ty, ty_fun))
            ty_fun0 omitted
        end
    in
    let (args, cty) =
  ...
  | Pcl_let (rec_flag, sdefs, scl') ->
    let (defs, val_env) = Typecore.type_let In_class_def val_env rec_flag sdefs
    in
    let (vals, met_env) =
      List.fold_right
        ...
        (let_bound_idents_full defs) ([], met_env)
    in
    let cl = class_expr cl_num val_env met_env virt self_scope scl' in
    let () = if rec_flag = Recursive then check_recursive_bindings val_env defs
    in
    rc
      {
  ...
  | Pcl_constraint (scl', scty) ->
    ...
    Typetexp.widen context;
    Ctype.end_def ();
    Ctype.limited_generalize_class_type (Btype.self_type_row cl.cl_type)
      cl.cl_type;
    Ctype.limited_generalize_class_type (Btype.self_type_row clty.cltyp_type)
      clty.cltyp_type;
...
let rec approx_declaration cl =
  ...
  | Pcl_fun (l, _, _, cl) ->
    let arg =
      if Btype.is_optional l then Ctype.instance var_option else Ctype.newvar ()
    in
...
let initial_env
    define_class approx (res, env) (cl, id, ty_id, obj_id, cl_id, uid)
=
...
=
  ...
  let ci_params =
    let make_param (sty, v) =
      ...
      with
      | Already_bound -> raise (Error (sty.ptyp_loc, env, Repeated_parameter))
    in
  ...
  (try
     Ctype.unify env (constructor_type constr obj_type)
       (Ctype.instance constr_type)
   with
   | Ctype.Unify err ->
     raise
       (Error
          (cl.pci_loc, env, Constructor_type_mismatch (cl.pci_name.txt, err))));
    ...
    req =
      {
        ci_loc = cl.pci_loc;
        ci_virt = cl.pci_virt;
        ci_params;(* TODO : check that we have the correct use of identifiers *)
    ...
    List.map
      (function
       | cl ->
         ...
         Ident.create_scoped ~scope cl.pci_name.txt,
         Ident.create_scoped ~scope ("#" ^ cl.pci_name.txt),
         Uid.mk ~current_unit:(Env.get_unit_name ())) cls
  in
  Ctype.begin_class_def ();
  let (res, env) =
    List.fold_left (initial_env define_class approx) ([], env) cls
  in
  let (res, env) = List.fold_right (class_infos define_class kind) res ([], env)
  in
  Ctype.end_def ();
...
let class_declaration env virt sexpr =
  ...
  let expr = class_expr (Int.to_string !class_num) env env virt self_scope sexpr
  in
...
let non_virtual_string_of_kind =
  function
  | Object -> "object"
...
let report_error env ppf =
  ...
  | Unexpected_field (ty, lab) ->
    ...
    fprintf ppf
      "@[@[<2>This object is expected to have type :@ %a@]@ This type does \
       not have a method %s." Printtyp.type_expr ty lab
  ...
  | Abbrev_type_clash (abbrev, actual, expected) ->
    ...
    fprintf ppf
      "@[The abbreviation@ %a@ expands to type@ %a@ but is used with type@ \
       %a@]" !Oprint.out_type (Printtyp.tree_of_typexp Type abbrev)
      !Oprint.out_type (Printtyp.tree_of_typexp Type actual) !Oprint.out_type
  ...
  | Undeclared_methods (kind, mets) ->
    ...
    fprintf ppf
      "@[This %s has undeclared virtual methods.@ @[<2>The following methods \
       were not declared : %a@]@]" kind
      (pp_print_list ~pp_sep:pp_print_space pp_print_string) mets
  | Parameter_arity_mismatch (lid, expected, provided) ->
    fprintf ppf
      "@[The class constructor %a@ expects %i type argument(s),@ but is here \
       applied to %i type argument(s)@]" Printtyp.longident lid expected
      provided
  | Parameter_mismatch err ->
    Printtyp.report_unification_error ppf env err
      (function ppf -> fprintf ppf "The type parameter")
      (function ppf -> fprintf ppf "does not meet its constraint: it should be")
  | Bad_parameters (id, params, cstrs) ->
    Printtyp.reset_and_mark_loops_list [ params; cstrs ];
    fprintf ppf
      "@[The abbreviation %a@ is used with parameters@ %a@ which are \
       incompatible with constraints@ %a@]" Printtyp.ident id !Oprint.out_type
      (Printtyp.tree_of_typexp Type params) !Oprint.out_type
      (Printtyp.tree_of_typexp Type cstrs)
  ...
  | Unbound_type_var (printer, reason) ->
    let print_reason ppf (ty0, real, lab, ty) =
      let ty1 = if real then ty0 else Btype.newgenty (Tobject (ty0, ref None))
      in
      List.iter Printtyp.mark_loops [ ty; ty1 ];
      fprintf ppf "The method %s@ has type@;<1 2>%a@ where@ %a@ is unbound" lab
        !Oprint.out_type (Printtyp.tree_of_typexp Type ty) !Oprint.out_type
  ...
  | Non_generalizable_class (id, clty) ->
    fprintf ppf
      "@[The type of this class,@ %a,@ contains type variables that cannot \
       be generalized@]" (Printtyp.class_declaration id) clty
  | Cannot_coerce_self ty ->
    fprintf ppf
      "@[The type of self cannot be coerced to@ the type of the current \
       class:@ %a.@.Some occurrences are contravariant@]" Printtyp.type_scheme
      ty
  ...
  | Mutability_mismatch (_lab, mut) ->
    let (mut1, mut2) =
      if mut = Immutable then "mutable", "immutable" else "immutable", "mutable"
    in
  ...
  | Duplicate (kind, name) ->
    fprintf ppf "@[The %s `%s'@ has multiple definitions in this object@]" kind
      name
  | Closing_self_type sign ->
    fprintf ppf
      "@[Cannot close type of object literal:@ %a@,\
       it has been unified with the self type of a class that is not yet@ \
       completely defined.@]" Printtyp.type_scheme sign.csig_self
