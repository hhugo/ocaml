...
let virtual_methods sign =
  ...
  List.fold_left
    (fun virt (lab, _, _) ->
      if lab = dummy_method
      then virt
      else if Concr.mem lab sign.csig_concr
      then virt
      else lab :: virt )
    []
    fields
...
let enter_val cl_num vars inh lab mut virt ty val_env met_env par_env loc =
  let id, virt =
    try
      ...
      if mut' <> mut
      then raise (Error (loc, val_env, Mutability_mismatch (lab, mut)));
  ...
  | Cty_signature cl_sig ->
      ...
      ( try Ctype.unify env self_type cl_sig.csig_self
        with Ctype.Unify trace -> (
          ...
          | Diff _ :: Incompatible_fields {name = n; _} :: rem ->
              raise
                (Error (loc, env, Field_type_mismatch ("method", n, rem)))
          | _ -> assert false ) );
      (* Overriding *)
      ...
      | Some Fresh ->
          let cname =
            ...
            | _ -> "inherited"
          in
          if not (Concr.is_empty over_meths)
          then
            Location.prerr_warning
              loc
              (Warnings.Method_override (cname :: Concr.elements over_meths));
          if not (Concr.is_empty over_vals)
          then
            Location.prerr_warning
              loc
...
let virtual_method val_env meths self_type lab priv sty loc =
  ...
  ( try Ctype.unify val_env ty ty'
    with Ctype.Unify trace ->
      raise
        (Error (loc, val_env, Field_type_mismatch ("method", lab, trace)))
  );
  cty
...
and class_field_aux self_loc cl_num self_type meths vars
  ...
  | Pcf_inherit (ovf, sparent, super) ->
      ...
      let val_env, met_env, par_env, super =
        match super with
        | None -> val_env, met_env, par_env, None
        | Some {txt = name} ->
  ...
  | Pcf_val (lab, mut, Cfk_virtual styp) ->
      ...
      , lazy
          (mkcf
             (Tcf_val (lab, mut, id, Tcfk_virtual cty, met_env == met_env')))
        :: fields
      , concr_meths
      , warn_vals
      , inher
  ...
  | Pcf_val (lab, mut, Cfk_concrete (ovf, sexp)) ->
      if Concr.mem lab.txt local_vals
      then
        raise
          (Error (loc, val_env, Duplicate ("instance variable", lab.txt)));
      if Concr.mem lab.txt warn_vals
      then (
        if ovf = Fresh
        then
          Location.prerr_warning
  ...
  | Pcf_method (lab, priv, Cfk_virtual sty) ->
      let cty =
        virtual_method val_env meths self_type lab.txt priv sty loc
      in
      ( val_env
      , met_env
      , par_env
  ...
  | Pcf_method (lab, priv, Cfk_concrete (ovf, expr)) ->
      ...
      in
      if Concr.mem lab.txt local_meths
      then raise (Error (loc, val_env, Duplicate ("method", lab.txt)));
      if Concr.mem lab.txt concr_meths
      then (
        if ovf = Fresh
        then Location.prerr_warning loc (Warnings.Method_override [lab.txt]) )
      else if ovf = Override
      then raise (Error (loc, val_env, No_overriding ("method", lab.txt)));
      let _, ty =
      ...
      ( try
        ...
        with Ctype.Unify trace ->
          raise
            (Error
               (loc, val_env, Field_type_mismatch ("method", lab.txt, trace)))
      );
              ...
              mk_expected
                (Ctype.newty
                   (Tarrow
                      ( Nolabel
                      , self_type
                      , Ctype.instance Predef.type_unit
                      , Cok )))
            in
            vars := vars_local;
...
and class_structure cl_num final val_env met_env loc
  ...
  let ty =
    if final
    then Ctype.newty (Tobject (Ctype.newvar (), ref None))
    else self_type
  in
  ...
  then (
    ...
    if not (Ctype.close_object self_type)
    then raise (Error (loc, val_env, Closing_self_type self_type));
  ...
  if added <> []
  then Location.prerr_warning loc (Warnings.Implicit_public_methods added);
  let sign =
    if final
    then sign
...
and class_expr_aux cl_num val_env met_env scl =
  ...
  | Pcl_fun (l, None, spat, scl') ->
      ...
      Ctype.end_def ();
      if Btype.is_optional l && not_function cl.cl_type
      then
        Location.prerr_warning
          pat.pat_loc
  ...
  | Pcl_apply (scl', sargs) ->
        ...
        | Cty_arrow (l, ty, ty_fun), Cty_arrow (_, ty0, ty_fun0)
          when sargs <> [] || more_sargs <> [] ->
            ...
            let sargs, more_sargs, arg =
              if ignore_labels && not (Btype.is_optional l)
              then
                ...
                | _, (l', sarg0) :: more_sargs ->
                    ...
                    then
                      raise
                        (Error
                           (sarg0.pexp_loc, val_env, Apply_wrong_label l'))
                    else
                      ( []
                      , more_sargs
                ...
                try
                  ...
                  , more_sargs
                  , if (not optional) || Btype.is_optional l'
                    then Some (type_argument val_env sarg0 ty ty0)
                    else
            ...
            in
            let omitted =
              if arg = None then (l, ty0) :: omitted else omitted
            in
            type_args
              ((l, arg) :: args)
              omitted
      ...
      let args, cty =
        ...
        if ignore_labels
        then type_args [] [] cl.cl_type ty_fun0 [] sargs
  ...
  | Pcl_constraint (scl', scty) ->
      ...
      | [] -> ()
      | error ->
          raise (Error (cl.cl_loc, val_env, Class_match_failure error)) );
...
let class_infos define_class kind
  ...
  (let ty = Ctype.self_type obj_type in
   ...
   if not (Ctype.close_object ty)
   then raise (Error (cl.pci_loc, env, Closing_self_type ty));
       ...
       raise
         (Error
            ...
            , Bad_parameters
                ( obj_id
                , constr
                , Ctype.newconstr (Path.Pident obj_id) obj_params' ) )) );
  ...
  let pub_meths =
    let fields, _ =
      Ctype.flatten_fields
        (Ctype.object_fields (Ctype.expand_head env obj_ty))
    in
...
let final_decl env define_class
  ...
  ( try Ctype.collapse_conj_params env clty.cty_params
    with Ctype.Unify trace ->
      raise
        (Error
           (cl.pci_loc, env, Non_collapsable_conjunction (id, clty, trace)))
  );
  ...
  if not (closed_class clty)
  then raise (Error (cl.pci_loc, env, Non_generalizable_class (id, clty)));
  ( match
      Ctype.closed_class
        clty.cty_params
    ...
    (Env.add_type
       ...
       (Env.add_cltype
          ...
          then
            Env.add_class
              id
              (Subst.class_declaration Subst.identity clty)
              env
          else env )))
...
let approx_class sdecl =
  ...
  let self' = Typ.any () in
  let clty' =
    Cty.signature ~loc:sdecl.pci_expr.pcty_loc (Csig.mk self' [])
  in
...
let report_error env ppf = function
  ...
  | Abbrev_type_clash (abbrev, actual, expected) ->
      ...
      fprintf
        ppf
        "@[The abbreviation@ %a@ expands to type@ %a@ but is used with \
         type@ %a@]"
        !Oprint.out_type
  ...
  | Virtual_class (cl, imm, mets, vals) ->
      ...
      let print_msg ppf =
        if imm
        then fprintf ppf "This object has virtual %s" missings
        else if cl
        then fprintf ppf "This class should be virtual"
  ...
  | Parameter_arity_mismatch (lid, expected, provided) ->
      fprintf
        ppf
        "@[The class constructor %a@ expects %i type argument(s),@ but is \
         here applied to %i type argument(s)@]"
        Printtyp.longident
        lid
        expected
        provided
  ...
  | Non_collapsable_conjunction (id, clty, trace) ->
      fprintf
        ppf
        "@[The type of this class,@ %a,@ contains non-collapsible \
         conjunctive types in constraints.@ %t@]"
        (Printtyp.class_declaration id)
        clty
        (fun ppf ->
          Printtyp.report_unification_error
  ...
  | Final_self_clash trace ->
      Printtyp.report_unification_error
        ...
        trace
        (function
          | ppf -> fprintf ppf "This object is expected to have type")
        (function ppf -> fprintf ppf "but actually has type")
  | Mutability_mismatch (_lab, mut) ->
      let mut1, mut2 =
        if mut = Immutable
        then "mutable", "immutable"
  ...
  | Closing_self_type self ->
      fprintf
        ppf
        "@[Cannot close type of object literal:@ %a@,it has been unified \
         with the self type of a class that is not yet@ completely \
         defined.@]"
        Printtyp.type_scheme
        self
