...
let raise_add_instance_variable_failure loc env label failure =
  ...
  | Ctype.Type_mismatch trace ->
      raise
        (Error
           (loc, env, Field_type_mismatch ("instance variable", label, trace)))
    ...
    ->
      ...
      let met_env =
        ...
        | Some name ->
            ...
            let attrs = [] in
            let _id, met_env =
              enter_ancestor_met ~loc name ~sign ~meths ~cl_num ~ty ~attrs
                met_env
            in
            met_env
      in
...
and class_expr_aux cl_num val_env met_env virt self_scope scl =
  ...
  | Pcl_constraint (scl', scty) ->
      ...
      Ctype.limited_generalize_class_type
        (Btype.self_type_row cl.cl_type)
        cl.cl_type;
      Ctype.limited_generalize_class_type
        (Btype.self_type_row clty.cltyp_type)
        clty.cltyp_type;
...
let report_error env ppf = function
  ...
  | Bad_parameters (id, params, cstrs) ->
      ...
      fprintf ppf
        ...
        (Printtyp.tree_of_typexp Type params)
        !Oprint.out_type
  ...
  | Unbound_type_var (printer, reason) ->
      let print_reason ppf (ty0, real, lab, ty) =
        ...
        fprintf ppf "The method %s@ has type@;<1 2>%a@ where@ %a@ is unbound"
          lab !Oprint.out_type
          (Printtyp.tree_of_typexp Type ty)
          !Oprint.out_type
