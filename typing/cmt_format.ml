...
let read filename =
  ...
  Misc.try_finally
    ...
    (fun () ->
      ...
      let cmi, cmt =
        if magic_number = Config.cmt_magic_number
        then None, Some (input_cmt ic)
        else if magic_number = Config.cmi_magic_number
        then
...
let record_value_dependency vd1 vd2 =
  if vd1.Types.val_loc <> vd2.Types.val_loc
  then value_deps := (vd1, vd2) :: !value_deps

let save_cmt filename modname binary_annots sourcefile initial_env cmi =
  if !Clflags.binary_annotations && not !Clflags.print_types
  then
    Misc.output_to_file_via_temporary
      ...
      (fun temp_file_name oc ->
        ...
        let cmt =
          ...
          ; cmt_initial_env =
              ( if need_to_clear_env
              then keep_only_summary initial_env
              else initial_env )
