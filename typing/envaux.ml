...
let rec env_from_summary sum subst =
  ...
  | Not_found ->
    let env =
      ...
      | Env_type (s, id, desc) ->
        Env.add_type ~check:false id (Subst.type_declaration subst desc)
          (env_from_summary s subst)
      | Env_extension (s, id, desc) ->
        Env.add_extension ~check:false ~rebind:false id
          (Subst.extension_constructor subst desc) (env_from_summary s subst)
      | Env_module (s, id, pres, desc) ->
        Env.add_module_declaration ~check:false id pres
          (Subst.module_declaration Keep subst desc) (env_from_summary s subst)
      ...
      | Env_functor_arg (Env_module (s, id, pres, desc), id')
        ...
        Env.add_module_declaration ~check:false id pres
          (Subst.module_declaration Keep subst desc) ~arg:true
          (env_from_summary s subst)
        ...
        Path.Map.fold
          (fun path info ->
             Env.add_local_type (Subst.type_path subst path)
               (Subst.type_declaration subst info)) map
          (env_from_summary s subst)
      | Env_copy_types s ->
        let env = env_from_summary s subst in
