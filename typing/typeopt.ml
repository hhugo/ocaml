...
let maybe_pointer_type env ty =
  let ty = scrape_ty env ty in
  if is_immediate (Ctype.immediacy env ty) then Immediate else Pointer
...
let classify env ty =
  let ty = scrape_ty env ty in
  if maybe_pointer_type env ty = Immediate then
    Int
  else
    match get_desc ty with
          ...
          Path.same p Predef.path_bytes ||
            Path.same p Predef.path_array ||
              Path.same p Predef.path_nativeint ||
                Path.same p Predef.path_int32 || Path.same p Predef.path_int64
      then
        Addr
      else
        (try
...
let bigarray_decode_type env ty tbl dfl =
  ...
  | Tconstr (Pdot (Pident mod_id, type_name), [], _)
    ...
    (try List.assoc type_name tbl
     with
     | Not_found -> dfl)
  | _ -> dfl

let kind_table =
  [ "float32_elt", Pbigarray_float32; "float64_elt", Pbigarray_float64;
    "int8_signed_elt", Pbigarray_sint8; "int8_unsigned_elt", Pbigarray_uint8;
    "int16_signed_elt", Pbigarray_sint16; "int16_unsigned_elt", Pbigarray_uint16;
    "int32_elt", Pbigarray_int32; "int64_elt", Pbigarray_int64;
    "int_elt", Pbigarray_caml_int; "nativeint_elt", Pbigarray_native_int;
...
let value_kind env ty =
  let ty = scrape_ty env ty in
  if is_immediate (Ctype.immediacy env ty) then
    Pintval
  else
    match get_desc ty with
