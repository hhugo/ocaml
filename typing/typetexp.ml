...
and transl_type_aux env policy styp =
  ...
  | Ptyp_package (p, l) ->
      ...
      let ty =
        newty
          (Tpackage
             (path, List.map (fun (s, cty) -> (s.txt, cty.ctyp_type)) ptys))
      in
      ctyp
        (Ttyp_package
           {
      ...
      | Otag (s, ty1) ->
          let ty1 =
            Builtin_attributes.warning_scope of_attributes (fun () ->
                transl_type env policy (Ast_helper.Typ.force_poly ty1))
          in
  ...
  fun () ->
    List.iter
      (function
        | loc, t1, t2 -> (
            try unify env t1 t2
            with Unify err -> raise (Error (loc, env, Type_mismatch err))))
      !r
