...
let rec narrow_unbound_lid_error : 'a. _ -> _ -> _ -> _ -> 'a =
 fun env loc lid make_error ->
  ...
  | Longident.Ldot (mlid, _) -> (
      check_module mlid;
      let md =
        Env.find_module (Env.lookup_module ~load:true mlid env) env
      in
  ...
  | Longident.Lapply (flid, mlid) -> (
      check_module flid;
      let fmd =
        Env.find_module (Env.lookup_module ~load:true flid env) env
      in
      let mty_param =
        ...
        | Mty_functor (_, None, _) ->
            error
              (Wrong_use_of_module (flid, `Generative_used_as_applicative))
      ...
      | mty_arg ->
          let details =
            try
              Includemod.check_modtype_inclusion
                ~loc
                env
                mty_arg
                mpath
...
let find_component (lookup : ?loc:_ -> ?mark:_ -> _) make_error env loc lid
  ...
  | Env.Recmodule ->
      raise (Error (loc, env, Illegal_reference_to_recursive_module))
  | err -> raise (!typemod_update_location loc err)
...
let create_package_mty fake loc env (p, l) =
  let l =
    List.sort
      (fun (s1, _t1) (s2, _t2) ->
        if s1.txt = s2.txt
        then raise (Error (loc, env, Multiple_constraints_on_type s1.txt));
...
and transl_type_aux env policy styp =
  ...
  | Ptyp_any ->
      let ty =
        ...
        then raise (Error (styp.ptyp_loc, env, Unbound_type_variable "_"))
        else newvar ()
      in
      ctyp Ttyp_any ty
  | Ptyp_var name ->
  ...
  | Ptyp_tuple stl ->
      ...
      let ty =
        newty (Ttuple (List.map (fun ctyp -> ctyp.ctyp_type) ctys))
      in
  ...
  | Ptyp_class (lid, stl) ->
      ...
      let ty =
        ...
        | Tvariant row ->
            ...
            let row =
              if static
              then {row with row_more = newty Tnil}
  ...
  | Ptyp_alias (st, alias) ->
      let cty =
        ...
        with Not_found ->
          ...
          if !Clflags.principal then ( end_def (); generalize_structure t );
          let t = instance t in
  ...
  | Ptyp_variant (fields, closed, present) ->
      ...
      let add_typed_field loc l f =
        ...
        try
          ...
          if l <> l'
          then raise (Error (styp.ptyp_loc, env, Variant_tags (l, l')));
          ...
          | Rinherit sty ->
              ...
              let fl =
                ...
                | {desc = Tvar _}, Some (p, _) ->
                    raise
                      (Error
                         (sty.ptyp_loc, env, Unbound_type_constructor_2 p))
      ...
      | Some present ->
          List.iter
            (fun l ->
              ...
              then raise (Error (styp.ptyp_loc, env, Present_has_no_type l))
              )
            present );
      let row =
  ...
  | Ptyp_package (p, l) ->
      ...
      widen z;
      let ptys =
        List.map (fun (s, pty) -> s, transl_type env policy pty) l
      in
    ...
    let of_desc =
      ...
      | Oinherit sty -> (
          ...
          | {desc = Tobject ({desc = (Tfield _ | Tnil) as tf}, _)}, _ ->
              if opened_object t
              then raise (Error (sty.ptyp_loc, env, Opened_object nm));
    ...
    (fun name (ty, loc) ->
      ...
      then (
        ...
        with Not_found ->
          ...
          then raise (Error (loc, env, Unbound_type_variable ("'" ^ name)));
...
let fold_descr fold get_name f =
  fold (fun descr acc -> f (get_name descr) acc)
...
let fold_values f =
  ...
  Env.fold_values (fun name _path descr acc ->
      match descr.val_kind with Val_unbound _ -> acc | _ -> f name acc )
...
let fold_constructors =
  fold_descr Env.fold_constructors (fun d -> d.cstr_name)
...
let report_error env ppf = function
  | Unbound_type_variable name ->
      ...
      fprintf
        ...
        name
        did_you_mean
        (fun () -> Misc.spellcheck names name )
  ...
  | Present_has_no_type l ->
      fprintf
        ...
        "@[<v>@[The constructor %s is missing from the upper bound@ \
         (between '<'@ and '>')@ of this polymorphic variant@ but is \
         present in@ its lower bound (after '>').@]@,@[Hint: Either add \
         `%s in the upper bound,@ or remove it@ from the lower bound.@]@]"
        l
        l
  ...
  | Cannot_quantify (name, v) ->
      fprintf
        ...
        name;
      if Btype.is_Tvar v
      then fprintf ppf "it escapes its scope"
      else if Btype.is_Tunivar v
      then fprintf ppf "it is already bound to another variable"
