...
let make_poly_univars vars = List.map (fun name -> name, newvar ~name ()) vars
  ...
  vars |>
    List.map
      (fun (name, ty1) ->
         ...
         | _ -> raise (Error (loc, env, Cannot_quantify (name, v)))
         end;
         v)
  ...
  vs |>
    List.iter
      (fun v ->
         ...
         | Tunivar name -> set_type_desc v (Tvar name)
         | _ -> assert false);
  vs
...
and transl_type_aux env policy styp =
    ...
    let ty =
      ...
      | Tvariant row ->
        ...
        let row =
          ...
          else if policy <> Univars then
            row
          else
            { row with  row_more = new_pre_univar () }
        in
        newty (Tvariant row)
      | Tobject (fi, _) ->
    ...
    let cty =
      try
        let t =
          ...
          with
          | Not_found -> instance (fst (TyVarMap.find alias !used_variables))
        in
      ...
      | Not_found ->
        ...
        let t = newvar () in
        used_variables := TyVarMap.add alias (t, styp.ptyp_loc) !used_variables;
        let ty = transl_type env policy st in
        (try unify_var env t ty.ctyp_type
         with
         | Unify err ->
  ...
  | Ptyp_variant (fields, closed, present) ->
    ...
    let add_typed_field loc l f =
      ...
      try
        ...
        and ty' = mkfield l f'
        in
        if is_equal env false [ ty ] [ ty' ] then
          ()
        else
          try unify env ty ty'
          with
      ...
      with
      | Not_found -> Hashtbl.add hfields h (l, f)
    in
    let add_field field =
      ...
      let rf_desc =
          ...
          let f =
            ...
            | _ ->
              (if List.length stl > 1 || c && stl <> [] then
                 raise
                   (Error (styp.ptyp_loc, env, Present_has_conjunction l.txt)));
              begin match tl with
              | [] -> Rpresent None
        ...
        | Rinherit sty ->
          ...
          let fl =
            match get_desc (expand_head env cty.ctyp_type), nm with
    ...
    begin match present with
    | None -> ()
    | Some present ->
      List.iter
        (fun l ->
           if not (List.mem_assoc l fields) then
             raise (Error (styp.ptyp_loc, env, Present_has_no_type l))) present
    end;
    let row =
      {
  ...
  | Ptyp_package (p, l) ->
    ...
    let ty =
      newty
        (Tpackage (path, List.map (fun (s, cty) -> s.txt, cty.ctyp_type) ptys))
    in
    ctyp
      (Ttyp_package
         {
...
and transl_fields env policy o fields =
  ...
  let add_typed_field loc l ty =
    try
      ...
      if is_equal env false [ ty ] [ ty' ] then
        ...
        try unify env ty ty'
        with
        | Unify _trace -> raise (Error (loc, env, Method_mismatch (l, ty, ty')))
    with
    | Not_found -> Hashtbl.add hfields l ty
  in
  let add_field { pof_desc; pof_loc; pof_attributes } =
    ...
    let of_desc =
      ...
      | Oinherit sty ->
        ...
        begin match get_desc t, nm with
        | Tobject (tf, _), _
          when
            begin match get_desc tf with
            | Tfield _ | Tnil -> true
            | _ -> false
            end
          ->
  ...
  in
  let ty =
    List.fold_left (fun ty (s, ty') -> newty (Tfield (s, Fpresent, ty', ty)))
      ty_init fields
  in
  ty, object_fields
    ...
    List.iter
      (function
       | loc, t1, t2 ->
         (try unify env t1 t2
          with
          | Unify err -> raise (Error (loc, env, Type_mismatch err)))) !r
  ...
  TyVarMap.iter
    (fun name p ->
       if TyVarMap.mem name !type_variables then
         used_variables := TyVarMap.add name p !used_variables) new_variables;
...
let report_error env ppf =
  ...
  | Unbound_type_variable name ->
    ...
    fprintf ppf "The type variable %s is unbound in this type declaration.@ %a"
      name did_you_mean (fun () -> Misc.spellcheck names name)
  ...
  | Type_arity_mismatch (lid, expected, provided) ->
    fprintf ppf
      "@[The type constructor %a@ expects %i argument(s),@ but is here \
       applied to %i argument(s)@]" longident lid expected provided
    ...
    wrap_printing_env ~error:true env
      (fun () ->
         ...
         fprintf ppf "@[<hov>%s %a@ %s@ %a@]"
           ...
           (tree_of_typexp Type ty) "which should be" !Oprint.out_type
  ...
  | Cannot_quantify (name, v) ->
    fprintf ppf "@[<hov>The universal type variable %a cannot be generalized:@ "
