...
type type_iterators =
  ...
  ; it_extension_constructor :
      type_iterators -> extension_constructor -> unit
  ...
  and it_class_type it = function
    ...
    | Cty_signature cs ->
        ...
        List.iter
          ...
          cs.csig_inher
    | Cty_arrow (_, ty, cty) ->
        it.it_type_expr it ty; it.it_class_type it cty
...
module For_copy : sig
  ...
  let dup_kind copy_scope r =
    ...
    if not (List.memq r copy_scope.new_kinds)
    then (
...
let type_iterators =
  let it_type_expr it ty =
    let ty = repr ty in
    if ty.level >= lowest_level
    then ( mark_type_node ty; it.it_do_type_expr it ty )
  in
