...
let rec row_field tag row =
  let rec find =
    ...
    | [] ->
      begin match get_desc row.row_more with
...
let hash_variant s =
  let accu = ref 0 in
  for i = 0 to String.length s - 1 do accu := 223 * !accu + Char.code s.[i] done;
  (* reduce to 31 bits *)
...
module For_copy : sig
  ...
  type copy_scope = { mutable saved_desc : (transient_expr * type_desc) list }
...
let rec find_expans priv p1 =
  ...
  | Mcons (priv', p2, _ty0, ty, _)
    when lte_public priv priv' && Path.same p1 p2 ->
    Some ty
...
let self_type cty = (signature_of_class_type cty).csig_self
let self_type_row cty = (signature_of_class_type cty).csig_self_row
(* Return the methods of a class signature *)
let methods sign = Meths.fold (fun name _ l -> name :: l) sign.csig_meths []
...
let concrete_methods sign =
  Meths.fold
    (fun name (_priv, vr, _ty) s ->
       match vr with
       | Virtual -> s
       | Concrete -> MethSet.add name s) sign.csig_meths MethSet.empty
...
let public_methods sign =
  Meths.fold
    (fun name (priv, _vr, _ty) l ->
       match priv with
       | Private -> l
       | Public -> name :: l) sign.csig_meths []

(* Return the instance variables of a class signature *)
let instance_vars sign = Vars.fold (fun name _ l -> name :: l) sign.csig_vars []
...
let concrete_instance_vars sign =
  Vars.fold
    (fun name (_mut, vr, _ty) s ->
       match vr with
       | Virtual -> s
       | Concrete -> VarSet.add name s) sign.csig_vars VarSet.empty
...
let logged_mark_node ty = set_level ty (pivot_level - get_level ty)

let try_mark_node ty =
  not_marked_node ty &&
    (flip_mark_node ty;
     true)

let try_logged_mark_node ty =
  not_marked_node ty &&
    (logged_mark_node ty;
     true)
...
let type_iterators =
  let it_type_expr it ty = if try_mark_node ty then it.it_do_type_expr it ty in
  { type_iterators with  it_type_expr }

(* Remove marks from a type. *)
let rec unmark_type ty =
  if get_level ty < lowest_level then
