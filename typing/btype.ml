...
module TransientTypePairs = Hashtbl.Make (struct
...
let row_of_type t =
  ...
  | Tobject (t, _) ->
      let rec get_row t =
        match get_desc t with Tfield (_, _, _, t) -> get_row t | _ -> t
      in
      get_row t
  | Tvariant row -> row_more row
  | _ -> t
...
let virtual_methods sign =
  Meths.fold
    (fun name (_priv, vr, _ty) l ->
      match vr with Virtual -> name :: l | Concrete -> l)
    sign.csig_meths []
...
let concrete_methods sign =
  Meths.fold
    (fun name (_priv, vr, _ty) s ->
      match vr with Virtual -> s | Concrete -> MethSet.add name s)
...
let public_methods sign =
  Meths.fold
    (fun name (priv, _vr, _ty) l ->
      match priv with Private -> l | Public -> name :: l)
    sign.csig_meths []
...
let concrete_instance_vars sign =
  Vars.fold
    (fun name (_mut, vr, _ty) s ->
      match vr with Virtual -> s | Concrete -> VarSet.add name s)
...
let cstr_type_path cstr =
  match get_desc cstr.cstr_res with Tconstr (p, _, _) -> p | _ -> assert false
