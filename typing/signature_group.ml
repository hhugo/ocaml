...
let next x =
  ...
  let rec not_in_group pre l =
    ...
    | None ->
      ...
      None
    | Some (elt, q) ->
      begin match recursive_sigitem elt.src with
...
let replace_in_place f sg =
  ...
  and core_group f ~before ~ghosts ~before_group current ~sg =
    ...
    | a :: q ->
      begin match f ~rec_group:q ~ghosts a.src with
      | Some (info, { ghosts; replace_by }) ->
        ...
        Some (info, sg)
      | None ->
        let before_group = List.rev_append a.post_ghosts (a.src :: before_group)
        in
