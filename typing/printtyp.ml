...
module Namespace = struct
  ...
  let best_class_namespace =
    function
    | Papply _ | Pdot _ -> Module
    | Pident c ->
      begin match location Class c with
      | Some _ -> Class
      | None -> Class_type
      end
end
...
module Naming_context = struct
  ...
  *)
  ...
  let with_arg id f = protect_refs [ R (fuzzy, S.add (Ident.name id) !fuzzy) ] f
  ...
  let with_ctx f =
    ...
    try_finally f ~always:(fun () -> Array.blit old 0 map 0 (Array.length map))
end
...
let non_shadowed_pervasive =
  ...
  | Pdot (Pident id, s) as path ->
    Ident.same id ident_stdlib &&
      begin match in_printing_env (Env.find_type_by_name (Lident s)) with
...
let rec rewrite_double_underscore_paths env p =
  ...
  | Pident id ->
    let name = Ident.name id in
    begin match find_double_underscore name with
    | None -> p
    | Some i ->
      let better_lid =
        Ldot
      ...
      in
      begin match Env.find_module_by_name better_lid env with
      | exception Not_found -> p
      | p', _ -> if module_path_is_an_alias_of env p' ~alias_of:p then p' else p
      end
    end
...
let rec tree_of_path namespace =
  function
...
and raw_type_desc ppf =
  ...
  | Tpackage (p, fl) ->
    fprintf ppf "@[<hov1>Tpackage(@,%a@,%a)@]" path p raw_type_list
      (List.map snd fl)

and raw_row_fixed ppf =
  function
  | None -> fprintf ppf "None"
...
let rec normalize_type_path ?(cache=false) env p =
  try
    ...
    | Tconstr (p1, tyl, _) ->
      if
        List.length params = List.length tyl && List.for_all2 eq_type params tyl
      then
  ...
  with
  | Not_found -> Env.normalize_type_path None env p, Id
...
let set_printing_env env =
  printing_env := env;
  if
    !Clflags.real_paths || !printing_env == Env.empty || same_printing_env env
  then
    ()
  else (* printf "Reset printing_map@."; *)
    (printing_old := env;
       ...
       Env.iter_types
         (fun p (p', _decl) ->
            ...
            if s1 = Id then
              ...
              | Not_found ->
                printing_map :=
                  Path.Map.add p1 (ref (Paths [ p ])) !printing_map) env
     in
     printing_cont := [ cont ])
  ...
  | Paths l ->
    ...
    List.iter
      (fun p ->
         ...
         | _ -> if is_unambiguous p !printing_env then r := Best p)(* else Format.eprintf "%a ignored as ambiguous@." path p *)
       l;
    get_best_path r
...
let best_type_path p =
  if !printing_env == Env.empty then
    p, Id
  else if !Clflags.real_paths then
    p, Id
  else
...
(* We map from types to names, but not directly; we also store a substitution,
  ...
  let has_name ty = List.mem_assq (substitute ty) !names
  ...
  let rec new_weak_name ty () =
    ...
    incr weak_counter;
    if name_is_already_used name then
      new_weak_name ty ()
    else
  ...
  let name_of_type name_generator t =
    ...
    with
    | Not_found ->
      (try TransientTypeMap.find t !weak_var_map
       with
       | Not_found ->
         let name =
           match t.desc with
  ...
  row.row_name <> None &&
    List.for_all
      (fun (_, f) ->
         ...
         | _ -> true) row.row_fields

let rec mark_loops_rec visited ty =
  let px = proxy ty in
  if List.memq px visited && aliasable ty then
    add_alias_proxy px
  else
    ...
    | Tvariant row ->
      if List.memq px !visited_objects then
        add_alias_proxy px
      else
           ...
           List.iter
             (fun (_, kind, ty) ->
                if field_kind_repr kind = Fpresent then
                  mark_loops_rec visited ty) fields
...
let rec tree_of_typexp mode ty =
  ...
  if Names.has_name px && not (List.memq px !delayed) then
    ...
    let name =
      Names.name_of_type
        (if mark then Names.new_weak_name ty else Names.new_name) px
    in
    Otyp_var (mark, name)
  else
    let pr_typ () =
      ...
      | Ttuple tyl -> Otyp_tuple (tree_of_typlist mode tyl)
      | Tconstr (p, tyl, _abbrev) ->
        let (p', s) = best_type_path p in
        let tyl' = apply_subst s tyl in
        if is_nth s && not (tyl' = []) then
          tree_of_typexp mode (List.hd tyl')
        else
      ...
      | Tvariant row ->
        ...
        | Some (p, tyl) when namable_row row ->
          ...
          if row.row_closed && all_present then
            ...
            let tags = if all_present then None else Some (List.map fst present)
            in
            Otyp_variant (non_gen, Ovar_typ out_variant, row.row_closed, tags)
        | _ ->
          let non_gen =
            not (row.row_closed && all_present) &&
              is_non_gen mode (Transient_expr.type_expr px)
          in
      ...
      | Tobject (fi, nm) -> tree_of_typobject mode fi !nm
      | Tnil | Tfield _ -> tree_of_typobject mode ty None
      | Tsubst _ ->
        (* This case should only happen when debugging the compiler *)
        Otyp_stuff "<Tsubst>"
      | Tlink _ -> fatal_error "Printtyp.tree_of_typexp"
      | Tpoly (ty, []) -> tree_of_typexp mode ty
      | Tpoly (ty, tyl) ->
        (*let print_names () =
        ...
        if tyl = [] then
          ...
          (List.iter add_delayed tyl;
           ...
           delayed := old_delayed;
           tr)
      | Tunivar _ -> Otyp_var (false, Names.name_of_type Names.new_name tty)
      | Tpackage (p, fl) ->
        let fl =
          List.map
            (fun (li, ty) ->
               String.concat "." (Longident.flatten li), tree_of_typexp mode ty)
            fl
        in
        Otyp_module (tree_of_path Module_type p, fl)
    in
    ((if List.memq px !delayed then delayed := List.filter ((!=) px) !delayed);
     if is_aliased_proxy px && aliasable ty then
       (Names.check_name_of_type px;
        Otyp_alias (pr_typ (), Names.name_of_type Names.new_name px))
     else
       pr_typ ())
...
and tree_of_row_field mode (l, f) =
  ...
  | Reither (c, tyl, _, _) ->
    if c (* contradiction: constant constructor with an argument *) then
      l, true, tree_of_typlist mode tyl
    else
      l, false, tree_of_typlist mode tyl
  | Rabsent -> l, false, [] (* actually, an error *)

and tree_of_typlist mode tyl = List.map (tree_of_typexp mode) tyl
...
and tree_of_typobject mode fi nm =
  ...
  | None ->
    let pr_fields fi =
      ...
      in
      tree_of_typfields mode rest sorted_fields
    in
...
and tree_of_typfields mode rest =
  function
  | [] ->
    let rest =
...
let typexp mode ppf ty = !Oprint.out_type ppf (tree_of_typexp mode ty)
...
let filter_params tyl =
  let params =
    List.fold_left
      (fun tyl ty ->
         if List.exists (eq_type ty) tyl then
           newty2 ~level:generic_level (Ttuple [ ty ]) :: tyl
         else
           ty :: tyl)
      (* Two parameters might be identical due to a constraint but we need to
         ...
         We use [Ttuple [ty]] because it is printed as [ty]. *)(* Replacing fold_left by fold_right does not work! *)
       [] tyl
  in
  List.rev params
...
let rec tree_of_type_decl id decl =
  ...
  | Some ty ->
    ...
    List.iter
      (fun ty ->
         if get_desc ty = Tvar (Some "_") && List.exists (eq_type ty) vars then
           set_type_desc ty (Tvar None)) params
  | None -> ()
  end;
  ...
  | Type_variant (cstrs, _rep) ->
    List.iter
      (fun c ->
         ...
         Option.iter mark_loops c.cd_res) cstrs
      ...
      List.map2
        (fun ty v ->
           ...
           if abstr || not is_var then
             ...
             NoVariance, NoInjectivity) decl.type_params decl.type_variance
    in
    Ident.name id,
    List.map2 (fun ty cocn -> type_param (tree_of_typexp Type ty), cocn) params
      vari
  in
  ...
  let (ty, priv, unboxed) =
    ...
    | Type_abstract ->
      ...
      | Some ty -> tree_of_typexp Type ty, decl.type_private, false
      end
    ...
    | Type_open -> tree_of_manifest Otyp_open, decl.type_private, false
  in
  {
    otype_name = name;
    otype_params = args;
...
let rec prepare_class_type params =
  function
  | Cty_constr (_p, tyl, cty) ->
    let row = Btype.self_type_row cty in
    if
      List.memq (proxy row) !visited_objects ||
        not (List.for_all is_Tvar params) || List.exists (deep_occur row) tyl
    then
...
let rec tree_of_class_type mode params =
  ...
  | Cty_signature sign ->
    ...
    let csil = [] in
    let csil =
      List.fold_left (fun csil (ty1, ty2) -> Ocsg_constraint (ty1, ty2) :: csil)
    ...
    let csil =
      List.fold_left (fun csil meth -> tree_of_method mode meth :: csil) csil
        all_meths
    in
    Octy_signature (self_ty, List.rev csil)
  | Cty_arrow (l, ty, cty) ->
    let lab = if !print_labels || is_optional l then string_of_label l else ""
    in
    let tr =
      if is_optional l then
        match get_desc ty with
...
let tree_of_class_param param variance =
  begin match tree_of_typexp Type_scheme param with
  | Otyp_var (_, s) -> s
  | _ -> "?"
  end,
  (if is_Tvar param then Asttypes.(NoVariance, NoInjectivity) else variance)
...
let class_variance =
  ...
  List.map
    (fun v ->
       (if not (mem May_pos v) then
          Contravariant
        else if not (mem May_neg v) then
          Covariant
        else
          NoVariance),
       NoInjectivity)
...
let tree_of_cltype_declaration id cl rs =
  ...
  let has_virtual_meths =
    Meths.fold (fun _ (_, vr, _) b -> vr = Virtual || b) sign.csig_meths false
  in
  Osig_class_type
    (has_virtual_vars || has_virtual_meths,
     Ident.name id,
...
let wrap_env fenv ftree arg =
  ...
  (if !Clflags.real_paths || same_printing_env env then
   ...
   else (* we restore the snapshotted cache before calling set_printing_env *)
     (printing_old := env;
...
let add_sigitem env x = Env.add_signature (Signature_group.flatten x) env
...
let rec tree_of_modtype ?(ellipsis=false) =
  ...
  | Mty_functor (param, ty_res) ->
    let (param, env) = tree_of_functor_parameter param in
...
and tree_of_functor_parameter =
  ...
  | Named (param, ty_arg) ->
    let (name, env) =
      ...
      | Some id ->
        Some (Ident.name id), Env.add_module ~arg:true id Mp_present ty_arg
    in
...
and tree_of_signature sg =
  wrap_env (fun env -> env)
    (fun sg ->
       ...
       List.concat_map (fun (_env, l) -> List.map snd l) tree_groups) sg
...
and trees_of_recursive_sigitem_group
    env (syntactic_group : Signature_group.rec_group)
=
...
and tree_of_sigitem =
  ...
  | Sig_class_type (id, decl, rs, _) -> tree_of_cltype_declaration id decl rs
...
let rec functor_parameters ~sep custom_printer =
  function
  | [] -> ignore
  | [ id, param ] ->
    Format.dprintf "%t%t" (custom_printer param)
      (functor_param ~sep ~custom_printer id [])
  | (id, param) :: q ->
    Format.dprintf "%t%a%t" (custom_printer param) sep ()
  ...
  eq_type t t' ||
    ...
    | Tconstr (p, tl, _), Tconstr (p', tl', _) ->
      ...
      | (Id | Map _), (Id | Map _) when Path.same p1 p2 ->
        ...
        in
        List.length tl = List.length tl' && List.for_all2 eq_type tl tl'
      | _ -> false
      end
    | _ -> false
...
let trees_of_type_path_expansion (tp, tp') =
  if Path.same tp tp' then
    Same (tree_of_path Type tp)
  else
    Diff (tree_of_path Type tp, tree_of_path Type tp')

let type_path_expansion ppf =
  function
...
let diff_printing_status
    Errortrace.
    { got = { ty = t1; expanded = t1' }; expected = { ty = t2; expanded = t2' }
    }
=
  if
    is_constr_row ~allow_ident:true t1' || is_constr_row ~allow_ident:true t2'
  then
    Discard
...
let prepare_trace f tr = prepare_any_trace printing_status (Errortrace.map f tr)

(** Keep elements that are not [Diff _ ] and take the decision
    for the last element, require a prepared trace *)
let rec filter_trace (trace_format : 'variety trace_format) keep_last :
  ('a, 'variety) Errortrace.t -> _
=
  function
  | [] -> []
...
let explain_fixed_row_case ppf =
  function
...
let explain_fixed_row pos expl =
  ...
  | Reified p ->
    dprintf "The %a variant type is bound to %t" Errortrace.print_pos pos
      (print_path p)
  | Rigid -> ignore

let explain_variant (type variety) : variety Errortrace.variant -> _ =
  function
  (* Common *)
  ...
  | Errortrace.No_tags (pos, fields) ->
    Some
      (dprintf "@,@[The %a variant type does not allow tag(s)@ @[<hov>%a@]@]"
         Errortrace.print_pos pos print_tags (List.map fst fields))
    ...
    Some
      (dprintf
         "@,\
          ...
          but not in the %a@]" s Errortrace.print_pos
         (Errortrace.swap_position pos) Errortrace.print_pos pos)
  | Errortrace.Openness pos ->
    Some
      (dprintf "@,The %a variant type is open and the %a is not"
         Errortrace.print_pos pos Errortrace.print_pos
         (Errortrace.swap_position pos))

let explain_escape pre =
  function
  | Errortrace.Univ u ->
    Some
...
let explain_object (type variety) : variety Errortrace.obj -> _ =
  function
  | Errortrace.Missing_field (pos, f) ->
    Some
      (dprintf "@,@[The %a object type has no method %s@]" Errortrace.print_pos
         pos f)
...
let prepare_expansion_head empty_tr =
  function
  | Errortrace.Diff d ->
    Some (Errortrace.map_diff (may_prepare_expansion empty_tr) d)
  | _ -> None

let head_error_printer mode txt_got txt_but =
  function
  | None -> ignore
  | Some d ->
    let d = Errortrace.map_diff (trees_of_type_expansion mode) d in
    dprintf "%t@;<1 2>%a@ %t@;<1 2>%a" txt_got type_expansion d.Errortrace.got
      txt_but type_expansion d.Errortrace.expected

let warn_on_missing_defs env ppf =
  function
  | None -> ()
  | Some
      Errortrace.
      { got = { ty = te1; expanded = _ }; expected = { ty = te2; expanded = _ }
      }
    ->
...
let error trace_format mode subst env tr txt1 ppf txt2 ty_expect_explanation =
  ...
  let tr =
    prepare_trace
      (fun ty_exp ->
         Errortrace.(
           { ty_exp with  expanded = hide_variant_name ty_exp.expanded }
         )) tr
  in
  ...
  | elt :: tr ->
    (try
       ...
       fprintf ppf "@[<v>@[%t%t@]%a%t@]" head_error ty_expect_explanation
         (trace false (incompatibility_phrase trace_format)) tr (explain mis);
...
let report_error
    trace_format ppf mode env tr ?(subst=[])
...
=
  report_error Equality ppf mode env ~subst ?type_expected_explanation:None
    trace

let report_moregen_error ppf mode env ({ trace } : Errortrace.moregen_error) =
  report_error Moregen ppf mode env ?subst:None ?type_expected_explanation:None
    trace

let report_comparison_error ppf mode env =
  function
...
(* There's a frustrating amount of code duplication between this module and
  ...
  let rec filter_subtype_trace keep_last =
    function
    | [] -> []
  ...
  let unification_get_diff =
    function
    | Errortrace.Diff diff ->
      Some (Errortrace.map_diff (trees_of_type_expansion Type) diff)
    | _ -> None
  
  let subtype_get_diff =
    function
    | Errortrace.Subtype.Diff diff ->
      Some (Errortrace.map_diff (trees_of_type_expansion Type) diff)
  
  let report_error
      ppf env Errortrace.Subtype.{ trace = tr_sub; unification_trace = tr_unif }
      txt1
  =
    wrap_printing_env ~error:true env
      (fun () ->
         ...
         if tr_unif = [] then
           ...
           fprintf ppf "%a%t%t@]"
             ...
             (explain mis) Conflicts.print_explanations)
end
    ...
    (fun () ->
       ...
       | [ tp ] ->
         fprintf ppf "@[%t@;<1 2>%a@ %t@;<1 2>%a@]" txt1 type_path_expansion
           (trees_of_type_path_expansion tp) txt3 type_path_expansion tp0
       | _ ->
         fprintf ppf "@[%t@;<1 2>@[<hv>%a@]@ %t@;<1 2>%a@]" txt2 type_path_list
           (List.map trees_of_type_path_expansion tpl) txt3 type_path_expansion
           tp0)
