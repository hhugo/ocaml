...
module Names : sig
  ...
  let add_subst subst =
    name_subst :=
      List.map
        (fun (t1, t2) -> (Transient_expr.repr t1, Transient_expr.repr t2))
        subst
      @ !name_subst
...
let rec mark_loops_rec visited ty =
  let px = proxy ty in
  if List.memq px visited && aliasable ty then add_alias_proxy px
  else
  ...
  if Names.has_name px && not (List.memq px !delayed) then
    ...
    let pr_typ () =
      ...
      | Tconstr (p, tyl, _abbrev) ->
          ...
          if is_nth s && not (tyl' = []) then tree_of_typexp mode (List.hd tyl')
      ...
      | Tpoly (ty, tyl) ->
          (*let print_names () =
            ...
            prerr_string "; " in *)
          if tyl = [] then tree_of_typexp mode ty
          else
...
let rec tree_of_type_decl id decl =
  ...
  let type_defined decl =
    ...
    ( Ident.name id,
      List.map2
        (fun ty cocn -> (type_param (tree_of_typexp Type ty), cocn))
        params vari )
  in
...
let rec tree_of_class_type mode params = function
  | Cty_constr (p', tyl, cty) ->
      let row = Btype.self_type_row cty in
      if
        List.memq (proxy row) !visited_objects
        || not (List.for_all is_Tvar params)
      then tree_of_class_type mode params cty
      else
...
let with_hidden_items ids f =
  ...
  in
  if not !Clflags.real_paths then with_hidden_in_printing_env ids f
...
and functor_param ~sep ~custom_printer id q =
  ...
  | Some id ->
      Naming_context.with_arg id (fun () ->
          functor_parameters ~sep custom_printer q)
...
let diff_printing_status
    Errortrace.
      {
        got = { ty = t1; expanded = t1' };
        expected = { ty = t2; expanded = t2' };
      } =
  if is_constr_row ~allow_ident:true t1' || is_constr_row ~allow_ident:true t2'
  then Discard
  ...
  | elt :: tr -> (
      try
        ...
        fprintf ppf "@[<v>@[%t%t@]%a%t@]" head_error ty_expect_explanation
          (trace false (incompatibility_phrase trace_format))
          tr (explain mis);
...
let report_error trace_format ppf mode env tr ?(subst = [])
...
let report_equality_error ppf mode env
    ({ subst; trace } : Errortrace.equality_error) =
...
module Subtype = struct
  ...
  let report_error ppf env
      Errortrace.Subtype.{ trace = tr_sub; unification_trace = tr_unif } txt1 =
    wrap_printing_env ~error:true env (fun () ->
        reset ();
