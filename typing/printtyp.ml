...
module Namespace = struct
  ...
  let lookup =
    let to_lookup f lid =
      fst @@ f ?loc:None ?mark:(Some false) (Lident lid) !printing_env
    in
    function
    | Type ->
        fun id ->
...
module Conflicts = struct
  ...
  let explain namespace n id =
    ...
    if not (M.mem name !explanations)
    then
  ...
  let pp_explanation ppf r =
    Format.fprintf
      ppf
      "@[<v 2>%a:@,Definition of %s %s@]"
      Location.print_loc
      r.location
      (Namespace.show r.kind)
      r.name
  ...
  let print ppf =
    ...
    let l =
      List.filter
        (* remove toplevel locations, since they are too imprecise *)
          (fun a ->
          a.location.Location.loc_start.Lexing.pos_fname <> "//toplevel//"
          )
        (take ())
    in
    match l with [] -> () | l -> Format.fprintf ppf "%t%a" sep pp l
...
module Naming_context = struct
  ...
  *)
  ...
  let ident_name_simple namespace id =
    if not !enabled
    then Out_name.create (Ident.name id)
    else
      ...
      | Uniquely_associated_to (id', r) ->
          ...
          Out_name.set r (human_unique hid' id');
          List.iter
            (fun (id, hid) -> Conflicts.explain namespace hid id)
            [id, hid; id', hid'];
          set namespace @@ M.add name (Need_unique_name m) (get namespace);
          Out_name.create (human_unique hid id)
...
let rec rewrite_double_underscore_paths env p =
  ...
  | Pident id -> (
      ...
      | Some i -> (
          ...
          | p' ->
              if module_path_is_an_alias_of env p' ~alias_of:p
              then p'
              else p ) )
      ...
      Env.iter_types
        (fun p (p', _decl) ->
          ...
          then
            ...
            with Not_found ->
              printing_map :=
                Path.Map.add p1 (ref (Paths [p])) !printing_map )
        env
    in
    printing_cont := [cont] )
...
let wrap_printing_env ~error env f =
  if error
  then Env.without_cmis (wrap_printing_env env) f
  ...
  then add_alias px
    ...
    | Tvariant row -> (
        ...
        then add_alias px
          ...
          then visited_objects := px :: !visited_objects;
          match row.row_name with
          ...
          | None ->
              ...
              List.iter
                (fun (_, kind, ty) ->
                  ...
                  then mark_loops_rec visited ty )
                fields
...
let reset_except_context () = reset_names (); reset_loop_marks ()

let reset () =
  reset_naming_context (); Conflicts.reset (); reset_except_context ()

let reset_and_mark_loops ty = reset_except_context (); mark_loops ty
...
let rec tree_of_typexp sch ty =
  let ty = repr ty in
  let px = proxy ty in
  if List.mem_assq px !names && not (List.memq px !delayed)
  then
    ...
    let pr_typ () =
      ...
      | Tarrow (l, ty1, ty2, _) ->
          let pr_arrow l ty1 ty2 =
            let lab =
              if !print_labels || is_optional l
              then string_of_label l
              else ""
            in
            let t1 =
              if is_optional l
      ...
      | Tconstr (p, tyl, _abbrev) ->
          ...
          if is_nth s && not (tyl' = [])
          then tree_of_typexp sch (List.hd tyl')
      ...
      | Tvariant row -> (
          ...
          | _ ->
              ...
              in
              Otyp_variant
                (non_gen, Ovar_fields fields, row.row_closed, tags) )
    ...
    then delayed := List.filter (( != ) px) !delayed;
  ...
  let params =
    List.fold_left
      (fun tyl ty ->
        let ty = repr ty in
        if List.memq ty tyl
        then Btype.newgenty (Tsubst ty) :: tyl
        else ty :: tyl )
      []
      tyl
  in
...
let rec tree_of_class_type sch params = function
  ...
  | Cty_signature sign ->
      ...
      let self_ty =
        if is_aliased sty
        then Some (Otyp_var (false, name_of_type new_name (proxy sty)))
        else None
      in
      let fields, _ =
        Ctype.flatten_fields (Ctype.object_fields sign.csig_self)
...
let recursive_sigitem = function
  ...
  | Sig_type (id, _, rs, _) | Sig_module (id, _, _, rs, _) ->
      Some (id, rs, 0)
  | _ -> None
...
let protect_rec_items items =
  let rec get_ids recs = function
    ...
    | item :: rem -> (
      ...
      | Some (id, r, k) when r = recs ->
          id :: get_ids Trec_next (skip k rem)
      | _ -> [] )
  in
...
let rec tree_of_modtype ?(ellipsis = false) = function
  ...
  | Mty_functor (param, ty_arg, ty_res) ->
      let res =
        ...
        | Some mty ->
            wrap_env
              (Env.add_module ~arg:true param Mp_present mty)
              (tree_of_modtype ~ellipsis)
              ty_res
      in
      Omty_functor
        ( Ident.name param
        , may_map (tree_of_modtype ~ellipsis:false) ty_arg
        , res )
...
let refresh_weak () =
  let refresh t name (m, s) =
    ...
    then TypeMap.add t name m, String.Set.add name s
    else m, s
  in
  let m, s =
...
let trees_of_type_path_expansion (tp, tp') =
  if Path.same tp tp'
  then Same (tree_of_path Type tp)
...
let printing_status = function
  | Trace.(Diff {got = t1, t1'; expected = t2, t2'}) ->
      ...
      then Discard
      else if same_path t1 t1' && same_path t2 t2'
      then Optional_refinement
      else Keep
  | _ -> Keep
...
let hide_variant_name t =
  ...
  | {desc = Tvariant row} as t when (row_repr row).row_name <> None ->
      newty2
        ...
        (Tvariant
           { (row_repr row) with
             row_name = None; row_more = newvar2 (row_more row).level })
  | _ -> t
...
let may_prepare_expansion compact (t, t') =
  ...
  | (Tvariant _ | Tobject _) when compact -> mark_loops t; t, t
...
let unifiable env ty1 ty2 =
  ...
  let res = try Ctype.unify env ty1 ty2; true with Unify _ -> false in
  Btype.backtrack snap; res
...
let explain_escape intro prev ctx e =
  ...
  | Trace.Univ u ->
      Some
        (dprintf
           "%t@,The universal variable %a would escape its scope"
           pre
           type_expr
           u)
  | Trace.Constructor p ->
  ...
  | Trace.Equation (_, t) ->
      Some
        (dprintf
           "%t @,@[<hov>This instance of %a is ambiguous:@ %s@]"
           pre
           type_expr
           t
...
let explain_object = function
  ...
  | Trace.Missing_field (pos, f) ->
      Some
        (dprintf "@,@[The %a object type has no method %s@]" print_pos pos f)
  | Trace.Abstract_row pos ->
      Some
        (dprintf
           "@,@[The %a object type has an abstract row, it cannot be \
            closed@]"
           print_pos
           pos)

let explanation intro prev env = function
  | Trace.Diff {Trace.got = _, s; expected = _, t} ->
...
let explain mis ppf =
  match mis with None -> () | Some explain -> explain ppf
...
let report_unification_error ppf env tr
  ...
  wrap_printing_env
    env
    (fun () ->
      unification_error env tr txt1 ppf txt2 type_expected_explanation )
    ~error:true
