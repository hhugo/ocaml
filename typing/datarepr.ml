...
let constructor_descrs ~current_unit ty_path decl cstrs rep =
  ...
  List.iter
    ...
    cstrs;
  let rec describe_constructors idx_const idx_nonconst =
    function
    | [] -> []
...
let rec find_constr tag num_const num_nonconst =
  ...
  | ({ cd_args = Cstr_tuple []; _ } as c) :: rem ->
    if tag = Cstr_constant num_const then
      c
    else
      find_constr tag (num_const + 1) num_nonconst rem
  | c :: rem ->
    if tag = Cstr_block num_nonconst || tag = Cstr_unboxed then
      c
    else
...
let labels_of_type ty_path decl =
  ...
  | Type_record (labels, rep) ->
    label_descrs (newgenconstr ty_path decl.type_params) labels rep
      decl.type_private
