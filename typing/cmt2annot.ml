...
let rec iterator ~scope rebuild_env =
  ...
  and expr sub exp =
    ...
    | Texp_let (Recursive, bindings, _) -> bind_bindings exp.exp_loc bindings
    ...
    | Texp_letmodule (_, modname, _, _, body) ->
        Stypes.record
          (Stypes.An_ident
             ( modname.loc,
               Option.value ~default:"_" modname.txt,
  ...
  let structure_item_rem sub str rem =
    ...
    | Tstr_module mb ->
        record_module_binding { scope with Location.loc_start = loc.loc_end } mb
    | Tstr_recmodule mbs ->
        List.iter
  ...
  let structure sub l =
    let rec loop = function
      | str :: rem ->
          structure_item_rem sub str rem;
          loop rem
      | [] -> ()
    in
    loop l.str_items
...
let gen_annot target_filename ~sourcefile ~use_summaries annots =
  let open Cmt_format in
  let scope =
    match sourcefile with None -> Location.none | Some s -> Location.in_file s
  in
