...
let rec scrape_lazy env mty =
  ...
  match mty with
  | MtyL_ident p ->
    (try scrape_lazy env (Env.find_modtype_expansion_lazy p env)
     with
     | Not_found -> mty)
  | _ -> mty
...
let scrape env mty =
  match mty with
  | Mty_ident p -> Subst.Lazy.force_modtype (scrape_lazy env (MtyL_ident p))
  | _ -> mty
...
and strengthen_lazy_sig' ~aliasable env sg p =
  ...
  | SigL_module (id, pres, md, rs, vis) :: rem ->
    let str = strengthen_lazy_decl ~aliasable env md (Pdot (p, Ident.name id))
    in
    let env =
  ...
  | SigL_modtype (id, decl, vis) :: rem ->
    ...
    SigL_modtype (id, newdecl, vis) :: strengthen_lazy_sig' ~aliasable env rem p
...
let strengthen_decl ~aliasable env md p =
  let md = strengthen_lazy_decl ~aliasable env (Subst.Lazy.of_module_decl md) p
  in
  ...
  | Mty_ident p ->
    ...
    | Some id ->
      let expansion =
        ...
        with
        | Not_found -> raise (Ctype.Nondep_cannot_erase id)
      in
...
and nondep_sig_item env va ids =
  function
  | Sig_value (id, d, vis) ->
    Sig_value
  ...
  | Sig_modtype (id, d, vis) ->
    (try Sig_modtype (id, nondep_modtype_decl env ids d, vis)
     with
     | Ctype.Nondep_cannot_erase _ as exn ->
       begin match va with
       | Co ->
         Sig_modtype
           (id,
  ...
  | None ->
    ...
    | orig_decl ->
      if decl.type_arity <> orig_decl.type_arity then
        ...
        begin match Ctype.mcomp env orig_ty new_ty with
        ...
        | () ->
          let orig_ty = Btype.newgenty (Tconstr (p, decl.type_params, ref Mnil))
          in
...
and type_paths_sig env p sg =
  ...
  | Sig_module (id, pres, md, _, _) :: rem ->
    type_paths env (Pdot (p, Ident.name id)) md.md_type @
      type_paths_sig (Env.add_module_declaration ~check:false id pres md env) p
        rem
...
let rec no_code_needed_mod env pres mty =
  ...
  | Mp_absent -> true
  | Mp_present ->
    begin match scrape env mty with
    | Mty_ident _ -> false
...
let scrape_for_type_of ~remove_aliases env mty =
  if remove_aliases then
    ...
    let (_, mty) =
      remove_aliases_mty env { modified = false; exclude; scrape } Mp_present
        mty
    in
    mty
  else
