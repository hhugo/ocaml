        ...
        | _ ->
            let manif =
              Some
                (Btype.newgenty
                   (Tconstr
                      (Pdot (p, Ident.name id), decl.type_params, ref Mnil)))
            in
...
let rec nondep_mty_with_presence env va ids pres mty =
  ...
  | Mty_functor (param, arg, res) ->
      ...
      let mty =
        Mty_functor
          ( param
          , Misc.may_map (nondep_mty env var_inv ids) arg
          , nondep_mty
              (Env.add_module
                 ~arg:true
                 param
...
and nondep_sig_item env va ids = function
  ...
  | Sig_typext (id, ext, es, vis) ->
      Sig_typext
        (id, Ctype.nondep_extension_constructor env ids ext, es, vis)
...
and nondep_sig env va ids sg = List.map (nondep_sig_item env va ids) sg
    ...
    try
      ...
      then decl
        ...
        let new_ty =
          Ctype.reify_univars
            (Btype.newgenty
               (Tconstr (Pident id, decl.type_params, ref Mnil)))
        in
...
and enrich_item env p = function
  ...
  | Sig_module (id, pres, md, rs, priv) ->
      Sig_module
        ...
        , { md with
            md_type =
              enrich_modtype env (Pdot (p, Ident.name id)) md.md_type }
        , rs
        , priv )
  | item -> item
