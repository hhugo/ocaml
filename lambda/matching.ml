...
module MayCompat =
  Parmatch.Compat
  (struct let equal = Types.may_equal_constr end)
...
module Half_simple : sig
  ...
  let of_clause ~arg cl =
    let rec aux ((p, patl), action : General.clause) : clause =
      ...
      | `Alias (p, id, _) ->
        aux ((General.view p, patl), bind_alias p id ~arg ~action)
      ...
      | `Or _ ->
        ...
        begin match orpat.pat_desc with
...
module Simple : sig
  ...
  *)
  let explode_or_pat
      ~arg (p : Half_simple.pattern) ~mk_action ~patbound_action_vars
    :
    (pattern * lambda) list
  =
    let rec explode p aliases rem =
      ...
      | `Var (id, str) ->
        explode { p with  pat_desc = `Alias (Patterns.omega, id, str) } aliases
          rem
      | #view as view ->
        (* We are doing two things here:
          ...
          *)
        let rec fresh_clause arg_id action_vars renaming_env =
          function
          | [] ->
          ...
          | pat_id :: rem_vars ->
            if not (List.mem pat_id aliases) then
              ...
              begin match arg_id, arg with
              | Some id, _ | None, Lvar id ->
  ...
  let lub p ctx =
    List.filter_map
      (fun { Row.left; right } ->
         match right with
         | q :: rem ->
           (try Some { Row.left; right = lub p q :: rem }
            with
            | Empty -> None)
         | _ -> fatal_error "Matching.Context.lub") ctx
...
let flatten_matrix size pss =
  List.fold_right
    (fun ps r ->
       ...
       | _ -> fatal_error "Matching.flatten_matrix") pss []
...
module Default_environment : sig
  ...
  let specialize_matrix arity matcher pss =
    ...
    *)
    and filter_one p ps = filter_rec [ p, ps ]
    and filter_rec_or p1 p2 ps rem =
      ...
      | 0 ->
        ...
        begin match filter_one p1 ps with
      ...
      | 1 ->
        ...
        begin match filter_one p1 ps, filter_one p2 ps with
  ...
  let specialize_ arity matcher env =
    let rec make_rec =
      ...
      | (pss, i) :: rem ->
        ...
        begin match specialize_matrix arity matcher pss with
        | [] -> make_rec rem
        | [] :: _ -> [ [ [] ], i ]
  ...
  let pop_compat p def =
    let compat_matcher q rem =
      if may_compat p (General.erase q) then rem else raise NoMatch
    in
...
module Jumps : sig
  ...
  let singleton i ctx = if Context.is_empty ctx then [] else [ i, ctx ]
  ...
  let add i ctx jumps =
    ...
    in
    if Context.is_empty ctx then jumps else add jumps
...
let same_actions =
  ...
  | [ _, act ] -> Some act
  | (_, act0) :: rem ->
    begin match make_key act0 with
    | None -> None
    | key0_opt ->
      let same_act (_, act) = make_key act = key0_opt in
      if List.for_all same_act rem then Some act0 else None
    end
  ...
  List.for_all
    (fun ((q, qs), act_q) ->
       same_actions act_p act_q ||
         not (may_compats (General.erase p :: ps) (General.erase q :: qs))) l

let half_simplify_nonempty
    ~arg (cls : Typedtree.pattern Non_empty_row.t clause)
  :
  Half_simple.clause
=
  cls |> map_on_row (Non_empty_row.map_first General.view) |>
    Half_simple.of_clause ~arg

let half_simplify_clause ~arg (cls : Typedtree.pattern list clause) =
  cls |> map_on_row Non_empty_row.of_initial |> half_simplify_nonempty ~arg
...
let rec what_is_cases ~skip_any cases =
  ...
  | ((p, _), _) :: rem ->
    ...
    begin match head.pat_desc with
...
let pm_free_variables { cases } =
  List.fold_right (fun (_, act) r -> Ident.Set.union (free_variables act) r)
      ...
      | ((q, qs), act_q as cl) :: rem ->
        ...
        if not (is_or q) || disjoint p q then
          ...
          Typedtree.pat_bound_idents p = [] &&
            Typedtree.pat_bound_idents q = [] && equiv_pat p q
        then
...
let rec split_or ~arg (cls : Half_simple.clause list) args def =
  let rec do_split (rev_before : Simple.clause list) rev_ors rev_no =
    function
    ...
    | ((p, ps), act as cl) :: rem ->
      ...
      | #Simple.view as view when safe_before cl rev_ors ->
        do_split ((({ p with  pat_desc = view }, ps), act) :: rev_before)
          rev_ors rev_no rem
      | _ ->
        let (rev_ors, rev_no) =
...
and split_no_or cls args def k =
  ...
  let rec split (cls : Simple.clause list) =
    ...
    collect discr [] [] cls
  and collect group_discr rev_yes rev_no =
    function
    | [ (p, ps), _ as cl ]
    ...
    | [] ->
      let yes = List.rev rev_yes
      and no = List.rev rev_no
      in
  ...
  and insert_split group_discr yes no def k =
    ...
    | _ ->
      ...
      let idef = next_raise_count () in
      precompile_group args yes (Default_environment.cons matrix idef def)
        ((idef, next) :: nexts)
    ...
    | _ ->
      ...
      let var_cls =
        List.map
          (fun ((p, ps), act) ->
             ...
             half_simplify_clause ~arg:(fst arg) (ps, act)) cls
      and var_def = Default_environment.pop_column def
      in
      ...
      | _ ->
        ...
        let rebuild_default nexts def =
          ...
          List.fold_right
            (fun (e, pmh) ->
               Default_environment.cons (add_omega_column (rebuild_matrix pmh))
                 e) nexts def
        in
        ...
        and rnexts = rebuild_nexts nexts k
        in
        rfirst, rnexts
      end
    end
...
and precompile_or ~arg (cls : Simple.clause list) ors args def k =
  ...
  *)
  let rec do_cases =
    ...
    | ((p, patl), action) :: rem ->
      ...
      | `Or _ ->
        ...
        in
        let new_cases =
          Simple.explode_or_pat ~arg p ~mk_action:mk_new_action
            ~patbound_action_vars:(List.map fst patbound_action_vars) |>
            List.map (fun (p, act) -> (p, new_patl), act)
        in
        let handler =
          {
            provenance = [ [ orp ] ];
  ...
  and body = { cases = cls @ cases; args; default = def }
  in
  {
...
let divide
    get_expr_args eq_key get_key get_pat_args ctx
    (pm : Simple.clause pattern_matching)
=
  let add ((p, patl), action) division =
    let ph = Simple.head p in
    let p = General.erase p in
    add_in_div (make_matching get_expr_args ph pm.default ctx) eq_key
      (get_key p) (get_pat_args p patl, action) division
  in
...
let divide_line
    make_ctx get_expr_args get_pat_args discr ctx
    (pm : Simple.clause pattern_matching)
=
...
let divide_constant ctx m =
  divide get_expr_args_constant (fun c d -> const_compare c d = 0)
    (get_key_constant "divide") get_pat_args_constant ctx m
...
let divide_constructor ~scopes ctx pm =
  divide (get_expr_args_constr ~scopes)
  ...
  let rec divide =
    ...
    | ((p, patl), action) :: rem ->
      ...
      then
        ...
        | None ->
          add_in_div (make_matching get_expr_args_variant_constant head def ctx)
            (=) (Cstr_constant tag) (patl, action) variants
        | Some pat ->
          add_in_div
            (make_matching (get_expr_args_variant_nonconst ~scopes) head def ctx)
            (=) (Cstr_block tag) (pat :: patl, action) variants
        end
  in
  divide cl
(*
...
let get_mod_field modname field =
  lazy
    (let mod_ident = Ident.create_persistent modname in
     ...
     | Error `Not_found -> fatal_error ("Module " ^ modname ^ " unavailable.")
     | Ok env ->
       begin match Env.find_value_by_name (Longident.Lident field) env with
       | exception Not_found ->
...
let divide_lazy ~scopes head ctx pm =
  divide_line (Context.specialize head) (get_expr_args_lazy ~scopes)
...
let divide_tuple ~scopes head ctx pm =
  let arity = Patterns.Head.arity head in
  divide_line (Context.specialize head) (get_expr_args_tuple ~scopes)
...
let divide_record all_labels ~scopes head ctx pm =
  ...
  divide_line (Context.specialize head) (get_expr_args_record ~scopes)
    (get_pat_args_record (Array.length all_labels)) head ctx pm
...
let divide_array ~scopes kind ctx pm =
  divide (get_expr_args_array ~scopes kind) (=) get_key_array get_pat_args_array
    ctx pm
(*
...
let rec do_make_string_test_tree loc arg sw delta d =
  ...
  if len <= strings_test_threshold + delta then
    ...
    bind_sw
      ...
      (fun r ->
         tree_way_test loc r (do_make_string_test_tree loc arg lt delta d) act
...
let rec do_tests_fail loc fail tst arg =
  function
  | [] -> fail
  | (c, act) :: rem ->
    Lifthenelse
...
let rec do_tests_nofail loc tst arg =
  function
...
let make_test_sequence loc fail tst lt_tst arg const_lambda_list =
  ...
  let (hs, const_lambda_list, fail) = share_actions_tree const_lambda_list fail
  in
...
*)
    i
  in
  let rec nofail_rec cur_low cur_high cur_act =
    function
    | [] ->
    ...
    | (i, act_i) :: rem as all ->
      ...
      if cur_high + 1 = i then
        ...
        (cur_low, cur_high, cur_act) ::
          (cur_high + 1, i - 1, 0) :: nofail_rec i i act_index rem
  and fail_rec cur_low cur_high =
    function
    | [] -> [ cur_low, cur_high, 0 ]
    | (i, act_i) :: rem ->
...
let as_interval_nofail l =
  ...
  in
  let rec i_rec cur_low cur_high cur_act =
    function
    | [] -> [ cur_low, cur_high, cur_act ]
    | (i, act) :: rem ->
...
let complete_pats_constrs =
  function
  | constr :: _ as constrs ->
    let constr_of_pat cstr_pat = cstr_pat.pat_desc in
    let pat_of_constr cstr =
      let open Patterns.Head in
      to_omega_pattern { constr with  pat_desc = Construct cstr }
    in
...
let mk_failaction_neg partial ctx def =
  match partial with
  | Partial ->
    begin match Default_environment.pop def with
    | Some ((_, idef), _) ->
...
let mk_failaction_pos partial seen ctx defs =
    ...
    | [], _ | _, None ->
      List.fold_left
        (fun (klist, jumps) (pats, i) ->
           ...
           let klist =
             List.fold_right (fun pat r -> (get_key_constr pat, action) :: r)
               pats klist
           and jumps = Jumps.add i (Context.lub (list_as_pat pats) ctx) jumps
           in
           klist, jumps) ([], Jumps.empty) env
    | _, Some ((pss, idef), rem) ->
      let (now, later) =
        List.partition (fun (_p, p_ctx) -> Context.matches p_ctx pss) to_test
      in
      begin match now with
  ...
  if List.length fail_pats < !Clflags.match_context_rows then
    let (fail, jmps) =
      scan_def [] (List.map (fun pat -> pat, Context.lub pat ctx) fail_pats)
        defs
    in
    ((if dbg then
        (eprintf "POSITIVE JUMPS [%i]:\n" (List.length fail_pats);
         Jumps.eprintf jmps));
     None, fail, jmps)
  else (* Too many non-matched constructors -> reduced information *)
...
let combine_constant
    loc arg cst partial ctx def (const_lambda_list, total, _pats)
=
  ...
  let lambda1 =
    ...
    | Const_char _ ->
      let int_lambda_list =
        List.map
          (function Const_char c, l -> Char.code c, l | _ -> assert false)
          const_lambda_list
      in
      ...
      let sw =
        List.map
          (fun (c, act) ->
             ...
             | _ -> assert false) const_lambda_list
      in
    ...
    | Const_float _ ->
      make_test_sequence loc fail (Pfloatcomp CFneq) (Pfloatcomp CFlt) arg
        const_lambda_list
    | Const_int32 _ ->
      make_test_sequence loc fail (Pbintcomp (Pint32, Cne))
        (Pbintcomp (Pint32, Clt)) arg const_lambda_list
    | Const_int64 _ ->
      make_test_sequence loc fail (Pbintcomp (Pint64, Cne))
        (Pbintcomp (Pint64, Clt)) arg const_lambda_list
    | Const_nativeint _ ->
      make_test_sequence loc fail (Pbintcomp (Pnativeint, Cne))
        (Pbintcomp (Pnativeint, Clt)) arg const_lambda_list
  in
...
let combine_constructor
    loc arg pat_env cstr partial ctx def (descr_lambda_list, total1, pats)
=
  ...
  | Cstr_extension _ ->
    ...
    let lambda1 =
      let (consts, nonconsts) =
        split_extension_cases (List.map tag_lambda descr_lambda_list)
      in
  ...
  | _ ->
    ...
    and nconstrs = cstr.cstr_consts + cstr.cstr_nonconsts
    in
    ...
    let (consts, nonconsts) =
      split_cases (List.map tag_lambda descr_lambda_list)
    in
    let lambda1 =
...
let make_test_sequence_variant_constant fail arg int_lambda_list =
  let (_, (cases, actions)) = as_interval fail min_int max_int int_lambda_list
  in
...
let combine_variant
    loc row arg partial ctx def (tag_lambda_list, total1, _pats)
=
  let row = Btype.row_repr row in
  ...
  (if row.row_closed then
     List.iter
       (fun (_, f) ->
          ...
          | _ -> incr num_constr) row.row_fields
   else
     num_constr := max_int);
  ...
  and one_action = same_actions tag_lambda_list
  in
  ...
  let lambda1 =
    ...
    | _, _ ->
      ...
      | _, _ ->
        ...
        and lam_nonconst = call_switcher_variant_constr loc fail arg nonconsts
        in
...
let combine_array
    loc arg kind partial ctx def (len_lambda_list, total1, _pats)
=
  let (fail, local_jumps) = mk_failaction_neg partial ctx def in
  let lambda1 =
    let newvar = Ident.create_local "len" in
    let switch = call_switcher loc fail (Lvar newvar) 0 max_int len_lambda_list
    in
  ...
  let rec c_rec totals =
    ...
    | (key, cell) :: rem ->
      if Context.is_empty cell.ctx then
        ...
        begin match compile_fun cell.ctx cell.pm with
  ...
  let rec do_rec r total_r =
    ...
    | { provenance = mat; exit = i; vars; pm } :: rem ->
      ...
      | handler_i, total_i ->
        ...
        | _ ->
          do_rec (Lstaticcatch (r, (i, vars), handler_i))
            (Jumps.union (Jumps.remove i total_r)
               (Jumps.map (Context.rshift_num (ncols mat)) total_i)) rem
        end
      end
  in
...
let rec lower_bind v arg lam =
  ...
  | Lifthenelse (cond, ifso, ifnot) ->
    ...
    and pnot = approx_present v ifnot
    in
      ...
      | (i, pm) :: rem ->
        ...
        if Context.is_empty ctx_i then
          ...
          let partial =
            ...
            | _ -> Partial
          in
          begin match comp_fun partial ctx_i pm with
          | li, total_i ->
            c_rec (Lstaticcatch (body, (i, []), li))
              (Jumps.union total_i total_rem) rem
          | exception Unused ->
            c_rec (Lstaticcatch (body, (i, []), lambda_unit)) total_rem rem
          end
    in
    begin match comp_fun Partial ctx first_match with
    | first_lam, total -> c_rec first_lam total rem
    | exception Unused ->
      begin match next_matchs with
      | [] -> raise Unused
...
*)
let rec compile_match
    ~scopes repr partial ctx (m : initial_clause pattern_matching)
=
  match m.cases with
  | ([], action) :: rem ->
...
and compile_match_nonempty
    ~scopes repr partial ctx
...
=
  ...
  | { args = (arg, str) :: argl } ->
    ...
    let (first_match, rem) = split_and_precompile_half_simplified ~arg:newarg m
    in
    combine_handlers ~scopes repr partial ctx (v, str, arg) first_match rem
  | _ -> assert false

and compile_match_simplified
    ~scopes repr partial ctx (m : Simple.clause pattern_matching)
=
  match m with
  | { cases = []; args = [] } -> comp_exit ctx m
  ...
  let (lam, total) =
    comp_match_handlers
      ((if dbg then
            ...
            do_compile_matching ~scopes) repr) partial ctx first_match rem
  in
...
and do_compile_matching ~scopes repr partial ctx pmh =
  ...
  | Pm pm ->
    ...
    let open Patterns.Head in
    begin match ph.pat_desc with
    | Any ->
      compile_no_test ~scopes divide_var Context.rshift repr partial ctx pm
    | Tuple _ ->
      compile_no_test ~scopes (divide_tuple ~scopes ph) Context.combine repr
        partial ctx pm
    | Record [] -> assert false
    | Record (lbl :: _) ->
      compile_no_test ~scopes (divide_record ~scopes lbl.lbl_all ph)
        Context.combine repr partial ctx pm
    | Constant cst ->
      compile_test (compile_match ~scopes repr partial) partial divide_constant
        (combine_constant ploc arg cst partial) ctx pm
    | Construct cstr ->
      compile_test (compile_match ~scopes repr partial) partial
        (divide_constructor ~scopes)
        (combine_constructor ploc arg ph.pat_env cstr partial) ctx pm
    | Array _ ->
      let kind = Typeopt.array_pattern_kind pomega in
      compile_test (compile_match ~scopes repr partial) partial
        (divide_array ~scopes kind) (combine_array ploc arg kind partial) ctx pm
    | Lazy ->
      compile_no_test ~scopes (divide_lazy ~scopes ph) Context.combine repr
        partial ctx pm
    | Variant { cstr_row = row } ->
      compile_test (compile_match ~scopes repr partial) partial
        (divide_variant ~scopes !row) (combine_variant ploc !row arg partial)
        ctx pm
    end
  | PmVar { inside = pmh } ->
    let (lam, total) =
  ...
  | PmOr { body; handlers } ->
    let (lam, total) = compile_match_simplified ~scopes repr partial ctx body in
    compile_orhandlers (compile_match ~scopes repr partial) lam total ctx
      handlers

and compile_no_test ~scopes divide up_ctx repr partial ctx to_match =
  let { pm = this_match; ctx = this_ctx } = divide ctx to_match in
  let (lambda, total) = compile_match ~scopes repr partial this_ctx this_match
  in
...
*)
let check_partial has_mutable has_lazy pat_act_list =
  function
  | Partial -> Partial
  | Total ->
    if
      pat_act_list = [] || (* allow empty case list *)
        List.exists
          (fun (pats, lam) ->
             has_mutable pats && (is_guarded lam || has_lazy pats)) pat_act_list
    then
      Partial
    else
      Total
...
let failure_handler ~scopes loc ~failer () =
  ...
  | Raise_match_failure ->
    ...
    let slot =
      transl_extension_path sloc Env.initial_safe_string
        Predef.path_match_failure
    in
...
let check_total ~scopes loc ~failer total lambda i =
  if Jumps.is_empty total then
    ...
    Lstaticcatch (lambda, (i, []), failure_handler ~scopes loc ~failer ())
...
let toplevel_handler ~scopes loc ~failer partial args cases compile_fun =
  ...
  | Partial ->
    ...
    | lam, total -> check_total ~scopes loc ~failer total lam raise_num
    end
...
let for_function ~scopes loc repr param pat_act_list partial =
  compile_matching ~scopes loc ~failer:Raise_match_failure repr param
...
let for_trywith ~scopes loc param pat_act_list =
  ...
  compile_matching ~scopes loc ~failer:(Reraise_noloc param) None param
    pat_act_list Partial

let simple_for_let ~scopes loc param pat body =
  compile_matching ~scopes loc ~failer:Raise_match_failure None param
    [ pat, body ] Partial
...
let rec map_return f =
  ...
  | Lswitch (s, sw, loc) ->
    let map_cases cases = List.map (fun (i, l) -> i, map_return f l) cases in
    Lswitch
      (s,
       { sw with
...
let assign_pat ~scopes opt nraise catch_ids loc pat lam =
  ...
  let push_sublet code (_ids, pat, lam) =
    simple_for_let ~scopes loc lam pat code
  in
...
let for_let ~scopes loc param pat body =
  ...
  | _ ->
    ...
    let bind = map_return (assign_pat ~scopes opt nraise ids loc pat) param in
    if !opt then
...
let for_tupled_function ~scopes loc paraml pats_act_list partial =
  ...
  let handler =
    toplevel_handler ~scopes loc ~failer:Raise_match_failure partial args
      pats_act_list
  in
  handler
    (fun partial pm ->
       compile_match ~scopes None partial (Context.start (List.length paraml))
         pm)
...
let flatten_simple_pattern size (p : Simple.pattern) =
  ...
  | `Array _ | `Variant _ | `Record _ | `Lazy _ | `Construct _ | `Constant _ ->
    ...
    let msg =
      Format.fprintf Format.str_formatter "Matching.flatten_pattern: got '%a'"
        top_pretty (General.erase p);
      Format.flush_str_formatter ()
    in
    fatal_error msg
...
let flatten_cases size cases =
  List.map
    (function
     | (p, []), action ->
       begin match flatten_simple_pattern size p with
       | p :: ps -> (p, ps), action
       | [] -> assert false
       end
     | _ -> fatal_error "Matching.flatten_hc_cases") cases
...
let do_for_multiple_match ~scopes loc paraml pat_act_list partial =
  ...
  let arg =
    let sloc = Scoped_location.of_location ~scopes loc in
    Lprim (Pmakeblock (0, Immutable, None), paraml, sloc)
  in
  let handler =
    let partial = check_partial pat_act_list partial in
    let rows = map_on_rows (fun p -> p, []) pat_act_list in
    toplevel_handler ~scopes loc ~failer:Raise_match_failure partial
      [ arg, Strict ] rows
  in
  handler
    (fun partial pm1 ->
       ...
       and idl =
         List.map (function Lvar id -> id | _ -> Ident.create_local "*match*")
           paraml
       in
