...
let meth obj lab =
  ...
  if not (!cache_required && !Clflags.native_code) then
    tag, []
  else if not (is_path obj) then
    next_cache tag
  else
    try
...
let transl_label_init_general f =
  let (expr, size) = f () in
  let expr =
    Hashtbl.fold (fun c id expr -> Llet (Alias, Pgenval, id, Lconst c, expr))
      consts expr
  in
  (*let expr =
    List.fold_right
    ...
    if !method_count = 0 then
      ...
      Llet
        (Strict,
         ...
         method_cache_id,
         Lprim
           (Pccall prim_makearray, [ int !method_count; int 0 ], Loc_unknown),
         expr)
  in
...
let transl_store_label_init glob size f arg =
  ...
  method_cache :=
    Lprim
      (Pfield size, [ Lprim (Pgetglobal glob, [], Loc_unknown) ], Loc_unknown);
  let expr = f arg in
  let (size, expr) =
    if !method_count = 0 then
      size, expr
      ...
      Lsequence
        (Lprim
           (Psetfield (size, Pointer, Root_initialization),
...
let oo_wrap env req f x =
  if !wrapping then
    (if !cache_required then
       ...
       Misc.protect_refs [ Misc.R (cache_required, true) ] (fun () -> f x))
  else
