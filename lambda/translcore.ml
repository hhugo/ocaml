...
let transl_module =
  ref
    ...
    :
    scopes:scopes -> module_coercion -> Path.t option -> module_expr -> lambda)
...
let transl_extension_constructor ~scopes env path ext =
  let path =
    Printtyp.wrap_printing_env env ~error:true
      (fun () -> Option.map (Printtyp.rewrite_double_underscore_paths env) path)
  in
  let name =
  ...
  | Text_decl _ ->
    Lprim
      (Pmakeblock (Obj.object_tag, Immutable, None),
...
let rec trivial_pat pat =
  ...
  | Tpat_tuple patl -> List.for_all trivial_pat patl
  | _ -> false
  ...
  | [
      { c_lhs = pat; c_guard = None;
        c_rhs =
          {
            exp_attributes =
              [ { Parsetree.attr_name = { txt = "#modulepat" }; _ } ];
            exp_desc =
              Texp_letmodule
                (Some id,
                 name,
    ...
    let exp =
      let cases =
        let pure_case ({ c_lhs; _ } as case) =
          { case with  c_lhs = as_computation_pattern c_lhs }
        in
        List.map pure_case cases
      in
      { exp with
    ...
    [
      {
        ...
        c_guard = None;
        c_rhs = wrap_bindings bindings exp
      } ]
  | _ -> cases
...
let assert_failed ~scopes exp =
  let slot =
    transl_extension_path Loc_unknown Env.initial_safe_string
...
let transl_ident loc env ty path desc =
  ...
  | Val_prim p -> Translprim.transl_primitive loc p env ty (Some path)
  | Val_anc _ -> raise (Error (to_location loc, Free_super_var))
  | Val_reg | Val_self _ -> transl_value_path loc env path
  | _ -> fatal_error "Translcore.transl_exp: bad Texp_ident"

let rec transl_exp ~scopes e = transl_exp1 ~scopes ~in_new_scope:false e
...
and transl_exp1 ~scopes ~in_new_scope e =
  ...
  in
  if eval_once then
    transl_exp0 ~scopes ~in_new_scope e
  else
...
and transl_exp0 ~in_new_scope ~scopes e =
  ...
  | Texp_ident (path, _, desc) ->
    transl_ident (of_location ~scopes e.exp_loc) e.exp_env e.exp_type path desc
  ...
  | Texp_function { arg_label = _; param; cases; partial } ->
    let scopes =
      if in_new_scope then scopes else enter_anonymous_function ~scopes
    in
  ...
  | Texp_apply
      (({ exp_desc = Texp_ident (path, _, { val_kind = Val_prim p });
       ...
       oargs)
    when
      List.length oargs >= p.prim_arity &&
        List.for_all (fun (_, arg) -> arg <> None) oargs
    ->
    let (argl, extra_args) = cut p.prim_arity oargs in
    let arg_exps = List.map (function _, Some x -> x | _ -> assert false) argl
    in
    let args = transl_list ~scopes arg_exps in
    let prim_exp = if extra_args = [] then Some e else None in
    let lam =
      Translprim.transl_primitive_application (of_location ~scopes e.exp_loc) p
        e.exp_env prim_type path prim_exp args arg_exps
    in
    if extra_args = [] then
      lam
    else
      let (tailcall, funct) = Translattribute.get_tailcall_attribute funct in
      let (inlined, funct) =
      ...
      event_after ~scopes e
        (transl_apply ~scopes ~tailcall ~inlined ~specialised lam extra_args
  ...
  | Texp_tuple el ->
    ...
    (try Lconst (Const_block (0, List.map extract_constant ll))
     ...
     | Not_constant ->
       Lprim
         (Pmakeblock (0, Immutable, Some shape),
          ll,
          of_location ~scopes e.exp_loc))
  | Texp_construct (_, cstr, args) ->
    ...
    if cstr.cstr_inlined <> None then
      ...
      | Cstr_constant n -> Lconst (const_int n)
      | Cstr_unboxed ->
        begin match ll with
        | [ v ] -> v
        | _ -> assert false
      ...
      | Cstr_block n ->
        (try Lconst (Const_block (n, List.map extract_constant ll))
         with
         | Not_constant ->
           Lprim
             (Pmakeblock (n, Immutable, Some shape),
              ll,
              of_location ~scopes e.exp_loc))
  ...
  | Texp_record { fields; representation; extended_expression } ->
    transl_record ~scopes e.exp_loc e.exp_env fields representation
      extended_expression
  | Texp_field (arg, _, lbl) ->
    let targ = transl_exp ~scopes arg in
    begin match lbl.lbl_repres with
    | Record_regular | Record_inlined _ ->
      Lprim (Pfield lbl.lbl_pos, [ targ ], of_location ~scopes e.exp_loc)
    | Record_unboxed _ -> targ
    | Record_float ->
      Lprim (Pfloatfield lbl.lbl_pos, [ targ ], of_location ~scopes e.exp_loc)
    | Record_extension _ ->
      Lprim (Pfield (lbl.lbl_pos + 1), [ targ ], of_location ~scopes e.exp_loc)
    end
  | Texp_setfield (arg, _, lbl, newval) ->
    ...
    Lprim
      (access,
       [ transl_exp ~scopes arg; transl_exp ~scopes newval ],
       of_location ~scopes e.exp_loc)
  | Texp_array expr_list ->
    ...
    (try
     (* For native code the decision as to which compilation strategy to
        ...
        kinds of array definitions to symbols. *)(* Deactivate constant optimization if array is small enough *)
       ...
       | exception Not_constant when kind = Pfloatarray ->
         ...
         let imm_array =
           Lprim
             (Pmakearray (kind, Immutable), ll, of_location ~scopes e.exp_loc)
         in
         Lprim
           (Pduparray (kind, Mutable),
            [ imm_array ],
            of_location ~scopes e.exp_loc)
       | cl ->
         let imm_array =
  ...
  | Texp_letmodule (_, _, Mp_absent, _, body) -> transl_exp ~scopes body
  ...
  | Texp_assert cond ->
    if !Clflags.noassert then
      ...
      Lifthenelse
        (transl_exp ~scopes cond, lambda_unit, assert_failed ~scopes e)
  | Texp_lazy e ->
    ...
    | `Float_that_cannot_be_shortcut ->
      ...
      Lprim
        (Pmakeblock (Obj.forward_tag, Immutable, None),
         [ transl_exp ~scopes e ],
         of_location ~scopes e.exp_loc)
         ...
         [
           Lprim
             (Pmakeblock (Obj.forward_tag, Immutable, None),
              ...
              of_location ~scopes e.exp_loc) ],
         of_location ~scopes e.exp_loc)
    | `Identifier `Other -> transl_exp ~scopes e
    | `Other ->
        ...
        Lfunction
          {
            ...
            body = transl_exp ~scopes e
          }
      in
      Lprim
        (Pmakeblock (Config.lazy_tag, Mutable, None),
         [ fn ],
         of_location ~scopes e.exp_loc)
    end
          ...
          (fun (body, pos) id ->
             Llet
               (Alias,
                ...
                id,
                Lprim
                  (Pfield pos, [ Lvar oid ], of_location ~scopes od.open_loc),
                body),
             pos + 1) (transl_exp ~scopes e, 0)
...
and transl_list ~scopes expr_list = List.map (transl_exp ~scopes) expr_list
...
and transl_cases ~scopes cases =
  let cases = List.filter (fun c -> c.c_rhs.exp_desc <> Texp_unreachable) cases
  in
  List.map (transl_case ~scopes) cases

and transl_case_try ~scopes { c_lhs; c_guard; c_rhs } =
  iter_exn_names Translprim.add_exception_ident c_lhs;
  Misc.try_finally (fun () -> c_lhs, transl_guard ~scopes c_guard c_rhs)
...
and transl_apply
    ~scopes ?(tailcall=Default_tailcall) ?(inlined=Default_inline)
...
=
  ...
  let rec build_apply lam args =
    ...
    | (None, optional) :: l ->
      ...
      in
      let (args, args') =
        if List.for_all (fun (_, opt) -> opt) args then [], args else args, []
      in
      let lam = if args = [] then lam else lapply lam (List.rev_map fst args) in
      let handle = protect "func" lam in
      let l = List.map (fun (arg, opt) -> Option.map (protect "arg") arg, opt) l
      in
      let id_arg = Ident.create_local "param" in
      let body =
        match build_apply handle ((Lvar id_arg, optional) :: args') l with
        | Lfunction
            { kind = Curried; params = ids; return; body = lam; attr; loc }
          ->
          Lfunction
            {
              kind = Curried;
      ...
      in
      List.fold_left
        (fun body (id, lam) -> Llet (Strict, Pgenval, id, lam, body)) body !defs
    | (Some arg, optional) :: l -> build_apply lam ((arg, optional) :: args) l
    | [] -> lapply lam (List.rev_map fst args)
  in
  (build_apply lam []
    (List.map
       (fun (l, x) -> Option.map (transl_exp ~scopes) x, Btype.is_optional l)
       sargs)
  :
  Lambda.lambda)

and transl_curried_function
    ~scopes loc return repr partial (param : Ident.t) cases
=
  ...
  let rec loop ~scopes loc return ~arity partial (param : Ident.t) cases =
    ...
    | [
      ...
      if Parmatch.inactive ~partial pat then
        ...
        let ((_, params, return), body) =
          loop ~scopes exp_loc return_kind ~arity:(arity + 1) partial' param'
            cases'
        in
        (Curried, (param, kind) :: params, return),
        Matching.for_function ~scopes loc None (Lvar param) [ pat, body ]
          partial
      else
        (begin match partial with
         ...
         transl_tupled_function ~scopes ~arity loc return repr partial param
           cases)
    | cases ->
      transl_tupled_function ~scopes ~arity loc return repr partial param cases
  in
  loop ~scopes loc return ~arity:1 partial param cases

and transl_tupled_function
    ~scopes ~arity loc return repr partial (param : Ident.t) cases
=
  match cases with
  | { c_lhs = { pat_desc = Tpat_tuple pl } } :: _
    when
      !Clflags.native_code && arity = 1 && List.length pl <= Lambda.max_arity ()
    ->
    (try
       ...
       let pats_expr_list =
         List.map
           (fun { c_lhs; c_guard; c_rhs } ->
              Matching.flatten_pattern size c_lhs, c_guard, c_rhs) cases
       in
       let kinds =
         ...
         | (pats, _, _) :: cases ->
           ...
           List.fold_left
             (fun kinds (pats, _, _) ->
                List.map2
                  (fun kind pat ->
                     value_kind_union kind (value_kind pat.pat_env pat.pat_type))
                  kinds pats) first_case_kinds cases
       in
       let tparams =
...
and transl_function0 ~scopes loc return repr partial (param : Ident.t) cases =
  let kind =
    match cases with
    | [] ->
    ...
    | { c_lhs = pat } :: other_cases ->
      ...
      List.fold_left
        (fun k { c_lhs = pat } ->
           Typeopt.value_kind_union k (value_kind pat.pat_env pat.pat_type))
    ...
    event_function ~scopes e
      (function
       | repr ->
         ...
         transl_curried_function ~scopes e.exp_loc return_kind repr partial
           param pl)
  in
...
and transl_scoped_exp ~scopes expr = transl_exp1 ~scopes ~in_new_scope:true expr
...
and transl_setinstvar ~scopes loc self var expr =
  Lprim
    (Psetfield_computed (maybe_pointer expr, Assignment),
     [ self; var; transl_exp ~scopes expr ],
     loc)
...
and transl_record ~scopes loc env fields repres opt_init_expr =
  ...
  let no_init =
    ...
    | _ -> false
  in
  if no_init || size < Config.max_young_wosize then
      ...
      Array.mapi
        (fun i (_, definition) ->
           ...
           | Overridden (_lid, expr) ->
             ...
             transl_exp ~scopes expr, field_kind) fields
    in
    ...
    | None -> lam
    | Some init_expr ->
      Llet (Strict, Pgenval, init_id, transl_exp ~scopes init_expr, lam)
  else
    ...
    let update_field cont (lbl, definition) =
      ...
      | Overridden (_lid, expr) ->
        ...
        Lsequence
          (Lprim
             (upd,
              [ Lvar copy_id; transl_exp ~scopes expr ],
...
and transl_match ~scopes e arg pat_expr_list partial =
  ...
  =
    if c_rhs.exp_desc = Texp_unreachable then
      acc
    else
      ...
      | Some pv, None ->
        let val_case = transl_case ~scopes { case with  c_lhs = pv } in
      ...
      | Some pv, Some pe ->
        ...
        let rhs =
          Misc.try_finally
            (fun () -> event_before ~scopes c_rhs (transl_exp ~scopes c_rhs))
            ~always:(fun () ->
  ...
  *)
  ...
  let classic =
    ...
    | { exp_desc = Texp_tuple argl }, [] ->
      ...
      Matching.for_multiple_match ~scopes e.exp_loc (transl_list ~scopes argl)
        val_cases partial
      ...
      let val_ids =
        List.map
          (fun arg ->
             ...
             Typeopt.value_kind arg.exp_env arg.exp_type) argl
      in
      let lvars = List.map (fun (id, _) -> Lvar id) val_ids in
      static_catch (transl_list ~scopes argl) val_ids
        (Matching.for_multiple_match ~scopes e.exp_loc lvars val_cases partial)
    | arg, [] ->
      assert (static_handlers = []);
      Matching.for_function ~scopes e.exp_loc None (transl_exp ~scopes arg)
        val_cases partial
    | arg, _ :: _ ->
      ...
      static_catch [ transl_exp ~scopes arg ] [ val_id, k ]
        (Matching.for_function ~scopes e.exp_loc None (Lvar val_id) val_cases
           partial)
  in
  List.fold_left
    (fun body (static_exception_id, val_ids, handler) ->
       Lstaticcatch (body, (static_exception_id, val_ids), handler)) classic
    static_handlers

and transl_letop ~scopes loc env let_ ands param case partial =
  let rec loop prev_lam =
    function
    | [] -> prev_lam
    | and_ :: rest ->
  ...
  in
  let op =
    transl_ident (of_location ~scopes let_.bop_op_name.loc) env let_.bop_op_type
      let_.bop_op_path let_.bop_op_val
  in
  ...
  let func =
    ...
    let ((kind, params, return), body) =
      event_function ~scopes case.c_rhs
        (function
         | repr ->
           transl_curried_function ~scopes case.c_rhs.exp_loc return_kind repr
             partial param [ case ])
    in
    let attr = default_function_attribute in
