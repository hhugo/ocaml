...
*)
  ...
  let less_tests c1 c2 =
    if c1.n < c2.n then
      true
    else if c1.n = c2.n then
      (if c1.ni < c2.ni then true else false)
    else
      false
  ...
  let case_append c1 c2 =
    ...
    | _, _ ->
      ...
      if act1 = act2 then
        ...
        (for i = 0 to len1 - 2 do r.(i) <- c1.(i) done;
         let l =
           if len1 - 2 >= 0 then
         ...
         in
         r.(len1 - 1) <- l, h, act1;
         for i = 1 to len2 - 1 do r.(len1 - 1 + i) <- c2.(i) done;
         r)
      else if h1 > l1 then
        let r = Array.make (len1 + len2) c1.(0) in
        (for i = 0 to len1 - 2 do r.(i) <- c1.(i) done;
         r.(len1 - 1) <- l1, l2 - 1, act1;
         for i = 0 to len2 - 1 do r.(len1 + i) <- c2.(i) done;
         r)
      else if h2 > l2 then
        let r = Array.make (len1 + len2) c1.(0) in
        (for i = 0 to len1 - 1 do r.(i) <- c1.(i) done;
         r.(len1) <- h1 + 1, h2, act2;
         for i = 1 to len2 - 1 do r.(len1 + i) <- c2.(i) done;
         r)
      else
        Array.append c1 c2
    ...
    let rec make_rec i pl =
      if i < 0 then
        ...
        if pl = h + 1 then
          ...
          Kempty :: make_one l h act :: make_rec (i - 1) l
    in
...
*)
  ...
  and divide cases =
    ...
    and (_, (cmr, cright)) = opt_count false right
    in
    add_test ci cleft;
    add_test ci cright;
  ...
  and heuristic cases =
    ...
    and (inter, cinter) =
      if !ok_inter then
        ...
        and (_, _, act1) = cases.(lcases - 1)
        in
        (if act0 = act1 then
           let (low, high, inside, outside) = coupe_inter 1 (lcases - 2) cases
           in
           ...
           and cij = { n = 1; ni = (if low = high then 0 else 1) }
           in
  ...
  and enum top cases =
    ...
    let (lim, with_sep) =
      let best = ref (-1)
      and best_cost = ref (too_much, too_much)
      in
    ...
    let (ilow, ihigh, with_inter) =
      if not !ok_inter then
        ...
        (for i = 1 to lcases - 2 do
         ...
         !rlow, !rhigh, !best_cost)
    in
    let r = ref (Inter (ilow, ihigh))
    and rc = ref with_inter
    in
  ...
  let make_if_out ctx l d mk_ifso mk_ifno =
    ...
    | _ ->
      Arg.bind (Arg.make_offset ctx.arg ~-l)
        (fun arg ->
  ...
  let rec c_test ctx ({ cases = cases; actions = actions } as s) =
    ...
    if lcases = 1 then
      ...
      | Inter (i, j) ->
        ...
        and (_, (coutside, _)) = opt_count false outside
        in
      ...
      | Sep i ->
        ...
        and (_, (cright, _)) = opt_count false right
        in
        let left = { s with  cases = left }
        and right = { s with  cases = right }
        in
        if i = 1 && lim + ctx.off = 1 && get_low cases 0 + ctx.off = 0 then
          Arg.make_if ctx.arg (c_test ctx right) (c_test ctx left)
  ...
  *)
  ...
  let make_switch loc { cases = cases; actions = actions } i j =
    ...
    for k = i to j do
      ...
      for kk = l - ll to h - ll do tbl.(kk) <- index done
    done;
    ...
    fun ctx ->
      ...
      | _ ->
        Arg.bind (Arg.make_offset ctx.arg (~-ll - ctx.off))
          (fun arg -> Arg.make_switch loc arg tbl acts)
  
  let make_clusters
      loc ({ cases = cases; actions = actions } as s) n_clusters k
  =
    let len = Array.length cases in
    let r = Array.make n_clusters (0, 0, 0)
    and t = Hashtbl.create 17
    and index = ref 0
    and bidon = ref (Array.length actions)
    in
    let get_index act =
      try
...
*)
  ...
  and test_sequence arg cases actions =
    ...
    ok_inter := false;
    (if !ok_inter <> old_ok then Hashtbl.clear t);
    let s = { cases = cases; actions = Array.map (fun act _ -> act) actions } in
    (*
