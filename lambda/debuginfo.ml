...
module Scoped_location = struct
  ...
  let cons item str = Cons { item; str; str_fun = str ^ ".(fun)" }
  let empty_scopes = Empty
  
  let add_parens_if_symbolic =
    function
    | "" -> ""
    | s ->
      ...
      List.map
        (fun d ->
           Printf.sprintf "%s:%d,%d-%d" d.dinfo_file d.dinfo_line
             d.dinfo_char_start d.dinfo_char_end) ds
    in
    "{" ^ String.concat ";" items ^ "}"

let item_from_location ~scopes loc =
  let valid_endpos = String.equal loc.loc_end.pos_fname loc.loc_start.pos_fname
  in
  {
