...
let make_key e =
  ...
  and make_key = Ident.make_key_generator ()
  in
...
let shallow_iter ~tail ~non_tail:f =
  function
  ...
  | Llet (_, _k, _id, arg, body) | Lmutlet (_k, _id, arg, body) ->
    f arg;
    tail body
  | Lletrec (decl, body) ->
    tail body;
  ...
  | Lfor (_v, e1, e2, _dir, e3) ->
    ...
    f e3
  | Lassign (_, e) -> f e
  | Lsend (_k, met, obj, args, _) -> List.iter f (met :: obj :: args)
...
let rec free_variables =
  ...
  | Lstaticcatch (body, (_, params), handler) ->
    Ident.Set.union
      (Ident.Set.diff (free_variables handler)
         (Ident.Set.of_list (List.map fst params))) (free_variables body)
  | Ltrywith (body, param, handler) ->
    Ident.Set.union (Ident.Set.remove param (free_variables handler))
      (free_variables body)
  | Lifthenelse (e1, e2, e3) ->
    Ident.Set.union (Ident.Set.union (free_variables e1) (free_variables e2))
      (free_variables e3)
  | Lsequence (e1, e2) ->
    Ident.Set.union (free_variables e1) (free_variables e2)
  | Lwhile (e1, e2) -> Ident.Set.union (free_variables e1) (free_variables e2)
  ...
  | Lsend (_k, met, obj, args, _) ->
    free_variables_list
      (Ident.Set.union (free_variables met) (free_variables obj)) args
...
and free_variables_list set exprs =
  List.fold_left (fun set expr -> Ident.Set.union (free_variables expr) set) set
    exprs
...
let rec transl_address loc =
  function
  | Env.Aident id ->
    if Ident.global id then Lprim (Pgetglobal id, [], loc) else Lvar id
  | Env.Adot (addr, pos) -> Lprim (Pfield pos, [ transl_address loc addr ], loc)
...
let subst update_env ?(freshen_bound_variables=false) s input_lam =
  ...
  let rec subst s l lam =
    ...
    | Lstaticcatch (body, (id, params), handler) ->
      ...
      Lstaticcatch (subst s l body, (id, params), subst s l' handler)
...
let duplicate lam =
  subst (fun _ _ env -> env) ~freshen_bound_variables:true Ident.Map.empty lam

let shallow_map f =
  function
  | (Lvar _ | Lmutvar _ | Lconst _) as lam -> lam
  | Lapply { ap_func; ap_args; ap_loc; ap_tailcall; ap_inlined; ap_specialised }
    ->
    Lapply
      {
        ap_func = f ap_func;
  ...
  | Lmutlet (k, v, e1, e2) -> Lmutlet (k, v, f e1, f e2)
  | Lletrec (idel, e2) -> Lletrec (List.map (fun (v, e) -> v, f e) idel, f e2)
  ...
  | Lstringswitch (e, sw, default, loc) ->
    Lstringswitch
      (f e, List.map (fun (s, e) -> s, f e) sw, Option.map f default, loc)
  | Lstaticraise (i, args) -> Lstaticraise (i, List.map f args)
  | Lstaticcatch (body, id, handler) -> Lstaticcatch (f body, id, f handler)
