...
let transl_type_extension ~scopes env rootpath tyext body =
  List.fold_right
    (fun ext body ->
       ...
       in
       Llet (Strict, Pgenval, ext.ext_id, lam, body)) tyext.tyext_constructors
    body
...
and apply_coercion_result loc strict funct params args cc_res =
  ...
  | Tcoerce_functor (cc_arg, cc_res) ->
    ...
    apply_coercion_result loc strict funct ((param, Pgenval) :: params)
      (arg :: args) cc_res
  | _ ->
    ...
    List.fold_left
      (fun (lam, s) (id', pos, c) ->
         if Ident.Set.mem id' fv then
           ...
           lam, s) (lam, Ident.Map.empty) id_pos_list
  in
    ...
    let ids1 =
      List.map
        (fun (id, pos1, c1) ->
           ...
           id, pos2, compose_coercions c1 c2) ids1
    in
    Tcoerce_structure
      (List.map
         (fun pc ->
            ...
            | p1, c1 ->
              let (p2, c2) = v2.(p1) in
              p2, compose_coercions c1 c2) pc1,
       ids1 @ ids2)
...
let undefined_location loc =
  ...
  Lconst
    (Const_block
       (0,
        [ Const_base (Const_string (fname, loc, None)); const_int line;
          const_int char ]))
...
let init_shape id modl =
  ...
  and init_shape_struct env sg =
    ...
    | Sig_value (subid, { val_kind = Val_reg; val_type = ty; val_loc = loc }, _) ::
      ...
      let init_v =
        ...
        | Tarrow (_, _, _, _) -> const_int 0 (* camlinternalMod.Function *)
        | Tconstr (p, _, _) when Path.same p Predef.path_lazy_t -> const_int 1
    ...
    | Sig_module (id, Mp_present, md, _, _) :: rem ->
      init_shape_mod id md.md_loc env md.md_type ::
        init_shape_struct
          (Env.add_module_declaration ~check:false id Mp_present md env) rem
  ...
  with
  | Initialization_failure reason -> Result.Error reason
...
let extract_unsafe_cycle id status init cycle_start =
  let info i =
    ...
    | Result.Error r ->
      ...
      | Id id -> id, r
      | Ignore_loc _ -> assert false
      end
    (* Can't refer to something without a name. *)
    | Ok _ -> assert false
  in
...
let reorder_rec_bindings bindings =
  ...
  and rhs = Array.of_list (List.map (fun (_, _, _, rhs) -> rhs) bindings)
  in
...
let transl_class_bindings ~scopes cl_list =
  ...
  List.map
    (fun ({ ci_id_class = id; ci_expr = cl; ci_virt = vf }, meths) ->
       id, transl_class ~scopes ids id meths cl vf) cl_list
  ...
  let (params, body) =
    List.fold_left
      (fun (params, body) (param, loc, arg_coercion) ->
         ...
         params, body) ([], transl_module ~scopes res_coercion body_path body)
      functor_params_rev
  in
  Lfunction
    {
...
and transl_module ~scopes cc rootpath mexp =
  ...
  | Tmod_apply (funct, arg, ccarg) ->
    ...
    in
    oo_wrap mexp.mod_env true (apply_coercion loc Strict cc)
      (Lapply
         {
           ap_loc = loc;
...
and transl_struct
    ~scopes loc fields cc rootpath { str_final_env; str_items; _ }
=
...
and transl_structure ~scopes loc fields cc rootpath final_env =
            ...
            (Pmakeblock (0, Immutable, None),
             List.map
               (fun (pos, cc) ->
                  match cc with
                  | Tcoerce_primitive p ->
                    Translprim.transl_primitive (of_location ~scopes p.pc_loc)
  ...
  | item :: rem ->
    ...
    | Tstr_value (rec_flag, pat_expr_list) ->
      ...
      in
      let ext_fields = List.rev_append (let_bound_idents pat_expr_list) fields
      in
    ...
    | Tstr_typext tyext ->
      ...
      let (body, size) =
        transl_structure ~scopes loc (List.rev_append ids fields) cc rootpath
          final_env rem
      in
    ...
    | Tstr_include incl ->
      ...
      let rec rebind_idents pos newfields =
        function
        ...
        | id :: ids ->
          ...
          Llet
            (Alias,
             Pgenval,
             id,
             Lprim (Pfield pos, [ Lvar mid ], of_location ~scopes incl.incl_loc),
             body),
          size
      in
          ...
          | id :: ids ->
            ...
            Llet
              (Alias,
               Pgenval,
               id,
               Lprim (Pfield pos, [ Lvar mid ], of_location ~scopes od.open_loc),
               body),
            size
        in
        let (body, size) = rebind_idents 0 fields ids in
...
let required_globals ~flambda body =
  ...
  in
  let required =
    List.fold_left (fun acc path -> add_global (Path.head path) acc)
...
let transl_implementation_flambda module_name (str, cc) =
  ...
  let (body, size) =
    Translobj.transl_label_init
      (fun () ->
         transl_struct ~scopes Loc_unknown [] cc (global_path module_id) str)
  in
  {
...
let transl_implementation module_name (str, cc) =
  let implementation = transl_implementation_flambda module_name (str, cc) in
  let code =
    Lprim
      (Psetglobal implementation.module_ident,
       [ implementation.code ],
       Loc_unknown)
  in
...
and all_idents =
  ...
  | item :: rem ->
    ...
    | Tstr_module
        { mb_id = Some id; mb_presence = Mp_present;
...
let transl_store_structure ~scopes glob map prims aliases str =
  ...
  let rec transl_store ~scopes rootpath subst cont =
    function
    | [] ->
    ...
    | item :: rem ->
      ...
      | Tstr_eval (expr, _attrs) ->
        Lsequence
          (Lambda.subst no_env_update subst (transl_exp ~scopes expr),
      ...
      | Tstr_module
        ...
        let lam =
          Translattribute.add_inline_attribute
            (transl_module ~scopes Tcoerce_none None modl) loc mb_attributes
        in
        Lsequence
          (Lprim
             (Pignore,
      ...
      | Tstr_module
          { mb_id = Some id; mb_loc = loc; mb_presence = Mp_present;
            mb_expr = { mod_desc = Tmod_structure str } as mexp; mb_attributes }
        ->
        List.iter (Translattribute.check_attribute_on_module mexp) mb_attributes;
        let loc = of_location ~scopes loc in
        let lam =
          transl_store ~scopes:(enter_module_definition ~scopes id)
           ...
           Llet
             (Strict,
              ...
              Lambda.subst no_env_update subst
                (Lprim
                   (Pmakeblock (0, Immutable, None),
                    List.map (fun id -> Lvar id) (defined_idents str.str_items),
                    loc)),
              Lsequence
                (store_ident loc id,
                ...
                mod_desc =
                  Tmod_constraint
                    (({ mod_desc = Tmod_structure str } as mexp),
                     _,
                     _,
                     (Tcoerce_structure (map, _) as _cc)) }; mb_attributes }
        ->
          ...
          (lam,
           Llet
             (Strict,
              ...
              Lambda.subst no_env_update subst
                (Lprim
                   (Pmakeblock (0, Immutable, None), List.map field map, loc)),
              Lsequence
                (store_ident loc id,
                 transl_store ~scopes rootpath (add_ident true id subst) cont
                   rem)))
      | Tstr_module
        ...
        let lam =
          Translattribute.add_inline_attribute
            (transl_module ~scopes:(enter_module_definition ~scopes id)
               Tcoerce_none (field_path rootpath id) modl) loc mb_attributes
        in
      ...
      | Tstr_class cl_list ->
        let (ids, class_bindings) = transl_class_bindings ~scopes cl_list in
        let lam = Lletrec (class_bindings, store_idents Loc_unknown ids) in
        Lsequence
          (Lambda.subst no_env_update subst lam,
           transl_store ~scopes rootpath (add_idents false ids subst) cont rem)
      | Tstr_include
          ...
          | id :: idl ->
            Llet
              (Alias,
               Pgenval,
               id,
               Lprim (Pfield pos, [ Lvar mid ], of_location ~scopes loc),
               Lsequence
                 (store_ident (of_location ~scopes loc) id,
      ...
      | Tstr_open od ->
        ...
        | Tmod_structure str ->
          ...
          let rec store_idents pos =
            function
            | [] ->
              transl_store ~scopes rootpath (add_idents true ids0 subst) cont
                rem
            | id :: idl ->
              Llet
                (Alias,
          ...
          in
          Lsequence
            (lam, Lambda.subst no_env_update subst (store_idents 0 ids0))
        | _ ->
          let pure = pure_module od.open_expr in
          (* this optimization shouldn't be needed because Simplif would
             actually remove the [Llet] when it's not used.
          ...
          | _ ->
            ...
            let rec store_idents pos =
              function
              | [] ->
                transl_store ~scopes rootpath (add_idents true ids subst) cont
                  rem
              | id :: idl ->
                Llet
                  (Alias,
...
let build_ident_map restr idlist more_ids =
  let rec natural_map pos map prims aliases =
    function
    | [] -> map, prims, aliases, pos
    | id :: rem ->
      natural_map (pos + 1) (Ident.add id (pos, Tcoerce_none) map) prims aliases
        rem
  in
  let (map, prims, aliases, pos) =
    ...
    | Tcoerce_structure (pos_cc_list, _id_pos_list) ->
      ...
      let rec export_map pos map prims aliases undef =
        function
        ...
        | (_source_pos, Tcoerce_alias (env, path, cc)) :: rem ->
          export_map (pos + 1) map prims ((pos, env, path, cc) :: aliases) undef
            rem
        | (source_pos, cc) :: rem ->
          let id = idarray.(source_pos) in
          export_map (pos + 1) (Ident.add id (pos, cc) map) prims aliases
...
let toploop_getvalue_pos = 0
(* position of getvalue in module Toploop *)
let toploop_setvalue_pos = 1
(* position of setvalue in module Toploop *)
...
let close_toplevel_term (lam, ()) =
  Ident.Set.fold
    (fun id l -> Llet (Strict, Pgenval, id, toploop_getvalue id, l))
...
let transl_toplevel_item ~scopes item =
  ...
  | Tstr_module { mb_id = Some id; mb_presence = Mp_present; mb_expr = modl } ->
    ...
    let lam =
      transl_module ~scopes:(enter_module_definition ~scopes id) Tcoerce_none
        (Some (Pident id)) modl
    in
  ...
  | Tstr_recmodule bindings ->
    ...
    compile_recmodule ~scopes
      (fun id modl _loc ->
         ...
         | Some id ->
           transl_module ~scopes:(enter_module_definition ~scopes id)
             Tcoerce_none (Some (Pident id)) modl) bindings
      (make_sequence toploop_setvalue_id idents)
  | Tstr_class cl_list ->
...
let transl_toplevel_definition str =
  reset_labels ();
  Translprim.clear_used_primitives ();
  make_sequence (transl_toplevel_item_and_close ~scopes:empty_scopes)
    str.str_items
...
let print_cycle ppf cycle =
  ...
  Format.fprintf ppf "%a%a%s" (Format.pp_print_list ~pp_sep print_ident) cycle
    pp_sep () (Ident.name @@ fst @@ List.hd cycle)
...
let () =
  Location.register_error_of_exn
    (function Error (loc, err) -> Some (report_error loc err) | _ -> None)
