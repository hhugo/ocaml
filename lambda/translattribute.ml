...
let parse_id_payload txt loc ~default ~empty cases payload =
  let[@local] warn () =
    let (%>) f g x = g (f x) in
    let msg =
      cases |> List.map (fst %> Printf.sprintf "'%s'") |> String.concat ", " |>
  ...
  | Ok None -> empty
  | Ok (Some id) ->
    begin match List.assoc_opt id cases with
    | Some r -> r
    | None -> warn ()
    end
...
let parse_inline_attribute attr =
  ...
  | Some { Parsetree.attr_name = { txt; loc } as id; attr_payload = payload } ->
    if is_unrolled id then
      (* the 'unrolled' attributes must be used as [@unrolled n]. *)
      let warning txt =
        Warnings.Attribute_payload (txt, "It must be an integer literal")
      in
      begin match get_payload get_int_from_exp payload with
      | Ok n -> Unroll n
      | Error () ->
      ...
      parse_id_payload txt loc ~default:Default_inline ~empty:Always_inline
        [ "never", Never_inline; "always", Always_inline; "hint", Hint_inline ]
        payload
...
let parse_specialise_attribute attr =
  ...
  | Some { Parsetree.attr_name = { txt; loc }; attr_payload = payload } ->
    parse_id_payload txt loc ~default:Default_specialise
      ~empty:Always_specialise
      [ "never", Never_specialise; "always", Always_specialise ] payload

let parse_local_attribute attr =
  match attr with
  | None -> Default_local
  | Some { Parsetree.attr_name = { txt; loc }; attr_payload = payload } ->
    parse_id_payload txt loc ~default:Default_local ~empty:Always_local
      [ "never", Never_local; "always", Always_local; "maybe", Default_local ]
      payload
...
let check_local_inline loc attr =
  ...
  | Always_local, (Always_inline | Hint_inline | Unroll _) ->
    Location.prerr_warning loc (Warnings.Duplicated_attribute "local/inline")
  | _ -> ()
...
let add_specialise_attribute expr loc attributes =
  ...
  | Lfunction ({ attr = { stub = false } as attr } as funct), specialise ->
    ...
    | Always_specialise | Never_specialise ->
      Location.prerr_warning loc (Warnings.Duplicated_attribute "specialise")
    end;
    let attr = { attr with  specialise } in
    Lfunction { funct with  attr }
  | expr, (Always_specialise | Never_specialise) ->
    Location.prerr_warning loc (Warnings.Misplaced_attribute "specialise");
    expr
...
let add_local_attribute expr loc attributes =
  ...
  | Lfunction ({ attr = { stub = false } as attr } as funct), local ->
    ...
    | Always_local | Never_local ->
      Location.prerr_warning loc (Warnings.Duplicated_attribute "local")
    end;
    let attr = { attr with  local } in
    check_local_inline loc attr;
    Lfunction { funct with  attr }
  | expr, (Always_local | Never_local) ->
    Location.prerr_warning loc (Warnings.Misplaced_attribute "local");
    expr
...
let get_tailcall_attribute e =
  let is_tailcall_attribute =
    function
    | { Parsetree.attr_name = { txt = ("tailcall" | "ocaml.tailcall") }; _ } ->
      true
    | _ -> false
  in
  ...
  let tailcall_attribute =
    ...
    | { Parsetree.attr_name = { txt; loc }; attr_payload = payload } :: r ->
      ...
      begin match get_optional_payload get_bool_from_exp payload with
...
let add_function_attributes lam loc attr =
  ...
  let lam = add_local_attribute lam loc attr in
  lam
