...
let print_bigarray name unsafe kind ppf layout =
  fprintf ppf "Bigarray.%s[%s,%s]" (if unsafe then "unsafe_" ^ name else name)
...
let block_shape ppf shape =
  ...
  | Some [ elt ] -> Format.fprintf ppf " (%s)" (field_kind elt)
  | Some (h :: t) ->
    Format.fprintf ppf " (%s" (field_kind h);
    List.iter (fun elt -> Format.fprintf ppf ",%s" (field_kind elt)) t;
    Format.fprintf ppf ")"

let integer_comparison ppf =
  function
  | Ceq -> fprintf ppf "=="
  | Cne -> fprintf ppf "!="
  | Clt -> fprintf ppf "<"
...
let primitive ppf =
  ...
  | Pstring_load_16 unsafe ->
    if unsafe then
      fprintf ppf "string.unsafe_get16"
    else
      fprintf ppf "string.get16"
  | Pstring_load_32 unsafe ->
    if unsafe then
      fprintf ppf "string.unsafe_get32"
    else
      fprintf ppf "string.get32"
  | Pstring_load_64 unsafe ->
    if unsafe then
      fprintf ppf "string.unsafe_get64"
    else
      fprintf ppf "string.get64"
  | Pbytes_load_16 unsafe ->
    if unsafe then
      fprintf ppf "bytes.unsafe_get16"
    else
      fprintf ppf "bytes.get16"
  | Pbytes_load_32 unsafe ->
    if unsafe then
      fprintf ppf "bytes.unsafe_get32"
    else
      fprintf ppf "bytes.get32"
  | Pbytes_load_64 unsafe ->
    if unsafe then
      fprintf ppf "bytes.unsafe_get64"
    else
      fprintf ppf "bytes.get64"
  | Pbytes_set_16 unsafe ->
    if unsafe then
      fprintf ppf "bytes.unsafe_set16"
    else
      fprintf ppf "bytes.set16"
  | Pbytes_set_32 unsafe ->
    if unsafe then
      fprintf ppf "bytes.unsafe_set32"
    else
      fprintf ppf "bytes.set32"
  | Pbytes_set_64 unsafe ->
    if unsafe then
      fprintf ppf "bytes.unsafe_set64"
    else
      fprintf ppf "bytes.set64"
  | Pbigstring_load_16 unsafe ->
    if unsafe then
      fprintf ppf "bigarray.array1.unsafe_get16"
    else
      fprintf ppf "bigarray.array1.get16"
  | Pbigstring_load_32 unsafe ->
    if unsafe then
      fprintf ppf "bigarray.array1.unsafe_get32"
    else
      fprintf ppf "bigarray.array1.get32"
  | Pbigstring_load_64 unsafe ->
    if unsafe then
      fprintf ppf "bigarray.array1.unsafe_get64"
    else
      fprintf ppf "bigarray.array1.get64"
  | Pbigstring_set_16 unsafe ->
    if unsafe then
      fprintf ppf "bigarray.array1.unsafe_set16"
    else
      fprintf ppf "bigarray.array1.set16"
  | Pbigstring_set_32 unsafe ->
    if unsafe then
      fprintf ppf "bigarray.array1.unsafe_set32"
    else
      fprintf ppf "bigarray.array1.set32"
  | Pbigstring_set_64 unsafe ->
    if unsafe then
      fprintf ppf "bigarray.array1.unsafe_set64"
    else
      fprintf ppf "bigarray.array1.set64"
...
let apply_specialised_attribute ppf =
  function
  | Default_specialise -> ()
  | Always_specialise -> fprintf ppf " always_specialise"
  | Never_specialise -> fprintf ppf " never_specialise"
...
let rec lam ppf =
  ...
  | Lfunction { kind; params; return; body; attr } ->
    let pr_params ppf params =
      ...
      | Curried ->
        List.iter
          (fun (param, k) -> fprintf ppf "@ %a%a" Ident.print param value_kind k)
          params
      | Tupled ->
        fprintf ppf " (";
  ...
  | (Llet (_, k, id, arg, body) | Lmutlet (k, id, arg, body)) as l ->
    ...
    let rec letbody =
      function
      | (Llet (_, k, id, arg, body) | Lmutlet (k, id, arg, body)) as l ->
        fprintf ppf "@ @[<2>%a =%s%a@ %a@]" Ident.print id (let_kind l)
          value_kind k lam arg;
        letbody body
      | expr -> expr
    in
    fprintf ppf "@[<2>(let@ @[<hv 1>(@[<2>%a =%s%a@ %a@]" Ident.print id
      (let_kind l) value_kind k lam arg;
    ...
    let bindings ppf id_arg_list =
      ...
      List.iter
        (fun (id, l) ->
           ...
           fprintf ppf "@[<2>%a@ %a@]" Ident.print id lam l) id_arg_list
    in
  ...
  | Lswitch (larg, sw, _loc) ->
    let switch ppf sw =
      ...
      List.iter
        (fun (n, l) ->
           (if !spc then fprintf ppf "@ " else spc := true);
           fprintf ppf "@[<hv 1>case int %i:@ %a@]" n lam l) sw.sw_consts;
      List.iter
        (fun (n, l) ->
           (if !spc then fprintf ppf "@ " else spc := true);
           fprintf ppf "@[<hv 1>case tag %i:@ %a@]" n lam l) sw.sw_blocks;
  ...
  | Lstaticcatch (lbody, (i, vars), lhandler) ->
    fprintf ppf "@[<2>(catch@ %a@;<1 -1>with (%d%a)@ %a)@]" lam lbody i
      (fun ppf vars ->
         List.iter
           (fun (x, k) -> fprintf ppf " %a%a" Ident.print x value_kind k) vars)
      vars lam lhandler
  ...
  | Lsend (k, met, obj, largs, _) ->
    ...
    let kind = if k = Self then "self" else if k = Cached then "cache" else ""
    in
