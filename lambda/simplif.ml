...
let rec eliminate_ref id =
  ...
  | Lfunction _ as lam ->
    if Ident.Set.mem id (free_variables lam) then raise Real_reference else lam
  ...
  | Lletrec (idel, e2) ->
    Lletrec
      (List.map (fun (v, e) -> v, eliminate_ref id e) idel, eliminate_ref id e2)
  | Lprim (Pfield 0, [ Lvar v ], _) when Ident.same v id -> Lmutvar id
      ...
      (eliminate_ref id e,
       {
         ...
         sw_consts = List.map (fun (n, e) -> n, eliminate_ref id e) sw.sw_consts;
         sw_numblocks = sw.sw_numblocks;
         sw_blocks = List.map (fun (n, e) -> n, eliminate_ref id e) sw.sw_blocks;
  ...
  | Lifthenelse (e1, e2, e3) ->
    Lifthenelse (eliminate_ref id e1, eliminate_ref id e2, eliminate_ref id e3)
  | Lsequence (e1, e2) -> Lsequence (eliminate_ref id e1, eliminate_ref id e2)
  | Lwhile (e1, e2) -> Lwhile (eliminate_ref id e1, eliminate_ref id e2)
  | Lfor (v, e1, e2, dir, e3) ->
    Lfor (v, eliminate_ref id e1, eliminate_ref id e2, dir, eliminate_ref id e3)
...
let simplify_exits lam =
  ...
  let rec count ~try_depth =
    ...
    | Llet (_, _kind, _v, l1, l2) | Lmutlet (_kind, _v, l1, l2) ->
      count ~try_depth l2;
      count ~try_depth l1
    ...
    | Lstaticcatch (l1, (i, _), l2) ->
      ...
      if (get_exit i).count > 0 then count ~try_depth l2
    | Ltrywith (l1, _v, l2) ->
  ...
  and count_default ~try_depth sw =
    ...
    | Some al ->
      ...
      and nblocks = List.length sw.sw_blocks
      in
  ...
  *)
  ...
  let rec simplif ~try_depth =
    ...
    | Lapply ap ->
      Lapply
        { ap with
          ...
          ap_func = simplif ~try_depth ap.ap_func;
    ...
    | Lletrec (bindings, body) ->
      Lletrec
        (List.map (fun (v, l) -> v, simplif ~try_depth l) bindings,
         simplif ~try_depth body)
    | Lprim (p, ll, loc) ->
      let ll = List.map (simplif ~try_depth) ll in
      begin match p, ll with (* Simplify Obj.with_tag *)
    ...
    | Lswitch (l, sw, loc) ->
      ...
      and new_fail = Option.map (simplif ~try_depth) sw.sw_failaction
      in
      Lswitch
        (new_l,
         { sw with
    ...
    | Lstringswitch (l, sw, d, loc) ->
      Lstringswitch
        (simplif ~try_depth l,
         List.map (fun (s, l) -> s, simplif ~try_depth l) sw,
         Option.map (simplif ~try_depth) d,
         loc)
    | Lstaticraise (i, []) as l ->
      (try
    ...
    | Lstaticraise (i, ls) ->
      ...
      (try
         ...
         let env =
           List.fold_right2 (fun (x, _) (y, _) env -> Ident.Map.add x y env) xs
             ys Ident.Map.empty
         in
        ...
        *)
         List.fold_left2 (fun r (y, kind) l -> Llet (Strict, kind, y, l, r))
    ...
    | Lstaticcatch (l1, (i, xs), l2) ->
      ...
      if count = 0 then
        ...
        simplif ~try_depth l1
      else if count = 1 && max_depth <= try_depth then
    ...
    | Lifthenelse (l1, l2, l3) ->
      Lifthenelse
        (simplif ~try_depth l1, simplif ~try_depth l2, simplif ~try_depth l3)
    | Lsequence (l1, l2) ->
      Lsequence (simplif ~try_depth l1, simplif ~try_depth l2)
    | Lwhile (l1, l2) -> Lwhile (simplif ~try_depth l1, simplif ~try_depth l2)
    | Lfor (v, l1, l2, dir, l3) ->
      Lfor
        (v,
         simplif ~try_depth l1,
         simplif ~try_depth l2,
         dir,
...
let exact_application { kind; params; _ } args =
  match kind with
  | Curried ->
    if List.length params <> List.length args then None else Some args
  | Tupled ->
    begin match args with
    | [ Lprim (Pmakeblock _, tupled_args, _) ] ->
      if List.length params <> List.length tupled_args then
        None
      else
        Some tupled_args
    | [ Lconst (Const_block (_, const_args)) ] ->
      if List.length params <> List.length const_args then
        None
      else
        Some (List.map (fun cst -> Lconst cst) const_args)
    | _ -> None
    end

let beta_reduce params body args =
  List.fold_left2
    (fun l (param, kind) arg -> Llet (Strict, kind, param, arg, l)) body params
    args
...
let simplify_lets lam =
  ...
  let rec count bv =
    ...
    | Lfunction { body } -> count Ident.Map.empty body
  ...
  let rec simplif =
    ...
    | Lfunction { kind; params; return = return1; body = l; attr; loc } ->
      ...
      | Lfunction
          { kind = Curried; params = params'; return = return2; body; attr; loc
          }
        when
          kind = Curried &&
            optimize &&
      ...
      | body -> Lfunction { kind; params; return = return1; body; attr; loc }
      end
    | Llet (_str, _k, v, Lvar w, l2) when optimize ->
...
let rec emit_tail_infos is_tail lambda =
  ...
  | Lfunction { body = lam } -> emit_tail_infos true lam
...
let split_default_wrapper ~id:fun_id ~kind ~params ~return ~body ~attr ~loc =
  let rec aux map =
    function
    | Llet (Strict, k, id, (Lifthenelse (Lvar optparam, _, _) as def), rest)
      when
        Ident.name optparam = "*opt*" &&
          List.mem_assoc optparam params && not (List.mem_assoc optparam map)
      ->
    ...
    | body ->
      ...
      let subst =
        List.fold_left2 (fun s id new_id -> Ident.Map.add id new_id s)
  ...
  with
  | Exit -> [ fun_id, Lfunction { kind; params; return; body; attr; loc } ]
...
*)
type slot = { func : lfunction; mutable scope : lambda option }
...
let simplify_local_functions lam =
  ...
  let enabled =
    ...
    | { local = Default_local; inline = (Never_inline | Default_inline); _ }
      ->
      true
    | { local = Default_local; inline = (Always_inline | Unroll _ | Hint_inline);
        _ }
    | { local = Never_local; _ }
      ->
      false
    ...
    | Llet (_str, _kind, id, Lfunction lf, cont) when enabled lf.attr ->
      ...
      | Some { scope = Some scope; _ } ->
        ...
        let sc =
          ...
          if scope == !current_scope then cont else scope
        in
  ...
  let rec rewrite lam0 =
    let lam =
      match lam0 with
      | Llet (_, _, id, _, cont) when Hashtbl.mem static_id id -> rewrite cont
      | Lapply { ap_func = Lvar id; ap_args; _ }
        when Hashtbl.mem static_id id ->
      ...
      | lam -> Lambda.shallow_map rewrite lam
    in
    List.fold_right
      (fun (st, lf) lam -> Lstaticcatch (lam, (st, lf.params), rewrite lf.body))
      (LamTbl.find_all static lam0) lam
  in
...
let simplify_lambda lam =
  let lam =
    lam |>
      (if !Clflags.native_code || not !Clflags.debug then
