...
let lapply ap =
  ...
  | Lapply ap' ->
    Lapply
      { ap with  ap_func = ap'.ap_func; ap_args = ap'.ap_args @ ap.ap_args }
  | _ -> Lapply ap
...
let transl_meth_list lst =
  if lst = [] then
    ...
    share (Const_block (0, List.map (fun lab -> Const_immstring lab) lst))

let set_inst_var ~scopes obj id expr =
  Lprim
    (Psetfield_computed (Typeopt.maybe_pointer expr, Assignment),
     [ Lvar obj; Lvar id; transl_exp ~scopes expr ],
     Loc_unknown)
...
let transl_vals tbl create strict vals rem =
  List.fold_right
    (fun (name, id) rem ->
       Llet (strict, Pgenval, id, transl_val tbl create name, rem)) vals rem
    ...
    (fun (nm, id) rem ->
       try
         (nm,
          id,
          mkappl
            (oo_prim "get_method", [ Lvar tbl; Lvar (Meths.find nm meths) ])) ::
           rem
       with
       | Not_found -> rem) inh_meths []

let bind_super tbl (vals, meths) cl_init =
  transl_vals tbl false StrictOpt vals
    (List.fold_right
       (fun (_nm, id, def) rem -> Llet (StrictOpt, Pgenval, id, def, rem)) meths
       cl_init)
       ...
       Lsequence
         (obj_init,
          (if not has_init then
             ...
             mkappl
               (oo_prim "run_initializers_opt", [ obj; Lvar obj'; Lvar cl ]))))
...
let rec build_object_init ~scopes cl_table obj params inh_init obj_init cl =
  ...
  | Tcl_ident (path, _, _) ->
    ...
    let env =
      ...
      | None -> []
      | Some envs ->
        [ Lprim (Pfield (List.length inh_init + 1), [ Lvar envs ], Loc_unknown)
        ]
    in
         ...
         let (inh_init, obj_init, has_init) =
           List.fold_right
             (fun field (inh_init, obj_init, has_init) ->
                ...
                | Tcf_initializer _ -> inh_init, obj_init, true) str.cstr_fields
...
=
  ...
  | _ ->
    ...
    in
    let obj_init =
      if ids = [] then obj_init else lfunction [ self, Pgenval ] obj_init
    in
...
let bind_method tbl lab id cl_init =
  Llet
    (Strict,
     Pgenval,
     id,
     mkappl (oo_prim "get_method_label", [ Lvar tbl; transl_label lab ]),
     cl_init)

let bind_methods tbl meths vals cl_init =
  let methl = Meths.fold (fun lab id tl -> (lab, id) :: tl) meths [] in
  let len = List.length methl
  and nvals = List.length vals
  in
  if len < 2 && nvals = 0 then
    Meths.fold (bind_method tbl) meths cl_init
  else if len = 0 && nvals < 2 then
    transl_vals tbl true Strict vals cl_init
  else
    ...
    Llet
      (Strict,
       ...
       List.fold_right
         (fun (_lab, id) lam ->
            decr i;
            Llet (StrictOpt, Pgenval, id, lfield ids !i, lam)) (methl @ vals)
         cl_init)
  ...
  | _ ->
    lsequence
      (mkappl
         (oo_prim "set_methods",
          [ Lvar tbl;
            Lprim (Pmakeblock (0, Immutable, None), methods, Loc_unknown) ]))
      lam
...
let rec build_class_init ~scopes cla cstr super inh_init cl_init msubst top cl =
    ...
    | (_, path_lam, obj_init) :: inh_init ->
      ...
      Llet
        (Strict,
         ...
         obj_init,
         mkappl
           (Lprim (Pfield 1, [ path_lam ], Loc_unknown),
            Lvar cla ::
              (if top then
                 [ Lprim (Pfield 3, [ path_lam ], Loc_unknown) ]
               else
                 [])),
      ...
      List.fold_right
        (fun field (inh_init, cl_init, methods, values) ->
           ...
           | Tcf_inherit (_, cl, _, vals, meths) ->
             ...
             let (inh_init, cl_init) =
               build_class_init ~scopes cla false
                 (vals, meths_super cla str.cstr_meths meths) inh_init cl_init
                 msubst top cl
             in
           ...
           | Tcf_method (name, _, Tcfk_concrete (_, exp)) ->
             ...
             let met_code = msubst true (transl_scoped_exp ~scopes exp) in
             let met_code =
             ...
             Lsequence
               (mkappl
                  (oo_prim "add_initializer",
                   Lvar cla :: msubst false (transl_exp ~scopes exp)),
                cl_init),
             methods,
             values
           | Tcf_attribute _ -> inh_init, cl_init, methods, values)
        str.cstr_fields (inh_init, cl_init, [], [])
    in
    let cl_init = output_methods cla methods cl_init in
  ...
  | Tcl_constraint (cl, _, vals, meths, concr_meths) ->
    ...
    let narrow_args =
      [ Lvar cla; transl_meth_list vals; transl_meth_list virt_meths;
          ...
          (fun init (nm, id, _) ->
             Llet
               (StrictOpt,
                ...
                init)) cl_init methids
      in
      let cl_init =
        List.fold_left
          (fun init (nm, id) ->
             Llet (StrictOpt, Pgenval, id, lfield inh (index nm vals + 1), init))
          cl_init valids
      in
      inh_init,
      Llet
    ...
    | _ ->
      ...
      if cstr then
        ...
        in
        inh_init, Lsequence (mkappl (oo_prim "narrow", narrow_args), cl_init)
    end
  | Tcl_open (_, cl) ->
    build_class_init ~scopes cla cstr super inh_init cl_init msubst top cl
...
let rec build_class_lets ~scopes cl =
  ...
  | Tcl_let (rec_flag, defs, _vals, cl') ->
    ...
    env, (fun x -> Translcore.transl_let ~scopes rec_flag defs (wrap x))
  | _ -> cl.cl_env, (fun x -> x)
...
let rec transl_class_rebind ~scopes obj_init cl vf =
  ...
  | Tcl_fun (_, pat, _, cl, partial) ->
    let (path, path_lam, obj_init) = transl_class_rebind ~scopes obj_init cl vf
    in
    let build params rem =
      let param = name_pattern "param" pat in
      Lfunction
...
let transl_class_rebind ~scopes cl vf =
  try
    ...
    let (_, path_lam, obj_init') =
      transl_class_rebind_0 ~scopes self obj_init0 cl vf
    in
    ...
    if id then
      ...
      Llet
        (Strict,
         Pgenval,
         new_init,
         lfunction [ obj_init, Pgenval ] obj_init',
         Llet
           (Alias,
            Pgenval,
            cla,
...
let rec builtin_meths self env env2 body =
  ...
  let conv =
    ...
    | Lsend (Self, met, Lvar s, [], _) when List.mem s self -> "meth", [ met ]
    | _ -> raise Not_found
  in
  match body with
  ...
  | Lsend (Self, met, Lvar s, [], _) when List.mem s self -> "get_meth", [ met ]
...
let free_methods l =
  ...
  let rec free l =
    ...
    match l with
    | Lsend (Self, Lvar meth, _, _, _) -> fv := Ident.Set.add meth !fv
    | Lsend _ -> ()
    | Lfunction { params } ->
      List.iter (fun (param, _) -> fv := Ident.Set.remove param !fv) params
    | Llet (_, _k, id, _arg, _body) | Lmutlet (_k, id, _arg, _body) ->
    ...
    | Ltrywith (_e1, exn, _e2) -> fv := Ident.Set.remove exn !fv
    | Lfor (v, _e1, _e2, _dir, _e3) -> fv := Ident.Set.remove v !fv
    | Lassign _
    | Lvar _
    | Lmutvar _
    | Lconst _
...
let transl_class ~scopes ids cl_id pub_meths cl vflag =
  ...
  if rebind <> lambda_unit then
    rebind
  else (* Prepare for heavy environment handling *)
    ...
    let subst env lam i0 new_ids' =
      ...
      List.fold_left
        (fun subst id ->
           incr i;
           Ident.Map.add id (lfield env !i) subst) Ident.Map.empty !new_ids'
    in
      ...
      | Lfunction { kind = Curried; params = (self, Pgenval) :: args; body } ->
        ...
        let body' =
          if new_ids = [] then
            ...
            Lambda.subst no_env_update (subst env body 0 new_ids_meths) body
        in
        (try
         (* Doesn't seem to improve size for bytecode *)(* if not !Clflags.native_code then raise Not_found; *)
         ...
         | Not_found ->
           [
             lfunction ((self, Pgenval) :: args)
               (if not (Ident.Set.mem env (free_variables body')) then
                  body'
                else
                  Llet
                    (Alias,
                     ...
                     env,
                     Lprim
                       (Pfield_computed, [ Lvar self; Lvar env2 ], Loc_unknown),
                     body')) ])
      | _ -> assert false
    in
    ...
    let copy_env self =
      if top then
        ...
        Lifused
          (env2,
           Lprim
             (Psetfield_computed (Pointer, Assignment),
              [ Lvar self; Lvar env2; Lvar env1' ],
              Loc_unknown))
    and subst_env envs l lam =
      if top then
        lam
      else (* must be called only once! *)
        let lam = Lambda.subst no_env_update (subst env1 lam 1 new_ids_init) lam
        in
        Llet
          (Alias,
           Pgenval,
     ...
     and ldirect obj_init =
       Llet
         (Strict,
          ...
          cl_init,
          Lsequence
            (mkappl (oo_prim "init_class", [ Lvar cla ]),
             mkappl (Lvar obj_init, [ lambda_unit ])))
     in
     (* Simplest case: an object defined at toplevel (ids=[]) *)
     if top && ids = [] then
       llets (ltable cla (ldirect obj_init))
     else
       ...
       and lclass lam =
         ...
         in
         Llet
           (Strict, Pgenval, class_init, cl_init, lam (free_variables cl_init))
       and lbody fv =
           ...
           ltable table
             (Llet
                (Strict,
                 Pgenval,
                 env_init,
                 mkappl (Lvar class_init, [ Lvar table ]),
                 Lsequence
       ...
       if top && concrete then
         lclass lbody
       else if top then
         llets (lbody_virt lambda_unit)
       else (* Now for the hard stuff: prepare for table caching *)
         let envs = Ident.create_local "envs"
         and cached = Ident.create_local "cached"
         in
         let lenvs =
           if !new_ids_meths = [] && !new_ids_init = [] && inh_init = [] then
             lambda_unit
           else
             Lvar envs
         in
         ...
         and linh_envs =
           List.map
             (fun (_, path_lam, _) ->
                Lprim (Pfield 3, [ path_lam ], Loc_unknown)) (List.rev inh_init)
         in
         let make_envs lam =
           Llet
             (StrictOpt,
         ...
         in
         let inh_paths =
           List.filter (fun (path, _, _) -> List.mem (Path.head path) new_ids)
             inh_init
         in
         let inh_keys =
           List.map
             (fun (_, path_lam, _) ->
                Lprim (Pfield 1, [ path_lam ], Loc_unknown)) inh_paths
         in
         let lclass lam =
           Llet
             (Strict,
         ...
         and lcache lam =
           if inh_keys = [] then
             Llet (Alias, Pgenval, cached, Lvar tables, lam)
           else
             Llet
               (Strict,
                Pgenval,
                ...
                mkappl
                  (oo_prim "lookup_tables",
                   [ Lvar tables;
                     Lprim
                       (Pmakeblock (0, Immutable, None), inh_keys, Loc_unknown)
                   ]),
                lam)
         and lset cached i lam =
           Lprim
             (Psetfield (i, Pointer, Assignment),
              [ Lvar cached; lam ],
              Loc_unknown)
         in
         let ldirect () =
                       ...
                       Lprim
                         (Pmakeblock (0, Immutable, None),
                          (if concrete then
                             ...
                             [ lambda_unit; lfield cached 0; lambda_unit; lenvs
                             ]),
                          Loc_unknown))))))
...
let () =
  transl_object :=
    fun ~scopes id meths cl -> transl_class ~scopes [] id meths cl Concrete
...
let report_error ppf =
  function
  | Tags (lab1, lab2) ->
    fprintf ppf "Method labels `%s' and `%s' are incompatible.@ %s" lab1 lab2
      "Change one of them."
