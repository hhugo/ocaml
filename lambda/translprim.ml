...
let add_used_primitive loc env path =
  ...
  | Some (Path.Pdot _ as path) ->
    ...
    if Ident.global unit && not (Hashtbl.mem used_primitives path) then
      Hashtbl.add used_primitives path loc
  | _ -> ()
...
let primitives_table =
  create_hashtable 57
    [ "%identity", Identity; "%bytes_to_string", Primitive (Pbytes_to_string, 1);
      "%bytes_of_string", Primitive (Pbytes_of_string, 1);
      "%ignore", Primitive (Pignore, 1); "%revapply", Revapply; "%apply", Apply;
      "%loc_LOC", Loc Loc_LOC; "%loc_FILE", Loc Loc_FILE;
      "%loc_LINE", Loc Loc_LINE; "%loc_POS", Loc Loc_POS;
      "%loc_MODULE", Loc Loc_MODULE; "%loc_FUNCTION", Loc Loc_FUNCTION;
      "%field0", Primitive (Pfield 0, 1); "%field1", Primitive (Pfield 1, 1);
      ...
      "%raise", Raise Raise_regular; "%reraise", Raise Raise_reraise;
      "%raise_notrace", Raise Raise_notrace;
      "%raise_with_backtrace", Raise_with_backtrace;
      "%sequand", Primitive (Psequand, 2); "%sequor", Primitive (Psequor, 2);
      ...
      "%frame_pointers", Frame_pointers; "%negint", Primitive (Pnegint, 1);
      "%succint", Primitive (Poffsetint 1, 1);
      "%predint", Primitive (Poffsetint (-1), 1);
      "%addint", Primitive (Paddint, 2); "%subint", Primitive (Psubint, 2);
      "%mulint", Primitive (Pmulint, 2); "%divint", Primitive (Pdivint Safe, 2);
      "%modint", Primitive (Pmodint Safe, 2); "%andint", Primitive (Pandint, 2);
      "%orint", Primitive (Porint, 2); "%xorint", Primitive (Pxorint, 2);
      "%lslint", Primitive (Plslint, 2); "%lsrint", Primitive (Plsrint, 2);
      "%asrint", Primitive (Pasrint, 2); "%eq", Primitive (Pintcomp Ceq, 2);
      ...
      "%obj_is_int", Primitive (Pisint, 1); "%lazy_force", Lazy_force;
      ...
      "%opaque", Primitive (Popaque, 1); "%sys_argv", External prim_sys_argv;
      "%send", Send; "%sendself", Send_self; "%sendcache", Send_cache;
...
let simplify_constant_constructor =
  function
  | Equal -> true
  | Not_equal -> true
  | Less_equal -> false
...
let specialize_primitive env ty ~has_constant_constructor prim =
  let param_tys =
    ...
    | None -> []
    | Some (p1, rhs) ->
      begin match is_function_type env rhs with
      | None -> [ p1 ]
      | Some (p2, _) -> [ p1; p2 ]
      end
  in
  ...
  | Primitive (Parraylength t, arity), [ p ] ->
    ...
    if t = array_type then
      None
    else
      Some (Primitive (Parraylength array_type, arity))
  | Primitive (Parrayrefu t, arity), p1 :: _ ->
    let array_type = glb_array_type t (array_type_kind env p1) in
    if t = array_type then
      None
    else
      Some (Primitive (Parrayrefu array_type, arity))
  | Primitive (Parraysetu t, arity), p1 :: _ ->
    let array_type = glb_array_type t (array_type_kind env p1) in
    if t = array_type then
      None
    else
      Some (Primitive (Parraysetu array_type, arity))
  | Primitive (Parrayrefs t, arity), p1 :: _ ->
    let array_type = glb_array_type t (array_type_kind env p1) in
    if t = array_type then
      None
    else
      Some (Primitive (Parrayrefs array_type, arity))
  | Primitive (Parraysets t, arity), p1 :: _ ->
    let array_type = glb_array_type t (array_type_kind env p1) in
    if t = array_type then
      None
    else
      Some (Primitive (Parraysets array_type, arity))
  | Primitive
  ...
  | Primitive (Pmakeblock (tag, mut, None), arity), fields ->
    ...
    if useful then
      Some (Primitive (Pmakeblock (tag, mut, Some shape), arity))
    else
      None
  | Comparison (comp, Compare_generic), p1 :: _ ->
    if has_constant_constructor && simplify_constant_constructor comp then
      Some (Comparison (comp, Compare_ints))
    else if
      is_base_type env p1 Predef.path_int ||
        is_base_type env p1 Predef.path_char ||
          maybe_pointer_type env p1 = Immediate
    then
      Some (Comparison (comp, Compare_ints))
    else if is_base_type env p1 Predef.path_float then
      Some (Comparison (comp, Compare_floats))
    else if is_base_type env p1 Predef.path_string then
      Some (Comparison (comp, Compare_strings))
    else if is_base_type env p1 Predef.path_bytes then
      Some (Comparison (comp, Compare_bytes))
    else if is_base_type env p1 Predef.path_nativeint then
      Some (Comparison (comp, Compare_nativeints))
    else if is_base_type env p1 Predef.path_int32 then
      Some (Comparison (comp, Compare_int32s))
    else if is_base_type env p1 Predef.path_int64 then
...
let lambda_of_loc kind sloc =
  ...
  let file =
    if Filename.is_relative file then
      ...
      Location.rewrite_absolute_path file
  in
  let enum =
  ...
  | Loc_POS ->
    Lconst
      (Const_block
         (0,
          [ Const_immstring file; Const_base (Const_int lnum);
            Const_base (Const_int cnum); Const_base (Const_int enum) ]))
...
let lambda_of_prim prim_name prim loc args arg_exps =
  ...
  | Lazy_force, [ arg ] -> Matching.inline_lazy_force arg loc
  | Loc kind, [] -> lambda_of_loc kind loc
  | Loc kind, [ arg ] ->
    let lam = lambda_of_loc kind loc in
    Lprim (Pmakeblock (0, Immutable, None), [ lam; arg ], loc)
  | Send, [ obj; meth ] -> Lsend (Public, meth, obj, [], loc)
  | Send_self, [ obj; meth ] -> Lsend (Self, meth, obj, [], loc)
  ...
  | Identity, [ arg ] -> arg
  | Apply, [ func; arg ] | Revapply, [ arg; func ] ->
    Lapply
      {
        ap_func = func;
        ap_args = [ arg ];
...
let transl_primitive loc p env ty path =
  ...
  let rec make_params n =
    if n <= 0 then
      ...
      (Ident.create_local "prim", Pgenval) :: make_params (n - 1)
  in
...
let lambda_primitive_needs_event_after =
  function
  ...
  | Pbytesrefs
  | Pbytessets
  | Pmakearray (Pgenarray, _)
  | Pduparray _
  | Parrayrefu (Pgenarray | Pfloatarray)
  | Parraysetu (Pgenarray | Pfloatarray)
  | Parrayrefs _
  | Parraysets _
  | Pbintofint _
  | Pcvtbint _
  ...
  | Pbytesrefu
  | Pbytessetu
  | Pmakearray ((Pintarray | Paddrarray | Pfloatarray), _)
  | Parraylength _
  | Parrayrefu _
  | Parraysetu _
  | Pisint
...
let primitive_needs_event_after =
  function
  | Primitive (prim, _) -> lambda_primitive_needs_event_after prim
  | External _ -> true
  | Comparison (comp, knd) ->
    lambda_primitive_needs_event_after (comparison_primitive comp knd)
  | Lazy_force | Send | Send_self | Send_cache | Apply | Revapply -> true
...
let transl_primitive_application loc p env ty path exp args arg_exps =
  let prim = lookup_primitive_and_mark_used (to_location loc) p env (Some path)
  in
  ...
  let lam =
    if primitive_needs_event_after prim then
      match exp with
      | None -> lam
